...
fail). Any two big_arbiter runs compared for a DECISION must be on identical part sets. Run this
...
for t, s in sets.items():
    ok = s == ref
    allsame &= ok
    print(f"  {t:22s} {len(s):4d} parca  {'AYNI' if ok else 'FARKLI -> KARSILASTIRMA GECERSIZ'}")
common = set.intersection(*[set(s) for s in sets.values()])
print(f"\n  ortak parca: {len(common)}")
