...
connection points on parts the robot never wires):
  ...
  2. GUARD      -- anything already on the guard list (a previously-rejected
                   catalog) is deleted on sight.
...
OUT_OF_SCOPE = {
    ...
    # model to hunt for connection points on a part that has none.
...
def has_body(path, min_points=20):
    ...
    it just stops reading once the body is proven."""
    try:
...
def main():
    ...
    # -- identical verdicts, a fraction of the wall clock on a 1000-file batch.
