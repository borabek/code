# -*- coding: utf-8 -*-
"""SONDA: SERBEST-DERINLIK YON YELPAZESI pool tavanini ne up to acar?

FIZIK: real giris yonu, agizdan DISARI correct most uzun empty yolu which is yondur
(tel oradan gelir). Bu, markadan bagimsiz and mesh-only a YON ONERI kaynagidir.

OLCUM SORUSU (D6, seyreltilmis pool):
  yonlu recall  BANKA           0.7264   (own+komsu+silindir+ana)
  yonlu recall  BANKA + YELPAZE   ?      (candidate basina +K direction onerisi)
...
def main ():
    import trimesh 
    kay =d6_record .yukle ()
    # MARKA SUZGECI: kaldirac BOSLUGUN OLDUGU places sinanmali. Ilk 100 D6
    # parcasinda banka already 0.9293 veriyor and olculecek pay absent; NIT'te banka
    # 0.5254 -- new a direction kaynagi however orada anlam carries.
    ...
    for i ,f in enumerate (fs ,1 ):
        ...
        # yelpaze: each candidate x 64 direction for DISARI serbest path
        nc =len (P )
