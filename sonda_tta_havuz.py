# -*- coding: utf-8 -*-
"""TTA SONDASI: cikarim aninda DONDURME ortalamasi havuzu iyilestirir mi?

NEDEN BU KOL FARKLI: bugun denenen dokuz kolun hepsi SECICI tarafindaydi ve
hepsi ayni takas egrisi uzerinde gezindi (kesinlik artarken geri cagirma
dusuyor). Bu kol GIRDI KALITESINI degistirir: `p_pos` iyilesirse hem ADAYLAR
hem de 58 gate ozniteliginin cogu birden iyilesir -- egriyi YUKARI kaydirabilir.

DAYANAK: `results/_p1_olasilik_d7` **tek checkpoint** ile uretilmis
(`pbs` sekli (1, N, 5)); urun tarihsel olarak topluluk kullaniyordu. Ayrica
agin girdi ozniteligi `xyz` (ckpt cfg), yani DONDURMEYE DUYARLI -- ayni mesh
farkli yonelimlerde farkli tahmin verir. TTA bu varyansi ortalar.

FARKI: [[b9-rotation-augmentation]] EGITIMDE dondurme artirmasiydi ve kahini
dusurmustu. Bu, EGITIME DOKUNMAZ; yalnizca cikarimda K yonelimin ortalamasi
alinir. Tez degismezleri (5 sinif, ~6000 remesh, `v_o`) AYNEN korunur.

OLCUT: HAVUZ RECALL (F1 degil) -- yeni bir seg ciktisi once bununla sinanir
([[kismi-ce-denetimi-recall-dusuruyor]] dersi). Ornek: D7'den 100 parca.
"""
import json
import os
import sys
...
def _dondur(V, aci_derece, eksen):
    a = np.radians(aci_derece)
    k = np.asarray(eksen, float)
...
def main():
    ...
    print(f"ckpt {CKPT} | cihaz {dev} | parca {len(secili)} | "
    ...
    for i, pid in enumerate(secili, 1):
        ...
        # TTA: ayni mesh, K yonelim; olasiliklar ORTALANIR (tepe sirasi korunur,
        # yalnizca girdi koordinatlari doner -> cikti tepe basina hizali kalir)
        pbs = []
        for a in ACILAR:
        ...
        for ad, pb in (("tek", pb_tek), ("tta", pb_tta)):
            cps, _o, _c, _p = robot_cp.adaylari_uret(V, F, [pb], S.get(pid))
            P = np.asarray([c["point"] for c in cps], float) if cps \
                else np.zeros((0, 3))
            Dd = np.asarray([c["direction"] for c in cps], float) if cps \
                else np.zeros((0, 3))
            tp, _f, fn = esle_macar(P, Dd, G, Gd, dg, tol, 180.0, True)[:3]
            top[ad][0] += tp
            top[ad][1] += fn
        if i % 20 == 0:
            print(f"  {i}/{len(secili)} ({(time.time()-t0)/i:.1f}s/parca)",
                  flush=True)
    out = {}
    for ad, v in top.items():
        out[ad] = v[0] / max(v[0] + v[1], 1)
        print(f"{ad:<5} havuz recall {out[ad]:.4f}  (TP {v[0]} FN {v[1]})")
    d = out["tta"] - out["tek"]
    ...
    print("makbuz -> results/tta_havuz.json")
