...
DAYANAK (`results/b2_yon_isaret.json`, D7 marka-disi): YON_YOK kovasindaki 705
GT'nin **%59.1'inde (417)** adaya 2mm mesafedeki bir mesh tepesinin normali
dogru yonu ZATEN tasiyor. Ayrica %44.3'u duz 180 derece ISARET hatasi.

Kova aritmetigi (`results/kazanan_hata_bankasi.json`): TP 710 / FN 2377 / FP 1323.
417 kurtarilirsa F1 0.2773 -> **0.407**. Kol aday EKLEMEZ -> FP artamaz,
tespit metrigi (aci serbest) DEGISMEZ; bu, olcumun kendi kontrolu olur.

ONCEKI YON KOLU NEDEN BASARISIZDI: `yon_odunc.py` secenekleri komsu ADAY
yonleri + silindir eksenleri + baskin yondu; **mesh normali YOKTU**. B2 olcumu
eksik kaynagin tam da o oldugunu gosterdi.

TEZE SADIK: `v_o` KONUMU ve segmentasyon degismez; tezin kendi yonu HER ZAMAN
0. secenek ve esitlikte KAZANIR.
"""
import collections
import json
import os
...
NORMAL_R = 2.0        # adaya bu mesafedeki tepelerin normalleri secenek olur
AYIRT = 5.0           # bu aciya yakin secenekler AYNI sayilir
...
def secenekler(p, d, V, NV, ppos, gate_s, n_aday):
    """Bu aday icin (yon, oznitelik) listesi. 0. oge HER ZAMAN MEVCUT yon."""
    d = _birim([d])[0]
    V = np.asarray(V, float)
    uz = np.linalg.norm(V - p, axis=1)
    k = uz <= NORMAL_R
    adaylar = [(d, 0.0, float(gate_s), 1.0, 0.0)]     # mevcut
    if k.any():
        for i in np.where(k)[0]:
            n = NV[i]
            for s in (1.0, -1.0):
                adaylar.append((s * n, float(uz[i]), float(ppos[i]), 0.0,
                                float(s)))
    secili, oz = [], []
    for v, mes, pp, mevcut, isaret in adaylar:
        v = _birim([v])[0]
        if any(float(np.degrees(np.arccos(np.clip(abs(float(v @ w)), -1, 1))))
               < AYIRT for w, _, _, _, _ in secili):
            continue
        secili.append((v, mes, pp, mevcut, isaret))
    for v, mes, pp, mevcut, isaret in secili:
        aci_mevcut = float(np.degrees(np.arccos(np.clip(float(v @ d), -1, 1))))
        oz.append([mevcut, aci_mevcut, mes, pp, isaret, float(gate_s),
...
def parca_yukle(on, pid, r):
    ...
    return {"X": X, "P": np.asarray(z["P"], float)[m],
            ...
            "NV": brep_havuz.tepe_normalleri(V, F),
            "ppos": pb[:, CE] + pb[:, CT]}


def gate_sec(model, d):
...
def main():
    ...
    for i, pid in enumerate(pid_tam):
        ...
        if (i + 1) % 400 == 0:
            print(f"  egitim {i+1}/{len(pid_tam)} secenek {len(X)}", flush=True)
    ...
    for ad, kullan in (("YON SABIT", False), ("MESH NORMALI", True)):
        ...
        for pid in [f[3:-4] for f in sorted(os.listdir(OZ))
            ...
            dg = float(r["diag"])
            tp, fp, fn = esle_macar(P, D, G, Gd, dg, YANAL, ACI, False,
                                    isaretli=True)[:3]
            a_ = rob[r["mfg"]]
    ...
    json.dump({"damga": makbuz_hash.damga(), "sonuc": out, "fark": b - a,
               "not": "Yon secenekleri = mevcut + 2mm icindeki mesh tepelerinin "
                      "+/- normalleri. Konum ve aday sayisi DEGISMEZ. "
                      "D7 marka-disi, TAM ZINCIR, MIKRO."},
              open("results/b1_mesh_normal.json", "w"), indent=1)
    print("makbuz -> results/b1_mesh_normal.json")
