...
Checks, in order of how badly they would burn us:
 ...
 1. LEAK: the same catalog number downloaded twice (208 of them) must not end up
    on both sides of the split -- that inflates val F1 and would fake a pass.
 2. LABEL QUALITY ON NEW VENDORS: the CAD labeller was tuned on Phoenix Contact.
    If Weidmueller / the other vendors are drawn differently, their labels are
    junk and the model both learns AND is scored on junk. Measured as the
...
def main():
    ...
    print(f"\n[1] KOPYA SIZINTISI")
    print(f"    corpus'ta ayni katalogdan >1 parca: {len(dup_cats)}")
    if dup_cats:
    ...
    for part in jd.iter_parts(args.corpus):
        ...
        # to every model that will ever be trained on this corpus
    ...
    print(f"    {'uretici':<16}{'parca':>7}{'sifir-CP':>10}{'CP/parca':>10}{'TAVAN F1':>10}")
    for v, d in sorted(by_vendor.items(), key=lambda kv: -kv[1]["n"]):
        tp, fp, fn = d["tp"], d["fp"], d["fn"]
        f1 = 2 * tp / (2 * tp + fp + fn) if (tp + fp + fn) else 0.0
        pos = d["n"] - d["zero"]
        cpp = d["cps"] / pos if pos else 0
        flag = "  <-- SUPHELI" if (f1 < 0.97 and pos > 20) or \
                                 (d["zero"] / max(d["n"], 1) > 0.15) else ""
