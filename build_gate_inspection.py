# -*- coding: utf-8 -*-
"""P1.9-11: wire-gate HATA inceleme paketi. Gate'in iki hata tipini parca baglaminda renkli goster ki
insan real_wire / tool / ambiguous diye yargilayabilsin:
  YESIL  = dropped_real_wire  (gate ATTI ama uretici-CP'ye eslesiyor -> gate yanlis attiysa gercek tel)
  TURUNCU= kept_FP            (gate TUTTU ama uretici-CP'ye eslesmez -> tool VEYA listelenmemis tel)
  GRI    = dogru (kept-TP + dropped-tool)
Her CP'de wire_score yazili. NOT: gate zaten dogrulandi (vote-override %95 tool cikti); bu paket
kalibrasyon-ince-ayar icin, dusuk oncelik.
"""
...
def main():
    ...
    fig.suptitle("WIRE-GATE INCELEME: YESIL=gate-atti-ama-mfg-eslesen (gercek tel mi?) TURUNCU=gate-tuttu-ama-eslesmeyen (tool mu, listelenmemis tel mi?)",
                 fontsize=12)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig("results/_gate_inspection.png", dpi=95, bbox_inches="tight")
    print(f"-> results/_gate_inspection.png ({n} parca, gate hatalari renkli)")
