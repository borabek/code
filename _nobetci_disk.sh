...
while true; do
  ...
  if [ "$BOS" -lt "$ESIK_MB" ]; then
    echo "$(date +%H:%M:%S) DISK ESIGI ($BOS MB) -- training durduruluyor" >> results/_nobetci.log
  ...
  if ! powershell.exe -NoProfile -Command "if ((Get-CimInstance Win32_Process | Where-Object { \$_.CommandLine -match 'train_seg_extra' }).Count -gt 0) { exit 0 } else { exit 1 }" ; then
    echo "$(date +%H:%M:%S) training bitti, nobetci cikiyor (bos $BOS MB)" >> results/_nobetci.log
    exit 0
  fi
  sleep 30
done
