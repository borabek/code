...
pos = (a[:, 1] > 0).sum()
print(f"  eksen yonu: {pos}/{len(rows)} tahmin DISARIDA (+), {len(rows)-pos} iceride (-)")
print("\n  -> eksen ortalamasi |buyuk| ve tek yonlu ise SISTEMATIK (duzeltilebilir);")
print("     teget baskin ve yon dagilmissa RASTGELE (duzeltilemez).")
