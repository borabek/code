...
DUZELTILEN: `METAL_RGB` tek bir gumus tonuna sabitti; olculdu (40 parca) ki parcalarin
%22'sinde renk VAR ama o tona uyan yuz YOK. "Gri = metal" demek de yanlis olurdu (gri PLASTIK
govde klemenslerde cok yaygin). Renk-bagimsiz olcut: parcanin BASKIN renginden farkli yuzler.
Ayrica kati-seviyesi renk kurtarma eklendi (dogrulanan parca 23->34/40).

OLCULEN: kapsama 14 -> 19 parca (+%36). AMA KAPSAMA AYIRT EDICILIK DEGILDIR -- bu betik
...
def main():
    ...
    print(f"{len(Y)} aday | TP {int(Y.sum())} | kapsama: metal-tonu {kap_m}, kontrast {kap_k}")
    rng2 = np.random.RandomState(0)
    print(f"\n{'olcut':<28}{'AUC':>8}{'null p95':>10}{'TP ort':>10}{'FP ort':>10}{'|AUC-.5|':>10}")
    res = {}
    ...
    print("makbuz -> results/q12_kontrast.json")
