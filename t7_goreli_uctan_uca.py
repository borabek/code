...
ADAY DUZEYINDE OLCULDU (t6): goreli 0.5 + taban 0.20
  tanidik (geometri-disi) 0.7422 -> 0.7348 (-0.0074)
  uretici-disi EN KOTU    0.2799 -> 0.4402 (+0.160)

AMA aday-duzeyi F1 uctan uca F1 DEGILDIR (E maddesi gate duzeyinde kazanip uctan uca kaybetmisti).
Bu betik ayni adaylar ve ayni skorlar uzerinde YALNIZ KARAR KURALINI degistirir -- tek degisken,
yeniden cikarim yok, yeniden turetme yok.

KILL (onceden yazili): tanidik (DEV+VAL) tespit F1 kaybi 0.02'yi asarsa ACILMAZ. Kazanc zaten
tanidik veride degil, GORULMEMIS URETICIDE bekleniyor; burada olculen sey BEDEL.
"""
...
def main():
    ...
    # --- TURETME + SKOR bir kez; karar kurallari sonra bedava ---
    DER = []
    for r in cache:
    ...
    ADAY = [("sabit (mevcut)", "sabit"), ("goreli 0.5 + taban 0.20", (0.5, 0.20)),
            ("goreli 0.5 + taban 0.25", (0.5, 0.25)), ("goreli 0.4 + taban 0.20", (0.4, 0.20))]
    ...
    pickle.dump(R, open(f"results/t7_parca_{kume}.pkl", "wb"))
    json.dump({k: {"tespit": float(f1w(v[0])), "robot": float(f1w(v[1]))} for k, v in R.items()},
              open(f"results/t7_goreli_{kume}.json", "w"), indent=1)
    print(f"\nKILL: tanidik tespit kaybi > 0.02 ise ACILMAZ. makbuz -> results/t7_goreli_{kume}.json")
