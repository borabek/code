# -*- coding: utf-8 -*-
"""R8: ISARETLI KAHIN -- iki yon sozlugunun GERCEK tavani (ve bir hukmun iptali).

BULGU (2026-08-04): yon tarafindaki TUM kahin olcumleri `abs(v . Gd)` ile secim yapiyordu.
Yani kahin, 180 derece TERS bir yonu "mukemmel" sayip seciyordu; sonra sonuc ISARETLI
metrikle olculunce dusuk cikiyordu. Bu, bugun yon SECICISININ ETIKETINDE duzelttigim
hatanin AYNISI -- ama bu sefer KAHINDE, yani TAVAN olcumunde.

IKI SONUCU GECERSIZ KILAR:
  1. `c1_yon_sozlugu` KILL'i ("kahin 0.6263 < 0.70 -> Rota C KAPANIR", gecti=false).
     O sozlukte FIZIKSEL girisler vardi: brep (B-rep analitik silindir ekseni), kanal
     (ic noktalarin PCA ekseni), normal (yerel yuzey normali), duzlem_A/B (agiz
     duzleminin ana/yan ekseni), uye_k. Isaretli olcumu 0.5748 = TABANDAN DUSUK cikmisti
     -- ama secimi yapan kahin ISARETSIZDI. Sozlukte dogru cevap YOK demek DEGIL.
  2. R1'in "%87.5 yon kahini / %94.1 birlesik" sayilari da isaretsiz kahinle uretildi;
     dagitilan secici ISARETLI metrikte olculuyor. Ikisi karsilastirilamaz, yani
     "secici kahininin %39'unu yakaladi" ifadesi DAYANAKSIZ.

BU BETIK: iki sozlugu de AYNI parcalarda kurar ve UC tavani ISARETLI kahinle olcer:
    D1   dagitilan sozluk (mevcut, obb+-, uzlasi, yuz_uzlasi, dik+-, yuzn+-)
    C1   fiziksel sozluk  (brep, kanal, normal, duzlem_A/B ve isaret esleri, uye_k)
    BIRLESIK
Her biri icin hem ISARETLI hem ISARETSIZ kahin raporlanir ki artefakt gorunur olsun.

CIKTI: results/_r8_sozluk.pkl (secici egitimi icin) + results/r8_isaretli_kahin.json
DAGITIM YOK.
"""
...
def main():
    ...
    if os.path.exists(ONB):
        ...
        print(f"onbellekten: {len(PARCA)} parca", flush=True)
    else:
        PARCA = {}; t0 = time.time()
        for kk, r in enumerate(DER, 1):
            ...
            if d_ is None:
                continue
            P = d_["P"].copy()
            Pd = yon_uygula(d_, YS)        # DAGITILAN zincirin ciktisi = taban
            V = None
            try:
    ...
    def kahin(hangi, isaretli_secim):
        """hangi: 'D1'|'C1'|'BIRLESIK'. isaretli_secim: True ise v.Gd, False ise |v.Gd|."""
        rob, gg = [], []
        for r in DER:
            d_ = PARCA.get(r["pid"])
    ...
    for r in DER:
        ...
        rob0.append((rj,) + esle(P, Pn, G, Gd, r["diag"], 2.0, 10.0, False, isaretli=True))
    b = f1w(rob0)
    ...
    for h in ("D1", "C1", "BIRLESIK"):
        ri, _ = kahin(h, True)
        ru, _ = kahin(h, False)
        S[h] = {"isaretli": f1w(ri), "isaretsiz": f1w(ru)}
        print(f"{h:<12}{f1w(ri):>16.4f}{f1w(ru):>17.4f}{f1w(ru)-f1w(ri):>+11.4f}")
    print("\n('artefakt' = isaretsiz kahinin SISIRDIGI miktar; secim ters yonu de dogru sayiyor)")
    d1i = S["D1"]["isaretli"]; bri = S["BIRLESIK"]["isaretli"]
    print(f"\nDAGITILAN secici {b:.4f}; D1 sozlugunun ISARETLI tavani {d1i:.4f} "
          f"-> yakalanan pay %{100*(b-0.5818)/max(d1i-0.5818,1e-9):.0f} "
          f"(0.5818 = secici oncesi)")
    print(f"C1'in KATKISI (birlesik - D1): {bri-d1i:+.4f}")
    hukum = ("C1 SOZLUGU YENIDEN ACILIR -- fiziksel girisler ISARETLI tavani yukseltiyor"
    ...
    with io.open("results/r8_isaretli_kahin.json", "w", encoding="utf-8") as f:
        json.dump({"taban": b, "sozlukler": S, "c1_katkisi": bri - d1i,
                   "kapsam": kaps, "hukum": hukum}, f, indent=1)
    print("makbuz -> results/r8_isaretli_kahin.json")
