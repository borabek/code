# -*- coding: utf-8 -*-
"""PARCA DUZEYINDE YONLENDIRICI: B-rep'i HER parcada not, GEREKEN parcada kullan.

GOZLEM: B-rep havuzu recall'u markalara according to very different aciyor (CWT +0.2984,
KLM +0.2651, EFX +0.2331; ELMEX/C3 +0.0000). Seg havuzunun already iyi oldugu
parcalarda B-rep only CELDIRICI ekliyor -- hibrit gate'in tabanin under
kalmasinin sebebi this may be.

Bu probe before TAVANI olcer: each parcada IKI havuzdan iyisini secen MUKEMMEL
router ne gives? Tavan tabanin belirgin ustunde degilse part-duzeyi
router INSA EDILMEZ.

Ayrica yonlendirilebilirlik for UCUZ sinyaller de olculur (segmentasyonun own
guveni, candidate count, silindir count) -- ceiling varsa hangi sinyalle yakalanir.
"""
...
print (f"D7 {len (te )} part",flush =True )
...
def say (P ,D ,d ):
    return match_hungarian (P ,D ,d ["G"],d ["Gd"],d ["diag"],K .YANAL ,K .ACI ,False ,
    signed =True )[:3 ]


arm ={"TEZ-SAF":[],"HIBRIT":[],"KAHIN (part basina iyisi)":[]}
sinyal =[]
for d in te :
...
print (f"\nB-rep'in DAHA IYI oldugu part: {n_b }/{len (sinyal )} "
f"(esit {n_e }, tez-saf iyi {len (sinyal )-n_b -n_e })")
print ("\nB-rep'in kazandigi parts hangi markalarda:")
