...
> eşik ayarı→sabit ürün eşiği) ve each düzeltme bazı "kazançları" none etti.
> Aşağıdaki "ölen kollar" tablosu neyin why düştüğünü taşıyor.

## MANŞET — İKİ SAYI (2026-08-01)

Eski manşet geometri-ayrıktı but **üretici-karışıktı**. Görülmemiş a üreticide ne olduğu hiç
...
**Geri alma:** `wire_gate.pkl.pre_topo` / `.pre_fiz` + `cp_config` bayrakları — each biri tek satır.

Makbuz: `results/t15_topo_uctan_uca.json`, `results/t8_manufacturer_out_end_to_end.json`,
...
keskin geometri anahtarı, eşli bootstrap. **Yeni axis:** each arm artık **üretici-dışı** da ölçülüyor.

---

## CEILING — nereye up to gidilebilir *(ölçüldü, prediction değil)*

`m_konum_tavani.py`: konum **ve** axis ORACLE with kusursuz yapıldığında robot-hazır F1
**tespit F1'e eşitleniyor**. Yani:

> **Robot-hazır'ın tavanı = tespit F1.** Konum rötuşu ve axis iyileştirmesi tek başına hiçbir
> şey açmaz; kazanç yalnızca **more çok açıklık bulmaktan** gelir.
...
| bulgu | durum |
|---|---|
| DEV/VAL adları yanlış bağlıydı (gerçek bölme `split3.json`) | ✅ `measure_set.py` tek source |
...
| eğitim ≠ runtime candidate üretimi (`votes` 12 vs 4) | ✅ `robot_cp.derive_candidates` tek source + 3 test; corpus yeniden üretildi |
| GA karara katılmıyordu | ✅ `decision_criterion` (5) EVIDENCE şartı |
...
## AÇIK LİSTE — öncelik sırası, each madde ölçülmüş gerekçeyle
...
**Adaylar already GT'nin %93.7'sini içeriyor** → tespit 0.90 a **gate** problemi, türetme değil.
...
Doğrulama: 5/5 seed pozitif (ort +0.0141, sd 0.0013) but bootstrap **[−0.0010, +0.0330]** —
...
**107/107 dik nokta (eşleşmelerin %12.3'ü), KESİN analitik B-rep axis verildiğinde bile
...
(ve this *mükemmel* lateral varsayımıyla).

| ölçülen arm | sonuç |
|---|---|
| parça-içi axis uzlaşısı | kâhin tavanı +1.5 puan; gerçekte **net −110 nokta**. ÖLÜ |
| B-rep kapsaması → %100 | **+3.2 puan** ceiling. Küçük |
| BREP_MAX_OFF (kodda asılı "P2 taraması") | **yapıldı: fark none** (0.4500/0.4495/0.4492). TODO kapatıldı |
...
| S2: yarık dedektörü hedefte ne verir? | 107 dik noktanın 74'ünde yarık present, 26'sının yönü ≤10° (medyan 26° vs 90°) |
| S3: rule **herkese** uygulanınca | dedektör eşleşmelerin **%80.4'ünde** ateşliyor → düzelen 41 / bozulan 228 = **net −187** |
| **S4: seçiciyi ÖĞREN** (8 özellik, fold-dışı) | **mekanizma düzeldi: bozulan 228 → 0**, but net yalnız **+11** (geçiş +%1.26 → robot ~0.4585). Bar %2 → dağıtılmadı |

**Bu dalın kâhin tavanı: +41 nokta = robot 0.4816.** Mükemmel a seçici bile 0.48'e up to taşır.

Yani missing which a algoritma değil **bilgi**: telin hangi yarıktan girdiğini söyleyen işaret none.
Tez §5.3.6'nın kendi normali (`v_o − v_s`, bbox'a hizalı) da this sınıfta %0.
...
### 0b. ~~TOPOLOJİ YARIÇAPI~~ — **CLOSED 2026-08-01, R=6.0 kalır**
...
lafzını geçiyordu but eşleştirilmiş bootstrap bitirdi: kazancı **gürültü**
(WEI +0.018, GA [−0.007, +0.044]), bedeli **gerçek** (PXC −0.024, GA [−0.044, −0.006]).

**Dünkü karar doğruymuş:** candidate düzeyine bakıp korpusu yeniden üretseydim a **gerileme**
dağıtmış olacaktım. Bu, candidate düzeyinin this araştırmadaki **dördüncü** yanıltması.

**Yan kazanım:** yarıçap artık ayarlanabilir (`wire_gate.TOPO_R` / `cp_config.gate_topo_r`) ve
topoloji sütunlarını ağ çıkarımı olmadan yeniden üretme yolu present (`u1_topo_r_yeniden.py`,
...
`brep_r` with mevcut `size` **korelasyonu −0.009** → kopya değil, **gerçekten yeni bilgi**.
...
sıfır olurdu — E maddesinde tam as this yaşandı). Tam corpus verisi 18 sütunla üretiliyor.
...
eğitilmiş; 5-üyeli havuzda yeniden eğitilmeden this karar conclusive değil.
...
| katkı | tespit | robot |
|---|---|---|
| depth (19−18) | +0.0069 gürültü | +0.0003 none |
| renk (22−19) | −0.0015 gürültü | +0.0103 gürültü |
| **total (22−18)** | **+0.0054 gürültü** | +0.0107 belirgin |
...
this gece L2'yi öldüren hatanın aynısı olurdu. Ayrıca bedeli present: parça başına a OCC STEP
...
- `ic_derinlik` doğru çalışıyor (nötr %16.3, medyan 3.32 mm) — `bos_derinlik`'in yanlışlıkla
  **dışarı** baktığı this sayede anlaşıldı.
- Sessiz-nötrleşme sayacı: `kanalda_metal` adayların **%99'unda** hiç ateşlemiyormuş —
  sayaç olmasa ölü sütun dağıtılacaktı.
- Renk kapsaması iki koldan iyileştirildi (**this veriden SONRA**, i.e. ölçüme yansımadı):
  katı-seviyesi renk kurtarma (doğrulanan parça 23→34/40) ve **renkten bağımsız kontrast**
  ölçütü (kapsama 14→19). `METAL_RGB` tek a gümüş tonuna sabitti; parçaların %22'sinde renk
  present but o ton none. "Gri = metal" demek yanlış olurdu (gri plastik gövde yaygın), o yüzden
...
### 5. Veri
Korpus ×1.92 büyümesi WORK CP-F1'e +0.0107 vermişti; eğri düzleşiyor. WSCAD indirme arm ölü
...
yüzey noktalarının **centroid'inden** hesaplanıyordu; oysa ölçüldü ki a parçadaki silindirik
...
## BİLİNEN UYUMSUZLUK — gate eğitimi vs ürün *(ölçüldü, gürültü, but kayıtlı)*

Gate **eğitim verisi** adayları `f1_sweep.union_all` with birleştiriyor: yakındaki **each** üye
...
dolayısıyla this a eğitim/çalışma uyumsuzluğudur. Bu geceki parite düzeltmesi `_vote2`'yi benzersiz
...
**Kural:** gate verisi a more üretilirken **ürünün kendi birleştiricisi** kullanılmalı.

---

### 6. İÇBÜKEY KENAR TOPOLOJİSİ → gate *(sıradaki candidate, henüz ölçülmedi)*

Delik-tanıma alanının birinci sinyali: **a açıklık, içbükey kenarlarla çevrili yüz kümesidir**
...
**Neden teze dokunmuyor (doğrulandı):** gate tezde **none**. Tez Contact sınıfını tanımı gereği
*"Kontaktierung bzw. Werkzeugeinschub"* diye birleştiriyor, i.e. tel/alet ayrımını hiç sormuyor;
...
**bağımsız a dedektör** kuruyorduk ve içbükeylik kullanmıyorduk; burada çalışan gate'e sütun ekliyoruz.

**Ölçüm sırası (kısayol none):** (1) candidate düzeyinde AUC + null testi ~1 saat — ayırt edicilik
otherwise orada biter · (2) geçerse tam corpus verisi ~1.5 saat · (3) DEV → VAL uçtan uca.
...
8 candidate türetmesi + B-rep/EK özellikleri), total ~85 dk, ve this sürede **CPU %33** — i.e.
makinenin çoğu boşta. Parçalar birbirinden bağımsız; `multiprocessing` with 3–4 fold hızlanır.

Bu a doğruluk sorunu değil, but **each özellik denemesi 85 dk bekleme** demek ve this gece
üretim iki kez yeniden başlatıldı. Bir sonraki tam üretimden önce yapılmalı.
**Dikkat:** zehirli-parça koruması (INFLIGHT dosyası) süreç başına ayrılmalı, otherwise paralel
işçiler birbirinin kaydını ezer.

---

## KURALLAR *(this gece üçü de çiğnendiği için yazılı)*

1. **Kill ölçütü ÜRÜN metriğine yazılır** (F1), AUC'ye değil — ve koşmadan önce.
2. **İki gate however AYNI candidate havuzunda** karşılaştırılır.
...
6. Bir yardımcı fonksiyonun ne yaptığını varsayma, **docstring'ini oku** — eski F maddesi
   (`family_key` = parça numarasının kendisi) tam as this yüzden doğmuştu.
