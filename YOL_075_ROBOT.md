...
**Ürün:** 22 sütunlu gate (13 taban + 5 B-rep fiziksel + 4 içbükey topoloji) + **göreli eşik**
(parça-içi 0.5 × en yüksek, mutlak taban 0.25). 109/109 test yeşil.
...
*tespit* = yanal ≤ max(3 mm, %6×köşegen), eksen serbest · *robot-hazır* = yanal ≤2 mm **ve** eksen ≤10°

**Ölçüm altyapısı:** `results/split3.json` (DEV karar / VAL sınav / **LOCKED harcanmadı**),
keskin geometri anahtarı, eşli bootstrap. **Yeni eksen:** her kol artık **üretici-dışı** da ölçülüyor.

---

## TAVAN — nereye kadar gidilebilir *(ölçüldü, tahmin değil)*

`m_konum_tavani.py`: konum **ve** eksen ORACLE ile kusursuz yapıldığında robot-hazır F1
**tespit F1'e eşitleniyor**. Yani:

> **Robot-hazır'ın tavanı = tespit F1.** Konum rötuşu ve eksen iyileştirmesi tek başına hiçbir
...
| bulgu | durum |
|---|---|
| DEV/VAL adları yanlış bağlıydı (gerçek bölme `split3.json`) | ✅ `olcum_kumesi.py` tek kaynak |
...
| eğitim ≠ runtime aday üretimi (`votes` 12 vs 4) | ✅ `robot_cp.adaylari_uret` tek kaynak + 3 test; korpus yeniden üretildi |
...
**`0.9372` recall DEĞİL, kâhin F1.** Gerçek aday recall'ı ağırlıklı **0.8795** / ham 0.8073;
çok-CP **0.7691** → çok-CP F1 tavanı **0.8695**, rejim-ağırlıklı tavan 0.9354.
...
### 0a. TESPİT 0.90 / ROBOT 0.75 — tavan merdiveni *(2026-08-01)*
...
| **dur/devam kararı** (aday başına, 3277 satır) | +0.0142 |
| **+ F1 ağırlıklı** | **+0.0150** |

Doğrulama: 5/5 tohum pozitif (ort +0.0141, sd 0.0013) ama bootstrap **[−0.0010, +0.0330]** —
sıfırı kıl payı içeriyor. Bar +0.02 → **dağıtılmadı**, istiflenebilir aday.
...
(ve bu *mükemmel* yanal varsayımıyla).

| ölçülen kol | sonuç |
|---|---|
| parça-içi eksen uzlaşısı | kâhin tavanı +1.5 puan; gerçekte **net −110 nokta**. ÖLÜ |
| B-rep kapsaması → %100 | **+3.2 puan** tavan. Küçük |
| BREP_MAX_OFF (kodda asılı "P2 taraması") | **yapıldı: fark yok** (0.4500/0.4495/0.4492). TODO kapatıldı |
| noktayı analitik eksene izdüşürme | aday +2.1 → **uçtan uca −0.045**. ÖLÜ (`CP_AXIS_PROJECT`, kapalı) |
...
| R (mm) | aday düzeyi F1 | **tanıdık** | PXC-dışı | WEI-dışı |
|---|---|---|---|---|
| **6.0 (dağıtılan)** | 0.7404 | **0.7410** | 0.7203 | 0.4832 |
| 8.0 | 0.7451 | 0.7422 | 0.6961 | 0.5013 |
| 12.0 | **0.7504** ← aday kazananı | **0.7301** | 0.7121 | 0.4871 |
...
**Dünkü karar doğruymuş:** aday düzeyine bakıp korpusu yeniden üretseydim bir **gerileme**
dağıtmış olacaktım. Bu, aday düzeyinin bu araştırmadaki **dördüncü** yanıltması.
...
yarıçap <1 mm %21, eksen-dik %35) ve gate'in 13 özelliğinin **hiçbiri bunları taşımıyor**.

Ölçüldü (`q4_brep_fiziksel.py`, 3277 aday, 200 ayrık parça, bağ düzeltmeli Mann-Whitney + null):
...
**ARTIMLI DEĞER ÖLÇÜLDÜ** (`q5_artimli_deger.py`, aynı adaylar, grup-çapraz OOF):

| gate | OOF AUC | aday F1 | kesinlik | recall |
|---|---|---|---|---|
...
sıfır olurdu — E maddesinde tam olarak bu yaşandı). Tam korpus verisi 18 sütunla üretiliyor.
**Sıradaki adım:** `q6_fiz_uctan_uca.py` — iki gate **aynı aday havuzunda**, uçtan uca.
**Kill:** DEV'de uçtan uca tespit F1 artmazsa dağıtılmaz; artarsa VAL'de sınanır.
**Uyarı:** aday-düzeyi F1 uçtan uca F1 değildir — E maddesi gate düzeyinde +0.0068 kazanıp
...
VAL **+0.0208**, DEV **−0.0038**; ikisinde de **recall ↑ / kesinlik ↓**.
...
İçeri kanal derinliği + 3 renk özelliği (22 sütun). Tam korpus verisi üretildi, uçtan uca
...
| katkı | tespit | robot |
|---|---|---|
| derinlik (19−18) | +0.0069 gürültü | +0.0003 yok |
...
okuması (0.24 s) + aday başına içeri ışın.
...
| **oy ölçeği** (votes ×4/5) | VAL +0.0164 (ölçeksizden kötü) | 5. üyenin kesinlik kaybı ölçek kayması **değil** |
...
**`brep_axes` silindir yarıçapı 3.5 kat küçüktü.** Yarıçap ve "eksen üzeri nokta" örneklenen
...
**Doğrulama: STEP metniyle 327/327, medyan hata 0.0000 mm.**
...
### 6. İÇBÜKEY KENAR TOPOLOJİSİ → gate *(sıradaki aday, henüz ölçülmedi)*
...
**Ölçüm sırası (kısayol yok):** (1) aday düzeyinde AUC + null testi ~1 saat — ayırt edicilik
yoksa orada biter · (2) geçerse tam korpus verisi ~1.5 saat · (3) DEV → VAL uçtan uca.
...
8 aday türetmesi + B-rep/EK özellikleri), toplam ~85 dk, ve bu sürede **CPU %33** — yani
makinenin çoğu boşta. Parçalar birbirinden bağımsız; `multiprocessing` ile 3–4 kat hızlanır.
...
2. **İki gate ancak AYNI aday havuzunda** karşılaştırılır.
3. Bir kümede seçilen kol, **başka kümede sınanmadan** manşete girmez.
