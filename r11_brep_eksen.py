...
`_snap_axis` docstring'i (olculmus): uretici CP'lerinin %19.1'i 10 dereceden EGIK (43'e kadar);
eslesen ciftlerin %18.2'sinde eksenimiz 15-90 derece sapiyor, cogu ~90. Yuvarlamayi kapatmak
DENENDI ve KAYBETTI, cunku teshis su: "olculen HAM eksenler de zaten eksene hizali --
channel_axis egimi GOREMIYOR". Yani suclu yuvarlama degil, EKSEN OLCUMU.

`channel_axis` yalniz X/Y/Z dener ve isin-sondasi deligin koni acisindan (~11 derece) keskin
olamaz. B-REP SILINDIR EKSENI ise CAD'de ANALITIK durur -- cozunurluk siniri YOK.
`brep_axes.axis_at` mevcut ve cp_openings:370'te kullaniliyor AMA sonra `_snap_axis`'ten
geciyor. Soru: yuvarlamadan ONCE B-rep ekseni GT'yi ne kadar buluyor?
...
def kos(DER, ad, sinir=90):
    ...
    for r in DER:
        ...
        if pid not in cyl_cache:
            try: cyl_cache[pid]=BX.cylinders(yol[pid])
        ...
        for dd,a_,b_ in sorted((pe2[a,b],a,b) for a in range(len(P)) for b in range(len(G))):
            ...
            if ax is None:
                stat['brep_YOK']+=1; acilar['brep'].append(a_mev)   # yedek: mevcut
            else:
