...
BULGU. Analitik silindir ekseni GT yonunu ZATEN tutuyor (isaretsiz tavan
SUPU 0.612 / MOR 0.700 / UPUN 0.793). Geriye kalan tek belirsizlik ISARET.

KURESEL SOZLESME CALISMIYOR: "parca merkezinden disari" olcusu UPUN'da
0.130, "iceri" 0.743 veriyor; SUPU'da tersi. Ve GT olcumu gosterdi ki UPUN
(0.597) ve DIN (0.540) parca ICINDE BILE karisik -- iki yuzunde de giris
olan klemensler. Yani isaret bir MARKA ozelligi degil, YEREL bir ozellik.

BU SONDA isareti agiz noktasindaki MESH YUZEY NORMALINDEN turetir:
    disari = mesh'in o noktadaki dis normali
    yon    = eksenin, dis normalle POZITIF ic carpim veren hali

Kiyas: kuresel "disari" ve kuresel "iceri" sozlesmeleriyle ayni tabloda.
...
def agiz_ve_eksen(sil):
    """(agiz konumu, EKSEN (isaretsiz), silindir merkezi)."""
    P, A, C = [], [], []
    for c in sil:
...
def main():
    ...
    for pid, r in kay.items():
        ...
        if not len(AP):
            continue
        # YEREL DIS NORMAL: agiz noktasina en yakin yuzeyin normali
        try:
        ...
        isaretsiz = np.degrees(np.arccos(
            np.clip(np.abs(AX @ Gn.T), -1, 1)))
        a = ist[r["mfg"]]
        a["gt"].append(len(G))
        a["konum"].append(int(konum.any(0).sum()))
        a["eksen"].append(int((konum & (isaretsiz <= K.ACI)).any(0).sum()))
        ...
        # YEREL: mesh dis normaliyle ayni isaretli
        d_yer = np.where((NN * AX).sum(1, keepdims=True) > 0, AX, -AX)
        a["yerel"].append(puan(d_yer))
        a["yerel_ters"].append(puan(-d_yer))
        n += 1
    print(f"{n} parca islendi, {atlanan} atlandi\n")
    print(f"{'marka':<7}{'GT':>7}{'KONUM':>8}{'eksen':>8}{'kur.disari':>11}"
          f"{'kur.iceri':>11}{'YEREL':>8}{'yerel ters':>11}")
    out = {}
    for m_ in sorted(ist, key=lambda x: -sum(ist[x]["gt"])):
        a = ist[m_]
        g = max(sum(a["gt"]), 1)
        r = {k: sum(a[k]) / g for k in ("konum", "eksen", "kuresel_disari",
                                        "kuresel_iceri", "yerel",
                                        "yerel_ters")}
        r["gt"] = g
        out[m_] = r
        print(f"{m_:<7}{g:>7}{r['konum']:>8.3f}{r['eksen']:>8.3f}"
    ...
    json.dump({"marka": out,
               "not": "Isaret kaynaklari: kuresel (merkezden agza) vs YEREL "
                      "(mesh dis normali). 'eksen' = isaretsiz tavan. "
                      "D7'ye BAKILMADI."},
              open("results/yerel_isaret.json", "w"), indent=1)
    print("\nmakbuz -> results/yerel_isaret.json")
    print("OKUMA: YEREL, kuresel secenklerin ikisini de asiyorsa isaret")
    print("       GEOMETRIDEN cozulmus demektir (marka sozlesmesi gerekmez).")
