# -*- coding: utf-8 -*-
"""P1: GATE KORPUSU OGRENME EGRISI -- "kac parca daha gerekir?" sorusunun SAYISAL cevabi.

NEDEN SIMDI: gece boyunca dokuz mekanizma ayni cepheye carpti ve geriye TEK canli kol
kaldi: UCUNCU URETICI VERISI. Ama "daha cok veri" bir plan degildir; plan, KAC parca
gerektigini bilmektir. Bunu ancak ogrenme egrisi soyler.

W2 kanitladi ki veri gercek bir kaldiractir: +110 yeni WEI parcasi gorulmemis parcada
+0.0391 verdi (GA sifiri disliyor). Ama egri duz mu, logaritmik mi, doymus mu -- bilmiyoruz.

YONTEM: egitim korpusunu GEOMETRI GRUBU bazinda alt-orneklle (parca degil grup: ikizler
birlikte gider, yoksa "yeni veri" yanilsamasi olur). Her boyutta 3 tohum. Olcum kumesi
SABIT kalir. Uc bolmede birden raporlanir.

Sonra log-egri uydurulur ve EKSTRAPOLASYON verilir: hedefe ulasmak icin gereken korpus
buyuklugu. Ekstrapolasyon DURUSTCE "egri doymuyorsa gecerlidir" serhiyle raporlanir.

Hicbir sey dagitilmaz; bu bir OLCUMDUR.
"""
import io
import json
import os
...
def main():
    ...
    def kol_yap(pay, tohum_alt):
        def kol(X_, y_, pid_, mfg_, kp, th):
            # GRUP bazinda alt-ornekleme: ikizler AYNI tarafta kalir
    ...
    for pay in PAYLAR:
        ...
        EGRI[pay] = {"grup": n_g, "havuzlanmis": hav, "sapma": sd,
                     "uretici_ort": ud, "robot": rob}
        print(f"pay {pay:<6.3f} grup {n_g:>4}  havuz {hav:.4f} (+-{sd:.4f})  "
              f"uretici-ort {ud:.4f}  robot {rob:.4f}", flush=True)
        bekci(f"p1 pay={pay}")
    ...
    print(f"          uretici-disi: F1 = {ku[0]:.4f} + {ku[1]:.4f} * ln(grup)")
    # son iki noktanin egimi: DOYUM var mi
    ...
    print("makbuz -> results/p1_ogrenme_egrisi.json")
