...
S1 kapatici bir sey gosterdi: son yon 45 derecenin uzerinde saptiginda zincirin HER halkasi da
sapmis (tez normali dahil, hepsi %0). Yani duzeltme, mevcut asamalari yeniden siralamakla
olmaz -- YENI BIR OZELLIK tespit etmek gerekiyor: telin girdigi YARIGIN kendisi.

geo_g2_yarik `detect_slots` tam bunu ariyor: KARSILIKLI IKI DUZLEM arasindaki 1-6mm acikliklar
(yay-kelepceli / push-in terminal girisleri). Silindir arayan dedektor onlari GOREMEZ -- G1
otopsisi kacan GT'lerin %93.3'unun hicbir silindire yakin olmadigini olcmustu.

SORU (tek ve net): dik sinifa giren her CP icin, YAKININDA yonu GT ile uyusan bir yarik adayi
VAR MI? Varsa duzeltme somut (o CP'nin yonunu yariktan al). Yoksa bu sinif mevcut araclarla
cozulemez ve bunu DURUSTCE raporlarim.

DIKKAT: yarik adayinin yonu de tasarim geregi koordinat eksenine hizali (`out[la] = sgn`).
Yani egik agizlari da cozmez; ama DIK sinifta sorun egiklik degil, YANLIS EKSEN secimi --
onu cozebilir.
"""
import collections
import json
...
def main():
    ...
    # 1) DIK sinifi topla (parca basina)
    HEDEF = collections.defaultdict(list)
    toplam = 0
    for r in DER:
        if r["X"] is None:
            continue
        s = wire_gate.karar_skoru(m, r["X"]); k = wire_gate.karar_maskesi(s)
        if not k.any():
            continue
        P = r["P"][k]; Pd = r["Pd"][k]
    ...
    # 2) O parcalarda yarik adaylari uret ve yonleri karsilastir
    with open("results/_u4_der.pkl", "rb") as f:
        pass
    VF = {}
    for kume in ("dev", "val"):
    ...
    for i_, (pid, hedefler) in enumerate(HEDEF.items(), 1):
        if i_ % 10 == 0:
            print(f"  {i_}/{len(HEDEF)} parca", flush=True)
        if pid not in VF:
            continue
        V, F = VF[pid]
        try:
            ...
            print(f"    {pid}: yarik dedektoru hata {type(e).__name__}")
            continue
        if not slots:
    ...
    if bulundu:
        ...
        print(f"  yarik yonu medyan aci          : {np.median(ay):.1f} deg "
              f"(mevcut yon: {np.median(ae):.1f} deg)")
    ...
    with open("results/s2_yarik_yonu.json", "w", encoding="utf-8") as f:
        json.dump({"dik_sinif": n_dik, "yarik_var": bulundu, "uyan": uyan,
                   ...
                   "oran": (uyan / bulundu) if bulundu else None}, f, indent=1)
