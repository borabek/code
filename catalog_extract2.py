...
The first version of this took every 7-digit number on any line and shipped 612
...
at all, which would have taught the model to find connection points on a cover.
            ...
            for line in (page.extract_text() or "").splitlines():
                for typ, art in ROW.findall(line):
                    ...
                    if fam not in TERMINAL:
                        stats[f"aksesuar/bilinmeyen:{fam}"] += 1
                        continue
                    seen.add(art)
                    if art in have:
