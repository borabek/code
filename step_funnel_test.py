...
ile, alet-delikleri degil. extract_cylinders zaten koni cikariyor.
Test: her STEP silindir-acikligi -> wire(uretici GT'ye yakin) vs non-wire etiketle -> koni-varligi
bu etiketi ayiriyor mu (AUC). AUC>>0.5 -> gercek lever. ~0.5 -> huni hipotezi olu.
DURUST: bu sadece 'sinyal VAR mi' testi; entegrasyon/kazanc ayri."""
...
# Argv SAGLAM okunur: bu dosya pytest adlandirmasina uyuyor ve toplanirsa argv pytest'in
# kendi bayraklarini icerir ('-q' gibi). Eskiden int('-q') COLLECTION'i patlatiyordu.
...
for mfg, pid, jf, stp in parts:
    try:
        ...
        for c in cyl:
            ...
            # wire etiketi: bu silindir uretici GT'ye axis-aware yakin mi
            ywire = 0
            if len(Gs):
            ...
            # KONI feature: en yakin koni mesafesi + var mi (<8mm)
            if len(conePos):
...
print(f"{len(parts)} parca -> {len(rows)} STEP silindir-aciklik | wire {int(Y.sum())} / non-wire {int((Y==0).sum())}")
...
print(f"  wire'da koni-var oran: {HC[Y==1].mean():.2f} | non-wire'da: {HC[Y==0].mean():.2f}")
...
for m in ("WEI", "PXC"):
    ...
    if mk.sum() > 10 and Y[mk].sum() > 3 and (Y[mk] == 0).sum() > 3:
        print(f"    {m}: has_cone AUC {roc_auc_score(Y[mk], HC[mk]):.3f} (wire koni-oran {HC[mk & (Y==1)].mean():.2f} vs non {HC[mk & (Y==0)].mean():.2f})")
