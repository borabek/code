...
def reorient (V ):
    ...
    ever opens ten a pure edge. Rather than guess which axis a given viewer looks down (Windows
...
def cpreview_glb (V ,F ,cps ,out ):
    ...
    for i ,cp in enumerate (cps ):
        ...
        s .apply_translation (p +d *diag *0.012 )# small proud offset -> sits ten the opening
...
def main ():
    ...
    ap .add_argument ("--vertex-conf",type =float ,default =0.9 )# per-vertex mask ten MODEL preds
    ...
    for s in samples :
        ...
        # bake orientation so any viewer opens ten the large face (not the thin edge)
