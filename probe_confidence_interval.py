...
PARCA duzeyi (option duzeyi DEGIL) because same parcanin CP'leri bagimsiz
...
DOKUM =os .environ .get ("GA_DOKUM","results/_dump_baseline.json")
YOL =os .environ .get ("GA_YOL","field")
...
def parca_sayilari (r ):
    ...
    for ad ,im in (("signed",True ),("unsigned",False )):
        ...
        signed =im )[:3 ]
    out ["detection"]=match_hungarian (P ,D ,G ,Gd ,dg ,0.0 ,180.0 ,True )[:3 ]
    return out 
...
def main ():
    ...
    print (f"{len (d )} part | {N_BOOT } bootstrap | path={YOL }\n")
    print (f"{'metrik':<12}{'F1':>9}{'%95 GA':>22}")
    out ={}
    for m_ in ("detection","unsigned","signed"):
    ...
    json .dump ({"path":YOL ,"n_parca":len (d ),"n_boot":N_BOOT ,"sonuc":out ,
