# -*- coding: utf-8 -*-
"""URUN DEMOSU: random WSCAD .stp -> robot CP'leri bulur. Tam kullanicinin istedigi sey.

Uretici ground-truth GEREKMEZ -- ham, hic gorulmemis bir parca ver, robot (vote>=2 urun) CP listesini
ve gorseli uretir. Her CP: nokta + yon (kirmizi ok) + boyut + derinlik + guven + tier.
...
def main():
    ...
    for k, pid in enumerate(pids):
        ...
        for c in cps:
            print(f"     nokta {c['point']}  yon {c['direction']}  boy {c['size_mm']}mm  "
                  f"derinlik {c['depth_mm']}mm  guven {c['confidence']}  oy {c['votes']}  [{c['tier']}]")
        # gorsel
    ...
    fig.suptitle("ROBOT: random WSCAD .stp -> CP'ler (kirmizi=otonom, turuncu=insan-kontrol; ok=nokta+yon)  --  hic gorulmemis parcalar",
                 fontsize=13)
