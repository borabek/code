...
Olculenler (none of them gate kullanmaz -> gate kalitesinden BAGIMSIZ):
  pool recall     GT'nin yuzde kaci havuzda ULASILABILIR (tespit toleransi)
  donusum          ulasilabilir GT'lerin yuzde kaci ROBOT olcutunu de saglar
  tespit tavani    mukemmel selector with tespit F1
...
def main ():
    ...
    kesisim =set (val )&d7 
    print (f"VAL {len (val )} part | D7 ile kesisim {len (kesisim )} "
    ...
    for ad ,c in out .items ():
        for criterion ,v in (("tespit",c ["tespit_tavani"]),("robot",c ["robot_tavani"])):
