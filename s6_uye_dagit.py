# -*- coding: utf-8 -*-
"""S6: UYE SECICIYI dagit ve UCTAN UCA olc (tam korpus verisiyle).

R7 (kahin):   dort modelin ATILAN yonlerinde robot 0.5523 -> 0.6059 yetecek bilgi VAR (+0.0536)
R9 (on-kestirim, YALNIZ 194 parcanin verisiyle): ogrenilmis secici +0.0301, kahinin %56'si,
    GA [+0.0070, +0.0560] KANITLI

Bu betik TAM KORPUS uye verisiyle (r8_uye_veri.py) secicinin son halini egitir, uctan uca
olcer ve gecerse dagitir.

POSE HEAD DERSI: ayni fikir 1068 satirla KANITSIZDI (+0.0222), 6330 satirla KANITLI (+0.0428).
Burada 3215 -> ~30000 satir bekleniyor.
...
def main():
    ...
    print(f"uye verisi: {len(UY)} satir -> sizinti disi {int(disi.sum())} "
    ...
    print(f"\n{'kol':<26}{'tespit':>10}{'ROBOT':>10}")
    print(f"{'A dagitilan':<26}{f1w(det['A']):>10.4f}{f1w(rob['A']):>10.4f}")
    print(f"{'B +uye secici':<26}{f1w(det['B']):>10.4f}{f1w(rob['B']):>10.4f}")
        ...
        with open("results/uye_secici.pkl", "wb") as f:
            pickle.dump({"sec": sec, "n_feat": UX.shape[1], "yakin_mm": YAKIN,
                         "note": ("UYE SECICI 2026-08-02: _vote2 birlestirmede ATILAN uye "
                                  ...
                                  f"secici {dr:+.4f} (GA [{lo:+.4f},{hi:+.4f}]). Tespit bedeli "
                                  "YAPISAL SIFIR: tespit olcutu aciya bakmaz.")}, f)
        print("-> results/uye_secici.pkl")
    print("makbuz -> results/s6_uye_dagit.json")
