...
NEDEN YENIDEN: `results/_agiz_tan` yalnizca `brep_adaylari` ciktisi icin
uretilmisti; `birlesik_havuz` segmentasyon adayina yakin B-rep onerilerini
ELEDIGI icin siralamalar ORTUSMUYOR. Hizalanmamis oznitelik sessizce yanlis
satira baglanir -- bu projede en pahali hata turu.

Burada tanimlayicilar dogrudan `_tam_oz`'daki (P, D) uzerinden hesaplanir:
her aday icin EN YAKIN silindir (eksene dik mesafeyle) ebeveyn sayilir, yoksa
yaricap/derinlik 0 kalir ve yalnizca isin olculeri dolar.

Ayrica DAHA ONCE OLCULEN AUC'lar BOZUK ETIKETLE alinmisti; bu kol duzeltilmis
etiketle (yanal + eksenel 40 + acgozlu bire-bir) yeniden sinanacak.
"""
import json
import os
import pickle
...
def ebeveyn(P, cyl):
    """Her aday icin en yakin silindirin (merkez, eksen, yaricap) kaydi."""
    if not cyl:
...
def main():
    ...
    for ad in ("d7", "d6", "tam"):
        ...
        if ad == "d6":
            kay = d6_kayit.yukle(set(d6_kayit.sinav()["pidler"]))
        elif ad == "d7":
        ...
        for pid, r in sorted(kay.items()):
            ...
            T = AT.tanimla(P, D, ebeveyn(P, cy.get(pid)), mesh,
                           float(r["diag"]))
            if len(T) != len(P):
                raise SystemExit(f"{ad}_{pid}: tanimlayici {len(T)} vs aday "
                                 f"{len(P)} -- HIZALAMA BOZUK, duruyorum")
            np.savez_compressed(hedef, T=T.astype(np.float32))
            n += 1
            if n % 200 == 0:
                print(f"  {ad} {n} yazildi {(time.time()-t0)/n:.2f}s/parca",
                      flush=True)
