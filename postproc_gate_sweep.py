...
min_v 30 / vc 0.5 gate YOKKEN secilmisti (kucuk/dusuk-guven fragmentler gurultu diye atiliyordu). Gate
artik precision'i temizledigi icin, union'a DAHA COK recall sokup gate'e filtreletmek F1'i artirabilir.
Her parca: 4 model probs BIR kez -> her (min_v, vc) kombosunda per-model CP turet -> union(vote>=1) ->
gate ozellik -> OOF gate skoru -> F1. Sizinti-siz (GroupKFold). 823 parca.
"""
...
def main():
    ...
    print(f"{len(parts)} parca | {len(GRID)} (min_v,vc) kombosu x union+gate", flush=True)

    # her kombo icin: X listesi, y, votes, grp, mfg + ngt
    ...
    GT = sum(ngt.values())
    print(f"\n=== POST-PROC + GATE SWEEP (uretici {GT}) | mevcut urun min_v30/vc0.5+gate0.35 = 0.693 ===")
    ...
    for (mv, vc) in GRID:
        ...
        # her min_v/vc icin en iyi gate-esik
