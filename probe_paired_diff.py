# -*- coding: utf-8 -*-
"""ESLI FARK BOOTSTRAP — a last-islem kuralinin kazanci GERCEK mi?

`halka_disari` kurali robot ISARETLI F1'i 0.4839 -> 0.5168 (+0.0329)
cikardi. Marjinal confidence araligi (±0.08) here YANLIS testtir: kiyas AYNI
parcalarda yapiliyor, therefore anlamli which is FARKIN dagilimidir.

Bu betik part duzeyi ESLI bootstrap yapar: each tekrarda same part
kumesi secilir, two arm da that kumede is computed, FARK kaydedilir.
...
def say (r ,arm ):
    ...
    P ,D =uygula (P .copy (),D .copy (),arm ,G ,Gd ,
    r .get ("mesh_merkez"),r .get ("yerel_normal"),
    r .get ("halka_normal"))
    return match_hungarian (P ,D ,G ,Gd ,float (r ["diag"]),K .YANAL ,K .ACI ,
    False ,signed =True )[:3 ]
...
def main ():
    ...
    print (f"{len (d )} part | arm={KOL } | {N_BOOT } ESLI bootstrap\n")
    print (f"  baseline      {fa :.4f}")
