# -*- coding: utf-8 -*-
"""OTOPSI: GLB'de "yanlis" gorunen CP'lerin FIZIKSEL olcumu (FAZ 1 -- KANIT, DUZELTME YOK).

BELIRTI (2026-08-04, gozle): uretilen GLB'lerde isaretciler govde ucundan tasiyor, bazi oklar
seridin UZUN EKSENI boyunca, yogun parcada isaretciler goruntudeki yariklarla hizali degil.
CELISKI: ayni parcalardan biri (2669020000) metrikte **F1 1.000** aliyor.

ILK IPUCU (eslesme log'u): perp 0.4-2.1mm ama **AXIAL +12..+15mm**. `sina_kume.esle` eksenel
toleransi **40mm** -- yani metrik eksende 40mm uzaktaki noktayi "dogru" sayar.

HIPOTEZ SIRALAMASI (Faz 3'te teker teker sinanacak, once KANIT toplanir):
  H1  TANIM FARKI: tez CP'yi ACIKLIGIN AGZINDA (v_o), uretici KONTAKTA (seat) tanimlar.
      Eksenel 12-15mm bu farkin ta kendisidir; hata degildir. -> Sinamasi: bizim noktamizi
      uretici seat'inin KENDI AGZIYLA (`cp_geometry.seat_to_mouth`) karsilastir. Fark
      COKERSE H1 dogrulanir.
  H2  YON HATASI: yon serbest uzayi gostermiyor (kanala girmiyor). -> Sinamasi: +yon ve
      -yon boyunca serbest mesafe. Takma yonu ACIK olmali, ters yon MALZEMEYE carpmali.
  H3  YERLESIM HATASI: nokta bir agizda degil, duz yuzeyde/govde icinde. -> Sinamasi:
      `is_inside` ve `mouth_width`.
  H4  CIZIM HATASI: nokta ve yon dogru ama GLB yanlis ciziyor. -> Sinamasi: bu betigin
      olcumleri ile GLB makbuzundaki degerleri karsilastir (ayri kosuda).

Bu betik HICBIR SEYI DUZELTMEZ. results/otopsi_cp.json uretir.
"""
...
def serbest(mesh, p, d, max_mm=60.0):
    """p'den d yonunde ILK yuzeye uzaklik (yoksa inf). 'Bu yon acik mi' sorusunun olcusu."""
...
def main():
    ...
    for pid in PARCALAR:
        ...
        # --- H1: URETICI SEAT'inin KENDI AGZI (tezle ayni tanim)
        Gm = []
        ...
        for i in range(len(P)):
            p, d = P[i], Dd[i]
            # en yakin GT (seat) -- metrigin kullandigi olcut
            ...
            # ayni GT'nin AGZINA uzaklik -- H1'in sinavi
            db = p - Gmouth[b]
            ...
            satir.append({
                ...
                # mouth_width -> (ic_cap, ort_cap). Nokta govdenin DISINDAYSA (0.0, 0.0) doner:
                # "olculemedi" demektir, "genislik sifir" DEMEZ. Ikisi ayri okunur.
        ...
        R_ = {"parca": pid, "boyut_mm": [round(float(x), 1) for x in boyut],
        ...
        print(f"uretici seat'inin KENDI agzina uzakligi (medyan): {R_['seat_offset_medyan']:.1f}mm")
    ...
    print("\nOKUMA KILAVUZU:")
    print("  perp_AGIZ ~ perp_seat AMA ax_AGIZ ~ 0  -> H1 DOGRU (tanim farki, hata degil)")
    print("  ax_AGIZ hala buyuk                     -> H1 YANLIS, gercek yerlesim hatasi")
    print("  serbest_ileri kucuk / geri buyuk       -> yon TERS (H2)")
    print("  iceride_mi True                        -> nokta govde icinde (H3)")
