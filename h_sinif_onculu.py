# -*- coding: utf-8 -*-
"""H: argmax yerine SINIF-ONCULU karar. Bir cikarim, sonra bedava suepurme.

NEDEN: etiket su an 5 sinifli softmax'in ARGMAX'i. Baglanti siniflari (CableEntry/Contact)
NADIR; argmax bu yuzden yapisal olarak Housing lehine yanli. Bir kose %35 CableEntry / %40
Housing oldugunda Housing kazanir ve o aciklik HIC atesnlenmez. Kayitli bulgu: dusuk-CP'de
"ag atesledikten sonra hicbir sey kaybedilmiyor, tavan dogrudan ATESLEME ORANI" -- yani bu
eksen tam da tavana bakiyor ve bugune kadar HIC taranmadi (vertex_confidence_mask bir MASKE,
oncul duzeltmesi degil).

YONTEM: p_duzeltilmis = p / oncul**alpha, sonra argmax. alpha=0 mevcut davranis.
Cikarim BIR KEZ kosar, olasiliklar saklanir, alpha bedava suepurulur -- fark boylece modelden
degil YALNIZ karar kuralindan gelir.

KILL (olcumden once): tespit F1 gerilerse duser. Recall artip precision'i gate geri
alamiyorsa duser.
"""
...
def main():
    ...
    print(f"{len(sel)} parca | esikler {thr} | gate {int(keep.sum())} aday", flush=True)

    if os.path.exists(CACHE):
        cache = pickle.load(open(CACHE, "rb"))
        print(f"onbellek diskten: {len(cache)} parca (cikarim tekrar kosmuyor)", flush=True)
    else:
        ...
        print(f"onbellek yazildi: {len(cache)} parca", flush=True)

    # sinif oncullerini EGITIM verisinden degil, cikarim olasiliklarinin ortalamasindan al
    acc = np.zeros(5)
    for r in cache:
        for pb in r["pbs"]:
    ...
    def score(alpha):
        ...
        for r in cache:
            ...
            # URUNLE AYNI olmali: yonlendirici cok-CP derse promote ile YENIDEN turet.
            # Ilk surumde is_hi hesaplanip KULLANILMIYORDU -> alpha=0 tabani 0.7518 cikti,
            # oysa urunun gercek tabani 0.7784. Goreli etki icin zararsizdi ama mutlak
    ...
    print(f"{'alpha':>7}{'tespit F1':>11}{'dusuk':>9}{'cok':>9}{'kesinlik':>10}{'recall':>9}")
    res = {}
