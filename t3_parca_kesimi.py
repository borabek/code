# -*- coding: utf-8 -*-
"""T3: PARCA BASINA KESIMI OGREN (tespit tavanina giden en ucuz yol).

T2 bosslugu ikiye boldu:
    ESIK kaybi   +0.0731 (%38) -- kural degisikligiyle alinabilir
    SIRALAMA     +0.1202 (%62) -- yeni bilgi gerekir

Ve kritik ayrinti: TEK bir sabit esik hicbir sey vermiyor (en iyi global esik +0.0039,
en iyi goreli oran +0.0000). Yani kayip "esigi yanlis sectik"ten DEGIL, "her parcada dogru
kesim farkli"dan geliyor. Kahin parca-ici kesim +0.0731.

Bugun ayni desen IKI KEZ ise yaradi: kurali sabit bir sayiya degil, parcanin KENDI sinyaline
baglamak (cokus yonlendirmesi) ve ogrenilmis secici (yarik yonu). Burada da ayni sey:
parcanin SKOR DAGILIMINDAN kac aday tutulacagini ogren.

OZELLIKLER (hepsi calisma aninda, GT'siz):
    n_aday, maks, ortalama, std, medyan
    ilk-ikinci farki, ilk-ucuncu farki  (tepe ne kadar ayrik?)
    esik ustu sayilar (0.25/0.35/0.5/0.7)
    skor dusus profili: sirali skorlarin ardisik farklarinin maksimumu ve yeri (DOGAL KESIM)
    kosegen, cok-CP yonlendirici bayragi

HEDEF: kahin K (F1'i en cok yapan tutulan aday sayisi).
DEGERLENDIRME: geometri anahtarina gore GroupKFold, FOLD DISI tahmin.

KILL (onceden yazili): tespit >= +0.02 VE DEV ile VAL ayni yonde VE gorulmemis uretici
...
def main():
    ...
    print(f"{len(PAR)} parca", flush=True)

    # KAHIN K: F1'i en cok yapan tutulan aday sayisi (skor sirasina gore ilk K)
    for p in PAR:
        en, enk = -1, 1
    ...
    su_K = np.array([int(wire_gate.karar_maskesi(p["s"]).sum()) for p in PAR], float)
    ...
    kah = puanla(yk)
    yeni = puanla(oof)
    print(f"\n{'kol':<26}{'TESPIT':>9}{'DEV':>9}{'VAL':>9}{'PXC':>9}{'WEI':>9}")
    ...
    print(f"\nKILL: tespit >= +0.02 VE DEV/VAL ayni yonde VE uretici ort. dusmeyecek")
    print(f"  tespit {d_:+.4f} | DEV {dev_ok:+.4f} | VAL {val_ok:+.4f} | "
          f"uretici ort {mfg_b - mfg_a:+.4f} -> {'GECTI' if gecti else 'GECMEDI'}")
    print(f"  kahinin {d_ / max(kah - a, 1e-9):.0%}'i yakalandi (kahin {kah - a:+.4f})")
    with open("results/t3_parca_kesimi.json", "w", encoding="utf-8") as f:
    ...
    print("\nmakbuz -> results/t3_parca_kesimi.json")
