...
HIPOTEZ (R0+R1'den): CP'nin ekseni uc kaynaktan gelebiliyor --
    (a) B-rep silindir ekseni  : ANALITIK, egimi tam bilir      -> aci ~0
    (b) normal-kovaryans        : mesh'ten, egimi gorebilir
    (c) koordinat yuvarlamasi   : (a) ve (b) susarsa kalan TOHUM -> egik agizda 90 dereceye kadar hata
R1'de tahminler tam [1,0,0]/[0,1,0] cikti = (c). Ayni CP'lerde nokta da centroid'de kaliyor,
yani YANAL hata da orada.

Yani tek bir kok neden iki metrigi birden dusuruyor olabilir: B-REP ESLESMESININ TUTMAMASI.

Bu onemli, cunku kodda ASILI bir is var (cp_openings.py icindeki not): B-rep kapilari
(BREP_MAX_OFF, BREP_R_MAX) yaricap YAY HATASI varken ayarlanmisti -- yaricaplar 3.5 kat kucuk
cikiyordu. Hata 2026-07-31'de duzeltildi ama kapilar YENIDEN TARANMADI ("P2 taramasi").

OLCULEN: eslesen her GT noktasi icin adayin `brep_r` degeri (gate sutunu 13; 0 = silindir
eslesmedi) ile yanal/aci basarisi arasindaki iliski.
"""
import json
import os
import pickle
...
BREP_R_SUT = 13          # FEAT_NAMES: 0..12 taban, 13 = brep_r
...
def main():
    ...
    for r in DER:
        ...
        for d_, a_, b_ in sorted((pe_t[a, b], a, b)
            ...
            SAT.append({"brep_r": float(r["X"][idx[a_], BREP_R_SUT]),
                        "yanal": float(pe[a_, b_]), "aci": float(an[a_, b_]),
                        "pid": r["pid"], "rejim": "cok" if r["n"] >= 8 else "dusuk"})

    br = np.array([x["brep_r"] for x in SAT])
    ya = np.array([x["yanal"] for x in SAT])
    ...
    print(f"\n{'grup':<22}{'n':>6}{'yanal med':>11}{'aci med':>10}{'yanal<=2':>10}"
          f"{'aci<=10':>9}{'IKISI':>8}")
    ...
    print(f"\n{'rejim':<10}{'n':>6}{'B-rep var':>11}{'IKISI OK':>10}")
    for rj in ("dusuk", "cok"):
        i = np.array([x["rejim"] == rj for x in SAT])
        if i.any():
