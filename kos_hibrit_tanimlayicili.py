...
Onceki hibritte B-rep adaylari yalniz 58 gate ozniteligiyle puanlaniyordu ve kol
NULL cikti (tavan 0.5748 ama uctan uca 0.1959 <= taban 0.1970). Olculdu ki o 58
oznitelik bir agzin gercek tel girisi olup olmadigini SOYLEMIYOR.

`agiz_tanimlayici.py` o bilgiyi uretti. Parca-ICI AUC (egitim yok, saf ayrilabilirlik,
`results/agiz_ayirt_edicilik.json`): yaricap 0.690, es_eksen 0.293, narinlik 0.317,
girme_kenar 0.331, girme 0.343 -- 0.29 ile 0.69 esit guclu, yon ters.

MIMARI DEGISMEDI ve bu KASITLI: seg adaylari DAGITILAN v6 ile puanlanir (tez-saf
kol AYNEN korunur, kol kotu calisirsa taban KAYBEDILMEZ); yalniz B-rep modeli
58 + 9 = 67 sutunla, parca-ici z-skor donusumuyle yeniden egitilir.
...
def tanim(on, pid, n_bek):
    """Agiz tanimlayicilari. Sayi tutmazsa SESSIZ KABUL YOK -- None doner."""
...
def egitim_seti():
    ...
    for f in sorted(os.listdir(OZ)):
        ...
        if T is None:
            uyumsuz += 1
            continue
        M.append(wire_gate.parca_ici(np.hstack([Xb, T]), DONUSUM))
        Y.append(yb)
    print(f"  tanimlayici sayisi tutmayan/eksik parca: {uyumsuz}", flush=True)
...
def sinav_seti():
    ...
    for f in sorted(os.listdir(OZ)):
        ...
        te.append({"pid": pid, "X": X, "T": T, "P": z["P"], "D": z["D"],
                   "kaynak": kay})
    print(f"  tanimlayicisi eksik sinav parcasi: {atlanan}", flush=True)
    return te


def main():
    print("egitim seti kuruluyor...", flush=True)
    M, Y = egitim_seti()
    ...
    def kos(b_esik):
        ...
        for tip, e in ([("mutlak", x) for x in (0.30, 0.40)] +
            ...
            for d in te:
                ...
                if len(P) > 1:
                    nm = wire_gate.kalabalik_maskesi(P, np.asarray(S, float))
                    P, D = P[nm], D[nm]
    ...
    for ad in out:
        if ad == "TEZ-SAF (B-rep KAPALI)":
            continue
        r = out[ad]
        art = sum(1 for m in r["marka"] if r["marka"][m] > t["marka"][m] + 1e-9)
        yik = [m for m in r["marka"] if r["marka"][m] == 0 and t["marka"][m] > 0]
        print(f"  {ad:<14} robot {r['robot']-t['robot']:+.4f} | tespit "
              f"{r['tespit']-t['tespit']:+.4f} | artan marka {art}/12"
              + (f" | YIKILAN: {','.join(yik)}" if yik else ""))
    json.dump({"damga": makbuz_hash.damga(), "sonuc": out,
               "not": "HIBRIT + agiz tanimlayicilari (58+9=67 sutun, parca-ici "
                      "zskor). Seg kolu DAGITILAN v6, degistirilmedi. D7 "
                      "marka-disi, MIKRO. B-rep TEZ TURETMESI DEGIL."},
              open("results/hibrit_tanimlayicili_d7.json", "w"), indent=1)
    print("makbuz -> results/hibrit_tanimlayicili_d7.json")
