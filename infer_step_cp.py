# -*- coding: utf-8 -*-
"""Thesis inference pipeline (§5.3.7) end-to-end ten a RAW WSCAD STEP -> CP. This is the
...
# Averaging seeds is conservative: it flatters precision ten a precision-limited test set (the 18-CP
# arbiter, the 82-CP human held-out) and costs RECALL, which is what collapses ten an unseen
...
def step_to_mesh (path ):
    """STEP -> ucgen mesh (gmsh with surface mesh'i).

    KRITIK -- try/finally SART (2026-07-28'de a gecelik is kaybettirdi):
    `gmsh.model.mesh.generate(2)` some STEP'lerde istisna atar (measured: WEI.2502750000 ->
    "Impossible to mesh periodic surface 65"). Eskiden `finalize()` fonksiyonun SONUNDAYDI, i.e.
    istisna halinde HIC CALISMIYORDU: gmsh baslatilmis and kirli halde kaliyor, sonraki each part
    birikmis durumun uzerine yukleniyordu. Sonuc: 220 part normal hizda islendi, first hatadan after
    is surunmeye basladi and 3 saat along TEK SATIR ilerlemedi -- error mesaji da yoktu, only
    "calisiyor" like gorundu. finalize() residual finally'de: hatali part temiz sekilde atlanir.
    """
    # OCC-ONCE ROTASI (2026-08-05) -- ISTEGE BAGLI, VARSAYILAN KAPALI.
    # MEASURED: SE.NTSXTB18000H (30 MB STEP) gmsh'te 25+ DAKIKA after still bitmemisti;
    # OCC same parcayi 15.3s'de ucgenledi (9150v -> tez remesh 8889v). Yani very large
    ...
    # OCC FIX YEDEGI (2026-07-28): some STEP'ler "Impossible to mesh periodic surface" with
    ...
    # and tum onbellekler/olcumler gecersiz becomes. Normal path aynen korunur.
        ...
        # and tum onbellekler AYNEN korunur.
            ...
            # DORDUNCU KADEME (2026-08-05, DataSet 6): three gmsh kademesi de duseni
            # OCC'nin KENDI tessellatoru with al. Butun gmsh kademeleri same places
            ...
            # 2B parametre uzayinda mesh'lemek for KAPALI tel ister; BRepMesh whereas
            # yuzeyi dogrudan ucgenler and bozuk tele TAHAMMUL eder.
            #
            # TEZ ACISINDAN NOTR: this only ILK tessellasyon kaynagi. Hemen ardindan
            # `thesis_remesh.remesh_uniform(V, F, target=6000)` works, i.e. part
            # yine tezin ~6000 tepeli uniform izotropik mesh'i becomes. Degisen single sey,
            # aksi halde TAMAMEN KAYBEDILECEK parcanin kurtarilmasi.
                return _occ_mesh (path )


OCC_SAPMA =0.25 # mm, lineer tessellasyon sapmasi (remesh already 3mm hedefinde)
OCC_ACI =0.5 # rad, acisal deviation


def _occ_mesh (path ):
    """DORDUNCU KADEME: OCC BRepMesh with dogrudan tessellasyon (gmsh'siz).

    `_gmsh_mesh` with AYNI tamlik disiplinine tabidir: kac yuzeyin ucgeni output sayilir,
    %95 under kalirsa YARIM MESH sayilip atilir. Yarim kabugu korpusa sokmak, parcayi
    durustce kaybetmekten KOTUDUR (see. `_tamlik_kontrol`: DST2.5_GY'de 202 yuzeyin 92'si).
    """
    ...
    while exp .More ():
        ...
        for i in range (1 ,nn +1 ):
            ...
            # TERS YUZ (reversed face): OCC ucgen sirasini yuzun yonelimine according to gives.
    ...
    F =np .asarray (Fs ,int )
    # Yuz face uretilen vertices DIKISSIZ gelir: same fiziksel kose each yuzde ayri vertex.
    # thesis_remesh this gecirmez a surface bekledigi for kaynaklanir.
    V ,F =_kaynakla (V ,F )
    return V ,F 
...
def _kaynakla (V ,F ,tol =1e-6 ):
    ...
    # dogrusal may be. Bunlar normal hesabinda 0'a bolunme -> NaN normal produces
    # (diffusion_net/geometry.py:109 "invalid value encountered in divide").
    # Yalniz OCC yolunda temizlenir; gmsh ciktilari and onbellekleri DEGISMEZ.
    if len (F2 ):
        e1 =V2 [F2 [:,1 ]]-V2 [F2 [:,0 ]]
        e2 =V2 [F2 [:,2 ]]-V2 [F2 [:,0 ]]
...
def _tamlik_kontrol (nyuzey ):
    ...
    WHY IT EXISTS (2026-08-04): ucuncu kademeyi yazarken "gmsh error verse bile geriye kismi
    mesh birakiyor mu, onu kullansak mi" diye olctum. Birakiyor -- but DST2.5_GY'de
    **202 yuzeyin only 92'si** mesh'lenmisti (%46). O mesh korpusa girseydi part
    half kabuk halinde egitime and olcume katilirdi: fake agizlar, fake CP'ler.
    Parcayi DURUSTCE kaybetmek, bozugunu sessizce eklemekten iyidir.

    Ayrica gmsh sometimes istisna ATMADAN da surface atlayabilir (only uyari gives); this
    kontrol that silent durumu da yakalar.
    """
...
def render (V ,F ,plab ,cps ,out ):
    ...
    # left: semantic; right: CP ten grey
