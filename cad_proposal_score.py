# -*- coding: utf-8 -*-
"""ONER-SONRA-PUANLA kaskadi (fbi_recall bulgusundan dogdu; tez-sadik: YOLOv6 kolu da bolge onerip
sonra segmentasyonla dogruluyor -- Abbildung 45/46).

BULGU: CAD-geometri model-FN'lerinin %60'ini kapsiyor ve model bunlarin %63'unde ZATEN atesliyor,
ama ham CAD-union precision'i cokertiyor (P 0.590->0.210) cunku step_openings HER deligi buluyor
(montaj/vida dahil) -- hangisi TEL GIRISI ayirt edemiyor.
COZUM: CAD acikliklarini ADAY olarak al, her adayi MODELIN baglanti-olasiligiyla PUANLA
(aciklik ekseni boyunca silindirde), esigin ustundekileri tut. Modelin kendi CP'leriyle union.
Esigi sup: precision/recall sinirini cikar -> tek-model urunu (F1 0.563) geciyor mu?
"""
...
def score_candidate(V, conn_p, entry, axis, radius_mm):
    """Modelin bu CAD acikligina verdigi destek: aciklik ekseni boyunca silindirde baglanti
    olasiliginin ust yuzdeligi. entry/axis STEP(=mesh) frame'inde."""
    ...
    near = (al >= -6.0) & (al <= 20.0) & (perp <= rad)   # agizdan ice dogru
    if near.sum() < 3: return 0.0
    return float(np.percentile(conn_p[near], 90))        # ust yuzdelik = "model burayi baglanti goruyor mu"
...
def main():
    ...
    print(f"\n=== ONER-SONRA-PUANLA ({len(parts)} parca) | urun tek-model F1 0.563 ===")
    print(f"{'CAD esik':>9s} | {'P':>6s} {'R':>6s} {'F1':>6s}")
    best = None
    for thr in THRS:
        T = Fp = Fn = 0
        for c in parts:
    ...
    print(f"\n  EN IYI kaskad F1={best[0]:.3f} (P{best[1]:.3f} R{best[2]:.3f}) @ CAD esik {best[3]:.2f}")
