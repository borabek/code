...
GORULEBILEN a istatistige according to yonlendirme kurali ogrenir:
    ...
    aksi halde                 ->  DAGITILAN TABAN (product_genis) kullan
...
def istatistik (d ,s_tb ):
    ...
    product_genis .ESIK ).sum ())}


def taban_cikti (d ,model ):
    """DAGITILAN yolun ciktisi, onbellekten yeniden kurulmus.

    `product_genis.sec` with same: A+B, part-ici z-skor, HGB-derin, threshold 0.05,
    ...
    return (P ,product_genis .isaret_duzelt (D ,T )),s 
