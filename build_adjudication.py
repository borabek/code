...
is useful whichever way it goes:
  "gercek aciklik"  -> the annotator missed it -> POSITIVE label correction (recall)
  "CP degil"        -> a genuine distractor    -> NEGATIVE example (precision -- the signal this
...
def main():
    ...
    for k, ((pid, root), items) in enumerate(sorted(by_part.items()), 1):
        ...
        rows = "".join(
            ...
            f'<label><input type=radio name="{pid}_{i}" value="opening"> gercek aciklik (atlamisim)</label>'
...
<header><h1>CP hakemleme &mdash; {d['n_disagreements']} bolge / {len(by_part)} parca</h1>
...
function dl(){{
 ...
 const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='adjudication.json';a.click();}}
    ...
    print(f"-> {OUT}/index.html  ({d['n_disagreements']} bolge, {len(by_part)} parca)")
