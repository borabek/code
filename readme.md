...
| `make_samples.py` | Generate valid synthetic ABB-format parts — CLI entry point with `--mode flat/feat/hard/varied` (default `hard`) |
| `make_samples_feat.py` | Synthetic parts where each CP is a distinctive raised **boss** apex (learnable by the geometric model — fixes the flat-plate heatmap collapse) |
| `make_samples_hard.py` | Adds non-CP **decoys** (sharp cones / pits / box-like blocks) + zero-CP negatives so the model also learns to **discriminate** (cuts false positives on unfamiliar geometry) |
| `make_samples_varied.py` | Several CP feature **types** (boss / pin / rectangular pad / recessed **socket**) so the model recognises a wider range of real connection geometry |
| `step_to_json.py` | Convert raw 3D models (STEP/STL/OBJ/OFF) → ABB JSON (empty CPs) for inference; its `iter_parts_any()` also lets `predict.py` read `.stp` **directly** |
| `viz_preds.py` | Overlay detected connection points on a part mesh → `.obj` scene to eyeball in Blender / MeshLab |
| `render_scene.py` | Render mesh + detected CPs to **PNG** images (height-coloured point cloud) — eyeball detections with no 3D software |
| `step_openings.py` | **Extract connection points directly from a STEP B-rep** — the circular openings are CAD cylinders; via gmsh this is exact and ML-free. `--label-corpus` writes a REAL labelled corpus to fine-tune on |
| `detect_openings.py` | Geometric (no-ML) recessed-**pocket** detector on a mesh (works for deep sockets; kept as reference — these connectors' openings are flush so it does not apply) |
| `run_full.py` | Thin wrapper: full-corpus run with the known-good flags **and the held-out-best decode operating point** baked in (forwards to `train_cp.main`) |
...
```bash
# generate LEARNABLE + discriminative synthetic parts (default --mode hard),
# then smoke-test any backbone on them
python make_samples.py ./synth 400
python train_cp.py ./synth --backbone knngraph --device cuda --epochs 40 \
    --split-group none --run-name smoke --export-model model_smoke.pt

# run every module's selftest
python run_selftests.py
```

> **`--mode`** picks the generator: `flat` = legacy plate with CPs at
> indistinguishable spots (a geometric model **collapses** on it — kept only for
> comparison); `feat` = each CP is a distinctive raised boss (learnable);
> `hard` = `feat` + non-CP decoys + negatives so the model also learns to
> **discriminate** (the default, fewest false positives); `varied` = several CP
> feature types incl. a recessed **socket**, broadening real-connector coverage.

### Detect connection points on a raw `.stp` / 3D model

`predict.py` accepts a STEP/STL/OBJ/OFF file **directly** — it tessellates the
STEP via gmsh internally (no separate conversion step), then runs the detector:

```bash
# one command: raw STEP -> robot-ready connection points
python predict.py model_smoke.pt part.stp --device cuda --out preds.json

# to also SEE where the points landed, convert the mesh and overlay them:
...
STEP tessellation needs gmsh (`pip install gmsh`).

### Connection points straight from the STEP CAD (geometric, no ML)

When a `.stp` is available, the connection openings are real CAD features.
`step_openings.py` reads them via gmsh — centres + insert axes, in the mesh
frame, **no model needed**. HONEST accuracy (measured 2026-07-06 against
independent human GT, see RESULTS.md §10): **F1 ~73%** with
`--auto --dir-consensus` — good, exact where it fires, but NOT the "exact,
100%" it was previously assumed to be (rectangular push-in openings are a
structural blind spot).

```bash
python step_openings.py part.stp --list                 # see the cylinder radii
python step_openings.py part.stp --out cp.json --auto    # auto-pick the terminal radius
python step_openings.py part.stp --out cp.json --rmin 1.3 --rmax 3.0   # or set it manually
python predict.py --from-cad part.stp --out cp.json      # same, via the unified CLI

# turn STEP parts into a REAL labelled corpus, then fine-tune the ML model on it
# (so it also works on mesh-only parts that have no STEP):
python step_openings.py ./stp_dir --label-corpus ./real_labeled
python train_cp.py ./real_labeled --backbone knngraph --device cuda \
    --split-group none --epochs 200 --run-name cp_real

# score CAD predictions against a human-labeled corpus (frame auto-alignment):
python cad_eval.py --preds cp.json --gt-dir <labeled_json_dir> --step-dir <stp_dir>
# visual QA (RED = CAD detections, GREEN = human GT) -> .glb files:
python cad_glb_qa.py --preds cp.json --gt-dir <labeled_json_dir> --step-dir <stp_dir>
# hybrid: CAD detections take priority, ML fills CAD's blind spots:
python cad_ml_fuse.py --cad-preds cp.json --gt-dir <dir> --step-dir <stp_dir> \
    --ckpt checkpoints/<model>_best.ckpt --device cpu
```

The radius filter selects which holes are connection points. Use **`--auto`** to
pick it per part (drops sub-1.3 mm fillets, keeps the terminal cluster up to the
gap before large mounting holes) — no manual tuning for a new family. Or set it
manually with `--rmin/--rmax` after inspecting `--list`. One unreadable STEP
does not abort a batch.

Detection coverage (2026-07 additions, defaults chosen from the measured eval):
- **Racetrack/oval slots** (push-in wire entries = two parallel half-cylinders
  ~2x r apart) are fused into ONE opening — default ON (`--no-slot-pairs` to
  disable). Fixed a systematic 1-TP+1-FP double-detection per slot.
- **Conical funnel lead-ins** are treated like cylinders — default ON
  (`--no-cones`).
- **`--dir-consensus`** (opt-in): keeps only openings facing the dominant
  approach direction (kills side/bottom DIN-latch and test-point cylinders);
  measured F1 57→73% on the human-GT eval, but risky on genuinely multi-face
  connectors.
- **`--rect-slots`** (opt-in, default OFF): rectangular spring-clamp openings
  from corner-fillet clusters — the only path that sees them at all, but noisy;
  do not enable until a bigger eval set exists to tune on.
- **`--check-dirs`** / label-corpus (always): every approach vector is verified
  against the tessellated mesh (free-space clearance probe) and flipped if it
  points into the part — geometrically grounded, replaces pure bbox heuristics.

`--label-corpus` **skips geometric duplicates** by default (near-identical
re-downloads would otherwise leak across the train/val split) — pass
`--keep-duplicates` to keep them. `--drop-corner FRAC` is an EXPERIMENTAL,
opt-in filter for mounting holes near the part edge (off by default — it can also
drop real edge terminals).
