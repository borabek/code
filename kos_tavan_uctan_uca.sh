#!/usr/bin/env bash
# TAVAN-24 UCTAN UCA: tavan kazanci gercekten F1'e donuyor mu?
#
# KAPI A bir TAVAN olcumudur: "mukemmel secici olsa ne alirdik". Tavanin
# yukselmesi, secici o yonleri SECEBILIRSE F1'e doner -- secemezse tavan
# bosuna yukselir. Bu betik uctan uca farki olcer.
#
# NEDEN d6 UZERINDE: `tam` bolumu her iki korpusta da saatler surecek, ama d6
# IKISINDE DE TAMAM. d6 gelistirme kumesidir (SINAV DEGIL), marka katlariyla
# LOMO kurulabilir. Tek degisken KORPUS (tavan 12 vs 24); ayni parcalar, ayni
# katlar, ayni tohum, ayni kurallar.
#
# UYARI: d6'nin TABANI zayiftir ve bu kampanyada bir kez ALDATTI (D6'da +0.1681
# vaat eden kol `tam` katlarinda +0.0230 verdi). Buradaki sayi YON GOSTERIR,
# manset DEGILDIR; `tam` korpusu bitince orada tekrarlanir.
set -u
cd "$(dirname "$0")"
G=results/_gece
mkdir -p "$G"
...
if len(r) == 2:
    a, b = r["tam3"], r["tam4"]
    print("\n=== TAVAN UCTAN UCA (d6 marka katlari) ===")
    print(f"{'':<14}{'tavan 12':>12}{'tavan 24':>12}{'fark':>10}")
    for k in ("robot", "recall", "kesinlik"):
        print(f"{k:<14}{a[k]:>12.4f}{b[k]:>12.4f}{b[k]-a[k]:>+10.4f}")
    print("\nNOT: d6 GELISTIRME kumesidir, manset DEGIL. Yon gosterir.")
PY
say "=== BITTI ==="
