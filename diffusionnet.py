...
def build_diffusionnet (config ,n_classes =N_CLASSES ,hks_count =16 ):
    ...
    outputs_at ="vertices",
    # diffusion_net's `dropout` is only an ten/off switch with FIXED internal
...
def set_dropout_rate (model ,p ):
    """Set real dropout rate ten all nn.Dropout modules (§1.2).
    ...
    ten and patch the rate here. returns: number of modules patched.
    """
    torch =_require ("torch","pip install torch")
    ...
    # scales eigen-decomposition cost/VRAM steeply (problematic for big meshes ten
...
def precompute_operators (verts ,faces ,k_eig ,op_cache_dir =None ):
    ...
    GPU memory ten large meshes. When `op_cache_dir` is given the eigenbasis is
    cached/loaded by diffusion_net (keyed ten the mesh) -- strongly recommended
...
def _forward (model ,ops ,x ):
    ...
    # ---------------------------------------------------------------------------
    # the network trains ten NLL but is evaluated ten IoU/Dice -- a poor surrogate
...
gamma =1.0 ):
    """Multi-class Tversky loss ten probabilities (probs: (V, C)).
...
def sinir_kaybi (probs ,target ,edge_ ,eps =1e-6 ):
    ...
    ek weight gives: `edge` maskesi, komsulari FARKLI sinifta which is
    tepeleri isaretler.

    Uygulama: boundary tepelerinde correct sinifin olasiligina odaklanan
...
def _kenar_maskesi (lab ,F ):
    """komsusu FARKLI sinifta which is vertices (sinif siniri)."""
    torch =_require ("torch","pip install torch")
...
def _compute_loss (out ,lab ,w ,meta ,cfg ,faces =None ):
    ...
    # ---------------------------------------------------------------------------
    # dataset pre-computation ten CPU (§2.1 / §2.2)
    # ---------------------------------------------------------------------------
    ...
    # all tensors stay ten CPU and are moved to GPU individually during the training
    # step -> the full operator cache does not live permanently in VRAM (avoids OOM
    # with many meshes / large k_eig ten 11 GB).

def precompute_dataset (samples ,meta ,op_cache_dir =None ,strict =False ):
    """Pre-compute operators + model input per sample ten CPU.
    ...
    for s in samples :
        try :
            ...
            if strict :
                part_id =s .get ("part_id","<unknown>")
                raise RuntimeError (
                f"operator build failed for part {part_id }: {exc }"
                )from exc 
            skipped +=1 
...
checkpoint_path =None ,op_cache_dir =None ):
    ...
    # otherwise depend ten per-mesh class mixture).
    ...
    # §2.1/§2.2: pre-compute operators (incl. HKS) ONCE ten CPU; move each
    ...
    strict_precompute =bool (cfg .get ("strict_precompute",False ))
    train =precompute_dataset (train_samples ,meta ,op_cache_dir ,strict =strict_precompute )
    val =precompute_dataset (val_samples ,meta ,op_cache_dir ,strict =strict_precompute )
    if not train or not val :
        raise ValueError (
        f"operator precomputation produced an empty split: train={len (train )}, val={len (val )}"
        )
    ...
    for ep in range (epochs ):
        ...
        if ep %eval_every ==0 or ep ==epochs -1 :
            ...
            if rep ["mean_iou"]>best ["mean_iou"]:
                ...
                # Persist every new best immediately so an interrupted long run
                # does not lose the strongest validation-selected checkpoint.
...
def refit_diffusionnet (config ,samples ,epochs ,device ="cpu",weights =None ,
report =None ,checkpoint_path =None ,op_cache_dir =None ):
    """Fixed-epoch refit without validation or model selection.

    Use this only after hyperparameters and the epoch count have been frozen ten
    a separate train/validation run.  It exists so the final model can learn
    from the full development pool without peeking at the locked test or using
    an already-trained validation split to select a checkpoint.
    """
    import random 
    torch =_require ("torch","pip install torch")

    if int (epochs )<1 :
        raise ValueError ("fixed refit epochs must be >= 1")
    cfg ={**DEFAULTS ,**config }
    model ,meta =build_diffusionnet (cfg )
    model =model .to (device )
    w =torch .tensor (weights if weights is not None else LOSS_WEIGHTS_BY_ID ,
    ...
    strict_precompute =bool (cfg .get ("strict_precompute",False ))
    logger .info (
    "DiffusionNet fixed refit: precomputing operators (%d development parts, k_eig=%d) ...",
    len (samples ),meta ["k_eig"],
    )
    prepared =precompute_dataset (
    samples ,meta ,op_cache_dir ,strict =strict_precompute 
    )
    if not prepared :
        raise ValueError ("operator precomputation produced an empty refit dataset")
    ...
    for epoch in range (int (epochs )):
        ...
        for number ,index in enumerate (order ,1 ):
            ops ,x_in ,labels =_to_device (prepared [index ],device )
            output =_forward (model ,ops ,x_in )
            unscaled_loss =_compute_loss (output ,labels ,w ,meta ,cfg )
            loss =unscaled_loss /accum 
            loss .backward ()
            if number %accum ==0 or number ==len (order ):
                opt .step ()
                opt .zero_grad (set_to_none =True )
            losses .append (float (unscaled_loss .detach ().cpu ()))
            del ops ,x_in ,labels ,output ,loss ,unscaled_loss 
        sched .step ()
        last_mean_loss =float (np .mean (losses ))
        epoch_report ={
        "epoch":epoch ,
        ...
        if report is not None :
            report (epoch_report )
        logger .info (
        "fixed refit epoch %3d/%d train_loss=%.6f",
        epoch +1 ,int (epochs ),last_mean_loss ,
        )

    if checkpoint_path is not None :
        save_checkpoint (model ,cfg ,meta ,checkpoint_path )
        logger .info ("fixed-epoch refit model saved: %s",checkpoint_path )
    return {
    "epochs":int (epochs ),
    "samples":len (prepared ),
    "last_train_loss":last_mean_loss ,
...
def save_checkpoint (model ,config ,meta ,path ):
    ...
    can run inference directly ten it.
    """
...
def load_checkpoint (path ,device ="cpu",n_classes =N_CLASSES ):
    ...
    Returns (model, meta, config). model is ten `device` and in eval mode.
    """
    torch =_require ("torch","pip install torch")
...
def free_gpu_memory ():
    ...
    No-op when torch / CUDA are unavailable, so it is safe to call ten CPU-only
