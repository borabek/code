# -*- coding: utf-8 -*-
"""GECE ISI: ZENGIN TEMSIL cikarimi (tum korpus, urun-sadik yol).
GEREKCE (bu oturumda olculdu): 13 el-yapimi feature TEMSIL DARBOGAZI. Sadece 'parcada nerede'
eklemek WEI havuzunda top-N F1 +0.041 (parca-out) / +0.031 (AILE-out, genellenir) getirdi.
Bu script ayni urun yolunu (4-model union + avg probs) kosar ama her aday icin ZENGIN feature dumper:
  A konum (mesh-bbox normalize, 3) + bbox-yuz uzakliklari (6)
  B cok-yaricap sinif-olasilik profili (r=3,6,10,15 x 5 sinif = 20) + yogunluk (4)
  C TAPER profili (derinlik 0..8mm'de agiz yaricapi, 5) -- mesh-tabanli huni testi (B-rep koni olmustu)
  D normal-degisim / egrilik proxy (3 yaricap)
...
DD = 10.0
# post-isleme override (ortam degiskeni) -- tavan/F1 deneyleri icin
...
def rich_feats(V, F, probs, cps, Nrm):
    ...
    for c in cps:
        ...
        f += list((p - lo) / diag) + list((hi - p) / diag)                # A2 yuz uzakliklari (6)
        rel = V - p; dist = np.linalg.norm(rel, axis=1)
        for r in RAD:                                                     # B cok-yaricap (20+4)
            m = dist <= r
            if m.any():
...
def main():
    ...
    # FAZ2/P0: TEMIZ set = sizinti-muhafizi ACIK (segmentasyon egitiminde gorulmemis parcalar).
    # Once temizi al, sonra BA_ALLOW_SEEN ile tamamini al ve FARKI 'seen' olarak isaretle ->
    # tek kosuda hem temiz hem canonical-kiyaslanabilir sayi uretilebilir.
    ...
    if "--list" in sys.argv:                       # acik parca listesi (ek gate-egitim havuzu icin)
    ...
    print(f"{len(parts)} parca ({sum(1 for m,*_ in parts if m=='WEI')} WEI + {sum(1 for m,*_ in parts if m=='PXC')} PXC)"
    ...
    if os.path.exists(OUT_NPZ):
        try:
            ...
            print(f"  [devam] {len(_done)} parca zaten islenmis, atlanacak", flush=True)
        except Exception:
            _done = set()
    ...
    for mfg, pid, jf, stp in parts:
        ...
        # (2026-07-28: bir cikarim 3 saat sessizce durdu ve suclu parcayi bulmak ayri bir sorusturma
        # oldu; log yalnizca her 25 parcada bir yaziyordu ve parca ADINI hic yazmiyordu.)
        try:
    ...
    np.savez(OUT_NPZ, X13=np.vstack(X13), XR=np.vstack(XR), y=np.concatenate(YY),
             ...
             part_ids=np.array(PIDS), seen=np.array(SEEN))
    print(f"-> {OUT_NPZ}  {len(np.concatenate(YY))} aday, {len(NGT)} parca "
