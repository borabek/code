# -*- coding: utf-8 -*-
"""F0-4: HATA BANKASI -- her FP'ye FIZIKSEL etiket, her FN'e SEBEP etiketi.

NEDEN: F3 kollari ve F1-8 "hangi hatayi olduruyorum" sorusunu yanitlayamadan kosuluyordu.
[[ablasyon-cephe-kararliligi]] ve F0-5 hedef aritmetigi bu bankayi girdi alir.

FN SEBEP ETIKETI -- uc kova, ONBELLEKTEN kesin ayrilabiliyor:
  ADAY_YOK    : GT'nin toleransinda HIC aday yok            -> TURETME/COZUNURLUK sorunu
  GATE_REDDI  : aday vardi ama karar kurali reddetti        -> GATE sorunu
  ATAMA       : aday vardi, kabul edildi, ama BASKA GT'ye atandi -> METRIK/coklu esleme

Bu ayrim onemli cunku uc kova UC AYRI kola bakar: ADAY_YOK -> F4 (temsil), GATE_REDDI ->
C/E kollari, ATAMA -> metrik. Havuzlanmis "366 FN" sayisi hangisine bakacagini soylemiyor.

FP FIZIKSEL ETIKETI: A4 bayraklari (govde ici / onu kapali / agiz ic capi / ileri mesafe).
[[fizik-bayes-tavanini-oynatmiyor]]: bunlar TAVANI oynatmiyor ama HATA TARIFI icin gecerli --
"tavani oynatmiyor" ile "hatayi tarif etmiyor" ayni sey degil.
...
def esle(P, G, Gd, tol):
    """Metrikle AYNI tekil eslesme. Doner: (aday->gt sozlugu, eslesmeyen gt listesi)."""
...
def main():
    ...
    for r in DER:
        ...
        Ptum = np.asarray(r["P"], float)             # TUM adaylar (kapi ONCESI)
        P = Ptum[k]                                   # kabul edilenler
        G = np.asarray(r["G"], float); Gd = np.asarray(r["Gd"], float)
        if not len(G):
            continue
        tol = max(3.0, 0.06 * float(r["diag"]))
        rejim = "dusuk-CP" if len(G) < 4 else "yuksek-CP"
        ortak = {"pid": r["pid"], "mfg": r["mfg"], "geo": r["geo"],
                 "rejim": rejim, "n_gt": int(len(G))}
        ...
        # FN SEBEBI: kapi ONCESI havuzda aday var miydi?
        #
        # IKI FARKLI SORU, ONCEKI SURUM KARISTIRIYORDU (2026-08-04'te yakalandi):
        #   (1) TEKIL eslemede bu GT'ye aday dustu mu?   -> urunun gercekten alabilecegi
        #   (2) Toleransinda HERHANGI bir aday var mi?    -> havuzun ham kapsamasi
        # Hafizadaki "aday havuzu GT'nin %93.7'sini iceriyor" (2) ile olculmus; benim ilk
        # surumum (1) ile olcup %80.7 buldu ve celiski gibi gorundu. Ikisi FARKLI SEY:
        # yogun parcada birkac GT ayni adayi paylasir, tekil esleme birini disarida birakir.
        # Ayrimi korumak SART, cunku farkli kola bakiyorlar:
        #   ADAY_YOK    -> gercek uretim/cozunurluk kaybi   -> A/F4 (temsil)
        #   ADAY_KALABALIK -> aday var ama PAYLASILDI       -> cozunurluk/ayirma (F1-7)
        ...
        if len(Ptum) and len(G):
            ...
            yakin_var = (pp.min(0) <= tol)       # GT basina: toleransta HERHANGI aday
        else:
        ...
        for b in fn_idx:
            ...
            if b not in gt_tum:
                ...
                kayit["sebep"] = "GATE_REDDI"        # aday vardi, kapi reddetti
            else:
                kayit["sebep"] = "ATAMA"             # kabul edildi ama baska GT'ye gitti
            FN.append(kayit)
        ortak["_havuz_kapsama"] = float(yakin_var.mean()) if len(G) else 1.0
        KAPSAMA.append((len(G), float(yakin_var.sum())))

    print(f"TP {len(TP)} | FP {len(FP)} | FN {len(FN)}")
    print(f"\n{'rejim':<12}{'TP':>7}{'FP':>7}{'FN':>7}{'kesinlik':>11}{'duyarlilik':>12}")
    for rj in ("dusuk-CP", "yuksek-CP"):
        t = sum(1 for x in TP if x["rejim"] == rj)
        f_ = sum(1 for x in FP if x["rejim"] == rj)
        n = sum(1 for x in FN if x["rejim"] == rj)
        print(f"{rj:<12}{t:>7}{f_:>7}{n:>7}{t/max(t+f_,1):>11.4f}{t/max(t+n,1):>12.4f}")

    print("\nFN SEBEP DAGILIMI (hangi kol bakacak):")
    KOL = {"ADAY_YOK": "F4/A -- TEMSIL (havuzda HIC yok)",
           "ADAY_KALABALIK": "F1-7 -- COZUNURLUK (aday var, PAYLASILDI)",
           "GATE_REDDI": "C/E -- GATE (bilgi var, karar yanlis)",
           "ATAMA": "METRIK -- coklu esleme"}
    for rj in ("dusuk-CP", "yuksek-CP"):
        c = collections.Counter(x["sebep"] for x in FN if x["rejim"] == rj)
        tp = sum(c.values())
        print(f"  {rj} ({tp} FN):")
    ...
    print("\nHAVUZ KAPSAMASI (GT'nin toleransinda HERHANGI aday var mi):")
