...
KAHIN OLCUMU (temiz sinav): dogru (konum, eksen) cifti B-rep adaylari arasindan
secilebilseydi robot 0.2060 -> **0.3880**. Yani bilgi VAR, secim YOK.

DUSEN BASIT KURALLAR (ikisi de olculdu, tekrar acilmayacak):
  * yakinlik-oncelikli oturtma      -> gorulmemiste NET -30 (`r13`)
  * parca-ici eksen UZLASISI        -> 0.2089 -> 0.1773 (`p3b`)
Ikisi de tek bir sinyale guveniyordu. Secici BIRDEN COK zayif sinyali birlestirir.

ADAY UZAYI: her aday icin {MEVCUT (oldugu gibi kal)} + agzi yakin her uygun
silindirin (agiz, +eksen) ve (agiz, -eksen) secenekleri.

OZNITELIKLER URETICI KIMLIGI ICERMEZ -- hepsi geometrik/istatistikseldir, yoksa
[[gate-memorizes-not-learns]] tuzagina duseriz.

EGITIM/OLCUM AYRIMI: secici YALNIZ egitim korpusu ureticilerinde (TOGI/PXC/WEI/SIE/
TE...) egitilir; sinavin 8 ureticisi orada YOKTUR. Esik DEV yarisinda secilir,
SINAV yarisinda TEK ATIS olculur.

TEZ DEGISMEZ: `v_o` turetmesi, 5 sinif ve ~6000 remesh aynen kalir. Bu bir SON ISLEM
...
MODEL = "results/p3c_eksen_secici.pkl"
MAKBUZ = "results/p3c_eksen_secici.json"
DEV_MFG = {"SUPU", "NIT", "S+S", "SE"}
ROBOT_YANAL, ROBOT_ACI = 2.0, 10.0
MM_MAX = 8.0            # aday uzayina alinacak en buyuk agiz mesafesi
OZ_AD = ["aci_mevcut", "mesafe", "mesafe_norm", "yaricap", "uzunluk", "mevcut_mu",
         "paralel_sayi", "paralel_pay", "eksen_hiza", "komsu_uyum", "gate_skoru",
         "n_aday", "n_silindir",
         ]

# --- K1.7: YEREL goreli yaricap (OPT-IN, VARSAYILAN KAPALI) ---------------
# Fikir: ham "yaricap" ureticiler arasi kiyaslanamaz (tel girisleri 0.1-3.8mm
# arasi her yerde), ayirt edici olan adayin komsulugundaki DIGER silindirlere
# gore buyuklugu. Sonda destekliyordu: dogru silindir komsulugun en buyugu
# %54.2, ilk ikisinde %87.8 (D6, 640 eslesme).
#
# AMA TEMIZ A/B NULL CIKTI (ayni 2584 parca, ayni olcum, tek degisken ozellik):
#     13 ozellik -> DEV 0.3035 / SINAV 0.2676
#     17 ozellik -> DEV 0.3122 / SINAV 0.2621
# Ayar kumesinde +0.0087, AYRIK kumede -0.0055 = klasik sahte kazanc deseni.
# DAGITILMADI. Kod duruyor cunku LOMO secimi (SIRA-7) ya da farkli bir
# birlestirmeyle yeniden denenebilir; ama urun yolunun bedava hesap yapmamasi
# icin varsayilan KAPALI.
...
def _uyumlu(sec, X):
    ...
    K1.7 ile OZ_AD 13 -> 17'ye cikti. Eski ckpt'ler (p3c_eksen_secici.pkl) 13
    bekliyor. Genislik sessizce uyusmazsa sklearn ya patlar ya da -- daha kotusu --
    baska bir yerde sessizce yanlis skor uretir. Yeni ozellikler LISTENIN SONUNA
    eklendigi icin ilk n sutun eski sirayla ayni; bastan kesmek GUVENLI.
    Genisletme YAPILMAZ: model daha genis bekliyorsa bu bir hatadir, patlasin.
    """
...
def _yerel_yaricaplar(uy, p):
    """Adayin MM_MAX komsulugunda agiz veren silindirlerin yaricaplari."""
    out = []
    for c in uy:
...
def secenekler(cyls, p, d, diag, gate_s, komsu, n_aday):
    """Bu aday icin (konum, eksen, oznitelik) listesi. Ilk oge HER ZAMAN MEVCUT."""
    import brep_snap
...
def parca_adaylari(r, model, ob, oran=0.40, taban=0.30):
    """Gate'ten gecen adaylar + parca duzeyi baglam."""
    import wire_gate
    from p1c_esik import maske
    M = d6_kayit.x58(r)
    if M is None or r.get("P") is None or not len(r["P"]) or M.shape[1] * 2 != model["n_feat"]:
        return None
    s = np.asarray(wire_gate.karar_skoru(model, M), float)
    k = maske(s, oran, taban)
    if not k.any():
        return None
...
def veri_kur(kayit, model, ob, esle_detay):
    """(oznitelik, etiket) ciftleri. Etiket: bu secenek adayi ROBOT-HAZIR yapar mi."""
    X, y, grp = [], [], []
    for pid, r in kayit.items():
        ...
        if not len(G):
            continue
        _t, _f, _n, bi = esle_detay(P, D, G, Gd, r["diag"], 0.0, 180.0, True)
        cy = ob.get(pid) or []
...
def uygula(kayit, model, ob, sec, esik, esle_detay, f1w, mfgler=None, sayac=None):
    T, R = [], []
    for pid, r in kayit.items():
        ...
        if pak is not None:
            ...
            if sec is not None:
                ...
                for i in range(len(P)):
                    ...
                    j = int(np.argmax(sk))
                    # MEVCUT'u yalnizca secenek BELIRGIN sekilde onden ise birak
                    if j != 0 and sk[j] >= sk[0] + esik:
...
def main():
    ...
    ap.add_argument("--egitim-parca", type=int, default=900)
    ...
    # --- EGITIM: yalniz egitim korpusu ureticileri (sinavin 8'i BURADA YOK)
    # KORPUS ORTAM DEGISKENI (2026-08-08 / A5): g10 ile yeniden turetilen korpus
    # (`zengin_parite_v4_g10.npz`) ile fit edilebilsin. Sabit kalsaydi p3c ESKI
    # dagilimda kalir ve gate v7 ile UYUMSUZ olurdu ("iki gate ayni dagilimda
    # egitilmeli" dersinin p3c karsiligi).
    _kk = os.environ.get("P3C_KORPUS", "results/zengin_parite_v3.npz")
    print(f"korpus: {_kk}", flush=True)
    d = np.load(_kk, allow_pickle=True)
    egt_pid = sorted(set(map(str, d["pids"])))
    ek = d6_kayit.yukle(set(egt_pid))
    # uretici-dengeli ornekle
    ...
    print(f"EGITIM: {len(ek)} parca | uretici "
          f"{dict(collections.Counter(r['mfg'] for r in ek.values()))}")
    ob_e = silindir_onbellek(list(ek), SILINDIR_EGT)
    X, y, grp = veri_kur(ek, gate, ob_e, esle_detay)
    print(f"  egitim cifti: {len(y)} | pozitif %{100*y.mean():.1f} | "
          f"parca {len(set(grp))}")
    ...
    # --- OLCUM: temiz sinav, DEV yarisinda esik secimi
    sv = d6_kayit.sinav()
    ...
    d0t, d0r = uygula(dev, gate, ob_s, None, 0, esle_detay, f1w)
    print(f"\nDEV secicisiz: tespit {d0t:.4f} robot {d0r:.4f}")
    en, en_r, izgara = None, d0r, {}
    for esik in (0.00, 0.05, 0.10, 0.20, 0.30):
        tf, rf = uygula(dev, gate, ob_s, sec, esik, esle_detay, f1w)
        izgara[str(esik)] = {"tespit": tf, "robot": rf}
        print(f"  esik {esik:.2f}: tespit {tf:.4f} robot {rf:.4f}")
        if rf > en_r:
            en_r, en = rf, esik
    if en is None:
    ...
    s0t, s0r = uygula(sin, gate, ob_s, None, 0, esle_detay, f1w)
    s1t, s1r = uygula(sin, gate, ob_s, sec, en, esle_detay, f1w, sayac=sc)
    print(f"\n--- SINAV YARISI (TEK ATIS) ---")
    print(f"{'ayar':<20}{'TESPIT':>9}{'ROBOT':>9}")
    print(f"{'secicisiz':<20}{s0t:>9.4f}{s0r:>9.4f}")
    print(f"{f'secici esik {en:.2f}':<20}{s1t:>9.4f}{s1r:>9.4f}")
    print(f"{'FARK':<20}{s1t-s0t:>+9.4f}{s1r-s0r:>+9.4f}")
    print(f"  degistirilen aday: {sc['degisen']}/{sc['aday']}")
    ...
    with io.open(MAKBUZ, "w", encoding="utf-8") as f:
        json.dump({"egitim_parca": len(ek), "egitim_cifti": int(len(y)),
                   ...
                   "degisen": sc["degisen"], "aday": sc["aday"], "karar": karar},
