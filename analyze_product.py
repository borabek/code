...
Hepsi OOF (sizinti yok). Kullanici P0.5/P1.3/P1.4 maddeleri.
"""
...
for name, m in (("WEI", mfg == 1), ("PXC", mfg == 0)):
    ...
    print(f"  {name}: en iyi esik {best[0]:.2f} -> P{best[1]:.3f} R{best[2]:.3f} F1 {best[3]:.3f}")
print(f"  (mevcut ortak esik 0.35; per-mfg ayirmak F1 artirir mi -> yukaridaki en-iyiler)")

# --- P1.4 CP-count prior (metadata-assisted) ---
print("\n=== P1.4 CP-COUNT PRIOR (uretici CP sayisi kadar en-guvenli CP tut) ===")
def count_prior(mask):
    ...
    for g in np.unique(grp[mask]):
        ...
        n = ngt_of[g]
        keep = idx[np.argsort(-oof[idx])[:max(n, 0)]]   # en yuksek wire_score N tanesi
...
tool_fp = int(((y == 0) & dropped).sum())              # gate dogru eledi (dusuk wire_score, mfg-eslesmez)
unlisted_or_resid = int(((y == 0) & kept).sum())       # kept ama mfg-eslesmez = listelenmemis tel VEYA artik tool
kept_tp = int(((y == 1) & kept).sum())
dropped_wire = int(((y == 1) & dropped).sum())         # gate yanlislikla gercek teli attigi (mfg-eslesen)
n_gt = sum(ngt_of.values())
missed_fn = n_gt - kept_tp                              # hic yakalanmayan uretici CP (dropped_wire dahil)
...
print(f"  uretici CP toplam: {n_gt}")
...
print(f"  -> gate'in NET etkisi: {tool_fp} tool temizledi, {dropped_wire} gercek tel kaybetti (oran {tool_fp}/{dropped_wire})")
