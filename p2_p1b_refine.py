...
P1 bulgusu: GO kriteri MLP'de gecti AMA en iyi mutlak sayi 0.7560 -> 0.7596 (+0.004) kaldi;
cunku RF zaten zengin feature'lardan bilgiyi cikariyor, 188 embedding kolonu RF'yi seyreltiyor.
Iki mesru rafinasyon:
  (a) EMBEDDING SECIMI: train-fold'da onem/varyansla top-K embedding kolonu (sizintisiz secim)
  (b) ENSEMBLE: rf(rich) + mlp(rich+emb) -- farkli hata yapan modelleri birlestir
Karar split'i: family-out. Hedef: en iyi MUTLAK family-out ALL."""
import json
import numpy as np
...
print(f"WORK {int(WORK.sum())} aday / {len(set(gpid[WORK]))} parca | rich {RICH.shape[1]}d emb {EM.shape[1]}d")
...
def oof(build, Xa, groups, topk=0):
    """topk>0: embedding kolonlarini SADECE train-fold'da secer (sizintisiz)."""
...
print("\n--- ENSEMBLE (rank-ortalama, farkli hata yapan modeller) ---")
def rank01(s):
