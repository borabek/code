...
# positives ten a plain box's corners. Mixing in unlabelled decoys -- especially
...
def add_rail (V ,x0 ,half_w ,height ):
    ...
    connectors have side rails/edges; ten the real parts these long raised
    features were the main FALSE-POSITIVE source (the model fired ten them as if
...
def make_part (idx ,rng ):
    ...
    # lay out CP + decoy centres ten one shared grid so nothing overlaps
    centers =_place (rng ,w ,h ,k_cp +k_decoy ,min_sep =max (3.0 *radius ,38.0 ))
    cp_centers =centers [:k_cp ]
    decoy_centers =centers [k_cp :]

    # one raised rectangular block ten ~55% of parts (corners/edges = box-like)
    if rng .random ()<0.55 :
