# OTOPSİ: Robot why tool/actuator ağzını CP sayıyor? (2026-07-24)
...
> base F1 0.693→**0.750**. Kanon: `results/product_f1_receipt.json`. Aşağıdaki kök-why analizi geçerli.

## Belirti (kullanıcı gözüyle yakalandı)
Robot a push-in/kelepçe klemenste "each açıklığı / each boşluğu" CP işaretliyor. 6 CP dediği yerde
belki 2-4 gerçek **tel girişi** present; gerisi **tornavida/mandal (Werkzeugeinschub) ağzı** veya montaj
boşluğu — telin GİRMEDİĞİ delikler. Sayı şişiyor, precision düşüyor.

## Kök why zinciri (kanıtla)

### L1 — TANIM düzeyi (tez, satır 1303) ★ asıl source
...
positionen ... sowie ... **Werkzeuge**" diyor: tez tool ağzını bilerek a özellik sayıyor, but
**Contact sınıfı as, Kabeleinführung değil.** Tool ağzı with kontakt, tez tasarımı gereği ayrılmamış.
...
**PXC push-in/kelepçe tel-girişleri Contact etiketli** (ayrı a CableEntry none). Yani sınıf düzeyinde
**tel-girişi with tool ağzı AYNI sınıf (Contact)** → segmentasyon ikisini ayıramaz.
...
(Contact'lar tel-girişlerinden ~%50 excess). Contact→en yakın CableEntry medyanı **17mm** (= ağız→kontakt
axis mesafesi; aynı tel-yolu). Fazladan Contact'ların a kısmı tool ağzı. (Uyarı: corpus etiketlemesi
tutarsız — bazı parçalarda CableEntry hiç işaretlenmemiş; this ölçüm yön verir, tek başına kanıt değil.)

### L4 — TÜRETME (cp_openings)
cp-v3 = CableEntry VEYA depth-kapılı Contact. Her depth-kapılı Contact bölgesinden a CP üretir
→ **tool ağzını da CP üretir.** ct_depth_min_mm=1.0 kapısı düz pad'i eler but tool ağzının da derinliği
present → geçer.

## Neden basit a bug değil
...
şart: geometri (boyut/depth) this ikisini ayırıyor mu?

## DECISION DENEYİ (recon_wire_vs_tool.py — KOŞUYOR)
Üretici CP'sini ayraç-etiketi al: robot-CP'si üretici CP'ye eşleşiyorsa TEL, değilse TOOL/excess.
İki grubun size_mm/depth_mm/votes/confidence AUC-ayrılabilirliğini ölç.
- AUC ~0.5 → ayırt edilemez → **alt-sınıf etiketi + yeniden eğitim** gerekir (tool'u Contact'defn ayır)
- AUC >0.75 → **geometrik kapı** genel fix (cp_openings'e, yeniden eğitim YOK, tüm modeller kazanır)

## DECISION DENEYİ SONUCU (2026-07-24, recon_wire_vs_tool.py, 770 robot-CP: 465 tel / 305 tool-excess)
| özellik | tel medyan | tool medyan | AUC |
|---|---|---|---|
| size_mm | 12.84 | 11.26 | **0.61** (zayıf) |
| depth_mm | 3.71 | 3.48 | 0.52 (none) |
| votes | 3.00 | 2.00 | 0.50 (none) |
| confidence | 0.84 | 0.82 | 0.57 (zayıf) |
**GEOMETRİ AYIRMIYOR → DAL A ÖLÜ.** Ayrım fonksiyonel, yerel geometriden gelmiyor.
**BONUS FINDING (önemli):** this, önceki "precision ~0.98 / FP'lerin %96'sı gerçek açıklık" hakemliğinin
YANLIŞ soruyu sorduğunu gösteriyor — "gerçek açıklık mı?" (tool=evet) yerine "tel giriyor mu?" (tool=hayır)
sorulmalıydı. Tool ağızları "doğru" sayılmış → robot-etkin ~0.80 sayısı ŞİŞİK. Gerçek wire-precision more düşük.
NOT: 305 "tool" grubu saf tool değil — üretici listelemediği gerçek telleri de içerir (missing-GT). Ama
sonuç değişmez: this iki grup GEOMETRİK as ayrılamıyor.

## ⚔️ ASKERİ PLAN (karar deneyi ÖLÜ Dal A -> Dal B'ye geçildi)
Amaç: 2-3 modeli değil, **kodun (cp_openings + tanım) genelini** this sorunu aşacak hale getirmek.
...
2. **DAL A (AUC yüksek):** cp_openings'e **wire-vs-tool geometrik kapısı** ekle (boyut/depth/eşlik
   eşiği). Yeniden eğitim none → each modele uygulanır. Hakemde precision↑ / tel-recall fixed doğrula.
3. **DAL B (AUC düşük):** sınıfı **un-merge et** — a parça setinde tool ağzını ayrı işaretle (insan),
   6. sınıf/alt-baş as öğret; VEYA **eşleme sezgiseli**: kutup başına tel+tool çifti detection et,
...
- DEĞİŞİR: cp_openings türetmesi (+ belki 6. sınıf etiketi/eğitim), cp_config CP tanımı, robot çıktısı
  (CP'ye "kind: wire/tool" tipi eklenebilir — tez tool'u da istiyor, but robot yalnız wire'a tel sokar).
...
- P1.1 recall labeling: en very FN veren WEI/PXC ailelerinden 40-80 part (very-CP/push-in). Recall tavani for.
- P1.2 wire/tool alt-sinif (6. sinif veya aux head): Contact'i wire_contact vs tool_actuator ayir. Precision sicramasi.
  (wire-gate simdilik post-hoc bunu yapiyor; egitimde ayirmak more guclu but insan etiketi ister.)
- P2.1 Manifold remesh (current product + OOF with); P2.2 YOLOv6 2D arm (ciddi proje, klasik CV hack YOK).
