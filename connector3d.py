...
# standard union-find, used twice: before for fragment detection
# and before to merge fragments into instances
class UnionFind :
...
class ConnectionPoint :
    ...
    smooth_subdiv :int =0 ,body_center =None )->np .ndarray :
        ...
        self .insertion_depth_axis_mm ,self .normal_stability )=feature_direction (idx ,vertices ,faces ,self .normal ,smooth_subdiv =smooth_subdiv )
    ...
    def robotic_validation (self ,cfg =None ,max_skew_deg =75.0 ):
        """Decide whether this point is safe for the robot to act ten.
...
class WireLink :
    def __init__ (self ,src ,dst ,distance ,score ):
        ...
        #
        # IMPORTANT: this is NOT electrical wiring. we only provide candidates based ten
...
def build_wiring (instances ,k =3 ,see_tol =-0.1 ,cfg =None ):
    ...
    if cfg is not None :
        ...
        # only wire points the robot is actually allowed to act ten
