# -*- coding: utf-8 -*-
"""P5: GATE VE POZ BIRLIKTE SECILIR -- mevcut `gate -> poz` sirasi yanlis olabilir.

HIPOTEZ: bugunku boru hatti once gate'i uygular, sonra hayatta kalanlarin pozunu duzeltir.
Dogru poza sahip bir aday gate'te ERKENDEN elenirse o CP bir daha geri gelmez. Oysa gate
adayi LOOSE tespit etiketiyle (yanal <= max(3mm,%6), aci serbest) egitilmis; robot-hazir
olup olmayacagini hic sormamis.

BU KOL: her (aday x poz) ciftini DOGRUDAN **robot-hazir** etiketiyle skorlar
(yanal <= 2mm VE isaretli aci <= 10). Yani secici, "bu aday gecer mi" yerine
"bu aday BU POZLA robotun kullanabilecegi bir CP verir mi" sorusunu ogrenir.

DISIPLIN:
  * HAM havuz kullanilir (gate ONCESI) -- kolun butun anlami bu
  * uretici kimligi OZNITELIK DEGIL
  * URETICI-DISI dis capraz gecerleme; esik IC katmanda secilir
  * CAKISMA COZUMU: bir aday yalnizca BIR kez kullanilir (Macar atama)
  * guven dusukse MEVCUT poz korunur (geri donus)

TEZ DEGISMEZ: `v_o` turetmesi, 5 sinif, ~6000 remesh aynen. Bu bir SECIM katmanidir.
"""
import argparse
import collections
import glob
...
def poz_secenekleri(cyl, ac, p, d, diag):
    """MEVCUT + silindir agizlari + duzlemsel aciklik merkezleri (+- yon)."""
    import brep_snap
...
def parca_ciftleri(r, cyl, ac):
    """HAM havuzun tum (aday, poz) ciftleri + ozellikleri."""
    X58 = d6_kayit.x58(r)
...
def etiketle(meta, G, Gd):
    """Robot-hazir mi: yanal <= 2mm VE isaretli aci <= 10."""
...
def sec_ve_uygula(Z, meta, clf, esik, P0, D0):
    """Skorla, aday basina EN IYI pozu al, esigin altini MEVCUT poza dondur."""
...
def main():
    ...
    ap.add_argument("--egitim-parca", type=int, default=600)
    a = ap.parse_args()
    import protokol
    ...
    # EGITIM: egitim korpusu ureticileri
    ...
    print(f"EGITIM: {len(sec_pid)} parca")
    X, y, mf = [], [], []
    for pid in sec_pid:
        r = ek[pid]
    ...
    def olc(alt, esik):
        ...
        for pid, r in kayit.items():
            ...
            if out is not None:
                Z, meta = out
                P, D = sec_ve_uygula(Z, meta, clf, esik, None, None)
    ...
    sin_mfg = tum - DEV_MFG
    print(f"\n{'esik':<8}{'DEV tespit':>12}{'DEV robot':>11}")
    ...
    print(f"\n--- SINAV YARISI (TEK ATIS, esik {en:.2f}) ---")
    ...
    with io.open(MAKBUZ, "w", encoding="utf-8") as f:
        json.dump({"izgara": izg, "dev_esik": en, "sinav": {"tespit": st, "robot": sr},
