...
TESHIS: tezin `Contact` sinifi tasarimi geregi "Kontaktierung *bzw.* Werkzeugeinschub" --
tel girisi ve alet agzi TEK SINIF. Yani backbone, bizim ayirmak istedigimiz iki seyi
BIRLESTIRMEK uzere egitildi. Sonra asagidaki gate'ten o ayrimi geri kazanmasini istiyoruz.
Dokuz bagimsiz mekanizmanin ayni cepheye carpmasinin koku budur.

BU BETIK yardimci supervizyonun ETIKETINI uretir:
    bir aciklik URETICI CP'siyle eslesiyorsa      -> TEL   (0)
    eslesmiyorsa                                  -> ALET  (1)
    aciklik disindaki her tepe                    -> MASKELI (-1, sinyal yok)

TEZ IHLALI YOK: bu etiket ana 5-sinif etiketini DEGISTIRMEZ, ayri bir dosyaya yazilir ve
ayri bir kafayi besler. Tezin sinif tanimi, softmax'i ve kaybi aynen kalir.

ETIKET GURULTUSU (bilinerek kabul edilir): "eslesmeyen = alet" varsayimi, ureticinin
listelemedigi gercek bir tel girisini alet diye ogretebilir. Bu risk OLCULDU ve kucuk:
kutup-orgusu testi FP'lerin yalniz %0.4'unun unutulmus kutup oldugunu gosterdi
(listelenmis CP'lerde %70.1) -- yani eslesmeyen aciklklar gercekten tel girisi DEGIL.
"""
import io
import json
import os
...
YARICAP = 5.0          # mm, acikligin tepe bolgesi
...
def main():
    ...
    print(f"{len(hedef)} etiketli parca | uretici JSON'u olan: "
    ...
    for k, (d, pid, yol) in enumerate(hedef, 1):
        ...
        try:
            ...
            cps, _, _, _ = RC.adaylari_uret(V, F, pbs, stp, cfg=cfg)
            if not cps:
                continue
            P = np.array([c["point"] for c in cps], float)
            Pd = np.array([c["direction"] for c in cps], float)
            # --- uretici GT'sini BU mesh'in cercevesine tasi
    ...
    print(f"\n{ok} parca icin yardimci etiket yazildi")
    print(f"  TEL tepe {tel_t} | ALET tepe {alet_t} | oran {alet_t/max(tel_t,1):.2f}")
    with io.open("results/fb2_aux_etiket.json", "w", encoding="utf-8") as f:
        json.dump({"parca": ok, "tel_tepe": tel_t, "alet_tepe": alet_t,
                   "yaricap_mm": YARICAP, "dizinler": DIZINLER}, f, indent=1)
    print("makbuz -> results/fb2_aux_etiket.json")
