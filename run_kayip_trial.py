...
NIT'te 0.050 -- i.e. NIT'te correct option yanlistan only 0.05 more
...
HGB'de dogrudan focal absent; `sample_weight` with YAKLASILIR:
  w_i = (1-p_i)^gamma   -- p_i a ON GECISTEN gelen probability
...
def main ():
    ...
    for b in katlar :
        ...
        # Dogrusu: 2 katli ic split with KAT-DISI probability.
        def oof_p (Mx ,Yx ):
            pr =np .zeros (len (Yx ))
        ...
        # 4) AGIRLIK + FOCAL (same fold-disi probability, lower-sample on)
    ...
    for ad in KOLLAR :
        diff =last_ [ad ]-last_ ["baseline"]
        print (f"  {ad :<15}{last_ [ad ]:.4f}   {diff :+.4f}"
        +("  <- KAPI GECTI"if diff >=0.01 else ""))
    json .dump ({"damga":receipt_hash .damga (),"cluster":KUME ,"gamma":GAMMA ,
    "total":last_ ,"fold":kat_sonuc ,
