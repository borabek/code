# -*- coding: utf-8 -*-
"""Op 3: high-CP PXC aday genisletme. KILIT SORU: multi-res (6k+9k) + dusuk vertex_conf + cluster_mm=0
aday havuzunun CANDIDATE RECALL'ini (havuzdaki gercek-tel / uretici-CP) yukseltiyor mu?
Yukseltmezse top-N secimin sececek seyi yok. High-CP PXC parcalarda (>=11 CP) olc: 6k-standart vs
...
HICP = 11  # yuksek-CP esigi


def cand_recall(cps, V, Vr, Vj, G, Gd):
    """havuzdaki CP'lerin kaci uretici-CP'ye eslesiyor (candidate recall)."""
...
def main():
    ...
    print(f"high-CP PXC ({HICP}+): {len(hi)} parca", flush=True)
    ...
    for k, (pid, jf, stp, n) in enumerate(hi, 1):
        try:
            ...
            std = _vote2(per6)
            # 9k + dusuk vc + cluster0 (ekstra aday)
            ...
            # genisletilmis: 6k std + 9k ekstra (union, ayri frame ama ikisi de STEP frame -> Vr uzerinden)
            exp = _vote2(per6 + per9)   # ayni cluster mantigi
            s_tp, gt = cand_recall(std, V6, Vr, Vj, G, Gd)
            # exp icin 9k CP'ler V9 frame'inde, 6k V6 frame'inde -- ikisi de STEP frame (thesis_remesh recenter yok)
    ...
    print(f"\n=== high-CP PXC CANDIDATE RECALL ({len(hi)} parca) ===")
