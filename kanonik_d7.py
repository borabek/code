...
DUN NEDEN URETEMEDIM (2026-08-09): iki girdiyi yanlis kullanmisim.
  * kayitlar `_der_yeni_G7BIRLESIK.pkl` (ben d6_kayit.yukle ile baska shard'lar)
  * gate `wire_gate_v6.pkl` (ben v5)
Makbuzlardan (`audit_d7_pose_dictionary_oracle.json`) okundu.

Bu modul TEK giris noktasidir. Yeni bir yigin olculecekse BURADAN gecer.
"""
...
# YIGIN SECIMI (2026-08-10). Kanonik yigin YUKARIDAKI ucludur ve VARSAYILANDIR;
# manset o uclu ile uretilir. Ama bir baska segmentasyon agini AYNI olcum yoluyla
# sinamak icin ayri bir betik yazmak, olcum yolunun urunden ayrismasi demekti
# (bu projede iki kez oldu). Bunun yerine ayni harness cevre degiskeniyle
# yeniden hedeflenir; damga (`makbuz_hash`) hangi ucluyle olculdugunu tasir.
...
def mikro(rows):
    """MIKRO F1 (havuzlanmis TP/FP/FN) -- manset bu olcekte.

    DUN'KU HATA: `f1w` (rejim-agirlikli, dusuk 0.895 / cok 0.105) kullaniyordum;
    ayni veride 0.0838 veriyor, mikro ise 0.1956. Manset 0.2344 MIKRO olcekte.
    """
...
def urun_poz(r, gate, S, esik=(0.40, 0.30), tam_zincir=True, p3c=None,
             p3c_esik=0.0, cyl=None):
    """URUNUN cikti pozlari. Doner: (P, D).

    `p3c` verilirse EKSEN SECICISI de kosar -- manset yolunda kosuyordu ve
    bende yoktu (kalan farkin en guclu adayi).
    """
    ...
    k = maske(s, esik[0], esik[1])
    P, D = (P[k], D[k]) if k.any() else (P[:0], D[:0])
    # KALABALIK BASTIRMA -- urunun `wire_gate.apply` yolundakiyle AYNI fonksiyon.
    # Buraya konmazsa olcum yolu urunden ayrisir (2026-08-10'da tam bunu yakaladim).
    if len(P) > 1:
        _n = wire_gate.kalabalik_maskesi(P, s[k])
        P, D = P[_n], D[_n]
    if p3c is not None and len(P) and cyl is not None:
        import p3c_eksen_secici as P3
        cy = cyl.get(r["pid"])
        komsu = None
        if len(D) > 1:
