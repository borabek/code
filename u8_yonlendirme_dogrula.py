# -*- coding: utf-8 -*-
"""U8: yonlendirilmis kol (R0.1) DAGITILAN D'yi gercekten domine ediyor mu?

U7 tablosu (uctan uca):
    kol     WEI-disi  PXC-disi  tanidik  seri-ORT
    A ham     0.4832    0.7203   0.7410   +0.0000
    D         0.5702    0.6828   0.7390   -0.0075   <- SU AN DAGITIK
    R0.1      0.5682    0.7029   0.7439   -0.0029

R0.1, D'yi UC eksende geciyor ve dorduncude (WEI) -0.0020 geride. Ama bu sayilar tek
orneklemden; "domine ediyor" demeden once ESLESTIRILMIS BOOTSTRAP ile bakilmali -- ozellikle
WEI'deki -0.0020 gercekten ihmal edilebilir mi, ve PXC'deki +0.0201 gercek mi.

Bu betik A/D/R0.1'i parca duzeyinde saklar ve uc karsilastirmayi da GA ile verir.
"""
import collections
import json
import os
...
def main():
    ...
    def olc(alt, keep):
        ...
        for r in alt:
            ...
            cok = (sa is not None) and (float(sa.max()) < esik)
            yon += int(cok)
            for ad, s in (("A", sa), ("D", sd), ("R", sd if cok else sa)):
    ...
    SON = {}
    bolme = [(mad, (tr_mfg != k) & ~np.isin(tr_grp, list(tg)),
              [x for x in DER if x["mfg"] == mad]) for k, mad in kod.items()]
    bolme = [b for b in bolme if len(b[2]) >= 10]
    bolme.append(("tanidik", ~np.isin(tr_grp, list(tg)), DER))
    for ad, keep, alt in bolme:
        det, esik, yon = olc(alt, keep)
        SON[ad] = det
        print(f"\n=== {ad} (n={len(alt)}, esik={esik:.3f}, yonlendirilen {yon}/{len(alt)}) ===")
        for k in ("A", "D", "R"):
    ...
    for ad in SON:
        ...
        iyi = m > 0 or (lo < 0 < hi)          # ya daha iyi, ya farksiz
        ok &= iyi
    ...
    with open("results/u8_parca.pkl", "wb") as f:
        pickle.dump(SON, f)
    print("makbuz -> results/u8_yonlendirme_dogrula.json")
