...
permuted+translated); ML runs directly on the corpus mesh. In PRODUCTION the
flow is simpler -- a new part arrives as STEP, gets tessellated once, and BOTH
...
def fuse(cad_nodes, ml_nodes, dedup_mm, ml_add_thr=0.30):
    ...
    on purpose: the ML-only operating point is a LOW threshold (~0.15, because
    the heat head is under-confident and recall craters above it), but at that
    threshold ML also emits many weak false peaks -- measured on the 3-part
    human-GT eval, naive fusion (add every ML detection) dropped fused F1 to
    40% vs CAD-alone 72.7% purely from leaked ML FPs. Additions on top of CAD
    ...
    not tuned on the tiny fusion eval), so fusion can only add detections the
