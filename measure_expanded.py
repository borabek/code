# -*- coding: utf-8 -*-
"""R1-a: OGRENME EGRISINE ILK GERCEK NOKTA (ekstrapolasyon degil, olcum).
Ek veri: results/rich_extra.npz = 220 parca / 1058 CP (daha once kullanilmayan WEI parcalari).
Bunlar segmentasyon egitiminde GORULMUS parcalar -> SKORLAMADA kullanilmaz, sadece GATE EGITIMINE girer.
Aile-sizinti korumasi: test fold'undaki bir aileye ait ek-parca egitime ALINMAZ.
Olcum: WORK (541 temiz+kilitsiz parca) uzerinde family-out, nested-CV esik -- oncekiyle birebir ayni protokol.
Kiyas: mevcut 0.7516 (sadece WORK) / 0.7552 (WORK+153 seen)."""
import json
import numpy as np
...
XA = np.hstack([RICH, EM])                       # ana havuz: zengin + embedding
...
# --- EK havuz (yalnizca egitim) : embedding YOK -> ayni boyuta sifirla ---
...
print(f"skorlama (WORK): {int(WORK.sum())} aday / {len(set(gpid[WORK]))} parca")
print(f"ek egitim havuzu: {len(XY)} aday / {len(set(xgpid))} parca (embedding YOK -> sifir dolgulu)")
print(f"seen (ana havuzdan, egitime): {int(((gseen==1)).sum())} aday\n")
...
def oof(use_seen, use_extra):
    ...
    for tr, te in GroupKFold(5).split(XA[idx], Y[idx], FG[idx]):
        ...
        if use_extra:                                  # EK havuz (aile-korumali)
...
print(f"{'egitim havuzu':34s} {'GT':>6s} {'AUC':>8s} {'ALL':>8s} {'WEI':>8s} {'PXC':>8s}")
res = {}
for nm, us, ux in (("WORK (541 parca)", False, False),
                   ("+ seen (694 parca)", True, False),
                   ("+ seen + EK 220 (914 parca)", True, True)):
    sc = oof(us, ux)
    gt = gt_of(WORK) + (int(((gseen == 1)).sum() and sum(ngt[int(g)] for g in np.unique(G[gseen == 1]))) if us else 0) \
         + (int(x["ngt"].sum()) if ux else 0)
