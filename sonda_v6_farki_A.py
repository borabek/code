# -*- coding: utf-8 -*-
"""v6 FARKI, ADIM A: fark KORPUS/ETIKET'ten mi geliyor?

DURUM: dagitilan gate v6 tez-saf havuzda D7'de **0.1970**; ayni receteyle
(parca_ici zskor PARCA PARCA, RF 400/leaf3, urunun `karar_skoru`'su) benim
kurdugum gate ayni kumede **0.1159**. 0.08'lik fark kapanmadan genisletilmis
havuzun olculmus +0.0386'si gerceklesmiyor.

ADIM A: ayni receteyi v6'nin KENDI korpusu (`zengin_parite_v6.npz`,
53036 aday / 2583 parca, pozitif 0.2613) uzerinde kosarim.
  ~0.1970 cikarsa  -> fark KORPUS/ETIKET tanimindadir
  ~0.1159 kalirsa  -> fark `_cokus_yonlendir` ya da oznitelik kaynagindadir

SIZINTI DENETIMI YAPILDI: korpusun D7 ile kesisimi PARCA 0, MARKA 0.

Olcum: `_brep_oz` onbellegindeki D7 TEZ-SAF adaylari (kaynak==0), urunun karar
kurali + NMS, MIKRO. Bu, 0.1970 ve 0.1159'un olculdugu KUMENIN AYNISI.
"""
import collections
import json
import os
...
def korpustan_egit(yol):
    """refit_gate_v7 recetesi: parca_ici PARCA PARCA + RF 400/leaf3."""
...
def main():
    te = sinav()
    print(f"SINAV {len(te)} parca (D7 tez-saf havuz)\n", flush=True)
    out = {}
    g6 = K.gate_yukle()
