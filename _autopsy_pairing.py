# -*- coding: utf-8 -*-
"""OTOPSI KANITI: Contact bolgeleri tel-girisiyle (CableEntry) EŞLEŞIYOR mu, yoksa YALNIZ mi?

Tez: Contact sinifi = 'Kontaktierung bzw. Werkzeugeinschub' = kontakt YA DA alet-sokma agzi (ayni sinif).
Hipotez: GERCEK tel-CP'de yol = CableEntry (yuzey agzi) -> Contact (icerde). Bir TOOL agzi (Werkzeug-
einschub) ise Contact'tir ama yakininda CableEntry YOKTUR. Yani:
  Contact bileseni yakininda CableEntry VARSA -> gercek tel-baglantisi
  Contact bileseni YALNIZ (yakinda CableEntry yok) -> muhtemelen TOOL/actuator agzi -> CP DEGIL
Bu dogruysa: cp-v3'un fazla-saymasi = yalniz-Contact'lari CP sayması. Fix = Contact'i CableEntry ile ESLE.
Sadece etiket geometrisi (GPU yok). Korpus + insan etiketleri uzerinde olcer.
"""
...
def main():
    ...
    for sp in ("train", "val"):
        for s in ds.load_split("wscad_corpus_scheffler_exact", sp, verify_hashes=False):
            samples.append(s)
    # insan kismi etiketleri connection-kanali (CE/CT ayrimi yok) -> otopsi icin KORPUS'u kullan (tam 5-sinif)
    print(f"{len(samples)} tam-etiketli korpus parcasi (5-sinif)", flush=True)
    ...
    # en cok yalniz-Contact'li parcalar
    per_part.sort(key=lambda r: -r[4])
    print("\n  en cok YALNIZ-Contact'li 8 parca (pid, #CE, #CT, esli, yalniz):")
