# -*- coding: utf-8 -*-
"""M1c: rejim-kosullu cluster_mm (dusuk-CP 5.0 / cok-CP 3.0) GERCEK boru hattinda.

M1b (tek turetme): dusuk-CP 5.0 ile +0.019, cok-CP 3.0'da kalmali (-0.062 zarar).
Rejim ayrimi bu projede iki kez odedi (min_v, esik). Ama tek-turetme olcumu bu gece iki kez
yaniltti -> gercek 4-model union hattinda dogrulanmadan urune girmez.

Dagitilan mantigi taklit eder: once varsayilan turetme -> router -> rejime gore YENIDEN turet.
"""
...
def main():
    ...
    print(f"{len(sel)} parca (34 dusuk / 22 cok)", flush=True)
    ...
    res = {}
    for cl_lo, cl_hi, tag in ((3.0, 3.0, "3.0 / 3.0  (URUNDE)"), (5.0, 3.0, "5.0 / 3.0  (aday)"),
                              (5.0, 5.0, "5.0 / 5.0")):
    ...
    print("makbuz -> results/m1c_cluster.json")
