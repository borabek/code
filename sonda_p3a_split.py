...
KAPI (plan): genel aday tavani +0.02 VEYA kalabalik (cok-CP) recall +0.05
getirmezse OLDUR.

Olcum COK-CP parcalarda: bolme oralarda anlamli (komsu kutuplar birlesiyor).
"""
...
sys.path.insert(0, ".")
import d6_kayit, robot_cp, cp_openings
...
for sr in (0.0, 0.4, 0.5, 0.6):
    ...
    rec = tp / max(tp + fn, 1)
    sonuc[str(sr)] = {"kahin": f1w(T), "recall": rec, "aday": n_aday}
    print(f"  split_ratio={sr}: kahin {f1w(T):.4f} | recall {rec:.4f} | aday {n_aday}",
          flush=True)
t0 = sonuc["0.0"]
...
print("makbuz -> results/p3a_split_ratio.json")
