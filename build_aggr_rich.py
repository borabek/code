# -*- coding: utf-8 -*-
"""GECE 3. HAMLE: AGRESIF ADAY (cok-cozunurluk, recall tavani 0.969) + ZENGIN FEATURE.
NEDEN SIMDI: P2 (agresif aday) daha once ASKIYA alinmisti cunku zayif gate 6.5x adayi kaldiramiyordu
(WEI top-N 0.662 < baz 0.723). Bu gece gate MATERYAL guclendi (AUC 0.88 -> 0.955, zengin temsil).
P2'nin on-kosulu ("precision yukunu daha iyi gate tasir") ARTIK KARSILANDI -> yeniden test.
Cikti: results/aggr_rich_<mfg>.npz (X13, XR, y, groups, mfg, pos, ngt) -- analiz eval_rich_deploy ile ayni.
...
# --list dosya.txt: MFG yerine acik parca listesi (high-CP hibrit icin)
PLIST = None
if "--list" in sys.argv:
...
def main():
    ...
    print(f"{len(parts)} {MFG} parca | {len(m7)} model | agresif cok-cozunurluk + zengin feature", flush=True)
    ...
    print(f"-> {out}  {len(y)} aday ({len(y)/max(ng.sum(),1):.2f}x GT), {len(NGT)} parca, "
          f"aday-tavan recall {y.sum()}/{ng.sum()} = {y.sum()/max(ng.sum(),1):.3f}  {time.time()-t0:.0f}s")
