# -*- coding: utf-8 -*-
"""Q5: ALT-ESIK KOLLARINI YIGIN OLARAK OLC (bari indirmek yerine).

Elimde iki ALT-ESIK kol var, ikisi de gercek ama tek baslarina +0.02'yi gecmiyor:
    ACI SECICI      robot  +0.0126  GA [+0.0002, +0.0325]   tespit bedeli 0.0000
    DUR/DEVAM KESIM tespit +0.0150  5/5 tohum, GA sifiri kil payi iceriyor

Hafizadaki kayit ([[stacked-levers-2026-07-29]]) bu durum icin net: bari INDIRME, kollari
YIGIN olarak uygula ve yigini olc. Ikisi FARKLI metrige ve farkli asamaya dokunuyor
(biri yon, digeri kesim) -- ortusme beklenmez ama olculmeden varsayilmaz.
...
KILL (yigin icin, onceden yazili): robot >= +0.02 VEYA tespit >= +0.02, digerinde kayip
< 0.005, ve kazanan metrigin GA'si sifiri dislamali.
"""
import io
import json
import os
...
def main():
    ...
    print(f"aci secici + regresor hazir ({len(RY)} satir)", flush=True)

    # --- DUR/DEVAM kesim modeli: egitim korpusunun KENDI skorlarindan
    oof_pid = tpid[keep]
    ...
    for u in np.unique(oof_pid):
        ...
        # kahin K: F1'i en cok yapan onek
        en, enk = -1, 0
        ...
        for r in DER:
            ...
            if r["X"] is not None and r.get("XR") is not None:
                ...
                if kes_on and k.any():
                    ...
                    if K:
                        yeni[o[:K]] = True
                    # YIGIN KURALI: kesim yalniz DARALTIR (mevcut kabulun alt kumesi)
    ...
    KOL = {"A dagitilan": (False, False), "B +aci secici": (True, False),
           "C +kesim": (False, True), "D YIGIN (ikisi)": (True, True)}
    print(f"\n{'kol':<20}{'tespit':>10}{'ROBOT':>10}{'d_tespit':>10}{'d_robot':>10}")
    SON, PAR = {}, {}
    ...
    print("makbuz -> results/q5_yigin.json")
