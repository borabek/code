# -*- coding: utf-8 -*-
"""
MONO down-looking Mako G-234 mount on an item Profil 5 60x20 - every part is
...
Parts (each its own file):
  1. part_cam_holder   - foot plate (lens aperture) + vertical back plate
                         (camera screwed on with 2x M3) + top flange (2x M5
...
Camera dims from the Mako G drawing (user-supplied datasheet layout):
  ...
  Camera mounting uses the THREE M3 (depth 2.3) on the top face:
    1 hole on the centerline at 23.7 from the front face,
  ...
  longer screws bottom out).  The 2x M3 on the bottom face (12.2/42.6) stay
  free as a spare pattern.

Coordinate system: camera axis vertical, lens DOWN.  z=0 is the camera front
face resting on the holder foot; the profile runs along X above everything.
...
CORE_D = 4.3
PZ0    = ZFL + FLT       # profile bottom sits on the flange top = 105.5
...
def build_motor_holder(ox=0.0):
    ...
    like the camera holder.  Print flat on the plate, no supports."""
    parts = [
...
def build_laser_holder(ox=0.0, oz=0.0):
    """PRINTED part: clamp hub on the motor shaft (D5.2 bore + M3 set screw)
