# -*- coding: utf-8 -*-
"""H4: cok-CP ince-ayarli ckpt URUNU IYILESTIRIYOR mu? (rejim ayrimli, hizli teshis)

KILL (baslamadan yazildi):
  * cok-CP CP-F1 katkisi < +0.02  -> OLU
  * VEYA dusuk-CP'de -0.01'den fazla kayip -> REDDEDILIR
  (bu gece iki kaldirac tam bu imzayla oldu: promote ve adaptif yogunluk)

YONTEM: tam korpus cikarimi pahali oldugu icin once DOGRUDAN teshis --
segmentasyonun GT'lerde ATESLEME orani (T5'in ikinci adimi). Ince-ayar ise yaradiysa
cok-CP'de atesleme %76'dan yukari cikmali. Cikmazsa tam olcume gerek yok.
"""
...
def fire_rate(ckpt, parts, dev, tag):
    """Her GT agzinda ag CE+CT >= 0.30 uretiyor mu (T5 olcutu)."""
...
def main():
    ...
    # EGITIMDE KULLANILAN parcalar SKORLANMAZ (yeni ckpt onlari gordu)
    trained = set()
    ...
    print(f"skorlanan: {len(sel)} parca (egitimde kullanilan {len(trained)} parca HARIC)\n", flush=True)
    dev = "cuda" if torch.cuda.is_available() else "cpu"
    cfg = json.load(open("cp_config.json"))
    base = cfg["current_product"]["checkpoints"][1]      # keig96 urun ckpt'i (ayni k_eig)
