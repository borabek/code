# -*- coding: utf-8 -*-
"""URUN v2 PAKETI: 2026-07-28/29 gecesinde olculen +0.053'luk yapilandirmayi TEK KOMUTLA uretir.

    .venv/Scripts/python.exe urun_v2_paket.py            # yapilandirmayi dogrula + makbuz yaz
    .venv/Scripts/python.exe urun_v2_paket.py --holdout  # KILITLI HOLDOUT tek atis (geri donusu YOK)

YAPILANDIRMA (uc olculen degisiklik, hepsi WORK OOF ile dogrulandi):
  1. min_v10 post-isleme    (min_v 10 / vertex_conf 0.30 / cluster 3.0 / dedupe 6.0)
  2. ExtraTrees(800, leaf2) gate  -- RandomForest yerine
  3. conn_promote 0.25, SADECE geometri-yonlendiricisi cok-CP dedigi parcalarda

TEZ SADAKATI: segmentasyon agi, korpusu ve remesh hedefi (6000, izotropik) DEGISMEDI. Uc degisiklik
de tezin ustune bizim muhendisligimiz olan katmanda (post-isleme + gate), tezin kendi boru hattinda
degil. Segmentasyon korpusu yayinlanmis insan-etiketli bolunmesiyle aynen duruyor.

OLCULEN (korpus-temsili CP-F1, 759 temiz parca, aile-disi GroupKFold(5), ic-ice esik, 3 tohum):
    dondurulmus urun            0.7892
    + min_v10                   0.8101
    + ExtraTrees + yonlendirme  0.8423     (+0.053 toplam)
"""
...
ARM_ON = "results/rich_promote.npz"        # promote ACIK
ROUTER_THR = 0.30
...
def evaluate(score_parts, seeds=(0, 1, 2), label=""):
    """Yapilandirmayi verilen parca kumesinde olc. Doner: dict."""
...
def evaluate_holdout(work_parts, hold_parts, seeds=(0, 1, 2)):
    """GERCEK holdout protokolu: gate, esik ve yonlendirici SADECE WORK'te ogrenilir,
    kilitli parcalara BIR KEZ uygulanir.

    DIKKAT -- bu fonksiyon ayri duruyor cunku evaluate() kendi icinde GroupKFold kosar; onu
    holdout'a uygulamak gate'i HOLDOUT ILE egitmek olurdu ve tek atisi carcur ederdi.
    """
    lock = json.load(open("results/split_lock.json"))
    fams = {k: v.get("family", f"nr:{k}") for k, v in lock["parts"].items()}
    geom = json.load(open("results/cc_e_partgeom.json"))
    ...
    for s in seeds:
        # --- yonlendirici: SADECE WORK'te egitilir, holdout'a uygulanir ---
        def rrows(ps):
            r = []
            for q in ps:
                g = geom[q]; e = g["ext"]
        ...
        # --- gate: SADECE WORK adaylariyla egitilir; esik de WORK'te secilir ---
        sc = {}
        for tag, D in (("off", A), ("on", B)):
            X, y, pid, _, _ = D
            ...
            bt, bf = M.THRS[0], -1.0
            for t in M.THRS:                       # esik WORK'te secilir, holdout GORULMEZ
                k = pw >= t
...
def main():
    ...
    rec["work"] = work
    print(f"  parca {work['n_parts']} (cok-CP {work['n_high_cp']})")
    for k in ("low", "high", "corpus_weighted"):
    ...
    if a.holdout:
        ...
        if not hp:
            ...
            # GERCEK holdout: gate/esik/yonlendirici SADECE WORK'te ogrenilir (bkz evaluate_holdout)
            ho = evaluate_holdout(clean - LOCK, hp)
            ho["label"] = "KILITLI HOLDOUT (tek atis, WORK'te egitildi)"
            rec["holdout"] = ho
            print(f"  holdout parca {ho['n_hold']} | gate/esik/yonlendirici {ho['n_work']} WORK parcasinda egitildi")
            for k in ("low", "high", "corpus_weighted"):
                print(f"  {k:<18}{ho[k]['mean']:.4f}  (sd {ho[k]['sd']:.4f})")
            rec["holdout_spent"] = time.strftime("%Y-%m-%d %H:%M:%S")

    json.dump(rec, open("results/urun_v2_makbuz.json", "w"), indent=1, ensure_ascii=False)
    print(f"\nmakbuz -> results/urun_v2_makbuz.json   ({time.time()-t0:.0f}s)")
