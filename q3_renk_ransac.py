# -*- coding: utf-8 -*-
"""Q3: renk<->geometri eslemesi -- RANSAC ile (Q2'nin acgozlu esleyicisi cakmisti).

Q1: parcalarin %83.8'inde >=2 ayrik renk var (gri/gumus metal, mavi/turuncu govde) -> kanal DOLU.
Q2: yaricapla ACGOZLU esleme cakti (artik ~30mm). Sebep tanindi: bir parcada onlarca vida deligi
    AYNI yaricapta; ilk uygun olana eslemek korelasyonu coplestiriyor. Null test de ayni cikti.
gmsh yolu KAPALI (dogrulandi): Geometry.OCCImportLabels=1 ile bile tum yuzler TEK renk donuyor,
    yani OVER_RIDING_STYLED_ITEM uygulanmiyor.

YENI IMKAN: silindir yaricaplari artik TAM DOGRU (cember oturtma duzeltmesi; metin ile 327/327
eslesti, medyan hata 0.0000mm). Dogru yaricap = guvenilir aday-cift havuzu. Belirsizligi RANSAC
cozer: 3 cift ornekle, rijit donusumu coz, TUM ciftlerde ic-nokta say.

Donusum parca basina degisiyor (olculdu: kimi parcada birim, kimi parcada gercek rijit hareket) --
o yuzden her parca icin ayri cozulur.

KILL: gercek oran <0.60 ya da null testi gercege yaklasirsa (fark <0.3) is KAPANIR.
"""
import os, sys, json, glob
import numpy as np

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from q2_renk_hizalama import parse, yerel_silindirler, kabsch


def ransac(lc, lr, la, gc, gr, ga, tol=0.5, iters=4000, rng=None, karistir=False):
    """Yaricap-uyumlu ciftler uzerinde RANSAC rijit donusum. Doner: (R,t,ic_nokta,toplam_cift)."""
    rng = rng or np.random.RandomState(0)
    pairs = [(i, j) for i in range(len(lc)) for j in range(len(gc))
             if abs(lr[i] - gr[j]) < 0.01]
    if karistir:                      # NULL TEST: yaricap kisitini kaldirip rastgele cift uret
...
def main():
    ...
    print(f"{'parca':<14}{'cift':>6}{'ic-nokta':>10}{'oran':>7}{'artik':>10}"
          f"{'NULL ic':>9}{'NULL oran':>11}")
    iyi = nul = tot = 0
    kayit = []
    for f in sel:
    ...
    print("makbuz -> results/q3_renk_ransac.json")
