# -*- coding: utf-8 -*-
"""GATE'I BUYUMUS KORPUSLA YENIDEN EGIT -- olculmemis tek canli kaldirac.

DURUM: dagitilan wire-gate 1041 parcadan cikarilmis adaylarla egitildi (25.07). Korpus o
gunden beri 1906 uygun parcaya cikti: gate'in HIC gormedigi 865 parca var.

NEDEN UMUT VAR (S5 bulgusu, olculdu): aday arzini degiten bir mudahale KUCUK korpusta notr
okunur; korpus iki katina cikinca ayni cift +0.037 verdi. Gate de aday dagilimini ogrenen
bir bilesen -- ayni mantik gecerli.

GUVENLIK: urun gate'i (results/wire_gate.pkl) BU BETIKTE ASLA UZERINE YAZILMAZ. Yeni model
...
KILL (olcumden once yazildi):
  ...
  * VEYA uretici-disi sondada kayip > 0.01 -> reddedilir (ExtraTrees dersi: kolay bolunmede
    kazanan, dagilim kayinca kaybedebilir; holdout harcamadan ONCE sinanir)
"""
...
TAG = os.environ.get("GATE_TAG", "")          # cikti dosyalarini ayirmak icin (orn. "_minv4")
...
INFLIGHT = "results/gate_regrow_inflight.txt"    # su an islenen parca (zehirli-parca korumasi)
SKIPFILE = "results/gate_regrow_skip.txt"        # kalici atlama listesi


def _load_skip():
    """Daha once bizi kilitleyen parcalari atla.

    2026-07-29: tek bir WEI parcasi spektral ayristirmada 47 DAKIKA %100 CPU'da dondu ve
    hicbir cikti uretmedi; 1700 parcalik cikarim bellekteydi ve oldurulunce ucup gitti.
    Artik her parca ISLENMEDEN ONCE adi INFLIGHT'a yazilir. Kosu bir daha baslatildiginda
    orada duran parca "bizi olduren parca"dir ve kalici atlama listesine alinir.
    """
    skip = set()
    ...
    if os.path.exists(INFLIGHT):
        ...
        if stuck:
            # ILK KESINTIDE KARA LISTEYE ALMA. Mekanizma "parca asildi" ile "surec disaridan
            # olduruldu"yu ayirt edemiyor -- 2026-07-29 gece elektrik kesildi ve masum bir parca
            # (2466530000) zehirli sayilacakti. Gercekten asilan parca IKINCI denemede de asilir;
            # o yuzden kalici atlama ancak 2. kesintiden sonra yapilir.
            if attempt >= 2:
                skip.add(stuck)
                with open(SKIPFILE, "a") as fh:
                    fh.write(stuck + "\n")
                print(f"  [zehirli parca] {stuck} IKI kez kosuyu kilitledi -> kalici atlama",
                      flush=True)
            else:
...
_RETRY = {}          # bir kez yarim kalan parcalar -> sonraki deneme numarasi


def resume_partial(parts, partial_path):
    """PARTIAL'i oku ve satirlari GUNCEL parts listesine yeniden esle.

    Devam PARCA NUMARASINA gore yapilir, DONGU INDEKSINE gore DEGIL. eligible() diske yeni STEP
    dustukce buyur; bu her enumerate indeksini kaydirir. Eski indekslerle devam etmek YANLIS
    parcalari atlar ve ngt'yi (parca basina GT sayisi) kaydirir -> rejim ayrimi ve recall paydasi
    SESSIZCE bozulur, yani metrigin kendisi. tests/test_gate_regrow_resume.py bunu korur.
    ...
    if "pids" not in d:
        raise SystemExit(
            f"{partial_path} parca numarasi icermiyor (eski bicim) -> korpus buyuduyse guvenle "
...
def extract():
    ...
    if os.path.exists(PARTIAL):                      # ONCEKI KOSUDAN DEVAM ET
        ...
        print(f"  [devam] {PARTIAL} icinden {len(ngt)} parca yuklendi, indeksler guncel korpusa "
              f"yeniden eslendi" + (f" ({gone} aday artik korpusta olmayan parcalardan atildi)"
    ...
    for k, (mfg, pid, jf, stp) in enumerate(parts, 1):
        ...
        try:
            ...
            # CALISMA MANTIGINI BIREBIR TAKLIT ET (D1.3 bulgusu, olculdu 2026-07-30):
            # eskiden burada conn_promote HIC gecilmiyordu -> gate, calisma aninda cok-CP
            # adaylarinin %73'unu (256/350) EGITIMDE HIC GORMEDIGI turden aday olarak
            # skorluyordu. robot_cp once promote'SUZ uretir, yonlendirici cok-CP derse
            # promote'LU yeniden uretir. Ayni sirayi burada da uyguluyoruz.
            ...
            # PARITE (2026-08-01 denetimi): aday uretimi artik URUNUN kendi fonksiyonundan.
            # Eskiden burada AYRI bir yol vardi ve IKI yerde sapiyordu:
            ...
            #     -> egitimde votes 12'ye cikiyordu, uründe tavan 4. `votes` gate'in durust
            #        bolmede GENELLESEN TEK ozelligi oldugu icin bu ozellikle agir bir sapmaydi.
            ...
            cps, probs, _is_hi, _ = _RC.adaylari_uret(V, F, pbs, stp, cfg=cfg)
            if not cps:
                continue
            # step_path SART: WG_FIZ_FEATS=1 iken B-rep fiziksel ozellikleri buradan geliyor.
            # Verilmezse o 5 sutun SESSIZCE sifir olur ve gate onlari "bilgisiz" ogrenir --
            # E maddesinde tam olarak bu yasandi (17 sutunlu gate 4 yeni sutunu sifir aliyordu).
            X = wire_gate.feats_for(V, F, probs, cps, CE, CT, step_path=stp)

            # ETIKET: big_arbiter konvansiyonu (eksene dik mesafe + +-40mm eksenel pencere)
            ...
            # K7 icin: aday konum/yonu + parca no. Renk ozelligi GPU gerektirmez, offline eklenir.
...
def evaluate():
    """Leakage-free OOF: eski gate verisi vs buyumus veri, AYNI protokolle."""
    ...
    # ZOR BOLUNME: uretici-disi (ExtraTrees dersi -- holdout harcamadan once sinanir)
