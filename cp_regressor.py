...
def part_subsample_seed(seed, part_nr):
    ...
    evaluated/deployed on a vertex distribution it never trained on for that part
...
# so a connection-point hole sitting on a patch boundary is fully seen by both
...
def spatial_patches(V, cap, margin_frac=PATCH_MARGIN_FRAC, seed=0, max_patches=8):
    ...
    the model sees the same full-density local neighbourhood it must key on. Used on
    ...
    while stack:
        ...
        if len(left) == 0 or len(right) == 0:
            # median did not separate (many coincident coords on this axis): split by
...
def _init_training(backbone, config, resume_from, device, lr,
    ...
    pretrain on the large synthetic set, then `init_from` those weights and
    fine-tune on the real corpus. Ignored when resume_from is given (resume wins).
    ...
    normals) -- mismatched shapes raise on load_state_dict.
    """
    import random
    torch = _require_torch()
    ...
    # (observed: 10.6 GB "shared GPU memory", ~2 h/epoch on a 4 GB T1200).
    ...
    def after_epoch(self, ep, mean_loss):
        ...
        if self.metrics_fn is not None and (
            ...
            if isinstance(sel, float) and not np.isnan(sel):
                ...
                #
                # Gating (a) on min_delta silently throws away the real best: with
...
def cp_loss(pred, target, mask, w_heat=1.0, w_off=5.0, w_dir=1.0,
    ...
    offset_loss : "l1" (default, plain mean-abs-error -- constant-magnitude gradient
             ...
             offset_huber_beta, linear above). Measured on a real trained model:
             ...
             L1's constant gradient doesn't preferentially "polish" once roughly
             right. Huber concentrates gradient on shrinking the last few mm of an
    ...
    if heat_loss == "centernet":
        # alpha: focuses on hard examples. Lower alpha (1.0-1.5) penalises FN
        ...
        # beta: reduces penalty on the Gaussian skirt (near-positive vertices
    ...
    if mask.any():
        ...
        # the insert-axis sign is ambiguous, e.g. step_openings on symmetric holes)
    ...
    if as_tensors:
        ...
        # syncs/epoch measured on v19). Callers that need numbers stack these and
...
def _knn_graph(verts, k=16):
    ...
    training loop -- ~13GB at int64 on the 1749-part corpus, which is what drove
...
def _hier_worker(args):
    ...
    are configured, which is why 7118 patches took 6.2 HOURS on the corpus-v4 run.
...
def _disk_has_room(path, min_free_gb=3.0):
    ...
    cache WRITE is gated on headroom, and a full disk just means "no cache",
    never a crash."""
    import shutil
    try:
...
def _knn_graph_disk(verts, k, cache_dir):
    ...
    on large augmented train sets. Atomic write (tmp+replace) guards against
...
def _pca_normals_curvature(V, nbr):
    ...
    curvature = λ₀/(λ₀+λ₁+λ₂): near 0 on flat faces, higher in curved/concave pockets —
...
def _concavity_score(V, nbr):
    ...
    0/N nonzero on every real part checked). This silently made --knn-concavity a
...
def _edge_distance(V, nbr):
    ...
    Boundary vertices = those with the fewest kNN neighbours on only ONE side of the surface.
...
def train_knngraph_regressor(train_samples, config=None, epochs=200, lr=1e-3,
    ...
    # not a fixed seed=0 that silently differs from what the model was trained on.
    meta["seed"] = int(seed)
    ...
    # Marked True after every opt.step(); cleared on sync. This means load_state_dict
    # (an expensive GPU→CPU copy of all weights) happens at most ONCE per accumulation
    # window instead of once per big part -- critical when many big parts hit CPU each
    # epoch (on a T1200 with max_gpu_verts=7000, most parts go CPU).
    ...
    def _get_hier(d):
        ...
        # function of the patch geometry -- built once, cached on the dict (and
        # on disk next to the kNN graphs when a cache dir is configured). In lazy
        # mode it is NOT retained on the dict: the disk cache is the store.
        h = d.get("hier")
        if h is None:
    ...
    def _accumulate(d, run_device, src_device, loss_div, sign_inv=False,
        ...
        # kNN graph, so build once per part (after nbr exists) and cache on the dict.
        if use_normals:
        ...
        if is_hier:
            ...
            # EdgeConv's h[nbr_idx] gather needs int64 on device
    ...
    for s in train_samples:
        ...
        if do_patch:
            ...
            # ALL training graphs from only ~34% of train parts on the real corpus),
            ...
            for idx in patches:
                ...
                # per-part loss weight, precomputed on the numpy target so the hot
            ...
            prepared.append({"verts_norm": np.ascontiguousarray(base), "nbr": None,
                             ...
                             "pw": pw})
        # Free this sample's FULL-RESOLUTION source arrays once its patches/
        ...
        # are left intact. This is math-neutral -- the model trains on `prepared`,
        ...
        # instead of pinning it for the whole run, which on a 31GB box is the
        ...
        # hierpoint/knngraph path never reads again -- it trains on `prepared`,
    ...
    if _to_build:
        ...
        def _build_one(d):
            try:
                # kNN graphs are NOT disk-cached in the fit path on purpose: the
        ...
        # transient float64 gather buffers, so on a commit-tight box (31GB RAM,
    ...
    # first epoch doesn't stall on serial per-batch CPU feature computation.
    if use_normals:
    ...
    # doesn't stall on serial per-batch construction.
    if is_hier:
    ...
    # on this box's WDDM driver, pinning thousands of small tensors raises a CUDA
    ...
    # off unless the user explicitly enables it on a driver where it's safe.
    ...
    # normalisation and optimizer-step cadence see several parts at once.
    ...
    def _batch_accumulate(batch_dicts, run_device, loss_div):
        ...
        for chunk in chunks:
            ...
            for d in chunk:
                ...
                # parts the model keeps failing on" signal into "mine hard BATCHES").
    ...
    try:
        for ep in range(start_epoch, epochs):
            ...
            ep_ema_pairs = []   # (part_idx, loss) pairs, materialised once/epoch
            ...
            for k, (bdev, bidx) in enumerate(ep_batches, 1):
                ...
                if accum_since_step >= accum or k == len(ep_batches):
                    ...
                    _cpu["dirty"] = True
                    accum_since_step = 0
                # parts["total"]/per_part are detached 0-dim GPU tensors on the
                # batched GPU path (floats on the CPU path) -- accumulate without
            ...
            # float(cuda_tensor) syncs per part, ~14k/epoch on v19 -- a measured
            # throughput drain on the launch-latency-bound T1200 path).
            ...
            if str(device).startswith("cuda"):
                # hand cached blocks back once per epoch so allocator
...
def infer_knngraph(model, meta, verts_norm, device="cpu", max_gpu_verts=60000,
    ...
    the small CP openings keep the geometry the model keys on. Each block is run and
    ...
    only the fallback for older checkpoints. CUDA OOM on any run falls back to CPU.
    ...
    seed=0 -- a DIFFERENT vertex subset than what the model trained on for this
    part (silently drops recall on big ABB/general parts; this was an unreported
    train/inference mismatch, now fixed for every caller that passes part_nr).
    Only matters on the non-patch (v8, uniform-subsample) path; patch=True already
    covers every vertex via spatial_patches, no subsample seed involved.

    graph_cache / knn_cache_dir : graph memoisation (in-process / on-disk); used on
    ...
    def _run(Vq, nbr_cpu):
        def __run(rd):
            if _is_hier:
                ...
                nbr = nbr_cpu.to(rd).long()      # int32 host -> int64 on device
    ...
    # prevent. Measured on v31's val (2026-07-14): 27 of 278 parts (10%) exceed the
    # budget and carry 390 GT CPs (18% of ALL GT); the worst keeps only 16.9% of its
    # geometry and the model finds 14 of its 42 CPs -- one part scores TP=0 on 36 CPs.
    ...
    # the standard train-on-crops / infer-full-image pattern. Default 8 keeps the old
...
def infer_knngraph_tta(model, meta, verts, device="cpu", max_gpu_verts=60000,
    ...
    rotation. Run inference on `n_aug` random rotations of the part, rotate each
    ...
    precision gain (measured +7 micro-F1 / FP cut ~70% on the real corpus) with NO
...
def infer_knngraph_tta_batched(model, meta, verts, device="cpu", max_gpu_verts=60000,
    ...
    the batched version does ONE pass on 8*1000 = 8000 vertices (a single kNN build +
    ...
    views at once (identical neighbourhood structure; only input coords differ).
    ...
    # Concatenate all views into one large point cloud; kNN built on each view
    ...
    # Feature tensor (curvature/concavity computed per-view on the flat cat)
    ...
    def _post_np(raw, scale):
        """Apply sigmoid to heatmap + scale offsets on a (N,7) raw numpy slice."""
...
def infer_knngraph_multicrop(model, meta, verts_norm, device="cpu", max_gpu_verts=60000,
    ...
    F1 0.60). Here we run the model on MULTIPLE independent uniform subsamples ("crops"),
    ...
    (it just gets partial coverage, still far better than one crop). CUDA OOM on a crop
    ...
    def _run(Vq, nbr, rd):
        nb = nbr.to(rd).long()               # int32 host -> int64 on device
