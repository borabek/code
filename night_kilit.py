...
WHY: night along dagitilan artefaktlar degisecek. Sabah "ne degisti" sorusunun single
cevabi this receipt must be. Ayrica belgeler with config arasindaki known celiskiler
(LOCKED 98 vs 95) simdi kapatilir ki night along wrong number dolasmasin.

DISK BEKCISI: disk this an ~22GB. Tek agir writes works; 12GB'in altina inerse operasyon
DURUR (night along two times %100 doldu and kosulari oldurdu).
"""
import glob 
import hashlib 
import io 
...
def main ():
    ...
    if os .path .exists ("results/headline.json"):
        ...
        for k ,v in m .get ("bolmeler",{}).items ():
            headline [k .strip ()]={"detection":v .get ("tespit_F1"),"robot":v .get ("robot_hazir_F1"),
            "n":v .get ("n_parca")}
    ...
    for md_ in glob .glob ("*.md"):
        ...
        for m_ in re .finditer (r"(9[0-9])\s*(?:grup-temiz\s*)?LOCKED|LOCKED[^\n]{0,30}?\b(9[0-9])\b",s ):
            ...
            if n and n !="95":
                celiski .append ({"file":md_ ,"found":n ,"baglam":s [max (0 ,m_ .start ()-60 ):m_ .end ()+40 ].replace ("\n"," ")})
    print (f"\nartefakt: {len (artefakt )} file damgalandi")
    print (f"headline bolmesi: {len (headline )}")
    print (f"LOCKED celiskisi: {len (celiski )} yer")
    for c_ in celiski [:6 ]:
        print (f"  {c_ ['file']}: '{c_ ['found']}' -> {c_ ['baglam'][:90 ]}")

    kilit ={
    "tarih":"2026-08-02 night",
