# -*- coding: utf-8 -*-
"""H2: H'nin OLUM BICIMINI hedefleyen iki duzeltme.

H olctu: oncul duzeltmesi recall'i ARTIRMADI (0.747 -> 0.722), precision'i yikti (0.923 -> 0.791).
Yani fazladan atesnlenen koseler aciklik degil, DAGINIK KUCUK LEKELER. Iki hedefli care:

  A) KAPILI oncul  : oncul yalnizca zaten CE/CT sinyali olan koselerde uygulanir
                     (p_CE+p_CT >= floor). Esigin altinda kalmis GERCEK acikliklar kurtulur,
                     hicbir sinyali olmayan koseler hic girmez.
  B) min_v ile birlikte: fazladan gelen lekeler KUCUK. min_v yukseltmek onlari eler,
                     gercek acikliklar (daha buyuk) kalir. H tek basina degil, CIFT olarak.

Onbellekten kosar, GPU'ya dokunmaz.
KILL: tespit F1 mevcut 0.7783'u gecmezse H tamamen kapanir.
"""
...
def main():
    ...
    prior = acc / acc.sum()
    print(f"{len(cache)} parca | esikler {thr}\n", flush=True)
    ...
    print(f"{'ayar':<34}{'tespit F1':>11}{'kesinlik':>10}{'recall':>9}{'fark':>9}")
    base = None
    CFG = [("MEVCUT (alpha 0)", 0.0, 0.0, MINV0),
           ("A: alpha .3 + kapi .10", 0.3, 0.10, MINV0),
           ("A: alpha .3 + kapi .25", 0.3, 0.25, MINV0),
           ("A: alpha .5 + kapi .25", 0.5, 0.25, MINV0),
           ("B: alpha .3 + min_v 10", 0.3, 0.0, 10),
           ("B: alpha .3 + min_v 20", 0.3, 0.0, 20),
           ("A+B: alpha .3 kapi .25 min_v 10", 0.3, 0.25, 10),
           ("A+B: alpha .5 kapi .25 min_v 10", 0.5, 0.25, 10)]
    res = {}
