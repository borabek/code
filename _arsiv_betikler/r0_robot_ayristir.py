...
robot-hazir (0.4500) with tespit (0.7439) arasindaki difference, AYNI tahminlerin more DAR a olcute
sokulmasindan geliyor:

    tespit      : lateral <= max(3mm, %6 x kosegen), angle SERBEST (180)
    robot-hazir : lateral <= 2.0mm  VE  angle <= 10 deg

Yani loss YALNIZ two kaynaktan may be. Hangisi oldugunu bilmeden plan yapmak, this gece four times
...
CIKTI: tespitte eslesen each GT for lateral distance and angle dagilimi; kaybin yuzde kaci hangi
...
def eslesme_ayrinti (P ,Pd ,G ,Gd ,diag ):
    ...
    pe_t =np .where (np .abs (al )>40 ,np .inf ,pe )# tespit modu: angle serbest
...
def main ():
    ...
    print (f"  ACI tamamen cozulseydi (lateral ayni)  : {y_ok .mean ():.1%}")
    ...
    with open ("results/r0_kayit.pkl","wb")as f :
        pickle .dump (KAYIT ,f )
    print ("receipt -> results/r0_robot_ayristir.json")
