# -*- coding: utf-8 -*-
"""S0+S4: KUSUR HARITASI ve TRIYAJ -- onarimi HERKESE degil, KIME uygulamali?
...
SEBEP OLCULDU: uretici CP'lerinin ~%47'si GOVDENIN ICINDE (kontakta). Noktayi agza
tasimak fiziksel gecerliligi kazandirirken GT'den UZAKLASTIRIYOR. "Robot icin dogru"
ile "ureticiye yakin" bu noktada CELISIYOR.

TRIYAJ FIKRI: onarim herkese degil, KUSUR YOGUNLUGU YUKSEK olanlara uygulansin.
A4 olctu: bayrak sayisi arttikca FP olma olasiligi artiyor (zenginlesme 2-3x). Uc bayragi
birden yanan 36 CP buyuk olasilikla cop; tek bayrakli olanlar onarilabilir TP olabilir.

Bu betik onbellekten (S2 ciktisi) calisir, YENI mesh islemi YOK. Uc triyaj denenir:
    T1  yalniz >=1 bayrakli (= S3'un yaptigi, taban)
    T2  yalniz >=2 bayrakli
    T3  yalniz 3 bayrakli
...
def main():
    ...
    for r in DER:
        ...
        _, _, _, b = esle_detay(d["P0"], d["D0"], G, Gd, r["diag"], 0.0, 180.0, True)
    ...
    def kos(en_az, red_esigi=None):
        """en_az bayrakli olanlar ONARILIR; red_esigi kadar bayrakli olan ELENIR."""
        rows_d, rows_r = [], []
        t2f = f2t = 0
        kus = ntop = 0
        for r in DER:
            ...
            if d is None:
                ...
                for i in range(len(P)):
                    ...
                    kus += any(bb)
                P = P[tut]; D = D[tut]
            _, _, _, b0 = esle_detay(d["P0"] if d else P, d["D0"] if d else D, G, Gd,
                                     r["diag"], 0.0, 180.0, True)
            tp, fp, fn, b1 = esle_detay(P, D, G, Gd, r["diag"], 0.0, 180.0, True)
            rows_d.append((rj, tp, fp, fn))
            tpr, fpr, fnr, _ = esle_detay(P, D, G, Gd, r["diag"], 2.0, 10.0, False,
                                          isaretli=True)
    ...
    print(f"\nKILL (S4): tespit >= taban+0.005 VE kusur orani <= taban-%5 mutlak")
    ...
    with io.open("results/s0s4_triyaj.json", "w", encoding="utf-8") as f:
        json.dump({"harita": {str(k): {"cp": say[k], "tp": tpfp[k][0], "fp": tpfp[k][1]}
                              for k in sorted(say)},
                   "taban": {"tespit": tb_d, "robot": tb_r, "kusur": tb_k},
