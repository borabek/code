...
unusable. WEI first: the product scores 0.780 on PXC but 0.233 on Weidmueller, and manufacturer
...
HOW (learned by capturing one manual download, results/wscad_net.json):
    ...
    {"manufacturerId":88,"partNumber":"1010100000","partType":2,"norm":0,"language":"en"}
...
POLITE BY DESIGN: sequential, a real delay between parts, skips what is already on disk (so it
...
def todo(mfg, limit, list_path=LIST):
    ...
    with open(list_path, encoding="utf-8-sig") as fh:
        for r in csv.DictReader(fh):
            if r["parca_no"] in have or (mfg and r["uretici"] != mfg):
                continue
            if r["uretici"] not in MFG_ID:
                continue
            r["mid"] = MFG_ID[r["uretici"]]
            rows.append(r)
...
def todo_census(mfg, limit, census_path=CENSUS):
    ...
    returned for an EXACT part-number match. That fixes two things at once: parts whose maker is
    ...
    with open(census_path, encoding="utf-8-sig") as fh:
        for r in csv.DictReader(fh):
            ...
            if not same_maker(r["our_mfg"], r["wscad_mfg_name"]):
                ...
                continue
            rows.append({"parca_no": r["part"], "uretici": r["our_mfg"],
                         "mid": int(r["wscad_mfg_id"]), "wscad_mfg": r["wscad_mfg_name"]})
    if rejected:
        import collections
        c = collections.Counter(rejected)
        print(f"  [uretici uyusmuyor] {len(rejected)} parca ATLANDI (numara ayni, marka baska): "
...
def main():
    ...
    ap.add_argument("--from-census", action="store_true",
                    help=f"parca+uretici-id'sini {CENSUS} makbuzundan al (API'nin dogruladigi id; "
                         "orada olmayan parcalar hic denenmez)")
    a = ap.parse_args()
    from playwright.sync_api import sync_playwright

    rows = todo_census(a.mfg, a.limit) if a.from_census else todo(a.mfg, a.limit, a.list)
    if not rows:
        print("indirilecek parca yok (hepsi diskte olabilir)"); return
    src = "census (API-dogrulanmis id)" if a.from_census else "eksik listesi (MFG_ID tablosu)"
    print(f"{len(rows)} parca indirilecek ({a.mfg or 'hepsi'}) | kaynak: {src} | "
        ...
        for i, r in enumerate(rows, 1):
            ...
            try:
                ...
                if res.get("status") == 200 and body.startswith("ISO-10303-21"):
                    ...
                    # 404 = "bu parcanin STEP'i YOK", ariza DEGIL. Dogrulandi: govde uretilecek
                    # dosya adinin kendisi ("1492-H7_5_Step_IEC.stp"), ve parcanin kendi
                    # part.json'u formats:[0,1] diyor -- yani yalniz WSCAD verisi + EPLAN .edz,
                    # format 2 (STEP) hic yok. Bunu ard-arda-hata sayacina katmak kosuyu
                    # daha ilk uretici blogunda durduruyordu.
                    nostep += 1; streak = 0
            ...
            if streak >= a.max_fail:
                print(f"\n{streak} ard arda GERCEK hata (404 disi) -> duruyorum", flush=True); break
            time.sleep(a.delay)
        ctx.close()
