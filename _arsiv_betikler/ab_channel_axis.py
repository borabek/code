# -*- coding: utf-8 -*-
"""A/B: channel_axis (yon duzeltmesi) URUN F1'ini degistiriyor mu?

NEDEN ZORUNLU: yon duzeltmesi cizim icin degil, cp_openings'in ICINE kondu. Orada yon yalnizca
raporlanmiyor -- `outward_min` kapisinda ADAY ELEMEK icin de kullaniliyor. Aday sayilari fiilen
degisti (3270115 CP13->CP14, 2770943 CP41->CP40), yani F1 yukari da asagi da gidebilir.
"Gorsel bir duzeltme" diye olcmeden gecmek, bu depoda daha once tam olarak boyle patlamisti.

YONTEM: results/pitstop2_cache icindeki V/F/olasiliklar yeniden kullanilir -- GPU'ya gerek yok,
segmentasyon ciktisi ikisinde de AYNI. Degisen tek sey cp_openings'in yon hesabi.
Skorlama big_arbiter konvansiyonu: eksene dik mesafe + +-40mm eksenel pencere, rejim ayrimli
ve korpus-agirlikli (%89.5 dusuk / %10.5 cok).

KILL (olcumden once yazildi): korpus-agirlikli CP-F1 kaybi > 0.005 ise yon duzeltmesi URUNDEN
GERI ALINIR (cizime ozel bir bayrak arkasina tasinir).
"""
...
def derive(rows, use_axis):
    """Tum parcalar icin adaylari YENIDEN turet (yon duzeltmesi acik/kapali)."""
    ...
    for r in rows:
        per = []
        pb = r["probs"]
        for _ in range(1):        # onbellekte ORTALAMA olasilik var -> tek turetme
        ...
        cps = per[0]
        thr = float(cfg.get("robot_wire_gate_threshold_highcp", 0.25)) if r["regime"] == "cok" \
            else float(cfg.get("robot_wire_gate_threshold", 0.35))
        if cps:
...
def main():
    ...
    if not rows:
        print("onbellek bos -- once pitstop2_gate_nested.py cikarimini kos"); return 1
    print(f"{len(rows)} parca ({sum(r['regime']=='dusuk' for r in rows)} dusuk / "
    ...
    print(f"{'yon KAPALI':<14}{off['dusuk']:>10.4f}{off['cok']:>10.4f}{off['weighted']:>12.4f}")
    print(f"{'yon ACIK':<14}{on['dusuk']:>10.4f}{on['cok']:>10.4f}{on['weighted']:>12.4f}")
    delta = on["weighted"] - off["weighted"]
    print(f"\nFARK: {delta:+.4f}")
    verdict = "GUVENLI" if delta >= -0.005 else "GERI AL"
    print(f"KAPI (kayip <= 0.005): {verdict}")
    json.dump({"off": off, "on": on, "delta": delta, "verdict": verdict},
              open("results/ab_channel_axis.json", "w"), indent=1)
    print("makbuz -> results/ab_channel_axis.json")
    return 0
