# -*- coding: utf-8 -*-
"""DECISION: TUM WEI tipik parcalarda agresif candidate uretimi candidate oracle'i 0.826 ustune cikariyor mu?
baseline (6k, 4 model) vs agresif (6k+9k+12k, 7 model, low vc/min_v). reach = GT CP'ye tol inside
candidate present mi. Cikarsa (>=0.88) WEI 0.80 mumkun; kalirsa (~0.83) segmentasyon susuyor = label.
Bu SEFER 4 not TUM WEI tipik part (onceki probe yetersizdi)."""
...
def main ():
    ...
    print (f"{len (wei )} WEI tipik part | baseline(6k/4mdl) vs agresif(6k+9k+12k/7mdl)",flush =True )
