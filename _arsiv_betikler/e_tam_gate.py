# -*- coding: utf-8 -*-
"""E'yi TAM KORPUS gate'inde dogrula -- cikarim YAPMADAN.

KISAYOL: results/gate_regrow_data_rt2.npz aday NOKTA (pts), YON (dirs) ve PARCA NO (pids)
tasiyor. Eksen guveni ozellikleri yalnizca bunlara + STEP'in B-rep'ine baglidir; agin
olasiliklarina DEGIL. Yani 23551 aday icin ozellikler offline hesaplanip X'e eklenebilir --
1903 parcalik yeniden cikarim (saatler) gereksiz.

NEDEN ONEMLI: E, 100 parcalik zayif bir gate'te +0.0208 verdi. Ama bu projede bir kaldirac
"bayat/zayif gate"te kazanip tam korpus gate'inde KAYBETTI (P1 uzlasma yukseltici: +0.006 zayif
gate, -0.0225 tam gate). Bu yuzden tam korpusta dogrulanmadan urune GIRMEZ.

KILL: tam korpus OOF CP-F1 farki < +0.005 ise E duser.
"""
...
def main():
    ...
    print(f"{X.shape[0]} aday / {len(set(pids))} parca / {X.shape[1]} ozellik", flush=True)

    if os.path.exists(OUT):
        XE = np.load(OUT)["XE"]
        print(f"eksen ozellikleri onbellekten: {XE.shape}", flush=True)
    else:
        ...
        for k, pid in enumerate(sorted(set(pids))):
            ...
            if (k + 1) % 100 == 0:
                print(f"  {k+1} parca", flush=True)
        np.savez(OUT, XE=XE)
        print(f"eksen ozellikleri yazildi ({miss} parca STEP/B-rep okunamadi)", flush=True)

    print(f"\neksen kaynagi dagilimi: silindir %{100*XE[:,0].mean():.1f} | "
          f"duzlem %{100*XE[:,1].mean():.1f} | belirsiz %{100*XE[:,2].mean():.1f}")
    det = XE[:, 2] == 0
    print(f"  eksen BELIRLI  TP orani %{100*y[det].mean():.1f} (n={int(det.sum())})")
    print(f"  eksen BELIRSIZ TP orani %{100*y[~det].mean():.1f} (n={int((~det).sum())})\n")
    ...
    print(f"{'kol':<26}{'CP-F1':>9}{'dusuk':>9}{'cok':>9}{'esikler':>18}")
