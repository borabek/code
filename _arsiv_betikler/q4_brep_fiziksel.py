...
OLCULEN OZELLIKLER (all of them B-rep'ten, mesh tahmini DEGIL):
  brep_r      : eslesen silindirin ANALITIK yaricapi   (vida deligi small, tel girisi large)
...
def ozellikler (p ,d ,cyl ,mesh ):
    ...
    try :
        ...
        if len (h ):
            ...
            out ["gecen"]=int (len (h )<=1 )# single cikis = obur taraftan cikiyor
        else :
            out ["gecen"]=1 
    except Exception :
        pass 
...
def main ():
    ...
    for cluster in ("dev","val"):
        ...
        print (f"  {cluster } bitti (toplam {len (X )} candidate)",flush =True )
