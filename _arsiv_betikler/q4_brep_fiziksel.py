...
KANIT (kayitli FP otopsisi): yanlis tespitlerin %86.3'u UC fiziksel imzaya ait --
eksen-dik %35, BOYDAN BOYA DELIK %31, yaricap<1mm %21. Gate'in 13 ozelliginin HICBIRI
bunlari tasimiyor: `size` mesh agiz genisligi (yaklasik), `depth` isin sondaji, gerisi
olasilik/komsuluk istatistigi. Yani gate, yanlislarinin %86'sini olusturan fizigi GORMUYOR.

Silindir yaricaplari artik TAM DOGRU (cember oturtma duzeltmesi, metinle 327/327 eslesti) --
bu olcum ondan once yapilamazdi cunku yaricap 3.5 kat yanlisti.

OLCULEN OZELLIKLER (hepsi B-rep'ten, mesh tahmini DEGIL):
  brep_r      : eslesen silindirin ANALITIK yaricapi   (vida deligi kucuk, tel girisi buyuk)
  esesenli    : ayni eksen cizgisini paylasan silindir sayisi (havsa/dis imzasi)
  r_orani     : es-eksenli yaricaplarin max/min orani  (havsali vida deligi >1)
  bos_derinlik: eksen boyunca ilk "taban"a kadar mesafe (kor delik) -- yoksa BOYDAN BOYA
  gecen       : boydan boya mi (0/1)

YONTEM: DEV+VAL adaylari (200 parca, geometri olarak ayrik) uzerinde TP/FP etiketiyle
...
KILL (onceden yazildi): hicbir ozellik AUC >= 0.60 vermezse (null'a gore anlamli) kanal KAPANIR.
"""
...
def auc_mw(x, y):
    ...
    xs = x[o]
    i = 0
    while i < len(xs):                       # bagli degerlere ORTALAMA sira
        j = i
...
def ozellikler(p, d, cyl, mesh):
    """Bir aday icin B-rep fiziksel ozellikleri."""
    import cp_geometry as G
    C, A, R = cyl
    out = dict(brep_r=0.0, esesenli=0, r_orani=1.0, bos_derinlik=0.0, gecen=0)
    if len(C):
        rel = p - C
        al = (rel * A).sum(1)
        ...
        if ok.any():
            ...
            # ayni EKSEN CIZGISINI paylasanlar: yon parallel + eksenler cakisik
    ...
    try:
        ...
        if len(h):
            ...
            out["gecen"] = int(len(h) <= 1)      # tek cikis = obur taraftan cikiyor
        else:
            out["gecen"] = 1
    except Exception:
        pass
...
def main():
    ...
    for kume in ("dev", "val"):
        ...
        print(f"  {kume} bitti (toplam {len(X)} aday)", flush=True)

    names = list(X[0].keys())
    M = np.array([[x[n] for n in names] for x in X], float)
    Y = np.array(Y, bool)
    print(f"\n{len(Y)} aday | TP {int(Y.sum())} / FP {int((~Y).sum())}")
    ...
    print("makbuz -> results/q4_brep_fiziksel.json (+ .npz: ozellikler saklandi)")
