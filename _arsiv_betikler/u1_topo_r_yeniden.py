...
([[single-turetme-proxy-gecersiz]] dersinin same bicimi).

KULLANIM:
    python u1_topo_r_yeniden.py --dogrula          # only R=6 tekrar-uretim testi
    python u1_topo_r_yeniden.py --r 12.0           # new npz uret
"""
import argparse 
import os 
import sys 
...
def hesapla (R ,bound_ =0 ,ilerleme =True ):
    ...
    for k ,pid in enumerate (benzersiz ,1 ):
        if ilerleme and k %100 ==0 :
            print (f"  {k }/{len (benzersiz )} part | {time .time ()-t0 :.0f}s | atlanan {atlanan }",
            flush =True )
    ...
    print (f"  bitti: {len (benzersiz )-atlanan }/{len (benzersiz )} part | {time .time ()-t0 :.0f}s")
    return out ,tamam 
