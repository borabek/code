...
Pozisyonla: her aday icin within-part SPATIAL sinyaller (symmetry-partner, grid-consistency, neighbor
score-consistency, local density) -> RF top-N OOF. Baseline'i (ALL 0.813) gecer mi, 0.85 yolu var mi?
DURUST: WEI ve PXC ikisi de dusmemeli (win kriteri)."""
...
def spatial_feats(Pp, ws):
    """within-part spatial sinyaller (frame-invariant). Kucuk N'de bile symmetry/neighbor calisir."""
    ...
    # 1) mirror-partner: u-eksende yansimasi (-pu) yakin bir aday var mi (terminaller simetrik)
    mir = np.zeros(n)
    for i in range(n):
    ...
    # 2) neighbor score-consistency: en yakin komsunun ws'i
    nn = np.argmin(D, 1); nn_ws = ws[nn]
    # 3) grid-consistency: u ya da v ekseninde hizali yuksek-ws komsu agirligi
    ...
    # 4) local density (8% span icinde komsu ws)
    dens = np.array([float(ws[(D[i] < 0.10*span)].sum()) for i in range(n)]); dens = dens/max(dens.max(), 1e-9)
    # 5) centrality: parca merkezine goreli konum (kenar mi ici mi)
...
# spatial feat'lari ilk-pass ws ile hesapla (leakage yok: ws OOF)
first = oof(X)
SP = np.zeros((len(y), 6))
for g in np.unique(grp):
...
for tag, feat in [("13 + 6 spatial", Xsp)]:
    ...
    print(f"  -> standart havuz select-recall {base['ALL']:.3f} -> {r['ALL']:.3f}  (oracle tavan 0.895)")
