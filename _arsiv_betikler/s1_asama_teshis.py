# -*- coding: utf-8 -*-
"""S1: YONU HANGI ASAMA BOZUYOR? (yarik/push-in sinifina giden yol)

R4 gosterdi: 45 derece ustu sapan 107 noktanin 107'si, KESIN analitik B-rep ekseni verilse bile
dik kaliyor. Ama bu "hicbir sey yapilamaz" demek DEGIL -- yon bir ZINCIRDEN geciyor:

    tez normali (v_o - v_s, Abb.44)  ->  channel_axis  ->  YUVARLAMA  ->  normal-kovaryans  ->  B-rep

Zincirin her halkasi bir oncekini EZEBILIYOR. Hangi halkanin GT'ye en yakin oldugunu olcmeden
duzeltmek korlemesine olur. Burada her CP icin tum asamalar kaydedilip GT ile karsilastiriliyor.

TEZ NOTU: tez §5.3.6/Abb.44 normali `v_o - v_s` olarak tanimlar VE "bbox eksenlerine hizali"
der -- yani YUVARLAMA tezin kendi kuralidir. Ama uretici GT'sinin %19.1'i 10 dereceden fazla
egik. Bu olcum, tezin konvansiyonu ile gercek arasindaki acinin NEREDE olustugunu gosterir.

CIKTI: asama basina "GT'ye 10 derece icinde" orani + hangi asamanin ORACLE secimi ne kazandirirdi.
"""
import json
import os
import pickle
...
def main():
    ...
    for i_, r in enumerate(cache, 1):
        ...
        if not len(G):
            continue
        # TEK model yeter: asama karsilastirmasi icin oy birlesimi gerekmez, hatta zararlidir
        # (birlesim yonleri ortalar ve asama izini bozar).
        ...
        if not iz:
            continue
        # Her IZ kaydini EN YAKIN GT'ye bagla (kumeleme sonrasi CP listesiyle degil, iz noktasiyla)
        for z in iz:
            q = z.get("nokta")
            if q is None:
                continue
            ...
            if dd[b] > max(3.0, 0.06 * float(r["diag"])):
                continue                      # GT'ye eslesmeyen aday -- yon dogrulugu tanimsiz
