...
Manifold/ManifoldPlus derlemeye girismeden once hipotezi ucuza test et (teshis-once-tedavi).
Her WEI model-FN acikligi icin: acikligin ekseni boyunca [0,15]mm iceri, perp<r silindirde
ORIJINAL STEP tessellation'daki (Vr) verteks ORANI vs REMESH'teki (V) oran.
  remesh_orani << orijinal_orani  -> remesh tuneli MUHURLEDI (Manifold denemeye deger)
  oranlar benzer                  -> aciklik korunuyor, remesh SUCLU DEGIL (Manifold detourunu ATLA)
Karsilastirma yogunluk-normalize (iki mesh'in toplam verteks sayisi cok farkli).
GPU gerekir (model FN'lerini bulmak icin).
"""
...
def frac_in_cyl(P, origin, axis, lo, hi, rad):
    """verteslerin ORANI: eksen silindiri icinde kalanlar / toplam."""
    ...
    for k, (mfg, pid, jf, stp) in enumerate(raw, 1):
        try:
            ...
            for gi in range(len(G)):
                ...
                # aciklik AGZI ~15mm disarda (uretici CP kontakt) -> agizdan iceri [0,15]mm bak
                mouth = Gm[gi] + 15.0 * ax
