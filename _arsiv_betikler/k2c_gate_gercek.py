# -*- coding: utf-8 -*-
"""K2c: yeni gate'i GERCEK boru hattinda dogrula -- SIZINTISIZ.

SORUN: results/wire_gate_regrow_minv4.pkl TUM 1903 parcayla egitildi, yani test edecegim
parcalari gordu. Dogrulama sizintili olurdu.

COZUM: test parcalarinin AILELERINI disarida birakan ayri bir gate egit, sonra gercek
4-model union boru hattinda iki gate'i karsilastir. Kilitli holdout'a DOKUNULMAZ.
"""
...
def main():
    ...
    print(f"gate egitimi: {int(keep.sum())} aday ({len(y)-int(keep.sum())} tanesi test ailesi, cikarildi)",
          flush=True)
    ...
    print(f"{'gate':<34}{'esik':>10}{'dusuk':>9}{'cok':>9}{'agirlikli':>11}")
    res = {}
    ...
    # DURUST kol: esik BUYUK veride (1903 parca, aile-disi OOF) secildi, test kumesine BAKILMADI.
    # K2d olcumu: dusuk-CP 0.45 / cok-CP 0.25.
    r = score(HOLD_GATE, 0.45, 0.25)
    res["refit"] = r; res["refit_thr"] = [0.45, 0.25]
    print(f"{'B) YENI (esik buyuk veriden)':<34}{'0.45/0.25':>10}{r['dusuk']:>9.4f}"
    ...
    print("makbuz -> results/d1_gate_gercek.json")
