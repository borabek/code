# -*- coding: utf-8 -*-
"""FB-1: EMBEDDING UZAYININ TAVANI -- 0.80'in mumkun olup olmadigini soyleyen olcum.

TESHIS (adli tip raporundan, bugun dogrulandi): tezin `Contact` sinifi tasarimi geregi
"Kontaktierung *bzw.* Werkzeugeinschub" -- tel girisi ve alet agzi TEK SINIF. Yani backbone,
bizim ayirmak istedigimiz iki seyi BIRLESTIRMEK uzere egitildi. Dokuz gate mekanizmasinin
ayni cepheye carpmasinin koku bu.

AMA rapor kendi "backbone kor" tezini de curutmus (family-out AUC):
    yalniz backbone embedding'leri   0.9272
    yalniz 13 el-yapimi oznitelik    0.8997
Backbone ayrimi TASIYOR ve el-yapimini geciyor. Yani bilgi orada; biz onu 73 sutuna
sikistirirken kaybediyoruz.

BU OLCUM: daha once EL-YAPIMI 73 sutunda yaptigim komsu-uyusmazligi analizini (uyusmazlik
%28.2 -> tavan ~0.86) AYNEN embedding uzayinda tekrarlar.

OKUMA:
  embedding uyusmazligi BELIRGIN DUSUKSE -> tavan yukselir, tespit 0.84 (robot 0.80'in
      gerektirdigi) ULASILABILIR, ve yol TEMSILDEN gecer (yardimci supervizyon).
  FARK YOKSA -> problem gercekten doymus; 0.80 mevcut bilgi rejiminde yok.

Embedding: `last_lin` oncesi 128 boyutlu tepe temsili, adayin AGIZ BOLGESINDE havuzlanir
(ortalama + maksimum = 256 boyut). Havuzlama bolgesi, adayin cevresindeki 6mm.
"""
import io
import json
import os
...
def embed_al(model, meta, V, F, dev, op_cache):
    """last_lin ONCESI 128-boyutlu tepe temsili."""
...
def main():
    ...
    if os.path.exists(ONB):
        ...
        print(f"onbellekten: {len(RY)} aday", flush=True)
    else:
        ...
        for k, r in enumerate(D["DER"], 1):
            ...
            try:
                ...
                if not Z:
                    continue
                Zm = np.mean(Z, axis=0)                      # topluluk ortalamasi (V,128)
    ...
    print(f"\n{len(RY)} aday | embedding {EMB.shape[1]} boyut | el-yapimi {HX.shape[1]} sutun")
    ...
    print(f"\n{'uzay':<34}{'uyusmazlik':>12}{'ayrilamaz':>11}{'tavan':>9}")
    SON = {}
    for ad, S in (("EL-YAPIMI (73 sutun)", HX),
                  ("EMBEDDING (256, havuzlanmis)", EMB),
                  ("IKISI BIRDEN", np.hstack([HX, EMB]))):
        u, a = uyusmazlik(S)
        SON[ad] = {"uyusmazlik": u, "ayrilamaz": a, "tavan": 1 - u / 2}
        print(f"{ad:<34}{u:>11.1%}{a:>11.1%}{1-u/2:>9.3f}")

    e = SON["EL-YAPIMI (73 sutun)"]["tavan"]; z = SON["EMBEDDING (256, havuzlanmis)"]["tavan"]
    print(f"\nEMBEDDING TAVANI - EL-YAPIMI TAVANI = {z-e:+.3f}")
    print()
    if z - e >= 0.02:
        print("HUKUM: TEMSIL uzayi DAHA AYRILABILIR. ~0.86 tavan EL-YAPIMI uzayin tavaniydi;")
    ...
    print("makbuz -> results/fb1_embedding_tavani.json")
