# -*- coding: utf-8 -*-
"""G5: ESIK POLITIKASI -- son ucuz kol, ve DURUST olcumu.

DIGER KOLLARDAN FARKI: G1-G4 gate'in SIRALAMASINI degistirmeye calisti (hepsi kapandi).
Esik siralamayi degistirmez, CALISMA NOKTASINI degistirir. Su anki kural:
    sec  <=>  skor >= max(0.5 x parcanin_en_yuksegi, 0.25)
Iki serbestlik: CARPAN (0.5) ve TABAN (0.25). Bir de REJIM-KOSULLU olabilirler.

SISME TUZAGI: 194 parcada esik tarayip en iyisini bildirmek OVERFIT'tir. Bu yuzden
CAPRAZ SECIM: geometri gruplarini ikiye bol, esigi A'da SEC, B'de OLC (ve tersi).
Bildirilen sayi, "esik ayarlamasi gercekte ne kadar eder" sorusunun durust cevabidir.

KILL (onceden): capraz-secilmis esik havuzlanmisi +0.005 artirmazsa VE GA sifiri
kapsiyorsa G5 kapanir. Ayrica uretici-disi ORT dusmemeli.
"""
import io
import json
...
def main():
    ...
    # --- her bolme icin gate egit, olcum kumesinde SKORLARI al (bir kez)
    def gate_kur(kp):
    ...
    for b, mk in bolmeler:
        ...
        for r in D["DER"]:
            ...
            # POSE DUZELTMESI ADAY BASINA BAGIMSIZ (`m["model"].predict(X)`), yani esikten
            # ETKILENMEZ. Bir kez TUM adaylara uygula, sonra esik yalnizca ALTKUME alsin.
            # Ilk kosuda her esik icin yeniden cagriliyordu ve tarama bitmiyordu.
            Pd_ = r["Pd"]; P_ = r["P"]
            if cfg.get("robot_pose_head"):
                cc = [{"point": P_[i], "direction": Pd_[i]} for i in range(len(P_))]
                cc = wire_gate.pose_duzelt(Xr, cc)
                P_ = np.array([x["point"] for x in cc], float)
                Pd_ = np.array([x["direction"] for x in cc], float)
            S[r["pid"]] = (wire_gate.karar_skoru(g, Xr), P_, Pd_)
        SKOR[b] = S
        print(f"  skorlar hazir: {b}", flush=True)

    def puan(b, alt, carpan, taban, carpan_cok=None, taban_cok=None):
        det = []
        for r in alt:
            ...
            t = taban_cok if (rj == "cok" and taban_cok is not None) else taban
            if v is not None:
                s, P_, Pd_ = v          # P_/Pd_ pose duzeltmesi UYGULANMIS halde onbellekte
                k = (s >= c * max(float(s.max()), 1e-9)) & (s >= t)
                if k.any():
                    # aci_duzelt / uye_yonu_sec SADECE YONU degistirir; TESPIT eslesmesi
                    # 180 derece toleransla calistigi icin sonucu DEGISTIREMEZLER -> atlanir.
                    P = P_[k]; Pd = Pd_[k]
            det.append((rj,) + esle(P, Pd, r["G"], r["Gd"], r["diag"], 0.0, 180.0, True))
        return det

    DER = D["DER"]
    # --- 1) TAM TARAMA (SISIK -- yalniz tavani gormek icin)
    en, arg = -1, None
    for c in CARPANLAR:
        for t in TABANLAR:
    ...
    for sec_alt, olc_alt in ((altA, altB), (altB, altA)):
        ...
        print(f"  yarida secilen esik {a} -> diger yaride olculdu ({len(olc_alt)} parca)")
    ...
    det_rej = []
    for sec_alt, olc_alt in ((altA, altB), (altB, altA)):
        # REJIM-KOSULLU: once dusuk-CP icin en iyi (c,t), sonra SABIT tutup cok-CP carpanini ara.
        # Tam capraz carpim 567 puanlama demekti; ardisik arama 63+9 = 72 ile ayni tepeye ulasir.
        ...
        print(f"  rejim-kosullu esik {a} secildi")
    f_rej = T.f1w(det_rej)
    print(f"REJIM-KOSULLU (DURUST) {f_rej:.4f}   ({f_rej-su_an:+.4f})")

    # --- 4) uretici-disi bedeli (tam-tarama esigiyle)
    ...
    _, lo, hi = olcum_kumesi.grup_bootstrap(list(zip(det_a, det_capraz if len(det_capraz) == len(det_a) else det_a)),
                                            [r["geo"] for r in DER], fn, n=2000)
    gecti = (f_capraz - su_an >= 0.005) and lo > 0 and np.mean(ud_ye) >= np.mean(ud_su)
    print(f"\nKILL: capraz-secim +0.005 VE GA>0 VE uretici-disi dusmesin")
    print(f"  capraz {f_capraz-su_an:+.4f} | GA[{lo:+.4f},{hi:+.4f}] | "
          f"uretici-ort {np.mean(ud_su):.4f}->{np.mean(ud_ye):.4f}")
    ...
    print("makbuz -> results/g5_esik.json")
