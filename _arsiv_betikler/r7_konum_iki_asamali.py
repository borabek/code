...
R6-A: global tanim degisikligi YOK -- tezin v_o'su uc rakibi de yendi (cember -0.0875,
      agiz -0.1001, acik -0.5432). Yani kol ancak SECICI ile yasar.
R6-B: "mevcut kotu mu" OGRENILEBILIR (grup-capraz AUC 0.8223; en yuksek %10 skorda
      kotu orani %16.4 -> %59.6).

GERIYE KALAN BILINMEYEN: iyi bir noktaya YANLISLIKLA ates ettigimizde secilen giris de
<=2mm cikiyor mu? Cikiyorsa hasar yok ve kol yasar; cikmiyorsa 1:20 asimetri kolu oldurur.
Bu SADECE UCTAN UCA olculebilir.

BU BETIK DAGITMAZ. Olcum kumesinde GRUP-CAPRAZ (5 kat, geometri grubuyla) calisir ve
uctan uca robot(FIZIKSEL)+tespit uretir. Amaci TEK bir soruyu yanitlamak:
"45 dakikalik egitim-korpusu turetmesini harcamaya deger mi?"

TARAMA UYARISI: birden cok (esik, marj) ciftini AYNI olcum kumesinde deniyorum. En iyisi
bir UST SINIRDIR, dagitilabilir tahmin degil. O yuzden ONCEDEN KAYITLI bir ayar da
ayrica raporlanir: esik=%85 yuzdelik, marj=0.15 (yon kolunun 0.05'inden BUYUK secildi
cunku buradaki hasar asimetrisi 1:20).
"""
...
KAYITLI_MARJ = 0.15       # ONCEDEN secildi (yon kolu 0.05; burada asimetri 1:20)
GIRIS = ("agiz", "cember", "acik", "kirpik", "eksen",
...
def main():
    ...
    # ---------- satirlari kur (iki asama tek gecisde)
    AX, AY, AG, AK = [], [], [], []     # asama 1: aday duzeyi ("mevcut kotu mu")
    BX, BY, BG, BK = [], [], [], []     # asama 2: (aday, giris) duzeyi ("bu giris <=2mm mi")
    for r in DER:
        h = H[r["pid"]]
        if h is None:
            continue
    ...
    r0, d0, gg, _ = kos(2.0, 9.0)      # hicbir zaman atesleme = taban
    ...
    print(f"\nKAYITLI ayar (esik %{100*KAYITLI_ESIK_Q:.0f}, marj {KAYITLI_MARJ}): "
    ...
    print(f"TARAMA en iyisi (UST SINIR): robot {eniyi['d_robot']:+.4f} "
          f"(esik %{100*eniyi['esikQ']:.0f}, marj {eniyi['marj']})")
    deger = kay["d_robot"] >= 0.01 and kay["ga"][0] > 0 and kay["d_tespit"] >= -0.005
    print(f"\nHUKUM: 45 dakikalik egitim-korpusu turetmesi "
    ...
    print("makbuz -> results/r7_konum_iki_asamali.json")
