# -*- coding: utf-8 -*-
"""A/B: TOPLULUK BILESIMI -- urunun 4 ckpt'i en iyi secim mi?

NEDEN SORULUYOR
  Urundeki 4 ckpt'in 3'u AYNI tarif (recall_hard_keig96 s0/s1/s2), biri keig64. Cesitlilik
  dusuk; oysa union'in (vote>=1) butun kazanci, farkli modellerin FARKLI acikliklari
  kacirmasindan geliyor. Ayrica `best_full.pt` segmentasyonda 68.1% IoU ile urundeki
  recall_hard_s2'yi (63.3%) 4.8 puan geciyor ve CP-F1 acisindan HIC denenmedi.

  Onemli uyari: yuksek IoU otomatik olarak yuksek CP-F1 demek DEGIL. Topluluk CP-F1'e gore
  secilmisti, IoU'ya gore degil. Bu yuzden olculmeden degistirilmez.

KOLLAR
  A) urun          : cp_config.robot_vote2_checkpoints (4 ckpt)
  B) +best_full    : urun + best_full.pt (5 ckpt, cesitlilik artar)
  C) cesitli-4     : recall_hard_s2 + best_full + human103c_s0 + adj_s0 (4 farkli tarif)

KILL (olcumden once yazildi)
  korpus-agirlikli CP-F1 katkisi < +0.02 -> reddedilir (bu gece diger kaldiraclarla ayni bar)

MALIYET: her kol icin tam cikarim gerekir (segmentasyon ciktisi degisiyor), onbellek
kullanilamaz. Bu yuzden ORNEKLEM ile kosulur ve rejim-agirlikli raporlanir.
"""
import os, sys, json
import numpy as np
...
def run_arm(cks, parts, dev):
    ...
    for mfg, pid, jf, stp, n in parts:
        try:
            ...
            probs = acc / len(per)
            thr = float(cfg.get("robot_wire_gate_threshold_highcp", 0.25)) if hi \
...
def main():
    ...
    cfg = json.load(open("cp_config.json"))
    print(f"{len(sel)} parca (30 dusuk / 18 cok)\n", flush=True)
    print(f"{'kol':<18}{'dusuk-CP':>10}{'cok-CP':>10}{'agirlikli':>12}")
    res = {}
    base = None
    for name, cks in ARMS.items():
        ...
        if missing:
            print(f"{name:<18}  ATLANDI (eksik: {[os.path.basename(m) for m in missing]})")
            continue
    ...
    if res:
        ...
        print("makbuz -> results/ab_ensemble.json")
