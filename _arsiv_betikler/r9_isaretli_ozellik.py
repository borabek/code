...
KOK NEDEN (koda bakinca gorunuyor, `d1_yon_dagit.satirla`):
    oz.append(... abs(float(v @ d0)) ...)      # ISARETSIZ
    oz.append(... abs(float(v @ UZ)) ...)      # ISARETSIZ
Sozluk BILEREK +- ciftleri iceriyor (obb/dik/yuzn). Bu iki oznitelik `+a` ve `-a` icin
BIREBIR AYNI degeri veriyor. Geriye kalan tek fark, girisin DIZIN NUMARASI (`gi`).
Yani model bugun isareti yalnizca sozlukteki siradan tahmin edebiliyor -- fiziksel
hicbir isaret bilgisi GORMUYOR.

Bugun ETIKETI isaretli yaptim (+0.0363 kazandirdi). OZNITELIKLER hala isaretsiz.
Ayni hatanin ikinci yarisi.

EKLENEN OZNITELIKLER (hepsi FIZIKSEL, hicbiri GT gormez):
    v.d0 ISARETLI          mevcut yonle ayni tarafta mi (mevcut ~%80 dogru -> guclu onsel)
    v.UZ ISARETLI          parca uzlasisiyla ayni tarafta mi
    disariliK              v ile (p - parca agirlik merkezi) arasindaki isaretli uyum
    koni sayimi +-         v yonunde 45 derecelik koni icinde MESH TEPESI sayisi, ileri/geri.
                           Agiz DIS yuzeydedir: disari bakan yonde tepe AZ (bosluk),
                           ice bakan yonde COK (govde yuzeyleri). "Hangi taraf disarisi"
                           sorusunun dogrudan fiziksel olcumu -- ve `+a`/`-a` ayrimi TAM BUDUR.
    ayni olcum 8mm ve 15mm yaricapta (yerel ve genis olcek)

Bu betik DAGITMAZ: olcum kumesinde GRUP-CAPRAZ olarak "isaretsiz oznitelikler" ile
"isaretli oznitelikler" secicilerini AYNI katlarda yaristirir. Kazanirsa R10'da egitim
...
import olcum_kumesi, thesis_remesh, wire_gate
from big_arbiter import eligible
from infer_step_cp import step_to_mesh
from sina_kume import esle, f1w
from d1_yon_dagit import KAYNAK
...
def koni(V, p, v, R):
    """v yonunde 45 derecelik koni icinde, R yaricapinda MESH TEPESI sayisi (ileri, geri)."""
    rel = V - p
...
def main():
    ...
    # ---- geometrik ek bilgi (mesh gerekiyor, R8 onbelleginde yok)
    if os.path.exists(ONB):
        GEO = pickle.load(open(ONB, "rb"))
        print(f"geo onbellekten: {len(GEO)} parca", flush=True)
    else:
        GEO = {}; t0 = time.time()
    ...
    # ---- satirlari kur: ESKI (isaretsiz) ve YENI (isaretli) oznitelik kumeleri
    EX, YX, LY, LG, LK = [], [], [], [], []
    for r in DER:
        d_ = PARCA.get(r["pid"])
        if d_ is None:
        ...
        for i in range(len(P)):
            ...
            if pe[i, b] > tol:
                continue                      # etiket yalniz eslesen adaylarda tanimli
            d0 = Pd[i]
            for gi, (tip, v) in enumerate(d_["SD"][i]):
                oh = [1.0 if tip == t2 else 0.0 for t2 in KAYNAK]
                # --- ESKI (dagitilan) oznitelikler: ikisi de abs()
                ...
                # --- YENI: ayni + ISARETLI olcumler + fiziksel disari testi
                ek = [float(v @ d0),
                      ...
                      if uzn > 0.5 else 90.0]
                ek += list(E[i][gi]) if (E is not None and i < len(E) and gi < len(E[i])) \
                    else [0.0] * 7
    ...
    print(f"ISARETLI tavan (R8): 0.6580 -> kalan alan {0.6580-b_rob:+.4f}")
    ...
    print(f"\nHUKUM: {'ISARETLI OZNITELIKLER KAZANDIRIYOR -> R10 (egitim korpusunda yeniden turet + dagit)' if deger else 'isaretli oznitelikler anlamli katki vermiyor'}")
    with io.open("results/r9_isaretli_ozellik.json", "w", encoding="utf-8") as f:
        json.dump({"taban_robot": b_rob, "taban_tespit": b_det, "tavan": 0.6580,
    ...
    print("makbuz -> results/r9_isaretli_ozellik.json")
