...
"""!!! SUPERSEDED / LEAKY -- KULLANMA. Tek-model (recall_hard_s2) + deploy-gate (held parcalarda egitilmis)
    ...
    DURUST surum = w1_gate_oof.py (4-model union + leakage-free OOF gate + nested-CV): 0.35 zaten optimal.
    Sadece tarihsel referans icin tutuluyor. Bkz memory/085-decomposition-leakage-free.md
W1 KANTITATIF: WEI held-out tahminlerini TP/FP ayir, her birinin wire_score'unu kaydet.
Karar:
 - FP'lerin wire_score'u TP'den BELIRGIN DUSUK -> gate FP'leri zaten eliyor; ham 0.58 = gate-ONCESI olcum,
   GT-eksikligi DEGIL. Urun 0.69 bu farki zaten yakaliyor. -> W1 kapan, lever = wire/tool (W4) veya recall (W2).
 - FP wire_score ~ TP (yuksek) -> ya GT-eksik (gercek listelenmemis kablo girisi) ya gate kor.
Axis-aware eslesme (mouth-vs-seat), align_frames ile mesh frame."""
import os, sys, json
import numpy as np
...
def matched_pred_idx(P, G, tol, Gdir, axis_tol):
    """big_arbiter.greedy ile ayni eslesme; matched pred-index kumesini dondur."""
...
tp_ws, fp_ws = [], []          # TP ve FP wire_score'lari
fp_hi = 0                       # yuksek-skorlu FP (gate'i gecen, >=0.35) sayisi
total_G = 0                     # toplam manufacturer CP (recall paydasi)
for m, pid, jf, stp in parts:
    ...
    if not cps: continue
    wire_gate.apply(V, F, pb, cps, CE, CT, threshold=0.0)   # sadece wire_score yaz, filtreleme
...
print(f"  gate esigi 0.35 gecen FP: {fp_hi}/{len(fp_ws)} ({100*fp_hi/max(len(fp_ws),1):.0f}%)")
THR = 0.35
...
# post-gate (0.35): sadece wire_score>=THR
...
np.savez("results/w1_wei_scores.npz", tp=tp_ws, fp=fp_ws, total_G=total_G)   # sweep icin cache
