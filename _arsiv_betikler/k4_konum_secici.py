# -*- coding: utf-8 -*-
"""K4: AYRIK KONUM SECICI + IKISI BIRLIKTE -- robot metriginin son kolu.

GEREKCE: zengin konum sozlugu KAHINI 0.6244 (yanal tavani 0.6469, kapsam %61).
Sozluk-kapsami barim (%70) kacirildi AMA o bar YANLIS ALETTIR: yon kolunda ayni sinifta
bir bar (0.70) YAPISAL OLARAK IMKANSIZ cikti ve kol yine de +0.0238 KANITLI kazanc verdi.
Dogru olcut SECICININ UCTAN UCA kazancidir ve o bar DEGISMIYOR: robot +0.01 VE GA>0.

TASARIM (C4 ile ayni ayrik mantik, konum semantigiyle):
  * her (aday, konum girisi) cifti bir SATIR
  * etiket: bu giris GT'ye YANAL olarak <=2mm mi
  * ozellikler: adayin 58 gate sutunu + girise ozgu (kaynak tipi one-hot, mevcut noktaya
    uzaklik, agiz merkezine uzaklik, sozluk-ici sira/boyut)
  * cikarim: grup-capraz OOF -> aday basina ARGMAX; MARJ esigiyle mevcut korunur
  * marj DURUST secilir (yarida sec / diger yaride olc)

SON OLCUM: KONUM secici + YON secici (C4) BIRLIKTE -- robot metriginin ulasabildigi yer.
"""
import io
import json
import os
...
KAYNAK = ["mevcut", "ham", "uye", "agiz", "kirpik", "cember", "acik",
          "kesit", "acik3", "acik5", "eksen"]
...
def main():
    ...
    if os.path.exists(ONB):
        with open(ONB, "rb") as f:
            PARCA = pickle.load(f)
        print(f"konum sozlugu onbellekten: {len(PARCA)} parca", flush=True)
    else:
        PARCA = {}
        t0 = time.time()
    ...
    for r in D["DER"]:
        ...
        for i in range(len(P)):
            ...
            for a, q in SOZ[i]:
                if a == "agiz":
                    agiz = np.asarray(q, float)
            for gi, (a, q) in enumerate(SOZ[i]):
                q = np.asarray(q, float)
                w = q - G[b]
                yan = float(np.linalg.norm(w - float(w @ Gd[b]) * Gd[b]))
                oz = [1.0 if _tip(a) == t2 else 0.0 for t2 in KAYNAK]
                oz.append(float(np.linalg.norm(q - P[i])))
                oz.append(float(np.linalg.norm(q - agiz)) if agiz is not None else 9.0)
    ...
    print(f"\negitim satiri {len(RY)} | dogru giris orani {RY.mean():.1%} | sutun {RX.shape[1]}")
    ...
    if os.path.exists("results/c4_sozluk.pkl"):
        with open("results/c4_sozluk.pkl", "rb") as f:
            YSOZ = pickle.load(f)
        import c4_yon_secici as _c4   # noqa: F401  (yalniz KAYNAK icin)
        YON = YSOZ
        print("yon sozlugu yuklendi (birlesik olcum icin)")
    ...
    for sec, olc in ((altA, altB), (altB, altA)):
        ...
        rc += r1; rt += r0; gg2 += g1
        print(f"  yarida secilen marj {ea:.2f} -> diger yaride olculdu ({len(olc)} parca)")
    fn = lambda rows: T.f1w([q for _, q in rows]) - T.f1w([p for p, _ in rows])
    _, lo, hi = olcum_kumesi.grup_bootstrap(list(zip(rt, rc)), gg2, fn, n=3000)
    ...
    with io.open("results/k4_konum_secici.json", "w", encoding="utf-8") as f:
        json.dump({"taban": T.f1w(rt), "konum_secici": T.f1w(rc), "fark": dk,
                   "ga": [lo, hi], "gecti": bool(gecti)}, f, indent=1)
    print("makbuz -> results/k4_konum_secici.json")
