# -*- coding: utf-8 -*-
"""D1 ADIL KARSILASTIRMA: iki gate de AYNI kosulda (test aileleri haric) egitilir.
Tek fark: hangi MANTIKLA uretilmis adaylarla egitildikleri.
...
Calisma anindaki adaylar HER IKI kolda da YENI mantikla uretilir (urun artik boyle calisiyor).
Boylece olculen sey saf: gate'in EGITIM dagilimi ile CALISMA dagiliminin uyusmasi ne kazandiriyor.
"""
...
def main():
    ...
    print(f"{len(sel)} test parcasi, {len(tf)} aile her IKI gate'in egitiminden de cikarildi",
          flush=True)
    ...
    for tag, npz in (("A_eski_mantik", "results/gate_regrow_data_minv4.npz"),
        ...
        gates[tag] = path
        print(f"  {tag}: {int(keep.sum())} aday ile egitildi", flush=True)
    ...
    for mfg, pid, jf, stp, n in sel:
        try:
            ...
            # CALISMA mantigi: promote'suz turet -> yonlendirici -> cok-CP ise promote'lu
            def der(pr):
    ...
    print(f"{'gate egitim dagilimi':<24}{'esik':>10}{'dusuk':>9}{'cok':>9}{'agirlikli':>11}")
    res = {}
    ...
    print("makbuz -> results/d1_adil.json")
