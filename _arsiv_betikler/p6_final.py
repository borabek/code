...
500 training parcasi, test aileleri yine disarida. AYNI duzenek -> seviye and difference birlikte gecerli.
...
KOLLAR -- all of them KENDI dagilimiyla egitilmis gate with, test aileleri egitimden CIKARILMIS:
  ...
  B = 5 uye, 13 feature            (P1 single basina -- a5_adil'i dogrular)
  C = 5 uye, 13 + RENK             (YIGIN)
  D = 4 uye, 13 + RENK             (P2 single basina)

RENK: dar rule (most iyi olculen varyant), NaN = renk absent -> -1 + eksiklik bayragi.
"""
...
def main ():
    ...
    print (f"P6 DOGRULAMA -- test {len (sel )} part (70 dusuk/30 cok) | training {len (tr_parts )}",flush =True )
