...
SORUN: sinav kumesini "cerceve saglikli" diye filtrelediginde robot F1 0.2410 -> 0.4006
cikiyor. Ama filtre olcutu MODELIN ADAYLARINA bakiyordu (GT'nin en yakin adaya mesafesi),
yani sonucun KENDISIYLE filtreliyorduk -- DAIRESEL.

DOGRU OLCUT: `cad_eval.align_frames` kendi MESH-TO-MESH residual'ini dondurur (uctan uca
GT/mesh hizalamasi, modelden TAMAMEN bagimsiz). Sabah AL vakasinda bu kullanildi: residual
0.13mm cikti ve "hizalama bozuk" hipotezi CURUTULDU.

TEZ SADAKATI: bu bir OLCUM duzeltmesidir, urun degisikligi degil. Tezin `v_o` tanimina,
5 sinifa, remesh'e dokunulmuyor. Yalnizca "hangi parcalarda GT guvenilir" sorusu MODELDEN
BAGIMSIZ yanitlanip manset o zemine oturtuluyor.
"""
...
print(f"\nMESH-TO-MESH residual ({len(R)} parca): ortanca {np.median(R):.2f}mm | "
      f"%75 {np.percentile(R,75):.2f} | %90 {np.percentile(R,90):.2f} | maks {R.max():.2f}")
for e in (0.5,1.0,2.0):
    print(f"  <{e}mm: {(R<e).sum()} parca (%{100*(R<e).mean():.0f})")

m,_=egit('results/zengin_parite_v3.npz')
print(f"\n{'katman (MODELDEN BAGIMSIZ olcut)':<34}{'parca':>7}{'tespit':>10}{'ROBOT':>9}")
...
print("makbuz -> results/r14_cerceve_bagimsiz.json")
