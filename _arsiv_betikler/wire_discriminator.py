# -*- coding: utf-8 -*-
"""DAL B (own-kosabilir): tel-girisi vs tool-agzi AYIRICI, YAPISAL ozelliklerle + otomatik etiketle.

recon: 4 global feature (size/depth/vote/conf) ayirmiyor (AUC<=0.61). Bu, FIZIKSEL/YAPISAL sinyalleri
dener: acikligin kutup-esligi (most yakin diger acikliga distance, komsu yogunlugu), disa-doğruluk (dis
kabukta mi ic govdede mi), region sekli (yuvarlak hole vs uzun yariq), region yogunlugu. Otomatik label:
manufacturer CP'ye eslesen = TEL (1), digeri = TOOL/extra (0). PARCAYA GORE held-out (leakage absent) AUC.
AUC>0.75 -> cp_openings'e genel gate. Aksi halde: insan wire/tool etiketi is required.
"""
...
def feats_for (V ,F ,probs ,cps ):
    """each CP for yapisal feature vektoru (mesh frame)."""
    ...
    for i ,c in enumerate (cps ):
        ...
        # esleme: most yakin diger CP + 12mm ici komsu count
        if len (P )>1 :
        ...
        # region sekli: yakin verteks bulutunun PCA axis oranlari (yuvarlak~1, yariq>>1)
        if near .sum ()>=6 :
            ...
            aspect =1.0 ;flat =0.0 
            # axis-boyu depth profili: p'den d yonunde 15mm ici baglanti-verteks orani
...
def main ():
    ...
    print (f"{len (parts )} part | ozellik cikarimi",flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp )in enumerate (parts ,1 ):
        try :
            ...
            # otomatik label: mesh-frame CP -> JSON frame -> mfg CP eslesme
    ...
    # PARCAYA GORE 5-fold held-out AUC (leakage absent)
