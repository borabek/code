# -*- coding: utf-8 -*-
"""R6: NOKTAYI eksene izdusurme UCTAN UCA (R5 kapi taramasindan turetildi) (kodda ASILI kalan "P2 taramasi").
...
def main():
    ...
    SONUC = {}
    for kapi in KAPILAR:
        os.environ["CP_AXIS_PROJECT"] = "1" if kapi == "izdusum" else "0"
        import cp_openings, robot_cp
        importlib.reload(cp_openings)
        assert cp_openings.USE_AXIS_PROJECT == (kapi == "izdusum"), cp_openings.USE_AXIS_PROJECT
    ...
    for kapi in KAPILAR:
        ...
        # ve DEV ile VAL AYNI YONDE (havuzlanmis sayi DEV/VAL ayrismasini gizliyordu).
        ok = (dr >= 0.01 and dt >= -0.005 and kapi != "taban"
              and (s_["robot_dev"] - t["robot_dev"]) > 0 and (s_["robot_val"] - t["robot_val"]) > 0)
        print(f"{str(kapi):<10}{s_['tespit']:>9.4f}{s_['robot']:>9.4f}{dt:>+10.4f}{dr:>+9.4f}"
              f"{s_['aci_10_alti']:>9.1%}{'GECTI' if ok else ('taban' if kapi == 'taban' else 'gecmedi'):>10}")
    ...
    print("makbuz -> results/r6_izdusum_uctan_uca.json")
