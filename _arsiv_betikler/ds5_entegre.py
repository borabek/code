...
ONEMI: korpusta STEP'i EKSIK 2120 klemens vardi ve WSCAD'den indirmek KESIN KAPALIYDI
([[wscad-indirme-kesin-kapali]]: 24 denemede 0 basari, sebep portalda 3D VERI YOKLUGU).
Bu paket o eksigin **2391 parcasini / 8410 CP'sini** kapatiyor + 403 tamamen yeni parca
(IKI YENI URETICI: WIE 280, WEG 123) getiriyor.

DOGRULANDI (entegrasyondan ONCE): 8 farkli ureticiden rastgele parca -- STEP'ler
ayristi, JSON mesh'i ile STEP mesh'inin sinir kutusu <=0.11mm ortusuyor, yani ayni parca.

ADLANDIRMA: `eligible()` STEP'i `korpus_kimlik.step_kimlik` ile okur ve
`wscaduniverse_<pid>_<zaman>.stp` kalibini bekler. DataSet5 dosyalari JSON ile AYNI ada
sahip (`A-B.1492-H4_ElectricalTerminal_ElectricalTerminal.stp`) -- oldugu gibi kopyalanirsa
kimlik "ElectricalTerminal_ElectricalTerminal" cikar. Bu yuzden KOPYALARKEN yeniden adlandirilir.

GUVENLIK: STEP'i ZATEN OLAN parca ATLANIR (kopya olusturmaz). Olcum kumesinin (194 parca)
degismedigi entegrasyondan sonra DOGRULANIR.
"""
import io
import json
import os
...
def main():
    ...
    print("makbuz -> results/ds5_entegre.json")
