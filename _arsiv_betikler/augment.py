...
def point_dropout (vertices ,drop_frac ,rng =None ,spherical =True ):
    """Randomly drop a contiguous region of vertices (occlusion / partial-scan sim).

    With spherical=True (default): pick a random "seed" vertex, grow a sphere
    of radius = drop_frac * bbox_diagonal, remove all vertices inside it.
    This mimics a real cable or housing occluding part of the scan.

    With spherical=False: drop each vertex independently with probability
    drop_frac (uniform dropout -- simulates sparse point cloud).

    Returns: (kept_indices, V_dropped) where V_dropped is the retained subset.
    The caller must slice any parallel arrays (labels, etc.) with kept_indices.
    NOTE: faces become invalid after dropout -- this is for point-cloud backbones
    (knngraph / mlp) that don't use faces; don't pass to DiffusionNet.
    """
    rng =rng or np .random .default_rng ()
    V =np .asarray (vertices ,dtype =float )
    n =len (V )
    if n ==0 or drop_frac <=0.0 :
        return np .arange (n ),V .copy ()
    if spherical :
        diag =float (np .linalg .norm (V .max (0 )-V .min (0 )))or 1.0 
        r =drop_frac *diag 
        seed_idx =int (rng .integers (n ))
        dists =np .linalg .norm (V -V [seed_idx ],axis =1 )
        keep =np .where (dists >r )[0 ]
    else :
        keep =np .where (rng .random (n )>drop_frac )[0 ]
    if len (keep )==0 :
        keep =np .array ([int (rng .integers (n ))])
    return keep ,V [keep ]


def cable_distractor (vertices ,cp_points ,cp_dirs ,frac =0.3 ,rng =None ,
cable_r_mm =1.5 ,cable_len_mm =30.0 ,n_ring =10 ):
    """Add synthetic cable cylinders occluding some CP openings (negative augmentation).

    For a random subset of CPs, insert a cable (dense ring of points extruded along
    the insertion axis) that partially blocks the view of that CP opening. The cable
    vertices are added to the mesh but the CP labels are UNCHANGED — so the model
    must learn "cable-in-hole ≠ CP absence, cable-outside-hole ≠ CP presence".

    Returns: (new_vertices, cp_points, cp_dirs) — only vertices change, labels intact.
    frac : fraction of CPs to occlude (0 = never, 1 = always occlude every CP).
    """
    rng =rng or np .random .default_rng ()
    V =np .asarray (vertices ,dtype =np .float64 )
    if len (cp_points )==0 or frac <=0.0 :
        return V ,cp_points ,cp_dirs 

    cp_arr =np .asarray (cp_points ,dtype =np .float64 )
    # nearest-OTHER-CP distance per CP, so a cable never reaches into a
    # neighbouring CP's supervision region. Measured ten the real corpus: a
    # cable extruded the full default cable_len_mm=30 along ITS OWN CP's
    # occlusion axis passes within another CP's 3*sigma mask ten 95%+ of
    # multi-CP parts (CP pairs are very often <30mm apart) -- encode_targets
    # then assigns those cable-ring vertices to the NEIGHBOUR (nearest-CP by
    # raw distance), injecting cable-clutter density near a CP that was never
    # actually occluded. Capping length to half the gap keeps each cable
    # inside its own CP's "territory" even if the neighbour is also occluded.
    if len (cp_arr )>=2 :
        d =np .linalg .norm (cp_arr [:,None ,:]-cp_arr [None ,:,:],axis =2 )
        np .fill_diagonal (d ,np .inf )
        nearest_other =d .min (axis =1 )
    else :
        nearest_other =np .full (len (cp_arr ),np .inf )

    extra_verts =[]
    for i ,(cpt ,cdir )in enumerate (zip (cp_points ,cp_dirs )):
        if rng .random ()>frac :
            continue 
        cable_len_i =min (cable_len_mm ,max (0.5 *nearest_other [i ],2.0 *cable_r_mm ))
        # Build a local frame: z-axis along the insert direction
        z =np .asarray (cdir ,dtype =np .float64 )
        ...
        x =np .cross (z ,perp );x /=np .linalg .norm (x )
        y =np .cross (z ,x )
        # ring of points at the CP opening, extruded along the cable direction.
        # STANDOFF by one cable radius so the nearest ring doesn't start exactly
        # AT the CP point (with only a radial cable_r_mm gap) -- otherwise up to
        # n_ring*n_rings distractor vertices pack densely right around the exact
        # coordinate the model most needs a clean local-geometry signal at,
        # diluting it for graph/kNN-neighbourhood backbones. Length is also
        # capped per-CP (cable_len_i) to stay clear of the nearest OTHER CP.
        n_rings =max (2 ,int (cable_len_i /3 ))
        standoff =cable_r_mm 
        for ri in range (n_rings ):
            depth =standoff +ri *cable_len_i /max (1 ,n_rings -1 )
            theta =np .linspace (0 ,2 *np .pi ,n_ring ,endpoint =False )
            ring =(np .asarray (cpt ,dtype =np .float64 )
            +cable_r_mm *np .outer (np .cos (theta ),x )
            +cable_r_mm *np .outer (np .sin (theta ),y )
            +depth *z )
            extra_verts .append (ring )

    if not extra_verts :
        return V ,cp_points ,cp_dirs 
    V_new =np .concatenate ([V ]+extra_verts ,axis =0 )
    return V_new ,cp_points ,cp_dirs 
