# -*- coding: utf-8 -*-
"""D20: MESH-ONLY KORPUS -- STEP olmadan segmentasyon egitim verisi.

`_ds1/DataSet/*.json` dosyalarinda Graphic3d (Points+Indices) VE ConnectionPoints
AYNI dosyada. STEP GEREKMEZ -- segmentasyon zaten mesh uzerinde calisiyor.

NEDEN ONEMLI: FN'lerin %57'si ADAY_YOK (temsil). Ve yanal hata SEG KALITESIYLE
aciklanıyor ([[yanal-hata-segmentasyon-kalitesiyle-aciklanir]]) -- yani bu korpus
YALNIZ tespite degil DONUSUME de aday.

TEZE SADIK: ~6000 uniform izotropik remesh (thesis_remesh.remesh_uniform),
5 sinif, `v_o` degismez. Yalniz EGITIM VERISI eklenir.

SIZINTI: sinav kumelerinin URETICILERI dislanir (D6+D7 markalari).
"""
...
import connector3d
CE = int(connector3d.CABLE_ENTRY)          # urunun kendi sinif tanimi

KAYNAK = "_ds1/DataSet"
HEDEF = "_label_ds1_obj"
# D6 + D7 sinav markalari -- korpusa GIRMEZ
...
def _mfg_norm(b):
    ...
    SIZINTI (2026-08-09, olculdu): dosya adi `A-B_N.1492-J16_...json`.
    Ham onek "A-B_N" ve yasak listede "A-B" var -> filtre DELINDI ve
    **D7'nin 84 A-B test parcasi** egitim ciktisina girdi.
    Cozum: `_`/`-` ile ayrilan ekleri soyarak her on-eki dene.
    """
    ham = b.split(".", 1)[0]
    adaylar = {ham}
    x = ham
...
def _sinav_pidleri():
    """D6 + D7 parca kimlikleri -- marka filtresinin YEDEGI (kemer + askı)."""
    import json as _j
    out = set()
    for f in ("results/d6_sinav_kumesi.json", "results/d7_sinav_kumesi.json"):
        if os.path.exists(f):
...
def _xyz(o):
    """{'X':..,'Y':..,'Z':..} -> [x,y,z]. Bu veri kumesinde noktalar SOZLUK."""
...
def cp_al(d):
    ...
    Ilk surumde "Direction"/"Normal"/"Axis" ariyordum ve hicbiri yoktu; tum
    yonler [0,0,1] varsayilanina duserdi ve SESSIZCE BOZUK etiket uretirdi.
    Yon yoksa o CP ATLANIR -- uydurma yon boyamaktansa eksik boyamak yeg.
    """
    G, Gd = [], []
...
def main():
    ...
    var = {os.path.basename(os.path.normpath(x)) for x in glob.glob(f"{HEDEF}/*/")}
    global SINAV_PID
    SINAV_PID = _sinav_pidleri()
    print(f"sinav pid yasagi: {len(SINAV_PID)} parca", flush=True)
    ...
    for i, f in enumerate(fs, 1):
        ...
        pid = b[:-5]
        # IKINCI KATMAN: parca KIMLIGI sinav kumesinde mi (marka filtresi delinirse)
        ...
        say["yazildi"] += 1
        if i % 200 == 0:
            print(f"  {i}/{len(fs)} {(time.time()-t0)/i:.2f}s/dosya | {say}", flush=True)
    print(f"\nBITTI {say}")
    json.dump(say, open("results/d20_mesh_korpus.json", "w"), indent=1)
