...
GEREKCE: gate'in reddettigi 248 DOGRU adayin ayirt edici ozelligi DUSUK OY (ortalama 1.33
vs kabul edilenlerde 1.98). Ve `_vote2`'nin kendi yorumunda yazili: "dürüst (geometri)
...
bulup noktalari 5mm'den ayri dusuyorsa, sonuc IKI ayri aday x 1 oy olur -- oysa gercek
BIR aday x 2 oy. Yani "dusuk oy" bir zayiflik isareti degil, OLCUM PARCALANMASI olabilir.

BU BETIK: her uretici CP'si icin, o CP'ye yakin duşen UYE noktalarinin YAYILIMINI olcer.
Yayilim 5mm'yi asiyorsa oy parcalaniyor demektir.

Ayrica farkli cluster_mm degerlerinde oy dagilimi nasil degisiyor, onu da verir.
Bu bir OLCUM; hicbir sey dagitilmaz.
"""
import io
import json
import os
...
def main():
    ...
    print(f"\n{len(sec)} parca | {len(models)} uye | turetme: min_v {mv} vc {vc} cluster {cl}",
          flush=True)
    ...
    for k, r in enumerate(sec, 1):
        ...
        for b in range(len(G)):
            # her uyenin bu GT'ye EN YAKIN adayi (dik mesafe, tespit toleransi icinde)
            yakin = []
            for P in UYE:
                if not len(P):
                    continue
            ...
            if len(yakin) >= 2:
                ...
                for cm in OY:
                    # bu cluster_mm ile kac KUME olusurdu (acgozlu, tek gecis)
                    kume = []
                    for p in Y:
                        h = next((q for q in kume if np.linalg.norm(p - q) <= cm), None)
                        if h is None:
                            kume.append(p)
                    OY[cm].append(len(yakin) / max(len(kume), 1))   # kume basina ort. oy

    N_UYE = np.array(N_UYE); YAY = np.array(YAY)
    print(f"\n{len(N_UYE)} uretici CP incelendi")
    ...
    print()
    print(f"{'cluster_mm':<12}{'kume basina ort. oy':>22}")
    for cm in sorted(OY):
    ...
    print("makbuz -> results/r1_oy_parcalanmasi.json")
