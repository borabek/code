# -*- coding: utf-8 -*-
"""V3: CAPRAZ-COZUNURLUK UYUMU -- niteliksel olarak YENI bilgi denemesi.

TESPIT 0.85 OPERASYONU, ikinci kol.

BUGUN OLEN UC BLOGUN ORTAK YANI: hepsi AYNI 6k mesh uzerindeki geometriden turedi
(B-rep graf, uzamsal duzen, uye zenginlestirme). Ayni kuyudan bir kova daha.

BU KOL FARKLI: dort model de AYNI mesh'te calisiyor, yani hatalari ILISKILI. Farkli bir
COZUNURLUKTE yeniden orneklemek BAGIMSIZ kanit uretir:
    gercek bir aciklik yeniden ornekleme SONRASI da bulunur;
    segmentasyon artefakti bulunmayabilir.
Bu, tezin uniform-yogunluk kuralini bozmaz -- ayni ag, ayni sinif, ayni remesh yordami,
yalnizca ikinci bir uniform hedef (9000). "Multires" daha once ADAY BIRLESIMI olarak
denenmisti (cok-CP'de +0.0283); OZELLIK olarak hic denenmedi.

OZELLIKLER (6):
    mr_var      9k'da 5mm icinde aday VAR mi (0/1)
    mr_mes      en yakin 9k adayina mesafe (yoksa 99)
    mr_aci      yonler arasi aci (yoksa 90)
    mr_conf     o adayin guveni
    mr_n        5mm icindeki 9k aday sayisi
    mr_oy       o adayin oy sayisi (_votes)

ONCE UCUZ SINAMA: yalniz olcum kumesinde (194 parca) grup-capraz OOF ile sinyal var mi?
Varsa tam korpusa yatirim yapilir.
"""
import io
import json
import os
...
def main():
    ...
    if os.path.exists(OUT):
        with open(OUT, "rb") as f:
            MR = pickle.load(f)
        print(f"onbellekten: {len(MR)} parca", flush=True)
    else:
        MR = {}
        t0 = time.time()
        for i_, r in enumerate(DER, 1):
            ...
            try:
                ...
                cps9, _, _, _ = RC.adaylari_uret(V9, F9, pbs, stp, cfg=cfg)
    ...
    print(f"9k adayi olan parca: {kap}/{len(DER)}", flush=True)

    # --- SINYAL VAR MI: eslesen (TP) vs eslesmeyen (FP) adaylarda dagilim
    ...
    print(f"\n{len(RY)} aday | pozitif {RY.mean():.1%}", flush=True)
    ...
    print(f"KARAR: {'SINYAL VAR -> tam korpusa yatirim' if a64-a58 >= 0.01 else 'SINYAL YOK -> kol OLU'}")
    ...
    print("makbuz -> results/v3_capraz_coz.json")
