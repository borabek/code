# -*- coding: utf-8 -*-
"""TEL-E: kanal ozellikleri (TEL-B) GERCEKTEN ayirt ediyor mu? KILL KRITERI uygulanir.

KURAL (plana baslarken yazildi, sonuca gore degistirilmez):
    Mevcut gate seti UZERINE ek AUC katkisi < 0.02 ise bu ozellik ailesi OLU ilan edilir
    ve oyle raporlanir. "Ise yaramadi ama biraz daha ugrasayim" dongusu bastan kapali.

OLCUM:
  1  Her kanal ozelliginin TEK BASINA ayirt edici gucu (Mann-Whitney AUC, 0.5 = kor)
  2  TABAN: mevcut 13 gate ozelligiyle aile-disi CV AUC
  3  BIRLESIK: 13 + 10 kanal ozelligi
  4  EK KATKI = birlesik - taban   (kill kriteri buna bakar)
  5  Bootstrap guven araligi -- kucuk ornekte tek sayiya guvenilmez
"""
import json
import numpy as np
...
def main():
    ...
    print(f"veri: {len(y)} aday ({int(y.sum())} TP / {int((y==0).sum())} FP), "
          f"{len(set(pid))} parca, {len(set(fam))} aile\n")
    if y.sum() < 20 or (y == 0).sum() < 20:
        print("YETERSIZ ORNEK -- olcum yapilmadi"); return
    ...
    if delta < KILL:
        ...
        print("    (Hipotez: tel girisi kontakta biter, alet agzi kol yuvasinda -- olculdu, tutmadi")
