# -*- coding: utf-8 -*-
"""A4: FIZIKSEL GECERLILIK ORANI -- KORPUS CAPINDA (194 parca), metrikten BAGIMSIZ sutun.

OTOPSIDE (3 parca) uretilen CP'lerin %40/%50/%96'si fiziksel olarak kusurluydu. O bir
ORNEKLEMDI; bu betik 194 parcanin TAMAMINDA olcer.

ALETLER A0'DA DOGRULANDI: `is_inside` bilinen geometrilerde 26/26 (yanilma %0.0),
`mouth_width` sapma %1, isin sayisi 12/24/48'de birebir ayni. Yani asagidaki bayraklar
guvenilir ve B1-B4'te RED KRITERI olabilirler.

DORT BAYRAK (hepsi tez-notr; ag/remesh/v_o'ya dokunmaz):
    govde_ici      nokta malzemenin ICINDE  -> robot oraya tel sokamaz
    onu_kapali     takma yonunde <5mm'de malzeme -> yon kanala girmiyor
    duvara_yapisik agiz ic capi kucuk -> nokta agzin ORTASINDA degil, kenarda
    eksenel        GT agzina gore |eksenel| buyuk (yalniz ESLESENLERDE tanimli)

ASIL SORU (B kolunun kaderi): kusurlu CP'ler FP mi TP mi?
  * cogunlukla FP ise -> B1-B3 kesinligi yukseltir, kol yasar
  * cogunlukla TP ise -> reddetmek recall'u cokertir, kol olur
...
ILERI_MIN = 5.0        # takma yonunde en az bu kadar bosluk olmali
IC_CAP_MIN = 0.8       # agiz ic capi (merkezlenme) -- B2'de DEV'de ayarlanacak
EKSEN_MAX = 8.0
...
def bayraklari_uret(DER, CIKTI):
    if os.path.exists(ONB):
        ...
        print(f"bayraklar onbellekten: {len(d)} parca", flush=True)
        return d
    import trimesh
...
def main():
    ...
    for r in DER:
        ...
        tp, fp, fn, bi = esle_detay(P, Pd, G, Gd, r["diag"], 0.0, 180.0, True)
        eslesen = {e[0]: (e[3], e[4]) for e in bi["eslesme"]}
        rj = "cok" if r["n"] >= 8 else "dusuk"
        for i in range(len(P)):
            b = bl[i]
            ax, ac = eslesen.get(i, (None, None))
            satir.append({"pid": r["pid"], "mfg": r.get("mfg", "?"), "rejim": rj,
                          "tp": i in eslesen,
    ...
    print(f"\n{'='*78}\nA4 -- FIZIKSEL GECERLILIK (194 parca, {n} uretilen CP)\n{'='*78}")
    print(f"{'bayrak':<18}{'sayi':>7}{'oran':>8}   {'TP icinde':>10}{'FP icinde':>11}")
    ...
    print(f"\n{'rejim':<10}{'CP':>7}{'kusurlu':>9}{'oran':>8}{'TP kusur':>10}{'FP kusur':>10}")
    for rj in ("dusuk", "cok"):
        alt = [s for s in satir if s["rejim"] == rj]
    ...
    print(f"\n{'uretici':<10}{'CP':>7}{'kusurlu':>9}{'oran':>8}")
    ...
    # --- B kolunun kaderi
    # DUZELTME (kural 9): ilk surum HAM SAYI oranini kullaniyordu (FP 73 / TP 61 = 1.20x)
    # ve "TP agirlikli, reddetme" diyordu. YANLIS: korpusta TP sayisi FP'nin 2.6 KATI,
    # dolayisiyla ham oran her bayragi TP'ye kaydirir. Dogru olcu ZENGINLESME ORANI:
    # (bayragin FP'ler icindeki ORANI) / (TP'ler icindeki ORANI). Duzeltilince uc bayrak da
    # FP-ZENGIN cikiyor (2.0x - 3.2x) ve B kolu YASIYOR.
    print(f"\n{'='*78}\nB KOLU ICIN HUKUM (ZENGINLESME ORANI)\n{'='*78}")
    print(f"  {'bayrak':<16}{'FP oran':>9}{'TP oran':>9}{'zenginlesme':>13}   hukum")
    for k in BAY:
        t = sum(1 for s in satir if s[k] and s["tp"])
        ...
        hkm = ("YAPISAL (yalniz TP'de tanimli)" if k == "eksenel_buyuk" else
               "AYIRT EDICI -- kol yasar" if z >= 1.5 else
               "ZAYIF" if z >= 1.0 else "TERS (TP'de daha sik)")
        print(f"  {k:<16}{100*rf:>8.1f}%{100*rt:>8.1f}%{z:>12.2f}x   {hkm}")
    # BIRLESIK satiri EKSENEL HARIC hesaplanir: o bayrak yalniz eslesen CP'lerde tanimli,
    # birlige katilinca oran yapisal olarak TP'ye kayar ve kolu haksiz yere olu gosterir.
