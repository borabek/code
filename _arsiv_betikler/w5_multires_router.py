...
s9 NE YAPMISTI: hedef parcalari `r["n"] >= 8` ile, yani GT'DEKI CP SAYISIYLA secti ve
yalnizca o 60 parcayi puanladi. Iki kusur:
  1. Urun calisma aninda GT rejimini BILMEZ -- `_highcp_router` geometriden tahmin eder.
     (w4: 58 dogru / 3 YANLIS-POZITIF / 2 kacan. Router'in hedefi 61 parca.)
  2. Yalniz cok-CP alt kumesinde puanlandi; yanlis yonlendirilen DUSUK-CP parcalarin
     BEDELI hic olculmedi ve genel etki 0.105 agirlikla TAHMIN edildi.

W5 DUZELTIR: kol, router'in "+" dedigi parcalara uygulanir; puanlama TUM 194 parcada yapilir
(yonlendirilmeyen parcalar iki kolda ozdestir -> katkilari dogal olarak sifirdir). Boylece
cikan sayi dogrudan MANSETLE karsilastirilabilir, agirlikla tahmin edilmez.

Gate: DAGITILAN yol (zengin_parite_w2.npz), olcum gruplari egitimden CIKARILIR.
KILL (onceden): agirlikli tespit kazanci >= +0.02 VE grup bootstrap GA'si sifiri disliyor.
"""
import io
import json
import os
...
def main():
    ...
    with io.open("results/w4_router_rejim.json", encoding="utf-8") as f:
        W4 = json.load(f)
    yonlendirilen = {s["pid"] for s in W4["satirlar"] if s["router"]}
    print(f"\nrouter '+' parca: {len(yonlendirilen)} (GT cok-CP: "
          f"{sum(1 for s in W4['satirlar'] if s['gt_cok'])})", flush=True)

    # --- GATE: dagitilan veriyle, olcum gruplari disarida
    ...
    print(f"gate: {NPZ} | {int(keep.sum())} aday egitimde", flush=True)

    def son_islem(X, P, Pd):
        """URUNUN son islem zinciri: pose + secili aci duzeltmesi."""
        cps = [{"point": P[j], "direction": Pd[j]} for j in range(len(P))]
        cps = wire_gate.pose_duzelt(X, cps)
        if cfg.get("robot_aci_secici"):
            cps = wire_gate.aci_duzelt(X, cps)
    ...
    if os.path.exists(ONBELLEK):
        with open(ONBELLEK, "rb") as f:
            K9 = pickle.load(f)
        print(f"9k onbellegi: {len(K9)} parca", flush=True)
        ...
        if r["pid"] in yonlendirilen:
            if r["pid"] not in K9:
                try:
                    ...
                    cps9, probs9, _, _ = RC.adaylari_uret(V9, F9, pbs9, stp, cfg=cfg)
                    if cps9:
            ...
            if veri is not None:
                X9, P9a, Pd9a = veri
                k9 = wire_gate.karar_maskesi(wire_gate.karar_skoru(gate, X9))
                if k9.any():
    ...
    _, lo, hi = olcum_kumesi.grup_bootstrap(list(zip(det["A"], det["B"])), grup, fn, n=4000)
    d = f1w(det["B"]) - f1w(det["A"])
    gecti = (d >= 0.02) and lo > 0
    print(f"\ntespit AGIRLIKLI fark {d:+.4f}  GA[{lo:+.4f},{hi:+.4f}]"
    ...
    if idx:
        ...
        print(f"yanlis yonlendirilen {len(idx)} dusuk-CP parca: {ba:.4f} -> {bb:.4f} "
