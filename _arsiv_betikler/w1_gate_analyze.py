# -*- coding: utf-8 -*-
"""DURUST per-mfg gate esik degerlendirmesi: grouped 5-fold CV (esik train-fold'da secilir, test-fold'da
olculur -> in-sample iyimserlik YOK). Karsilastir: (a) mevcut global 0.35, (b) CV-secilmis per-mfg esik,
(c) in-sample optimal (ust-sinir referans). results/gate_scores.json'dan (w1_gate_collect ciktisi)."""
...
def totalG(rs):
    return sum({r["pid"]: r["N"] for r in rs}.values())   # her parca N'i bir kez
...
def cv_eval(rs, K=5):
    """grouped K-fold: esik train'de sec, test'te uygula; TP/FP/FN test-fold'lar boyunca topla."""
...
for mfg in ["WEI", "PXC", "ALL"]:
    ...
    print(f"\n=== {mfg} ({len({r['pid'] for r in rs})} parca, {totalG(rs)} GT) ===")
    print(f"  global 0.35      : P={g35[0]:.3f} R={g35[1]:.3f} F1={g35[2]:.3f}")
    print(f"  CV per-mfg esik  : P={cvp:.3f} R={cvr:.3f} F1={cvf:.3f}  (CV-median thr={cvthr:.2f}, folds={[round(c,2) for c in chosen]})")
