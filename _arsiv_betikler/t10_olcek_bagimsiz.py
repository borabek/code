# -*- coding: utf-8 -*-
"""T10: mutlak mm sutunlarini OLCEK-BAGIMSIZ yapmak transferi duzeltir mi?

T9 OLCTU (uretici imzasi = sutun tek basina "hangi uretici" sorusunu ne kadar cevapliyor):
    bos_derinlik 0.641 | ce_frac 0.620 | depth 0.612 | flat 0.604 | size 0.603 | brep_r 0.584
    guvenli cekirdek: nn_dist 0.504, n_close 0.513, esesenli 0.515, nverts 0.520
Imzali 4 sutunu ATMAK en kotu durumu 0.2799 -> 0.3121 yapiyor (+0.032) ama ATMAK bilgiyi de
goturuyor -- L2 dersi: ezberci damgasi tek basina atmak icin yeterli degil.

HIPOTEZ: `size`, `depth`, `nn_dist`, `bos_derinlik` MUTLAK mm. Bir ureticinin klemensleri
tipik olarak belli boyutta oldugu icin bu sutunlar ureticiyi ELE VERIYOR. Parca capina
bolununce IMZA silinir, ORAN bilgisi kalir.

KARSI HIPOTEZ (ayni derecede ciddi): mutlak buyukluk bazen GERCEKTEN dogru olcektir -- tel capi
ureticiden bagimsiz olarak 1-3mm'dir. T1 zaten fiziksel sutunlarin transferi +0.047 IYILESTIRDIGINI
olctu. O yuzden `brep_r`'ye DOKUNULMUYOR; yalniz parca-olcegine bagli olanlar normalize ediliyor.

UC KOL: (a) mevcut, (b) normalize EDILMIS (yerine koy), (c) HER IKISI (model secsin).
KILL: bir kol, uretici-disi EN KOTU durumda mevcudu gecmezse ve tanidik veride 0.02'den fazla
...
def main():
    ...
    # parca capi: mesh onbelleginden; yoksa o parcanin adaylarinin yayilimindan (yedek)
    cap = {}
    ...
    print(f"parca capi: {sum(1 for p in np.unique(pids) if os.path.exists(f'results/mesh_cache/{p}.npz'))}"
    ...
    def olc(M, ad):
        ...
        for u in sorted(set(mfg)):
            ...
            mv.append(f1(s, y[te]))
        print(f"{ad:<30}{tan:>9.4f}" + "".join(f"{x:>10.4f}" for x in mv) + f"{min(mv):>10.4f}")
        return {"tanidik": tan, "uretici": mv, "en_kotu": min(mv)}

    U = sorted(set(mfg))
    print(f"\n{'kol':<30}{'TANIDIK':>9}" + "".join(f"{'uret.' + u:>10}" for u in U) + f"{'EN KOTU':>10}")
    out = {}
    ...
    for ad, v in out.items():
        if ad == "mevcut (mutlak)":
            continue
        gecti = v["en_kotu"] > t["en_kotu"] and (t["tanidik"] - v["tanidik"]) <= 0.02
        print(f"  {ad:<24} en kotu {v['en_kotu']-t['en_kotu']:+.4f} | tanidik "
              f"{v['tanidik']-t['tanidik']:+.4f} -> {'GECTI' if gecti else 'GECMEDI'}")
    json.dump(out, open("results/t10_olcek_bagimsiz.json", "w"), indent=1)
    print("makbuz -> results/t10_olcek_bagimsiz.json")
