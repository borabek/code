# -*- coding: utf-8 -*-
"""Q8: RENK tel girisi ile vidayi ayiriyor mu? -- KARAR OLCUMU.

Q7 "OLCULEMEDI" dedi: silindir MERKEZLERINI eslestirmeye calisiyordum, oysa metnin
AXIS2_PLACEMENT_3D noktasi ile bizim cember-oturtma noktamiz ayni eksen uzerinde FARKLI
noktalar; hizalama %12'de kaldi.

COZUM: hizalamaya hic gerek yok. `step_face_colors.read_by_order` rengi metinden (yuz sirasi),
geometriyi OCC'den (KURESEL koordinat) alir. Sira hipotezi PARCA BASINA dogrulanir: her iki
taraf da silindir yaricapini verir, eleman eleman karsilastirilir (8 parcada 7'si tam).
Dogrulamayan parca ATLANIR -- varsayim korpus genelinde kabul edilmez.

OZELLIKLER (her aday icin):
  c_metal       : adayin oturdugu silindirin rengi gumusi metal tonunda mi
  c_govde       : rengi metal-disi (govde) mi
  kanalda_metal : ayni eksen cizgisi uzerindeki silindirlerden biri metal mi (kanalin dibi)
  metal_mesafe  : en yakin metal yuze uzaklik (mm)

KILL (onceden yazili, degismedi): null'un uzerinde AUC >= 0.60 veren renk ozelligi yoksa RENK
tel/vida ayrimi icin OLU. Verirse listeye kaldirac olarak girer ve gate'e ozellik olarak eklenir.
"""
...
def main():
    ...
    for r in parts:
        try:
            ...
            red += 1; continue
        if not ok:
            red += 1; continue           # sira dogrulanmadi -> parca ATLANIR
        cyl = [f for f in faces if f["axis"] is not None and f["rgb"] is not None]
        if len(cyl) < 2:
            red += 1; continue          # renk cozulemedi (STYLED_ITEM yuze degil katiya bagli)
        kabul += 1
    ...
    print(f"sira DOGRULANAN parca: {kabul} | atlanan: {red}")
    if kabul < 8 or len(Y) < 200:
        print("\nOLCULEMEDI (yetersiz parca/aday) -- 'renk ise yaramaz' DEMEK DEGILDIR.")
    ...
    json.dump(out | {"kabul": kabul, "red": red, "n": len(Y),
              ...
              open("results/q8_renk_karar.json", "w"), indent=1)
    print("makbuz -> results/q8_renk_karar.json")
