...
R1a olctu: kanal ekseni dikleri yalniz %13 kurtariyor -> dikler KARE/YAY giris (kanal yok).
Kare bir aciklikta giris yonu = ACIKLIK DUZLEMININ NORMALI. Sonda: aday cevresindeki yerel
yuzey yamasina PCA -> en kucuk ozdegerin vektoru (duzlem normali) vs GT ekseni.
Ayrica iki yaricap denenir (3mm/6mm) -- agiz kenari mi genis yama mi daha iyi.
"""
...
def normal_pca(pid,p,ry):
    if pid not in VC:
        V,F=step_to_mesh(yol[pid]); V,_=thesis_remesh.remesh_uniform(V,F,target=6000)
    ...
    return vv[:,0]
for ad,kume in (("DIK",dik),("duz-kontrol",duz)):
    sec=[kume[i] for i in rng.permutation(len(kume))[:50]]
    for ry in (3.0,6.0):
        n=k10=0
        for pid,p,gd in sec:
        ...
        print(f"  {ad:<12} r={ry}mm  olculen {n:>3}  duzlem normali GT'yi buluyor: %{100*k10/max(n,1):.0f}")
