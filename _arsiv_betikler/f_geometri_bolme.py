# -*- coding: utf-8 -*-
"""F: "aile-disi" dedigimiz koruma GERCEKTE ne kadar koruyor?

json_dataset.family_key bu korpusta parca numarasinin KENDISINI donduruyor (1542 parca ->
1542 "aile"). Bu bir hata degil, belgelenmis bir sinir: parca numarasi tamamen rakamsa soymak
o ureticinin TUM urunlerini tek gruba cokertirdi. Docstring cozumu de yaziyor: `geometry_key`.

NEDEN ONEMLI: kardes varyantlarin geometrisi BIREBIR ayni (olculdu: eksen farki 0.000 derece,
hizalama residual 0.3mm -- PXC 3211813/14/19, SIE 3RV2011-1AA15/-4AA10). Yani "test aileleri
gate egitiminden cikarildi" derken gercekte yalniz test PARCALARI cikarilmis; ikizleri egitimde
kalmis olabilir. Bu, tum "sizintisiz" iddialarimizin ne kadar korumali oldugunu belirler.

OLCUM: ayni gate verisi, ayni protokol, TEK degisken = gruplama anahtari.
  kol A: family_key   (= parca no; bugune kadarki "aile-disi")
  kol B: geometry_key (bbox boyutlari yuvarlanmis + kose/yuz sayisi log2 kovalanmis)
Fark ne kadar buyukse, eski sayilar o kadar iyimserdi.
"""
...
def main():
    ...
    print(f"{len(y)} aday / {len(uniq)} parca", flush=True)

    # geometri anahtari: mesh onbelleginden (varsa), yoksa STEP'ten
    gk = {}
    ...
    a = run(fams, "family_key (= parca no, ESKI)")
    ...
    print("makbuz -> results/f_geometri_bolme.json")
