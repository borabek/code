...
OTOPSI BULGUSU (2026-08-04): metrik three yerden kordu.
  ...
  A2  BELIRSIZ ESLESME -- GT'lerin %29.3'unun kabul kutusunda BASKA a GT present
      (low-CP %20.8, very-CP %33.0). "Dogru" sayilan eslesme komsu delige ait may be.
  A3  ACI -- tespit olcutu aciya TAMAMEN KOR (am=180). 180 derece TERS a CP tabloda
      "correct" gorunuyor.
...
def main ():
    ...
    print ("\n"+"="*78 )
    print ("A3 -- ACI GORUNURLUGU (tespit eslesmelerinin ACI dagilimi, ISARETSIZ axis)")
    print ("="*78 )
    _ ,aci ,_ ,_ =olc (15.0 )
    if len (aci ):
        ...
        for threshold in (10 ,30 ,60 ,90 ):
            print (f"  aci > {threshold :>2} derece olan eslesme: {int ((aci >threshold ).sum ())}/{len (aci )}"
