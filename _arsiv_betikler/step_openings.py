...
# Label-time face-merge ceiling. Kept FINER than the decoder's NMS ceiling on
# purpose (4.0 vs 5.0mm) -- coarsening labels to match the decoder would merge
# 37 pairs of REAL terminals (measured on the human GT, 8 parts with >100mm
...
def auto_radius_bands(radii, fillet_floor=1.3, split_gap=1.0, rcap=8.0,
    ...
    real entry-size classes at once (e.g. NEOZED 3048357: 2.6mm wire bores AND
...
def extract_cylinders(path, include_cones=True):
    ...
    include_cones: conical faces are funnel lead-ins above wire-entry bores on
    ...
    bore's opening instead of losing the entry entirely on parts where only the
...
def _planar_pocket_openings(planes, bbox, existing_ops, merge_tol, approach_dim=2):
    ...
    cosmetic edge rounds). Requiring walls on BOTH perpendicular axes (so they
    actually bound a rectangle, not a single flat lip) + a slot-sized footprint
    is what separates a genuine opening from housing planar faces. Verified on
    ...
    existing_pts = np.array([o["entry_point"] for o in existing_ops], float) \
        if existing_ops else np.zeros((0, 3))
    ops = []
    for members in groups.values():
        # must have walls facing on BOTH perp axes -> they bound a rectangle
        ...
        if len(existing_pts) and \
                np.linalg.norm(existing_pts - c, axis=1).min() < merge_tol:
            continue
...
def _dir_sign(c, ad, bbox):
    """Insert-direction sign along axis `ad` for an opening at centre `c`: a hole
...
def validate_directions(ops, V, part_nr="?"):
    ...
    for o in ops:
        ...
        # flip only on a CLEAR verdict (20% relative margin) -- near-ties keep
        # the heuristic sign rather than thrashing on tessellation noise
        if d_in > 1.2 * d_out:
...
def _pair_slot_halves(holes, merge_tol):
    ...
    opening. Measured on real PXC push-in blocks (3211813/3211822): each wire
...
def _rect_openings(cyls, rmin, bbox, existing_ops, merge_tol):
    ...
    Measured on PXC.3031238 (both GT CPs missed by the cylinder path): the GT
    ...
    # Corner fillets of REAL push-in wire slots are small (measured r=0.5 on
    # PXC.3031238); the 0.7-1.2mm rounds are housing cosmetics whose clusters
    # produced 9 false openings on PXC.3211813 before this band was tightened.
    ...
    for members in groups.values():
        ...
        # clusters that flooded the cylinder-only parts (10 of 14 FPs were on
        # PXC.3211813/3211822, which have NO rect slots) while keeping the genuine
        # 4-corner slot on PXC.3031238. Geometrically principled, not part-specific.
...
def mouth_coord(members, ad, sign, bbox):
    """Coordinate of an opening's MOUTH along its approach axis `ad`.
    ...
    extreme over the merged members lands on the outermost (lead-in) face.
...
def plausible_openings(ops, topk=2, radius_buckets=2, part_nr="?"):
    ...
    Two rules, both derived from what a terminal block IS. Neither is tuned on the
    ...
    1. DIRECTION, top-K. A wire goes in through ONE face -- or two, on a
       ...
       parts (37.9%) carry "connection points" on FOUR OR MORE faces, and those
    ...
    classes anyway (the 134-CP part's junk holes are 9mm deep on average).
    ...
    # face rule, never to all of them: mounting and screw holes on the minor faces skew
...
def openings_from_step(path, rmin=1.3, rmax=3.0, merge_tol=4.0, auto=False,
    ...
    group is a clear majority -- real wire entries on a terminal strip share one
    face/axis, while side/bottom-facing cylinders (DIN-rail latches, test
    points) are false positives; measured on PXC.3211813 the sideways
    ...
    double-count one real hole as two at eval time, on the same small part)."""
    ...
    # ground truth with no error anywhere. Warn (once per file) so it's noticeable
    ...
    for h in holes:
        ...
        if deep_axis:
            ...
            # majority: on fuse blocks the wire bore (deep) merges with several
            # shallow cavity/LED faces and the count vote picks the wrong axis
            # -- the measured ang=90 failure on ST 4-HESI*/PTME (RESULTS 11k #2).
        ...
        # earlier mean-of-hole-centres proxy silently flipped signs on parts with
        ...
        if entry_at_mouth:
            ...
            # human ABB ground truth marks the opening's mouth. Measured on the real
            ...
            # the DEEPEST member's -- so on a funnel+bore merge it overshot past the
            ...
            # the surface; taking the extreme over members lands on the outermost
    ...
    if not ops:
        return []
    # opt-in FP filter: real wire entries on a terminal strip share one dominant
    ...
    # that cannot be wire entries -- holes on the part's minor faces, and holes whose
...
def write_labeled_corpus(source, out_dir, rmin=1.3, rmax=3.0, merge_tol=MERGE_CEILING_MM,
    ...
    config relies on MUST be usable at label time too, or the corpus teaches the
