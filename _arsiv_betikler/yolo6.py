...
#
# The thesis uses the official meituan/YOLOv6 repo, trained ten the 6 grayscale
...
# detection (projected), and crop back to 3D.
    ...
    fixed =load_yolo_predictions (tmp .name ,res =256 )# remap ten (default)
    ...
    assert len (crops_fixed )==2 and all (ap_recall (c )>0 for c in crops_fixed ),"with remap, exactly 2 real snap-point crops must be produced"
    assert not (len (crops_buggy )==2 and all (ap_recall (c )>0 for c in crops_buggy )),"without remap, 2 real snap-point crops must NOT be produced (the bug)"
