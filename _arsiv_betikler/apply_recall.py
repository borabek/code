...
WHY: two adjudication rounds proved precision is ~98%; the remaining error is RECALL, worst on
...
everything else on the part stays 0 and is handled by the masked BCE (only supervises the connection
channel), exactly like _label_targets.

LEAKAGE: these 27 parts live in the WEI arbiter TEST set. Training on them means they must be excluded
...
OUT = "_label_targets_recall"  # WEI turu; PXC icin --out ile ayrilir
...
def main():
    ...
    for pid, verts in sorted(by_part.items()):
        ...
        if len(verts) < a.min_region:
            print(f"  {pid}: {len(verts)} vertex < esik, atlandi"); skip += 1; continue
        if pid not in step:
            print(f"  {pid}: STEP yok, atlandi"); skip += 1; continue
    ...
    print(f"\n{done} parca etiketlendi, {skip} atlandi -> {OUT}/")
    print(f"  bu {done} WEI parcasi artik EGITIMDE -> hakemde WEI test seti {done} parca kuculecek")
