# -*- coding: utf-8 -*-
"""R1b: yayilim EKSENEL mi YANAL mi? Cozumu bu belirler.

YANAL ise  -> cluster_mm buyutmek gerekir AMA komsu kutuplari birlestirir (adim 3.5-6mm) = tehlikeli
EKSENEL ise-> cozum EKSEN-FARKINDALIKLI oy havuzlamasi: dik mesafe kucukse ayni aciklik say,
              derinlik farkini YOK SAY. (big_arbiter puanlayicisi ZATEN boyle calisiyor;
              _vote2 ise duz Oklid kullaniyor -- urunun kendi icinde tutarsizlik.)
"""
import io, json, os, sys, time
import numpy as np, torch
os.environ.setdefault("BA_ALLOW_SEEN", "1"); sys.path.insert(0, ".")
import cp_openings, diffusionnet as D_, olcum_kumesi, thesis_remesh
    ...
    for b in range(len(G)):
        ...
        for a in range(len(Y)):
            for c in range(a+1,len(Y)):
                w=Y[a]-Y[c]
                EKS.append(abs(float(w@d)))                       # eksen boyunca
                YAN.append(float(np.linalg.norm(w-(w@d)*d)))      # eksene DIK
YAN=np.array(YAN); EKS=np.array(EKS)
print(f"\n{len(YAN)} uye cifti (ayni GT'yi bulan)")
print(f"{'olcu':<22}{'medyan':>9}{'%75':>9}{'%90':>9}{'>5mm':>8}")
print(f"{'EKSENEL (derinlik)':<22}{np.median(EKS):>9.2f}{np.percentile(EKS,75):>9.2f}{np.percentile(EKS,90):>9.2f}{float((EKS>5).mean()):>8.1%}")
print(f"{'YANAL (dik)':<22}{np.median(YAN):>9.2f}{np.percentile(YAN,75):>9.2f}{np.percentile(YAN,90):>9.2f}{float((YAN>5).mean()):>8.1%}")
print()
print(f"EKSEN-FARKINDALIKLI kume (yanal<=3mm, derinlik SERBEST) ile birlesecek cift: {float((YAN<=3.0).mean()):.1%}")
print(f"DUZ OKLID 5mm ile birlesen cift                                            : {float((np.sqrt(YAN**2+EKS**2)<=5.0).mean()):.1%}")
...
print("makbuz -> results/r1b_eksenel_yanal.json")
