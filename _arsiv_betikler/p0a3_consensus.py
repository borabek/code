# -*- coding: utf-8 -*-
"""P0-a3: KONSENSUS-EKSEN hipotezinin KESIN testi (P0-a2'de TP yon-yogunlugu 0.636 vs FP 0.308 cikti).
IKI TUZAK var, ikisini de kapatiyorum:
 (T1) resultant length KUCUK ORNEKLEMDE SISER (TP/parca ~2.9, FP ~16.7) -> BOYUT-ESLESMIS null sart.
 (T2) 'sinyal var' yetmez; 13 feature'in USTUNE EKLIYOR mu (gate zaten dolayli yakaliyor olabilir).
Deploy-edilebilir feature'lar (etiket GEREKMEZ, inference'ta hesaplanabilir):
  f1 cos(dir_i, sayim-baskin eksen)   f2 cos(dir_i, gate-skor-agirlikli konsensus)
  f3 kendi eksen-kumemin buyuklugu (pay)   f4 kendi kumemin ortalama gate skoru
KARAR: (a) boyut-eslesmis null'i geciyor mu, (b) 13-feature AUC'sine ne katiyor."""
import json, sys
import numpy as np
...
print(f"=== T1: TP yon-yogunlugu, BOYUT-ESLESMIS null ({len(obs)} parca) ===")
print(f"  TP resultant      : {obs.mean():.3f}")
print(f"  ayni-boyut RASGELE: {nul.mean():.3f}")
print(f"  GERCEK FAZLA      : {obs.mean()-nul.mean():+.3f} | parcalarin %{100*np.mean(obs>nul):.0f}'inde TP daha yogun")
t1 = obs.mean() - nul.mean()
print(("  -> GERCEK: wire-CP'ler rasgele adaylardan DAHA COK ortak eksen paylasiyor" if t1 > 0.05
...
for gi, (pid, v) in enumerate(d.items()):
    ...
    dom = np.bincount(lab).argmax()                       # sayim-baskin eksen kumesi
...
print(f"\n=== T2: 13 feature'in USTUNE KATIYOR MU ({len(Y)} aday, {int(Y.sum())} TP) ===")
print(f"  tek-feature: cos(dominant eksen)  AUC {roc_auc_score(Y, Fx[:,0]):.4f}  | kume-payi AUC {roc_auc_score(Y, Fx[:,1]):.4f}")
a13, _ = oof_auc(X, "13 feature (mevcut gate)")
a17, _ = oof_auc(np.hstack([X, Fx]), "13 + 4 konsensus-eksen feature")
print(f"\n  KAZANC: {a17-a13:+.4f} AUC")
print("  -> P0-c'ye DEVAM (gercek katki, F1'e cevir)" if a17 - a13 > 0.01
      else "  -> KATKI YOK: 13 feature bu bilgiyi zaten tasiyor; eksen kolu KAPANIR (durust)")
