...
def yeni_uye_probs (cluster ,cache ):
    """Yalniz YENI uye for olasilik uret -> _probs_<cluster>_k128.pkl (part kimligine according to devam)."""
    import torch 
...
def main ():
    ...
    json .dump ({"cluster":cluster ,"tespit":{k :float (f1w (v [0 ]))for k ,v in R .items ()},
