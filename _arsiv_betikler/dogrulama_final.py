# -*- coding: utf-8 -*-
"""E + I sonrasi TAM BORU HATTI dogrulamasi -- yeni manset sayisi.

NEDEN AYRI BIR BETIK: `tolerans_gercegi.py` gate'i npz'den 13 ozellikle kuruyor ve onbellegi
I'dan ONCE uretilmisti. Yeni urun iki degisiklik tasiyor:
  I -- silindir susunca DUZLEM ciftlerinden eksen (cp_openings icinde, step_path ile)
  E -- gate'e 4 EKSEN GUVENI ozelligi (17 ozellik) + dusuk-CP esigi 0.40 -> 0.45
Ikisi de ancak gercek hatta birlikte olculebilir.

SIZINTI: gate test AILELERI cikarilarak egitilir; esikler BUYUK veride ayni sekilde secilir.
(Uyari: bu korpusta family_key parca numarasinin kendisi -- bkz YOL_075_ROBOT.md F maddesi.
...
def main():
    ...
    print(f"{len(sel)} test parca | gate {int(keep.sum())} aday x 17 ozellik", flush=True)

    # esikler: buyuk veride, test aileleri disarida, 17 ozellikle
    ...
    for mfg, pid, jf, stp, n in sel:
        try:
            ...
            der = lambda pr: [cp_openings.connection_points(
                ...
                conn_promote=pr, step_path=stp) for pb in pbs]        # <- I burada
            ...
            if cps:
                ...
                X13c = X17c[:, :13]        # ayni adaylar, ESKI ozellik kumesi -> E izole edilir
    ...
    # IKI GATE, AYNI ADAYLAR: E'nin uctan uca katkisi boylece IZOLE olur.
    ...
    print()
    print(f"{'kol':<24}{'tespit':>9}{'yanal2':>9}{'ROBOT':>9}{'oran':>8}{'>15d':>8}")
    res = {}
    for lab, mdl, fk, TH in (("13 ozellik (E YOK)", clf13, "X13", thr13),
        ...
        res[lab] = dict(tespit=d_[0], yanal2=l_[0], robot=r_[0], oran=r_[0]/max(d_[0],1e-9),
                        dusuk=r_[1], cok=r_[2], precision=d_[3], recall=d_[4],
    ...
    print("makbuz -> results/dogrulama_final.json")
