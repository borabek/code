# -*- coding: utf-8 -*-
"""T13: icbukey edge topolojisi TP/FP ayirt ediyor mu? (madde 6, birinci gate)
...
def main ():
    ...
    for cluster in ("dev","val"):
        ...
        for i ,r in enumerate (cache ,1 ):
            ...
            step_path =r ["stp"])[:,:18 ]
            onb =topo_feats .icbukey_kenarlar (V ,F )# part basina BIR times
            ...
            for k in range (len (P )):
                X18 .append (Xg [k ])
                XT .append (topo_feats .topo_ozellik (V ,F ,P [k ],D [k ],cache =onb ))
    ...
    print (f"\nARTIMLI: AUC {a2 -a1 :+.4f} | candidate-duzeyi F1 {f2_ -f1_ :+.4f}")
    ...
    print ("receipt -> results/t13_topo_auc.json (+ .npz)")
