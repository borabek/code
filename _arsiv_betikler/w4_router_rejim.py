...
SORUN: cok-CP multires kolu (s9) hedef parcalari `r["n"] >= 8` ile secmisti -- yani
GT'DEKI CP SAYISIYLA. Urun calisma aninda bunu BILMEZ; rejimi `robot_cp._highcp_router`
geometriden tahmin eder. Dolayisiyla s9'un "+0.0283 cok-CP" sayisi MUKEMMEL REJIM BILGISI
varsayan bir TAVAN'dir, dagitilabilir bir kazanc degil.

BU BETIK: router'i 194 olcum parcasinda URUNUN KENDI YOLUNDAN calistirir (adaylari_uret ->
n_union -> _highcp_router) ve GT rejimiyle karsilastirir. Cikan karsiliklilik tablosu
multires kolunun gercek uygulama kumesini verir:
    router+ & GT cok  -> kazanc buraya gelir
    router+ & GT dusuk-> BEDEL buraya gelir (s9 bu parcalari hic olcmedi)
    router- & GT cok   -> kazanc KACIRILIR

Cikti: results/w4_router_rejim.json (parca basina karar + karsiliklilik)
"""
import io
import json
import os
...
def main():
    ...
    if os.path.exists(ONBELLEK):
        with open(ONBELLEK, "rb") as f:
            K = pickle.load(f)
        print(f"onbellekten {len(K)} parca", flush=True)
    ...
    if kalan:
        ...
        for i_, r in enumerate(kalan, 1):
            ...
            try:
                ...
                cps, _, is_hi, _ = RC.adaylari_uret(V, F, pbs, stp, cfg=cfg)
                K[r["pid"]] = {"router": bool(is_hi), "n_cand": len(cps)}
            except Exception as e:
                K[r["pid"]] = {"router": None, "hata": f"{type(e).__name__}: {e}"}
        with open(ONBELLEK, "wb") as f:
            pickle.dump(K, f)
    ...
    print(f"\nROUTER vs GT REJIM ({len(sat)} parca)")
    ...
    print(f"\n  kesinlik {kes:.3f} | recall {rec:.3f} | dogruluk {(tp+tn)/max(len(sat),1):.3f}")
    print(f"  s9 hedefi (GT cok-CP)      : {tp+fn} parca")
    print(f"  URUNUN hedefi (router +)   : {tp+fp} parca "
    ...
    with io.open(OUT, "w", encoding="utf-8") as f:
        json.dump({"karsiliklilik": {"tp": tp, "fp": fp, "fn": fn, "tn": tn},
                   "kesinlik": kes, "recall": rec, "satirlar": sat,
