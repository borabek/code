...
S2 umut verdi: dik sinifin 107 noktasindan 74'unun 6mm icinde bir yarik adayi var ve 26'sinin
yonu GT'ye <=10 derece (medyan 26 derece, mevcut 90 derece yerine).

AMA BU YETMEZ. Calisma aninda hangi CP'nin yonunun bozuk oldugunu BILEMEYIZ; kural HERKESE
uygulanir. Parca-ici eksen uzlasisi tam bu yuzden olmustu: dik noktalarda kazaniyordu ama
dogru olanlari bozunca NET -110 cikmisti.

Bu yuzden burada TUM eslesmelere uygulanip DUZELEN ve BOZULAN birlikte sayiliyor.

KURALLAR (hepsi calisma aninda uygulanabilir -- GT'ye BAKMAZ):
  R1: 6mm icinde yarik varsa HER ZAMAN yarigin yonunu al
  R2: yalniz mevcut yon ile yarik yonu 45 dereceden fazla AYRISIYORSA al
  R3: ayni, esik 60 derece

KILL (onceden yazili): net kazanc, eslesmelerin en az %2'si olmali (robot-hazirda ~+0.013).
Altindaysa uctan uca olcum bile yapilmaz -- bu kosuda aday duzeyi ALTI kez yaniltti.
"""
import collections
import json
import os
...
def main():
    ...
    print(f"{toplam} eslesme | dik sinif {n_dik} | {len(HEDEF)} parca", flush=True)
    ...
    if not gecti and en_iyi_ad:
        print(f"  en iyi kol {en_iyi_ad}: {en_iyi/max(toplam,1):+.2%} (bar %{bar*100:.0f})")
    ...
    print("makbuz -> results/s3_yarik_net.json")
