...
`kon_cevre` = icbukey kenarlarin eksen etrafindaki acisal kapsamasi, R yaricapi icinde. R'nin
kendisi hic taranmadi: cok kucukse agiz cemberi kacar, cok buyukse komsu deliklerin kenarlari
karisir. Aday duzeyinde ucuz taranir.

KILL (onceden yazili, BUYUKLUK dahil -- bu gecenin dersi): bir R, mevcut 6.0'i aday-duzeyi
artimli F1'de EN AZ +0.01 gecmezse tam korpus yeniden uretilmez. (Rejim-oran kolu "artmali"
dedigim icin +0.0018 ile teknik olarak gecmisti; artik buyukluk sart.)
"""
    ...
    for kume in ("dev", "val"):
        ...
        for i, r in enumerate(cache, 1):
            ...
            Xg = wire_gate.feats_for(V, F, sum(plist) / len(plist), cps, CE, CT,
                                     step_path=r["stp"])[:, :18]
            onb = topo_feats.icbukey_kenarlar(V, F)     # kenar yapisi BIR kez
            ...
            for k in range(len(P)):
                ...
                for R in R_LISTE:
                    XR[R].append(topo_feats.topo_ozellik(V, F, P[k], D[k], R=R, onbellek=onb))
