...
NEDEN ONCE BU: "hayatta kalan FP'ler TP'lerle geometrik olarak ozdes" yargisi O GUNKU ozelliklerle
verilmisti. Ne olduklarini GORMEDEN yeni ozellik icat etmek bu projede uc kez olu kaldiraca yol acti.
Once teshis, sonra ozellik.
...
SORULAR:
  ...
  S4  FP'ler birbirine benziyor mu (tek tip mi, yoksa farkli sebepler mi)?
"""
import json
import numpy as np
...
def main():
    ...
    print(f"calisma noktasi: esik {thr}  F1 {f1:.4f}  |  TP {n_tp}  HAYATTA KALAN FP {n_fp}\n")
    ...
    print(f"S1  FP'li parca {len(per)}/{parts_scored}  |  parca basi ort {n_fp/max(parts_scored,1):.2f}")
    print("    en cok FP ureten 8 parca:", ", ".join(f"{k}({v})" for k, v in per.most_common(8)))
    hi = sum(v for k, v in per.items() if v >= 3)
    print(f"    FP'lerin %{100*hi/max(n_fp,1):.0f}'i, 3+ FP ureten parcalardan geliyor "
          f"({sum(1 for v in per.values() if v>=3)} parca)\n")

    # --- S2: FP'ler TP'lerin yaninda mi (alet agzi hipotezi) --------------------------------------
    d_fp, d_tp = [], []
    for q in set(PID[keep]):
        ...
        for i, p_ in enumerate(tp_pos):                 # TP'nin en yakin DIGER TP'ye uzakligi (kiyas)
    ...
    # --- S3: mevcut ozelliklerden hangisi zaten ayirt ediyor --------------------------------------
    ...
    print("makbuz -> results/tel_a_fp_teshis.json")
