...
Yapilanlar:
 1) FRAME dogrulama: P (pozisyon) ve direction hangi frame'de? Karistiran yer var mi?
 2) AILE: STEP PRODUCT adindan (PXC'de gercek urun-tipi; WEI'de parca-no -> prefix fallback)
 3) GEOMETRY-HASH: JSON bbox + nokta sayisi -> tekrar/duplike parcalar
 4) SPLIT'leri sabitle: part-out / geometry-out / family-out
 5) KILITLI HOLDOUT: ailelerin ~%20'si, deterministik, BIR KEZ acilacak (P7'ye kadar dokunulmaz)
 6) Baseline dogrulama girdileri (rich_feats.npz ile eslestir)
 7) candidate-recall vs oracle-F1 ayrimini raporla
Cikti: results/split_lock.json (tek dogruluk kaynagi)"""
...
def step_product(stp):
    """STEP dosyasindan ilk PRODUCT adi (buyuk dosyada bastan sinirli okuma)."""
    try:
...
def norm_family(pid, prod):
    """Aile anahtari: PRODUCT adi bilgilendiriciyse onu (varyant eki soyularak), degilse parca-no prefix'i.
    'bilgilendirici degil' = PRODUCT adi parca numarasinin kendisi / tamamen rakam."""
    p = (prod or "").strip()
    digits_only = p.replace(".", "").replace("-", "").isdigit()
    if p and not digits_only and pid not in p:
        # varyant eki soy: sondaki '-<sayi>' veya '_<sayi>' gruplarini tek seferde
...
def main():
    ...
    print(f"[P0] {len(parts)} parca ({sum(1 for m,*_ in parts if m=='WEI')} WEI + {sum(1 for m,*_ in parts if m=='PXC')} PXC)", flush=True)
    ...
    print(f"\n[P0-2/3] {len(rows)} parca | {len(fams)} AILE (ort {fsz.mean():.2f}, max {fsz.max()}, "
          f"tek-parcali %{100*np.mean(fsz==1):.0f}) | {len(geos)} GEOMETRI (duplike-grup {int((gsz>1).sum())}, "
          f"duplike parca {int(gsz[gsz>1].sum())})")
    prod_informative = sum(1 for r in rows.values() if r["family"].startswith("prod:"))
    print(f"  PRODUCT-tabanli aile: {prod_informative} parca | parca-no fallback: {len(rows)-prod_informative}")
    for mfg in ("WEI", "PXC"):
        ...
        print(f"  {mfg}: {len(sub)} parca -> {len(f2)} aile (ort {s2.mean():.2f}, max {s2.max()})")

    # 5) KILITLI HOLDOUT: ailelerin ~%20'si, deterministik hash ile (seed'e bagli degil, tekrarlanabilir)
    ...
    print(f"\n[P0-5] KILITLI HOLDOUT (aile bazli, deterministik): {len(locked_f)} aile / {len(locked_parts)} parca "
          f"({lw} WEI + {len(locked_parts)-lw} PXC)")
    print(f"        CALISMA seti: {len(work_parts)} parca ({ww} WEI + {len(work_parts)-ww} PXC)")
    print(f"        -> P7'ye kadar KILITLI parcalar hicbir egitim/esik/secim kararinda KULLANILMAZ.")

    out = {"note": "FAZ2/P0 split kilidi. locked_parts P7'de TEK KEZ acilir.",
           "created": "2026-07-28", "n_parts": len(rows),
           "family_source": "STEP PRODUCT adi (varyant eki soyulmus); parca-no ise json_dataset.family_key fallback",
