...
HEDEF: tespit 0.7584 -> 0.85 (+0.092) | robot-hazir 0.5893 -> 0.70 (+0.111)

SORULAR (hepsi mevcut onbellekten, YENI CIKARIM YOK):
  A. TESPIT bosluğu: kahin gate ile tavan ne? Boslugun ne kadari FP-ATMA, ne kadari FN-KURTARMA?
  B. ROBOT bosluğu: TESPIT EDILMIS (eslesen) noktalarin kaci YANAL'dan, kaci EKSEN'den,
     kaci IKISINDEN birden dusuyor? -- 0.111 nereden gelecek, bunu bu sayi soyler.
  C. ROBOT KAHINI: her eslesen nokta icin UYE havuzundaki EN IYI yon secilebilseydi
     robot-hazir kac olurdu? (uye_yonu_sec'in tavani)
  D. Tespit tavani ile robot tavani ARASINDAKI bagimlilik: gate mukemmel olsaydi robot kac?
"""
import io
import json
import os
...
def agirlikli(det):
    """rejim -> (tp,fp,fn) toplami -> agirlikli F1."""
    A = {}
...
def main():
    ...
    for r in DER:
        ...
        # --- ADAY <-> GT eslesmesi (yanal mesafe, +-40mm eksenel pencere)
        diff = P[:, None, :] - G[None, :, :]
        ...
        # URUNUN DUZELTME ZINCIRI (manset.py:208-216 ile AYNI). Bunsuz olculen sey dagitilan
        # urun degil, HAM aday havuzudur -- ilk kosuda robot 0.4472 cikti, manset 0.5893.
        if k.any() and cfg.get("robot_pose_head", False):
            _c = [{"point": P[i], "direction": Pd[i]} for i in np.where(k)[0]]
            _c = wire_gate.pose_duzelt(X58[k], _c)
            if cfg.get("robot_aci_secici"):
                _c = wire_gate.aci_duzelt(X58[k], _c)
            if cfg.get("robot_uye_secici") and r.get("UYE"):
                _c = wire_gate.uye_yonu_sec(X58[k], _c, r["UYE"])
            P = P.copy(); Pd = Pd.copy()
        ...
        # --- C: YON KAHINI (yanal urunun, aci uye havuzunun EN IYISI)
    ...
    print(f"  kahin TP {tp_k} -> gate'in KACIRDIGI dogru aday: {tp_k - tp_u}")
    ...
    print(f"  yanal <=2mm     {y_ok.mean():6.1%}   (medyan {np.median(ya):.2f}mm)")
    ...
    print(f"  yanal <=2 VE aci-kahin <=10: {(y_ok&(ak<=10)).mean():6.1%}")

    print(f"\n{'='*66}\nC/D. ROBOT TAVANLARI")
    print(f"  urun                         {agirlikli(R_urun):.4f}")
    print(f"  + mukemmel YON secimi        {agirlikli(R_kahin_yon):.4f}")
    print(f"  + mukemmel GATE (yon dahil)  {agirlikli(R_kahin_gate):.4f}")
