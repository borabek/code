...
BULGU (2026-07-31): geometrik ikizler bolmeden cikarilinca gate precision'i 0.917 -> 0.630
dustu. Bu, gate'in "tel mi alet mi" sorusunu ogrenmek yerine buyuk olcude PARCA GEOMETRISINI
ezberledigini gosterir: ikiz egitimde oldugu surece test parcasinin cevabini zaten biliyor.

OLCUM: her ozellik icin, o ozellik TEK BASINA ne kadar tasiyor -- ve ezber bolmesi ile durust
bolme arasindaki farki. Fark buyukse o ozellik EZBERLETIYOR demektir.
Ayrica: modeli KUCULTMEK (daha sig agac) durust bolmede yardim eder mi?
"""
import os, sys, json
import numpy as np
...
def main():
    ...
    print(f"{len(y)} aday, {X.shape[1]} ozellik, TP orani %{100*y.mean():.1f}\n", flush=True)

    def auc(o):
        # Mann-Whitney (bag duzeltmeli) -- kisa-yol AUC dengesiz veride SISIYOR (auc-formula-bug)
