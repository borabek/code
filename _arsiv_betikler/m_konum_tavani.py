# -*- coding: utf-8 -*-
"""M: KONUM ve EKSEN icin ORACLE tavanlari -- madde 9/10'u kurmadan once.

SORU: konum refiner (madde 9) ve coklu-hipotez B-rep snap (madde 10) kurulursa robot-hazir
nereye cikabilir? Otopsi "kusursuz gate + kusursuz eksen = 0.8136" olctu ama KONUMU kusursuz
varsaymadi. Eger konum da kusursuzken tavan 0.75'in altindaysa, o iki madde 0.75'i ACAMAZ ve
bunu KURMADAN once bilmek gerekir.

YONTEM -- ORACLE (GT kullanir, ust sinirdir, urun degildir):
  konum kusursuz : her aday, esleseceği GT'nin EKSEN CIZGISINE dik olarak tasinir -> yanal 0
  eksen kusursuz : aday yonu GT yonune esitlenir
Esleme gevsek olcutle (tespit) yapilir; boylece "zaten bulunmus" adaylarin kalitesi olculur.
Bulunamamis GT'ler (recall kaybi) HICBIR oracle ile kurtarilmaz -- ayri satirda gosterilir.

Bu bir KALDIRAC DEGIL, bir TAVAN olcumudur. Sonucu: madde 9/10'a yatirim yapilir mi.
"""
...
def main():
    ...
    print(f"gate {int(keep.sum())} aday x {Xk.shape[1]} ozellik | esikler {THR}", flush=True)

    # URUN hatti ile aday uret (parite: robot_cp'nin kendi fonksiyonlari)
    DER = []
    for r in cache:
    ...
    print(f"{len(DER)} parca | gate sonrasi aday "
    ...
    def score(fix_pos=False, fix_axis=False, tol=2.0, am=10.0, pct=False):
        ...
        for r in DER:
            ...
            G, Gd = r["G"], r["Gd"]
            if len(P) and len(G):
                # ORACLE: once GEVSEK olcutle esle, sonra o esin GT'sine gore duzelt
                diff = P[:, None, :] - G[None, :, :]
                ...
                for a_, b_ in pair.items():
                    if fix_pos:      # GT eksen cizgisine DIK olarak tasi -> yanal hata 0
                        rel = P[a_] - G[b_]
    ...
    print(f"\n{'TESPIT (ayni adaylar, gevsek olcut)':<38}{det:>13.4f}{dl:>9.4f}{dh:>9.4f}")
    print("  ^ bu satir aday+gate kalitesinin tavani: hicbir konum/eksen duzeltmesi bunu asamaz")
    base = res["MEVCUT"]
    print(f"\nMADDE 9/10'un ORACLE degeri : {res['+ KONUM kusursuz (oracle, madde 9/10)']-base:+.4f}")
    print(f"MADDE 8'in (eksen) oracle degeri: {res['+ EKSEN kusursuz (oracle)']-base:+.4f}")
    print(f"IKISI birlikte                  : {res['+ IKISI de kusursuz']-base:+.4f}")
    print(f"\nSONUC: konum+eksen KUSURSUZ olsa bile robot-hazir tavani "
    ...
    print("makbuz -> results/m_konum_tavani.json")
