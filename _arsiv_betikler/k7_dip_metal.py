# -*- coding: utf-8 -*-
"""K7.2 + K7.3 -- `dip_metal`: kanalin dibinde METAL var mi, ve bu TP/FP'yi ayiriyor mu?

K7.1 (cozuldu): STEP metninden yuzey->renk bagi (108/108), gercek sinir koseleri, ve
`cad_eval.align_frames` ile mesh cercevesine TAM hizalama (residual 0.000mm, %100 icinde).

HIPOTEZ (tezden): tel girisi bir KONTAKTA biter, alet agzi bir kol/yayda. Kanalin dibinde
metal gorunmesi, geometrinin veremedigi FONKSIYONEL sinyaldir.
Dolayli kanit: `dip_CT` (segmentasyon tahmini) TP'de medyan 0.43, FP'de 0.22 -- yon dogru ama
girdisi gurultulu tahmin. Renk TAHMIN DEGIL, CAD'in kendi verisi.

KAPI (olcumden once yazildi): AUC >= 0.70. `dip_CT` zaten 0.61 veriyor; renk bunu belirgin
gecmezse yeni bilgi getirmiyor demektir ve K7 burada durur.

ONBELLEK MESRU: bu bir GATE ozelligi (gate union sonrasi calisir), dolayisiyla
`single-derivation-proxy-invalid` kurali bunu kapsamaz -- o kural ADAY OLUSTURMA kararlari icin.
"""
...
def dip_features(mp_all, metal_pts, point, direction, axis_r=2.5, depth=14.0):
    """CP agzindan ICERIYE dogru: en yakin metal ne kadar derinde, ve yakinda ne kadar metal var.

    Doner: (dip_metal_dist, metal_in_channel, metal_frac_6mm)
      dip_metal_dist   : eksen boyunca iceride ilk metal noktasina uzaklik (yoksa depth)
      metal_in_channel : kanal silindiri icinde metal nokta VAR mi (0/1)
      metal_frac_6mm   : 6mm kure icindeki noktalarin metal orani (NaN = hic nokta yok)
    """
    ...
    if len(metal_pts):
        rel = metal_pts - p
        al = rel @ (-d)                       # ICERIYE dogru (agiz normali disari bakar)
...
def auc(pos, neg):
    """Mann-Whitney U tabanli AUC -- BAGLARA ve DENGESIZLIGE dayanikli.

    ONCEKI SURUM YANLISTI: `(ortalama_rank_farki)/n + 0.5` yalnizca siniflar DENGELI oldugunda
    dogru sonuc verir. Dengesiz kumelerde sisiyor -- olculdu: TP orani 0.176 / FP orani 0.051
    olan ikili bir ozellige 0.953 dedi (dogrusu 0.563). Ikili ozelliklerde AUC bagli siralari
    dogru islemek zorunda, bu yuzden 'average' rank kullaniliyor."""
...
def main():
    ...
    for f in sorted(glob.glob(f"{CACHE}/*.npz")):
        ...
        try:
            ...
            if res > 1.0:
                n_skip += 1; continue          # hizalama guvenilir degil -> ATLA (uydurma yok)
            allm = (allp - t) @ R
            ...
            n_skip += 1
            continue
    print(f"{n_ok} parca islendi, {n_skip} atlandi\n")
    ...
    print("makbuz -> results/k7_dip_metal.json")
