# -*- coding: utf-8 -*-
"""R1: ROBOTUN ACI ARTIGI -- listenin son kalemi.

x1 teshisi: tespit edilen 873 ciftin %80.3'u iki olcutu de geciyor.
    yalniz YANAL dusen  %6.4
    yalniz ACI  dusen   %8.0
    ikisi de dusen      %5.3
Uye havuzunun ACI kahini yalnizca +%2.3 birakiyor -> uye seciciyi zaten bosalttik.

BU BETIK: kalan aci hatalarinin NE OLDUGUNU soyler.
  1. Hata dagilimi: 10-30 derece mi (kucuk sapma) yoksa ~90 derece mi (YANLIS EKSEN sinifi)?
  2. ~90 derece olanlar hafizadaki "uretici yonu silindir eksenine DIK" sinifi mi?
  3. Bu sinif TAHMIN EDILEBILIR mi (ozniteliklerden ayirt edilebiliyor mu)? Edilebiliyorsa
     ogrenilmis bir "dik mi" kafasi yazilabilir; edilemiyorsa kol kapanir.

Ayrica ROBOT TAVANI'ni sayiyla verir: aci MUKEMMEL olsa robot kac olurdu, yanal da
...
def main():
    ...
    print(f"  kucuk sapma (10-30 derece): {int(((ACI>10)&(ACI<30)).sum())}")
    ...
    print(f"  yanal mukemmel -> robot ~ {0.7584*a_ok.mean():.4f}")

    # --- 'YANLIS EKSEN' sinifi OZNITELIKLERDEN tahmin edilebilir mi?
    ...
    print("makbuz -> results/r1_aci_artigi.json")
