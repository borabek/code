# -*- coding: utf-8 -*-
"""0.85'E ITME (gece 2. hamle, makine-only). Zengin gate 0.783'e getirdi (parca-out) / 0.763 (aile-out).
Kazanan blok KONUM'du -> o damari derinlestir + siniflandiriciyi guclendir.
YENI BAGLAM feature'lari (POS'tan turetilir, etiketsiz, inference'ta hesaplanabilir):
  E1 PCA-eksenleri: aday-bulutunun ana eksenlerinde u,v,w koordinati (ray yonu = en uzun eksen)  [3]
  E2 her eksende parca-ici RANK (normalize)                                                     [3]
  E3 merkeze uzaklik + en yakin komsu mesafeleri (1/2/3. komsu)                                  [4]
  E4 cok-olcekli yerel yogunluk (5/10/20mm icindeki aday sayisi, normalize)                      [3]
  E5 AYNA-ES: bulut merkezine gore simetrik esi var mi + mesafesi                                [2]
SINIFLANDIRICI: RF vs HistGradientBoosting vs ortalama-ensemble.
Hepsi PARCA-out + AILE-out olculur (dogrulanmis kazanc = aile-out'ta da duran)."""
import numpy as np
...
BASE = np.hstack([X13, XR[:, 0:33]])                   # kazanan set: 13 + konum + cok-yaricap
...
BE = np.hstack([BASE, E])
print(f"{len(Y)} aday | {len(ngt_of)} parca | BASE {BASE.shape[1]}d | +baglam {BE.shape[1]}d\n")
for split_nm, groups in (("PARCA-out", G), ("AILE-out (KATI)", FG)):
    ...
    print()
print("KIYAS: canonical base 0.750 | gece-1 zengin gate: parca-out 0.7826 / aile-out 0.7628")
