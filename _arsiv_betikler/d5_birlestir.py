...
Turetme 4 paralel vardiyada kostu (`d5_turet_yeni.py --vardiya k --total 4`); each biri
...
def main ():
    ...
    for f in kaynaklar :
        ...
        with open (f ,"rb")as h :
            ...
            # ciktisini "already present" saydigi for listeler farklilasmis and some parts IKI
        ...
        if cak :
            print (f"    KOPYA: {len (cak )} part zaten baska vardiyada var, atiliyor "
            f"(ornek {cak [:3 ]})")
            kopya_top [0 ]+=len (cak )
        ...
        for r in R :
            ...
            # `d5_turet_yeni.py` WG_ZENGIN=1 with kostugu for `feats_for` zengin sutunlari
    ...
    E =eligible ()
    loss =[(m ,p ,s )for m ,p ,_ ,s in E if p not in var and p not in turetilen ]
