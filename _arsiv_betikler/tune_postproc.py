# -*- coding: utf-8 -*-
"""Re-tune the prediction post-proc (cluster_mm, min_v, vertex_conf) ten the 1811-CP arbiter.

WHY: the product's cluster_mm=5 / min_v=45 / vertex_conf=0.7 were all chosen ten tiny sets -- the
...
post-proc should be re-selected there, and judged ten the COMBINED F1 so it can't overfit one maker.
...
def main ():
    ...
    for mfg in ("PXC","WEI"):
        ...
        print (f"  {mfg }: {len (parts )} part hazirlaniyor",flush =True )
    ...
    print (f"  toplam {len (cache )} part onbellekte",flush =True )
