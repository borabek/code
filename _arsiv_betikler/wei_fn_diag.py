...
"""WEI kacirma TESHISI: recall neden ~0.55'te takili? Iki mekanizma ayirt et:
  (A) SEGMENTASYON kacisi -- model kacan uretici CP'nin yakininda HIC baglanti-verteksi ateslemi_yor
      -> yalnizca VERI cozer (etiket).
  (B) TURETME kacisi -- model ATESLIYOR (baglanti-verteksleri var) ama v_o/kumeleme/tolerans yanlis
      -> POST-PROC/derivation BEDAVA cozebilir.
Her WEI held-out parcasi icin: prob'lari al, urun CP'lerini turet, uretici G'ye axis-aware esle;
her FN (eslesmeyen G) icin, G'nin (mesh frame'e tasinmis) 8mm cevresinde kac CE/CT verteks var +
...
def main():
    ...
    print(f"{len(raw)} parca | {os.path.basename(a.ckpt)} | FN teshisi (yaricap {a.radius}mm)", flush=True)

    # FN kovalari: her FN icin (fires, nverts_near, maxp_near)
    seg_miss = 0      # model kor: yakinda ~0 baglanti-verteksi (A)
    derive_miss = 0   # model gordu: baglanti-verteksleri var ama CP olmadi (B)
    ...
    for k, (mfg, pid, jf, stp) in enumerate(raw, 1):
        try:
            ...
            Gm = (G - t) @ R
            conn_p = probs[:, CE] + probs[:, CT]          # her verteks baglanti olasiligi
            is_conn = np.isin(lab, (CE, CT))
            tol = max(3.0, 0.06 * float(np.linalg.norm(Vj.max(0) - Vj.min(0))))
            # axis-aware esleme ile TP/FN indekslerini bul
            # greedy sadece sayilari donuyor -> burada FN'leri kendimiz belirleyelim
            if len(P):
                diff = P[:, None, :] - G[None, :, :]
            ...
            for gi in range(len(G)):
                ...
                if nconn >= 5:       # model o acikligi ateslemis -> turetme kacisi
                    derive_miss += 1
                else:
                    seg_miss += 1
        except Exception:
