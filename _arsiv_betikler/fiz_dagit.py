...
GEREKCE -- uclu bolme kuralini gecti (`results/q6_fiz_uctan_uca_{dev,val}.json`):

    kol                     DEV tespit   VAL tespit
    rt2 13 (dagitilan)        0.7089       0.6417
    guncel hat, 13 sutun      0.6938       0.6477
    guncel hat, 18 sutun      0.7567       0.7326
...
VERI etkisi (fiz13-rt2) DEV -0.0147 / VAL +0.0057 -- ZIT ISARET, iki GA da sifiri iceriyor
-> "guncel hat kaybettiriyor" hipotezi COKTU, kazanc ozelliklerden geliyor.

Karar kumesinde kazandi, AYRIK sinav kumesinde dogrulandi, dort olcum de belirgin. Bu gece olen
bes kolun (J, L2, gate v6, 5. uye, oy olcegi) hicbiri bu esikten gecmemisti.

GERI ALMA TEK ADIM: results/wire_gate.pkl.pre_fiz -> results/wire_gate.pkl ve
...
def main():
    ...
    print(f"egitim: {X.shape[0]} aday x 18 sutun | pozitif {y.mean():.4f}")
    ...
    pickle.dump({
        ...
        "note": ("FIZ 2026-07-31: 13 + 5 B-rep FIZIKSEL ozellik (brep_r, esesenli, r_orani, "
                 ...
                 "brep_axes._fit_circle duzeltmesine BAGIMLI (yaricap oncesinde 3.5 kat "
