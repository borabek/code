    ...
    for i in range(len(SOZ)):
        ...
        for gi, (tip, v) in enumerate(SOZ[i]):
            ...
            if hedef_yon is not None:
                g = hedef_yon[i]
                # ISARETLI ETIKET: yon, ureticinin InsertDirection'iyla AYNI tarafa bakmali.
                # Onceki surum abs() kullaniyordu -> 180 derece ters bir yon "dogru" sayiliyor,
                # secici de onu secebiliyordu. Sonuc: EKSEN metrigi +0.0319 ama FIZIKSEL
                # (isaretli) metrik 0.5818 -> 0.5813, yani kazanc SAHTEYDI. Bu satir o hatayi
                # kapatir: isaret KORUNUR.
...
def main():
    ...
    with io.open("results/s2_yon_isaretli.json", "w", encoding="utf-8") as f:
        json.dump({"robot_once": T.f1w(rob0), "robot_sonra": T.f1w(rob1), "fark": d,
                  ...
                  f, indent=1)
    print("makbuz -> results/s2_yon_isaretli.json")
