...
"basarili" der. Kaynak dosyanin own yorumunda this full as yasanmis:
...
def dogrula (beklenen =None ,HEDEF =HEDEF_VARSAYILAN ):
    ...
    if beklenen is not None and len (R )<beklenen :
        ...
        f"SESSIZ DUSME: {beklenen } part yazildi ama load_extra {len (R )} okudu. "
...
if __name__ =="__main__":
    ...
    var =len ([x for x in os .listdir (_a .src_ )if x .endswith (".npz")])
    print (f"cevrildi {n } | zaten vardi {atl } | kaynak npz {var }")
    okunan =dogrula (beklenen =n +atl ,HEDEF =_a .hedef )
    print (f"DOGRULANDI: {okunan } part egitime hazir -> {_a .hedef }")
