# -*- coding: utf-8 -*-
"""N: GERCEK kilitli measurement kumesi kur (denetim maddesi 4).
...
def strict_key (pid ,stp ,mesh_cache ="results/mesh_cache"):
    ...
    try :
        ...
        # radius histogrami: 0.5mm kovalar, first 12 kova
...
def main ():
    ...
    print (f"{len (parts )} uygun part",flush =True )
    ...
    # ZEHIRLI PARCA: gmsh single a STEP'te asilabiliyor (this gece two times yasandi).
    # Islenmeden ONCE name yazilir; yeniden baslatmada orada duran part atlanir.
    skip =set ()
    ...
    print (f"temiz pool: {len (temiz )} part ({len (tlo )} dusuk / {len (thi )} cok-CP)")
    ...
    "eski_kume_kirliligi":f"{kirli }/100 part segmentasyon geometrisiyle ayni grupta",
    },open (OUT ,"w"),indent =1 )
    print (f"\nKILITLI KUME: {len (test )} part ({nlo } dusuk / {nhi } cok-CP), "
