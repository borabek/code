# -*- coding: utf-8 -*-
"""V1: TESPIT 0.85 OPERASYONU -- acilis teshisi (nerede duruyoruz, tavan nerede).

Hedef 0.85. Su an 0.7538. Fark +0.096.

Bu betik operasyonun ILK adimi: hedefin hangi rejimden gelmesi GEREKTIGINI ve o rejimlerin
TAVANLARININ nerede oldugunu olcer. Plan yapmadan once aritmetigi masaya koymak, bu
arastirmada defalarca zaman kazandirdi.

OLCULEN (rejim basina: dusuk-CP %89.5 / cok-CP %10.5):
    1. SU ANKI F1
    2. KAHIN GATE F1        -- mevcut adaylardan mukemmel secim
    3. ADAY RECALL          -- GT'nin yuzde kaci bir adayla ortuluyor
    4. ADAY F1 TAVANI       -- 2R/(1+R), yani FP=0 varsayimiyla ulasilabilecek en yuksek

ve 0.85'in HANGI rejim kombinasyonlariyla mumkun oldugunu cozer.
"""
import io
import json
import os
...
def main():
    import olcum_kumesi
    import wire_gate
    from sina_kume import W, esle, f1_rejim, f1w
    ...
    for r in DER:
        ...
        # KAHIN GATE: GT ile eslesen adaylarin TAMAMI, baskasi yok
        ...
        # ADAY TAVANI: her eslesen GT TP, kalani FN, FP=0
        tavan.append((rj, tp, 0, len(G) - tp))
        rec[rj][0] += tp; rec[rj][1] += len(G)

    print(f"{'olcu':<24}{'dusuk-CP':>10}{'cok-CP':>10}{'AGIRLIKLI':>12}")
    S = {}
    for ad, rows in (("1 SU AN", su), ("2 KAHIN GATE", kahin), ("4 ADAY TAVANI", tavan)):
        rr = f1_rejim(rows)
        S[ad] = rr
    ...
    # cok-CP tavanda olsa, dusuk-CP ne olmali?
    ...
    print(f"\n  MUTLAK UST SINIR (iki rejim de kendi ADAY tavaninda): {ust:.4f}")
    ...
    print("makbuz -> results/v1_tespit_085.json")
