# -*- coding: utf-8 -*-
"""CC-E adim 2: geometriden rejim tahmini + YONLENDIRILMIS CP-F1.

SORU: robot bilinmeyen bir parcada 'bu cok-CP' diyebilir mi? Diyebilirse conn_promote'u SADECE
orada acar; diyemezse promote uygulanamaz (kuresel uygulama -0.0087 ile ZARARLI).

KIYAS NOKTALARI (ayni 759 temiz parca, ayni protokol):
  her yerde mv10          0.8101   <- mevcut en iyi
  ORACLE yonlendirme      0.8177   <- gercek CP sayisini BILSEYDIK (ulasilamaz ust sinir)
  yonlendirilmis          ?        <- sadece geometriden

Yonlendirici GIRDILERI (hepsi cikarim aninda elde var, metadata YOK):
  parca bbox boyutlari/kosegen/hacim + promote'suz aday sayisi ve yogunlugu.
Yonlendirici AILE-DISI egitilir -- test ailesindeki parcalar egitimde gorulmez.
"""
import json
import numpy as np
...
HIGH_CP = 8
W_HIGH = 0.105          # gercek korpusta cok-CP orani
...
def scored_f1(y, oof, pid, G, NGT, mask, seed_split=0):
    """Ic-ice esikli F1 (esik test ailelerinden ogrenilmez)."""
...
def main():
    ...
    B = load("results/rich_promote.npz")       # promote ACIK
    ...
    print(f"ortak+geometrili skor kumesi {len(common)} parca (gercek cok-CP {len(hi_true)})\n")

    # ---- YONLENDIRICI: geometri + promote'suz aday istatistigi -> 'cok-CP mi' ----
    Xa, ya, pida, _, _ = A
    ...
    print(f"YONLENDIRICI (geometri->cok-CP), aile-disi AUC = {roc_auc_score(yr, pr):.4f}")
    for thr in (0.3, 0.4, 0.5, 0.6):
        pred = pr >= thr
        tp = int((pred & (yr == 1)).sum()); fp = int((pred & (yr == 0)).sum())
        fn = int((~pred & (yr == 1)).sum())
        print(f"   esik {thr}: dogru cok-CP {tp}/{int(yr.sum())}, yanlis isaret {fp}, kacan {fn}")
    print()

    # ---- her iki kol icin OOF skorlari ----
    out = {}
    res = {}
    ...
    def eval_route(route_hi):
        """route_hi: cok-CP diye ISARETLENEN parca kumesi -> orada promote ACIK, digerinde KAPALI."""
    ...
    def weighted(route_hi):
        """Rejim ayrimli + korpus agirlikli (duz ortalama cok-CP'yi fazla temsil eder)."""
        vals = {}
    ...
    for thr in (0.3, 0.4, 0.5, 0.6):
        scen[f"geometri yonlendirme (esik {thr})"] = {pids[i] for i in range(len(pids)) if pr[i] >= thr}
