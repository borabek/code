# -*- coding: utf-8 -*-
"""R9b: R9'un TABAN HATASINI duzelt -- iki oznitelik kumesini AYNI baslangictan yaristir.

R9 HATASI: satirlari `_r8_sozluk.pkl`'den kurdum; oradaki `Pd` ZATEN dagitilan yon
secicisinin ciktisi (`yon_uygula`). Yani olctugum sey "dagitilan secicinin USTUNE ikinci
bir secici" idi. Geriye pay kalmamasi dogaldi ve ESKI/YENI karsilastirmasi ANLAMSIZDI.

DUZELTME: satirlar `_r4_sozluk.pkl`'den kurulur -- oradaki `Pd` secici ONCESI yondur
(taban 0.5818) ve `SY` o yone gore kurulmus sozluktur. Iki oznitelik kumesi de AYNI
baslangictan, AYNI katlarda yarisir.

DURUST SERH (sonucu gormeden yaziliyor): buradaki egitim verisi olcum kumesinin kendi
satirlaridir (~27k, grup-capraz). Dagitilan secici 154k satirla egitildi. Yani bu testte
ESKI kolun da dagitilan surumden ZAYIF cikmasi BEKLENIR; karsilastirma ESKI-vs-YENI
olarak okunmalidir, mutlak degerler olarak degil.

KILL: YENI - ESKI >= +0.01 ise R10 (egitim korpusunda yeniden turetme, ~45 dk) HAK EDILIR.
"""
...
import olcum_kumesi, thesis_remesh
...
def satirlari_kur(DER, PARCA, GEO):
    """(EX, YX, LY, LG, LK) -- ESKI ve YENI oznitelik matrisleri, AYNI satirlar."""
    EX, YX, LY, LG, LK = [], [], [], [], []
    for r in DER:
        d_ = PARCA.get(r["pid"])
        if d_ is None:
...
def main():
    ...
    print(f"\nTABAN (secici ONCESI): robot(FIZ) {b_rob:.4f} | tespit {b_det:.4f}")
    print(f"dagitilan secici bu tabandan 0.6181'e cikardi (+{0.6181-b_rob:.4f}); "
          f"ISARETLI tavan 0.6580")
    ...
    print(f"\nHUKUM: {'R10 HAK EDILDI -- egitim korpusunda ISARETLI oznitelikle yeniden turet ve dagit' if deger else 'isaretli oznitelik farki < 0.01 -- R10 kosulmaz'}")
    ...
    print("makbuz -> results/r9b_taban_duzeltme.json")
