# -*- coding: utf-8 -*-
"""VERI KALDIRACI OLCUMU: buyutulmus korpus CP F1'i yukseltiyor mu?

DURUSTLUK KURALLARI (bkz cp_config.locked_holdout_policy_2026_07_28):
  * Kilitli holdout parcalari TAMAMEN disarida -- bu olcum ona DOKUNMAZ.
  * `seen` bayrakli parcalar (etiketleri segmentasyon egitiminde gorulmus) EGITIMDE kullanilabilir
    ama SKORLANMAZ; yoksa sizinti F1'i sisirir.
  * Aile-disi (GroupKFold family) + ic-ice CV esik secimi: esik test katindan OGRENILMEZ.
  * Iki kol AYNI protokolde, AYNI skorlanan parca kumesinde olculur -- tek degisen EGITIM havuzu.
    (Aksi halde 'daha cok veri' ile 'daha kolay test' karisir.)
"""
import json, sys
import numpy as np
...
def load(paths):
    """Birden cok npz'yi birlestir; parca kimligi ile TEKILLESTIR (ayni parca iki dosyada olabilir)."""
...
def run(paths, label, score_pids, lock, fams):
    ...
    # SKORLAMA: sadece ortak, temiz (seen==0), kilitli-olmayan parcalar
    s = np.array([q in score_pids for q in PID])

    # IC-ICE ESIK SECIMI: esik, test katindaki ailelerden OGRENILMEZ. (Skorlanan kumede en iyi esigi
    # secmek her iki kolu da sisirir -- daha once bu hataya dusuldu, bkz p2 notlari.)
    fam_s = FG[s]
    uf = np.unique(fam_s)
    ...
    for f_out in folds:
        ...
        for t in THRS:                                   # esik SADECE ic (train) ailelerde secilir
            k = tr & (oof >= t)
            f = f1_of(int(Y[k].sum()), int(k.sum()), gt_tr)
            if f > best_f: best_f, best_t = f, t
        k = te & (oof >= best_t)                         # secilen esik DIS katta uygulanir
    ...
    print(f"{label:<22} egitim-parca {n_train_parts:>4} | aday {len(Y):>5} | "
          f"skorlanan parca {len(set(PID[s])):>3} | F1 {f1:.4f}  (ic-ice esik)")
...
if __name__ == "__main__":
    ...
    # ORTAK SKOR KUMESI: iki kolda da bulunan, TEMIZ, kilitli-olmayan parcalar
    _, _, P0, _, S0, _ = load(BASE)
    _, _, P1, _, S1, _ = load(BASE + NEW)
    common = {q for q in set(P0) if S0.get(q, 1) == 0 and q not in LOCK}
    print(f"ortak skor kumesi: {len(common)} temiz parca (iki kolda da ayni)\n")

    a, na = run(BASE, "TABAN korpus", common, lock, fams)
    b, nb = run(BASE + NEW, "BUYUTULMUS korpus", common, lock, fams)
    print(f"\nEgitim havuzu {na} -> {nb} parca (+{nb-na}, x{nb/max(na,1):.2f})")
