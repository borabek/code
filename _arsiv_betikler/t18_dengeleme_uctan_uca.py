...
Aday duzeyinde (t17, 22 sutun, dagitilan goreli esik):
    agirlik              TANIDIK   PXC-disi  WEI-disi  EN KOTU
    yok (mevcut)          0.7468    0.7164    0.4597    0.4597
...
AMA aday duzeyi uctan uca DEGILDIR -- ExtraTrees tam bu adimda oldu (aday duzeyinde en iyiydi,
uctan uca birinde +0.053 otekinde -0.039). Karar burada.

KILL: dengeleme, tanidik tespiti dusurmeden (>= -0.01) HER IKI uretici-disi bolmede de
mevcudu gecmezse ALINMAZ.

Aday duzeyinde olculdu:
  t13: kon_cevre AUC 0.709 (TP medyan 1.000 = tam tur icbukey halka, FP 0.833), kon_sayi 0.703
  t14: 18 -> 18+4  tanidik +0.0167 | gorulmemis uretici EN KOTU +0.0176 (ikisi de POZITIF)

Bu gece iki kez aday duzeyi yanildi (EK blogu; goreli esigin bedeli), o yuzden karar burada.

TASARIM: adaylar BIR KEZ turetilir, 22 sutun bir kez hesaplanir; 18-sutunlu kol ayni matrisin
ilk 18 sutununu kullanir. Karar kurali her iki kolda da DAGITILAN goreli esik. Tek degisken:
gate'in egitildigi ozellik kumesi.

KILL (onceden yazili): TOPO, (a) tanidik DEV+VAL tespit F1'i DUSURMEMELI (<=0.01 kayip tolere)
VE (b) gorulmemis uretici EN KOTU durumunu ARTIRMALI. Ikisi birden saglanmazsa DAGITILMAZ.
"""
...
def main():
    ...
    for k in ("dev", "val"):
        ...
        for r in pickle.load(open(cf, "rb")):
            ...
            r["kume"] = k
            cache.append(r)
    print(f"{len(cache)} parca | {dict(collections.Counter(mfg_of.get(r['pid'],'?') for r in cache))}",
          flush=True)
    ...
    print(f"\n{'bolme':<22}{'sutun':>6}{'tespit':>9}{'ROBOT':>9}{'kesin':>9}{'recall':>9}")
    R = {}
    for cw, ad in ((None, "agirlik yok"), ("balanced_subsample", "balanced_subsample"),
                   ("balanced", "balanced")):
        nc = 22
    ...
    print(f"\n  gorulmemis uretici EN KOTU: {en_kotu18:.4f} -> {en_kotu22:.4f} ({en_kotu22-en_kotu18:+.4f})")
    ...
    print("makbuz -> results/t18_dengeleme.json")
