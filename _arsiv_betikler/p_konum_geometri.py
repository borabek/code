# -*- coding: utf-8 -*-
"""P: robot-hazir NEDEN DEV'de 0.4974, VAL'de 0.3470?

BULGU (olculdu): iki kumenin TESPIT F1'i cakisiyor (0.7073 [0.6523,0.7603] vs 0.6529
[0.5791,0.7222]) ama ROBOT-HAZIR araliklari neredeyse AYRIK (DEV alt sinir 0.4259 > VAL ust
sinir 0.4225). Yani "bulma" iki kumede ayni, "robota verilebilir kalitede bulma" degil.

HIPOTEZ: konum/eksen kalitesi TESPITTEN DAHA COK geometriye bagli. DEV eski kumeydi ve
segmentasyon egitim geometrilerine daha yakin; VAL o gruplarin TAMAMEN disindan secildi.

BU BETIK: her iki kumede, ESLESMIS (yani tespit edilmis) adaylar icin ham hata dagilimini
karsilastirir -- yanal mm ve eksen derece. Boylece kaybin
   (a) yanal hatadan mi, (b) eksen hatasindan mi, (c) ikisinden mi
geldigi ayrisir. Kaldirac hangisinde oldugunu bilmeden konum isine girilmez (M dersi).

Yeni cikarim YOK: sinav kosularinin turetmelerini yeniden kullanmak yerine onbellekten
hizlica yeniden turetir (gate uygulanmis hali degil, HAM aday -> esleme -> hata).
"""
...
def hatalar(kume):
    ...
    for r in cache:
        ...
        if not len(G):
            continue
        # GEVSEK (tespit) olcutle esle -> yalnizca BULUNMUS olanlarin kalitesine bak
        diff = P[:, None, :] - G[None, :, :]
...
def main():
    out = {}
    print(f"{'kume':<6}{'n':>6}{'yanal med':>11}{'yanal p90':>11}{'<=2mm':>8}"
    ...
    print("\nKAYBIN AYRISTIRILMASI (DEV -> VAL, esles"
          "mis adaylar uzerinde):")
    dv, vv = out["dev"], out["val"]
    print(f"  yalniz yanal esigi (<=2mm) : {dv['yanal_2mm_orani']:.3f} -> {vv['yanal_2mm_orani']:.3f}"
    ...
    # rejim ayrimi: dusuk-CP mi cok-CP mi bozuluyor?
    print(f"\n{'rejim':<8}{'DEV ikisi':>11}{'VAL ikisi':>11}{'fark':>9}")
    for rk in ("dusuk", "cok"):
        v = []
        for k in ("dev", "val"):
            lat, ang, rej = D[k]
