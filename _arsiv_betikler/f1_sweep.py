...
Union (vote>=1) recall'i ~0.74'e cikariyordu ama precision cokuyordu (0.40) -> OLU idi. Simdi
wire-gate precision'i geri getiriyor. HIPOTEZ: vote>=1 (max recall) + gate, vote>=2 + gate'i gecebilir.
Sizinti-siz: gate GroupKFold-OOF (bir parcaya, o parca disinda egitilmis gate uygulanir).

Faz 1 (GPU): her parca 4 model -> TUM union CP'leri (vote sayisiyla) + ozellik + tp-etiket + parca + mfg.
...
def union_all(cp_lists, cluster_mm=5.0):
    """TUM CP'ler dedup + vote sayisi (min filtre YOK -- vote>=1)."""
...
def extract():
    ...
    cap = int(os.environ.get("F1_PXC_CAP", "0"))     # 0 = tum PXC (tam hakem)
    parts = [p for p in eligible() if (p[0] == "WEI" and p[1] in HELD)]
    pxc = [p for p in eligible() if p[0] == "PXC"]
    parts += (pxc[:cap] if cap else pxc)
    print(f"{len(parts)} parca ({len(parts)-len(pxc if not cap else pxc[:cap])} WEI + {len(pxc if not cap else pxc[:cap])} PXC) | faz 1 (GPU)", flush=True)
...
def sweep():
    ...
    print(f"\n=== ORTAK SWEEP ({len(y)} union CP, uretici {n_gt}) | urun vote>=1 union + RF wire-gate 0.35 (base F1 0.750; GB-era 0.693 RETIRED) ===")
    ...
    # URUN config (vote>=1, gate 0.35) per-mfg + birlesik, OOF (sizinti yok)
