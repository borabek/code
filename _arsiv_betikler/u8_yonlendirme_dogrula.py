...
U7 tablosu (uctan uca):
    arm     WEI-disi  PXC-disi  tanidik  seri-ORT
...
def main ():
    ...
    def olc (alt ,keep ):
        ...
        for r in alt :
            ...
            cok =(sa is not None )and (float (sa .max ())<threshold )
            direction +=int (cok )
    ...
    split .append (("tanidik",~np .isin (tr_grp ,list (tg )),DER ))
    ...
    with open ("results/u8_parca.pkl","wb")as f :
        pickle .dump (SON ,f )
    print ("receipt -> results/u8_yonlendirme_dogrula.json")
