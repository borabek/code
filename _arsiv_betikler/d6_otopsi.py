...
TAVANLAR (same kayitlardan):
  ...
  KAHIN-ACI robot    : tespit same kalip TUM acilar <=10 olsaydi robot F1
"""
import collections 
import glob 
import io 
...
def main ():
    ...
    for pid ,r in rec_ .items ():
        ...
        if r .get ("X")is not None and len (P0 ):
            ...
            if M is not None and M .shape [1 ]*2 ==gate ["n_feat"]:
                ...
                if k .any ():
                    ...
                    # --- GT duzeyi siniflama (tespit kutusu: lateral<=tt, axial<=40)
        def kapsanan (Pq ):
        ...
        # --- KAHIN-ACI: tespit eslesmeleri same, angle kisiti kalkmis, lateral 2mm kalir
