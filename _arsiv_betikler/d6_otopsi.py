...
0.70 plani TAHMINLE yazilamaz. Her GT icin kayip nedeni TEK TEK siniflanir:

  ADAY_YOK    : gate'ten ONCE bile toleransta aday yok  -> TEMSIL kolu (seg/veri)
  GATE_REDDI  : aday vardi, gate hepsini reddetti       -> GATE kolu (v5 + esik)
  KALABALIK   : aday+gate gecti ama baska GT'ye atandi  -> cluster/dedupe kolu
  ESLESTI     : tespit TP

Tespit TP'lerinin robot kirilimi (2mm/10der/isaretli):
  ACI_FAIL, YANAL_FAIL, IKISI, TERS_ISARET, ROBOT_OK   -> secici/yanal kollari

TAVANLAR (ayni kayitlardan):
  KAHIN-GATE tespit  : mukemmel gate mevcut adaylarla ne yapardi (FP=0 varsayimi)
  KAHIN-ACI robot    : tespit ayni kalip TUM acilar <=10 olsaydi robot F1
"""
import collections
import glob
import io
...
def main():
    ...
    kova = collections.Counter()          # GT duzeyi kayip nedeni
    rb = collections.Counter()            # tespit-TP robot kirilimi
    T, R, KG, KA = [], [], [], []         # gercek tespit/robot + kahin satirlari
    for pid, r in kayit.items():
        ...
        # --- GT duzeyi siniflama (tespit kutusu: yanal<=tt, eksenel<=40)
        def kapsanan(Pq):
        ...
        tp, fp, fn, bi = esle_detay(P, D, G, Gd, diag, 0.0, 180.0, True)
        ...
        tpr, fpr, fnr, _ = esle_detay(P, D, G, Gd, diag, 2.0, 10.0, False, isaretli=True)
        R.append((rj, tpr, fpr, fnr))
        # --- KAHIN-GATE: mevcut adaylarla mukemmel gate (FP=0, kapsanan her GT TP)
        kg = int(on.sum())
        KG.append((rj, kg, 0, len(G) - kg))
        # --- KAHIN-ACI: tespit eslesmeleri ayni, aci kisiti kalkmis, yanal 2mm kalir
    ...
    print(f"TEMIZ SINAV {len(kayit)} parca, {n_gt} GT CP\n")
    ...
    print(f"{'KAHIN-ACI (yanal<=2 tavani)':<26}{'':>9}{f1w(KA):>9.4f}")
    json.dump({"kova": dict(kova), "robot_kirilim": dict(rb),
               "tespit": f1w(T), "robot": f1w(R),
