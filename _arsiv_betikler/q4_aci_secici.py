# -*- coding: utf-8 -*-
"""Q4: ACI duzeltmesini SECICI yap -- robot-hazir 0.60'a giden son kol.

POSE SONRASI AYRISTIRMA (867 eslesme):
    yanal gecis  %74.8 -> %88.9   (pose head calisti)
    aci   gecis  %81.4 -> %80.4   (degismedi)
    yalniz aci'dan kaybedilen %14.2 | yalniz yanaldan %5.7
Yani BAGLAYICI SART artik ACI. Ve aritmetik: aci tamamen cozulse gecis %88.9 -> robot ~0.60.

ACI duzeltmesi neden daha once KAYBETTIRDI (q1/q2): yonlerin cogu ZATEN TAM DOGRU (medyan
0.00 derece) ve duzeltme HEPSINE uygulaniyordu -> medyan 0.00 -> 2.36 dereceye cikti.
Bu, bu gecenin dort olu kolunun ortak kusuru.

COZUM (bu gece IKI KEZ ise yaradi -- cokus yonlendirmesi ve yarik secicisi): once
"BU ACIYA DOKUNULMALI MI" diye sor, sonra duzelt.

  1. SINIFLANDIRICI: "bu adayin acisi 10 dereceden fazla yanlis mi?"  (uretici GT'sinden)
  2. Yalniz P(yanlis) >= esik olanlarda yon duzeltmesi uygulanir
  3. Esik taranir; secici hicbir DOGRU yonu bozmamali
...
def main():
    ...
    # POSE verisi -> aci hedefi + "yanlis mi" etiketi
    ...
    print(f"aci secici egitimi: {len(RY)} satir | yanlis oran {yanlis.mean():.1%} "
    ...
    print(f"\n{'kol':<22}{'tespit':>10}{'ROBOT':>10}{'d_robot':>10}")
    ...
    for e in (0.05, 0.10, 0.15, 0.20, 0.25, 0.30):
        det, rob = puanla(e)
        SON[f"B esik {e:.2f}"] = (float(f1w(det)), float(f1w(rob)))
        PAR[f"B esik {e:.2f}"] = (det, rob)
        print(f"{'B secici esik ' + f'{e:.2f}':<22}{f1w(det):>10.4f}{f1w(rob):>10.4f}"
    ...
    if gecti:
        with open("results/aci_secici.pkl", "wb") as f:
            pickle.dump({"sec": sec, "reg": reg, "esik": float(en.split()[-1]),
                         "n_feat": RX.shape[1],
    ...
    print("makbuz -> results/q4_aci_secici.json")
