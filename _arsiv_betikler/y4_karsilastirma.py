# -*- coding: utf-8 -*-
"""Y4: KARSILASTIRMALI RENDER -- "ayni mi?" sorusu, "aciklik mi?" sorusu DEGIL.

NEDEN DEGISTI: ilk galeri MUTLAK yargi soruyordu ("bu gercek bir aciklik mi?"). Kullanici
hakli olarak "ben bakarak anlayamiyorum" dedi. Bir klemensin derinlik haritasina bakip
"bu tel girisi mi" demek uzmanlik ister; KARSILASTIRMA istemez.

YENI KURULUM: her FP icin, AYNI PARCADA ureticinin LISTELEDIGI bir girisi TIPATIP ayni
yontemle ciz ve yan yana koy, RENK OLCEGI ORTAK. Soru artik:
    "Solda ureticinin onayladigi giris. Sagda robotun bulup listede olmayan nokta.
     Bu ikisi AYNI TUR sey mi?"
Hem insan icin cok daha kolay, hem de bilimsel olarak tam sordugumuz sey: listelenmemis
tespit, listelenmislerle AYNI CINSTEN mi?

REFERANS GT SECIMI: FP'ye en yakin GT DEGIL (bitisik olur, ayirt edilemez); parcanin
GEOMETRIK ORTASINA en yakin GT -- yani TIPIK bir giris. Rastgele degil, tekrar uretilebilir.

Referansi olmayan parcalar (GT yok) AYRI isaretlenir ve orana KATILMAZ.
"""
import io
import json
import os
...
def yuzeye_gore(son):
    """Derinligi NOKTAYA gore degil CEVREDEKI YUZEYE gore ifade et.

    NEDEN SART: uretici CP'si kanalin ICINDE tanimli (isinlar ona varmadan ~5mm once yuzeye
    carpar -> merkez NEGATIF), robotun noktasi ise agizda (tezin v_o'su = sinir noktalarinin
    ortasi -> isin kanala girer, merkez POZITIF). Ayni fiziksel aciklik iki KONVANSIYONDA
    zit gorunur ve karsilastirma yaniltir. Cevre yuzeyi sifir kabul edince ikisi de
    "merkez cevresinden ne kadar derin" der ve AYNI dili konusur.
    """
...
def main():
    ...
    print(f"{len(sec)} FP / {len(byp)} parca", flush=True)
    ...
    for k, (pid, idxs) in enumerate(sorted(byp.items()), 1):
        ...
        for i in idxs:
            ...
            if son is None:
                continue
            son = yuzeye_gore(son)
            # ORTAK RENK OLCEGI: ayri olcekte iki harita gorsel olarak KIYASLANAMAZ
            ...
            cb.set_label("cevre yuzeyine gore derinlik (mm)", fontsize=8)
            ...
            fig.savefig(yol, dpi=84, bbox_inches="tight"); plt.close(fig)
            KAYIT.append({"idx": i, "pid": pid, "mfg": f_["mfg"], "rejim": f_["rejim"],
                          "png": yol, "ref_var": ref is not None,
    ...
    print("makbuz -> results/fp_karsilastirma.json")
