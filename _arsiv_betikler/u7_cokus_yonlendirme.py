...
FIKIR: cokusu CALISMA ANINDA sez, only orada z-skor gate'e gec. Sezgi metadata GEREKTIRMEZ --
ham gate'in KENDI skor dagilimi already cokusu gosteriyordu: olculmus teshis, cokus halinde model
...
whereas -> z-skor gate. Esik EGITIM parcalarindan (test geometrisi haric) is computed; test tarafina
BAKMAZ.
...
def main ():
        ...
        for r in alt :
            ...
            for ad in det :
                if sa is None :
                    ...
                    q =float (ad [1 :])
                    cok =float (sa .max ())<threshold [q ]# COKUS SEZGISI
                    yon_say [q ]+=int (cok )
                    s =sd if cok else sa 
    ...
    print (f"\n{'arm':<8}{'WEI-disi':>10}{'PXC-disi':>10}{'tanidik':>10}"
