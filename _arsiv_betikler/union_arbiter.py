# -*- coding: utf-8 -*-
"""UNION ensemble plato-kirici (ROBOT_STRATEJI yedek plani #2).

Softmax-ORTALAMA (big_arbiter'in coklu-ckpt yolu) recall'i kesiyordu -> oldu.
TERSI hic denenmedi: her model KENDI CP'lerini turetir, sonra CP'lerin BIRLESIMI alinir
(bir CP'yi HERHANGI bir model bulduysa tut), yakinlik ile dedup. Recall'i maksimize eder.
Ayni kosuda hem tek modelleri hem birlesimi ayni parca setinde skorlar -> adil karsilastirma.
...
def main():
    ...
    print(f"{len(raw)} uygun parca | {len(models)} model UNION | cluster {a.cluster_mm} min_v {a.min_v}", flush=True)

    # her parcayi bir kez remesh + her modelden CP turet (cache)
    parts = []; t0 = time.time()
