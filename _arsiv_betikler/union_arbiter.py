...
Ayni kosuda hem single modelleri hem birlesimi same part setinde skorlar -> adil karsilastirma.
...
def main ():
    ...
    print (f"{len (raw )} uygun part | {len (models )} model UNION | cluster {a .cluster_mm } min_v {a .min_v }",flush =True )

    # each parcayi a times remesh + each modelden CP turet (cache)
    parts =[];t0 =time .time ()
