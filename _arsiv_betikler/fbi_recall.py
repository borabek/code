...
H1 (teshis kusuru): eski FN teshisi uretici CP'sinin (KONTAKT, ~15mm icerde) 8mm cevresine bakti;
    model acikligin AGZINDA (v_o, ~15mm disarda) ateslerse "kor" sayilmis olabilir. DUZELT:
    her FN icin eksen-BOYUNCA (Gd yonunde) [-5,+25]mm, perp<6mm silindirde baglanti-verteksi ara.
H2 (tez kolu): CAD-geometrik aciklik dedektoru (step_openings, ogrenilmemis -> WEI domain-gap'i YOK)
    model kaciran acikligi yakaliyor mu? -> CAD-UNION plato-kiricisi.
Ayrica: model CP + CAD acikliklarinin BIRLESIMI WEI recall'i ne yapar (sizinti-siz 145 WEI).
"""
...
def main():
    ...
    fn_total = 0
    h1_mouth = 0      # model AGZINDA atesledi (duzeltilmis) -> turetme/sekil, post-proc adayi
    ...
    for k, (mfg, pid, jf, stp) in enumerate(raw, 1):
        try:
            ...
            # union recall (model CP + CAD entry, dedup 5mm; model once -> onceligi var)
            ...
            # FN teshisi (model-only FN'ler uzerinde)
    ...
    if fn_total:
        print(f"  H1 model AGIZDA atesledi (eksen-boyu, duzeltilmis): {h1_mouth} ({100*h1_mouth/fn_total:.0f}%) <- POST-PROC/derivation adayi")
