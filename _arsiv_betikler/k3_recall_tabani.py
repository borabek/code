# -*- coding: utf-8 -*-
"""K3: RECALL TABANI -- 'hicbir parcada N'den az aday kabul etme'.

Top-K KAPANDI cunku TAM K secmeye zorluyordu: K yanlissa esikten kotu (-0.0145).
TABAN farkli -- yanlis K'nin cezasi YOK, yalniz asiri sessiz parcalar kurtulur.
Gerekce: gate'in reddettigi 248 dogru adayin %77'si YOGUN parcalarda ve o parcalarda
goreli esik (0.5*max) tek bir guclu adayin golgesinde kaliyor.

DURUST SECIM: N bir yarida secilir, diger yaride olculur.
"""
import io, json, sys
import numpy as np
sys.path.insert(0, ".")
...
print(f"\n{'kol':<30}{'tespit':>9}{'robot':>9}")
T2.bas("TABAN (goreli esik)", d0, r0)
...
for N in (2, 3, 4, 5):
    ...
    TAM[N] = (f1w(d), f1w(r))
    T2.bas(f"N={N} tabani", d, r, taban=(d0, r0), gg=gg)
...
for sec, olc in ((altA, altB), (altB, altA)):
    ...
    dc += d1; dt += d2; gg2 += g1
    print(f"  yarida secilen N={ea} -> diger yaride olculdu ({len(olc)} parca)")
...
print("makbuz -> results/k3_recall_tabani.json")
