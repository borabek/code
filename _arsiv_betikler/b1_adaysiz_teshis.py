# -*- coding: utf-8 -*-
"""B1: 188 ADAYSIZ CP'NIN TESHISI -- Rota B'nin ilk kapisi.

OLCULDU: 1323 uretici CP'sinin 188'ine (%14.2) HICBIR aday ulasmiyor. Aday havuzunun
recall tavani 0.8579. Bu 188 CP, oznitelik-uzayi tavaninin (~0.86) DISINDAKI tek aciktir:
onlari kazanmak yeni oznitelik degil, DAHA IYI ADAY URETIMI gerektirir.

SORU (pahali bir etiketleme kararini saatler icinde cozer):
  (a) Segmentasyon o konumda CableEntry/Contact DEMIYOR
      -> ontoloji/etiket sorunu. Hedefli etiket kampanyasi HAKLI.
  (b) Diyor ama ADAY URETIMI eliyor (min_v, kume, derinlik esikleri)
      -> UCUZ esik duzeltmesi. Etiket kampanyasina GEREK YOK.

YONTEM: her adaysiz CP icin, konumunun cevresindeki mesh tepe noktalarinda 4 modelin
ortalama CE+CT olasiligina bakilir. Karsilastirma icin ULASILAN CP'lerde ayni olcum
yapilir -- yani "yeterli olasilik" esigi veriden gelir, elle konmaz.

KILL: (b) baskinsa (>%50) etiket kampanyasi ACILMAZ, turetme duzeltilir.
"""
import io
import json
import os
...
def main():
    ...
    for k, r in enumerate(DER, 1):
        ...
        if ul.all():
            continue                      # bu parcada adaysiz CP yok -> cikarim gereksiz
        try:
    ...
    esik = float(np.percentile(cmU, 10))
    b_pay = float((cmA >= esik).mean())
    print(f"\nESIK (ulasanlarin 10. yuzdeligi): CE+CT olasilik {esik:.3f}")
    ...
    print()
    if b_pay > 0.5:
        print("KARAR: (b) BASKIN -> etiket kampanyasi ACILMAZ. Once aday uretimi esikleri.")
    else:
    ...
    with io.open("results/b1_adaysiz_teshis.json", "w", encoding="utf-8") as f:
        json.dump({"ulasan": len(ULASAN), "adaysiz": len(ADAYSIZ), "esik": esik,
    ...
    print("makbuz -> results/b1_adaysiz_teshis.json")
