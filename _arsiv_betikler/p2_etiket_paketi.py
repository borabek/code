# -*- coding: utf-8 -*-
"""P2 PARTI-1 etiket paketi: `.obj` + MODELIN TAHMINIYLE DOLDURULMUS etiket.

NEDEN TAHMIN TOHUMLU: is SIFIRDAN BOYAMA degil DUZELTME. Olculdu
(`results/p2_getirisi.json`, D7 marka-disi, 3087 GT): agiz cevresi segmentasyon
kalitesinin en iyi ceyreginde robot 0.4870 / yanal<=2mm %65.4 / medyan yanal
1.38mm; en kotu ceyrekte 0.0557 / %10.4 / 8.64mm. Herkes en iyi ceyrek gibi
...
DEGISMEZ. Degisen tek sey etiket kalitesi.

Cikti: `_p2_etiket/<pid>/<pid>.obj` + `<pid>.labels.txt` (tohum) +
`<pid>.labels.template.txt` (bos) + `README.md`.
Etiketleyici `label_tool.html`'i acar, `.obj`'i secer, tohum etiketi yukler,
agizlari duzeltir, indirir.
"""
import argparse
import glob
import os
...
KILAVUZ = """# P2 parti 1 -- agiz segmentasyonu duzeltme
...
## Neden bu is
Olculdu (D7 gorulmemis marka, 3087 CP): agiz cevresindeki segmentasyon
kalitesinin en iyi ceyreginde robot **0.4870**, en kotu ceyrekte **0.0557**;
medyan yanal hata 1.38mm'ye karsi 8.64mm. Herkes en iyi ceyrek gibi davransa
robot recall **0.1995 -> 0.4870**. Bugune kadar denenen tum son-islem kollari
+-0.005 bandindaydi; bu kol iki buyukluk mertebesi daha buyuk.

## Nasil
1. `label_tool.html`'i tarayicida ac
2. `<pid>.obj` dosyasini sec
3. `<pid>.labels.txt` (tohum) dosyasini yukle
...
def main():
    ...
    if eksik:
        raise SystemExit(f"STEP bulunamayan {len(eksik)} parca: {eksik[:5]} "
                         "-- sessiz atlamak yerine DURUYORUM")
    os.makedirs(a.cikti, exist_ok=True)

    cihaz = "cuda" if torch.cuda.is_available() else "cpu"
    # URUNUN kendi yukleyicisi -- k_eig/meta ckpt'ten gelir. Elle `n_eig`
    # varsaymak bu projede bir kez agi YANLIS ozvektor tabaninda kosturmustu.
    model, meta = load_any(a.ckpt, dev=cihaz)[:2]
    print(f"ckpt {a.ckpt} | cihaz {cihaz} | parca {len(pids)}", flush=True)
    ...
    for i, pid in enumerate(pids, 1):
        ...
        tohum = os.path.join(d, f"{pid}.labels.txt")
        if os.path.exists(obj) and os.path.exists(tohum):
            basarili.append(pid)
            continue
        ...
        np.savetxt(tohum, pred, fmt="%d")
        np.savetxt(os.path.join(d, f"{pid}.labels.template.txt"),
                   np.zeros(len(V), int), fmt="%d")
        basarili.append(pid)
        if i % 5 == 0:
            print(f"  {i}/{len(pids)}  {(time.time()-t0)/i:.1f}s/parca", flush=True)
    ...
    print(f"\nBITTI: {len(basarili)} parca -> {a.cikti}")
