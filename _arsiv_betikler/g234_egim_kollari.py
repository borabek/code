...
gorulmemis-manufacturer tavani **0.5964**. Yani this three arm 0.70'i TEK BASINA GETIREMEZ; amaclari
...
UC KOL, UCU DE AYRI MODEL (birlestirilmez -- otherwise hangisinin ne getirdigi atfedilemez):
  ...
  G4 BUDAMA          : 13 ozniteligin only `votes`'u transfer ediyor
                       ([[gate-memorizes-not-learns]]). Dusuk kapasite + part-ici z-skor
                       sutunlari (mutlak buyuklukler ureticiye ozgudur, goreli olanlar not).

HER KOL IKI KUMEDE RAPORLANIR -- 194'luk cluster and gorulmemis-manufacturer sinavi. Yalniz birinde
bakmak yaniltir: [[gorulmemis-manufacturer-sinavi-first-measurement]] data kolunda +0.0058 vs +0.0535
gostermisti.
...
def _z (X ,pid ):
    """Parca-ici z-skor -- calisma anindaki with AYNI (part bazinda)."""
...
def egit (npz ,wgt_ =None ,cols =None ,depth =None ,yaprak =3 ,seed =0 ):
    ...
    # `wire_gate.decision_score` before TUM 58 sutuna part-ici z-skor uygular (->116), after
...
def main ():
    ...
    print (f"  NOT: G1 ceiling hesabi zaten 0.5964 diyordu -- bu kollar 0.70'i tek basina")
