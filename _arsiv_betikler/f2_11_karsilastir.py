# -*- coding: utf-8 -*-
"""F2-11 KARSILASTIRMA: A (STEP agi) vs B (JSON agi) vs C (JSON agi, STEP yok).

KILL (f2_11_parite.py'de onceden yazildi):
    aday recall farki <= 0.01  VE  uretici basina tespit F1 farki <= 0.015
GECERSE: STEP'siz 2759 parca kullanilabilir -> korpus 1926 -> ~4685 (2.4x).
GECMEZSE: yeni veri yalnizca segmentasyon on-egitimi icin kullanilabilir.

ADIL KARSILASTIRMA SARTI: A kolu, ayni gece ayni ortamda uretilmis `_der_kontrol.pkl`
olmalidir -- 2 gunluk `_der_tam.pkl` DEGIL. Sebep [[robot-nondeterminism]]: pymeshlab
remesh surecler arasi oynuyor (~0.4mm), ve aradaki kod degisiklikleri de karisir.

GATE: her kol KENDI aday dagilimiyla egitilmis gate ile puanlanmali (G3 dersi: dagitilan
gate'i baska bir dagilima uygulamak kolu HAKSIZ YERE oldurur). Burada gate olcum
gruplari CIKARILARAK egitilir (tezgah2 ile ayni yordam).
"""
import io
import json
import os
...
def gate_kur(DER):
    """Bu kolun KENDI aday dagilimiyla gate egit (olcum gruplari zaten disarida)."""
...
def puanla(DER, gate):
    """GRUP-CAPRAZ: her parcanin skoru, o parcanin grubu DISLANARAK egitilmis gateden."""
    rows, mfg_rows = [], {}
    for r in DER:
        G = np.asarray(r["G"], float); Gd = np.asarray(r["Gd"], float)
...
def main():
    ...
    if "A_STEP" in SON and "B_JSON" in SON:
        ...
        print(f"  aday recall farki : {dr:+.4f}   (esik <= 0.01)  "
              f"{'OK' if abs(dr) <= 0.01 else 'KALDI'}")
        print(f"  tespit F1 farki   : {b['tespit']-a['tespit']:+.4f}")
        print(f"\n  {'uretici':<10}{'A_STEP':>9}{'B_JSON':>9}{'fark':>9}")
        enb = 0.0
        for m in sorted(set(a["uretici"]) | set(b["uretici"])):
            va, vb = a["uretici"].get(m, 0), b["uretici"].get(m, 0)
            enb = max(enb, abs(vb - va))
            print(f"  {m:<10}{va:>9.4f}{vb:>9.4f}{vb-va:>+9.4f}")
        print(f"\n  uretici basina EN BUYUK fark: {enb:.4f}  (esik <= 0.015)  "
              f"{'OK' if enb <= 0.015 else 'KALDI'}")
        gecti = abs(dr) <= 0.01 and enb <= 0.015
        print(f"\n  KILL -> {'GECTI: JSON agi KULLANILABILIR, 2759 parca acilir' if gecti else 'GECMEDI'}")
