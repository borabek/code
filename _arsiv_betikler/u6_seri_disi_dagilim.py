# -*- coding: utf-8 -*-
"""U6: parca-ici z-skor kararini YEDI bolmede sina (tek bolmeye mi dayaniyordu?).

TEREDDUT: dagitim karari, gorulmemis-uretici ekseninde TEK bir sayiya dayaniyor (WEI disarida
0.4832 -> 0.5702). Elimizde yalniz IKI uretici oldugu icin o eksende TOPLAM iki bolme var; "en
kotu durum" tek bir olcumdur ve gurultulu olabilir. Karar o tek bolmeye fit olduysa gercek bir
ucuncu ureticide caresiz kaliriz.

BU DENEY: parca numarasinin ilk 2 hanesi = URUN SERISI. Seri-disi bolme, uretici-disindan
DAHA KOLAY bir eksendir (ayni ureticinin istatistigi egitimde kalir) -- yerine gecmez, ama
YEDI bagimsiz bolme verir. Sorulan sey sudur:

    z-skor GENEL bir saglamlik kazanci mi, yoksa yalniz WEI bolmesinde mi calisiyordu?

Bir kol yedi seride de ayni yone gidiyorsa karar saglamdir; yalniz birinde kazanip otekilerde
kaybediyorsa tek bolmeye fit olmusuz demektir.

TURETME onbellekten (results/_u4_der.pkl) -- ag cikarimi yok.
"""
import collections
import json
import os
...
def main():
    ...
    # HANGI SERILER: hem testte hem egitimde yeterli kutle olmali
    ...
    print(f"{len(seriler)} seri | DER {len(DER)} parca")
    ...
    def puanla(clf, alt, don):
        ...
        for r in alt:
            ...
            if r["X"] is not None:
                s = clf.predict_proba(wire_gate.parca_ici(r["X"], don))[:, 1]
    ...
    print(f"\n{'seri':<8}{'uretici':>9}{'n':>5}{'A ham':>9}{'D zskor':>10}{'fark':>9}")
    S, farklar = {}, []
    for k in seriler:
        ...
        print(f"{k:<8}{mf:>9}{len(alt):>5}{v['A ham']:>9.4f}{v['D zskor']:>10.4f}{fark:>+9.4f}",
              flush=True)
        S[k] = {"uretici": mf, "n": len(alt), "A": float(v["A ham"]), "D": float(v["D zskor"]),
                "fark": float(fark)}
    ...
    # ISARET TESTI (dagilimsiz): D gercekten genel mi, yoksa tek bolmeye mi fit?
    from math import comb
    n = int((f != 0).sum())
    p_iki_yonlu = min(1.0, 2 * sum(comb(n, i) for i in range(kazanan, n + 1)) / 2 ** n)
    print(f"isaret testi: {kazanan}/{n} pozitif -> p = {p_iki_yonlu:.4f}")
    ...
    print(f"YORUM: {yorum}")
    print("\nNOT: seri-disi, uretici-disindan DAHA KOLAY bir eksendir (ayni ureticinin\n"
    ...
    print("makbuz -> results/u6_seri_disi.json")
