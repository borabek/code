# -*- coding: utf-8 -*-
"""R8: UYE-YON verisi (full corpus) -- ogrenilmis uye selector for.

R7 olcumu: four modelin ATILAN yonlerinde robot-haziri 0.5523 -> 0.6059 yapacak bilgi VAR
(+0.0536). Konum tarafinda YOK (-0.0022; pose head already almis). Yani arm ACI'da canli.

Bu betik each BIRLESTIRILMIS CP for, ona katkida bulunan UYE adaylarini and each uyenin
ozelliklerini writes. Hedef: "this uyenin yonu GT'ye 10 derece inside mi".

UYE OZELLIKLERI (calisma aninda hesaplanabilir, GT'ye BAKMAZ):
    uye_conf      uyenin own guveni
    ...
    n_uye         this CP'ye katkida bulunan uye count
    spread       uye yonlerinin each other according to mean sapmasi
    uye_sira      guvene according to order
...
def main ():
    ...
    with io .open ("results/_strict_geometry_keys.json",encoding ="utf-8")as f :
        gk =json .load (f )
    parts =list (eligible ())
    print (f"{len (models )} model | {len (parts )} part",flush =True )
    ...
    geo =np .array (GEO ),cp =np .array (CPI ))
    Y =np .array (Y )
    print (f"\n-> {OUT } | {len (Y )} uye satiri / {len (set (CPI ))} CP / {len (set (PID ))} part")
    print (f"   'direction dogru' orani {Y .mean ():.1%} | atlanan {atlanan }")
