...
KARAR KAPISI:
  uyum >= %90 -> insan etiketi ureticinin tanimini yeniden uretiyor => 2838 STEP'i insan etiketiyle
                 acabiliriz (veri acligina dogrudan ilac, olcek meselesi)
  %75-90      -> kismi; sadece yuksek-guvenli (unsure disi) cevaplar kullanilabilir
  <  %75      -> ureticinin listesinde geometride OLMAYAN katalog bilgisi var (R4 kesinlesir)
                 => insan etiketi GURULTU ekler, yol KAPATILIR
...
for k, v in ans.items():
    ...
    y = truth[pid][i]                      # 1 = uretici CP ile eslesen aday
...
print(f"=== WIRE PILOT: insan vs URETICI ({len(per_part)} parca, {n} kesin cevap, {uns} 'emin degilim') ===")
print(f"  UYUM (accuracy)        : {acc:.3f}")
print(f"  insan 'TEL' dedi & uretici de CP diyor (TP): {tp}")
print(f"  insan 'TEL' dedi ama uretici CP DEMIYOR (FP): {fp}")
print(f"  insan 'ALET' dedi ama uretici CP DIYOR  (FN): {fn}")
...
print(f"         insan-F1 modelden YUKSEKse -> ogrenilecek sinyal VAR (tavan insan seviyesi)")
...
if acc >= 0.90:
    ...
    print("  => sinirli olcekte dene, gurultu etkisini ayrica olc.")
else:
    print(f"  KALDI (%{100*acc:.0f}) -> uretici listesi geometriden turetilemiyor (katalog bilgisi).")
