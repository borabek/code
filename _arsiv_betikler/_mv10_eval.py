...
Kiyas same protocol: WORK 541 part, family-out, nested-CV threshold, zengin feature (46d), RF.
Soru: candidate 1.5x arttiginda gate gurultuyu eleyebiliyor mu."""
...
def run (path ,label ):
    ...
    print ("%-26s candidate %5d (%.2fx GT) | ceiling %.3f | AUC %.4f | ALL %.4f | WEI %.4f | PXC %.4f"%(
    label ,int (M .sum ()),int (M .sum ())/max (gt ,1 ),ceil ,roc_auc_score (Y [M ],o [M ]),
    nested (M ),nested (M &(MF ==1 )),nested (M &(MF ==0 ))),flush =True )

print ("POST-ISLEME KIYASI (WORK 541 part, family-out, nested-CV threshold)")
