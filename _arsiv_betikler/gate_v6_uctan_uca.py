...
DURUM: dagitilan gate `gate_regrow_data_rt2.npz` (30 Tem 03:04) ile egitildi. Bu gecenin parite
duzeltmeleri (3/3 cagriya step_path -> dusuk-CP yolunda da B-rep ekseni, benzersiz oy sayimi,
agirlikli konum) turetmeyi DEGISTIRDI. Yani gate'in gordugu ozelliklerle calisma aninda aldigi
ozellikler ayni hattan gelmiyor olabilir -- tam olarak `gate_refit` dersindeki durum (+0.1273).

v6 verisi (31 Tem 10:38) guncel hatla uretildi. GATE DUZEYINDE -0.0185 olctum ve "olu" dedim;
ama gate duzeyi iki kez yaniltti ve dahasi iki gate FARKLI aday havuzlarinda karsilastirilmisti
(rt2 havuzu vs v6 havuzu) -- bu, `gate_refit` dersinin acikca yasakladigi kiyas.

BU BETIK dogru kiyasi yapar: iki gate de AYNI test adaylarinda (guncel hat) uctan uca olculur.
Egitim taraflari farkli (dogal, karsilastirilan sey bu), test tarafi AYNI.

KILL: v6 rt2'yi tespit F1'de gecmezse dagitilan gate degismez.
"""
...
def main():
    ...
    for tag in ("rt2", "v6"):
        ...
        print(f"gate {tag}: {int(keep.sum())}/{len(pids)} aday ile egitildi", flush=True)
    ...
    print(f"{len(DER)} parca | {sum(len(r['P']) for r in DER)} aday (AYNI havuz, iki gate icin de)",
          flush=True)
    ...
    print(f"\n{'gate egitim verisi':<24}{'tespit':>9}{'ROBOT':>9}{'kesin':>9}{'recall':>9}")
    R = {}
    for tag in ("rt2", "v6"):
        det, rob = kos(tag)
        R[tag] = (det, rob)
    ...
    json.dump({"kume": kume, "rt2_tespit": f1w(R["rt2"][0]), "v6_tespit": f1w(R["v6"][0]),
               "rt2_robot": f1w(R["rt2"][1]), "v6_robot": f1w(R["v6"][1]), "bootstrap": out},
              open(f"results/gate_v6_uctan_uca_{kume}.json", "w"), indent=1)
    print(f"makbuz -> results/gate_v6_uctan_uca_{kume}.json")
