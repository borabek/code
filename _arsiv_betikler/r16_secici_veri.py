...
R15 olctu: uc aday arasinda dogru eksen NEREDEYSE HEP VAR (kahin %84 aci<=10, dik %26 -> %2)
ama ELLE yazdigim secim kurali MEVCUT'tan kotu (%62 vs %68). Yani adaylar iyi, KURAL kotu.
Cozum: kurali OGREN -- `c4_yon_secici`nin isaret icin yaptigini eksen icin yap.

TEZ SADAKATI: `v_o` KONUMU degismez (tezin agiz-ortasi tanimi aynen), 5 sinif, remesh, ag
degismez. Yalniz YON tahmini etkilenir -- ve urunun yon hatti tezin ham `v_o - v_s`'sini
ZATEN asmis durumda (channel_axis -> _snap_axis -> normal-kovaryans -> B-rep, dort kademe).
Secici o zincire besinci kademe ekler; tezin ekseni ADAYLARDAN BIRI olarak kalir ve sonuc
tezin ham ekseniyle YAN YANA raporlanir.

SIZINTI: sinav kumesi ve ikizleri DISARIDA. Egitim yalniz `_der_yeni_g6` parcalarindan.
"""
...
def ozn(mesh, p, v, d0):
    """Aday basina GT'siz oznitelikler."""
...
def main():
    ...
    for i, r in enumerate(egt, 1):
        if i % 50 == 0:
            print(f"  {i}/{len(egt)} ({(time.time()-t0)/i:.1f}s/parca) satir {len(X)}", flush=True)
        ...
        for dd, a_, b_ in sorted((pe2[a,b],a,b) for a in range(len(P)) for b in range(len(G))):
            ...
            up.add(a_); ug.add(b_)
            g = Gd[b_]; ad = adaylar(mesh, cyl, P[a_], nP[a_])
            for nm, v in ad:
    ...
    print(f"\n{len(X)} satir | dogru aday orani %{100*Y.mean():.0f} -> results/r16_secici_veri.npz")
