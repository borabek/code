...
GEREKCE: gate'in 13 ozelliginden yalnizca `votes` durust bolmede transfer ediyor (AUC dususu
0.018). `votes` = kac bagimsiz uyenin ayni acikligi gordugu -- yani tek genelleyen sinyalin
COZUNURLUGU uye sayisiyla sinirli. Turetilmis 5. uye (olasilik ortalamasi) OLCULDU ve KAYBETTI
cunku bagimsiz degildi; bu kosu gercekten ayri tohumlu bir checkpoint kullanir.

KILL (onceden yazildi): VAL kumesinde tespit F1 +0.01 gelmezse uye ALINMAZ.

Not: gate `votes` ozelligini kullaniyor ve egitim verisi 4 uyeyle uretildi. 5 uyede votes'un
olcegi degisir, o yuzden gate 5-uyeli adaylarla YENIDEN egitilmeden karsilastirma adil olmaz.
Bu yuzden burada gate'i her iki kolda da AYNI aday havuzundan yeniden egitiyoruz (gate_refit
dersi: iki gate ancak AYNI havuzda karsilastirilir).
"""
...
def ek_cikarim(kume):
    """Onbellekteki her parca icin YALNIZ yeni uyenin olasiligini uret -> _probs_<kume>_u5.pkl"""
    import torch
    import diffusionnet as D
    from infer_step_cp import load_any

    src = f"results/_probs_{kume}.pkl"
    if not os.path.exists(src) and kume == "dev":
        src = "results/_h_probs.pkl"
    out = f"results/_probs_{kume}_u5.pkl"
    ...
    pickle.dump(done, open(out, "wb"))
    print(f"-> {out}: {len(done)} parca", flush=True)
    return cache, done
...
def main():
    ...
    print(f"\n{'topluluk':<20}{'tespit':>9}{'ROBOT':>9}{'kesin':>9}{'recall':>9}")
    R = {}
    for n in (4, 5):
        det, rob = kos(n)
        R[n] = (det, rob)
    ...
    json.dump({"kume": kume, "4uye_tespit": f1w(R[4][0]), "5uye_tespit": f1w(R[5][0]),
               "4uye_robot": f1w(R[4][1]), "5uye_robot": f1w(R[5][1]),
               "bootstrap": ok, "karar": "ALINIR" if d_ >= 0.01 else "ALINMAZ"},
              open(f"results/uye5_{kume}.json", "w"), indent=1)
    print(f"makbuz -> results/uye5_{kume}.json")
