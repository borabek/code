# -*- coding: utf-8 -*-
"""g9 (sig 2mm boyama) vs g7 (derin 6mm) -- D6'da ADAY KAHINI + gercek tespit/robot.

KILL: g9 aday kahininde g7'yi GECMEZSE sig boyama GERI ALINIR
(`_label_auto_derin6` duruyor, `g5_agiz_etiket._ESKI_DERINLIK` = (1.0, 6.0)).

Aday kahini gate'ten BAGIMSIZ oldugu icin agin TEMSIL gucunu dogrudan olcer;
gercek tespit/robot ise dagitilan gate ile uctan uca sonucu verir.
"""
...
def main():
    ...
    a = ap.parse_args()
    import protokol; protokol.tez_dogrula()
    ...
    pid_l = [p for p in sorted(kayit) if p in S]
    if a.sinir: pid_l = pid_l[:a.sinir]
    print(f"D6: {len(pid_l)} parca | cihaz {dev}\n")
    sonuc = {}
    for ck, ad in zip(a.ckpt, a.ad):
        ...
        for i, pid in enumerate(pid_l, 1):
            if i % 100 == 0:
                print(f"  {ad} {i}/{len(pid_l)} {(time.time()-t0)/i:.1f}s/parca", flush=True)
            try:
                r = kayit[pid]
                ...
                cps, _pr, _hi, _u = RC.adaylari_uret(V, F, pbs, S[pid], cfg=cfg)
                ...
                T.append((rj,) + esle_macar(P, Dd, G, Gd, r["diag"], 0.0, 180.0, True)[:3])
                R.append((rj,) + esle_macar(P, Dd, G, Gd, r["diag"], 2.0, 10.0, False,
                                            isaretli=True)[:3])
            except Exception:
                hata += 1
        ...
        print(f"\n{ad:<8} KAHIN {kh:.4f} (cok {kc:.4f}) | TESPIT {tf:.4f} | "
              f"ROBOT {rf:.4f} (cok {rc:.4f}) | hata {hata}")
        sonuc[ad] = {"kahin": kh, "kahin_cok": kc, "tespit": tf, "robot": rf,
                     "robot_cok": rc, "hata": hata}
    if len(sonuc) == 2:
        a1, a2 = a.ad
        print(f"\nFARK ({a2} - {a1}): kahin {sonuc[a2]['kahin']-sonuc[a1]['kahin']:+.4f} | "
              f"tespit {sonuc[a2]['tespit']-sonuc[a1]['tespit']:+.4f} | "
              f"robot {sonuc[a2]['robot']-sonuc[a1]['robot']:+.4f}")
        ok = sonuc[a2]["kahin"] > sonuc[a1]["kahin"]
    ...
    print("makbuz -> results/p9_ag_kiyas.json")
