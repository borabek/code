...
NEDEN YENI BIR KUME (2026-08-05 denetimi):
Eski kume (`d5_4_sinav_kumesi.json`, 250 parca) "gorulmemis URETICI sinavi" adiyla
kurulmustu ama dislama yalniz PARCA + IKIZ duzeyindeydi. Korpus buyudukce ayni
ureticilerin baska parcalari egitime girdi. OLCULDU:

  * seg oto-etiket korpusu (`_label_auto_obj`): sinav kumesinin 250 parcasindan
    **169'u DOGRUDAN icinde** (%68) + sinav ureticilerinden 958 parca
  * gate korpusu (`zengin_parite_v3`): sinav ureticilerinden 1502 tekil parca

Yani o kumede olculen "gorulmemis uretici" sayilari SISIK. Eski kumede kurtarilacak
temiz uretici de kalmadi (13'un 12'si seg egitiminde).

COZUM: DataSet 6 ile gelen ureticiler HICBIR egitim yapitinda yok -- bugun geldiler.
Onlardan kurulan kume, MEVCUT ckpt'ler icin gercekten gorulmemis uretici sinavidir
ve yeniden egitim GEREKTIRMEZ.

UC KATMANLI TEMIZLIK (eski kumeden aynen devralindi):
  1. URETICI: hicbir egitim yapitinda (w2, v3, _label_auto_obj, _label_targets*) YOK
  2. GEOMETRI GRUBU: anahtari egitim / 194'luk olcum / LOCKED anahtarlariyla carpismaz
  3. GRUP-ICI TEKILLIK: ayni geometri grubundan tek parca

URETICI KAPISI n>=5 ([[gorulmemis-uretici-sinavi-ilk-olcum]] WAGO dersi: tek parcali
uretici gurultuden ibaret).
...
def egitimde_gorulen_ureticiler():
    """HER egitim yapitindaki ureticiler. Biri bile atlanirsa kume KIRLENIR."""
    gor = set()
    kaynak = {}
    ...
    # seg egitim korpuslari: pid -> uretici esleme gerekiyor
    import big_arbiter
...
def main():
    ...
    if not aday_u:
        print("HIC temiz uretici yok -- kume kurulamaz."); return

    # turetme kayitlari (GT + geometri anahtari icin)
    R = []
    ...
    kayit = {}
    for r in R:                      # ayni pid birden cok shard'da olabilir
        kayit.setdefault(r["pid"], r)
    print(f"turetme kaydi: {len(kayit)} parca")

    YASAK = set()
    if os.path.exists(GEO_YASAK):
        YASAK = set(json.load(io.open(GEO_YASAK, encoding="utf-8")).values())
    print(f"yasak geometri anahtari (egitim/olcum/LOCKED ikizleri): {len(YASAK)}")
    ...
    for m, p, _jf, _s in sorted(E, key=lambda t: (t[0], t[1])):
        ...
        # `r["G"]` numpy dizisi olabilir -- `or` ile bosluk kontrolu ValueError verir
        _g = r.get("G")
        ...
        if anah and anah in YASAK:
            atlanan["yasak_anahtar"] += 1; continue      # egitim/olcum IKIZI
        if anah and anah in gruplar:
            atlanan["grup_ici_tekrar"] += 1; continue    # ayni geometri grubu
        if anah:
            gruplar.add(anah)
    ...
    out = {"n_parca": len(secili), "sha16": muhur, "uretici": dict(up),
           ...
           "pidler": pidler, "parcalar": secili,
           "min_uretici": MIN_URETICI,
           "not": ("TEMIZ gorulmemis-URETICI sinavi (D6). Bu ureticiler hicbir egitim "
                   "yapitinda YOK. Eski d5_4 kumesi URETICI duzeyinde kirlenmisti "
                   "(seg korpusu sinavin 169/250 parcasini iceriyordu) ve onunla "
                   "KIYASLANAMAZ. 194'luk olcum kumesi AYRI ve dondurulmus kalir.")}
    with io.open(CIKTI, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=1, ensure_ascii=False)
    print(f"\nSINAV KUMESI: {len(secili)} parca | {len(up)} uretici | "
