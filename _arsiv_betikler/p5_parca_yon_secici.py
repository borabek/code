...
OLCULMUS DAYANAK (2026-08-06, D6):
  ...
  * {+-mesh eksenleri, +-candidate bulutu PCA eksenleri} = 12 secenek uzerinden KAHIN robot
...
def main ():
    ...
    for threshold in (0.0 ,0.3 ,0.5 ,0.7 ):
        ...
        izg [str (threshold )]={"tespit":tf ,"robot":rf }
        print (f"  threshold {threshold :.1f}: tespit {tf :.4f} robot {rf :.4f}")
        if rf >en_r :
