...
on a cover plate. The STEP header is unreliable (653 files carry no product name at
...
# WSCAD category text that means "this is a terminal block we can train on"
...
def main():
    ...
    print(f"[{args.shard}] bitti: {len(rows)} parca -> {args.out}", flush=True)
