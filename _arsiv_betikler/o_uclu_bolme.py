...
hem de OLCMEK for kullanildi. Bu, secilen each seyin that kumeye a miktar uydurulmasi demek --
...
COZUM: keskin geometri anahtariyla (bbox 0.5mm + B-rep silindir/duzlem imzasi) UC AYRIK grup:
  ...
  LOCKED -- single atislik final. Ustunde HICBIR ayar yapilmaz.

Ek rule: LOCKED, segmentasyon training+val geometrileriyle de same grupta OLMAZ. Boylece
"gorulmemis geometri" iddiasi first times gercekten correct becomes.
...
def main ():
    ...
    print (f"\ntemiz pool (ne DEV ne segmentasyon): {len (kalan )} part "
