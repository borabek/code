# -*- coding: utf-8 -*-
"""Q11: `metal_mesafe` TUM metal yuzlerle mi, yoksa yalniz metal SILINDIRLERLE mi olculmeli?

GEREKCE: tel kelepcesi silindirik bir yuzey DEGIL, duz metal bir yuzeydir. Mevcut EK blogu
metal mesafesini yalniz metal SILINDIRLERDEN hesapliyor (q8'de olculen AUC 0.639 da oyleydi).
Tum yuzlere gecince metal yuz sayisi orneklemde 8 -> 38 cikiyor (cogu duzlem).

KILL: tum-yuz surumu silindir-surumunu AUC'de gecmezse degisiklik YAPILMAZ (uretim yeniden
baslatilmaz). Gecerse uretim yeniden baslatilir -- ~20dk hesap COPE gider, o yuzden once bu.
"""
...
def main():
    ...
    print(f"{len(Y)} aday | TP {int(Y.sum())} | renk cozulen parca: silindir-yol {kab_c}, "
          f"tum-yuz yol {kab_t} / {len(parts)}")
    rng2 = np.random.RandomState(0)
    print(f"\n{'surum':<26}{'AUC':>8}{'null p95':>10}{'TP ort':>10}{'FP ort':>10}")
    res = {}
    ...
    print(f"KILL: yeni surum eskiyi gecmezse DEGISIKLIK YOK -> {'DEGISTIR' if d > 0 else 'BIRAK'}")
    json.dump(res | {"fark": float(d), "kabul_silindir": kab_c, "kabul_tum": kab_t,
                     "n_parca": len(parts)},
              open("results/q11_metal_duzlem.json", "w"), indent=1)
    print("makbuz -> results/q11_metal_duzlem.json")
