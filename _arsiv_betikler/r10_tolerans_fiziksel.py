...
Kod tabaninda 2mm'nin gerekcesi YOK -- `GEO_TEZE_SADIK_PLAN.md:44` yalnizca "Robot hedefi
<2mm" diye BEYAN ediyor. Fiziksel dogrusu: tel agizdan gecebiliyorsa robot takabilir, yani
konum toleransi = (agiz ic yaricapi - tel yaricapi) = BOSLUK.

`mouth_width` A0'da kalibre edildi (%1 sapma). Her eslesen CP'de agzin gercek ic capini
olcup, klemens teli capina gore BOSLUGU cikariyoruz. Sonra: bu fiziksel toleransla
robot-hazir F1 kac?

BU SISIRME DEGIL. Sisirme, sonucu guzellestirmek icin esigi gevsetmektir. Burada esik
GEOMETRIDEN turetiliyor ve parca basina DEGISIYOR -- dar agizda 2mm'den SIKI olabilir.
"""
...
for r in D:
    ...
    for dd,a_,b_ in sorted((pe2[a,b],a,b) for a in range(len(P)) for b in range(len(G))):
        ...
        try:
            if r['pid'] not in VC:
                V,F=step_to_mesh(yol[r['pid']]); V,F=thesis_remesh.remesh_uniform(V,F,target=6000)
...
ok=ic>0.1
print(f"olculen {len(K)} cift | agiz capi olculebilen {ok.sum()}")
...
print(f"  bosluk >= 2mm olan: %{100*(bosluk[ok]>=2).mean():.0f}   (yani 2mm bunlarda GEREKSIZ SIKI)")
print(f"  bosluk <  1mm olan: %{100*(bosluk[ok]<1).mean():.0f}   (yani 2mm bunlarda FAZLA GEVSEK)")
def f1(msk):
    tp=int(msk.sum()); return tp
print(f"\n{'esik':<34}{'gecen cift':>12}{'oran':>8}")
