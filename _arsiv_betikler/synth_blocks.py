...
blocks. A from-scratch geometric detector trained on that few parts memorises those
specific shapes and fails on an UNSEEN part -- the user's core complaint. The fix is
...
Two uses, which COMBINE (the standard small-data recipe):
  1. PRETRAIN the knngraph on a large synthetic set -> weights that already know
     "concave cylindrical opening = CP".
  2. FINE-TUNE those weights on the 136 real parts -> adapts to real detail.
...
Geometry is in MILLIMETRES (the whole pipeline's unit). The knngraph backbone keys on
...
def sample_params(rng):
    ...
    # directly on the real corpus (nearest-vertex dist / bbox diagonal): ABB median
    ...
    r_distract = float(rng.uniform(0.6, 1.1)) if rng.random() < 0.5 \
        else float(rng.uniform(3.4, 4.5))            # clearly not the CP radius
...
def build_block(p, rng):
    ...
    # skip entirely if even that leaves no room (never crash on a tight blueprint).
    ...
    # BELOW the tessellated floor (not exactly on the floor-centre fan vertex) --
    ...
    # Apply axis permutation to put CP holes on the requested face.
    # "top"  (default): holes on XY-top face, dir=[0,0,1] — no change.
