# -*- coding: utf-8 -*-
"""R7: UYE-ORACLE -- dort modelin ATILAN yon/konumlarinda ne kadar bilgi var?

Denetimin P2 maddesi. `robot_cp._vote2` anlasan uyeleri birlestirirken KONUMU guven-agirlikli
ortaliyor ve YONU temsilciden aliyor; digerlerinin yonu ATILIYOR.

BU OLCUM NEDEN SIMDI DEGERLI: pose head'den sonra baglayici sart ACI'ya dondu
(yanal gecis %88.9, aci %80.4). Dort modelden BIRI dogru yonu bulmus olabilir ve biz onu
atiyor olabiliriz. Bu, aci kolunda hic denenmemis TEK yerdir.

OLCULEN (kahin = GT'ye bakip en iyi uyeyi secmek; ULASILABILIR HEDEF DEGIL, UST SINIR):
    K0  mevcut birlestirme (dagitilan)
    K1  + en iyi uye YONU secilseydi
    K2  + en iyi uye KONUMU secilseydi
    K3  + ikisi birden

KILL (denetimin yazdigi): oracle kazanci < 0.02 ise kol OLDURULUR; buyukse ogrenilmis bir
uye secici kurulur.
"""
import io
import json
...
def main():
    ...
    # UYE bazinda adaylar: olasilik onbelleginden yeniden turet (ag cikarimi YOK)
    VF = {}
    for kume in ("dev", "val"):
        ...
        with open(cf, "rb") as f:
            for r in pickle.load(f):
                ...
                if pid in izin:
                    VF[pid] = r
    print(f"{len(VF)}/{len(izin)} parcanin olasiligi var", flush=True)
    ...
    for i_, (pid, r0) in enumerate(VF.items(), 1):
        ...
        # her kabul edilen CP icin, 5mm icindeki UYE adaylarini topla
        UP, UD = [], []
        ...
        # KAHIN secim: her CP icin GT'ye EN YAKIN uye (yon / konum)
    ...
    print(f"\n{len(gruplar)} parca puanlandi")
    print(f"\n{'kol':<28}{'tespit':>10}{'ROBOT':>10}{'d_robot':>10}")
    ad_uzun = {"K0": "K0 mevcut birlestirme", "K1": "K1 +kahin uye YONU",
               "K2": "K2 +kahin uye KONUMU", "K3": "K3 +ikisi birden"}
    r0v = f1w(rob["K0"])
    SON = {}
    ...
    print(f"KILL (denetimin yazdigi): oracle kazanci < 0.02 ise KOL OLU -> "
          f"{'CANLI, ogrenilmis secici kurulabilir' if kaz >= 0.02 else 'OLU'}")
    with io.open("results/r7_uye_oracle.json", "w", encoding="utf-8") as f:
        json.dump({"kollar": {k: list(v) for k, v in SON.items()}, "en_iyi": en,
                   "oracle_kazanc": float(kaz), "canli": bool(kaz >= 0.02)}, f, indent=1)
    print("makbuz -> results/r7_uye_oracle.json")
