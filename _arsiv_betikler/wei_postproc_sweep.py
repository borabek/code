# -*- coding: utf-8 -*-
"""WEI BEDAVA-RECALL sweep: teshis dedi ki FN'lerin %11'i turetme-kacisi (min_v cozer) + bir kismi
esik-alti (FN max-olasilik medyan 0.36 -> vconf dusurmek yakalar). Sizinti-siz 145 WEI held-out'ta
2D grid (min_v x vertex_conf) tara; her hucrede WEI P/R/F1. Urunu (min_v30/vc0.5) referans al.
Robot tier mantigi: dusuk-vconf CP'ler dusuk-guven -> REVIEW'e duser, AUTO precision korunur.
"""
...
def main():
    ...
    print(f"{len(raw)} WEI held-out parca | {os.path.basename(CKPT)}", flush=True)
    ...
    print(f"  {len(cache)} parca onbellekte\n", flush=True)
