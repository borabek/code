# -*- coding: utf-8 -*-
"""P4 onkosulu: silindir DISI acikliklari parca basina bir kez cikar, diske yaz."""
...
def main():
    ...
    print(f"cikarilacak {len(hedef)} parca", flush=True)
    t0 = time.time()
    for i, p in enumerate(hedef, 1):
        try:
            ob[p] = brep_aciklik.acikliklar(S[p])
        except Exception:
            ob[p] = []
        if i % 25 == 0:
            print(f"  {i}/{len(hedef)}  {(time.time()-t0)/i:.1f}s/parca", flush=True)
            with open(yol, "wb") as f:
                pickle.dump(ob, f)
    with open(yol, "wb") as f:
        pickle.dump(ob, f)
    n = sum(1 for v in ob.values() if v)
    print(f"BITTI: {len(ob)} parca | aciklik BULUNAN {n} | toplam aciklik "
