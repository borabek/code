"""POST-ISLEME SWEEP: candidate-tavanini (kapsama) yukselten parametreleri bul.
Mevcut degerler (min_v 30 / vc 0.5 / cluster 5 / dedupe 10) F1 for ayarlanmisti.
YENI TARGET = KAPSAMA -> this parametreler yeniden ayarlanmali.
Verimli: each part for segmentasyon BIR KEZ runs, after parametreler ucuz taranir."""
...
print (f'{len (cache )} part hazir ({time .time ()-t0 :.0f}s). Sweep basliyor...\n',flush =True )
...
print ("%-42s %10s %10s"%("parametreler","TAVAN(kapsama)","candidate/GT"))
