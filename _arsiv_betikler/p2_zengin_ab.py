# -*- coding: utf-8 -*-
"""P2: ZENGIN BLOKLAR A/B -- 22 sutun vs 22+33, AYNI adaylarla.

Denetimin P1 maddesi. Eski kanit (results/rich_gate_receipt.json) 13 SUTUNLU gate zamanindan:
PART_out +0.0351, FAMILY_out_STRICT +0.0489, AUC 0.9275->0.9549 / 0.8956->0.9373.
Gate o gunden beri 22 sutuna cikti (B-rep fiziksel + icbukey topoloji). Zengin bloklarin
KATTIGI SEY hala var mi?

TASARIM: `build_zengin_parite.py` HEM 22 temel HEM 33 zengin sutunu AYNI adaylar icin uretti.
Yani A/B'de tek degisken ozellik kumesi. (Ayri kosularda uretilmis iki npz'yi karsilastirmak
bu projede daha once yaniltmisti.)
...
Dagitilan yapinin DONUSUMU (parca-ici z-skor) her kolda uygulanir -- urunle ayni.

KUME: olcum_kumesi (split3, dedup, LOCKED disarida). BOOTSTRAP: geometri grubu.
KARAR: karar_olcutu (GA karara katilir).
"""
import collections
import io
import json
...
def main():
    ...
    print(f"{X22.shape[0]} aday | temel {X22.shape[1]} | zengin {XR.shape[1]} "
          f"({len(ad)} ad) | {len(np.unique(pid))} parca | votes maks {d['votes'].max()}")
    ...
    # OLCUM tarafinda zengin sutunlar YOK (DER onbellegi yalniz 22 sutun tasiyor).
    # Bu yuzden A/B ADAY DUZEYINDE degil, ZENGIN NPZ'nin KENDI parcalari uzerinde,
    # grup-capraz OOF ile yapilir; sonra kazanan kol uctan uca dogrulanir.
    from sklearn.model_selection import GroupKFold

    def olc(M, maske=None):
        """Grup-capraz OOF + parca-ici goreli esik -> aday duzeyi F1 (rejim-agirliksiz)."""
    ...
    # Yeterli GRUBU olan ureticiler (GroupKFold 5 kat ister; WAGO gibi tek-parcali
    # ureticiler bolunemez).
    kodlar = sorted(k for k in set(mfg) if len(set(grp[mfg == k])) >= 10)
    print(f"\n{'kol':<18}{'genel':>9}" + "".join(f"{k + '-disi':>12}" for k in kodlar))
    SON = {}
    ...
    a = SON["A 22 temel"]
    print(f"\n{'kol':<18}{'genel fark':>12}" + "".join(f"{k:>10}" for k in kodlar))
    for k in KOL:
        if k.startswith("A"):
            continue
    ...
    print("makbuz -> results/p2_zengin_ab.json")
