...
Iki metrik: (i) threshold-modu F1 (deployed base urun), (ii) top-N F1 (metadata-assisted mod)."""
import json 
import numpy as np 
...
# aile anahtari (etiketsiz): candidate-bulutu bbox + log2(candidate count)
FAM ={}
...
print (f"{len (Y )} candidate | {int (Y .sum ())} TP | {len (ngt_of )} part | {len (uf )} aile | rich-dim {XR .shape [1 ]}")
print (f"  WEI {int ((MF ==1 ).sum ())} candidate / PXC {int ((MF ==0 ).sum ())} candidate\n")
