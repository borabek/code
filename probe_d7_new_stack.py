...
YANAL ,ACI =2.0 ,10.0 
sv =json .load (open ("results/d7_exam_set.json"))
...
def kos (ob ,gate ,p5 =None ):
    ...
    for pid in pidler :
        ...
        if p5 is None :
            ...
            secs =PS .options (P ,D ,p5 ["cyl"].get (pid ),p5 ["acik"].get (pid ),
            r ["diag"],gate_s =gs ,komsu =komsu )
            score =[p5 ["clf"].predict_proba (np .asarray ([s [2 ]for s in o ],float ))[:,1 ]
            for o in secs ]
            P ,D =PE .sec (secs ,score ,gate_skor =gs ,gate_esik =(0.30 ,0.30 ))
...
res_ ={}
t ,r =kos (OB_ESKI ,g5 );res_ ["1_eski (g7+gate v5)"]={"detection":t ,"robot":r }
print (f"1) eski yigin (g7 + gate v5)      detection {t :.4f} | robot {r :.4f}",flush =True )
t ,r =kos (OB_YENI ,g7g );res_ ["2_A3A4 (g10+gate v7)"]={"detection":t ,"robot":r }
print (f"2) g10 + gate v7 (A3-A4)          detection {t :.4f} | robot {r :.4f}",flush =True )
json .dump ({"damga":receipt_hash .damga (),"sonuc":res_ ,"n_parca":len (pidler ),
"not":"D7 = DEV (harcandi). FINAL DEGIL. 0.2344 with AYNI cluster."},
open ("results/d7_new_stack.json","w"),indent =1 )
print ("receipt -> results/d7_new_stack.json")
