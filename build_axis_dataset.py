# -*- coding: utf-8 -*-
"""C ADIMI, 1/2: EKSEN veri seti -- uretici InsertDirection'larini KOSE hedefine cevir.

NEDEN: elimizde 8534 uretici ConnectionPoint yonu var ve model bunlari HIC gormedi. Eksen su an
tamamen geometrik sezgiyle cikariliyor. Bugun olculdu: eksen yonu robot-hazir F1'in 0.109'unu
tek basina tutuyor, ve B-rep duzeltmesinden sonra bile CP'lerin %16'sinda eksen 15 dereceden
fazla sapiyor. Geometrinin cozemedigi kisim yuva/kelepce girisleri -- oralarda silindir yok,
yani analitik eksen de yok. Ogrenmek tek kalan yol.

TASARIM
  * Hedef yalnizca uretici CP'sinin YAKININDAKI kosede tanimlidir; gerisi MASKELENIR. Boylece
    CAD sozde-etiketlerine hic dokunmayiz (onlar insan GT'siyle ancak F1~0.46 ortusuyor).
  * Yon ISARETSIZ ogrenilir (kosinusun MUTLAK degeri): uretici isaret konvansiyonu parcadan
    parcaya degisiyor, isareti zaten geometrik olarak (outward) belirliyoruz.
  * Cerceve: hedefler cad_eval.align_frames ile MESH cercevesine tasinir -- egitim ve cikarim
    ayni cercevede olmali, yoksa ogrenilen sey gurultu olur.

Cikti: results/axis_dataset/<parca>.npz  (idx, target_dir) + results/axis_dataset/index.json
"""
...
def main():
    ...
    ap.add_argument("--radius", type=float, default=4.0,
                    help="uretici CP'sine bu mesafedeki koseler hedef alir (mm)")
    ...
    if a.limit:
        parts = parts[:a.limit]
    print(f"{len(parts)} parca | {len(done)} zaten hazir | yaricap {a.radius}mm", flush=True)
    ...
    for k, (mfg, pid, jf, stp, n) in enumerate(parts, 1):
        ...
        try:
            ...
            Gm = (G - t) @ R                      # uretici CP -> mesh cercevesi
            Gdm = Gd @ R
            ...
            continue
        if k % 50 == 0:
            print(f"  {k}/{len(parts)}  hazir {npart} parca / {nv_tot} hedef kose", flush=True)
    ...
    print(f"\nbitti: {npart} yeni parca, {nv_tot} hedef kose | toplam {len(allp)} parca")
    print(f"-> {OUT}/")
