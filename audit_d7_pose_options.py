...
This module deliberately has no dependency ten the production selector.  A branch is a
    ...
    for ci ,cylinder in enumerate (cylinders or ()):
        ...
        for mi ,mouth in enumerate ((mouth_a ,mouth_b )):
            if float (np .linalg .norm (mouth -p0 ))<=mm_max :
                ...
                # Candidate's axial coordinate ten the finite mouth-to-mouth centerline.
...
tuple [tuple [np .ndarray ,str ,str ],...],
    ...
    product can create thousands of redundant options ten cylinder-dense parts.
    """
    primitives =_union (
    catalog .current ,
