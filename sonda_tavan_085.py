# -*- coding: utf-8 -*-
"""ROBOT TAVANINI 0.85'e TASI: konum ve yon kaynaklarini kademe kademe zenginlestir.

DURUM (D7 marka-disi, `results/konum_yon_sik_d7.json`): robot F1 tavani 0.7273.
Kalan kayip: KONUM YOK %31.8, YON HICBIR KAYNAKTA YOK %11.0.
0.85 icin recall 0.5714 -> 0.7391 gerekiyor (F1 = 2r/(1+r)).

KONUM kaynaklari kademeli:
  P0  segmentasyon `v_o` adaylari (TEZ)
  P1  + B-rep agizlari (silindir uclari + duzlemsel aciklik merkezleri)
  P2  + eksen boyu ornekleme
  P3  + MESH TEPELERI  -- olculmustu ki GT'lerin %98.1'inin 2mm yanal
      komsulugunda tepe var ([[mesh-tavan-degil-yanal-model-hatasi]]); yani konum
      bilgisi meshte ZATEN VAR, sorun onu SECEBILMEK.

YON kaynaklari kademeli:
  Y0  adayin kendi yonu
  Y1  + 10mm icindeki komsu adaylarin yonleri
  Y2  + parcadaki B-rep silindir eksenleri (+/-)
  Y3  + parcanin ANA EKSENLERI (kutu eksenleri +/-, 6 yon) -- bedava, parca
      hizasi cogu klemenste giris yonunu tasiyor

DURUSTLUK: bunlar TAVAN. P3 kolu parca basina binlerce konum demektir, dagitilabilir
bir sistem DEGIL; sorusu "bilgi meshte var mi" sorusudur, "urun bunu yapabilir mi"
sorusu DEGIL. Tez turetmesi hicbir kolda degismiyor; `v_o` her zaman P0'da.
"""
import collections
import json
import os
...
def main():
    ...
    KOLLAR = [("P0+Y0  tez-saf", 0, 0), ("P1+Y0  +B-rep agiz", 1, 0),
              ("P2+Y0  +eksen ornek", 2, 0), ("P2+Y1  +komsu yonu", 2, 1),
    ...
    for pid, r in sorted(kay.items()):
        ...
        if eks:
            _e = _birim(eks)
            eks = np.vstack([_e, -_e])      # isaretli olcum icin +/- ciftleri
        else:
            eks = np.zeros((0, 3))
        ana = np.zeros((0, 3))
        if len(V) > 3:
            ...
            ana = np.vstack([_a, -_a])      # isaretli olcum icin +/- ciftleri
        havuz = {0: (P0, D0), 1: (Pb, _birim(Db)), 2: (P2p, D2p),
        ...
        for ad, pk, yk in KOLLAR:
            ...
            for j in range(len(G)):
                ...
                def uy(Dv):
                    """ISARETLI aci -- urunun metrigiyle AYNI.

                    Ilk surum `np.abs(...)` ile ISARETSIZ olcuyordu; bu, ters
                    yone bakan bir adayi dogru sayar ve tavani sisirir
                    ([[isaretsiz-kahin-artefakti]]: ~0.08). Yon kaynaklarinin
                    hepsi zaten +/- ciftleriyle havuzda oldugu icin isaretli
                    olcum kaynak cesitliligini KAYBETTIRMEZ.
                    """
                ...
                if yk >= 3 and len(ana) and uy(ana):
                    say[ad]["Y3 ana eksen"] += 1
                    continue
                say[ad]["YON YOK"] += 1

    print(f"D7 {n_gt} GT | mesh onbellegi olmayan parca {eksik}\n")
    print(f"{'kol':<28} {'recall':>8} {'F1 tavani':>10} {'KONUM YOK':>10} {'YON YOK':>9}")
    out = {}
    for ad, _, _ in KOLLAR:
        c = say[ad]
        # BASARI ANAHTARLARI ACIKCA SAYILIR.
        # ILK SURUM `k.startswith("Y")` diyordu ve "YON YOK" da Y ile basladigi
        # icin BASARISIZLIGI basariya ekliyordu: Y1/Y2/Y3 kollarinda YON YOK
        # duserken recall SABIT kaliyordu (imkansiz) ve P3 tavani 0.9989 gibi
        # SAHTE bir sayi veriyordu. Kalip eslesmesiyle sayim YAPILMAZ.
        BASARI = ("Y0 kendi", "Y1 komsu", "Y2 silindir ekseni", "Y3 ana eksen")
    ...
    json.dump({"damga": makbuz_hash.damga(), "n_gt": n_gt, "sonuc": out,
               "not": "TAVAN olcumu, mukemmel secici. P3 kolu parca basina binlerce "
                      "konum -- dagitilabilir sistem DEGIL, 'bilgi meshte var mi' "
                      "sorusunun cevabi. D7 marka-disi."},
              open("results/tavan_085.json", "w"), indent=1)
    print("makbuz -> results/tavan_085.json")
