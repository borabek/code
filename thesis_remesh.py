...
ADAPTIVE_DENSITY = float(os.environ.get("CP_ADAPTIVE_DENSITY", "0"))   # 0 = kapali (sabit hedef)


def adaptive_target(V, F, density=0.790, lo=4000, hi=20000):
    """Tezin izotropik kurali UNIFORM YOGUNLUK der; sabit vertex SAYISI parcalar farkli
    boyuttayken bunu ihlal eder (olculdu: cok-CP parcalari %51 daha buyuk ama ayni 6000 vertex ->
...
def remesh_uniform(V, F, target=6000, iterations=8):
    ...
    ayni STEP, ayri sureclerde farkli vertex kumesi ureti (4 surecte 3 ayni 1 farkli; olculdu
    `S1_mesh` hash'i ile). Bu, asagi akista farkli CP sayisina yol aciyordu (WEI.1547610000: 4/5/4).
    Cozum: sonucu DISKE onbellekle -- ilk hesap kazanir ve o parca icin sonsuza kadar ayni mesh
    doner. Yan fayda: tekrar kosularda remesh maliyeti sifir.
    Kapatmak icin: CP_REMESH_CACHE=0 (o zaman eski, oynak davranis geri gelir).
    Onbellek float32/int32 saklar; DEGERLER HER IKI YOLDA DA AYNI olsun diye hesaplanan sonuc da
    ayni yuvarlama uygulanarak dondurulur (yoksa ilk kosu sonrakilerden farkli olurdu).
    """
    ...
    Vo = np.asarray(cm.vertex_matrix(), np.float32)      # onbellekle AYNI yuvarlama (bkz docstring)
    Fo = np.asarray(cm.face_matrix(), np.int32)
    if use_cache:
        os.makedirs(REMESH_CACHE, exist_ok=True)
        tmp = fp + f".tmp{os.getpid()}"                  # atomik yazim: yarim dosya okunmasin
