# -*- coding: utf-8 -*-
"""U4: PARCA-ICI ozellik normalizasyonu UCTAN UCA (tanidik + iki gorulmemis uretici).

U3 aday duzeyinde: en kotu uretici-disi 0.4597 -> 0.5219 (+0.062), tanidik bedeli +0.0003.
Aday duzeyi bu gece UC KEZ yaniltti (EK blogu, goreli esigin bedeli, sinif dengeleme) --
karar BURADA.

DAGITILABILIRLIK KONTROLU (onemli): donusum PARCA ICINDE tanimli, yani calisma aninda bir
parcanin KENDI adaylarindan hesaplanir. Korpus istatistigi, komsu parca, uretici kimligi
GEREKMEZ -- gercek robot akisinda birebir uygulanabilir. (Aksi bir donusum olculebilir ama
dagitilamazdi; bu ayrimi bilerek yaziyorum.)

KILL (onceden yazili, BUYUKLUK dahil):
  (a) tanidik tespit F1 >= -0.01  VE
  (b) gorulmemis uretici EN KOTU durumu >= +0.01 artmali.
Ikisi birden olmazsa DAGITILMAZ. (t15/TOPO ile ayni cubuk + buyukluk sarti.)
"""
import collections
import json
import os
...
def uygula(X, fn):
    """Tek parcanin aday matrisine donusumu uygula (calisma anindaki islemin AYNISI)."""
...
def main():
    ...
    print(f"\n{'bolme':<24}{'arm':>14}{'tespit':>9}{'ROBOT':>9}{'kesin':>9}{'recall':>9}")
    R = {}
    for ad, fn in DONUSUM.items():
        M = XTR[ad]
    ...
    print(f"KARAR (taban A ham: tanidik {t_tan:.4f} | gorulmemis en kotu {t_kotu:.4f})")
    kazanan = None
    for ad in DONUSUM:
        if ad == "A ham":
            continue
    ...
    print("makbuz -> results/u4_parca_ici_uctan_uca.json")
