# -*- coding: utf-8 -*-
"""T9: HAVUZA ACI-DUZELTMESI ONCESI YONU DE KOY (tek degisken).

BULGU: dagitilan zincir su sirayla calisiyor
    pose -> aci_duzelt (YONU DEGISTIRIR) -> uye_yonu_sec (havuz = degistirilmis yon + uyeler)
Yani uye secicinin "aci duzeltmesini GERI AL" secenegi YOK; duzeltilmis yon havuzda tek
temsilci olarak duruyor.

T8'de havuza IKISI birden konmustu (duzeltme oncesi + sonrasi) ve kahinin %66'si yakalanmisti;
dagitilan zincirde %52. Ama T8'in tabani farkliydi, yani karsilastirma KIRLI.

BU BETIK tek degiskenle olcer:
    A  dagitilan zincir (havuz = aci-duzeltilmis yon + uyeler)
    B  havuz = aci-duzeltilmis yon + DUZELTME ONCESI yon + uyeler

Mantik: aci duzeltmesi bazen YANLIS duzeltiyor olabilir; secici o durumda eski yonu geri
secebilmeli. Bu, "kararsizsa dokunma" ilkesinin secici tarafindan uygulanmis hali.

KILL: robot >= +0.01 VE GA sifiri dislamali VE tespit degismemeli.
"""
import io
import json
import os
...
def main():
    ...
    print(f"gate + uye secici hazir ({uye_m['n_feat']} sutun)", flush=True)

    def sec_yon(X58i, havuz):
        DIR = np.array([h[0] for h in havuz])
        DIR = DIR / (np.linalg.norm(DIR, axis=1, keepdims=True) + 1e-9)
        CONF = np.array([h[1] for h in havuz]); MES = np.array([h[2] for h in havuz])
    ...
    dA, rA = puanla(False)
    dB, rB = puanla(True)
    print(f"\n{'kol':<34}{'tespit':>10}{'ROBOT':>10}")
    print(f"{'A dagitilan (havuz: duzeltilmis)':<34}{f1w(dA):>10.4f}{f1w(rA):>10.4f}")
    print(f"{'B +duzeltme ONCESI yon de':<34}{f1w(dB):>10.4f}{f1w(rB):>10.4f}")
    dr = f1w(rB) - f1w(rA); dt = f1w(dB) - f1w(dA)
    fn = lambda rows: f1w([y for _, y in rows]) - f1w([x for x, _ in rows])
    _, lo, hi = olcum_kumesi.grup_bootstrap(list(zip(rA, rB)), g, fn, n=2000)
    print(f"\nrobot {dr:+.4f} [{lo:+.4f}, {hi:+.4f}] | tespit {dt:+.4f}")
    gecti = dr >= 0.01 and lo > 0 and abs(dt) < 1e-9
    print(f"KILL: robot >= +0.01 VE GA sifiri dislamali VE tespit degismemeli -> "
    ...
    print("makbuz -> results/t9_havuza_ikisi.json")
