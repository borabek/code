...
C global olarak kaybetti (val >15d %30.2 vs urun %15.8). Ama urunun ekseni HER YERDE iyi degil:
adaylarin ~%15'inde B-rep hicbir yuzey eslestiremiyor ve orada mevcut yontem (normal-kovaryans
/ yuvarlama) zayif. Soru: C tam O ALT KUMEDE daha mi iyi? Oyleyse C "global kazanan" degil
"B-rep'in sustugu yerde konusan uye" olarak urune girer.

KILL: alt kumede C'nin >15d hatasi mevcut yontemi GECMEZSE C tamamen duser.
"""
...
def main():
    ...
    # DIKKAT: C'nin egitim seti PXC+WAGO+SIE, dogrulama WEI. Bu 100 parcanin bir kismi C'nin
    # EGITIMINDE olabilir -> sizinti. O yuzden yalnizca WEI parcalarinda (C icin gorulmemis)
    # rapor edilir; PXC ayri gosterilir ama karara girmez.
    ...
    for (m, pid, jf, stp, n), r in zip(sel, cache):
        ...
        for d_, a_, b_ in sorted((pe[a, b], a, b) for a in range(len(Q)) for b in range(len(G))):
            ...
            # C'nin tahmini: CP'nin 4mm yakinindaki koselerin ortalama yonu
            ...
            ref = v[0]
            v = v * np.sign((v @ ref))[:, None]        # isaretsiz ortalama
