# -*- coding: utf-8 -*-
"""R6: KONUM KOLUNUN IKI AYRI SORUSU -- once UCUZ olani.

R5 TESHISI: kurtarma adayi 42, bozma riski tasiyan gecen nokta 857. 1:20 ASIMETRI.
Yanlis atesleyen bir secici kurtardiginin yirmi katini kirar. O yuzden bu kolun sorusu
"hangi giris daha iyi" DEGIL.

IKI AYRI SORU VAR ve BIRINCISI SECICI GEREKTIRMIYOR:

  A) TANIM SORUSU (ucuz, ogrenme yok, tez-sadik):
     Tez v_o'yu "agiz sinir noktalarinin ortalamasi" diye tanimlar. [[brep-radius-arc-bug]]
     kaydi ayni ailedeki yanliligi olcmustu: kismi ornekleme varsa ORTALAMA dolu tarafa
     kayar, cember oturtma sart. `cember` (en kucuk kareler merkez) ve `acik` (agiz
     duzleminin Chebyshev merkezi = telin gecebilecegi en genis yer) AYNI BUYUKLUGUN
     daha iyi kestiricileri -- yeni bir kavram degil. Eger biri GLOBAL olarak daha iyiyse
     kol burada biter: secici yok, kumar yok, tek satirlik tanim degisikligi.
     R5'te kurtaranlarin basinda `acik` (13/42) ve `kesit*` (21/42) vardi -- ama bunlar
     KURTARDIKLARI yerde sayildi; BOZDUKLARI yerde sayilmadi. Net etki burada olculur.

  B) ATESLEME SORUSU (pahali, ogrenme var):
     Global kazanan yoksa secici sart; ve seciciyi mesru kilan TEK sey "mevcut nokta
     kotu mu"yu bilebilmektir. Onu bilmiyorsak argmax 1:20 kumarina girer. GRUP-CAPRAZ
     AUC ile olculur. AUC<0.65 ise kol KAPANIR ve 45 dakikalik turetme YAPILMAZ.
...
def main():
    ...
    # ---- hazirlik: her parca icin P, Pd(yon secicili), SK
    H = {}
    adlar = {}
    for r in DER:
        d_ = PARCA.get(r["pid"])
    ...
    def kos(secim):
        """secim(i, S, p) -> nokta. Robot(FIZIKSEL) + tespit dondurur."""
        rob, det, gg = [], [], []
        for r in DER:
            h = H[r["pid"]]
    ...
    for r in DER:
        ...
        for i in range(len(P)):
            ...
            # girisler birbirine yakinsa agiz iyi tanimli, saciliyorsa belirsiz.
            ...
            oz += [1.0 if a in S else 0.0 for a in
                   ("agiz", "cember", "acik", "kirpik", "eksen", "kesit1", "kesit3", "kesit5")]
    ...
    for q in (0.90, 0.95, 0.99):
        ...
        print(f"  en yuksek %{100*(1-q):.0f} skor: {int(sel.sum())} nokta, "
              f"gercekten kotu olan {FY[sel].mean():.1%} (taban {FY.mean():.1%})")
    B_gecti = auc >= 0.65
