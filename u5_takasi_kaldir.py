# -*- coding: utf-8 -*-
"""U5: parca-ici z-skorun TAKASINI kaldirmayi dene (WEI +0.086 ama PXC -0.037).

U4 olctu: D (ham+zskor) gorulmemis ureticide en kotu durumu 0.4832 -> 0.5702 cikariyor
(GA [+0.048,+0.126]) ama DIGER uretici-disi bolmede -0.0374 kaybettiriyor (GA [-0.065,-0.009]).
Ikisi de GERCEK. Yani bu bir RISK TAKASI, bedava kazanc degil.

Takasi kaldirmayi denemeye deger, cunku bedelin kaynagi belli degil. Uc hipotez, uc kol:

  E  z-skor YALNIZ ezberleyen sutunlara
     Gerekce: olculmus kayit -- 13 sutundan yalniz `votes` genelleşiyor (uretici-disi AUC dususu
     0.018; digerleri 0.12-0.18). Genelleşen sutunu HAM birakip yalniz ezberleyenleri parca-ici
     baglamla yumusatmak, bilgi kaybini azaltabilir.
  G  TOPLULUK: ham gate ile z-skor gate'in olasiliklarinin ortalamasi
     Gerekce: topluluk genelde kazancin cogunu korur, kaybin bir kismini geri verir.
  H  z-skor YALNIZ fiziksel/topolojik sutunlara (13..21)
     Gerekce: olasilik istatistikleri (0..12) zaten parcaya gore olceklidir; kayan sey
     fiziksel dagilimlar olabilir.

KILL (onceden yazili): bir kol D'yi degistirebilmek icin (a) en kotu uretici-disi durumu D
kadar iyi olmali (>= D - 0.01) VE (b) DIGER uretici-disi bolmedeki kaybi D'den EN AZ 0.015
daha az olmali. Yoksa D kalir ve takas OLDUGU GIBI raporlanir.

Turetme onbellege alinir (results/_u4_der.pkl) -- yeni kollar dakikalar icinde denenir.
"""
import collections
import json
import os
...
NJOB = -1
GENELLESEN = [11]                       # votes -- tek genelleşen sutun (olculmus)
EZBER = [i for i in range(22) if i not in GENELLESEN]
FIZTOP = list(range(13, 22))            # B-rep fiziksel + topoloji


def _z(sub, kol=None):
    k = list(range(sub.shape[1])) if kol is None else list(kol)
    s = sub[:, k]
    sd = s.std(0)
...
def main():
    ...
    def skorla(clfs, fns, r):
        """Bir parcanin adaylarini (birden fazla kol verilirse ORTALAMA olasilikla) skorla."""
        ss = []
    ...
    print(f"\nKARAR (D'nin takasi: en kotu {D['en_kotu']-A['en_kotu']:+.4f}, "
          f"diger bolme {d_kayip:+.4f})")
    kazanan = None
    for ad, s in S.items():
        if ad in ("A ham", "D zskor tum"):
            continue
        kayip = min(0.0, s["PXC_disi"] - A["PXC_disi"])
        gecti = (s["en_kotu"] >= D["en_kotu"] - 0.01) and (kayip - d_kayip >= 0.015)
        print(f"  {ad:<18} en kotu {s['en_kotu']:.4f} (D {D['en_kotu']:.4f}) | "
              f"kayip {kayip:+.4f} (D {d_kayip:+.4f}) -> {'D YERINE GECER' if gecti else 'GECMEZ'}")
        if gecti and (kazanan is None or s["en_kotu"] > S[kazanan]["en_kotu"]):
            kazanan = ad
    print(f"\nSONUC: {kazanan or 'D KALIR -- takas OLDUGU GIBI raporlanir'}")
    with open("results/u5_takas.json", "w", encoding="utf-8") as f:   # bkz. [[dosya-tanitici-sizintisi]]
        json.dump(S | {"kazanan": kazanan}, f, indent=1)
    print("makbuz -> results/u5_takas.json")
