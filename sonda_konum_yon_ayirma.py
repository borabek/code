# -*- coding: utf-8 -*-
"""KONUM ve YONU AYIRMAK robot tavanini ne kadar acar?

BULGU (`results/donusum_ayristir_*.json`): GT'lerin %3.6'sinda (D7'de %3.0)
YANAL olcutu bir aday sagliyor, ACI olcutunu BASKA bir aday. Ayrica VAL'de
%14.4'unde yanal TAMAM ama aci yanlis. Su an her aday kendi (konum, yon) ciftini
KATI tasiyor -- oysa dogru yon cogu zaman parcanin BASKA bir yerinde zaten var.

Bu sonda tavani olcer: konum bir adaydan, yon YAKINDAKI kaynaklardan secilebilse
robot recall nereye cikar? Yon kaynaklari kademe kademe acilir:
  K0  kendi yonu (MEVCUT durum -- taban)
  K1  + `r` mm icindeki diger adaylarin yonleri
  K2  + parcadaki B-rep silindir eksenleri (+/-)
  K3  + parcanin baskin komsu yonu (tum adaylarin isaret-hizali ortalamasi)

TAVAN olcumudur: gercek bir secici bunun ALTINDA kalir. Ama tavan acilmiyorsa
kol hic kurulmaz.

TEZE SADIK: `v_o` KONUMLARI degismiyor; yalnizca hangi YON'un secilebilecegi
genisliyor. Tezin kendi (konum, yon) cifti her zaman K0 olarak havuzda.
"""
import collections
import json
import os
...
YON_R = float(os.environ.get("YON_R", "10.0"))     # yon odunc alma yaricapi
...
def main():
    ...
    print(f"kayit {kayit} | kume {kume} | parca {len(R)} | "
          f"silindir {len(cy)} | aciklik {len(ac)} | yon yaricapi {YON_R}mm | havuz {'GENISLETILMIS' if GENIS else 'TEZ-SAF'}", flush=True)
    ...
    for r in R:
        ...
        D = _birim(r["Pd"])
        if SIK:
            # KONUM SIKLASTIRMA: silindir agzi TEK nokta veriyor; gercek CP agzin
            # birkac mm ilerisinde/gerisinde olabiliyor. Eksen boyunca ORNEKLE.
            # Bu bir TAVAN olcumudur -- havuzu buyutur, urun yolu DEGISMEZ.
            ek_p, ek_d = [], []
        ...
        if GENIS:
            # KONUM havuzunu da genislet: KONUM YOK kovasi D7'de %65.8 ile en
            # buyuk kayip; yon odunc almak o kovaya DOKUNMUYOR.
            P, D, _k = brep_havuz.birlesik_havuz(P, D, cy.get(str(r["pid"])),
                                                 ac.get(str(r["pid"])))
            D = _birim(D)
        if not len(P):
            n_gt += len(G)
        ...
        dg = float(r["diag"])
        pid = str(r["pid"])
        # parca duzeyinde yon kaynaklari
        eks = []
    ...
    json.dump({"damga": makbuz_hash.damga(), "kayit": kayit, "kume": kume,
               ...
               "not": "Konum havuzdan, YON yakin kaynaklardan secilebilse tavan. "
                      "Mukemmel secici varsayimi -- gercek secici ALTINDA kalir."},
