# -*- coding: utf-8 -*-
"""OTOPSI: yogun parca duvarinin KOKENI segmentasyonda mi?

BUGUN BES MEKANIZMA DENENDI, BESI DE ZINCIRIN SONUNDAYDI:
  baglam modeli (S4) · kafes yayilimi · analitik yon · mesh normali · adet
Hicbiri EN BASA, SEGMENTASYONA bakmadi.

HIPOTEZ. NIT'te 24 CP'nin yalnizca BIRKACINDA segmentasyon ateşliyor;
gerisi mesh seyreltmesinden geliyor ve ZAYIF oznitelik tasiyor. Bu, olculen
deseni birebir aciklar:
  * ILK dogru secenek 18. sirada, SONUNCUSU 1065. (model ilkini buluyor)
  * poz-neg skor ayrimi 0.050 (skor bilgi tasimiyor)
  * havuzda konum VAR (0.843) ama secilemiyor

OLCULEN: her GT'nin konumunda segmentasyon olasiligi (CE+CT) nedir?
  p_gt_ortanca : GT noktalarindaki olasiligin ortancasi
  p_gt_en_dusuk: parcadaki EN DUSUK GT olasiligi (en zor CP)
  p_gt>0.5     : olasiligi 0.5 ustunde olan GT orani
  p_gt>0.1     : 0.1 ustunde olan GT orani
  p_rastgele   : rastgele yuzey noktalarinda ortanca (taban cizgisi)

Eger NIT'te p_gt>0.5 dusuk ama p_gt>0.1 yuksekse: segmentasyon ZAYIF ama
VAR -> esik/duyarlilik kolu acilir.
Eger p_gt>0.1 de dusukse: segmentasyon o CP'leri HIC GORMUYOR -> darbogaz
en basta, ve asagidaki hicbir numara kurtaramaz.
...
def main():
    ...
    for pid, r in kay.items():
        ...
        p_ce = pb[:, CE] + pb[:, CT]
        # her GT'ye EN YAKIN tepenin olasiligi
        ...
        n += 1
    print(f"{n} parca | olasilik = CE + CT (segmentasyon)\n")
    print(f"{'marka':<7}{'GT':>7}{'p_gt ortanca':>14}{'p_gt en dusuk':>15}"
    ...
    print("     (esik/duyarlilik kolu acilir)")
