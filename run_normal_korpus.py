...
Bugun each adaya `direction_bank` ~24 secenek takiyor and selector 4857 secenek
icinden ~24 dogruyu bulmaya calisiyor (pozitif yogunlugu 1/202). Bu corpus
each adaya TEK secenek birakir -> ~350 secenek, yogunluk 1/15 (13.5 KAT).
...
TUTULUR -- features that secenek for already correct hesaplanmistir.

Kullanim:
    NK_ON=d6 NK_CIK=results/_p6_oz_normal python run_normal_korpus.py
"""
import os 
import sys 
import time 
...
def main ():
    ...
    for i ,f in enumerate (fs ,1 ):
        ...
        YDn =_birim (YD )
        # each secenek: own adayinin normaliyle ISARETLI angle
