# -*- coding: utf-8 -*-
"""HATA OTOPSISI — kayip NEREYE gidiyor, cevrimdisi

NEDEN. Sunum sayilarinda iki buyuk kayip var ve ikisinin de SEBEBI
olculmedi:
    tespit 0.7878 -> robot eksen 0.5764   (-0.2114)
    kesinlik 0.5008                        (ciktinin YARISI yanlis)

Onarilacak seyi bilmeden onarim denemek tahmindir. Bu betik dokumden
her FP ve FN'i SINIFLANDIRIR:

FP (yanlis pozitif) turleri:
  cift      : bir TP'ye 5mm'den yakin -- AYNI acikligin ikinci kopyasi
  aci       : bir GT'ye yanal olarak yakin ama ACI tutmuyor
  isaret    : GT ile ayni EKSENDE ama TERS yonde (180 derece)
  hayalet   : hicbir GT'ye yakin degil -- gercek yanlis tespit

FN (kacan) turleri:
  yakin_var : yaninda tahmin VAR ama kabul kutusuna girmiyor (aci/yanal)
  bos       : yakininda hicbir tahmin YOK -- havuz/tespit sorunu

Her turun PAYI, hangi onarimin ne kadar getirecegini soyler.
D7'ye BAKILMAZ.
"""
import collections
import json
...
def main():
    ...
    for r in d:
        ...
        if not len(G):
            continue
        sonuc = esle_macar(P, D, G, Gd, dg, K.YANAL, K.ACI, False,
                           isaretli=True)
        tp, fp, fn = sonuc[:3]
        ...
        if not len(P):
            fn_tur["bos"] += len(G)
            continue
        # hangi tahmin/GT eslesti: eslesme ayrintisi yoksa yeniden kur
        # (kaba: her GT icin kabul kutusuna giren EN YAKIN tahmin)
        v = P[:, None, :] - G[None, :, :]
    ...
    print("  isaret YUKSEK    -> isaret kurali ONARILABILIR")
    print("  aci YUKSEK       -> yon tahmini sorunu (model isi)")
    print("  hayalet YUKSEK   -> havuz/gate sorunu")
    print("  FN yakin_var     -> tahmin VAR, kabul kutusuna sokulamiyor")
    print("  FN bos           -> havuz o acikligi HIC uretmemis")
    ...
    print(f"makbuz -> results/hata_otopsisi_{YOL}.json")
