...
Burada single variable HAVUZ: same training parcalari, same recete (within_part zskor
...
for f in sorted (os .listdir (OZ )):
    if f .endswith (".npz")and (f .startswith ("tam_")or f .startswith ("d6_")):
        ...
        X =z ["X"]
        if nseg >len (X ):
            continue # tutarsiz cache -> skip (silent kabul YOK)
...
print (f"EGITIM {len (tr )} part | seg candidate {segp } | total candidate "
...
for ad ,segtek in (("TEZ-SAF pool",True ),("GENISLETILMIS pool",False )):
    ...
    r =out [ad ]
    print (f"{ad :<22} robot {r ['robot']:.4f} | detection {r ['detection']:.4f} | makro "
...
f"detection {b ['detection']-a ['detection']:+.4f} | makro {b ['makro']-a ['makro']:+.4f}")
print (f"(dagitilan gate v6 + NMS, tez-saf pool: robot 0.2029 / detection 0.4523 -- "
f"AYRI RECETE, kiyas for not referans)")
json .dump ({"damga":receipt_hash .damga (),"sonuc":out ,
"havuz_etkisi_robot":b ["robot"]-a ["robot"],
"havuz_etkisi_tespit":b ["detection"]-a ["detection"],
"not":"TEK DEGISKEN = pool. Ayni parts, same recete, same threshold "
"taramasi, same NMS. D7 brand-disi, MIKRO."},
open ("results/pool_single_degisken.json","w"),indent =1 )
