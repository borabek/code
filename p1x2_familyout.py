# -*- coding: utf-8 -*-
"""P1-x2: KONUM feature kazanci (+0.041 top-N F1) GERCEK mi yoksa AILE-EZBERI mi?
Tuzak: GroupKFold PARCA-bazliydi; kardes parcalar train'de olunca model 'bu ailede tel z=0.3'te'
ezberleyebilir. Receipt zaten part-out(0.807) vs family-out(0.799) farkini biliyor; KONUM'da bu fark
DAHA BUYUK olabilir (konum aileye ozgudur).
Aile anahtari (etiketsiz, geometry_key ruhunda): aday-bulutu bbox boyutlari (yuvarlanmis) + log2(aday sayisi).
KARAR: family-out'ta da kazanc kaliyorsa GERCEK; cokuyorsa aile-ezberi (durustce kapat)."""
import json, sys
import numpy as np
...
for gi, (pid, v) in enumerate(d.items()):
    ...
    dims = np.round(P.max(0) - P.min(0), 0)                 # aday-bulutu bbox (mm, 1mm yuvarlama)
...
print(f"{len(Y)} aday / {int(Y.sum())} TP / {len(Ns)} parca / {len(uf)} AILE (ort {len(Ns)/max(len(uf),1):.2f} parca/aile)")
