# -*- coding: utf-8 -*-
"""KARAR DENEYI: robotun buldugu acikliklarda TEL-girisi ile TOOL/mount agzi GEOMETRIYLE ayriliyor mu?

Sinif (Contact) ayirt edemiyor (tez: Contact=Kontaktierung bzw. Werkzeugeinschub). Peki geometri?
Uretici ConnectionPoint'i (= gercek tel-kontakt) AYRAC-ETIKETI olarak kullan: bir robot-CP'si bir
uretici CP'sine axis-aware eslesiyorsa = TEL-girisi; eslesmiyorsa = TOOL/mount/fazla.
Sonra iki grubun size_mm / depth_mm / votes / confidence dagilimlarini karsilastir.
  Ayriliyorsa (or. tel daha buyuk/derin) -> GEOMETRIK KAPI genel fix (yeniden egitim yok).
  Ayrilmiyorsa -> alt-sinif etiketi (tool'u Contact'tan ayir) + yeniden egitim gerekir.
...
def wire_flags(P, G, Gd, tol, axis_tol=40.0):
    """her robot-CP icin: bir uretici CP'ye eslesiyor mu (=tel-girisi)."""
...
def main():
    ...
    print(f"{len(parts)} parca | vote>=2 robot", flush=True)
    ...
    print("\n  AUC ~0.5 = ayirt edilemez; >0.65 = zayif sinyal; >0.75 = geometrik kapi ise yarar")
