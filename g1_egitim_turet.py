# -*- coding: utf-8 -*-
"""G1: EGITIM KORPUSUNU YENI TOPLULUKLA TURET -- gate'in darbogazini acmak icin.

K1 SONUCU: 8 uye + eksen-farkindalikli havuzlama aday havuzunun recall'ini
**0.7566 -> 0.9146** cikardi (+0.158). Yani "F1 0.87 havuz tavaninin ustunde,
yapisal olarak imkansiz" hukmu ARTIK GECERSIZ -- tavan kirildi.

AMA uctan uca tespit kipirdamadi (+0.0042, gurultu). Sebebi K1'in KENDI SINIRI:
oradaki gate olcum kumesi icinde 5-kat grup-caprazla, yani ~150 parcayla egitiliyordu ve
4135 adayi suzmeye calisiyordu. Dagitilan gate 1709 parcayla egitiliyor.

Bu betik o esitsizligi kapatir: EGITIM korpusunu (olcum ve LOCKED gruplari DISINDA) ayni
8 uyeli toplulukla ve ayni eksen-farkindalikli havuzlamayla turetir. Boylece gate YENI
aday dagilimini gorur.

PARITE SARTI: bu turetme ile olcum turetmesi (G2) AYNI topluluk ve AYNI havuzlama kuralini
kullanmak ZORUNDA. Hafizadaki ders: "iki gate AYNI olmali" -- dagitilan gate bir kez
...
def main():
    ...
    print(f"EGITIM havuzu: {len(E)} parca (olcum + LOCKED gruplari CIKARILDI)")
    ...
    if os.path.exists(ARA):
        ...
        print(f"ara kayittan devam: {len(Y)} aday, {basla}. parcadan", flush=True)
    else:
        basla = 0
    t0 = time.time(); ok = 0
    for k, (mfg, pid, jf, sp) in enumerate(E):
        ...
        if k % 20 == 0 and k > basla:
            ...
            print(f"  {k}/{len(E)} ok={ok} aday={len(Y)}  {time.time()-t0:.0f}s "
        ...
        try:
            ...
            cps, probs, _, _ = RC.adaylari_uret(V, F, pbs, sp, cfg=cfg)
            if not cps:
                continue
    ...
    if os.path.exists(ARA):
        os.remove(ARA)
    print("makbuz -> " + CIKTI.replace(".npz", "_koken.json"))
