# -*- coding: utf-8 -*-
"""Render the user-approved CP set (cp_openings.connection_points) on a part with the
semantic-coloured mesh + a GREEN ball at each CP (small outward offset so it sits ON the
opening, not floating). render->Read to confirm balls land on the physical openings."""
...
for c in cps:
    q = c["point"] + c["direction"]*diag*0.012   # small offset -> sits on opening
