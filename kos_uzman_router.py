# -*- coding: utf-8 -*-
"""Y2 — YOGUN-PARCA UZMANI + YONLENDIRICI (router)

NEDEN. Cok-CP'li parcalar TANIDIK markada bile zayif halka:
`cok_CP_F1 0.6583` vs `dusuk_CP_F1 0.7701` (cp_config). Gorulmemis markada
ise ucurum: NIT uctan uca 0.0089.

BU KOL DAHA ONCE KAPANAN HIGH-CP KOLLARINDAN FARKLIDIR:
  * `high-cp-segmentation-closed`  -> SEGMENTASYON duzeyinde bolmeydi
  * `h3-highcp-finetune-dead`      -> ayni modelin FINETUNE'u
  * `cc-d-minv10-regime-split`     -> ESIK/parametre rejimi
  * `pitstop-highcp-gate`          -> GATE rejimi
Burada AYRI EGITILMIS BIR SECICI + OGRENILMIS YONLENDIRICI var. Uzman,
yalnizca yogun parcalarda egitilir; boylece kayip fonksiyonunu seyrek
parcalarla paylasmaz.

REJIM TANIMI `manset.py` ile AYNI: `n_gt >= 8` -> "cok", degilse "dusuk".
Ama YONLENDIRICI GT KULLANAMAZ; parca duzeyi ozniteliklerden rejimi TAHMIN
eder ve dogruluğu ayrica raporlanir (yanlis yonlendirme kolun tavanini
belirler).

KOLLAR:
  taban    : tek genel model (bugunku)
  uzman_gt : rejim GT'den bilinseydi (UST SINIR -- dagitilamaz, tavan olcer)
  uzman    : rejim YONLENDIRICIDEN (dagitilabilir hali)

KAPI: uctan uca +0.01. D7'ye BAKILMAZ.
"""
import collections
import json
import os
import sys
import time

import numpy as np
from sklearn.ensemble import HistGradientBoostingClassifier

import makbuz_hash

os.environ.setdefault("BA_ALLOW_SEEN", "1")
os.environ["WG_FIZ_FEATS"] = "1"
os.environ["WG_TOPO"] = "1"
os.environ["WG_ZENGIN"] = "1"
os.environ.setdefault("P6_DIZIN", "results/_p6_oz_tam4")
sys.path.insert(0, ".")
import kanonik_d7 as K             # noqa: E402
import p6_karar                    # noqa: E402
from kos_p6_ortak import yukle     # noqa: E402
from sina_kume import esle_macar   # noqa: E402

KUME = os.environ.get("UR_KUME", "d6")
NEG_KAT = int(os.environ.get("UR_NEG", "12"))
COK_ESIK = int(os.environ.get("UR_COK", "8"))     # manset ile AYNI
ITER, LR, YAPRAK, L2R = 200, 0.06, 63, 1.0
NMS = 5.0
KURAL = ("goreli", 0.85, 0.20)


def temel(d):
    return np.hstack([p6_karar.donustur(d["X"]),
                      p6_karar.kaynak_blok(d["kaynak"][d["idx"]])]).astype(
                          np.float32)


def parca_oz(d):
    """YONLENDIRICI icin parca duzeyi oznitelik. GT KULLANMAZ."""
    idx = np.asarray(d["idx"], int)
    P = np.asarray(d["P"], float)
    n_kon = len(np.unique(idx))
    kut = P.max(0) - P.min(0) if len(P) else np.zeros(3)
    return np.array([len(idx), n_kon, float(d["diag"]),
                     kut[0], kut[1], kut[2],
                     float(np.prod(np.sort(kut)[-2:])),
                     n_kon / max(float(d["diag"]), 1e-6)], float)


def yap():
    return HistGradientBoostingClassifier(
        max_iter=ITER, learning_rate=LR, max_leaf_nodes=YAPRAK,
        l2_regularization=L2R, random_state=0)


def egit(veri, ic, cok_agirlik=1.0):
    """cok_agirlik > 1 ise COK-CP parcalarindan gelen orneklerin agirligi
    artirilir. Y23: veriyi BOLMEK yerine AGIRLIKLANDIRMAK. Uzman kolu
    kahin rejimle bile dustu (-0.0418) ve sebebi veri parcalanmasiydi;
    agirliklandirma parcalamaz."""
    n_s = sum(len(veri[i]["y"]) for i in ic)
    if not n_s:
        return None
    M = np.empty((n_s, veri[ic[0]]["_M"].shape[1]), np.float32)
    o = 0
    for i in ic:
        m_ = veri[i]["_M"]
        M[o:o + len(m_)] = m_
        o += len(m_)
    Y = np.concatenate([veri[i]["y"] for i in ic])
    W = np.concatenate([np.full(len(veri[i]["y"]),
                                cok_agirlik if veri[i]["_cok"] else 1.0,
                                np.float32) for i in ic])
    if Y.sum() == 0 or Y.sum() == len(Y):
        return None
    rng = np.random.default_rng(0)
    poz, neg = np.where(Y == 1)[0], np.where(Y == 0)[0]
    sec = np.concatenate([poz, rng.choice(
        neg, min(len(neg), NEG_KAT * max(len(poz), 1)), replace=False)])
    m = yap().fit(M[sec], Y[sec], sample_weight=W[sec])
    del M
    return m


def ince_ayar(genel_veri, ic_cok, veri, ek_agac=60):
    """UZMANLASMA, VERI PARCALAMADAN.

    Uzman kolu kahin rejimle bile dustu (-0.0418) ve sebep teshis edildi:
    alt kumede SIFIRDAN egitmek her uzmani daha az veriyle birakiyor.
    Duzeltme: genel modelden DEVAM ET (`warm_start`) ve yalnizca yogun
    parcalarda EK AGAC ekle. Boylece uzman TUM verinin bilgisiyle baslar,
    sonra yogun rejime uyarlanir.

    NOT: kayittaki "high-CP finetune olu" SEGMENTASYON agi icindi; bu
    SECICI tarafinda ayri bir mekanizma.
    """
    if genel_veri is None or not ic_cok:
        return None
    import copy
    m = copy.deepcopy(genel_veri)
    n_s = sum(len(veri[i]["y"]) for i in ic_cok)
    if not n_s:
        return None
    M = np.empty((n_s, veri[ic_cok[0]]["_M"].shape[1]), np.float32)
    o = 0
    for i in ic_cok:
        mm = veri[i]["_M"]
        M[o:o + len(mm)] = mm
        o += len(mm)
    Y = np.concatenate([veri[i]["y"] for i in ic_cok])
    if Y.sum() == 0 or Y.sum() == len(Y):
        return None
    rng = np.random.default_rng(0)
    poz, neg = np.where(Y == 1)[0], np.where(Y == 0)[0]
    sec = np.concatenate([poz, rng.choice(
        neg, min(len(neg), NEG_KAT * max(len(poz), 1)), replace=False)])
    # SESSIZ NO-OP DUZELTMESI (2026-08-13). Ilk yazim `m.max_iter_`
    # kullaniyordu -- boyle bir oznitelik YOK (dogrusu `n_iter_`).
    # AttributeError, genis bir `except` tarafindan yutuluyor ve fonksiyon
    # None donuyordu; iki kol da sessizce TABANA dusup tam +0.0000 veriyordu.
    # "Ince ayar ise yaramiyor" diye rapor edilecekti. Artik hata YUTULMAZ.
    m.set_params(warm_start=True, max_iter=int(m.n_iter_) + ek_agac)
    m.fit(M[sec], Y[sec])
    del M
    assert int(m.n_iter_) > ek_agac // 2, "ince ayar agac EKLEMEDI"
    return m


def f1(c):
    return 2 * c["tp"] / max(2 * c["tp"] + c["fp"] + c["fn"], 1)


def main():
    t0 = time.time()
    veri = yukle(KUME, int(os.environ.get("P6_TR", "0")))
    for d in veri:
        d["y"] = np.asarray(d["y"], int)
        d["_M"] = temel(d)
        d["_cok"] = int(len(d["G"]) >= COK_ESIK)
    n_cok = sum(d["_cok"] for d in veri)
    print(f"{len(veri)} parca | cok-CP {n_cok} ({n_cok/len(veri):.1%}) "
          f"| esik n_gt>={COK_ESIK}", flush=True)

    # TANIDIK MARKA kosulu: rastgele 3 kat (marka-KARISIK)
    # KAT TOHUMU: cok-CP agirligi 2x tek tohumda +0.0065 verdi; kat
    # gurultusu +-0.008 oldugu icin cok tohumlu dogrulama SART.
    rng = np.random.default_rng(int(os.environ.get("UR_TOHUM", "1")))
    pay = rng.permutation(len(veri)) % 3
    KOLLAR = ("taban", "uzman_gt", "uzman", "agirlik2", "agirlik4",
              "agirlik8", "ince_ayar_gt", "ince_ayar")
    agg = {k: collections.Counter() for k in KOLLAR}
    yon_dogru = yon_top = 0
    for f_ in range(3):
        ic = [i for i in range(len(veri)) if pay[i] != f_]
        dis = [i for i in range(len(veri)) if pay[i] == f_]
        genel = egit(veri, ic)
        agirlikli = {a: egit(veri, ic, cok_agirlik=a) for a in (2.0, 4.0, 8.0)}
        uz_cok = egit(veri, [i for i in ic if veri[i]["_cok"]])
        ia_cok = ince_ayar(genel, [i for i in ic if veri[i]["_cok"]], veri)
        uz_dus = egit(veri, [i for i in ic if not veri[i]["_cok"]])
        # YONLENDIRICI: parca duzeyi, GT'siz
        XR = np.vstack([parca_oz(veri[i]) for i in ic])
        YR = np.asarray([veri[i]["_cok"] for i in ic], int)
        yon = yap().fit(XR, YR) if 0 < YR.sum() < len(YR) else None
        for i in dis:
            d = veri[i]
            tah = (int(yon.predict(parca_oz(d)[None])[0]) if yon is not None
                   else 0)
            yon_dogru += int(tah == d["_cok"])
            yon_top += 1
            for kol in KOLLAR:
                if kol == "taban":
                    m = genel
                elif kol.startswith("agirlik"):
                    m = agirlikli[float(kol[7:])]
                elif kol.startswith("ince_ayar"):
                    rej = (d["_cok"] if kol.endswith("_gt") else tah)
                    m = ia_cok if rej else genel
                else:
                    rej = d["_cok"] if kol == "uzman_gt" else tah
                    m = uz_cok if rej else uz_dus
                if m is None:
                    m = genel
                s = m.predict_proba(d["_M"])[:, 1]
                P, D = p6_karar.sec(d["P"], d["idx"], d["YD"], s, KURAL,
                                    nms_mm=NMS)
                tp, fp, fn = esle_macar(P, D, d["G"], d["Gd"], d["diag"],
                                        K.YANAL, K.ACI, False,
                                        isaretli=True)[:3]
                c = agg[kol]
                c["tp"] += tp; c["fp"] += fp; c["fn"] += fn
        print(f"  kat{f_} bitti ({time.time() - t0:.0f} s)", flush=True)

    son = {k: f1(agg[k]) for k in KOLLAR}
    print(f"\nyonlendirici dogrulugu: {yon_dogru}/{yon_top} "
          f"({yon_dogru/max(yon_top,1):.3f})")
    print(f"\n=== TABAN {son['taban']:.4f} (TANIDIK marka kosulu) ===")
    for k in KOLLAR[1:]:
        fark = son[k] - son["taban"]
        et = "  <- KAPI GECTI" if fark >= 0.01 else ""
        ust = "  (UST SINIR, dagitilamaz)" if k == "uzman_gt" else ""
        print(f"  {k:<10}{son[k]:.4f}   {fark:+.4f}{et}{ust}")
    json.dump({"damga": makbuz_hash.damga(), "kume": KUME,
               "cok_esik": COK_ESIK, "n_cok": n_cok,
               "yonlendirici_dogruluk": yon_dogru / max(yon_top, 1),
               "toplam": son,
               "not": "Yogun-parca UZMANI + ogrenilmis yonlendirici. TANIDIK "
                      "marka kosulu (rastgele katlar). uzman_gt UST SINIRDIR. "
                      "D7'ye BAKILMADI."},
              open(f"results/uzman_router_{KUME}.json", "w"), indent=1)
    print(f"makbuz -> results/uzman_router_{KUME}.json")


if __name__ == "__main__":
    main()
