# -*- coding: utf-8 -*-
"""P4: KOSUDAN KOSUYA KARARLILIK -- "0.7584" ne kadar saglam bir sayi?

NEDEN: [[robot-nondeterminism]] kaydi remesh'in SURECLER ARASI degistigini soyluyor
(pymeshlab). Yani ayni parca, ayni model, ayni kod -- farkli bir Python surecinde farkli
bir mesh, dolayisiyla farkli adaylar. Bu bugune kadar hic OLCULMEDI: mansetin kendi
kosu-varyansini bilmeden "+0.008 kazandi" demek anlamsizdir.

YONTEM: N parcayi AYNI surecte iki kez turet (surec-ici kararlilik) ve ayrica bu betigi
IKI KEZ calistirip makbuzlari karsilastir (surec-arasi). Burada surec-ICI olculur;
`--tur 2` ile ikinci kosu ayri bir surecte yapilir ve fark raporlanir.

Olculen: aday sayisi farki, nokta eslesme orani, ve parca basina tespit F1 farki.
"""
import io
import json
import os
...
def main():
    ...
    if tur == 2 and os.path.exists(OUT.format(1)):
        ...
        print(f"\nSUREC-ARASI KARARLILIK ({len(dv)} parca)")
        print(f"  tepe sayisi farki : medyan {np.median(dv):.0f}  max {max(dv) if dv else 0}")
        print(f"  aday sayisi farki : medyan {np.median(da):.0f}  max {max(da) if da else 0}"
        ...
        print("makbuz -> results/p4_kararlilik.json")
