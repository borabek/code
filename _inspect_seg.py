# -*- coding: utf-8 -*-
"""Kendi teshisim: mesh baglanti-segmentasyonuyla boyali + CP noktalari, 4 acidan. Hangi CP gercek
aciklik bolgesinde (mavi) hangisi duz govdede (gri) -- durustce goreyim."""
...
fig.suptitle(f"{pid}: MAVI=model baglanti-segmentasyonu, kirmizi nokta+ok=robot CP  (CP mavide=makul, gride=yanlis)", fontsize=12)
plt.tight_layout(rect=[0,0,1,0.94])
plt.savefig(f"results/_inspect_{pid}.png", dpi=105, bbox_inches="tight")
print(f"-> results/_inspect_{pid}.png | {len(cps)} CP")
for i,c in enumerate(cps):
    # CP'nin 4mm cevresindeki ortalama baglanti olasiligi
    near = np.linalg.norm(V - np.asarray(c["point"],float), axis=1) <= 4.0
    mp = float(conn[near].mean()) if near.any() else 0.0
    print(f"  CP{i+1}: guven {c['confidence']} oy {c['votes']} | cevre baglanti-olasiligi {mp:.2f} {'(MAVI-makul)' if mp>=0.4 else '(GRI-supheli)'}")
