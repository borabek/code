# -*- coding: utf-8 -*-
"""E1: TOPLULUK SINAVI -- projenin EN IYI checkpointleri uctan uca hic sinanmamisti.

DENETIM BULGUSU: dagitilan topluluk 1x k_eig64 (val Conn-IoU 0.6481) + 3x k_eig96
(0.6378/0.6629/0.6597). Diskte duran ama DAGITILMAYAN iki checkpoint projenin EN YUKSEK
val Conn-IoU'suna sahip: keig128_s1 = 0.7079, keig128_s2 = 0.6949.

k_eig 128 kolu TEK TOHUMLA oldurulmustu ve o tohum (keig128_s0, val 0.6237) ailenin
EN KOTUSUYDU. s1 ve s2 uctan uca HIC olculmedi.

BU BETIK ikisini AYNI kod yolundan gecirip karsilastirir. Her kolun gate'i KENDI
adaylariyla egitilir (parite sarti: farkli topluluk = farkli aday dagilimi = gate yeniden
uydurulmali; hafizadaki `gate-refit-minv4` dersi).

Karsilastirma olcum kumesi ICINDE grup-caprazdir (her iki kol icin AYNI bolme), cunku
B kolunun egitim korpusu turetilmedi. Bu, kollari ADIL siralar; mutlak deger dagitilan
manset degildir.
...
def main():
    ...
    for ad, yol in KOL.items():
        ...
        for r in RJ:
            ...
            if m.any():
                ...
                c = wire_gate.pose_duzelt(X, c)
                c = wire_gate.aci_duzelt(X, c)
                if r.get("UYE"):
                    c = wire_gate.uye_yonu_sec(X, c, r["UYE"])
            ...
            gg.append(r["geo"])
        SON[ad] = {"tespit": f1w(det), "robot": f1w(rob), "aday": int(len(RY)),
        ...
        print(f"\n{ad}: tespit {s['tespit']:.4f} | robot {s['robot']:.4f} | "
              f"aday {s['aday']} | aday-recall {s['aday_recall']:.4f}", flush=True)
    ...
    if len(ROWS) == 2:
        ...
        for i, ad in ((0, "tespit"), (1, "robot")):
            ...
            print(f"\n{ad.upper()}: {SON[a][ad]:.4f} -> {SON[b][ad]:.4f} ({d:+.4f})"
                  f"  GA[{lo:+.4f},{hi:+.4f}] {'GERCEK' if (lo>0 or hi<0) else 'gurultu'}")
            SON[f"{ad}_ga"] = [lo, hi]
    ...
    print("makbuz -> results/e1_topluluk.json")
