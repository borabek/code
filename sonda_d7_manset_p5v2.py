# -*- coding: utf-8 -*-
"""D7 MANSET YOLUNDA p5-v2 -- 0.2344 ile AYNI OLCEKTE tek sayi.

MANSET YOLU (0.2344'u ureten): kayittaki adaylar (r["P"], r["Pd"]) -> gate maskesi
-> `urun_zinciri.tam_poz` -> Macar. Bu betik AYNI yolu kullanir, tek fark
p5-v2'nin araya girmesi.

  A) MANSET: gate maskesi -> tam_poz                      (beklenen ~0.2344)
  B) p5-v2 : HAM havuz -> ortak secim -> gate SONRA        (olculmek istenen)

EGITIM/OLCUM AYRIMI: p5-v2 D6'nin 8 markasinda egitilir, D7'nin 12 markasinda
olculur -- kumeler AYRIK. D7 = DEV (harcandi), bu FINAL DEGIL.
TEZE SADIK: `v_o` her iki kolda da havuzda ve p5-v2'de GERCEK fallback.
"""
...
sys.path.insert(0, ".")
import d6_kayit, wire_gate, urun_zinciri, p5v2_secenek as PS, p5v2_egit as PE
...
print(f"D6 egitim {len(k6)} parca | D7 olcum {len(k7)} parca", flush=True)
...
print("D6 egitim verisi kuruluyor...", flush=True)
tr = kur(k6, cy6, ac6, True)
clf, sh, poz = PE.egit(tr)
print(f"  egitim {len(tr)} parca | {sh} | pozitif {poz:.4f}", flush=True)
print("D7 olcum verisi kuruluyor...", flush=True)
te = kur(k7, cy7, ac7, False)
print(f"  olcum {len(te)} parca", flush=True)
...
for d in te:
    ...
    T.append((len(d["G"]),) + esle_macar(P, D, d["G"], d["Gd"], d["diag"], 0., 180., True)[:3])
    R.append((len(d["G"]),) + esle_macar(P, D, d["G"], d["Gd"], d["diag"], YANAL, ACI,
...
json.dump({"damga": makbuz_hash.damga(), "manset": {"tespit": tA, "robot": rA},
           "p5v2": {"tespit": tB, "robot": rB}, "n_parca": len(te),
           "not": "D7 = DEV, FINAL DEGIL. p5-v2 D6'da egitildi (marka-ayrik)."},
          open("results/d7_manset_p5v2.json", "w"), indent=1)
print("makbuz -> results/d7_manset_p5v2.json")
