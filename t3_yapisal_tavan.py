...
T1 (2026-08-04) FIZIKSEL olcumlerin tavani HIC oynatmadigini gosterdi (%12.8 -> %12.8).
[[fizik-bayes-tavanini-oynatmiyor]]: F3 kollari YAPISAL/TOPOLOJIK bilgi getirmek zorunda.

BU BETIK O SORUYU SORAR. Kod tabaninda olculdu: `wire_gate.py`'de **komsu ya da
periyodiklik ozniteligi SIFIR** -- her aday tek basina karar goruyor. Ama klemens bloklari
DUZENLI DIZIDIR: on kutbun sekizi ateslediyse eksik ikisinin nerede oldugu komsularindan
bellidir. Bu, mevcut uzayda HIC bulunmayan bir bilgi ekseni.

BEDAVA OLCUM: yapisal oznitelikler tamamen ADAY NOKTA KUMESINDEN hesaplanir; `_der_tam.pkl`
icindeki `P`/`Pd` yeterli, ag cikarimi GEREKMEZ. (T2 -- gizli oznitelik sondasi -- forward
pass ister ve turetme bitene kadar beklemek zorunda.)

TEZ DEGISMEZ: hicbir sey egitilmez, `v_o` turetmesi ve aday ureticisi aynen kalir. Bu bir
...
import wire_gate
from t1_bayes_tavani import ayrilamaz_pay
...
def _baskin_adim(t):
    """1B izdusumde BASKIN ADIM (pitch): ardisik farklarin en sik degeri.

    Klemens bloklarinda kutuplar sabit araliklidir. Farklarin MEDYANI degil MODU alinir --
    medyan, eksik kutuplarin actigi cift araliklardan etkilenir.
    """
    if len(t) < 3:
        return 0.0
    d = np.diff(np.sort(t))
...
def yapisal(P, Pd):
    """Aday basina ILISKISEL oznitelikler. Tek adayin kendi geometrisinden DEGIL,
    parcadaki DIGER adaylara gore konumundan uretilir."""
    n = len(P)
    F = np.zeros((n, len(AD)), float)
    if n == 0:
        return F
    C = P - P.mean(0)
    # ana eksenler: adaylarin yayildigi yon (dizinin uzun ekseni)
    if n >= 3:
    ...
    for i in range(n):
        ...
        if adim > 1e-6:
            # kafes dugune uzaklik: t[i] adimin tam kati mi?
            kalan = abs(t[i] / adim - round(t[i] / adim)) * adim
            F[i, 0] = kalan
            F[i, 1] = 1.0 if kalan < 0.25 * adim else 0.0
            # ayni sirada esdogrusal komsu sayisi (adimin katlarinda duranlar)
        ...
        # AYNA SIMETRISI: parcanin ortasina gore yansimasinda aday var mi?
        ayna = P[i] - 2 * (C[i] @ e0) * e0 - P.mean(0) + P.mean(0)
        F[i, 7] = float(np.min(np.linalg.norm(P - ayna, axis=1))) if n > 1 else 0.0
        # YON UYUMU: komsularin ekleme yonuyle ayni mi (dizide yonler paraleldir)
        if n > 1:
            j = int(np.argmin(d))
...
def main():
    ...
    X0, YP, Y, GR, NA = [], [], [], [], []   # NA = parcadaki KABUL EDILEN aday sayisi
    for r in DER:
    ...
    # ------------------------------------------------------------------ REJIM AYRIMI
    # ILK SURUM HAVUZLANMIS OLCTU VE BU BIR OLCUM HATASIYDI (2026-08-04, kendi hatam):
    # `_baskin_adim` en az 3 nokta ister, yoksa 0.0 doner. Korpusun %89.5'i DUSUK-CP
    # (1-3 CP) oldugu icin o satirlarda TUM kafes sutunlari SABIT SIFIR. Havuzlanmis
    # olcum, yapisal bilginin TANIMSIZ oldugu satirlarla dolu -> null sonuc garantiydi.
    # Dogru soru "yapi ise yariyor mu" degil, "yapinin VAR OLDUGU yerde ise yariyor mu";
    # zaten hedeflenen 366 FN'nin tamami orada.
    REJIM = [("HAVUZLANMIS", np.ones(len(Y), bool)),
             ("YAPI TANIMLI (>=4 aday)", NA >= 4),
             ("yapi tanimsiz (<4)", NA < 4)]
    S, gecti_her = {}, {}
    for rad, msk in REJIM:
        if msk.sum() < 60:
            print(f"\n{rad}: {int(msk.sum())} aday -- olcum icin AZ, atlandi"); continue
        print(f"\n=== {rad} ({int(msk.sum())} aday, iyi %{100*Y[msk].mean():.1f}) ===")
        print(f"{'oznitelik uzayi':<26}{'kNN uyusmazlik':>16}{'AYRILAMAZ pay':>16}{'tavan F1~':>11}")
        alt = {}
        for ad, M in (("MEVCUT (gate ozn.)", X0), ("+ YAPISAL/ILISKISEL", X1),
                      ("YALNIZ yapisal", YP)):
            u, a = ayrilamaz_pay(M[msk], Y[msk], GR[msk])
            alt[ad] = {"uyusmazlik": u, "ayrilamaz": a, "tavan": 1 - u / 2}
    ...
    kilit = gecti_her.get("YAPI TANIMLI (>=4 aday)", {})
    print(f"\nB KOLUNUN KAPISI = YAPI TANIMLI rejimi -> "
          f"{'GECTI, B KURULUR (yalniz o rejimde)' if kilit.get('gecti') else 'GECMEDI'}")
    with io.open("results/t3_yapisal_tavan.json", "w", encoding="utf-8") as f:
        json.dump({"n": int(len(Y)), "rejim": S, "karar": gecti_her,
                   "kapi": kilit, "sutunlar": AD}, f, indent=1)
    print("makbuz -> results/t3_yapisal_tavan.json")
