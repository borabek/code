...
# durdurulunca disk 13.11 GB'a FIRLADI -- i.e. disk sorunu a data sismesi
# not, BELLEK BASKISININ yan etkisiydi.
#
# Cikarim skip-present-olani calistigi for kaldigi yerden devam eder.
set -u
cd "$(dirname "$0")"
...
while [ $bek -lt 43200 ]; do
  ...
  if [ "$a" -eq 0 ] && [ "${r%%.*}" -ge 12 ]; then
    ...
    bash run_tam4_resume.sh
    say "inference turu bitti"
    exit 0
  fi
  say "bekliyor: $n/2583 file, agir is $a, ${r}GB bos"
  sleep 600
  bek=$((bek + 600))
done
say "ZAMAN ASIMI"
