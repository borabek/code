...
#
# WHY AYRI: `run_night.sh` kosarken duzenlenemez (bash betigi bayt konumundan
# okur; ortasindan degistirmek yurutmeyi bozar). Gece basladiktan SONRA written
# bloklar this yuzden oraya eklenmedi, yanina zincirlendi.
#
# IKI GATE (ikisi de saglanmadan hicbir blok baslamaz):
#   1) corpus TAM        -- missing korpusta measured_path number gecersizdir
#   2) bos RAM >= 10 GB  -- each EK kosusu ~6 GB yukluyor. Bu night UC surec
...
kapilari_bekle() {
  ...
  while [ $bek -lt $((10 * 3600)) ]; do
    ...
    if [ "$n" -ge $GEREK_DOSYA ] && [ "${r%%.*}" -ge $GEREK_RAM ] \
       && [ "$a" -eq 0 ]; then
      say "KAPILAR OPEN: $n file, ${r}GB bos, agir is none"
      return 0
    fi
    say "bekliyor: $n/$GEREK_DOSYA file, ${r}GB bos (gereken ${GEREK_RAM}GB), agir is $a"
    sleep 600
    bek=$((bek + 600))
  done
  say "ZAMAN ASIMI"
...
# Iki variable birden degisince B1'in kazanci KORPUSA mi YONTEME mi ait,
...
if kapilari_bekle; then
  say "BASLIYOR: tam3_taban (kademe2, duz setting)"
  t0=$(date +%s)
  if P6_DIZIN=results/_p6_oz_tam3 P6_KUME=tam,d6 P6_KAT_MIN=200 \
    ...
    cp -f results/p6_kademe2_full.json results/p6_kademe2_tam3_baseline.json \
      2>/dev/null || true
