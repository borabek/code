...
Kazanan: B-rep havuzu + agiz tanimlayicilari, D7 TAM ZINCIR robot **0.2649**
(urun 0.2029). Donusum %60 (0.2649/0.4409) -- urunun %45'inin cok ustunde,
yani havuz adaylari daha iyi KONUMLANIYOR. Kalan kayip TESPITTE.

Yon odunc alma bu gece -0.0044 olculmustu ama o olcum BOZUK ETIKETLI gate ile
yapilmisti ([[gate-etiket-tanimi-hatasi-ve-v6-kunyesi]]); dayanagi cokmus
durumda. Kol aday EKLEMEZ, yalnizca secilen adayin yonunu degistirir -> tespit
YAPISAL OLARAK bozulamaz.

Yon secici, gate'in KENDI skorunu ve `yon_odunc.OZ_AD` ozniteliklerini kullanir;
egitim etiketi "bu yon adayi ROBOT-hazir yapar mi" (isaretli aci <=10).
"""
import collections
import json
import os
...
def yon_egitim(veri, gate, cyl):
    X, Y = [], []
    for d in veri:
        s = np.asarray(wire_gate.karar_skoru(gate, d["X"]), float)
        k = s >= KURAL[1]
        if not k.any():
            continue
        ...
        for i in range(len(P)):
            ...
            for t in range(len(d["G"])):
                if konum_uyar(P[i], d["G"][t], d["Gd"][t]):
                    j = t
                    break
            if j < 0:
                continue
...
def main():
    ...
    json.dump({"damga": makbuz_hash.damga(), "sonuc": out, "fark": d,
               "not": "Kazanan kol (B-rep + tanimlayici) uzerinde yon odunc alma, "
                      "DUZELTILMIS etiketli gate ile. D7 marka-disi, TAM ZINCIR."},
              open("results/kazanan_yon_odunc.json", "w"), indent=1)
    print("makbuz -> results/kazanan_yon_odunc.json")
