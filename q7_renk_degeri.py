# -*- coding: utf-8 -*-
"""Q7: RENK, tel girisi ile vidayi AYIRIYOR MU? -- varsayimla degil, olcumle.

DURUM: "renk ayirt edici degil, cunku vida da metal kelepce de metal" dedim; bu bir VARSAYIMDI.
Bu betik onu kanitlar ya da curutur.

YOL (her adim ayri dogrulanir):
 1) STEP metninden RENKLI silindirler: OVER_RIDING_STYLED_ITEM -> ADVANCED_FACE ->
    CYLINDRICAL_SURFACE (yerel merkez/eksen/yaricap) + stil zinciri -> COLOUR_RGB.
 2) gmsh'ten KURESEL silindirler (brep_axes; yaricaplar artik TAM DOGRU, metinle 327/327).
 3) YEREL->KURESEL rijit donusum: TEKIL yaricapli (iki tarafta da bir kez gecen) ciftlerle
    tohumla, sonra RANSAC ile rafine et. Kabul olcutu ARTIK (<0.5mm) -- ic-nokta ORANI DEGIL
    (q2/q3'te oran yanlis olcuttu: binlerce aday cift varken oran hicbir zaman yuksek cikmaz).
 4) Kabul edilen parcalarda her ADAY icin renk ozellikleri:
      c_metal   : eslesen silindirin rengi gumusi metal tonunda mi
      c_govde   : eslesen silindirin rengi govde (metal olmayan) tonunda mi
      c_yok     : eslesen silindire renk atanamadi
 5) TP/FP'ye karsi AUC (bag duzeltmeli Mann-Whitney) + permutasyon null.

KILL (onceden yazili): hicbir renk ozelligi null'un uzerinde AUC >= 0.60 vermezse RENK KANALI
tel/vida ayrimi icin OLU ilan edilir ve bir daha acilmaz. Verirse listeye kaldirac olarak girer.

NOT: kabul edilen parca sayisi dusuk olabilir; o zaman sonuc "olculemedi" diye raporlanir,
"ise yaramaz" diye DEGIL -- ikisi ayni sey degil.
"""
...
from q2_renk_hizalama import parse, kabsch
...
def _renk_haritasi(d):
    ...
    def rgb_of(sid, derinlik=0):
        """stil varliginden COLOUR_RGB'ye derinlemesine in."""
        if derinlik > 8 or sid not in d:
            return None
        t, a = d[sid]
        if t == "COLOUR_RGB":
            try:
    ...
    for i, (t, a) in d.items():
        ...
        if not refs:
            continue
        # son referans genelde hedef ogedir; hepsini deneyip ADVANCED_FACE olani al
...
def _renkli_silindirler(step):
    """[(yerel_merkez, eksen, yaricap, rgb|None)] -- ADVANCED_FACE -> CYLINDRICAL_SURFACE."""
    d = parse(step)
...
def _eksen_artik(lc, la, R, t, gc, ga, gr, lr):
    """Her yerel silindiri, DONUSTURULDUKTEN sonra en uygun kuresel silindirin EKSEN CIZGISINE
    olan dik uzakligiyla olc. Merkez-merkez uzakligi DEGIL."""
    P = lc @ R.T + t
    A = la @ R.T
    out = []
...
def _donusum(loc, gc, ga, gr):
    ...
    KRITIK: silindirin "merkezi" karsilik gelen bir nokta DEGILDIR. STEP metnindeki
    AXIS2_PLACEMENT_3D noktasi eksen uzerinde HERHANGI bir yerde olabilir; bizim cember
    oturtmamiz ise ornekleme araliginin ortasini veriyor. Ayni silindir, ayni eksen, FARKLI
    nokta. Merkezleri eslestirmek (q2/q3'te yaptigim sey) bu yuzden ~30mm artik uretiyordu.

    DOGRU COZUM iki adimda:
      1) DONME: eksen YONLERINDEN. Yaricabi tekil olan ciftler dogrudan karsilik verir;
         Kabsch birim vektorler uzerinde cozulur (oteleme yon vektorlerini etkilemez).
      2) OTELEME: "donmus yerel nokta, kuresel eksen CIZGISI uzerinde olmali" kisitindan.
         Her cift icin (I - a a^T)(R c_l + t - c_g) = 0 -> t'de dogrusal en kucuk kareler.
    """
    ...
    # 1) DONME: eksen yonleri (isaret belirsiz -> kuresel yone en yakin isareti sec)
    ...
    # 2) OTELEME: nokta-CIZGI kisitindan dogrusal en kucuk kareler
...
def main():
    ...
    print(f"{len(parts)} parca | adim 1-3: renk + hizalama", flush=True)
    ...
    for r in parts:
        ...
        if R is None or art >= 0.5:
            red += 1; continue
        kabul += 1
        # kuresel silindirlere renk ata: donusturulmus yerel merkeze en yakin kuresel silindir
        # RENK ATAMASI da EKSEN CIZGISIYLE: merkez karsilik gelen nokta degil (bkz _donusum).
        ...
        for k in range(len(P)):
            ...
            if ok.any():
                ...
                if col is None:
                    ...
                    c_govde = 1.0
                # kanal boyunca (es-eksenli) metal var mi
    ...
    json.dump(out | {"kabul": kabul, "red": red, "n_aday": len(Y),
              ...
              open("results/q7_renk_degeri.json", "w"), indent=1)
    print("makbuz -> results/q7_renk_degeri.json")
