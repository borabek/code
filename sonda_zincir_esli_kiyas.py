# -*- coding: utf-8 -*-
"""E2 — SAHAYA INEN ZINCIR vs OLCULEN ZINCIR, ESLI KIYAS

SORUN. Ihracatcilar `robot_cp.extract` cagiriyor; kampanyada olculen
`urun_p6`/`urun_genis` sahaya HIC girmiyor. `export_robot_glb.py` icine
`cp_config.glb_kanonik_zincir` bayragi kondu ama ACILMADI: saglamlik
denetimi gecti (40/40 parca, cokme/NaN yok, yonler birim) fakat olculen
zincir **181 GT icin 309 CP** uretiyordu. Fonksiyonel karsiligi robotun
olmayan yerlere gitmesidir; F1 ayni kalsa bile bu bir gerileme olabilir.

BU BETIK AYNI PARCALARDA IKI ZINCIRI YAN YANA kosar ve UC sayiyi birden
verir: robot F1, KESINLIK, ve parca basina uretilen CP sayisi.

KESINLIK NEDEN AYRI RAPORLANIR. F1 kesinlik ile recall'i tek sayiya
katlar; robot icin bunlar ayni sey degildir. Yanlis bir CP, robotun bos
yere hareket etmesi (ve carpma riski) demektir. Bu yuzden bayrak
"F1 arttiysa ac" ile DEGIL, "F1 artti VE kesinlik gerilemedi" ile acilir.

D7'ye BAKILMAZ.
"""
import collections
import json
import os
import sys
import time

import numpy as np

import makbuz_hash

os.environ.setdefault("BA_ALLOW_SEEN", "1")
sys.path.insert(0, ".")
import kanonik_d7 as K             # noqa: E402
import d6_kayit                    # noqa: E402
from sina_kume import esle_macar   # noqa: E402

N_PARCA = int(os.environ.get("EZ_N", "24"))
MARKALAR = set(os.environ.get("EZ_MARKA", "NIT,MOR,SUPU,UPUN").split(","))


_NRM = {}


def _graf_duzelt(pb, F, tur, w):
    """Y24: mesh KENARLARI uzerinde olasilik duzeltme (ortalama-alan CRF).

    Her turda her tepenin olasilik vektoru, komsularinin ortalamasiyla
    `w` agirliginda harmanlanir: p <- (1-w)*p + w*komsu_ortalamasi.
    Bu, tam bir CRF'in ortalama-alan yaklasimidir ve ek parametre/egitim
    GEREKTIRMEZ. `tur=0` veya `w=0` -> bit-ayni taban.
    """
    pb = np.asarray(pb, float)
    if tur <= 0 or w <= 0:
        return pb
    F = np.asarray(F, np.int64)
    n = len(pb)
    # kenar listesi (her ucgenin uc kenari, iki yonlu)
    e = np.vstack([F[:, [0, 1]], F[:, [1, 2]], F[:, [2, 0]]])
    e = np.vstack([e, e[:, ::-1]])
    derece = np.bincount(e[:, 0], minlength=n).astype(float)
    derece[derece == 0] = 1.0
    for _ in range(int(tur)):
        top = np.zeros_like(pb)
        np.add.at(top, e[:, 0], pb[e[:, 1]])
        kom = top / derece[:, None]
        pb = (1.0 - w) * pb + w * kom
        s = pb.sum(1, keepdims=True)
        pb = pb / np.maximum(s, 1e-12)
    return pb


def _yerel_normal(V, F, P):
    """her tahmin noktasi icin YEREL DIS NORMAL (en yakin tepenin normali).

    "Disari bak" kurali icin dogru vekil budur: govde merkezi kaba bir
    yaklasimdir ve klemens gibi uzun parcalarda yaniltir.
    """
    import numpy as _np
    if not len(P):
        return _np.zeros((0, 3))
    anahtar = (len(V), len(F))
    try:
        import trimesh
        ag = trimesh.Trimesh(vertices=V, faces=F, process=False)
        N = _np.asarray(ag.vertex_normals, float)
    except Exception:                                  # noqa: BLE001
        return _np.zeros((len(P), 3))
    from scipy.spatial import cKDTree
    _ = anahtar
    return N[cKDTree(V).query(_np.asarray(P, float))[1]]


def _halka_normal(V, F, P, ic_r=3.0, dis_r=8.0):
    """AGIZ CEVRESINDEKI YUZUN normali -- "disari" icin DOGRU referans.

    TESHIS (Bolum 21.32): en yakin tepenin normali aslinda deligin DUVAR
    normalidir ve eksene DIKTIR (tahmin yonuyle arasindaki aci ortanca
    88.9 derece). O referansla isaret atamak rastgeleye yakindir; bu
    yuzden `disari_normal` kolu −0.0266 verdi.

    Dogrusu: CP'nin ETRAFINDAKI HALKADAN (ic_r ile dis_r arasi) normal
    ortalamasi. Bu halka delik duvarinin DISINDA, duz yuzeydedir.
    """
    import numpy as _np
    if not len(P):
        return _np.zeros((0, 3))
    try:
        import trimesh
        from scipy.spatial import cKDTree
        ag = trimesh.Trimesh(vertices=V, faces=F, process=False)
        N = _np.asarray(ag.vertex_normals, float)
    except Exception:                                  # noqa: BLE001
        return _np.zeros((len(P), 3))
    agac = cKDTree(V)
    out = _np.zeros((len(P), 3))
    for i, p in enumerate(_np.asarray(P, float)):
        dis_k = agac.query_ball_point(p, dis_r)
        if not dis_k:
            continue
        d = _np.linalg.norm(V[dis_k] - p[None, :], axis=1)
        halka = _np.asarray(dis_k)[d >= ic_r]
        if not len(halka):
            halka = _np.asarray(dis_k)
        v = N[halka].mean(0)
        n = _np.linalg.norm(v)
        out[i] = v / n if n > 1e-9 else 0.0
    return out


def _pd(cps):
    if not cps:
        return np.zeros((0, 3)), np.zeros((0, 3))
    P = np.asarray([c["point"] for c in cps], float).reshape(-1, 3)
    D = np.asarray([c["direction"] for c in cps], float).reshape(-1, 3)
    return P, D


def main():
    t0 = time.time()
    import torch
    import kanonik_zincir
    import robot_cp
    import thesis_remesh
    from infer_step_cp import load_any
    import diffusionnet as D_
    import connector3d

    cfg = json.load(open("cp_config.json"))
    ca = float(cfg.get("robot_conf_auto", 0.5))
    mav = int(cfg.get("robot_min_auto_votes", 3))
    dev = "cuda" if torch.cuda.is_available() else "cpu"
    # KONTROL NOKTASI SECIMI (2026-08-13). `EZ_CKPT` virgulle ayrilmis yol
    # listesi alir; verilmezse `cp_config.robot_vote2_checkpoints`. Boylece
    # yeni bir segmentasyon kontrol noktasi (orn. Y1 augmentasyonlu) TAM
    # ZINCIRDE, tek degiskenli olculebilir. Kanonik dersi: seg IoU kazanci
    # robot F1 kazanci DEGILDIR, dagitilacak yolda olculmelidir.
    _ck = os.environ.get("EZ_CKPT", "")
    ck_list = ([x.strip() for x in _ck.split(",") if x.strip()] if _ck
               else cfg["robot_vote2_checkpoints"])
    print(f"kontrol noktalari ({len(ck_list)}): "
          f"{[os.path.basename(x) for x in ck_list]}", flush=True)
    modeller = [load_any(c, dev=dev)[:2] for c in ck_list]
    CE, CT = int(connector3d.CABLE_ENTRY), int(connector3d.CONTACT)
    STEP = K.step_haritasi()

    kay = K.yukle()
    kay.update({str(p): r for p, r in d6_kayit.yukle().items()
                if str(p) not in kay})
    # PARCA LISTESI (2026-08-13). `EZ_LISTE` verilirse marka suzgeci yerine
    # ACIK parca listesi kullanilir. Sunulacak "tanidik marka" sayisi icin
    # gereken kume budur: `results/split3.json` -> VAL (100 parca,
    # geometri-ayrik, uretici-KARISIK) ve olculdu ki VAL parcalarinin
    # HICBIRI secicinin egitim kumesinde DEGIL (DEV'de 37 tanesi vardi --
    # DEV bu yuzden KULLANILMAZ).
    liste = os.environ.get("EZ_LISTE", "")
    if liste:
        import json as _j
        _s = _j.load(open(liste, encoding="utf-8"))
        istenen = set(_s[os.environ.get("EZ_BOLME", "val")]["parts"])
        aday = [(p, r) for p, r in kay.items()
                if str(p) in istenen and len(r.get("G", [])) and p in STEP]
        print(f"parca listesi: {liste} / "
              f"{os.environ.get('EZ_BOLME', 'val')} -> {len(aday)} parca "
              f"(STEP'i ve GT'si olan)", flush=True)
    else:
        aday = [(p, r) for p, r in kay.items()
                if r.get("mfg") in MARKALAR and len(r.get("G", []))
                and p in STEP]
    rng = np.random.default_rng(0)
    if len(aday) > N_PARCA:
        aday = [aday[i] for i in rng.choice(len(aday), N_PARCA,
                                            replace=False)]
    print(f"{len(aday)} parca | STEP'ten TAM zincir, iki yol yan yana",
          flush=True)

    agg = {k: collections.Counter() for k in ("saha", "olculen")}
    dokum = []
    n = 0
    for pid, r in aday:
        try:
            Vr, Fr = __import__("export_robot_glb").step_to_mesh(STEP[pid])
            # Y24: MESH-GRAFI UZERINDE OLASILIK DUZELTME (ortalama-alan CRF
            # yaklasimi). Kenar komsuluklari uzerinde birkac tur agirlikli
            # ortalama, tekil yanlis etiketli tepeleri bastirir. Segmentasyon
            # kalitesi YANAL hataya baglandigi icin (Bolum 21.46 baglayici
            # kisit) bu kol dogrudan darbogaza nisan aliyor.
            # "tur,agirlik" biciminde; bos = kapali (bit-ayni taban).
            # Y16: remesh hedefi cevreden ayarlanabilir. Remesh KENDISI
            # bir gurultu kaynagidir; farkli hedefler farkli ucgenleme ve
            # farkli tahmin uretir. Varsayilan 6000 = mevcut davranis.
            _hedef = int(os.environ.get("EZ_REMESH", "6000"))
            V, F = thesis_remesh.remesh_uniform(Vr, Fr, target=_hedef)
            V = np.ascontiguousarray(V, np.float64)
            F = np.ascontiguousarray(F, np.int64)
            # Y22: MC dropout. 0 = kapali (bit-ayni taban davranis).
            _mc = int(os.environ.get("EZ_MCDROP", "0"))
            pbs = []
            for model, meta in modeller:
                _, pb = D_.predict(model, meta, V, F, device=dev,
                                   return_probs=True, mc_dropout=_mc)
                pbs.append(np.asarray(pb, float))
            _crf = os.environ.get("EZ_CRF", "").strip()
            if _crf:
                _t, _w = (float(x) for x in _crf.split(","))
                pbs = [_graf_duzelt(pb, F, int(_t), _w) for pb in pbs]
            # HAM ADAY HAVUZU (2026-08-13). Hata otopsisi kacanlarin
            # %94'unun yakininda HIC tahmin olmadigini gosterdi. Ama bu
            # "havuzda aday YOK" mu, "aday VARDI gate eledi" mi -- ayrimi
            # tahminle gecmek yanlis olur. Havuz burada, gate'ten ONCE
            # kaydedilir; boylece HAVUZ RECALL'u ile CIKTI recall'u yan yana
            # okunur ve kalan emegin havuza mi skora mi gitmesi gerektigi
            # OLCUMLE belli olur.
            # DUZELTME 2026-08-14: havuz DISARIDAN yeniden uretilemez.
            # `extract` operator onbellegini kullanir, buradaki taze cikarim
            # kullanmaz -> olasiliklar farkli, havuz farkli. Kanit: disaridan
            # uretilen havuz ciktinin UST KUMESI bile degildi (84 GT ciktida
            # var/havuzda yok -- yapisal olarak imkansiz). Havuz artik
            # `extract`in ICINDEN, gate'ten hemen once yakalanir.
            del robot_cp.HAVUZ_KANCA[:]
            import wire_gate as _WG
            del _WG.POSE_KANCA[:]
            # SAHA yolu: ihracatcilarin bugun cagirdigi
            saha = robot_cp.extract(modeller, STEP[pid], dev, ca, mav)
            _poz = list(_WG.POSE_KANCA)
            _hav = (robot_cp.HAVUZ_KANCA[-1] if robot_cp.HAVUZ_KANCA
                    else None)
            # OLCULEN yol: kampanyanin olctugu
            olc = kanonik_zincir.urun_cikti(V, F, pbs, STEP[pid], cfg)
        except Exception as e:                       # noqa: BLE001
            print(f"  {pid}: {type(e).__name__}: {e}", flush=True)
            continue
        G = np.asarray(r["G"], float)
        Gd = np.asarray(r["Gd"], float)
        dg = float(np.linalg.norm(V.max(0) - V.min(0)))
        for ad, cps in (("saha", saha), ("olculen", olc)):
            P, D = _pd(cps)
            c = agg[ad]
            # UC METRIK BIRDEN. `manset.py` manseti IKI ayri olcutle
            # hesapliyor ve yapilandirmadaki `robot_hazir_F1` ISARETSIZ
            # olandir (`esle(..., 2.0, 10.0, False)` -- `isaretli`
            # varsayilani False), yani 180 derece TERS bir yon DOGRU
            # sayilir. Robot icin dogru olcut ISARETLI olandir.
            # Ikisi de raporlanir ki sunulan sayinin hangisi oldugu
            # ASLA belirsiz kalmasin.
            for etiket, im in (("robot_isaretli", True),
                               ("robot_isaretsiz", False)):
                tp, fp, fn = esle_macar(P, D, G, Gd, dg, K.YANAL, K.ACI,
                                        False, isaretli=im)[:3]
                c[etiket + "_tp"] += tp
                c[etiket + "_fp"] += fp
                c[etiket + "_fn"] += fn
            tp, fp, fn = esle_macar(P, D, G, Gd, dg, 0.0, 180.0, True)[:3]
            c["tespit_tp"] += tp; c["tespit_fp"] += fp; c["tespit_fn"] += fn
            tp, fp, fn = esle_macar(P, D, G, Gd, dg, K.YANAL, K.ACI, False,
                                    isaretli=True)[:3]
            c["tp"] += tp; c["fp"] += fp; c["fn"] += fn
            c["cp"] += len(P)
        # TAHMIN DOKUMU (2026-08-13). Her esik/adet denemesi bugun 27
        # dakikalik TAM ZINCIR kosusu gerektiriyor. Tahminler diske
        # dokulurse ayni denemeler SANIYELER icinde cevrimdisi yapilir.
        # Metrikleri DEGISTIRMEZ, yalnizca cikti ekler.
        for ad, cps in (("saha", saha), ("olculen", olc)):
            P, D = _pd(cps)
            dokum.append({
                "pid": str(pid), "yol": ad,
                "P": P.tolist(), "D": D.tolist(),
                "guven": [float(c.get("confidence", float("nan")))
                          for c in cps],
                "votes": [float(c.get("_votes", float("nan")))
                          for c in cps],
                "G": G.tolist(), "Gd": Gd.tolist(), "diag": float(dg),
                # GERCEK GOVDE BILGISI (2026-08-13). Ilk dokum yalnizca
                # tahminleri tasiyordu; "disari bak" kuralini denerken
                # govde merkezi yerine TAHMIN EDILEN CP'lerin ortalamasini
                # vekil kullanmak zorunda kaldim ve kol haksiz yere dustu
                # (-0.0749). Kahin isaret kolu tavanin +0.0142 oldugunu
                # gosterdi, yani mekanizma degil VEKIL kotuydu. Artik mesh
                # merkezi ve her tahminin YEREL YUZEY NORMALI dokuluyor.
                "havuz_P": (_hav["P"] if _hav else []),
                "havuz_D": (_hav["D"] if _hav else []),
                # POSE KIRPMA TESHISI: kirpma ONCESI yer degistirme.
                # Tek kosudan her `maks_mm` degeri yeniden kurulabilir.
                "pose_p0": [k["p0"] for k in _poz],
                "pose_dw": [k["dw"] for k in _poz],
                "pose_dir": [k["dir"] for k in _poz],
                "mesh_merkez": V.mean(0).tolist(),
                "yerel_normal": _yerel_normal(V, F, P).tolist(),
                "halka_normal": _halka_normal(V, F, P).tolist()})
        agg["saha"]["gt"] += len(G)
        n += 1
        if n % 5 == 0:
            print(f"  {n}/{len(aday)} ({time.time() - t0:.0f} s)", flush=True)

    print(f"\n{n} parca | GT {agg['saha']['gt']}")
    print("\n=== UC METRIK (ayni parcalar, ayni zincir) ===")
    print(f"{'yol':<10}{'tespit':>10}{'robot ISARETSIZ':>18}"
          f"{'robot ISARETLI':>17}")
    for _ad in ("saha", "olculen"):
        _c = agg[_ad]
        def _f(on, _c=_c):
            return (2 * _c[on + "_tp"] / max(
                2 * _c[on + "_tp"] + _c[on + "_fp"] + _c[on + "_fn"], 1))
        print(f"{_ad:<10}{_f('tespit'):>10.4f}"
              f"{_f('robot_isaretsiz'):>18.4f}"
              f"{_f('robot_isaretli'):>17.4f}")
    print("  NOT: yapilandirmadaki `robot_hazir_F1` ISARETSIZ olandir;")
    print("       robot icin gecerli olcut ISARETLI olandir.")
    print(f"{'yol':<10}{'robot F1':>10}{'kesinlik':>10}{'recall':>9}"
          f"{'uretilen CP':>13}")
    out = {}
    for ad in ("saha", "olculen"):
        c = agg[ad]
        f1 = 2 * c["tp"] / max(2 * c["tp"] + c["fp"] + c["fn"], 1)
        kes = c["tp"] / max(c["tp"] + c["fp"], 1)
        rec = c["tp"] / max(c["tp"] + c["fn"], 1)
        out[ad] = {"f1": f1, "kesinlik": kes, "recall": rec,
                   "cp": int(c["cp"]), "tp": int(c["tp"]),
                   "fp": int(c["fp"]), "fn": int(c["fn"])}
        print(f"{ad:<10}{f1:>10.4f}{kes:>10.4f}{rec:>9.4f}{c['cp']:>13}")
    df1 = out["olculen"]["f1"] - out["saha"]["f1"]
    dke = out["olculen"]["kesinlik"] - out["saha"]["kesinlik"]
    print(f"\n  F1 farki       {df1:+.4f}")
    print(f"  kesinlik farki {dke:+.4f}")
    ac = df1 > 0 and dke >= -0.02
    print(f"\nBAYRAK KARARI: {'ACILABILIR' if ac else 'ACILMAZ'}")
    print("  Kural: F1 ARTTI **VE** kesinlik 0.02'den fazla GERILEMEDI.")
    print("  Gerekce: yanlis CP = robotun bos yere hareketi; F1 ayni kalsa")
    print("           bile kesinlik dususu sahada GERILEMEDIR.")
    json.dump({"damga": makbuz_hash.damga(), "n_parca": n,
               "gt": int(agg["saha"]["gt"]), "yollar": out,
               "f1_farki": df1, "kesinlik_farki": dke, "acilabilir": bool(ac),
               "not": "Saha zinciri (robot_cp.extract) vs olculen zincir "
                      "(kanonik_zincir.urun_cikti), AYNI parcalarda. "
                      "D7'ye BAKILMADI."},
              open("results/zincir_esli_kiyas.json", "w"), indent=1)
    _dk = os.environ.get("EZ_DOKUM", "results/_tahmin_dokumu.json")
    json.dump(dokum, open(_dk, "w"), indent=0)
    print(f"tahmin dokumu -> {_dk} ({len(dokum)} kayit)")
    print(f"makbuz -> results/zincir_esli_kiyas.json "
          f"({time.time() - t0:.0f} s)")


if __name__ == "__main__":
    main()
