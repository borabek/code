# -*- coding: utf-8 -*-
"""T12 (t8 turevi): MODEL SINIFI uretici-disi UCTAN UCA fark yaratiyor mu?

Aday duzeyinde (t11, DAGITILAN goreli esikle) olculdu:
...
ExtraTrees en iyi transferi veriyor VE tanidik bedeli butce icinde (-0.0093).

AMA kayitli uyari: "ExtraTrees does NOT transfer, uretici-disi -0.061" -- o olcum 13 SUTUN ve
SABIT ESIK donemindendi; ikisi de degisti. Uctan uca olcmeden hangi hukmun gecerli oldugu bilinemez.

KILL: ExtraTrees, uretici-disi UCTAN UCA tespit F1'de RF'i HER IKI bolmede de gecmezse ALINMAZ.

Buraya kadar olculenler:
  T1/T2  uretici-disi cokus (aday duzeyi): 0.7422 -> 0.6399 / 0.2799; ~%42'si kalibrasyon
  T3/T6  goreli esik + taban (aday duzeyi): en kotu 0.2799 -> 0.4402 (taban .20) / 0.4222 (.25)
  T7     tanidik veride UCTAN UCA bedel: taban .25 -> DEV -0.0140, VAL -0.0022 (ort -0.0081)

EKSIK OLAN: uretici-disi kazanc UCTAN UCA dogrulanmadi. Bugun aday duzeyi iki kez yanildi
(EK blogu, goreli esigin bedeli), o yuzden karar bu betikle veriliyor.

TASARIM: DEV+VAL parcalari URETICIYE gore ayrilir. Her uretici icin gate, O URETICIYI HIC
GORMEDEN egitilir (ayrica test geometri gruplari da dislanir). Turetme bir kez; iki karar
kurali ayni skorlar uzerinde.

KILL (onceden yazili): goreli esik, uretici-disi UCTAN UCA tespit F1'de sabiti HER IKI
uretici de gecmezse DAGITILMAZ.
"""
...
def main():
    ...
    print(f"{len(cache)} parca | uretici dagilimi {dict(say)}", flush=True)
    ...
    # --- TURETME bir kez (ozellikler saklanir) ---
    DER = []
    ...
    print(f"turetme bitti ({sum(len(x['P']) for x in DER)} aday)", flush=True)
    ...
    # egitim korpusundaki uretici kodunu ADA cevir (0/1 -> WEI/PXC)
    kod = {}
    for k in np.unique(tr_mfg):
        adlar = collections.Counter(mfg_of.get(p, "?") for p in tr_pid[tr_mfg == k])
        kod[k] = adlar.most_common(1)[0][0]
    print(f"egitim korpusu uretici kodlari: {kod}", flush=True)

    print(f"\n{'uretici (test)':<18}{'kural':<26}{'tespit':>9}{'ROBOT':>9}{'kesin':>9}{'recall':>9}")
    out = {}
    for k, ad in kod.items():
        icinde = [x for x in DER if x["mfg"] == ad]
        if len(icinde) < 10:
            print(f"{ad}: {len(icinde)} parca, atlandi"); continue
    ...
    print(f"\nKILL: goreli esik HER IKI uretici-disi bolmede sabiti gecmezse DAGITILMAZ -> "
          f"{'GECTI' if ok else 'GECMEDI'}")
    json.dump(out, open("results/t12_model_uretici_disi.json", "w"), indent=1)
    print("makbuz -> results/t12_model_uretici_disi.json")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # SESSIZ OLUM YASAK: t8'in ilk kosusu hicbir iz birakmadan oldu (brep_axes dosya
        # tanitici sizintisi). Artik her cikis nedeni loga yazilir.
        traceback.print_exc()
        raise
