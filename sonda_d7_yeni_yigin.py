# -*- coding: utf-8 -*-
"""D7'de YENI YIGIN: g10 + gate v7 + p5-v2, ESKI 0.2344 ile AYNI KUMEDE.

D7 artik DEV (harcandi) -- bu FINAL DEGIL. Ama 0.2344'u ureten kumeyle AYNI
838 parca / 12 marka oldugu icin DOGRUDAN KIYASLANABILIR tek sayidir.

UC KOL:
  1. eski yigin (g7 onbellegi + gate v5 + tam_poz)      -> 0.2344 civari beklenir
  2. g10 + gate v7 (A3-A4)
...
sys.path.insert(0, ".")
import d6_kayit, robot_cp, wire_gate, urun_zinciri, p5v2_secenek as PS, p5v2_egit as PE
...
kayit = d6_kayit.yukle(set(map(str, sv["pidler"])))
...
pidler = sorted({f[:-4] for f in os.listdir(OB_ESKI) if f.endswith(".npz")}
                ...
                & set(kayit))
print(f"ortak parca {len(pidler)} (D7 = DEV, FINAL DEGIL)\n", flush=True)
...
json.dump({"damga": makbuz_hash.damga(), "sonuc": sonuc, "n_parca": len(pidler),
           "not": "D7 = DEV (harcandi). FINAL DEGIL. 0.2344 ile AYNI kume."},
          open("results/d7_yeni_yigin.json", "w"), indent=1)
print("makbuz -> results/d7_yeni_yigin.json")
