...
this rule garanti wrong produces. Sabit esikte this sorun absent (all of them elenir) but fixed threshold new
...
KILL (onceden yazili, T3/T4 with same):
  (a) manufacturer-disi HER IKI bolmede fixed esigi gecmeli,
  (b) familiar veride loss 0.02'yi asmamali.
"""
import json 
import numpy as np 
...
def main ():
    ...
    U =sorted (set (mfg ))
    taban_f =familiar ["fixed (mevcut)"][0 ]
    print (f"{'rule':<26}{'TANIDIK':>9}{'diff':>8}"+"".join (f"{'uret.'+u :>10}"for u in U )
    +f"{'EN KOTU':>9}{'karar':>9}")
    out ={}
    for ad ,_ ,_ in ADAY :
        t =familiar [ad ][0 ]
        mv =[mout [ad ][u ][0 ]for u in U ]
        gecti =(ad !="fixed (mevcut)"
        and all (mout [ad ][u ][0 ]>mout ["fixed (mevcut)"][u ][0 ]for u in U )
        and (taban_f -t )<=0.02 )
        print (f"{ad :<26}{t :>9.4f}{t -taban_f :>+8.4f}"+"".join (f"{x :>10.4f}"for x in mv )
        +f"{min (mv ):>9.4f}{('GECTI'if gecti else '-'):>9}")
        out [ad ]={"familiar":t ,"tanidik_fark":t -taban_f ,
        "manufacturer":{u :mout [ad ][u ][0 ]for u in U },"en_kotu":min (mv ),"gecti":gecti }
    print ("\nKILL: (a) each iki manufacturer-disi bolmede sabiti gecmeli, (b) familiar loss <= 0.02")
    json .dump (out ,open ("results/t6_relative_baseline.json","w"),indent =1 )
    print ("receipt -> results/t6_relative_baseline.json")
