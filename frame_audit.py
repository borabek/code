# -*- coding: utf-8 -*-
"""EVDE-2a: WEI olcum butunlugu -- align_frames residual audit.

FBI supheci D2: WEI yon hatasi 24-90 derece kararsizdi (PXC 0). Eger STEP<->JSON frame hizalamasi
WEI'de bozuksa, axis-aware esleme SISTEMATIK kaciriyor -> FN yapay sisiyor (PHANTOM recall kaybi).
Bu script her WEI held-out parcasinda align_frames residual_mm'i olcer. Kucuk residual (~tessellation,
<1mm) = hizalama saglam; buyuk (>2mm) = ayni geometri DEGIL / hizalama supheli -> o parcanin FN'leri
...
def main():
    ...
    print(f"  BOZUK  (>2mm):   {len(bad)}  <- bu parcalarin FN'leri PHANTOM olabilir")
    if bad:
        tot_bad_cp = sum(r["mfg_cps"] for r in bad)
        print(f"  bozuk parcalardaki uretici CP sayisi: {tot_bad_cp} (toplam recall paydasindan cikarsa recall yukselir)")
