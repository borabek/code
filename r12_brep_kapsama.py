...
BULGU (R11): B-rep ekseni bulunan aday orani tanidikta %47, gorulmemiste %24. B-rep
bulunmayinca `channel_axis + _snap_axis` yoluna dusuluyor -- ve o yol EGIMI GOREMIYOR
(kod yorumu: ray-probe cozunurlugu deligin koni acisindan ~11 derece keskin olamaz).
Uretici yonlerinin %19.1'i 10 dereceden egik oldugu icin bu, sistematik dik hataya cevriliyor.

KAPILAR (cp_openings.py): CP_BREP_MAX_OFF=5.0mm, max_turn_deg=60, r_range=(0.5, BREP_R_MAX)
Soru: kapsama kapilardan mi dusuk, yoksa aciklik gercekten silindir DEGIL mi?

OLCUM: her sinav parcasinda STEP'teki silindirleri oku, aday noktasina en yakin silindirin
mesafesini/aci sapmasini cikar. Kapiya TAKILAN mi, HIC SILINDIR YOK mu?
"""
...
for r in D:
    ...
    C=np.asarray(cyl[0],float)          # merkez
    A=np.asarray(cyl[1],float)          # eksen
    for i in range(len(P)):
        p=P[i]; d=Pd[i]/(np.linalg.norm(Pd[i])+1e-9)
        rel=p-C
        t=(rel*A).sum(1)
        dik=np.linalg.norm(rel-t[:,None]*A,axis=1)   # eksene dik mesafe
