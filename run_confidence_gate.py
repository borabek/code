...
SAHA GERCEGI: akis two rejimin karisimi -- known markadan new model + never
unknown markadan new model. Tam otomatik single number 0.90 this karisimda
...
(`parca_kirilim[pid]["score"] / ["correct"]`). Skor esigi t for:
    precision(t) = correct(score>=t) / prediction(score>=t)
    kapsama(t)  = prediction(score>=t) / tum tahminler        (sign kapsamasi)
    GT kapsama  = correct(score>=t) / GT                     (islenen CP orani)
...
sinavdan setting cekmek olurdu.

Kullanim:
    python run_confidence_gate.py <calibration.json> [exam.json] [hedef]
"""
import json 
import os 
import sys 
...
def cift (receipt ):
    """Makbuzdan (score, correct) dizileri + GT count."""
    s =receipt ["sonuc"]
...
def bas (ad ,S ,Y ,n_gt ,threshold =None ,hedef =HEDEF ):
    print (f"\n--- {ad } ---")
    if not len (S ):
        print ("  score/correct verisi YOK (receipt old surumle uretilmis)")
        return None 
    print (f"  tahmin {len (S )} | correct {int (Y .sum ())} | GT {n_gt } | "
    ...
    if threshold is None :
        ...
        if r is None :
            ...
            return None 
        threshold =float (r [0 ])
        print (f"  SECILEN threshold {threshold :.4f} (calibration)")
    k =S >=threshold 
    if not k .any ():
...
def main ():
    ...
    out ={"hedef_kesinlik":hedef ,"threshold":threshold ,"calibration":sys .argv [1 ]}
    if threshold is not None and len (sys .argv )>2 and os .path .exists (sys .argv [2 ]):
        ...
        if len (S2 ):
            k =S2 >=threshold 
            out ["exam"]={
            "file":sys .argv [2 ],"precision":float (Y2 [k ].mean ())if k .any ()else 0.0 ,
            ...
            f"{out ['exam']['precision']:.1%} kesinlikte; this kalitede "
            f"otomatik kapsama GT'nin {out ['exam']['gt_kapsama']:.1%}'i "
            f"({out ['exam']['onayli']} ONAYLI / "
            f"{out ['exam']['oneri']} ONERI).")
    json .dump (out ,open ("results/confidence_gate.json","w"),indent =1 )
    print ("\nmakbuz -> results/confidence_gate.json")
