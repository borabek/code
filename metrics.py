...
#
# The models are evaluated not just ten accuracy (which is misleading with
# heavily imbalanced labels) but also ten Dice coefficient and Jaccard index
...
def segmentation_report (pred ,true ,n_classes =5 ,class_names =None ):
    """All metrics at before as a dict.
    ...
    Builds the confusion matrix exactly before.
    """
...
def match_predictions (pred_points ,gt_points ,dist_thresh_mm ):
    """Optimal one-to-one matching (Hungarian algorithm ten distance cost).

    Was greedy nearest-unused-GT-within-threshold, consumed in score order. Greedy
    matching can ORPHAN an exact match: if a mediocre, higher-scored prediction is
    processed first and happens to be within threshold of a GT point, it claims
    that GT even when a later, near-perfect prediction of the SAME point exists --
    inflating both FP and FN for what is really one TP. The Hungarian algorithm
    considers all pairings jointly: it maximises the number of within-threshold
    matches first (any valid pairing is always cost-preferred over an invalid
    one), and among assignments achieving that maximum, minimises total match
    distance -- so this orphaning cannot happen, independent of prediction order.

    Returns (matches, unmatched_pred, unmatched_gt) where matches is a list of
    (pred_idx, gt_idx, dist). Prediction/GT order no longer matters for
    correctness (unlike the old greedy version).
    """
    ...
    dist =np .linalg .norm (pred_points [:,None ,:]-gt_points [None ,:,:],axis =2 )
    # Invalid (over-threshold) pairs get a huge but finite cost so the solver
    # always prefers any valid pairing over them, while still returning a full
    # assignment we can filter afterward (linear_sum_assignment has no native
    # "leave unmatched" option).
    big =float (dist_thresh_mm )*1000.0 +float (dist .max (initial =0.0 ))+1.0 
    cost =np .where (dist <=dist_thresh_mm ,dist ,big )
    row_idx ,col_idx =linear_sum_assignment (cost )

    matches =[]
    used_pred =np .zeros (n_pred ,dtype =bool )
    used_gt =np .zeros (n_gt ,dtype =bool )
    for pi ,gj in zip (row_idx ,col_idx ):
        if dist [pi ,gj ]<=dist_thresh_mm :
            matches .append ((int (pi ),int (gj ),float (dist [pi ,gj ])))
            used_pred [pi ]=True 
            used_gt [gj ]=True 
    unmatched_pred =[pi for pi in range (n_pred )if not used_pred [pi ]]
    unmatched_gt =[gj for gj in range (n_gt )if not used_gt [gj ]]
