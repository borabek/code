# -*- coding: utf-8 -*-
"""A4: g10 + gate v7 REFIT -- uctan uca A/B (D6, 468 parca).

UC KOL, AYNI PARCALARDA:
  1. g7  + gate v5  = DAGITILAN durum (taban)
  2. g10 + gate v5  = yeni ag, ESKI gate (refit YAPILMAMIS hali -- dusuk cikmasi
                      BEKLENIR; "iki gate ayni dagilimda egitilmeli" dersi)
  3. g10 + gate v7  = yeni ag + REFIT gate  <- olculmek istenen
Hata YUTULMAZ.
"""
...
sys.path.insert(0, ".")
import d6_kayit
from sonda_k65b_olc import olc
...
pidler = sorted(ortak & set(kayit))
print(f"ortak parca: {len(pidler)}\n", flush=True)

sonuc = {}
for ad, ob, gy in KOLLAR:
    gate = pickle.load(open(gy, "rb"))
    t, r = olc(pidler, ob, kayit, gate, S)
    sonuc[ad] = {"tespit": t, "robot": r, "onbellek": ob, "gate": gy}
    print(f"{ad:<28} tespit {t:.4f} | robot {r:.4f}", flush=True)

taban = sonuc["g7 + gate v5 (DAGITILAN)"]
for ad, v in sonuc.items():
    v["tespit_fark"] = v["tespit"] - taban["tespit"]
    v["robot_fark"] = v["robot"] - taban["robot"]
...
json.dump({"damga": makbuz_hash.damga(), "sonuc": sonuc, "n_parca": len(pidler),
           "not": "D6 = DEV. Muhurlu sinav DEGIL."},
