# -*- coding: utf-8 -*-
"""K2+K3: KONUM SOZLUGUNU ZENGINLESTIR -- yanal tarafin %44'lik kapsamini yukselt.

OLCULEN DURUM (r3): konum sozlugu KAHINI 0.6146, kendi tavani 0.6469 -> kapsam %44.
Yon sozlugu ayni olcumde %90 kapsiyordu. Eksik olan taraf BURASI.

K1 SONUCU: yanal hata hizalama artigindan GELMIYOR (Spearman +0.049, artik medyani
0.18mm). Yani model hatasi; yatirim mesru.

MEVCUT SOZLUGUN KUSURU: "agiz" girisi, agiz bandindaki noktalarin ORTALAMASI. Hafizada
[[brep-radius-arc-bug]] kaydi var: "silindirik yuzlerin %0'i tam tur; centroid tabanli
yaricap 3.5 kat KUCUKTU; cember oturtma sart". Ayni yanlilik MERKEZI de kaydirir --
kismi ornekleme varsa ortalama, dolu tarafa cekilir.

EKLENEN GIRISLER:
    cember     agiz noktalarina EN KUCUK KARELER cember merkezi (ortalama degil)
    acik       EN ACIK NOKTA: agiz duzleminde, en yakin malzemeye uzakligi MAKSIMUM olan
               nokta (Chebyshev merkezi). Kare/yarik agizlarda fiziksel olarak DOGRU
               tanim budur -- telin gecebilecegi en genis yer.
    kesit7/10  daha derin kesit merkezleri (GT kanalin derininde olabilir)
    kirpik     kirpilmis ortalama (aykiri sinir noktalarina dayanikli)
...
def en_acik_nokta(Q2, R=8.0, n=41):
    """Chebyshev merkezi: en yakin malzemeye uzakligi MAKSIMUM olan nokta.

    Kare/yarik agizlarda 'merkez'in fiziksel tanimi budur -- telin gecebilecegi
    en genis yer. Ortalama, kismi ornekleme varsa dolu tarafa cekilir.
    """
    if len(Q2) < 6:
        return None
    g = np.linspace(-R, R, n)
...
def konum_sozlugu(V, p, d, cyl, ba):
    ...
    agiz = (np.abs(t) <= 1.5) & (rad <= 9.0)
    if agiz.sum() >= 6:
        S["agiz"] = V[agiz].mean(0)
        Q = rel[agiz]
        Q2 = np.stack([Q @ u, Q @ v], 1)
        # KIRPIK ORTALAMA: aykiri sinir noktalarina dayanikli
        med = np.median(Q2, 0)
...
def main():
    ...
    ESKI = {"mevcut", "ham", "uye0", "uye1", "uye2", "uye3", "agiz",
            "kesit1", "kesit3", "kesit5", "eksen"}
    say = {}
    t0 = time.time()
    ...
    gecti = kaps >= 0.70
    print(f"\nKILL: zengin sozluk YANAL tavaninin >=%70'i -> {'GECTI' if gecti else 'GECMEDI'}")
    with io.open("results/k23_konum.json", "w", encoding="utf-8") as f:
        json.dump({k: float(v) for k, v in F.items()} |
                  {"kapsam": float(kaps), "gecti": bool(gecti)}, f, indent=1)
    print("makbuz -> results/k23_konum.json")
