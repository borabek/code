...
DENETIM TAVSIYESI: "konum-9 and very-radius-24 bloklarini guncel 22 ozellige yeniden uret.
...
O measurement 13 sutunlu gate zamanindan; gate that gunden beri 22 sutuna output (B-rep fiziksel +
icbukey topoloji). Zengin bloklarin KATTIGI SEY still present mi, YENIDEN olculmeli.

BU BETIK NE YAPAR: parite korpusunun AYNI adaylarini produces (robot_cp.derive_candidates) and each
candidate for HEM 22 temel sutunu HEM 33 zengin sutunu writes. Boylece A/B'de TEK DEGISKEN feature
kumesi becomes -- ayri kosularda uretilmis two npz'yi karsilastirmak this projede more before
yaniltmisti.
...
def zengin (V ,F ,probs ,cps ,Nrm ):
    ...
    for c in cps :
        ...
        f +=list ((p -lo )/diag )+list ((hi -p )/diag )# A2 face uzakligi (6)
        rel =V -p 
        dist =np .linalg .norm (rel ,axis =1 )
        for r in RAD :# B very-radius (24)
            m =dist <=r 
            if m .any ():
...
def _normaller (V ,F ):
    """Vertex normalleri (face normallerinin alan-agirlikli toplami)."""
...
def main ():
    with io .open ("cp_config.json",encoding ="utf-8")as f :
        cfg =json .load (f )
    cks =cfg ["current_product"]["checkpoints"]if "checkpoints"in cfg .get ("current_product",{})else cfg ["robot_vote2_checkpoints"]
