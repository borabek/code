# -*- coding: utf-8 -*-
"""THE BIG ARBITER: score the product against MANUFACTURER ConnectionPoints on ~379 parts / 1133 CPs.

WHY THIS EXISTS: every product decision in this project has been made on a 9-part / 18-CP manufacturer
arbiter whose noise is about +-0.10 -- big enough that it killed the 9k-resolution model (-0.156) and
contradicted the 82-CP held-out set on pos_weight, with no way to tell which was right. DataSet 1
...
Frame alignment VALIDATED on a 25-part sample: 25/25 aligned, median align residual 0.167, median
bounding-box difference 0.00mm on ~82mm parts (one outlier at 6.4mm).
...
def greedy(P, G, tol, Gdir=None, axis_tol=None):
    ...
    AXIS-AWARE MODE (Gdir given): match on the distance PERPENDICULAR to the manufacturer's own
    InsertDirection, i.e. "is this the same opening?", and allow any depth along that axis (up to
    axis_tol). WHY: measured on 61 manufacturer CPs, our prediction sits median 1.4mm off laterally
...
def gt_gecerli(jf):
    """GT PUANLANABILIR MI? Degilse parca korpusa GIRMEZ (2026-08-04'te eklendi).

    NEDEN VAR -- AL VAKASI: gorulmemis uretici sinavinda AL ureticisinde canli urun de,
    TABAN da, v3 de TAM 0.000 aldi. Uc hipotez sirayla curutuldu:
      1. "align_frames bozuk"      -> HAYIR, AL residual 0.13mm (ELMEX 0.11 ile ayni)
      2. "yon vektoru sifir"       -> filtrelenmis kumede oyle gorunmedi (AL zaten elenmisti)
      3. HAM JSON'a bakildi        -> `InsertDirection {'X':0.0,'Y':0.0,'Z':0.0}` HARFIYEN SIFIR
                                      ve 19 CP'nin HEPSI ayni noktada (bbox min == max)
    Yani GT'nin kendisi kullanilamaz durumda; urun yanlis bulmuyor, OLCULEMIYOR.

    TUM KORPUSTA OLCULDU: 4432 parcanin **27'sinde** yon sifir (26 AL + 1 A-B),
    **26'sinda** tum CP'ler ayni noktada. Korpusun %0.6'si -- ama AL kucuk bir uretici
    oldugu icin dengeli ornekleme onu sinav kumesinin %8.4'une tasiyor ve sonucu
    SAHTE olarak agirlastiriyordu.

    NEDEN BURADA: `eligible()` egitimin de, olcumun de, sinav kumesinin de TEK bogaz
    noktasi. Filtreyi kullanan yerlere tek tek eklemek yerine kaynakta kapatiliyor --
    yoksa bir sonraki betik yine dejenere GT'yi puanlar ve "model kotu" der.
    """
    # DIKKAT: burada `except Exception` KULLANMA. Ilk surumde oyleydi ve dosyayi
    # `io.open` ile aciyordum -- ama `io` bu modulde import EDILMEMIS. Dogan NameError'i
    # genis except YUTTU, fonksiyon her parcada False dondu ve korpus 4432'den **0'a**
    # dustu; hicbir hata mesaji cikmadi, yalnizca "27 cikarildi" yerine "4720 cikarildi"
    # yazdi. Genis except, kendi yazim hatani veri bulgusu gibi gosterir.
    # Cozum iki katmanli: (1) `open()` yeterli, `io` gerekmiyor -- `np` zaten satir 21'de
    # modul duzeyinde; (2) yalnizca GERCEK okuma/bicim hatalari yakalanir.
    try:
    ...
    if (np.linalg.norm(D, axis=1) < 1e-6).all():
        return False                      # yon yok -> eksenel tolerans anlamsiz
    if len(G) > 1 and float(np.abs(G.max(0) - G.min(0)).max()) < 1e-6:
        return False                      # butun CP'ler ayni noktada -> tekil esleme imkansiz
    return True
...
def eligible():
    ...
    gecersiz = []
    # KIMLIK AYRISTIRMASI (2026-08-04 duzeltmesi, H1): eskiden her iki tarafta da
    # `basename.split("_")[<n>]` kullaniliyordu. Bu, ALT CIZGI iceren uretici kodlarinda
    # (`A-B_N.1492-H4_...` -> mfg "A-B", pid BOS) ve alt cizgili parca numaralarinda
    # (`ELMEX.KUT16_GY_...` -> "KUT16") kiriliyordu: 481 dosya yanlis, 291'i BOS kimlik
    # (yani STEP'i gelse bile korpusa ASLA giremezdi), 35 kimlik cakisiyordu.
    # ETKI OLCULDU ve NOTR: eslesme 1926 -> 1926, olcum kumesi 194/194 korundu, yalniz
    # ELMEX.KUDD4D1_GY'nin kimligi "KUDD4D1" -> "KUDD4D1_GY" oldu (ayni dosya).
    # Bugun kazanc sifir cunku alt cizgili parcalarin STEP'i yok; deger ILERIDE.
    ...
    # URETICI TAKMA ADLARI (2026-08-04, H2): DataSet5 ile A-B ve A-B_N ayni parcayi IKI
    # KEZ getirdi -- olculdu: 288 parca, HEPSI ayni STEP dosyasini paylasiyor ve CP sayilari
    # BIREBIR ayni. Yani ayni fiziksel parcanin iki NORM kaydi (IEC/NEMA gibi).
    # UC ZARARI VARDI:
    #   1. gate egitiminde CIFT AGIRLIK (A-B'ye dogru sapma)
    #   2. bolme sizintisi (bir kopya egitimde, digeri testte) -- geometri grubu bunu
    #      yakaliyordu ama tesadufen
    #   3. EN SINSISI: "A-B'yi disarida birak" testinde A-B_N ICERIDE kalir; yani uretici
    #      HIC disarida birakilmamis olur ve producer-out satirlari SESSIZCE SISER.
    # COZUM: takma ad birlestirilir + ayni (pid, STEP) ciftinden TEK kayit tutulur.
    TAKMA = {"A-B_N": "A-B"}
    out, gorulen = [], set()
    for f in sorted(glob.glob(os.path.join(DS, "*ElectricalTerminal*.json"))):
        ...
        if (pid, step[pid]) in gorulen:      # ayni fiziksel parca, ikinci norm kaydi
            continue
        if not gt_gecerli(f):
    ...
    if gecersiz:
        ...
        print(f"  [GT gecerlilik] {len(gecersiz)} parca CIKARILDI (puanlanamaz GT): "
...
def main():
    ...
    print(f"{len(parts)} uygun parca | {len(models)} model | remesh {a.remesh_target} "
    ...
    for k, (mfg, pid, jf, stp) in enumerate(parts, 1):
        ...
        try:
            ...
            R, t, ares = align_frames(Vr, Vj)   # ares = hizalama residual (mm); buyuk = STEP!=JSON geometri
            if cps:
            ...
            if a.icp:                                    # FAZ 1: ICP-refined hizalama ile de eslestir
    ...
    print(f"\n=== BIG ARBITER ({len(rows)} parca, {T+Fn} uretici CP, {skipped} atlandi) ===")
    ...
    for m in sorted({r["mfg"] for r in rows}):
        ...
        print(f"    {m}: {len(s):4d} parca  F1={2*p_*r_/max(p_+r_,1e-9):.3f}  P={p_:.3f} R={r_:.3f}")

    # HIZALAMA-STRATIFIED (objektif, modelden bagimsiz): align_res buyuk = STEP!=JSON geometri -> GT yerlestirilemez.
    # Bu parcalarda modelin dogru tahmini bile FN sayilir. Kriter align residual, MODEL PERFORMANSI DEGIL.
    def f1_sub(sub):
    ...
    for thr in (10.0, 7.0, 5.0, 3.0):
        ...
        line = f"    residual<={thr:>4.1f}mm: {len(keep):4d} parca ({nx:3d} haric)  F1={f1k:.3f} P={pk:.3f} R={rk:.3f}"
        for m in sorted({r["mfg"] for r in rows}):
    ...
    if a.icp and any("tp_icp" in r for r in rows):     # FAZ 1: ICP-refined hizalama karsilastirmasi
        ...
        # ICP + residual-filtre (gercekten farkli parcalari da cikar)
        for thr in (7.0, 5.0):
            ...
            pp = tt/max(tt+ff, 1); rr = tt/max(tt+nn, 1)
            print(f"    ICP + residual<={thr}mm: {len(keep)} parca ({len(rows)-len(keep)} STEP!=JSON haric)  F1={2*pp*rr/max(pp+rr,1e-9):.3f}")
