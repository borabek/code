# -*- coding: utf-8 -*-
"""KONTROL: genisletilmis havuz mu kotu, yoksa D6-tek gate egitimi mi kucuk?

`kos_brep_gate.py` genisletilmis havuzda robot 0.1568 verdi; kanonik 0.2029.
Ama iki kol AYNI SEYI degistirmedi: refit gate yalniz D6'nin 468 parcasiyla
egitildi, kanonik gate v6 binlerce parcayla. Karistirici ayrilmadan "havuz kotu"
DENEMEZ.

Bu betik ayni onbellekli ozniteliklerle UC kolu ayni egitim buyuklugunde kiyaslar:
  A) SEG-TEK havuz  + D6-tek gate     <- kontrol
  B) GENISLETILMIS  + D6-tek gate     <- deney
  C) SEG-TEK havuz  + kanonik gate v6 <- referans (bilinen 0.2029)
A ile B arasindaki fark HAVUZUN etkisidir; A ile C arasindaki fark EGITIM
...
kay6 = None
import d6_kayit
kay6 = d6_kayit.yukle({d["pid"] for d in tr})
for d in tr:
...
def olc(skorla, segtek, esikler):
    ...
    for e in esikler:
        ...
        for d in te:
            ...
            if len(P2) > 1:
                nm = wire_gate.kalabalik_maskesi(P2, s[k]); P2, D2 = P2[nm], D2[nm]
            tp, fp, fn = esle_macar(P2, D2, d["G"], d["Gd"], d["diag"], K.YANAL,
                                    K.ACI, False, isaretli=True)[:3]
            a = rob[d["mfg"]]; a[0] += tp; a[1] += fp; a[2] += fn
            tes.append((len(d["G"]),) + esle_macar(P2, D2, d["G"], d["Gd"],
...
for ad, segtek in (("A) SEG-TEK + D6 gate", True),
    ...
    print(f"{ad:<28} robot {r['robot']:.4f} | tespit {r['tespit']:.4f} | "
          f"makro {r['makro']:.4f} | esik {r['esik']:.2f} | egitim {sh} poz {poz:.4f}",
          flush=True)
g6 = K.gate_yukle()
out["C) SEG-TEK + kanonik gate v6"] = olc(
    lambda X: np.asarray(wire_gate.karar_skoru(g6, X), float), True, ES)
r = out["C) SEG-TEK + kanonik gate v6"]
print(f"{'C) SEG-TEK + kanonik v6':<28} robot {r['robot']:.4f} | tespit {r['tespit']:.4f} | "
      f"makro {r['makro']:.4f} | esik {r['esik']:.2f}", flush=True)
...
json.dump({"damga": makbuz_hash.damga(), "sonuc": out,
           ...
           "not": "Ayni onbellekli oznitelikler, ayni esik taramasi. D7 marka-disi, MIKRO."},
