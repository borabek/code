# -*- coding: utf-8 -*-
"""CC-B: cok-CP parcalarinda KACIRILAN GT noktalarinda segmentasyon ATESLIYOR MU?

LISTENIN YARISININ KADERI BUNA BAGLI:
  * ATESLIYOR  -> engel post-isleme/cozunurluk. min_v10 + adaptif hedef (CC-C) ise yarar,
                  CC-G (segmentasyon yeniden egitimi, GUNLER) gereksiz.
  * ATESMIYOR  -> ag kor. Vertex'i acmak ise yaramaz (sinif gelmez); CC-C ve CC-E buyuk
                  olcude bosa gider ve CC-G ANA IS olur.

CC-A'yi TAMAMLAR: CC-A "o acikliklarda yeterli VERTEX yok" dedi (min_v=30 icin %82.7).
Bu, gerek kosulun ihlali. CC-B yeter kosulu sorar: vertexler olsa AGDA SINIF var mi?

YONTEM: her GT agzi icin cevresindeki vertexlerin CE/CT olasiligina bak. Bulunan GT'lerle
(robotun eslestirdigi) kacirilanlari KIYASLA -- mutlak sayi degil FARK anlamli.
"""
...
def main(limit=60):
    ...
    print(f"{len(parts)} cok-CP parca ({HIGH_CP}+ CP)\n", flush=True)
    ...
    if not missed:
        print("kacirilan GT yok -- olcum anlamsiz"); return
    print(f"\n=== {len(found)} BULUNAN vs {len(missed)} KACIRILAN GT ===\n")
    print(f"{'olcut':<22}{'BULUNAN':>10}{'KACIRILAN':>11}   yorum")
    out = {}
    for r in RADII:
    ...
    # KARAR: kacirilanlarda baglanti olasiligi VAR MI
