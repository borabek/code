# -*- coding: utf-8 -*-
"""HIGH-CP SECICI (deploy modulu, yan-etkisiz). high-CP(>=11) parcalarda genisletilmis 6k+9k candidate havuzunu
lattice-augmented RF with top-N sirala. Egitim etiketi = manufacturer-CP hakemi (certain source: Desktop\\JSON).
Bu, wire_gate'in USTUNE gelen 2. kademe selector: only high-CP parcalarda devreye girer.
...
def lattice_feats (P ,ws ):
    """each candidate for lattice sinyalleri: consistency, grid-fit(u,v), yogunluk, row/kolon, u/v-pozisyon.
    Frame-invariant (SVD with surface eksenleri + goreli pozisyon) -> STEP ya da JSON frame same sonuc."""
...
def rank_feats (P ,ws ,N ):
    """4 ranking-mode sinyali (kullanici #12): within-part ws-yuzdelik, duplicate-penalty (more high-ws
    yakin komsu count), grid-cell-best (lattice hucresinde most high-ws mi), N-orani. leave-FAMILY-out 3/3
    win (part-out'ta hafif zarar; new-aile genellemesi = deployment metrigi kazaniyor)."""
...
def train_and_save (pool ="results/highcp_pool.json",out =MODEL_PATH ):
    """full-train RF (deploy). OOF/family-out DEGERLENDIRME leave_family_out.py'de -- this SADECE deploy artefact."""
    ...
    print (f"high-CP selector RF ({len (y )} candidate, {len (pids )} part, {len (names )} feat) -> {out }")
    return out 
...
def apply (cps ,V ,probs ,CE ,CT ,N ,base_ws =None ,model_path =MODEL_PATH ):
    ...
    if base_ws is None :
        wm =wire_gate ._load ()
        # DUZELTME 2026-08-14: here `wm["clf"].predict_proba(X13)` cagriliyordu,
        # i.e. gate'in PARCA-ICI Z-SKOR donusumu ATLANIYORDU. Gate that donusumle
        # yeniden egitildikten after (58 -> 116 column) this path ValueError gives
        # hale geldi: **dense-part yolu bugun HIC KOSAMIYOR**. `decision_score`
        # already "single source" as yazilmis and donusumu own inside does;
        # dosyanin own kurali buydu, burasi ona uymuyordu.
        base_ws =(wire_gate .decision_score (wm ,X13 )if wm is not None 
    ...
    # `feats_for` **13** column donduruyordu (13 + 8 lattice + 4 rank = 25).
    # Bugun 58 column donduruyor (fizik/topoloji/zengin sonradan eklendi) ->
    # augment 70 uretiyor and selector ValueError veriyordu. Yani this path
    # BUGUN HIC KOSAMIYORDU.
    # DOGRULANDI (varsayilmadi): `FEAT_NAMES = FEAT_NAMES_13 + ...`, i.e.
    # new features SONA eklenmis; secicinin sakladigi 25 ismin first 13'u
    # `FEAT_NAMES_13` with BIREBIR same. Dolayisiyla first 13 sutunu almak,
    # secicinin egitildigi girdiyi TAM as yeniden kurar.
    # NOT: `base_ws` YUKARIDA TAM feature matrisiyle is computed -- gate 58
