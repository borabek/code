# -*- coding: utf-8 -*-
"""TEL-G'yi korpusa kos: dusuk-CP adaylari icin B-rep ozellikleri.

Model KOSULMAZ -- aday konumlari rich_mv10_all.npz'de zaten var; tek gereken STEP'i acmak
(meshlemeden) ve cerceveyi geri cevirmek. ~1.5s/parca.

KILL KRITERI (baslamadan yazildi, sonuca gore degistirilmez):
    Mevcut 41 ozellik UZERINE ek CP-F1 katkisi < 0.02 ise bu aile OLU ilan edilir.
    (TEL-B dersi: kriter AUC'ye degil URUN METRIGINE yazilir.)
"""
...
def main():
    ...
    print(f"{len(todo)} dusuk-CP parca", flush=True)
    ...
    if os.path.exists(OUT):
        ...
        for q, arr in zip(z2["pid_part"], z2["blocks"]):
            feats[str(q)] = arr
        print(f"  [devam] {len(feats)} parca zaten islenmis", flush=True)
    ...
    for k, q in enumerate(todo, 1):
        ...
        try:
            ...
            P = (pos[pid == q] - t) @ R                 # JSON -> STEP cercevesi (bkz tel_g_brep)
            S = B.read_brep(stp)
    ...
    print(f"-> {OUT}  {len(feats)} parca / {tot} aday  {time.time()-t0:.0f}s")
