# -*- coding: utf-8 -*-
"""Op 4 hazirlik: high-CP PXC genisletilmis havuzu CACHE'le (a GPU kosusu). Her candidate for:
part, point (JSON frame), wire_score (sizintisiz RF gate), y (manufacturer-tel mi), main-axis projeksiyonu.
Sonra lattice_rerank.py grid mantigini OFFLINE (fast) dener."""
...
def main ():
    ...
    models =[load_any (c ,dev =dev )[:2 ]for c in cks ]# 7 model: candidate cesitliligi
    ...
    for k ,(pid ,jf ,stp ,n )in enumerate (hi ,1 ):
        try :
            ...
            for model ,meta in models :
                ...
                per9 .append (derive (V9 ,F9 ,pb ,12 ,0.20 ,0.0 ))# vc 0.2 = more very candidate
                per9 .append (derive (V9 ,F9 ,pb ,4 ,0.05 ,0.0 ))# AGRESIF: probe +6/32 (yapisal-gorunmez outside kurtarir)
                # 12k pass (agresif) -- 4GB'de OOM may be -> guard, atlanirsa 6k+9k with devam
            try :
                V12 ,F12 =thesis_remesh .remesh_uniform (Vr ,Fr ,target =12000 );V12 =np .ascontiguousarray (V12 ,np .float64 );F12 =np .ascontiguousarray (F12 ,np .int64 )
                for model ,meta in models :
                    _ ,pb =D .predict (model ,meta ,V12 ,F12 ,device =dev ,op_cache_dir =f"{OP }_k{int (meta .get ('k_eig',64 ))}_12k",return_probs =True )
                    per9 .append (derive (V12 ,F12 ,np .asarray (pb ,float ),6 ,0.10 ,0.0 ))
            except Exception :
                pass 
            cps =_vote2 (per +per9 )
            if not cps :continue 
