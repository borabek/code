...
def main():
    ...
    for k, (pid, jf, stp, n) in enumerate(hi, 1):
        try:
            ...
            for model, meta in models:
                ...
                per9.append(derive(V9, F9, pb, 12, 0.20, 0.0))   # vc 0.2 = daha cok aday
                per9.append(derive(V9, F9, pb, 4, 0.05, 0.0))    # AGRESIF: probe +6/32 (yapisal-gorunmez disinda kurtarir)
            # 12k pass (agresif) -- 4GB'de OOM olabilir -> guard, atlanirsa 6k+9k ile devam
            try:
    ...
    print(f"-> results/highcp_pool_aggr.json ({len(out)} parca) -- AGRESIF (9k-aggr+12k); leave_family_out ile karsilastir")
