# -*- coding: utf-8 -*-
"""CC-D: min_v10 post-islemesinin etkisi REJIM AYRIMLI (dusuk-CP vs cok-CP).

Onceki olcum (+0.021) agirlikli olarak DUSUK-CP parcalarindan geliyordu; CC-A ve CC-B, asil
engelin cok-CP rejiminde oldugunu gosterdi. Bu olcum ikisini AYIRIR.

DURUSTLUK: aile-disi GroupKFold(5) + IC-ICE esik secimi (esik test katindan ogrenilmez),
`seen` bayrakli parcalar skorlanmaz, kilitli holdout'a DOKUNULMAZ, iki kol AYNI parca kumesinde.
"""
import json, sys
import numpy as np
...
def evaluate(X, Y, PID, NGT, G, score_mask, seeds=(0, 1, 2)):
    """Aile-disi OOF + ic-ice esik. Doner: (F1 ortalamasi, sd)."""
    outs = []
    for s in seeds:
        oof = np.zeros(len(Y))
...
def main():
    ...
    # ORTAK, TEMIZ, kilitli-olmayan parcalar -- iki kol AYNI kumede olculur
    common = None
    ...
    lo = common - hi
    print(f"ortak skor kumesi {len(common)} temiz parca  ->  dusuk-CP {len(lo)} | cok-CP {len(hi)}\n")

    print(f"{'kol':<24}{'DUSUK-CP':>12}{'COK-CP':>12}{'HEPSI':>12}")
    res = {}
    ...
    print(f"{'(tohum sd)':<24}{res[b]['dusuk'][1]:>12.4f}{res[b]['cok'][1]:>12.4f}{res[b]['hepsi'][1]:>12.4f}")
    ...
    print("makbuz -> results/cc_d_minv_rejim.json")
