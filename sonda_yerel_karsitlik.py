...
Yani yogun parcada segmentasyon TUM govdeyi boyuyor. Zincirin en basindaki
en guclu oznitelik orada bilgi tasimiyor; asagidaki bes mekanizmanin hepsi
o skoru tuketiyordu.

HIPOTEZ. 0.44 zemin uzerinde 0.52 tepe KURESEL siralamada kaybolur ama
YEREL komsulukta hala tepedir. Oyleyse olasiligin KENDISI degil, yerel
KARSITLIGI ayirt edici olabilir.

OLCULEN (GT'de vs RASTGELE yuzeyde, R = 2/5/10 mm toplarda):
  ham        : p
  fark       : p - ortanca(p, R topu)
  yuzdelik   : p'nin R topundaki yuzdelik sirasi
  tepe       : p, R topundaki en yuksek mi (0/1)
Her biri icin AYRIM = ortanca(GT) - ortanca(rastgele), ve AUC.
AUC, olcegi ne olursa olsun kiyaslanabilir tek sayidir.

KAPI: NIT'te herhangi bir yerel olcunun AUC'si ham AUC'yi >= 0.05 asarsa
...
def yerel(p, V, agac, R):
    """her tepe icin (fark, yuzdelik, tepe_mi) -- R yaricapli topta."""
...
def main():
    ...
    for pid, r in kay.items():
        ...
        p = p[:, CE] + p[:, CT]
        agac = cKDTree(V)
        gi = agac.query(G)[1]                       # GT'ye en yakin tepeler
        rng = np.random.default_rng(0)
        # RASTGELE taban: GT'den >= 4mm uzak tepeler (komsulari sayma)
    ...
    print(f"\n{n} parca | AUC: GT tepeleri vs GT'den >=4mm uzak tepeler")
    print(f"{'marka':<7}{'GT':>7}" + "".join(f"{k:>14}" for k in ad))
    out = {}
    ...
    json.dump({"marka": out, "yaricap": YARICAP,
    ...
    print("makbuz -> results/yerel_karsitlik.json")
