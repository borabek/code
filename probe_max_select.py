...
Maliyet dogrusaldir: ceiling two katina cikarsa kademe-2 row count da approximately
two katina cikar. Bu yuzden karar "recall ne up to artiyor" with verilir.

Kullanim:  MS_N=40 python probe_max_select.py
"""
import json 
import os 
import sys 
...
def main ():
    ...
    from run_p6_ortak import kayitlar 

    on =os .environ .get ("MS_ON","d6")
    n_parca =int (os .environ .get ("MS_N","40"))
    sil_y ,ob =KUME [on ]
    cy =pickle .load (open (sil_y ,"rb"))
    # MARKA SUZGECI SART: dosyalar pid'e according to sirali oldugu for first N file
    ...
    if hedef :
        ...
        print (f"MARKA SUZGECI '{hedef }': {len (cift_ )} part bulundu",
        flush =True )
    else :
    ...
    for i ,(f ,pid )in enumerate (zip (fs ,pidler ),1 ):
        ...
        if int ((kay ==2 ).sum ()):
            ...
            tut [thin_pool .seyrelt (P2 ,s2 ,n01 ,2.5 ,350 ,5 )]=True 
            m2 =m .copy ()
            m2 [i2 ]=tut 
            m =m2 
    ...
    f"{len (fs )}.json")
    json .dump ({"on":on ,"n_parca":len (fs ),"brand":hedef or "karisik",
    "yelpaze":YB .FAN_N ,
    "sonuc":out ,
