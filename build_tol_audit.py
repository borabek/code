# -*- coding: utf-8 -*-
"""GOREV 2a: physical-tolerance audit verisi. Held-out arbiter (WEI+PXC, leakage-guard) each part for
candidate pozisyon+size+wire_score + manufacturer CP pozisyon+direction KAYDET. Sonra tol_audit_sweep.py OFFLINE
different tolerans modlarini (current / opening_radius / size_scaled / wire_fit) dener -> ALL/WEI/PXC F1.
Amac: mevcut tolerans FIZIKSELDEN sikiysa, correct toleransla ALL 0.756 kalkar mi (metrik duzeltmesi)."""
...
def main ():
    ...
    print (f"{len (parts )} held-out arbiter part",flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp )in enumerate (parts ,1 ):
        try :
            ...
            # each (candidate a, manufacturer b): dik distance + axis distance
