...
B1 BASARISIZ OLDU (+0.0023) ve sebebi olculdu: aday basina 2mm icindeki HER
tepenin +/- normali AYRI secenekti; 58582 secenekte pozitif orani yalnizca
%6.4 -- secici samanlikta igne ariyordu. Sinyal VAR (results/b2_yon_isaret.json:
normaller YON_YOK'un %59.1'ini tasiyor), GURULTULU olan SECENEK KUMESIYDI.

DUZELTME: ham normaller yerine OZET yonler --
  agirlikli : p_pos agirlikli ortalama normal
  pca       : normallerin birinci ana bileseni (isaret-hizali)
  enyuksek  : en yuksek p_pos'lu tek tepenin normali
Her biri +/- ile; mevcut yonle birlikte aday basina en fazla ~7 secenek.

TABAN ARTIK ISARET DUZELTMELI kol (B2a, 0.2773 -> 0.3090); B1b onun USTUNE gelir.
TEZE SADIK: konum ve aday sayisi degismez, yalniz yon.
"""
import collections
import json
import os
...
def main():
    ...
    out = {}
    for ad, kul in (("TABAN (isaret duzeltmeli)", False), ("+OZET NORMAL", True)):
        ...
        print(f"{ad:<26} robot {mi:.4f} | tespit {out[ad]['tespit']:.4f} | "
              ...
              flush=True)
    a = out["TABAN (isaret duzeltmeli)"]["robot"]
    ...
    json.dump({"damga": makbuz_hash.damga(), "sonuc": out, "fark": b - a,
               "not": "Mesh normalleri OZETLENDI (agirlikli/pca/en-yuksek). "
                      "Taban ISARET DUZELTMELI kol. D7 marka-disi, TAM ZINCIR."},
              open("results/b1b_ozet_normal.json", "w"), indent=1)
    print("makbuz -> results/b1b_ozet_normal.json")
