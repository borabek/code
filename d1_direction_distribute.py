...
def sozluk_kur (V ,P ,Pd ,Dham =None ,UYE =None ,YUZN =()):
    """Yon sozlugu -- EGITIMDE, OLCUMDE and URUNDE BIREBIR AYNI.

    PARITE DUZELTMESI (2026-08-03 denetimi, bulgu 3+4): onceki surumde dictionary three places
    FARKLI kuruluyordu:
      * training  `sozluk_kur(..., Dham, None, YUZN)`  -> "uye" girdisi HIC YOK
    ...
    for i in range (len (SOZ )):
        ...
        for gi ,(tip ,v )in enumerate (SOZ [i ]):
            ...
            if hedef_yon is not None :
                ...
                # i.e. 180 derece TERS a direction "correct" sayiliyordu. Sozluk whereas BILEREK +-
                # ciftleri iceriyor (obb/dik/yuzn), therefore +a and -a AYNI etiketi aliyor
                # and model ikisini ayirt etmeyi ASLA ogrenemiyordu. Sonuc: EKSEN metriginde
                # +0.0319 but FIZIKSEL (signed) metrikte 0.5818 -> 0.5813 = SAHTE KAZANC.
...
def main ():
    ...
    with io .open ("results/d1_direction_distribute.json","w",encoding ="utf-8")as f :
        ...
        f ,indent =1 )
    print ("receipt -> results/d1_direction_distribute.json")
