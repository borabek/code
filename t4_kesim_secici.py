# -*- coding: utf-8 -*-
"""T4: parca kesimini SECICI degistir (t3'un dogrudan regresyonu dustu).

T3 kahin K'yi dogrudan kestirdi ve KAYBETTI (-0.0148; DEV/VAL/uretici hepsi negatif). Sebep
hedefin gurultulu olmasi: ayni F1'i veren bircok K var, regresyon ortalamaya kaciyor ve
mevcut kuraldan HER PARCADA sapiyor.

Bugun ISE YARAYAN desen farkliydi: kurali herkese uygulamak yerine YALNIZ gerektigi yerde
uygulamak (cokus yonlendirmesi, yarik secicisi). Burada da ayni:

  A  mevcut kural (taban)
  B  BUZULME: K = round(alfa*K_tahmin + (1-alfa)*K_mevcut)   -- alfa taranir
  C  SECICI : yalniz |K_tahmin - K_mevcut| >= esik ISE degistir, yoksa mevcut kural
  D  TEK YON: yalniz AZALTMA yonunde degistir (kahin K medyani mevcuttan KUCUK: 3 vs 4)

KILL (t3 ile AYNI, degistirilmedi): tespit >= +0.02 VE DEV ile VAL ayni yonde VE gorulmemis
uretici ortalamasi dusmeyecek.
"""
import json
import os
import pickle
...
def main():
    ...
    suK = np.array([int(wire_gate.karar_maskesi(p["s"]).sum()) for p in PAR], float)
    print(f"{len(PAR)} parca | kahin K medyan {np.median(yk):.0f} | mevcut K medyan "
    ...
    a = puanla(suK)
    a_dev = puanla(suK, lambda p: p["kume"] == "dev")
    a_val = puanla(suK, lambda p: p["kume"] == "val")
    a_mfg = np.mean([puanla(suK, lambda p, x=x: p["mfg"] == x) for x in ("PXC", "WEI")])
    print(f"\nTABAN: tespit {a:.4f} | DEV {a_dev:.4f} | VAL {a_val:.4f} | uretici ort {a_mfg:.4f}")
    ...
    for es in (1.5, 2.5, 4.0):
        ...
        f = np.abs(oof - suK) >= es
        K[f] = oof[f]
        KOL[f"C secici |fark|>={es}"] = K
    for es in (0.5, 1.5, 2.5):
        K = suK.copy()
        f = (suK - oof) >= es              # yalniz AZALTMA
        K[f] = oof[f]
        KOL[f"D yalniz azalt >={es}"] = K

    print(f"\n{'kol':<24}{'tespit':>9}{'fark':>9}{'DEV':>9}{'VAL':>9}{'mfg ort':>10}{'KILL':>9}")
    kazanan, en = None, -10
    for ad, K in KOL.items():
        t = puanla(K)
        dv = puanla(K, lambda p: p["kume"] == "dev") - a_dev
        vl = puanla(K, lambda p: p["kume"] == "val") - a_val
    ...
    with open("results/t4_kesim_secici.json", "w", encoding="utf-8") as f:
        json.dump({"taban": float(a), "kahin": float(puanla(yk)),
                   "kollar": {k: float(puanla(v)) for k, v in KOL.items()},
                   "kazanan": kazanan}, f, indent=1)
    print("makbuz -> results/t4_kesim_secici.json")
