# -*- coding: utf-8 -*-
"""A3: SIRALAMA hedefi -- GATE_REDDI bir esik degil SIRALAMA hatasi.

Olculdu: karar kurali ailesinin HEPSI tabanin altinda kaldi
(`results/a1_esik_kalibrasyon.json`) -> esigi nasil kurarsan kur GATE_REDDI
kovasi (531 GT, %17.2) bosalmiyor. Demek ki uygun adaylar yanlislarin ALTINDA
siralaniyor; sorun modelin OGRENME HEDEFINDE.

Pointwise BCE her adayi BAGIMSIZ ogreniyor ve cok adayli parcalar kaybi domine
ediyor. Denenen agirliklandirmalar (tek degisken: `sample_weight`):
  duz          agirlik yok (kazanan taban, 0.3070/0.3090)
  parca_esit   1/n_parca -- her parca kayba ESIT katkida bulunur
  poz_dengeli  parca ICINDE pozitif/negatif dengelenir
  karma        parca_esit x poz_dengeli
Ayrica PAIRWISE kol: parca icinde (pozitif, negatif) ciftlerinin FARK vektoru
uzerinde ikili siniflandirici -- dogrudan siralama ogrenir.

Esik her model icin D6'da AYRI secilir. Isaret duzeltmesi HER kolda acik.
"""
import collections
import json
import os
...
def main():
    S = K.step_haritasi()
    tr = kume("tam")
    dev = kume("d6", mesh=True)
    te = kume("d7", mesh=True)
    M = np.vstack([wire_gate.parca_ici(d["X"], "zskor") for d in tr])
    Y = np.concatenate([d["y"] for d in tr])
    print(f"egitim {M.shape} pozitif {Y.mean():.4f} | D6 {len(dev)} | "
          f"D7 {len(te)}\n", flush=True)
    sonuc = {}
    for kip in ("duz", "parca_esit", "poz_dengeli", "karma"):
        ...
        sonuc[kip] = dict(r7, esik=e)
        print(f"{kip:<12} esik {e:.2f} -> D7 robot **{r7['robot']:.4f}** | "
              f"tespit {r7['tespit']:.4f} | makro {r7['makro']:.4f}", flush=True)

    # --- PAIRWISE: parca icinde (poz - neg) fark vektorleri
    A, B = [], []
    for d in tr:
        Md = wire_gate.parca_ici(d["X"], "zskor")
        pi = np.where(d["y"] == 1)[0]
        ni = np.where(d["y"] == 0)[0]
    ...
    def sk_pair(X):
        """Siralama skoru: adayin PARCA ORTALAMASINA gore ustunlugu."""
        Md = wire_gate.parca_ici(X, "zskor")
    ...
    sonuc["pairwise"] = dict(r7, esik=e)
    print(f"{'pairwise':<12} esik {e:.2f} -> D7 robot **{r7['robot']:.4f}** | "
          f"tespit {r7['tespit']:.4f} | makro {r7['makro']:.4f}", flush=True)

    iyi = max(sonuc, key=lambda k: sonuc[k]["robot"])
    print(f"\nEN IYI: {iyi} {sonuc[iyi]['robot']:.4f} | taban (duz) "
    ...
    json.dump({"damga": makbuz_hash.damga(), "sonuc": sonuc, "en_iyi": iyi,
               "not": "Tek degisken: OGRENME HEDEFI (agirlik / pairwise). Esik "
                      ...
                      "acik. D7 marka-disi, TAM ZINCIR, MIKRO."},
              open("results/a3_siralama.json", "w"), indent=1)
    print("makbuz -> results/a3_siralama.json")
