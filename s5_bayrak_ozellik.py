# -*- coding: utf-8 -*-
"""S5: FIZIKSEL BAYRAKLARI ONARIM TETIGI DEGIL, GATE OZNITELIGI OLARAK KULLAN.

S3/S4 SONUCU: onarim fiziksel kusuru yariya indiriyor (%18.7 -> %10.2) AMA metrik
kayitsiz (tespit +0.0016, robot -0.0053) ve dort triyajin dordu de kill'i gecemedi.
SEBEP OLCULDU: uretici CP'lerinin ~%47'si GOVDENIN ICINDE; noktayi agza tasimak
fiziksel gecerlilik kazandirirken GT'den UZAKLASTIRIYOR.

S0 HARITASI BASKA BIR KULLANIM ONERIYOR:
    bayrak yok  -> FP orani %21.8
    1 bayrak    -> %49.1
    2 bayrak    -> %43.8
    3 bayrak    -> %44.4
Herhangi bir bayrak FP olasiligini IKI KATINA cikariyor (sayisi artinca artmiyor:
dereceli degil IKILI sinyal). Bayraklari NOKTAYI OYNATMAK icin degil, gate'e
"bu aday fiziksel olarak suphelidir" demek icin kullanmak o celiskiye GIRMEZ.

Bu betik grup-capraz bir FIZIBILITE olcumudur (dagitim DEGIL): olcum kumesinin kendi
satirlariyla, gate'e 3 bayrak eklemenin tespit F1'ine etkisi. Gecerse egitim korpusunda
bayrak uretilip (yaklasik 1700 parca, ~40 dk mesh isi) gercek gate yeniden egitilir.

TEZ-NOTR: bayraklar aday duzeyinde FIZIKSEL olculerdir; ag, remesh ve v_o degismez.
KILL: tespit >= +0.010 VE uretici-disi dusmuyor.
"""
import io
import json
import os
...
def main():
    ...
    # --- satirlari kur: gate ozellikleri + 3 fiziksel bayrak + etiket (GT'ye yakin mi)
    X0, X1, Y, GG, KEY = [], [], [], [], []
    for r in DER:
        ...
        Xk = X[k]
        # etiket: bu aday GT'ye TESPIT toleransinda mi
        if len(G):
            diff = P[:, None, :] - G[None, :, :]
    ...
    print(f"satir {len(Y)} | iyi aday %{100*Y.mean():.1f} | sutun {X0.shape[1]} -> {X1.shape[1]}")

    ug = np.unique(GG); rng = np.random.RandomState(0); rng.shuffle(ug)
    kat = {g: j % 5 for j, g in enumerate(ug)}
    kk = np.array([kat[g] for g in GG])
    OOF = {}
    for ad, M in (("TABAN", X0), ("+BAYRAK", X1)):
        ...
        for f_ in range(5):
            ...
            for pid in {k[0] for k in KEY}:
                ...
                Z[idx] = wire_gate.parca_ici(M[idx], "zskor")
    ...
    def kos(ad):
        ...
        for r in DER:
            ...
            m = wire_gate.karar_maskesi(sk)
            tp, fp, fn, _ = esle_detay(P[m], D[m], G, Gd, r["diag"], 0.0, 180.0, True)
    ...
    gecti = (b - a) >= 0.010
    print(f"KILL: tespit >= +0.010 -> {'GECTI -- egitim korpusunda bayrak uretilir' if gecti else 'GECMEDI'}")
    ...
    print("makbuz -> results/s5_bayrak_ozellik.json")
