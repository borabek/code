...
COZUM: seffafliga never guvenme. Her CP for govdenin ICINDEN GECEN and IKI UCTAN TASAN a IGNE ciz.
Hangi taraf disarida olursa olsun gorunur; goruntuleyiciden bagimsiz.

RENKLER:  YESIL igne = manufacturer CP (ground truth)
          KIRMIZI igne = robotun bulduğu
          MAVI ince line = eslesme (correct bulunmus)
...
from viz_contract import (RGBA ,RECEIPT_VERSION ,receipt_path ,# CIZIM SOZLESMESI (see .md)
...
def greedy_match (P ,G ,Gd ,tol ,axis_tol =40.0 ):
    """big_arbiter with AYNI eslesme: each GT and each prediction EN FAZLA BIR KEZ is used."""
    pairs =[]
...
def pin (p ,d ,L ,rad ,col ):
    """Noktadan DISARI correct single tarafli igne: baseline noktada, three govdenin outside."""
    ...
    c .apply_translation (np .asarray (p ,float )+d *L /2.0 )# baseline p'de, three p+d*L
...
def cone (p ,d ,L ,rad ,col ):
    """p'den d yonunde L boyunda konik ok ucu (baseline p'de, sivri three p+d*L)."""
    d =np .asarray (d ,float );n =np .linalg .norm (d )
    d =d /n if n >1e-9 else np .array ([0.0 ,0.0 ,1.0 ])
...
def surface_along (body ,p ,d ):
    """CP govdenin ICINDEYSE d yonunde ILK surface kesisimine tasi. Disaridaysa OYNATMA.
    ...
    if not len (h ):
        return p 
    t =float (h [0 ])
    # SINIR: a acikligin agzi CP'den birkac mm otededir. Ilk surface bundan uzaktaysa that direction this
    # CP'ye ait degildir (direction wrong cikmis) -- isaretciyi OYNATMA. Bu kap olmadan, uzun axis
    # along serbest a kanala bakan isaretciler still 20-89mm firliyordu (measured: 26/259).
    return p +d *t if t <=MAX_MARKER_OFFSET_MM else p 


def escape_dir (body ,p ,d ,arrow_len ):
    """Isaretcinin GERCEKTEN disari cikabildigi yonu sec: before d, olmazsa -d.

    WHY: outward_along_axis a ISARET seciyor and this sign sometimes ters cikiyor. Sonuc, okun
    govdenin ICINDE kalmasi (measured 2026-07-29: 4/19 three iceride, 5 uctan ileride malzeme).
    Gorunmeyen a isaretci, wrong yere firlamis a isaretci up to kotudur.

    Cizim here own kendinin testi becomes: a direction secilir, ucun body outside kalip kalmadigi
    OLCULUR, kalmiyorsa ters direction denenir. Ikisi de olmazsa d korunur (most azindan konum correct
    kalir) -- and denetci bunu M3/M5 with already yakalar, sessizce gecmez.
    """
    p =np .asarray (p ,float );d =np .asarray (d ,float )
    for cand in (d ,-d ):
...
def visualize (pid ,models =None ,dev =None ,mode ="robot_only"):
    ...
    try :
        if jf is None :
            raise _NoReference # above already acikca yazildi, tekrar basma
        ...
        if len (G ):
            ...
            Gm =(G -t )@R ;Gdm =Gd @R # manufacturer CP -> mesh frame
    except _NoReference :
        pass 
    ...
    if mode =="compare"and len (Gm ):
        ...
        for i in range (len (P )):
            c =cps [i ]if cps else {}
            # Robotun gercekten ihtiyaci which is full kayit: nerede, hangi yone, hangi genislige,
            # ne up to derine, and otonom mu otherwise insana mi sorulacak.
            ...
            # Igne, deligin GERCEK ekseni along disari bakar (see cp_geometry.outward_along_axis).
            # Boy olculu tutulur: parcadan uzun igne okunmaz hale getiriyordu; gerekirse below buyutulur.
    ext =V .max (0 )-V .min (0 )
    ...
    # ROBOT-ONLY: manufacturer isaretcisi CIZILMEZ. Karsilastirma renkleri only teshis modunda.
    draw_gt =(mode =="compare")
    # GT'yi YUVADAN AGIZA tasi (otherwise isaretci kati inside kalir, insan 'hole absent' gorur)
    Gmouth =[]
    if draw_gt :
    ...
    # DISARI YONU: each isaretci for BIR KEZ, real govdeye bakarak (bbox with DEGIL -- kutu testi
    # noktanin parcadaki instead of according to isareti ters cevirip igneleri saga-sola savuruyordu).
    Gout =[outward_along_axis (body ,g ,gd )for g ,gd in zip (Gmouth ,Gdm )]
    Pout =[outward_along_axis (body ,p_ ,d_ )for p_ ,d_ in zip (P ,Pd )]

    # IGNE BOYU: prediction absent -- each isaretci for gereken uzunluk OLCULUR (see exit_length).
    ...
    def add_arrow (start ,od ,role ,index ,shaft_scale =1.0 ):
        ...
        if is_inside (body ,s +od *L )or len (ray_hits (body ,s +od *L ,od ,_reach (body ,s )))>0 :
            ...
            # YON GUVENILIR MI: uzattiktan after bile three govdede kaliyor ya da hemen onunde malzeme
            # varsa, this CP'nin ekseni belirlenememis demektir. O zaman OK CIZILMEZ -- only kure.
            # Guvenmedigimiz a yonu robota gostermektense, "konumu biliyoruz, yonu bilmiyoruz"
            # demek dogrudur. Denetci bunu makbuzdaki 'arrow' alanindan bilir and that isaretciye
            # M3/M5 uygulamaz; silent gecis not, OPEN beyandir.
        tip_ =s +od *L 
        ...
        # KURE = CP'nin kendisi (robotun gidecegi point), okun TABANINDA.
        # Ok yonu gosterir, kure YERI gosterir; ikisi ayri bilgidir and ikisi de gerekli.
    ...
    # robot_only: TEK renk (robotun CP'si). compare: correct/fazladan ayrimi.
    ...
    # AYNI PARCANIN ESKI CIZIMLERINI SIL. Dosya adi CP sayisini/F1'i tasidigi for number degisince
    # new file YENI ada yazilir and eskisi klasorde KALIR -- 2026-07-29'da full this became: ok yonu
    # duzeltilince CP count degisti and old, bozuk okli GLB'ler results inside durdu.
    ...
    # --- RECEIPT: sozlesmenin denetlenebilir kaydi (see CIZIM_SOZLESMESI.md) ---
    # CIZILEN numbers yazilir: robot_only'de manufacturer isaretcisi and line YOKTUR -> 0.
...
if __name__ =="__main__":
    ...
    for pid in ids :
        ...
        # Hata YUTULMAZ: part atlanir but sayilir and cikis kodu bunu yansitir. Sessiz basarisizlik
        # (2026-07-28 dersi) a more "each sey yolunda" gorunemez.
            n_fail +=1 
    ...
    if mode =="robot_only":
        ...
        # cagrildiginda (internetten inen a STEP'i denemenin most dogal yolu this) no GLB
        # dogrulanmadan uretiliyordu. Cizen kod kendini dogrulayamaz -- denetci here,
        # cizimden after, dosyalari DISKTEN geri okuyarak runs.
