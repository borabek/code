...
def segment_regions(V, F, cav=None, min_faces=25, cav_thr=0.55):
    """Over-segment into units a human would click. Dihedral flood-fill alone FAILS on these meshes
...
def render(V, F, reg, cav, out, title):
    ...
    is seen flat-on, plus the two edge faces (a terminal block is a thin slice: the wire openings
    live on the edges/recesses, so both matter)."""
    nreg = reg.max() + 1
