# -*- coding: utf-8 -*-
"""SECICI VERIMI: genisletilmis havuzda hangi EK oznitelik ise yariyor?

Havuz acildi (robot tavani 0.5748) ama secici tavanin ~%35'ini yakaliyor. Bu
sonda, YENIDEN HESAP GEREKTIRMEYEN ek oznitelikleri onbellekli veriyle dener:
  K) KAYNAK   -- aday segmentasyondan mi B-rep'ten mi geldi (1 sutun)
  Y) YOGUNLUK -- en yakin komsuya uzaklik, 5/10mm yaricapta komsu sayisi (3)
  M) MERKEZ   -- parca kutusuna gore konum + eksenlere hizalanma (3)
Egitim D6 (468), sinav D7 (835) -- marka kumeleri AYRIK. Tek degisken, ayni
esik taramasi, ayni RF ayarlari.

UYARI: bu D6-olcekli bir kiyas; mutlak degerler tam olcekli gate'ten DUSUK olur.
Aranan sey MUTLAK DEGER DEGIL, hangi oznitelik grubunun ISE YARADIGI.
"""
...
k6 = d6_kayit.yukle({d["pid"] for d in tr})
for d in tr:
    d["mfg"] = k6[d["pid"]]["mfg"]
...
sonuc = {}
for ad, kul in (("taban (58)", ""), ("+KAYNAK", "K"), ("+YOGUNLUK", "Y"),
                ("+MERKEZ", "M"), ("+HEPSI", "KYM")):
    ...
    for e in (0.20, 0.30, 0.40, 0.50):
        ...
        for d in te:
            ...
            tes.append((len(d["G"]),) + esle_macar(P, D, d["G"], d["Gd"],
        ...
        r = {"esik": e, "robot": mi, "tespit": K.mikro(tes),
    ...
    print(f"{ad:<12} sutun {X.shape[1]:>3} | robot {en['robot']:.4f} | "
          f"tespit {en['tespit']:.4f} | makro {en['makro']:.4f} | esik {en['esik']:.2f}",
          flush=True)
t = sonuc["taban (58)"]["robot"]
print()
for ad in sonuc:
    if ad != "taban (58)":
        print(f"  {ad:<12} {sonuc[ad]['robot']-t:+.4f}")
json.dump({"damga": makbuz_hash.damga(), "sonuc": sonuc,
           "not": "D6-olcekli kiyas (egitim 468). Mutlak degerler tam olcekten "
                  "DUSUK; aranan hangi oznitelik grubunun ise yaradigi. D7 marka-disi."},
