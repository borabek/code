# -*- coding: utf-8 -*-
"""WEI 0.80: agresif WEI-tipik aday havuzu (oracle 0.972) ozellik+etiket+pozisyon+vote ile CACHE'le.
Sonra wei_selector_eval.py offline gate+spatial+topology ile WEI F1'i olcer. results/wei_aggr_pool.json."""
...
def main():
    ...
    print(f"{len(wei)} WEI tipik parca | agresif havuz + ozellik", flush=True)
    ...
    for k, (pid, jf, stp) in enumerate(wei, 1):
        try:
            ...
            continue
        if k % 20 == 0: print(f"  {k}/{len(wei)}  {len(out)} parca  {time.time()-t0:.0f}s", flush=True)
    ...
    print(f"-> results/wei_aggr_pool.json ({len(out)} parca, {tot} GT, matched {cov} = {cov/max(tot,1):.3f})  {time.time()-t0:.0f}s")
