# -*- coding: utf-8 -*-
"""Yeni manset: 18-sutunlu gate, URUN birlestiricisi, DEV+VAL havuzlanmis."""
import os, sys, json, pickle
import numpy as np
os.environ.setdefault("BA_ALLOW_SEEN", "1"); os.environ["WG_FIZ_FEATS"] = "1"
os.environ["MERGE"] = "urun"
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from sina_kume import f1w, pr

# q6 kosulari parca-basina sayimlari yazmiyordu; makbuzlardan HAVUZLANMIS deger
# tam olarak hesaplanamaz -> iki kumenin agirlikli ortalamasi (esit n) alinir ve
# GA'lar icin q6 makbuzlarindaki kume-ici degerler raporlanir.
d = {k: json.load(open(f"results/q6_fiz_uctan_uca_{k}.json")) for k in ("dev", "val")}
print(f"{'kol':<22}{'DEV tespit':>12}{'VAL tespit':>12}{'ort':>9}"
      f"{'DEV robot':>11}{'VAL robot':>11}{'ort':>9}")
for tag, lab in (("rt2 13", "onceki (13 sutun)"), ("fiz18", "YENI (18 sutun)")):
    dt, vt = d["dev"]["tespit"][tag], d["val"]["tespit"][tag]
    dr, vr = d["dev"]["robot"][tag], d["val"]["robot"][tag]
    print(f"{lab:<22}{dt:>12.4f}{vt:>12.4f}{(dt+vt)/2:>9.4f}"
          f"{dr:>11.4f}{vr:>11.4f}{(dr+vr)/2:>9.4f}")
o = {}
for m in ("tespit", "robot"):
    a = (d["dev"][m]["rt2 13"] + d["val"][m]["rt2 13"]) / 2
    b = (d["dev"][m]["fiz18"] + d["val"][m]["fiz18"]) / 2
    o[m] = {"onceki": a, "yeni": b, "fark": b - a}
    print(f"\n{m}: {a:.4f} -> {b:.4f}  ({b-a:+.4f})")
o["bootstrap_dev"] = d["dev"]["bootstrap"]; o["bootstrap_val"] = d["val"]["bootstrap"]
o["kesinlik_yeni"] = {k: d[k]["kesinlik"]["fiz18"] for k in ("dev", "val")}
o["recall_yeni"] = {k: d[k]["recall"]["fiz18"] for k in ("dev", "val")}
json.dump(o, open("results/manset_fiz.json", "w"), indent=1)
print("\nmakbuz -> results/manset_fiz.json")
