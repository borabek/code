# GECE PLANI — unseen brand robot F1 0.80 hedefi (2026-08-11 gecesi)
...
| kaldirac | measured_path | durum |
|---|---|---|
| mesh probability esigi 0.50 -> 0.05 | CWT konum 0.5532 -> **0.7914** | hazir, corpus cikarimi gerek |
...
*Bedel:* option/part ~2500 -> ~6000. Rejim kapisi + kaskad bunu yonetmek
...
prediction et; secimi tam o sayida kes. Esikten count karari KOTUYDU but this
YAPISAL a prediction, ogrenilmis not.

**B4. Parca-ici feat-calibration (transduktif).** Parcanin kendi en guvenli
tespitlerinden yerel a siniflandirici kur, remaining adaylari ona per yeniden
...
**GATE C:** each blok ayri olculur; +0.01 altinda remaining bloк ATILIR (oznitelik
...
**D3. KURESAL COZUMLEME (Sutun 3).** Adim/axis tahmini + izgara/count
...
parametrik (sira x adim x yaricap x body). CP'leri kurulustan known on
...
B4 feat-calibration                      ~30 dk  GATE B
C1 depth profili                    ~1 sa   GATE C
B5 symmetry                             ~40 dk  GATE B
...
morning rapor edilir ve okuma harcanmaz.
...
4. Sisik number YOK: gelistirme kumesinden read kazanc baseline gucuyle birlikte
   yazilir.
