...
`docs/SUNUM_PAKETI.md` sunuma gidebilir. Cekirdek sayilar (VAL 100 parca):
**tespit 0.7878** [0.7464-0.8263] · robot-eksen 0.5764 ·
**robot-ISARETLI 0.4839** [0.4003-0.5671] · **alan farki +0.2468**.
Asagidaki yarim kalan olcumlerin hicbiri bu sayilari degistirmez;
yalnizca "bir kol daha" ekleyebilir.

## DENETIM
* `duman_testi.py` **GECTI** · `geri_al.py --kontrol` -> **1 bilincli
  sapma** (`cp_config.json`)
...
Hepsi **egitim gerektirmez**, yalnizca cikarim. Her biri ~25-50 dk.

### 1) ~~CRF dorduncu ayar (10 tur / 0.9)~~ — **IPTAL EDILDI**
Kol monotoniklik gerekcesiyle KAPANDI (asagidaki tabloya bak); dorduncu
ayar hukmu degistirmez. Kosu suresi topluluk kollarina (2 ve 3) ayrildi.
Komut, kol ileride yeniden acilirsa kullanilmak uzere duruyor:
```bash
...
.venv/Scripts/python.exe sonda_zincir_esli_kiyas.py > logs/val_crf_t10w09.log 2>&1
# sonra:
.venv/Scripts/python.exe sonda_dokum_kiyas.py \
...
.venv/Scripts/python.exe sonda_zincir_esli_kiyas.py > logs/val_top5.log 2>&1
# sonra:
.venv/Scripts/python.exe sonda_dokum_kiyas.py \
...
.venv/Scripts/python.exe sonda_zincir_esli_kiyas.py > logs/val_top8.log 2>&1
```

### 4) ~~KAZANAN CRF AYARI SAHA YOLUNDA~~ — GEREKSIZ (kol kapandi)
Asagidaki komut, ileride kol yeniden acilirsa kullanilmak uzere duruyor.
...
.venv/Scripts/python.exe sonda_zincir_esli_kiyas.py > logs/val_crf_saha.log 2>&1
...
yani **tepe yapip donuyor: MONOTONIK DEGIL**. Bu, kampanyada halka-isaret
esik taramasinda da gorulen "gurultuye uydurma" imzasidir. Tepe degeri
...
## HATIRLATMA — bu gece ogrenilen dort olcum kurali
1. Segmentasyon **tohum gurultusu 0.046**; altindaki hicbir seg kolu tek
   kosuyla hukum giymez.
2. Sonda kancalari yalniz **`olculen`** yoluna ulasir; `saha`da +0.0000
   gormek kolun olu oldugu anlamina GELMEZ.
3. Ara degerler **uretildikleri yerden** yakalanmali (onbellek).
