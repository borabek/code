...
cikardi. Daha very secenek = more very CELDIRICI. Verimlilik OLDUGU YERDE
...
EGITIMDEDIR: negatifler `NEG_KAT=6` with rastgele orneklenıyor ve secenek
...
Tavan-24 with direction kaybi NIT'te SIFIRLANDI; kalan loss saf konum. Mesh esigi,
...
2. AUTO katmani: gorulmemis markada REVIEW BOS ve precision 0.3471. Kalibre a
   skor cikana up to AUTO **kapatilmali**.
...
| S1 budama | S0'dan after | recall kaybi <= 0.01, secenek -%40 | ceiling-24 ertelenir |
...
**Yeni birinci soru:** COKEN MARKA with CALISAN MARKA arasindaki fark NE?
...
rule secimi, GT'nin havuzdaki skor sirasi. Kapi none -- this a DIAGNOSIS, arm
...
**NIT'te correct secenek yanlistan yalnizca 0.05 more high puan aliyor.**
Model orada pratikte ayrim YAPMIYOR. Havuz cevabi tasiyor (0.898) but skor
onu gostermiyor.

## Yogunluk REASON not, CARPAN

`sira%` hepsinde very small (0.001-0.016): correct secenek NIT'te bile ilk ~40
...
- **S1 (budama) this sorunu COZMEZ.** Dogru secenek already ilk ~40'ta; celdirici
...
- **YENI S8: MARKA-ICI SKOR NORMALIZASYONU.** `skor araligi` NIT'te 0.651,
  ...
  (%85 x part-maks) this olcege duyarli. Parca-ici z-skor / yuzdelik
...
kullaniyor gorunuyor; fark aciklanmadi. Ihtimaller: (a) `yukle` with
...
IYILESTIRMIYOR, belirgin sekilde BOZUYOR. Muhtemel reason: mutlak skor
duzeyinin kendisi bilgi tasiyor (kolay part = high skor) ve normalizasyon
...
- **S4 cokmeyi cozerse:** **0.45-0.50 mumkun.** Aritmetigi: D7'de CWT tek
  ...
  calisan seviyeye cikarsa mikro F1 kollarin TOPLAMIYLA not SICRAYARAK gelir.
...
yolun hangisi oldugunu gosteren a DIAGNOSIS present (S7: poz-neg skor ayrimi).
...
for negatifler kaybi boguyor. Bu **adalet kusuru**, metrige per ayar not.
...
| kalan bloklar (simetri/depth/kafes_adet) | ambiguous, tarihsel baseline ratio low |

**Beklenen varis: 0.34-0.38.** 0.50'ye this yoldan ULASILAMAZ ve bunu soylemek,
ulasilacakmis gibi davranmaktan iyidir. O noktada secenekler: (a) more fazla
ve more CESITLI veri, (b) different a problem kurgusu (for example dogrudan
tepe-basi ag), (c) hedefi olculen gercege per revize etmek.
