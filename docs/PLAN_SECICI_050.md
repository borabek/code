...
**Bugunku gercek sayi:** D7 sinavi **0.3115**, `tam` brand katlari **0.3091**
(baseline) / **0.3195** (B1, henuz tek degiskenli degil). Hedef **0.50**.

## Aritmetik -- neyi degistirmemiz gerekiyor

F1 ~ (selector verimliligi) x (pool tavani).

| | bugun | ceiling-24 sonrasi | 0.50 icin gereken |
|---|---|---|---|
...
**Yeni risk:** ceiling-24 secenek sayisini part basina 3217 -> 4857 (1.51x)
cikardi. Daha cok secenek = daha cok CELDIRICI. Verimlilik OLDUGU YERDE
KALIRSA ceiling kazanci F1'e tam donmez; bu yuzden asagidaki S0 olcumu once
gelir.
...
`kos_tavan_uctan_uca.sh`, d6 brand katlari, tek degisken corpus.

- **Cikti:** ceiling 12 vs 24 uctan uca robot F1 farki.
- **Karar:** fark **>= +0.02** ise ceiling-24 urune alinir ve `tam` korpusunda
  tekrarlanir. **< 0** ise celdirici etkisi kazanci yiyor demektir; o zaman
  once **S1 (budama)** gelir, ceiling ondan sonra acilir.
- d6 GELISTIRME kumesidir; sayi YON gosterir, headline degildir.

## S1. SECENEK BUDAMA (yeni, ceiling-24'un dogal esi)

Tavan-24 dogru yonu havuza sokuyor ama yaninda ~1600 ek celdirici getiriyor.
Iki kademeli budama: UCUZ bir on-siralayici secenekleri part basina ~1200'e
indirsin, pahali selector yalniz onlari gorsun.

- **Kapi:** budama sonrasi YONLU recall kaybi **<= 0.01** iken secenek sayisi
  **>= %40** azalmali.
- **Neden simdi:** bu arm ceiling-24 OLMADAN anlamsizdi; celdirici sorunu yeni
  dogdu.

**Budamanin F1'e hangi YOLDAN dokundugu onemli.** Karar kurali GORELI
(part-maksimumunun %85'i), yani DUSUK skorlu secenekleri atmak secimi
...
ekseni), `cluster` (candidates arasi rekabet), `topoloji` (es-eksenli aile),
`ozkalib`, `kafes_adet`, `simetri`, `depth`.
...
- **`cluster` ayrica bir SONDADIR:** kazandirirsa S4 (candidate-kumesi modeli)
  gerekcelenir, kazandirmazsa o kolun hipotezi zayiflar.

## S3. ZOR NEGATIF (B1) DOGRULAMASI

B1 +0.0104 verdi ama corpus VE yontem birlikte degisti. `tam3` TABAN kosusu
kuyrukta; gelince:
...
Bugunku selector NOKTASAL: her secenegi tek basina puanliyor. Karar kurali ise
GORELI. Aradaki bosluk yapisaldir ve oznitelik eklemekle kapanmaz.

- Adaylar uzerinde dikkat (set transformer): part icindeki tum secenekler
  BIRLIKTE puanlanir.
- **Kapi:** LOMO'da HGB'ye **+0.05**. Alti kalirsa arm kapanir.
- **On kosul:** S2'deki `cluster` blogu pozitif olmali (ucuz probe).

## S5. KONUM KAYBI (%10.1) -- ceiling-24 sonrasi kalan tek pool acigi

Tavan-24 ile direction kaybi NIT'te SIFIRLANDI; kalan loss saf konum. Mesh esigi,
seyreltme yaricapi ve candidate ust siniri bu kayba gore YENIDEN taranmali
(bugunku ayarlar direction darbogazi varken secilmisti).
...
ZORUNLU kiliyor:** 458 candidate x ~17 secenek = part basina ~7800 secenek
(bugun 4857). Budama olmadan bu arm celdirici sayisini patlatir.
...
bu kampanyada "kapali arm aslinda sondanin kusuruydu" hatasi bir kez yasandi.
...
2. AUTO katmani: gorulmemis markada REVIEW BOS ve precision 0.3471. Kalibre bir
...
|---|---|---|---|
| S0 ceiling uctan uca | KOSUYOR | >= +0.02 | once S1 |
| S1 budama | S0'dan sonra | recall kaybi <= 0.01, secenek -%40 | ceiling-24 ertelenir |
| S2 EK bloklari | KOSUYOR | +0.01/blok | blok atilir |
| S3 zor negatif | baseline gelince | >= +0.01 | atilir |
| S4 candidate-kumesi | `cluster` pozitifse | +0.05 | arm kapanir |
...
zor negatif (~+0.01) = **~0.36-0.40**. **0.50'ye S4 (candidate-kumesi modeli)
...
`results/_gece/UCTAN_UCA.log` · d6 brand katlari, tek degisken corpus.
...
(budama), ceiling ondan sonra**.

**KAYIT:** bu deney d6-only LOMO'dur; her fold yalnizca ~400 part uzerinde
egitiliyor (gercek kurulumda 3051). Zayif model ek celdiricilerden DAHA COK
zarar gorur, yani -0.0092 ceiling-24 aleyhine YANLI olabilir. Kol
KAPATILMIYOR; `tam` korpusunda tekrarlanacak (S0b).

## ASIL BULGU: selector verimliligi IKI KUTUPLU
...
NIT'te pool cevabi TASIYOR (ceiling 0.9147) ama selector bulamiyor.

**Ayni imza baska kumelerde de var:** `tam` katlarinda TOGI 0.1689 (recall
0.1054, precision 0.4240 -- yani "az ama dogru"), D7'de CWT 0.0466. Yani bu,
d6'nin kucuk training kumesine bagli bir artefakt DEGIL; sistemin sureklilesen
kor noktasi.
...
sistematik farki olc: part basina CP sayisi, candidate yogunlugu, skor dagilimi,
kural secimi, GT'nin havuzdaki skor sirasi. Kapi yok -- bu bir TESHIS, arm
...
| brand | CP/p | candidate/p | secenek/p | havuzda | ilk10 | ilk50 | sira% | **poz-neg** | skor araligi | verimlilik |
|---|---|---|---|---|---|---|---|---|---|---|
...
icinde. Ama NIT'te part basina **24.4 CP** var -- bir tanesini tepede bulmak
yetmez, ~24'unu bulmak gerekir. Zayif ayrim + yuksek yogunluk carpimi cokusu
uretiyor. MOR dense DEGIL (3.4) ama ayrimi da zayif (0.277) ve havuzu dusuk
...
- **S4 (candidate-kumesi modeli) DOGRU HEDEFTE:** part icinde secenekleri
...
- **YENI S8: MARKA-ICI SKOR NORMALIZASYONU.** `skor araligi` NIT'te 0.651,
  UPUN'da 0.981 -- dagilimlar brand basina farkli olcekte. Goreli kural
  (%85 x part-maks) bu olcege duyarli. Parca-ici z-skor / yuzdelik
...
MOR icin yonlu recall **0.8686** demisti. Iki measurement ayni kabul kutusunu
kullaniyor gorunuyor; fark aciklanmadi. Ihtimaller: (a) `yukle` ile
`probe_pool_tavani`in GT kayitlarini farkli okumasi, (b) axis toleransinin
yuzdelik (`0.06 x diag`) vs sabit (40mm) uygulanmasi. **Cozulmeden MOR'un
pool sayisi bu tablodan alintilanmamali.**

---

# S2 ILK SONUC: `ozkalib` COKTU (-0.0482)

Makbuz `results/ek_blok_ozkalib.json` (3051 part, `tam` brand katlari).

| blok | bloksuz | blokla | fark | karar |
|---|---|---|---|---|
| ozkalib | 0.3124 | 0.2642 | **-0.0482** | **GECMEDI** |

`ozkalib` = part-ici skor yuzdeligi + en yuksekten fark + medyandan fark.
...
duzeyinin kendisi bilgi tasiyor (kolay part = yuksek skor) ve normalizasyon
bu bilgiyi siliyor.
...
**Bes koldan dordu negatif.** Bu, projenin baseline oranina uygun: kollarin cogu
duser. Kalan bloklardan (kanonik, cluster, topoloji, kafes_adet, simetri,
...
**Bugun 0.50'yi vaat eden hicbir measurement YOK.** Elimizde olan, 0.50'ye giden tek
...
## Uc bagimsiz arm, ayni direction
...
**Muhtemel sebep:** karar kurali ZATEN GORELI (part-maksimumunun %85'i), yani
part-ici normalizasyonu KURAL yapiyor. Baglami bir kez daha vermek ya tekrar
oluyor ya da noise/asiri uyum ekliyor.

**En can sikici ayrinti:** TOGI (coken brand, S4'un asil hedefi) DeepSets ile
...
0.50 icin gereken 1.45x'lik selector sicramasinin tek yapisal adayi S4 idi.
...
| kalan bloklar (simetri/depth/kafes_adet) | belirsiz, tarihsel baseline orani dusuk |
...
Daha once "B1 +0.0104" diye raporladigim kazanc **tamamen corpus
degisikligindendi**. Tek degiskenli baseline kosusu olmasaydi bir NO-OP'u
...
(80 part, ayni seed) ZORNEG=0 ve =1 **TP/FP/FN'e kadar birebir ayni** cikti.

**DUZELTME:** skorlar her arm icin zaten var ve `egit`/`skorla` onlari yalnizca
...
baglayici degil: 80 part, 30 iterasyon.)

## Bugunun dersi -- ayni sinif error DORT kez
...
| 4 | `P6_ZORNEG` bayragi okundu ama kosul hep False | tek degiskenli baseline kosusu |
