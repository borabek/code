...
- MOR'da ~9 CP for **11.555 candidate** present. AUC 0.9657 olsa bile beklenen
  ...
  olculen median son-correct sirasi **624**. Ayni mertebe.
...
`n_aday` **konum** sayisi not **secenek** sayisidir. Her konum
...
denemeden birinde high skor kapma olasiligi, correct konumun tek gercek
...
2. **Havuz tavani "iyi" gorunuyordu** (0.843). O ceiling KONUM tavaniydi;
   secenek/GT ratio no ceiling as okunmadi.
...
```
  ...
  say     : skoru 0.5 ustunde which secenek ORANI
  maxxort : max x mean
  ust2    : en high IKI secenegin ortalamasi
```

Gerekce: **wrong** konumda 24 skor rastgele dagilir (high max, low
median); **correct** konumda bircok secenek makul skor alir (high median).
