...
Tarih: 2026-08-13 · Olcut: bu projede **olculmus** kanit.
Hukum uc turlu: **OLCULDU-DUSTU** · **ZATEN VAR** · **CANLI (denenmedi)**

---

## 1. OLCULDU ve DUSTU — yeniden acmak icin YENI gerekce sart

| madde | bu projede olculen | makbuz |
|---|---|---|
...
| Veri kaynagi genisletme | stok BITTI: WSCAD olu, SIE'de yeni yalniz 22 parca | `wscad-data-lever-dead` |
| Noise injection / B9 augmentation | KAPANDI | `g10-sig-boyama-gecti` |
| Threshold optimizasyonu | ZATEN yapiliyor (kat icinde kural aramasi) | `kos_p6_kademe2` |
| Feature engineering | 25+ blok olculdu, biri bile uretimde kalmadi | Bolum 19 |
...
|---|---|
| Failure autopsy | hata bankasi + FN taksonomisi + `OTOPSI_YOGUN_PARCA.md` |
| K-fold CV | marka-disi katlar (her olcumde) |
...
| Robust scaling | parca-ici z-skor DAGITILDI |
...
| Uncertainty | vote sayisi = topluluğun kendi guven sinyali |
...
2. **Yogun-parca UZMANI (specialist)** — NIT tipi parcalar GT'nin %51'i ve
   uctan uca 0.0089. Ayri egitilmis bir uzman + yonlendirici hic denenmedi.
...
4. **Snapshot ensembling (tek kosudan)** — cosine restart + epoch
   toplulugu. Mevcut topluluk AYRI kosulardan; bu ayri bir sey.
...
9. **Oznitelik secimi / boyut indirgeme** — 100+ sutun var, sistematik
   eleme yapilmadi.
10. **Self-supervised / contrastive on-egitim** — pahali, ama sentetik
...
| MLflow / W&B | makbuz + damga sistemi ayni isi goruyor |

---

## HUKUM

Listenin **%60'i bu projede zaten olculdu ve dustu ya da zaten var**.
...
- **yogun-parca uzmani** (NIT = GT'nin %51'i, uctan uca 0.0089)
...
| madde | hukum | kanit |
|---|---|---|
| Probabilistic calibration (Platt/isotonic) | **DUSTU** | onceki listede olculdu |
...
| Per-class threshold sweep | **ZATEN VAR** (kat icinde kural aramasi) | `kos_p6_kademe2` |
...
|---|---|---|
| Y13 | **Sinir-farkindali kayip** (edge/contour IoU) | CP fiziksel olarak bir SINIR (delik agiz cemberi); kayip bunu hic hedeflemiyor |
| Y14 | **Focal-Tversky** | Tversky ACIK ama focal varyanti denenmedi |
| Y15 | Tepe duzeyi esik taramasi (segmentasyon) | `BH_MESH_ESIK=0.05` elle secilmis, taranmadi |
...
| Y20 | **Cok-gorunumlu render fuzyonu** (normal/derinlik render + CNN) | TAMAMEN YENI kiplik; mevcut hicbir kol goruntu sinyali kullanmiyor |
...
| Y27 | Hiyerarsik parca duzeyi cozucu | Y2 uzman koluyla sinerjik |
...
2. **Y13** — CP bir SINIRDIR (agiz cemberi) ve kayip bunu hedeflemiyor.
