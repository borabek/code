...
| madde | this projede olculen | receipt |
|---|---|---|
...
| Sentetik veri (rule tabanli) | uretec calisiyor, **segmentasyon sentetikte ateslemiyor (0.92x)** | `sentetik_duman` |
...
| Robust scaling | part-ici z-skor DEPLOYED |
...
| Seed / repro | `makbuz_hash.damga()` each makbuzda |
...
1. **SEG augmentasyonu -- YALNIZ HAFIF OLANI.** *(DUZELTME 2026-08-13:
   ...
   ~ 60 derece) denendi ve KALITEYI DUSURDU. Denenmemis which HAFIF ayar
...
10. **Self-supervised / contrastive on-training** — pahali, but sentetik
    dustukten after "temsil" yolunun kalan tek adayi.
...
| Tversky loss | **ZATEN OPEN** | `--tversky` varsayilan 0.25, `alpha 0.3 / beta 0.7` (satir 179, 317) |
