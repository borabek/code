...
> Dogrudan measurement tersini soyledi (`results/position_auc_d6.json`):
>
...
> kucultme) ve **CEPHE D** (data); CEPHE A ve B closed.
>
> Ders: option sayisindan konum/direction payini TAHMIN ETME, OLC.
> Gecerli teshis: `docs/autopsy_dense_part.md` + [[konum-vs-direction-ayrimi]].



Tarih: 2026-08-12 · Hedef: unseen brand **robot F1 = 0.75**
...
`docs/autopsy_dense_part.md` — candidate duzeyinde measured:
...
```
  NIT:  6322 option  =  ~260 KONUM  ×  24 YON        (MAX_SEC = 24)
        konum/GT = 11:1   IYI
        option/GT = 263:1  YONETILEMEZ
```
...
Bugun measured_path 22 kolun 20'si skoru iyilestirmeye calisiyordu. Yanlis cephe.
...
|---|---|---|---|
| **(A) BULUNURLUK** | correct direction 24 option arasinda present mi | **0.593** | %41'inde HIC YOK |
...
kucuur**, because option sayisi duser.
...
alindi (24 option uret, skorla). Bir OLCUM problemi oldugu no denenmedi.
...
unsigned sec, isareti "govdeden disari"dan derive (GT sozlesmesi 1.000 disari).
...
**Kapi:** +0.03 mikro. En large tek data kaldiraci.

### D2 SIE +310 PARCA
Ucuncu manufacturer stogunda kullanilabilir tek brand (A-B/KLM/CWT/EFX = D7 exam
markalari, DOKUNULMAZ). Olculmus data kaldiraci: +110 WEI → +0.0391.
**Kapi:** +0.01.
...
- **E2 GLB dogrulamasi** — `product_p6`/`product_wide` sahaya girmiyor
  (`glb-measured_path-zinciri-kullanmiyor`). Kazanc olculse bile sahaya inmez.
- **E3 Sismis siralama altinda closed kollar** — S4, lattice yayilimi, count
...
```
  A1 ISIN EKSENI  ──gate 0.40──┬── GECERSE ──> C1 pool kucultme
       │                       │                    │
       │ dusrse                │                    v
       v                       │              E3 closed kollari YENIDEN AC
...
4. E3'te yeniden opened kollar (S4, lattice, count) sismis siralama kalkinca
   ise yarar

Bu dortlunun tamaminin tutmasi olasi degildir. **Ama A1 tutarsa denklemin
sekli ilk kez 0.75'e izin verir** — this morning izin vermiyordu. Guzergah budur;
