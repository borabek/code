...
Temmuz sayilari **farkli bir degerlendirme cercevesinde** olculdu
(hold-out, eski bolme, eski zincir). Bugunku sayilar **VAL 100 parca**
...
> = VAL 100 parca. Kesisim sifir (egitim/gelistirme/sinav).
...
| Robot-hazir, eksen (isaretsiz) | 0.5764 | [0.5067, 0.6438] |
...
| 10 | Robot-ready (2mm + 10 deg) | 41.6% | -> **isaretli 0.4839** (yeni: isaretli/isaretsiz AYRIMI) |
| 10 | weaker split (siblings shared) | 77.9% | kalabilir (sisme ornegi) |
| 10 | Ceiling with a perfect filter | 89.4% | **yeni tavan olcumu var**: 21.60 |
| 11 | Low / High CP-F1 | 61.8% / 68.2% | -> 0.7701 / 0.6583 |
| 12 | ornek parca F1'leri | 100% / 0% | yeni ornek secilebilir (10 parcalik ara test var) |
...
1. **Alan farki +0.2468** (tanidik uretici 0.5603 / gorulmemis 0.3135).
   Ayni sistem, ayni veri, yalniz bolme tipi farkli. Projenin nereye
   yatirim yapmasi gerektigini soyleyen tek sayi.

2. **Baglayici kisit tablosu** (VAL 100 / 660 CP): yonu TAMAMEN mukemmel
   yapmak kapsamayi +0.170 aciyor, yanal toleransi 2->10 mm yapmak
   **+0.358**. Yani darbogaz yon degil **yanal konum**.

Ayrica bir metodoloji sayisi: ayni recete, yalniz tohum farkli ->
...
Bunlar bugun olculdu ve sunumda YOK:

**1. Offset basi: yon ogreniliyor, buyukluk buzuluyor.**
13 epok, 700 parca, sizinti kapisi kurulu:
* hedef offset buyuklugu 4.299 mm, tahmin 2.871 mm
* **yon uyumu kosinus +0.675** (~48 derece ortanca aci hatasi;
  rastgele 0 olurdu)
Bu, "sonraki adim" satirini tahminden OLCUME cevirir.

**2. Yogun yolda dort bayatlama onarildi.**
tespit 0.3402 -> **0.3673**, isaretsiz robot 0.1546 -> **0.2143**.
...
skor >= parca maksimumunun %50'si VE skor >= %20.
