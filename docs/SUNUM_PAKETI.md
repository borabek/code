...
2. **Darbogaz kapasite not, YENI URETICIYE TRANSFER.** Ayni sistem,
   same veri, yalnizca split tipi degistirilerek olculdugunde: tanidik
...
Robot kabloyu fiziksel as sokacaksa gecerli criterion **isaretlidir**.
...
Ayni sistem, same veri, same oznitelikler. **Tek fark: degerlendirme
...
| zor/gorulmemis brand (150) | saha | −0.0016 | small negatif |
...
**Kapi altinda kalan, but direction dogrulanan kollar** (hepsi measured, hicbiri
dagitilmadi):

| kalem | kazanc | dogrulama |
|---|---|---|
| Etiket kalitesi agirliklandirma | +0.0082 | **4 fold seed**, tek pozitif kalan |
...
Augmentasyon notu: agresif ayar (60° donme) more before denenip
**zarar verdigi** for arm "olu" sayilmisti. Hafif ayar (9–17°)
...
1. **Isaretli/unsigned karisikligi**: headline metrigi isaretsizdi; robot
   for gecerli criterion signed. Ikisi de raporlaniyor.
...
Ayni arm, same veri, same kod -- yalnizca katlarin nasil bolundugu different.
...
cikti.** Ayni recete, same veri, yalnizca seed different:
...
| 4. Self-supervised / contrastive on-training | sentetik veri dustukten after temsil yolunun kalan adayi |
