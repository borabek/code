...
for sr in (0.0 ,0.4 ,0.5 ,0.6 ):
    ...
    rec =tp /max (tp +fn ,1 )
    res_ [str (sr )]={"kahin":f1w (T ),"recall":rec ,"candidate":n_aday }
    print (f"  split_ratio={sr }: kahin {f1w (T ):.4f} | recall {rec :.4f} | candidate {n_aday }",
    flush =True )
t0 =res_ ["0.0"]
en =max ((k for k in res_ if k !="0.0"),key =lambda k :res_ [k ]["kahin"])
dk =res_ [en ]["kahin"]-t0 ["kahin"];dr =res_ [en ]["recall"]-t0 ["recall"]
...
json .dump ({"damga":makbuz_hash .damga (),"sonuc":res_ ,"en_iyi":en ,
