...
def main ():
    ...
    for k ,(pid ,jf ,stp ,n )in enumerate (hi ,1 ):
        try :
            ...
            for model ,meta in models :
                ...
                per9 .append (derive (V9 ,F9 ,np .asarray (pb ,float ),12 ,0.20 ,0.0 ))# vc 0.2 = more very candidate
            cps =_vote2 (per +per9 )
            if not cps :continue 
    ...
    print (f"-> results/highcp_pool.json ({len (out )} part) -- lattice_rerank.py offline")
