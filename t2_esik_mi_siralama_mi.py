# -*- coding: utf-8 -*-
"""T2: uretici-disi cokus SIRALAMA hatasi mi, ESIK (kalibrasyon) hatasi mi?

T1 OLCTU: 18 sutunlu gate uretici-disi bolmede F1 0.7422 -> 0.6399 / 0.2799 (ortalama -0.282).
Iki cok farkli hastalik olabilir ve tedavileri zit:
  SIRALAMA bozuksa  -> model o ureticide dogru/yanlisi ayirt EDEMIYOR; ozellik/veri isi.
  ESIK bozuksa      -> siralama iyi ama 0.40 yanlis yerde; KALIBRASYON isi (ucuz cozum).

Ayrim: AUC siralamayi olcer (esikten bagimsiz). Sabit esikteki F1 ile EN IYI esikteki F1
arasindaki fark, yalnizca kalibrasyonun bedelidir.
"""
import json
import numpy as np
...
def main():
    ...
    print(f"{'bolme':<26}{'AUC':>8}{'F1@0.40':>9}{'F1@enIyi':>10}{'esik':>8}"
    ...
    for u in sorted(set(mfg)):
        ...
        out[f"uretici{u}"] = rapor(f"  -> uretici {u} disarida", s, y[te])
    ...
    for u in sorted(set(mfg)):
        ...
        siralama = toplam + kalib          # esik duzeltilse bile kalan kayip
        print(f"  uretici {u}: toplam {toplam:+.4f} = KALIBRASYON {-kalib:+.4f} "
