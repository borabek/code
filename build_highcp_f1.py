# -*- coding: utf-8 -*-
"""Op 3 F1: high-CP PXC genisletilmis havuz (6k+9k) + RF gate + top-N -> F1 yukseliyor mu?
SIZINTISIZ: RF gate high-CP parcalar DISINDA (standart f1_sweep_data) egitilir, high-CP'ye uygulanir.
Karsilastir: standart-6k top-N vs genisletilmis-6k9k top-N (ikisi de ayni gate)."""
...
def main():
    ...
    print(f"gate egitildi: {keep.sum()} CP (high-CP {len(hicp_grp)} parca HARIC)", flush=True)
    ...
    for k, (pid, jf, stp, n) in enumerate(hi, 1):
        try:
            ...
            et, en = topN(exp, V6, avg6, Vr, Vj, G, Gd, N)   # avg6 features; 9k CP'ler icin de V6-avg yeterli (yaklasik)
    ...
    print(f"\n=== high-CP PXC top-N F1 (sizintisiz gate, {len(hi)} parca, {GT} CP) ===")
    print(f"  6k standart havuz:      P{ps[0]:.3f} R{ps[1]:.3f} F1 {ps[2]:.3f}")
