# -*- coding: utf-8 -*-
"""REJIM YONLENDIRICI v2: TEK ESIK yerine OGRENILMIS karar.

NEDEN. v1 tek bir istatistige tek esik koyuyordu (`n01 >= 90 -> P6`). D7'de
sonuc: 8/12 markada arti ama WIE **-0.0802**, A-B/CCD/KLM ~-0.03. Yani kapi
bazi parcalarda yanlis tarafa yonlendiriyor ve tek esik bunu ayirt edemiyor.

Bu betik parca duzeyinde KUCUK bir siniflandirici ogrenir:
    girdi   : cikarim aninda gorulebilen parca istatistikleri
    etiket  : P6 mi TABAN mi DAHA IYI (parca basina F1 farki)
    cikti   : P6 kullan / TABAN kullan

Etiket GT'den turer -- ama bu EGITIM verisidir, sinav degil. Urun sadece
istatistikleri gorur. Kural yine `tam` MARKA KATLARINDA dogrulanir: disarida
birakilan markada yonlendirici TARANMAZ.

DURUSTLUK: v1'in D7 sayisi (0.3115) ILAN EDILMIS okumaydi. Bu v2, D7'ye
BAKILMADAN `tam` katlarinda olculur; dagitilirsa D7 okuma #2 harcanir.
"""
import collections
import json
import os
...
def oznitelik(d, s_tb, V):
    ...
    return [float(n01), float(nm), nm / max(n01, 1), float(len(d["P"])),
            ...
            float((np.asarray(s_tb, float) >= urun_genis.ESIK).sum()),
...
def main():
    ...
    for i, d in enumerate(veri, 1):
        ...
        W.append(abs(f_p - f_t))          # farkin BUYUKLUGU kadar onemli
        if i % 500 == 0:
            print(f"  {i}/{len(veri)} ({time.time() - t0:.0f} s)", flush=True)
    X = np.asarray(X, float)
    Y = np.asarray(Y, int)
    W = np.asarray(W, float)
    print(f"P6 daha iyi olan parca: {Y.mean():.3f} | ortalama |fark| "
    ...
    pk["rejim2"] = {"model": son, "oz_ad": OZ_AD, "esik": 0.5}
    ...
    json.dump({"damga": makbuz_hash.damga(),
               ...
               "not": "Ogrenilmis rejim yonlendirici. Etiket EGITIM verisinden "
                      "(parca basina F1 farki); urun yalniz istatistikleri "
                      "gorur. D7'ye BAKILMADI. HEPSI/v1 satirlari ORNEKLEM-ICI."},
              open("results/rejim2.json", "w"), indent=1)
    print(f"makbuz -> results/rejim2.json  ({time.time() - t0:.0f} s)")
