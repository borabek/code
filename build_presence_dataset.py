# -*- coding: utf-8 -*-
"""G ADIMI 1/2: TESPIT data seti -- hedefler AGIZDA, yuvada DEGIL.

C'NIN OLUMUNDEN OGRENILEN (2026-07-30, measured): `build_axis_dataset.py` hedefleri ureticinin
ConnectionPoint'ine 4mm yakin koselere koyuyor. Ama manufacturer CP'si bizim mouth noktamizdan eksende
**+7.79mm ICERIDE** (medyan; %91.7 single yonlu) -- i.e. YUVADA. C that hedeflerle egitildi, after
AGIZDA sorgulandi and own training parcalarinda bile %70.6 error verdi. Ag yeteneksiz degildi;
wrong places sorgulanmisti.

BU BETIK O HATAYI TEKRARLAMAZ: each manufacturer CP'si before `cp_geometry.seat_to_mouth` with axis
along ILK YUZEYE (agza) tasinir, hedef koseler AGZIN cevresinde isaretlenir. Aday turetme de
yuzeydeki acikliklarda calistigi for training and kullanim AYNI YERDE becomes.

Ek guvenlik: CP govdenin DISINDAYSA seat_to_mouth onu OYNATMAZ (parcaya according to degisir; measured
16/16 iceride vs 0/19 iceride). Offset also kaydedilir ki distribution denetlenebilsin.

Cikti: results/presence_dataset/<part>.npz (idx, n_verts, offset_mm) + index.json
"""
...
def main ():
    ...
    if a .limit :
        parts =parts [:a .limit ]
    print (f"{len (parts )} part | {len (done )} hazir | yaricap {a .radius }mm | hedefler AGIZDA",
    flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp ,n )in enumerate (parts ,1 ):
        ...
        try :
            ...
            if os .path .exists (mc ):
                ...
                Vr ,_ =step_to_mesh (stp )# hizalama for ham mesh sart
            else :
