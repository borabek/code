# -*- coding: utf-8 -*-
"""TR -> EN 2. kind: YALNIZ yorum and docstring metni.

WHY TOKENIZER: 2. kind sozlukte `and->and`, `or->or`, `not->not`,
`if->if` like PYTHON ANAHTAR KELIMELERI present. Bunlari koda uygulamak
`and` adli a degiskeni `and` yapar and dosyayi tamamen breaks. Bu yuzden
donusum Python'un own `tokenize` modulu with YALNIZ COMMENT and
docstring (ifade as duran STRING) belirteclerine uygulanir.
"""
...
def main ():
    ...
    if not uygula :
        print ("(kuru kosum -- --uygula with writes)")
