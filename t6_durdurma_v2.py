...
T5 formulasyon degisikligiyle -0.0148'den +0.0142'ye cikti (parca basina regresyon yerine
ADAY BASINA dur/devam karari; egitim satiri 200 -> 3277). Kahinin %14'u.

Iki eksik vardi, ikisi de ucuz:

  1. MODEL SIFIRDAN OGRENIYOR. Mevcut kuralin karari ozellik olarak VERILMIYORDU; model
     "kesim nerede" sorusunu bastan cozmeye calisiyordu. Oysa istedigimiz sey mevcut kuralin
     DUZELTMESI. Mevcut kararin bayragi ve mevcut K'ya gore goreli sira eklendi -> model
     artik "burada kuraldan sapmali miyim" sorusunu ogreniyor.

  2. HER SATIR ESIT AGIRLIKTAYDI. Oysa bir adayin alinip alinmamasi parcanin F1'ini bazen
     hic degistirmiyor, bazen cok. Siniflandirici, hic fark etmeyen satirlari dogru bilmek
     icin fark eden satirlari feda edebiliyordu. Satirlar artik MARJINAL F1 ETKISIYLE
     agirliklandiriliyor -- ogrenilen sey urun metrigine baglandi.

KILL (t3/t4/t5 ile AYNI, degistirilmedi): tespit >= +0.02 VE DEV ile VAL ayni yonde VE
gorulmemis uretici ortalamasi dusmeyecek. Gecerse eslestirilmis bootstrap ile dogrulanir.
"""
import json
import os
import pickle
...
def main():
    ...
    print(f"KILL: tespit >= +0.02 VE DEV/VAL ayni yonde VE uretici ort dusmeyecek -> "
          f"{'GECTI' if SON[iyi][6] else 'GECMEDI'}")
    with open("results/t6_durdurma_v2.json", "w", encoding="utf-8") as f:
        json.dump({k: {"esik": v[0], "tespit": v[1], "fark": v[2], "dev": v[3],
                       "val": v[4], "mfg": v[5], "gecti": bool(v[6])}
                   for k, v in SON.items()}, f, indent=1)
    print("makbuz -> results/t6_durdurma_v2.json")
