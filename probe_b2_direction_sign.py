...
Ayrica adayin YEREL MESH NORMALI that yonu tasiyor mu diye bakilir; ceiling olcumu
normalin 'direction absent'u %23.0 -> %1.9 indirdigini soyluyordu (B1 kolunun dayanagi).
"""
...
sys .path .insert (0 ,".")
import brep_pool ,canonical_d7 as K 
...
for pid in pids :
    ...
    if os .path .exists (f ):
        ...
        NV =brep_pool .vertex_normals_at (V ,np .asarray (zz ["F"],np .int64 ))
    ...
    for j in range (len (G )):
        ...
        if not kon .any ():
            continue # ADAY_YOK kovasi, here not
        ...
        if (180.0 -aci <=ACI ).any ():
            ...
            # yerel mesh normali that yonu tasiyor mu (B1'in dayanagi)
        if V is not None :
