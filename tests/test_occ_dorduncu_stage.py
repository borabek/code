...
Uc gmsh kademesi de same places kiriliyordu: OCC->gmsh ithalinde TEL/ILMEK topolojisi
("Could not fix wire in surface 139", "The 1D mesh seems not to be forming a closed loop").
gmsh yuzeyi 2B parametre uzayinda mesh'lemek for KAPALI tel ister; BRepMesh yuzeyi
dogrudan ucgenler and bozuk tele tahammul eder.

KALIBRASYON (two yolun da calistigi 4 parcada measured): bbox BIREBIR same, hacim farki
%0.01-0.3, ikisi de this gecirmez. Yani same KATI, only more kaba tessellasyon --
and `thesis_remesh.remesh_uniform(target=6000)` bunu already normalize ediyor.
"""
import os 
import sys 
...
def test_kaynakla_ayni_konumdaki_tepeleri_birlestirir ():
    """Yuz-face tessellasyon DIKISSIZ gelir: each yuzde ayri vertex. Kaynak bunu kapatir."""
    V =np .array ([[0. ,0 ,0 ],[1 ,0 ,0 ],[0 ,1 ,0 ],# ucgen A
    [0. ,0 ,0 ],[1 ,0 ,0 ],[0 ,0 ,1 ]])# ucgen B, two tepesi ORTAK
...
def test_kaynakla_cokmus_ucgeni_atar ():
    """Kaynaktan after two kosesi same which is ucgen gecersizdir."""
...
def test_tamlik_esigi_OCC_yolunda_da_gecerli ():
    """Yarim kabuk korpusa GIRMEZ -- gmsh yolundaki disiplin OCC'de de present."""
    import inspect 
...
def test_kademe_sirasi_OCC_EN_SON ():
    """OCC only UC gmsh kademesi de dustugunde works -- running parcalarin
    mesh'i and TUM onbellekler aynen korunmali."""
    import inspect 
    src =inspect .getsource (I .step_to_mesh )
    i1 =src .index ("heal=False")
    i2 =src .index ("heal=True)")
    i3 =src .index ("tol=1e-2")
    # rindex: OCC adi two places gecer -- istege bagli OCC-ONCE rotasi (most ustte, default
    # closed) and YEDEK zincirin last kademesi. Burada YEDEK zincirdeki order sinaniyor.
...
def test_gercek_parcada_OCC_gmsh_ile_AYNI_KATIYI_verir ():
    """Kalibrasyon testi: two path da running a parcada bbox and hacim ortusmeli."""
    import glob 
    import trimesh 
    from corpus_identity import step_kimlik as SK 
    S ={SK (s ):s for s in glob .glob ("all_wscad_stp/*.stp")}
    if "3024407"not in S :
        pytest .skip ("calibration parcasi none")
    ...
    assert abs (m1 .volume -m2 .volume )/m1 .volume <0.01 ,"hacim %1'den excess sapmamali"


def test_occ_once_VARSAYILAN_KAPALI ():
    """Dondurulmus kumelerin mesh KAYNAGI degismemeli: rota acikca istenmedikce closed."""
    import inspect 
    import os as _os 
    src =inspect .getsource (I .step_to_mesh )
...
def test_kaynakla_sifir_alanli_ucgeni_atar ():
    """Dogrusal koseli ucgen NaN normal produces -- OCC yolunda temizlenmeli."""
    V =np .array ([[0. ,0 ,0 ],[1 ,0 ,0 ],[2 ,0 ,0 ],# UCU DE same dogruda
    [0. ,1 ,0 ]])
    ...
    remaining ={tuple (p )for p in V2 [F2 [0 ]]}
    assert remaining =={(0. ,0 ,0 ),(1. ,0 ,0 ),(0. ,1 ,0 )}
