# -*- coding: utf-8 -*-
"""B-rep genisletilmis pool testleri. Makbuz: results/brep_filtre_taramasi.json."""
import os 
import sys 
...
def sil (c ,a ,b ,axis =(1.0 ,0.0 ,0.0 ),r =1.5 ):
    return {"axis":list (axis ),"radius":r ,"center":list (c ),
    "mouth_a":list (a ),"mouth_b":list (b )}


def test_iki_agiz_iki_aday_ZIT_yonlu ():
    P ,D =brep_pool .brep_adaylari ([sil ((0 ,0 ,0 ),(-5 ,0 ,0 ),(5 ,0 ,0 ))],[])
    assert len (P )==2 
    assert np .allclose (D [0 ],-D [1 ])


def test_sifir_eksen_ATLANIR_patlamaz ():
    P ,_ =brep_pool .brep_adaylari (
...
def test_dedupe_ayni_agzi_teke_indirir ():
    ...
    assert len (brep_pool .brep_adaylari (c ,[],dedupe_mm =3.0 )[0 ])<4 
...
def test_aciklik_merkezi_normal_yonuyle_eklenir ():
    P ,D =brep_pool .brep_adaylari ([],[{"center":[3 ,3 ,3 ],"normal":[0 ,0 ,2 ]}])
...
def test_meta_siralamasi_P_ile_BIREBIR ():
    """Tanimlayicilar meta'dan okunacak; ranking kaymasi silent error olurdu."""
    c =[sil ((0 ,0 ,0 ),(-9 ,0 ,0 ),(9 ,0 ,0 ),r =1.5 ),
    sil ((0 ,40 ,0 ),(-9 ,40 ,0 ),(9 ,40 ,0 ),r =2.5 )]
    a =[{"center":[0 ,80 ,0 ],"normal":[0 ,0 ,1 ]}]
    P ,D ,met =brep_pool .brep_adaylari (c ,a ,meta =True )
...
def test_meta_kapaliyken_eski_imza_KORUNUR ():
    out =brep_pool .brep_adaylari ([sil ((0 ,0 ,0 ),(-9 ,0 ,0 ),(9 ,0 ,0 ))],[])
    assert len (out )==2 


def test_meta_bos_girdide_uclu_doner ():
    P ,D ,met =brep_pool .brep_adaylari ([],[],meta =True )
...
def test_mesh_adayi_esigi_gecmeyeni_ALMAZ ():
    V ,F =_kutu_mesh ()
    P ,D =brep_pool .mesh_adaylari (V ,F ,np .zeros (len (V )),threshold =0.5 )
    assert len (P )==0 and len (D )==0 


def test_mesh_adayi_normali_BIRIM ():
    V ,F =_kutu_mesh ()
    P ,D =brep_pool .mesh_adaylari (V ,F ,np .ones (len (V )),threshold =0.5 ,
    dedupe_mm =0.0 )
    assert len (P )==len (V )
...
def test_mesh_seyreltme_aday_sayisini_DUSURUR ():
    V ,F =_kutu_mesh ()
    p =np .ones (len (V ))
    a =len (brep_pool .mesh_adaylari (V ,F ,p ,threshold =0.5 ,dedupe_mm =0.0 )[0 ])
    b =len (brep_pool .mesh_adaylari (V ,F ,p ,threshold =0.5 ,dedupe_mm =25.0 )[0 ])
    assert b <a 
...
def test_tam_havuz_kaynak_isaretleri_0_1_2 ():
    ...
    P ,D ,kay =brep_pool .tam_havuz (Ps ,Ds ,c ,[],V =V ,F =F ,
    ppos =np .ones (len (V )))
...
def test_tam_havuz_mesh_verilmezse_ESKI_davranis ():
    ...
    a =brep_pool .merged_pool (Ps ,Ds ,c ,[])
    b =brep_pool .tam_havuz (Ps ,Ds ,c ,[])
