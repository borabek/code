# -*- coding: utf-8 -*-
"""`sec_ayrintili`nin BUTUN cikis dallari AYNI sayida value dondurmeli.

WHY IT EXISTS: score alani eklendiginde normal dal 4'e output, erken cikis dali 3'te
kaldi and D7 exam okumasinin P6 kolu "expected 4, got 3" with coktu. Cikis
...
def test_skor_alani_secilen_secenegin_skorunu_tasir ():
    ...
    _ ,_ ,ai ,sc =p6_decision .sec_ayrintili (P ,idx ,YD ,s ,0.1 ,nms_mm =0.0 )
    assert len (sc )==len (ai )
    assert np .isclose (sc [0 ],s .max ())# most high score ONCE secilir
