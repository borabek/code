...
GECENIN GERCEK KOLLARINI yeniden ureteibilmeli. Uretemiyorsa ya rule ya da that night verilen
...
def test_isaret_yetmez_BUYUKLUK_sart ():
    ...
    baseline ={"familiar":0.74 ,"A":0.50 ,"B":0.70 }
    assert not K .degerlendir (baseline ,{"familiar":0.74 ,"A":0.5018 ,"B":0.7018 })
    # this test BUYUKLUK sartini sinar, KANIT sartini not -> evidence acikca kapatilir
    assert K .degerlendir (baseline ,{"familiar":0.74 ,"A":0.52 ,"B":0.72 },kanit_gerekli =False )


def test_riski_TASIMAK_gecmez ():
    """ExtraTrees dersi: a bolmede +0.05, digerinde -0.04 = mean sifir."""
    baseline ={"familiar":0.74 ,"A":0.50 ,"B":0.70 }
    assert not K .degerlendir (baseline ,{"familiar":0.74 ,"A":0.55 ,"B":0.66 })


def test_ortalama_tek_bolmedeki_COKUSU_gizleyemez ():
    """(4) maddesi: mean guzel olsa bile a split cokerse gecmez."""
    baseline ={"familiar":0.74 ,"A":0.50 ,"B":0.70 }
    k =K .degerlendir (baseline ,{"familiar":0.74 ,"A":0.70 ,"B":0.60 })
    assert not k and "buyuk loss"in k .rationale 


def test_tanidik_veride_buyuk_kayip_gecmez ():
    baseline ={"familiar":0.74 ,"A":0.50 ,"B":0.70 }
    assert not K .degerlendir (baseline ,{"familiar":0.70 ,"A":0.56 ,"B":0.73 })


def test_ga_sifiri_iceriyorsa_GURULTU_diye_isaretlenir ():
    k =K .degerlendir ({"familiar":0.74 ,"A":0.50 ,"B":0.70 },
    {"familiar":0.74 ,"A":0.53 ,"B":0.72 },
...
def test_gorulmemis_bolme_yoksa_PATLAR ():
    """Yalniz familiar veriyle dagitim karari verilemez -- sessizce 'GECTI' dememeli."""
    with pytest .raises (AssertionError ):
        K .degerlendir ({"familiar":0.74 },{"familiar":0.75 })
...
def test_GA_sifiri_iceriyorsa_DAGITIMA_izin_vermez ():
    ...
    baseline ={"familiar":0.74 ,"A":0.50 ,"B":0.70 }
    candidate ={"familiar":0.74 ,"A":0.56 ,"B":0.73 }
...
def test_kanitlanmis_BUYUK_kayip_engeller ():
    baseline ={"familiar":0.74 ,"A":0.50 ,"B":0.70 }
    candidate ={"familiar":0.74 ,"A":0.62 ,"B":0.63 }
