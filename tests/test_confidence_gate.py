...
Saha sozu "robotun kullandigi isaretler >=0.90 kesinliktedir" this fonksiyona
dayaniyor; hesap yanlissa soz de wrong becomes.
"""
import numpy as np 

import run_confidence_gate as G 


def _makbuz (score ,correct ):
    return {"sonuc":{"parca_kirilim":{
    f"p{i }":{"mfg":"X","rob":[int (d ),1 -int (d ),0 ],"tes":[0 ,0 ,0 ],
    "score":[float (s )],"correct":[int (d )]}
    for i ,(s ,d )in enumerate (zip (score ,correct ))}}}
...
def test_hedef_imkansizsa_None_doner ():
    ...
    d =np .zeros (100 ,int )# none of them correct not
    S ,Y ,n =G .cift (_makbuz (s ,d ))
