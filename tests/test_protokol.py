# -*- coding: utf-8 -*-
"""F0-1 PROTOKOL BEKCISI TESTLERI -- 'silent gecis' a more olmasin.

Bu testler two seyi PIN'ler:
  1. SIZINTI: LOCKED geometri gruplarindaki a part training kumesine girerse
     `protocol.dogrula()` HATA FIRLATMALI. (2026-08-03'te dagitilan gate LOCKED'in
     77/95'ini gormustu and no sey uyarmadi.)
  2. TEZ DEGISMEZLERI: 34 maddelik list along network/remesh/CP-tanimi degismemeli.
     Degisirse this test kirmizi yanar -- soz not, measurement.
"""
import os 
import sys 
...
def test_locked_gruplari_bos_degil (P ):
    assert len (P .yasak_gruplar ())>0 ,"LOCKED grubu bos -- exam korumasiz"
...
def test_tez_degismezleri_yerinde (P ):
    ...
    deviation =P .tez_dogrula (sert =False )
    assert not deviation ,f"TEZDEN SAPMA: {deviation }"
