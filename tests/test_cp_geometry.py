# -*- coding: utf-8 -*-
"""cp_geometry primitifleri -- ANALITIK olarak bilinen sekillerde sabitlenir.

Neden: 2026-07-28'de bu fonksiyonlar rtree yoklugunda sessizce girdilerini geri donduruyordu ve
hicbir sey fark etmedi. Burada beklenen degerler ELDE hesaplanabilir olduğu icin, fonksiyon
calismayi birakirsa test ANINDA kirmizi olur. Denetcinin (glb_audit) guvendigi taban budur.
"""
import numpy as np
import pytest
import trimesh
...
def test_ray_hits_from_center(box):
    """Merkezden +Z: tam olarak BIR kesisim, 15.0 mm'de."""
...
def test_ray_hits_through_from_outside(box):
    """Disaridan gecen isin: IKI kesisim (giris + cikis), 35 ve 65 mm."""
...
def test_ray_hits_normalises_direction(box):
    """Yon vektoru birim olmasa da mesafe MM cinsinden dogru kalir."""
...
def test_seat_to_mouth_from_center(box):
    """Merkezden +Z ekseni: iki yon de 15mm; agiz yuzeyde, offset buyuklugu 15."""
...
def test_seat_to_mouth_picks_nearest_exit(box):
    """z=+5'te: +Z'de 10mm, -Z'de 20mm -> YAKIN olan secilir."""
...
def test_seat_to_mouth_leaves_outside_point_alone(box):
    """DISARIDAKI nokta OYNATILMAZ (yoksa isin karsi duvara carpip noktayi firlatir)."""
    p = [0, 0, 100.0]
...
def test_outward_from_surface_point_points_away(box):
    """Yuzeyin hemen disindaki nokta: bos tarafa (+Z) bakmali."""
...
def test_outward_from_surface_point_flips_sign_when_needed(box):
    """Karsi yuzeyde ayni eksen: bu kez -Z disari olmali (isaret KENDILIGINDEN doner)."""
    d = outward_along_axis(box, [0, 0, -15.5], [0, 0, 1])
    assert d[2] == pytest.approx(-1.0, abs=1e-9)


def test_outward_inside_point_takes_nearest_exit(box):
    """ICERIDEKI nokta: kisa yoldan cikilan taraf disaridir (z=+14 -> +Z, 1mm)."""
    d = outward_along_axis(box, [0, 0, 14.0], [0, 0, 1])
    assert d[2] == pytest.approx(+1.0, abs=1e-9)


def test_outward_free_space_prefers_more_room(box):
    """Serbest alandaki nokta: onunde daha COK bosluk olan taraf disaridir."""
    d = outward_along_axis(box, [0, 0, 40.0], [0, 0, 1])
    assert d[2] == pytest.approx(+1.0, abs=1e-9)      # +Z: hic malzeme yok; -Z: kutu var
...
def test_exit_length_outside_is_just_margin(box):
    """Bos tarafa bakan disarisi nokta: sadece pay kadar."""
...
def test_exit_length_tip_is_actually_outside(box):
    """SOZLESME M3'un tabani: olculen boyla uc GERCEKTEN disarida olmali."""
...
@pytest.fixture
def tube():
    """Ic yaricap 3, dis 8, yukseklik 20 tup: eksen uzerinde MALZEME YOK (delik)."""
    return trimesh.creation.annulus(r_min=3.0, r_max=8.0, height=20.0)


def test_tube_axis_is_hollow(tube):
    """Delik ekseninde isin hicbir yuzey kesmez -- 'aciklik' tanimi budur."""
    assert len(ray_hits(tube, [0, 0, 0], [0, 0, 1])) == 0
    assert is_inside(tube, [0, 0, 0]) is False


def test_tube_wall_is_solid(tube):
    """Duvarin icindeki nokta ICERIDEDIR (r=5.5, ic 3 ile dis 8 arasi)."""
    assert is_inside(tube, [5.5, 0, 0]) is True


def test_tube_marker_at_bore_mouth_points_out(tube):
    """Delik agzindaki isaretci eksen boyunca DISARI bakar ve ucu govdeye girmez."""
    p = np.array([0.0, 0.0, 10.5])                  # ust agzin hemen ustu
