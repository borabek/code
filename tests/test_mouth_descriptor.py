# -*- coding: utf-8 -*-
"""Agiz tanimlayici testleri: each sutunun FIZIKSEL anlamini dogrular.

Bir tanimlayici sessizce sifir/fixed donerse model onu ogrenemez but HATA DA
VERMEZ -- this projede more before boyle a arm a geceye mal became. O yuzden each
column own anlamiyla ayri sinanir.
"""
import os 
import sys 
...
def test_erisim_disari_bos_ise_UZAK_doner ():
    """Kutunun outside, disari bakan mouth -> onunde no sey absent."""
...
def test_girme_govdeye_carpinca_SONLU ():
    """Kutunun ustunde, govdeye correct bakan mouth -> sonlu mesafede carpar."""
    X =AT .tanimla (np .array ([[0.0 ,0 ,30 ]]),np .array ([[0.0 ,0 ,1.0 ]]),
    [sil (1.0 ,(0 ,0 ,30 ),(0 ,0 ,31 ))],kutu (),50.0 )
...
def test_kenar_isini_govdeyi_gorur ():
    """Kenar isinlari da same govdeye carpmali (eksende empty + kenarda full ayrimi)."""
    X =AT .tanimla (np .array ([[0.0 ,0 ,30 ]]),np .array ([[0.0 ,0 ,1.0 ]]),
...
def test_aciklik_kaydinda_esd_r_kullanilir ():
    """Duzlemsel acikliklarda `radius` absent, `esd_r` present."""
    X =AT .tanimla (np .array ([[0.0 ,0 ,30 ]]),np .array ([[0.0 ,0 ,1.0 ]]),
