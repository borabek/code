# -*- coding: utf-8 -*-
"""PARCA-ICI donusum testleri (wire_gate.parca_ici).

Bu donusum GORULMEMIS URETICI icin dagitildi (en kotu uretici-disi 0.4832 -> 0.5702). Kazanci
saglayan OZELLIGIN kendisi test ediliyor: parca-ici z-skor, bir sutunun PARCA CAPINDA kaymasina
ve olceklenmesine BAGISIK olmali. Ureticiden ureticiye degisen tam olarak bu -- dagilimin yeri
ve genisligi. Ozellik bozulursa transfer kazanci sessizce kaybolurdu ve F1 tablosu bunu bir
sonraki tam olcume kadar gostermezdi.
"""
import os
import sys
...
def test_donusum_yoksa_matris_aynen_doner():
    ...
    assert wg.parca_ici(X, None) is X
    assert wg.parca_ici(X, "").shape == (4, 3)


def test_zskor_genisligi_ikiye_katlar_ve_ham_sutunlari_KORUR():
    """Ham sutunlar KORUNMALI: yalniz goreli kullanmak olculdu ve tanidik veride -0.090."""
    wg = _wg()
    X = np.random.default_rng(0).normal(size=(6, 5)) * 3.0 + 7.0
    M = wg.parca_ici(X, "zskor")
    assert M.shape == (6, 10)
...
def test_zskor_parca_ici_KAYMA_ve_OLCEGE_bagisik(kaydir, olcek):
    ...
    z0 = wg.parca_ici(X, "zskor")[:, 4:]
    z1 = wg.parca_ici(X * olcek + kaydir, "zskor")[:, 4:]
    assert np.allclose(z0, z1, atol=1e-9), "z-kismi parca-ici kayma/olcege bagisik degil"


def test_sabit_sutun_sifir_verir_NaN_uretmez():
    """sd=0 -> bolme yok. NaN uretilirse RandomForest patlar ya da sessizce sacmalar."""
    wg = _wg()
    X = np.ones((5, 3))
    X[:, 1] = np.arange(5)
    M = wg.parca_ici(X, "zskor")
...
def test_tek_aday_NaN_uretmez():
    wg = _wg()
    for d in ("zskor", "sira"):
        M = wg.parca_ici(np.array([[1.0, 2.0, 3.0]]), d)
        assert M.shape == (1, 6) and np.isfinite(M).all(), d


def test_bilinmeyen_donusum_SESSIZ_gecmez():
    """Yanlis yazilmis bir donusum adi sessizce 'donusum yok'a dusmemeli."""
    wg = _wg()
    with pytest.raises(ValueError):
        wg.parca_ici(np.zeros((3, 2)), "zkor")


def test_dagitilan_model_donusumu_KENDI_tasiyor():
    """Geriye donuk uyum tasarimi: donusum modelde, config'te DEGIL.

    cp_config alani yalniz belge; model degistirilmeden bayrak cevrilirse gate yanlis
    genislikte beslenirdi. Kaynak TEK olmali."""
    import pickle
    ...
    if m.get("clf_z") is not None:
        # YONLENDIRMELI model: ham genislik n_feat, z modeli onun iki kati bekler.
        assert wg.parca_ici(np.zeros((3, m["n_feat"])), don).shape[1] == 2 * m["n_feat"]
    else:
        ham = len(m["feat_names"]) // 2
        assert wg.parca_ici(np.zeros((3, ham)), don).shape[1] == m["n_feat"]


def test_yaricap_uyusmazligi_SESSIZ_gecmez():
    """MAYIN (2026-08-01'de acildi, ayni gun kapatildi): TOPO_R ayarlanabilir yapildi ama model
    onu tasimiyordu. R=6'da egitilmis gate R=8 ozellikleriyle beslenseydi SUTUN SAYISI AYNI
    KALDIGI ICIN hata vermezdi -- gate sessizce kotulesirdi.

    `n_feat` bu hatayi YAKALAYAMAZ: sutunlarin sayisi degil ANLAMI degisir. Bu yuzden ayarin
    kendisi modelde saklanir ve calisma aninda karsilastirilir."""
    import importlib
    ...
    m6 = {"topo_r": 6.0}
    wire_gate._dogrula_uyum(m6)                     # ayni yaricap: sorun yok
    wire_gate._dogrula_uyum({})                     # eski model (kayitsiz): engellenmez
...
def test_dagitilan_model_yaricabini_TASIYOR():
    """Dagitilan gate topoloji kullaniyorsa `topo_r` yazili OLMALI -- yoksa mayin geri acilir."""
    import json, pickle
    ...
    with open(yol, "rb") as f:
        m = pickle.load(f)
    assert m.get("topo_r") is not None, \
        "dagitilan gate topoloji kullaniyor ama egitildigi yaricapi TASIMIYOR"
    assert float(m["topo_r"]) == float(cfg.get("gate_topo_r", 6.0)), \
        f"model {m['topo_r']} mm ile egitildi, config {cfg.get('gate_topo_r', 6.0)} mm diyor"


def test_cokus_yonlendirme_ESIGE_gore_model_degistirir():
    """Yonlendirme SADECE cokuste tetiklenmeli.

    Bu, urunun en ince davranisi: z-skor modeli her parcaya uygulanirsa saglikli ureticide
    vergi odenir (olculdu: PXC-disi -0.0375). Esigin YANLIS yonde calismasi bu vergiyi geri
    getirirdi ve toplam F1'de kucuk gorunecegi icin gozden kacardi."""
    wg = _wg()
...
def test_yonlendirme_yoksa_skor_AYNEN_doner():
    """Eski (yonlendirmesiz) modeller degismeden calismali."""
    wg = _wg()
    s = np.array([0.4, 0.9])
    X = np.zeros((2, 3))
