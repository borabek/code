...
def test_f1w_TEK_REJIMDE_o_rejimin_F1ini_verir ():
    ...
    """
    very =_satir ("very",3 ,1 ,2 ,10 )
    correct =f1_rejim (very )["F1"]["very"]
    assert abs (f1w (very )-correct )<1e-9 ,"tek rejimde f1w o rejimin F1'ini vermeli"
...
def test_f1_rejim_dogru_kirilim_ve_agirlikli_toplam_verir ():
    ...
    assert abs (r ["agirlikli_F1"]-f1w (rows ))<1e-9 ,"agirlikli total f1w with tutmuyor"
    for k in ("dusuk","very"):
...
def test_SINAV_egitim_maskesi_LOCKED_gruplarini_atar ():
    ...
    remaining ={gk .get (x ,"none:"+x )for x in np .unique (pids [m ])}
    ihlal =[x for x in rap ["locked_temiz"]if gk .get (x ,"none:"+x )in remaining ]
    assert not ihlal ,f"maskeden after hala {len (ihlal )} LOCKED grubu egitimde: {ihlal [:5 ]}"
...
def test_f1w_TEK_REJIMLI_bolmede_agirligi_YENIDEN_NORMALLESTIRIR ():
    ...
    tek_cok =[("very",8 ,2 ,2 )]
    assert abs (f1w (tek_cok )-0.8 )<1e-6 ,f"tek rejimde (very) 0.8 olmali, {f1w (tek_cok )}"
