...
def test_hierarchy_invariants (graphs ):
    for V ,nbr ,hier ,x in graphs :
        ...
        for li in range (3 ):
            ...
            assert torch .allclose (uw .sum (1 ),torch .ones (n [li ]),atol =1e-5 ),"upsample weights must sum to 1 per fine vertex"
...
def test_registry_roundtrip (model_meta ):
    ...
    m2 .eval ()
    m2 .load_state_dict (model .state_dict ())# raises ten any shape mismatch
...
def test_sign_inv_prefix_default_off ():
    ...
    assert 'startswith("wscaduniverse")'not in src ,"hard-coded wscad sign-invariance gate is back"
