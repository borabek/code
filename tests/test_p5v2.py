"""p5-v2: secenek uretici + bire-bir etiketleme + ortak secim kisitlari."""
import numpy as np
...
def test_mevcut_ilk_null_son():
    """TEZ SADAKATI: `v_o` HER ZAMAN 0 numarali secenek; NULL en sonda."""
...
def test_ayni_agiz_ayni_kimlik():
    """Bir silindirin iki isareti AYNI fiziksel agzi paylasir -> ayni kimlik."""
    P = np.array([[0., 0, 0]]); D = np.array([[0., 0, 1]])
    o = PS.secenekler(P, D, [_cy([1, 0, 0], [1, 0, 0], 2.0)], [], 50.0)[0]
    kim = [s[3] for s in o if s[3] >= 0]
    assert len(kim) > len(set(kim)), "isaret ciftleri ayni kimligi paylasmali"


def test_etiket_BIRE_BIR():
    """Iki aday AYNI GT'yi saglasa bile YALNIZ BIRI pozitif olmali (greedy DEGIL)."""
    ...
    assert sum(int(v.any()) for v in y) == 1, "iki aday da pozitif olmus (greedy)"


def test_secim_agzi_TEK_adaya_verir():
    """Ayni agza iki aday talip; ORTAK secim yalniz birine vermeli."""
    P = np.array([[0., 0, 0], [0.6, 0, 0]])
    D = np.array([[0., 0, 1], [0., 0, 1]])
    secs = PS.secenekler(P, D, [_cy([1, 0, 0], [1, 0, 0], 2.0)], [], 50.0)
    skor = [np.full(len(o), 0.1) for o in secs]
    for s in skor:                    # agiz secenekleri MEVCUT'tan cazip
        s[1:-1] = 0.9
    Pf, Df = PE.sec(secs, skor)
    kullanilan = [tuple(np.round(p, 3)) for p in Pf]
    assert len(kullanilan) == len(set(kullanilan)), "ayni agiz iki adaya verilmis"


def test_mevcut_GERCEK_fallback():
    """Hicbir secenek cazip degilse aday SILINMEZ, MEVCUT'ta kalir."""
    P = np.array([[1., 2, 3]]); D = np.array([[0., 0, 1]])
    secs = PS.secenekler(P, D, [], [], 50.0)
    skor = [np.array([0.9, 0.1])]      # MEVCUT yuksek, NULL dusuk
...
def test_NULL_adayi_atabilir():
    P = np.array([[1., 2, 3]]); D = np.array([[0., 0, 1]])
    secs = PS.secenekler(P, D, [], [], 50.0)
    skor = [np.array([0.1, 0.9])]      # NULL yuksek
    Pf, _ = PE.sec(secs, skor)
    assert len(Pf) == 0


def test_siralayici_dogruyu_UST_SIRAYA_koyar():
    """Pairwise siralayici, ayni aday icinde dogru secenegi en yuksek skorlamali."""
    import numpy as np
    import p5v2_egit as PE
    rng = np.random.RandomState(0)
    veri = []
    for _ in range(40):
        # 4 secenek; DOGRU olan 3. sutunu (mesafe) EN KUCUK olan
