"""p5-v2: option manufacturer + bire-a etiketleme + ortak secim kisitlari."""
import numpy as np 
...
def test_mevcut_ilk_null_son ():
    """TEZ SADAKATI: `v_o` HER ZAMAN 0 numarali option; NULL most probe."""
    P =np .array ([[0. ,0 ,0 ]]);D =np .array ([[0. ,0 ,1 ]])
    o =PS .options (P ,D ,[_cy ([1 ,0 ,0 ],[1 ,0 ,0 ],2.0 )],[],50.0 )[0 ]
...
def test_ozn_uzunlugu ():
    P =np .array ([[0. ,0 ,0 ]]);D =np .array ([[0. ,0 ,1 ]])
    for s in PS .options (P ,D ,[_cy ([1 ,0 ,0 ],[1 ,0 ,0 ],2.0 )],[],50.0 )[0 ]:
...
def test_secim_agzi_TEK_adaya_verir ():
    ...
    D =np .array ([[0. ,0 ,1 ],[0. ,0 ,1 ]])
    secs =PS .options (P ,D ,[_cy ([1 ,0 ,0 ],[1 ,0 ,0 ],2.0 )],[],50.0 )
    score =[np .full (len (o ),0.1 )for o in secs ]
    for s in score :# mouth secenekleri MEVCUT'defn cazip
        s [1 :-1 ]=0.9 
    Pf ,Df =PE .sec (secs ,score )
...
def test_mevcut_GERCEK_fallback ():
    """Hicbir option cazip degilse candidate SILINMEZ, MEVCUT'ta kalir."""
    P =np .array ([[1. ,2 ,3 ]]);D =np .array ([[0. ,0 ,1 ]])
    secs =PS .options (P ,D ,[],[],50.0 )
    score =[np .array ([0.9 ,0.1 ])]# MEVCUT high, NULL low
    Pf ,_ =PE .sec (secs ,score )
...
def test_NULL_adayi_atabilir ():
    P =np .array ([[1. ,2 ,3 ]]);D =np .array ([[0. ,0 ,1 ]])
    secs =PS .options (P ,D ,[],[],50.0 )
    score =[np .array ([0.1 ,0.9 ])]# NULL high
    Pf ,_ =PE .sec (secs ,score )
    assert len (Pf )==0 
...
def test_siralayici_dogruyu_UST_SIRAYA_koyar ():
    ...
    data_ =[]
    for _ in range (40 ):
    # 4 option; DOGRU which is 3. sutunu (distance) EN KUCUK which is
        F =rng .rand (4 ,len (PS .OZ_AD ))
        correct =int (np .argmin (F [:,3 ]))
    ...
    for d in data_ :
        ...
        if int (np .argmax (s ))==int (np .argmax (d ["y"][0 ])):
            dogru_ust +=1 
    assert dogru_ust /len (data_ )>0.7 ,f"only {dogru_ust }/{len (data_ )} correct"
