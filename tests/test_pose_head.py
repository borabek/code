# -*- coding: utf-8 -*-
"""POSE HEAD testleri -- gate KARARINDAN SONRA calisan yanal duzeltme.

Tavan olcumu (results/t_tavan.json): kahin gate robot-haziri yalniz +0.059 tasiyor, KONUM
+0.325. Yani gate'in otesindeki tek gercek kaldirac buydu. 2026-08-01'de KURAL tabanli dort
konum/yon kolu denendi ve dordu de oldu; fark, bu kafanin duzeltmenin BUYUKLUGUNU aday basina
OGRENMESI.
"""
import os
import sys
...
def test_yerel_cerceve_ortonormal_ve_eksene_hizali():
    wg = _wg()
    # TOL 1e-8: 1e-9 KAYAN NOKTA gurultusune takiliyor (olculdu: 1.0000000827e-09).
    # Testin kendisi fazla sikiydi, kod degil.
    T = 1e-8
...
def test_duzeltme_EKSENE_DIK_kalir():
    """Eksenel derinlik DEGISMEMELI -- yalniz eksene dik sapma duzeltilir."""
    wg = _wg()
    ...
    out = wg.pose_duzelt(np.zeros((1, wg._load(wg.POSE_PATH)["n_feat"])), cps)
    kayma = np.asarray(out[0]["point"], float) - onc
    assert abs(float(kayma @ d)) < 1e-9, "duzeltme EKSEN boyunca kaymis (derinlik degismis)"
...
def test_model_yoksa_CP_degismez():
    ...
    out = wg.pose_duzelt(np.zeros((1, 58)), cps, model_path="results/_yok_boyle_bir_dosya.pkl")
    assert np.allclose(out[0]["point"], [1.0, 2.0, 3.0])


def test_yanlis_genislik_SESSIZ_gecmez_ve_CP_bozmaz():
    """Ozellik genisligi uymuyorsa duzeltme YAPILMAZ ve sayac artar."""
    wg = _wg()
    m = wg._load(wg.POSE_PATH)
    if m is None:
        pytest.skip("pose modeli yok")
    ...
    cps = [{"point": np.array([1.0, 2.0, 3.0]), "direction": np.array([0.0, 0.0, 1.0])}]
    out = wg.pose_duzelt(np.zeros((1, m["n_feat"] - 3)), cps)
    assert np.allclose(out[0]["point"], [1.0, 2.0, 3.0])
...
def test_urun_pose_head_i_GATE_SONRASI_cagiriyor():
    ...
    i_pose = s.index("pose_duzelt")
    assert i_pose > i_gate, "pose duzeltmesi gate KARARINDAN ONCE cagriliyor"
    assert '_c.get("_gate_hata")' in s or "_gate_hata" in s[:i_pose], \
        "gate hatasi olan parcada pose uygulanmamali"
