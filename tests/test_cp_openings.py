...
def test_cp_v2_still_reachable_for_provenance():
    # the superseded CableEntry-only definition (cp-v2; arbiter F1 0.000 on manufacturer PXC CPs)
...
def test_snap_axis_keeps_a_genuine_tilt():
    """Gercek egim YUVARLANMAZ -- yalnizca kucuk titreme temizlenir.

    Bu test eskiden tersini dogruluyordu: 11.7 derece egik bir yon eksene yuvarlanmali diyordu.
    Olcum bunu curuttu (8534 uretici ConnectionPoint): yonlerin yalnizca %75.6'si eksene hizali,
    **%19.1'i 10 dereceden fazla egik** (en fazla 43.1), ve bu tek ureticiye ozgu degil
    (PXC %16.9, WEI %21.7) -- klemenste tel acili bir huniden girer, egim GERCEKTIR.
    Her yonu yuvarlamak eslesen CP'lerin %18.2'sinde ekseni 15-90 derece saptiriyordu ve hata
    parca basina ya hep ya hic goruluyordu (93 parcanin 19'u tamamen yanlis).
    Esik 10 derece: korpusta 1-10 derece bandi tum CP'lerin yalnizca %5.3'u (titreme bandi),
    gercek egimler 22-43 derecede kumeleniyor.
    """
    # Esik ACIKCA verilir: urun varsayilani 90 (her zaman yuvarla), cunku 10 derece OLCULDU ve
    # kaybetti (-0.0248 robot-hazir F1). Test MEKANIZMAYI korur, varsayilani degil.
    ...
    # cikti her durumda birim vektor
