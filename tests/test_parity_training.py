...
Mevcut parite testi (`test_parity.py`) YALNIZ inference tarafina bakiyordu; training yolu
...
def test_vote2_BENZERSIZ_model_sayar_ve_tavan_MODEL_SAYISI ():
    ...
    lists =[[{"point":p ,"direction":np .array ([0 ,0 ,1.0 ]),"confidence":0.9 },
    ...
    assert out [0 ]["_votes"]<=len (lists ),"votes model sayisini ASIYOR"
...
def test_egitim_verisi_VOTES_tavanina_uyuyor ():
    ...
    # DAGITILAN training verisi (config'ten) -- fixed file adi not.
