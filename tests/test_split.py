...
def test_family_key_matches_accessory_suffix_variant():
    # A "+B056"-style accessory-option suffix must land on the SAME family key
...
def test_balance_search_fixes_skewed_grouped_split():
    ...
    # val fraction wildly off the requested one (measured on the real corpus:
    ...
    assert bal_skew < raw_skew              # balance search must improve on it
