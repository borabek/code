# -*- coding: utf-8 -*-
"""Ikinci kademe agiz olculeri: her sutunun FIZIKSEL anlamini dogrular.

Sessizce sabit/sifir donen bir sutun modele bilgi tasimaz ama HATA DA VERMEZ;
o yuzden her olcu kendi anlamiyla ayri sinanir.
"""
import os
import sys
...
def delikli_kutu(r=2.0, k=20.0):
    """Z ekseni boyunca bastan sona R yaricapli delik acilmis kutu."""
...
def test_delikte_profil_ACIK_kalir():
    """Gercek kanalda serbest yaricap derinlik boyunca ~sabit ve r'ye yakin."""
    m = delikli_kutu(r=2.0)
    X = AD.tanimla(np.array([[0.0, 0, 10.0]]), np.array([[0.0, 0, 1.0]]),
                   m, 35.0)
    assert 1.0 < X[0, 0] < 4.0            # profil_ort delik yaricapi mertebesinde
    assert X[0, 2] > 0.7                  # daralma yok


def test_duz_yuzeyde_profil_KAPALI():
    """Deliksiz kutunun yuzunde iceri bakan agiz -> serbest yaricap kucuk."""
    m = kutu()
    X = AD.tanimla(np.array([[0.0, 0, 10.0]]), np.array([[0.0, 0, 1.0]]),
                   m, 35.0)
    d = AD.tanimla(np.array([[0.0, 0, 10.0]]), np.array([[0.0, 0, 1.0]]),
                   delikli_kutu(r=2.0), 35.0)
    assert X[0, 0] < d[0, 0]              # delikli olan DAHA ACIK
