# -*- coding: utf-8 -*-
"""P6 HAZIRLIK KONTROLU -- D7 okumasindan ONCE runs.

D7 butcesi 3 okuma. Bir okumayi "model paketi urun yolunda does not work" diye
harcamak kabul edilemez. Bu betik birkac D6 parcasinda URUNUN TAM yolunu runs
and sunlari dogrular:

  1. model paketi yukleniyor and beklenen anahtarlari tasiyor
  2. `product_p6.secenek_tablosu` calisiyor, column count EGITIMDEKIYLE same
  3. skorlar DEJENERE not (all of them same / all of them 0 / NaN not)
  4. secim BOS DEGIL and makul sayida CP donuyor
  5. ikinci kademe varsa gercekten devrede (kisa list empty donmuyor)
...
def main ():
    ...
    f"{'ACIK'if product_p6 .MESH_HAVUZ else 'KAPALI'}")
    if not product_p6 .ACIK :
    ...
    for pid in pidler :
        ...
        cps ,avg ,_hi ,_u =robot_cp .derive_candidates (V ,F ,pbs ,S [pid ],cfg =cfg )
        if not cps :
            continue 
        tab =product_p6 .secenek_tablosu (V ,F ,avg ,cps ,S [pid ],
