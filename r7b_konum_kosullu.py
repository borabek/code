# -*- coding: utf-8 -*-
"""R7b: KOSULLU 2. ASAMA -- R7'nin tasarim kusurunu duzelt, sonra hukum ver.

R7 SONUCU: sifir ates. Sebep model degil TASARIM:
  * 2. asama TUM adaylarin satirlarinda egitildi. O dagilimda `mevcut` girisi %83.6
    dogru; digerleri ~%45. Model dogru sekilde HEP `mevcut`i secti ve argmax hic
    degismedi. Yani 2. asama, 1. asamanin verdigi bilgiyi (bu nokta KOTU) hic kullanmadi.
  * Bu, yon kolundaki hatanin ayni ailesi: sorun modelde degil, EGITIM DAGILIMINDA.
    Orada etiket isaretsizdi; burada kosul yok sayildi.

DUZELTME (iki degisiklik):
  1. 2. asama YALNIZ `mevcut`in KOTU oldugu adaylarin satirlariyla egitilir.
     Ogrenilen soru artik dogru soru: "mevcut zaten kotuyken hangi giris <=2mm?"
  2. `mevcut` 2. asamanin secenekleri arasindan CIKARILIR. 1. asama zaten onu reddetti;
     secenekler arasinda birakmak prior'u geri sokar. Karar kurali: 1. asama esigi
     GECERSE, alternatiflerin argmax'i alinir ve MUTLAK guven esigi aranir.

Bu, hasar asimetrisine (1:20) uygun tek mesru mimari: YUKSEK KESINLIKLI kapi + kosullu secim.

Hala DAGITIM DEGIL: grup-capraz fizibilite. Gecerse R8 (egitim korpusu) kosulur.
"""
...
def main():
    ...
    Bk = np.array([k[3] for k in BK])          # bu satirin adayinda mevcut KOTU mu
    print(f"asama1 {len(AY)} aday (kotu {AY.mean():.1%})")
    print(f"asama2 TUM {len(BY)} satir (iyi {BY.mean():.1%}) | "
          f"KOSULLU alt kume {int(Bk.sum())} satir (iyi {BY[Bk == 1].mean():.1%})", flush=True)

    ug = np.unique(np.concatenate([AG, BG])); rng = np.random.RandomState(0); rng.shuffle(ug)
    kat = {g: j % 5 for j, g in enumerate(ug)}
    ka = np.array([kat[g] for g in AG]); kb = np.array([kat[g] for g in BG])
    oofA = np.zeros(len(AY)); oofB = np.zeros(len(BY))
    for f_ in range(5):
        ta = ka != f_; tb = (kb != f_) & (Bk == 1)      # KOSULLU egitim
    ...
    def kos(esik, guven):
        ...
        for r in DER:
            ...
            rob.append((rj,) + esle(P, Pd, G, Gd, r["diag"], 2.0, 10.0, False, isaretli=True))
    ...
    print(f"\n{'esikQ':>7}{'guven':>7}{'ates':>6}{'kotuya':>8}{'iyiye':>7}"
          f"{'robot':>10}{'d':>9}{'GA':>22}{'tespit':>10}{'d':>9}")
    SON = {}
    for q in (0.70, 0.80, 0.85, 0.90):
        esik = float(np.quantile(oofA, q))
        # GUVEN ESIGI DUSURULDU: kosullu alt kumede pozitif oran %13.3 oldugu icin
        # skorlar 0.5'e hic ulasmiyordu. 0.00 = "1. asama atesleyince KOSULSUZ argmax"
        # ki bu mimarinin MUTLAK TAVANIDIR. Bunda bile kaybediyorsa kol kapanir.
        for guven in (0.0, 0.15, 0.25, 0.35):
            ra, da, _, na, kv, iv = kos(esik, guven)
            _, lo, hi = olcum_kumesi.grup_bootstrap(list(zip(r0, ra)), gg, fn, n=1500)
            dr = f1w(ra) - b_rob
            print(f"{q:>7.2f}{guven:>7.2f}{na:>6}{kv:>8}{iv:>7}{f1w(ra):>10.4f}{dr:>+9.4f}"
                  f"   [{lo:+.4f},{hi:+.4f}]{f1w(da):>10.4f}{f1w(da)-b_det:>+9.4f}")
            SON[f"{q}_{guven}"] = {"esikQ": q, "guven": guven, "ates": na,
    ...
    print(f"\nTARAMA en iyisi (UST SINIR, secim yanlisi var): robot {eniyi['d_robot']:+.4f} "
          f"(esik %{100*eniyi['esikQ']:.0f}, guven {eniyi['guven']}) "
    ...
    print(f"\nHUKUM: {'UST SINIR bile gecerse egitim-korpusu turetmesi DEGER' if deger else 'UST SINIR bile GECMIYOR -- KONUM KOLU KAPANIR'}")
    ...
    print("makbuz -> results/r7b_konum_kosullu.json")
