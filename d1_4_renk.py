# -*- coding: utf-8 -*-
"""D1.4: renk ozelligi TAM korpusta (1903 parca) gate'e ne kazandiriyor?

Erken sinyal (64 parca): +0.0195. Simdi gercek olcum.
Ozellikler cikarilmis adaylarin KONUMLARINDAN offline hesaplanir (GPU gerekmez).
NaN = renk yok; 0 YAZILMAZ ("metal yok" yalani uretir). Eksiklik bayragi ayri ozellik.
KILL: korpus-agirlikli CP-F1 katkisi < +0.02.
"""
import os, sys, json
import numpy as np
...
def main():
    ...
    for pid in np.unique(pids):
        ...
        try:
            ...
            flag = np.concatenate([np.full(len(x["pts"]), 1.0 if x["is_metal"] else 0.0)
                                   for x in faces])
            # KAPSAMA DUZELTMESI (2026-07-30, olculdu):
            #  (a) hizalama RENKLI yuzeylerle degil, katinin TUM koseleriyle yapilir --
            #      renkli yuzeyler katinin bir kismini kapladiginda oncekisi cakiyordu
            #  (b) residual esigi 1.0 -> 2.0mm; 1-2mm bandindaki 7 parcada donusturulmus
            #      koselerin mesh yuzeyine medyan uzakligi 0.8-1.3mm, yani pratikte kullanilir
            allv = L.all_vertex_points(stp_of[pid])
            R, t, res = align_frames(Vr, allv if len(allv) else allp)
            if res > 1.0:   # 2.0 denendi: kapsama %36->%58 ama katki +0.0078 -> -0.0044
                continue
            allm = (allp - t) @ R
            ...
            continue
        if done % 200 == 0:
            print(f"  {done} parca ({ok} renkli)", flush=True)
    print(f"\n{done} parca islendi, {ok} tanesinde renk (%{100*ok/max(done,1):.0f})")
    print(f"renk ozelligi olan aday: {int((~np.isnan(XC).any(1)).sum())}/{len(y)}\n", flush=True)
    ...
    print("makbuz -> results/d1_4_renk.json")
