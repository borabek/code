...
Bugune kadarki her gate BAGIMSIZ ikili siniflandirmaydi: her aday tek basina puanlandi.
Oysa karar PARCA ICINDE goreceli: "bu parcadaki 17 adayin hangileri tel girisi?" Set-transformer
adaylar arasi self-attention kurar (sira/pitch/ayna/komsuluk iliskisi ORTUK ogrenilir) ve
LISTWISE loss dogrudan parca-ici siralamayi optimize eder (top-N modunun tam hedefi).
Modlar: count-BILINMEYEN (esik) ve count-BILINEN (top-N) AYRI egitilir/olculur.
GO (kullanici): family-out'ta P1 sonucunun uzerine >= +0.010 ve 3 seed'de kararli.
...
if USE_EMB:
    ...
    print(f"embedding dahil ({EM.shape[1]}d), eslesen aday {int(ok.sum())}/{len(Y)}")
...
XN = (RICH - mu) / sd
print(f"WORK {int(WORK.sum())} aday / {len(set(gpid[WORK]))} parca | feat {XN.shape[1]}d | dev {dev}")


class SetNet(nn.Module):
    """DeepSets + self-attention: aday-ici temsil + parca-ici baglam -> skor."""
    ...
    def forward(self, x):                       # x: (1, n, d) tek parca
...
def train_setnet(idx_tr, seed, listwise, epochs=60):
    ...
    for ep in range(epochs):
        ...
        for gi in groups:
            ...
            s = net(x).squeeze(0)
            if listwise:                        # LISTWISE: parca-ici softmax capraz-entropi
                tgt = yy / yy.sum()
...
print(f"{'RF (taban)':22s} {nested(base,masks['ALL'],FG):7.4f} {nested(base,masks['WEI'],FG):7.4f} "
