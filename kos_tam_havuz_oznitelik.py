# -*- coding: utf-8 -*-
"""TAM HAVUZ (seg + B-rep + mesh tepeleri) icin gate ozniteliklerini uret.

Olculdu (`results/tavan_080_eksensiz.json`, D7 marka-disi, isaretli aci,
mukemmel secici): bu havuzun robot F1 TAVANI **0.8347**, parca basina ~318 aday.
Karsilastirma: urunun bugunku havuzu 13.4 aday / tavan 0.3430; bu gece kazanan
B-rep havuzu 98 aday / tavan 0.5761.

Yeniden baslatilabilir: her parca kendi npz'sine yazilir, var olan atlanir.
Diske `X` (58 sutun), `y` (PROJENIN etiket tanimi), `P`, `D`, `kaynak` yazilir.

ETIKET: `build_zengin_parite.py` ile BIREBIR -- yanal mesafe + eksenel 40mm
kapisi + acgozlu BIRE-BIR eslesme. (Oklid + coka-bir etiketi gate'i 0.08
bozuyordu, bkz. gate-etiket-tanimi-hatasi-ve-v6-kunyesi.)
"""
import argparse
import json
import os
...
def main():
    ...
    for ad in a.kume:
        ...
        for i, (pid, r) in enumerate(sorted(kay.items()), 1):
            ...
            y = proje_etiketi(P, G, np.asarray(r["Gd"], float), float(r["diag"]))
            ...
            if yazilan % 50 == 0:
                ...
                print(f"  {ad} {i}/{len(kay)} yazilan={yazilan} {h:.2f}s/parca "
