# -*- coding: utf-8 -*-
"""TEMIZ VERI-LEVER TESTI: yeni indirilen 220 parca (2327 CP) gate egitimine eklenince ne oluyor?
ONCEKI TESTIN HANDIKAPLARI KALDIRILDI:
  * bu parcalar 'seen' DEGIL -- gercekten yeni, segmentasyon gormedi
  * embedding yok diye SIFIR DOLGU yapmiyoruz; her iki taraf da SADECE zengin feature (46d) kullanir
Olcum: WORK (541 temiz+kilitsiz parca), family-out, nested-CV esik -- onceki protokolle ayni."""
import json
import numpy as np
...
base_gt = gt_of(WORK)
print(f"skorlama: {int(WORK.sum())} aday / {len(set(gpid[WORK]))} parca / GT {base_gt}")
for t, v in EX.items(): print(f"  ek havuz {t}: {v[3]} parca / GT {v[4]}")
print(f"\n{'egitim havuzu':30s} {'GT':>6s} {'AUC':>8s} {'ALL':>8s} {'WEI':>8s} {'PXC':>8s}")
res = {}
combos = [("TABAN (541 parca)", []), ("+ YENI 220", ["YENI220"]),
