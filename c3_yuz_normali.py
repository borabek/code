...
C1 SONUCU: fiziksel yon sozlugu uzerindeki kahin 0.6263 (bar 0.70) -> "dogru yon
elimizdeki geometriden turetilemiyor" dedim. O cumle FAZLA GENISTI: dogru yon BENIM
KURDUGUM sozlukte yoktu. Sozlugun tamami YERELDI (aday cevresindeki geometri):
mevcut/ham yon, uye yonleri, B-rep silindir ekseni, kanal ekseni, yuzey normali,
agiz duzlemi eksenleri.

EKSIK OLAN: PARCA DUZEYI. Klemens guclu bir global yonelime sahiptir -- teller parcanin
ON YUZUNDEN girer ve takma yonu genellikle parcanin KENDI eksenleriyle hizalidir. Ayni
yuzdeki butun CP'ler AYNI yone bakar. Bu bilgi calisma aninda mevcuttur (GT gerekmez).

EKLENEN GIRISLER (hepsi calisma aninda turetilebilir):
    obb_x/y/z (+/-)   parcanin yonelimli sinir kutusu eksenleri (6 yon)
    uzlasi            parcadaki TUM adaylarin yon medyani (en buyuk kume)
    yuz_uzlasi        adayla AYNI YUZDEKI adaylarin yon uzlasisi
    duzlem_obb        adayin eksenine DIK duzlemde, OBB eksenlerine en yakin 4 yon

ONCE TESHIS: GT yonu ne kadar siklikla bir OBB eksenine yakin? Cevap yuksekse kol canli.

KILL (C1 ile ayni bar): genisletilmis kahin < 0.70 ise Rota C GERCEKTEN kapanir.
"""
import io
import json
import os
...
def obb_eksenleri(V):
    """Parcanin yonelimli sinir kutusu eksenleri (PCA)."""
    Q = V - V.mean(0)
    try:
...
def uzlasi_yonu(D, agirlik=None):
    """Yonlerin EN BUYUK kumesinin medyani (isaret duyarli degil -> hizala)."""
    if not len(D):
        return None
    ref = D[0]
    A = np.array([d if float(d @ ref) >= 0 else -d for d in D])
    # en buyuk kume: her yona 20 derece icinde kac komsu var
...
def main():
    ...
    for kk, r in enumerate(D["DER"], 1):
        ...
        if r["X"] is not None and r.get("XR") is not None:
            ...
            if k.any():
                ...
                # C3: B-rep BASKIN DUZLEM NORMALLERI. OBB bir PCA YAKLASIGIDIR; klemenste
                # tel DUZ ON YUZDEN girer ve o yuzun GERCEK normali B-rep'te durur.
                YUZN = []
                try:
                    import brep_axes as _ba
                    pl = _ba.planes(stp[r["pid"]])
                    if pl is not None and len(pl):
                        _c, _n, _rr = (np.asarray(pl[0], float), np.asarray(pl[1], float),
                                       np.asarray(pl[2], float))
                        srt = np.argsort(-_rr)[:6]          # en BUYUK 6 duzlem yuzu
                        for j in srt:
                            b = _birim(_n[j])
                            if b is not None:
                                YUZN.append(b)
                ...
                for i in range(len(P)):
                    ...
                    if UZ is not None:
                        S2["uzlasi"] = UZ
                    for j, a in enumerate(YUZN):
                        S2[f"yuzn{j}"] = a; S2[f"yuzn{j}_"] = -a
                    # AYNI YUZ: en buyuk OBB ekseni boyunca benzer konumdakiler
                    if OBB and len(P) > 2:
                        n0 = OBB[0]
                        t_ = P @ n0
                        ...
                        if ayni.sum() >= 2:
                            ...
                            if fu is not None:
                                S2["yuz_uzlasi"] = fu
                    # adayin eksenine DIK duzlemde, OBB eksenlerine en yakin yonler
                    if d0 is not None:
                        for j, a in enumerate(OBB):
                            if a is None:
                                continue
                    ...
                    for a in SOZ2[-1]:
                        kk2 = a.rstrip("0123456789_") if (a.startswith("uye") or a.startswith("obb") or a.startswith("dik") or a.startswith("yuzn")) else a
    ...
    print(f"{'KAHIN yerel sozluk (C1)':<28}{r1:>9.4f}   yon boslugunun %{100*(r1-rm)/max(ust-rm,1e-9):.0f}'i")
    print(f"{'KAHIN +PARCA DUZEYI (C2)':<28}{r2:>9.4f}   yon boslugunun %{100*(r2-rm)/max(ust-rm,1e-9):.0f}'i")
    print(f"{'ust sinir (=tespit)':<28}{ust:>9.4f}")
    gecti = r2 >= 0.70
    print(f"\nKILL: kahin >= 0.70 -> {'GECTI, ayrik secici egitilir' if gecti else 'GECMEDI'}")
    ...
    print("makbuz -> results/c3_parca_yon.json")
