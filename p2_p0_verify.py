# -*- coding: utf-8 -*-
"""FAZ 2 / P0 -- DOGRULAMA + KILITLI BASELINE (TODO_085_PHASE2.md P0 maddeleri 1,4,6,7).
 (1) FRAME dogrulama: pos ve direction ayni frame'de mi -- pipeline bazinda raporla
 (4) part-out / geometry-out / family-out splitlerini split_lock.json'dan sabitle
 (6) baseline'lari TEMIZ CALISMA setinde yeniden dogrula (sizinti-muhafizi ACIK, kilitli holdout HARIC)
 (7) candidate-recall ile oracle-F1'i AYRI raporla
...
RICH = np.hstack([X13, XR[:, 0:33]])                      # 13 + konum(9) + cok-yaricap(24) = kazanan set
...
# --- (1) FRAME dogrulama (statik denetim + veri kontrolu) ---
print("=== (1) FRAME DOGRULAMA ===")
print("  build_rich_feats.rich_feats(): V/F/cps HEPSI mesh frame -> konum+yon TUTARLI  [OK]")
...
print("                  kullandi, parca-ici tutarli oldugu icin gecerliydi; yine de not edildi.)")
...
WORK = (gseen == 0) & (~is_locked)          # TEMIZ + kilitli-degil = tum gelistirme burada
sets = {"WORK (temiz, kilitsiz)": WORK,
        "CANONICAL (823, seen dahil)": np.ones(len(Y), bool)}
print(f"\n=== (4) SPLIT'LER (split_lock.json) ===")
print(f"  parca {len(ngt_of)} | aile {len(set(fam))} | geometri {len(set(geo))}")
print(f"  WORK aday {int(WORK.sum())} / {len(set(gpid[WORK]))} parca | KILITLI {len(LOCKED)} parca (P7'de acilir)")
...
for nm, mk in sets.items():
    for sub, smk in (("ALL", mk), ("WEI", mk & (MF == 1)), ("PXC", mk & (MF == 0))):
        ...
        print(f"  {nm:28s} {sub:4s}: aday {int(smk.sum()):5d} ({smk.sum()/max(gt,1):.2f}x) | "
              f"candidate-recall {rec:.3f} | oracle-F1 (mukemmel gate) {2*rec/(1+rec):.3f}")

# --- (6) baseline yeniden dogrulama ---
report = {}
print(f"\n=== (6) BASELINE YENIDEN DOGRULAMA (nested-CV esik) ===")
...
print("     egitiminde gorulmus). WORK satiri bu sizintiyi DISLAR = daha durust taban.")
