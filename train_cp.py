...
def three_way_split(ids, val_frac=0.2, test_frac=0.0, seed=0, group_keys=None,
    ...
    requested 0.2 across seeds 0-4 on the real corpus, making F1 across different
    ...
    SPECIFIC small family is starved: measured on cp_knn_v17 (real corpus, geometry
...
def train_and_eval(source, backbone="mlp", epochs=300, val_frac=0.2,
    ...
    # much harder object class. Drives are 88% of all CPs and the model fails on them
    # (F1~30%), dragging the metric AND diluting capacity away from connectors. With
    # the filter the whole model focuses on the connector task. None = keep all.
    if max_bbox_mm:
        kept = []
        for p in parts:
    ...
    # family_keys docstring; measured on cp_knn_v17: SIE/PXC/A-B/RIT were left with
    ...
    # ensemble of the replicas is ILLEGITIMATE on any of their val sets (each model
    # trained on parts that sit in another replica's val -> leakage). Default None
    ...
    # balance search depends on the corpus CONTENTS (and their order): add or drop
    ...
    # measured on a leaked val set. The checkpoint's config fingerprint cannot
    # catch this: seed/val_frac/split_group are all unchanged. So on a resume we
    ...
    # already found and fixed once this session for several diagnostic scripts).
    if best_path:
        try:
    ...
    _FAMILY_AUG_CAP = 8   # hard ceiling on the EXTRA (beyond n_aug) clones/part

    # EXPLICIT per-family boost ("PXC:3,foo:2"): multiplies that family's clone
    # count on top of the automatic sqrt scaling. The automatic boost equalizes
    ...
    # 3-part human-GT eval shows ML blind/near-miss on PXC push-in geometry,
    ...
    for p in parts:
        ...
        # question a deployed detector is judged on -- "does it invent connection points
        # on a part that has none?" -- because a zero-CP part can only ever produce FPs.
        # An empty prediction on an empty part is a legitimate, and perfect, result.
    ...
    train_config = {"val_frac": val_frac, "seed": seed,
                    ...
                    # trained on without re-reading the shell history.
                    "extra_sources": [str(s) for s in (extra_sources or [])],
                    "max_bbox_mm": max_bbox_mm, "keep_prefixes": keep_prefixes,
    ...
    if backbone == "mlp":
        ...
        # torch-only backbones (no native geometry deps) -> run on native Windows.
        ...
        # its graph structure differ (cp_regressor dispatches on `backbone`).
    ...
    # checkpoint can do. Report on the artifact that will actually be used.
    ...
    # trainer has copied what it needs into `prepared` (~30GB on corpus v8 -- the
    # difference between finishing and dying at 88% commit during prep). Those parts
    # can no longer be re-evaluated, so report on the ones that survive rather than
...
def main(argv=None):
    ...
    if hasattr(pre, "config") and pre.config:
        ...
        # --extra-source to REPLACE the config's sources would silently train on
