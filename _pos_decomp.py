# -*- coding: utf-8 -*-
"""Robot konum hatasini eksen-boyu (derinlik/konvansiyon) vs DIK (gercek nisan hatasi) ayristir.
Ops cache dolu oldugu icin hizli. Eslesenlerde: along = |proj on mfg InsertDir|, perp = kalan.
"""
...
def run(mfg, limit, models, dev):
    ...
    for m, pid, jf, stp in parts:
        try:
            ...
            pe = np.linalg.norm(diff-al[...,None]*Gd[None,:,:],axis=-1)  # DIK mesafe
            ...
            continue
    if along:
        print(f"  {mfg}: {len(along)} eslesme | eksen-boyu (derinlik/konvansiyon) medyan {np.median(along):.1f}mm"
              f" | DIK nisan hatasi medyan {np.median(perp):.1f}mm | yon medyan {np.median(dirs):.1f} deg")
    else:
