...
WHY THIS IS THE STRONGEST REMAINING LEVER (evidence, 2026-07-22):
  * The 379-part manufacturer arbiter says the product is F1 0.752 on PXC but **0.217 on Weidmueller**
    (recall 0.138; it emits ZERO CPs on 51% of WEI parts, vs 3% of PXC parts). That is a genuine
  ...
  * Human partial labels have a built-in flaw: openings the annotator did NOT mark are supervised as
    NEGATIVES (train_seg_extra's masked BCE pushes p_pos down on every unmarked vertex). Manufacturer
...
def _ray_hits(V, F, o, d, eps=1e-9):
    """Moller-Trumbore, vectorised over all triangles: distances t>0 where ray o+t*d hits the mesh."""
...
def mouth_from_contact(V, c, d, F=None, max_walk=60.0, step=0.4, tube=4.0, need=4):
    """The opening MOUTH = the point on the insertion axis where the surrounding MATERIAL ENDS.
...
def main():
    ...
    ap.add_argument("--list", default="", help="sadece bu dosyadaki parca no'lari (satir/bosluk ayrik)")
    a = ap.parse_args()
    ...
    for f in files:
        ...
        try:
            ...
            n_ok += 1; n_cp += len(G)
            if n_ok % 25 == 0: print(f"  {n_ok} parca ({time.time()-t0:.0f}s)", flush=True)
    ...
    print(f"\n{n_ok} parca etiketlendi ({n_cp} uretici CP), {n_skip} atlandi, {time.time()-t0:.0f}s")
    print(f"  train {tr} | test {n_ok-tr}")
    print(f"  uretici dagilimi: {dict(Counter(v['mfg'] for v in split.values()))}")
    print(f"  -> {a.out}/")
