# -*- coding: utf-8 -*-
"""G1: TRANSFER-KARARLI OZNITELIK SECIMI (+ G0 taban).

TESHIS (x1 + hafiza): gate EZBERLIYOR. Havuzlanmis 0.7584 ama gorulmemis ureticide
0.5968/0.6680. 13 taban ozellikten yalniz `votes` transfer ediyordu (AUC dususu 0.018,
digerleri 0.12-0.18) ve en ezberci UCUNU atmak +0.007 vermisti -- SISTEMATIK olarak hic
denenmedi. Su an 58 ham sutun var.

YONTEM: her ham sutunun TRANSFER KARARLILIGI = |AUC_ic - AUC_disi|
    AUC_ic  : ureticinin KENDI parcalarinda ayirt etme gucu
    AUC_disi: BASKA ureticinin parcalarinda ayni sutunun gucu
Dususu buyuk olan sutun, o ureticinin geometrisine OZEL bir kisayol ogrenmistir.
Sutunlari kararliliga gore sirala, en kararli k tanesiyle yeniden egit.

KILL (onceden yazildi): uretici-disi ORTALAMA +0.01 gelmezse VE havuzlanmis dusmezse
G1 KAPANIR. Kazanan k, havuzlanmisi 0.005'ten fazla dusurmemelidir.
"""
import io
import json
import sys
...
def auc(v, pos):
    """Mann-Whitney AUC. Bagli degerleri DOGRU isler (kisa-yol surumu sismisti)."""
...
def main():
    ...
    # --- TRANSFER KARARLILIGI: her uretici icin ic-vs-disi AUC farki
    ureticiler = [k for k in np.unique(mfg)
                  if (keep & (mfg == k)).sum() >= 500 and (keep & (mfg != k)).sum() >= 500]
    print(f"kararlilik uretici sayisi: {len(ureticiler)} ({[D['kod'][k] for k in ureticiler]})")
    ...
    sira = np.argsort(dus)                     # en KARARLI once
    ...
    # URUNUN karar yolu tam sutun kumesini bekler -> alt kume icin sarmalayici
    import wire_gate
    _asil_skor = wire_gate.karar_skoru

    def skor_altkume(m, Xh):
        if isinstance(m, dict) and "_sut" in m:
            Xh = np.asarray(Xh, float)[:, m["_sut"]]
        return _asil_skor(m, Xh)
    wire_gate.karar_skoru = skor_altkume
    ...
    for k in SON:
        print(f"{k:<6}{SON[k]['havuzlanmis']['tespit']:>13.4f}"
              ...
              f"{SON[k]['havuzlanmis']['tespit']-taban['havuzlanmis']['tespit']:>+11.4f}"
              f"{SON[k]['_URETICI_DISI_ORT']-taban['_URETICI_DISI_ORT']:>+10.4f}")

    aday = [k for k in SON if k != 58
            and SON[k]["_URETICI_DISI_ORT"] - taban["_URETICI_DISI_ORT"] >= 0.01
            and SON[k]["havuzlanmis"]["tespit"] - taban["havuzlanmis"]["tespit"] >= -0.005]
    if aday:
        en = max(aday, key=lambda k: SON[k]["_URETICI_DISI_ORT"])
        lo, hi = T.ga(PARCA[58], PARCA[en], "havuzlanmis")
        print(f"\nKAZANAN k={en} | uretici-ort {taban['_URETICI_DISI_ORT']:.4f} -> "
              f"{SON[en]['_URETICI_DISI_ORT']:.4f} | havuzlanmis GA[{lo:+.4f},{hi:+.4f}]")
    else:
        en = None
        print("\nKILL: hicbir k uretici-disi ORT'de +0.01 vermedi -> G1 KAPANDI")
    ...
    print("makbuz -> results/g1_transfer_secim.json")
