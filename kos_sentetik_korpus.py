...
`sentetik_klemens.uret()` parametrik klemens ureti; bu betik onu urun
boru hattinin BIRINCI asamasindan gecirir:

    uret -> remesh(6000) -> DiffusionNet cikarimi -> npz(V, F, pbs)
                                                  -> kayit(G, Gd, P, Pd, diag)

`P`/`Pd` (urun adaylari) segmentasyondan cikarilir; boylece sentetik parca
gercek parcayla AYNI aday uretim yolunu kullanir. Aksi halde sentetik veri
"kolay" olur ve olculen kazanc sahte cikar.

DUMAN TESTI ZORUNLU: uretilen npz'de segmentasyonun GT agizlarinda gercekten
ateşleyip atesle(me)digine bakilir. Model sentetik geometride hic ateslemiyorsa
korpusun egitim degeri yoktur ve bunu SONRA degil SIMDI bilmek gerekir.
...
def main():
    ...
    if os.path.exists(KAYIT):
        ...
        print(f"onbellek: {len(kayit)} kayit", flush=True)
    ...
    for t in range(TOHUM0, TOHUM0 + N_PARCA):
        ...
        # URUN ADAYLARI: gercek parcayla AYNI yol
        try:
            ham = urun_genis.adaylar(V, F, pbs) \
                if hasattr(urun_genis, "adaylar") else None
        except Exception:                            # noqa: BLE001
            ham = None
        if ham is None:
            # geri dusus: yuksek olasilikli tepelerden kumeleme
        ...
        if yazilan % 25 == 0:
            ...
            print(f"  {yazilan} yazildi {h:.2f}s/parca "
    ...
    if tani:
        ...
        oran = np.median(a[:, 0]) / max(np.median(a[:, 1]), 1e-6)
        print(f"  oran                          : {oran:.2f}x")
        print("  OKUMA: oran ~1 ise model sentetik geometride AYIRT ETMIYOR,")
        print("         korpusun egitim degeri yoktur -- SIMDI bilinmeli.")
        json.dump({"n": yazilan,
                   "gt_olasilik": float(np.median(a[:, 0])),
                   "zemin": float(np.median(a[:, 1])), "oran": float(oran),
                   "not": "Sentetik korpus duman testi. Oran ~1 ise korpusun "
                          "egitim degeri yok. D7'ye BAKILMADI."},
