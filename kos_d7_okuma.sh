...
for i in $(seq 0 $((N-1))); do
  ( URUN_P6=0 URUN_GENIS=1 DOG_POZ=1 DOG_SHARD="$i/$N" \
    ...
    python probe_dagitim_verify.py > "results/_d7t_$i.log" 2>&1 ) &
done
wait
...
# uzerine dogrulanmamis a son islem koymak, olculen seyden baska a sey
# dagitmak olurdu.
echo
...
for i in $(seq 0 $((N-1))); do
  ( URUN_P6=1 DOG_POZ=0 DOG_SHARD="$i/$N" \
    ...
    python probe_dagitim_verify.py > "results/_d7p_$i.log" 2>&1 ) &
done
wait
...
# Manset this koldan SECILMEZ -- sinava bakip yapilandirma secmek, sinavdan ayar
# cekmektir.
echo
...
for i in $(seq 0 $((N-1))); do
  ( URUN_P6=1 DOG_POZ=1 DOG_SHARD="$i/$N" \
    ...
    python probe_dagitim_verify.py > "results/_d7pp_$i.log" 2>&1 ) &
done
wait
