# -*- coding: utf-8 -*-
"""B-rep oto-etiketinin KOR OLDUGU parcalar -- insan etiketi buraya gitmeli.

Oto-etiket GT'lerin yalnizca %35.2'siyle eslesti. Eslesmeyen %64.8, silindir
olarak modellenmemis girisler (kare agiz, yay-kelepce). Havuz recall'unu dibe
ceken markalar da onlar. Insan emegini otomatigin ZATEN yaptigi yere degil,
YAPAMADIGI yere koymak gerekir.

Cikti: parca basina GT eslesme orani; en dusukler onceliklidir.
"""
...
for p, m, n, o in sat:
    mk[m].append(o)
print(f"{'marka':<8} {'parca':>6} {'ort eslesme':>12} {'kor (<0.2)':>11}")
...
print(f"\nKOR parca (eslesme<0.2, GT>=4): {len(kor)}")
...
json.dump({"n_kor": len(kor), "ilk60": [list(x) for x in kor[:60]],
           "not": "B-rep oto-etiketinin eslestiremedigi parcalar -- insan etiketi "
                  "ONCELIGI. Otomatigin zaten yaptigi yere emek harcanmaz."},
          open("results/p2_kor_nokta.json", "w"), indent=1)
print("-> results/p2_kor_nokta_pidler.txt (ilk 60)")
