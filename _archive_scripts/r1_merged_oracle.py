...
robot = 0.777 x detection (measured). Carpani yukseltmek gecis oranini yukseltmek demek:
...
hedef =0.90 *tesp 
print (f"\n%90 gecis orani for robot {hedef :.4f} gerekir; birlesik oracle {S ['IKISI']:.4f}")
print ("VERDICT:","%90 MUMKUN (oracle asiyor)"if S ["IKISI"]>=hedef else "%90 SOZLUKLERLE ULASILAMAZ")
json .dump ({k :float (v )for k ,v in S .items ()}|{"detection":tesp ,
"gecis_ikisi":S ["IKISI"]/tesp ,"kayma_medyan":float (np .median (K )),
"kayma_p90":float (np .percentile (K ,90 ))},
io .open ("results/r1_merged_oracle.json","w"),indent =1 )
print ("receipt -> results/r1_merged_oracle.json")
