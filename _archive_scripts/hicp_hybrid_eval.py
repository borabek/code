...
Mimari: tipik part -> mevcut kilitli score (6k union + prob:RF+SetNet, DOKUNULMADI)
        high-CP part -> AGRESIF multires pool + zengin RF gate (new arm)
High-CP gate egitimi: WEI agresif pool (2750 candidate, ayri parts) + 18-part havuzunda
leave-one-part-out -> test parcasinin verisi egitime ASLA girmez.
Yonlendirme two modda olculur:
  (a) ORACLE (GT>=11 biliniyor) -> upper-boundary, VARSAYIMSAL etiketli
  (b) GIRDI-BAZLI (candidate count + part diyagonali esigi, GT'siz) -> deploy edilebilir
...
sc6 =(d ["rf"]+d ["setnet"])/2 # kilitli final score (tipik for)
THR6 =0.34 
...
XW ,YW =RICHX (aw ),aw ["y"]
print (f"18-part agresif pool: {len (Y18 )} candidate | candidate-ceiling recall {int (Y18 .sum ())}/{int (a18 ['ngt'].sum ())} "
...
def prf (tp ,nk ,gt ):
    ...
    # high-CP kolunda threshold: WEI havuzunda (ayri data) F1-optimal -> 18'e uygula (sizintisiz)
gtW =int (aw ["ngt"].sum ())
...
print (f"HIGH-CP kolu (18 part, LOO): P {p18v :.3f} R {r18v :.3f} F1 {f18v :.3f}   (6k'da F1 0.458 idi)")

# --- HIBRIT ALL (WORK): tipik = kilitli score; high-CP = new arm ---
def hybrid (route_hi_gids ):
    TP =NK =GT =0 
    for g in np .unique (G [WORK ]):
        pid =pid_of_g [int (g )]
        if int (g )in route_hi_gids :
            continue # high-CP kolu below toplu
...
# (b) GIRDI-BAZLI yonlendirme: candidate count >= T VE/VEYA diag -> high-CP kabul
cand_ct ={int (g ):int (((G ==g )&WORK ).sum ())for g in np .unique (G [WORK ])}
best_route =None 
for T in (8 ,9 ,10 ,12 ,14 ):
    route ={g for g in cand_ct if cand_ct [g ]>=T }
    # wrong yonlendirilen tipikler high-CP koluna DUSMEZ (pool absent) -> onlari 6k'da tut, only
    # real-hi & route kesisimi new kola gider; kacan hi'lar 6k'da kalir
    ...
    for g in np .unique (G [WORK ]):
        ...
        m =WORK &(G ==g );k =m &(sc6 >=THR6 )
        TP +=int (Y [k ].sum ());NK +=int (k .sum ());GT +=ngt [int (g )]
        # caught kolu: 18-havuzdan only caught parts (grup->pid map'i with candidate maskesi)
    ...
    p ,rr ,f =prf (TP ,NK ,GT )
    tag =f"T>={T }: yakalanan {len (caught )}/18, wrong-alarm {len (wrong )}"
