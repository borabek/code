# -*- coding: utf-8 -*-
"""R4: YON + KONUM SECICISINI BIRLIKTE OLC (R2 + R3 single kosuda).

R1 BULGUSU: two dictionary TAMAMLAYICI -- ayri ayri oracle %87.5 and %81.5, BIRLIKTE %94.1
(robot 0.7133). Yani ayri ayri olcmek yaniltiyor; birlikte olculmeli.

R3 DUZELTMESI: direction secicisinin etiketi ISARETLI yapildi (v.Gd >= cos10). Onceki unsigned
version axis metriginde +0.0319 verip FIZIKSEL metrikte sifir birakmisti -- because dictionary
BILEREK +- ciftleri iceriyor and unsigned etiketle +a with -a AYNI etiketi aliyordu.
Egitim label orani %31.4 -> %12.7 dustu; this BEKLENEN (residual ciftin only biri correct).

Bu betik measurement kumesinin sozluklerini KENDI kurar (c4 onbellegi parite duzeltmesinde
silindi) and DORT kolu olcer:
    baseline / only direction / only konum / IKISI
Her biri hem EKSEN (unsigned) hem FIZIKSEL (signed) robot metrigiyle.

KILL: robot +0.01 VE GA sifiri disliyor. Tespit yapisal as does not change.
"""
...
import thesis_remesh ,wire_gate ,measure_set 
from big_arbiter import eligible 
from infer_step_cp import step_to_mesh 
from sina_cluster import esle ,f1w 
from d1_direction_distribute import sozluk_kur ,satirla ,_birim 
...
if os .path .exists (ONB ):
    PARCA =pickle .load (open (ONB ,"rb"));print (f"dictionary onbellekten: {len (PARCA )} part",flush =True )
else :
    PARCA ={};t0 =time .time ()
    for kk ,r in enumerate (DER ,1 ):
        if kk %25 ==0 :print (f"  dictionary {kk }/{len (DER )} {time .time ()-t0 :.0f}s",flush =True )
...
print (f"direction selector: {'VAR'if YS else 'YOK'} | konum selector: {'VAR'if KS else 'YOK'}")

def uygula (direction ,konum ):
    rob ,rbi ,gg =[],[],[]
    for r in DER :
        d_ =PARCA .get (r ["pid"])
        ...
        if d_ is None :P =np .zeros ((0 ,3 ));Pd =np .zeros ((0 ,3 ))
            ...
            if direction and YS is not None :
...
print (f"\n{'arm':<24}{'robot (axis)':>15}{'robot (FIZIKSEL)':>18}")
print (f"{'baseline':<24}{f1w (r0 ):>15.4f}{f1w (i0 ):>18.4f}")
print (f"{'+ ISARETLI direction selector':<24}{f1w (r1 ):>15.4f}{f1w (i1 ):>18.4f}")
lo ,hi =T2 .ga (i0 ,i1 ,gg )
d =f1w (i1 )-f1w (i0 )
print (f"\nFIZIKSEL metrikte diff: {d :+.4f}  GA[{lo :+.4f},{hi :+.4f}] "
f"{'GERCEK'if (lo >0 or hi <0 )else 'noise'}")
lo2 ,hi2 =T2 .ga (r0 ,r1 ,gg )
print (f"EKSEN   metrikte diff: {f1w (r1 )-f1w (r0 ):+.4f}  GA[{lo2 :+.4f},{hi2 :+.4f}]")
...
io .open ("results/r4_merged_selector.json","w"),indent =1 )
print ("receipt -> results/r4_merged_selector.json")
