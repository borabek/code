...
Usage:
  python wscad_download.py --list WSCAD_list_phoenix.txt --mid 63 --out all_wscad_stp
"""
import argparse
import os
import re
...
def main():
    ...
    print(f"list: {len(arts)} | already present: {len(arts) - len(todo)} | indirilecek: {len(todo)}",
          flush=True)
    ...
    with sync_playwright() as p:
        ...
        for i, art in enumerate(todo, 1):
            try:
                ...
                if not btn.count():
                    skip += 1
                    print(f"[{i}/{len(todo)}] {art}: part/indirme none", flush=True)
                    continue
                btn.click()
    ...
    print(f"\nBITTI: {ok} indi, {skip} atlandi (STEP none), {fail} error "
