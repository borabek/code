...
yaricapli disk boyuyordu. Olculdu ki that yolla egitilen g10, unseen markada
...
tasiyor, duzlemsel opening own halkasini. Etiket fixed a diskle not,
...
YONTEM (part basina):
 ...
 2. Eslesen agzin GERCEK disk bolgesini paint: eksene dik uzaklik <= R*`R_PAY`,
...
def paint (V ,G ,Gd ,cyl ,acik ):
    """Doner: (label dizisi, matched GT count, total GT)."""
    ...
    for j in range (len (G )):
        ...
        L [k ]=CE 
        matched +=1 
    return L ,matched ,len (G )


def obj_yaz (path ,V ,F ):
...
def main ():
    ...
    top ={"written":0 ,"skipped":0 ,"gt":0 ,"matched":0 }
    t0 =time .time ()
    for ad ,ob ,cylf ,acf ,kay in isler :
        ...
        for pid ,r in sorted (kay .items ()):
            ...
            L ,es ,ng =paint (V ,G ,np .asarray (r ["Gd"],float ),
    ...
    print (f"\nYAZILAN {top ['written']} | ATLANAN {top ['skipped']}")
    print (f"GT eslesme: {top ['matched']}/{top ['gt']} "
    f"(%{100 *top ['matched']/max (top ['gt'],1 ):.1f})")
    json .dump ({"written":top ["written"],"skipped":top ["skipped"],
    "gt":top ["gt"],"matched":top ["matched"],
    ...
    open ("results/p2_brep_label.json","w"),indent =1 )
    print (f"-> {a .out_ }")
