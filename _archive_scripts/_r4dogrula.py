...
print (f"\n{'':<26}{'detection':>9}{'robot':>9}{'fiziksel':>10}")
...
print (f"  TESPIT degisimi: {f1w (d1 )-f1w (d0 ):+.6f}  (yapisal as 0 olmali)")
# URETICI-DISI
X =ek ["Z"];y =ek ["y"];pid =ek ["pid"];mfg =ek ["mfg"];keep =ek ["keep"]
print (f"\n{'manufacturer-disi':<26}{'detection':>9}{'robot':>9}{'fiziksel':>10}")
UD ={}
for k in np .unique (mfg ):
    ...
    UD [str (k )]={"baseline":{"detection":f1w (da ),"robot":f1w (ra ),"fiziksel":f1w (ia )},
    "new":{"detection":f1w (db ),"robot":f1w (rb ),"fiziksel":f1w (ib )}}
...
dus =[k for k ,v in UD .items ()if v ["new"]["fiziksel"]<v ["baseline"]["fiziksel"]-0.002 ]
print (f"\nFIZIKSEL metrikte DUSEN manufacturer: {dus if dus else 'YOK'}")
print ("DECISION:","DAGITILABILIR"if not dus and abs (f1w (d1 )-f1w (d0 ))<1e-6 else "NOTE")
json .dump ({"pool":{"baseline":{"detection":f1w (d0 ),"robot":f1w (r0 ),"fiziksel":f1w (i0 )},
"new":{"detection":f1w (d1 ),"robot":f1w (r1 ),"fiziksel":f1w (i1 )}},
