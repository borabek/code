...
Kullanim:  python headline_050.py results/<receipt>.json [results/<baseline>.json]

Tek a number vermez. Bir sayinin sisik olup olmadigini anlamak for gereken
each seyi yan yana koyar:

  1. MIKRO robot F1 (headline) and detection -- havuzlanmis TP/FP/FN uzerinden.
...
tes:[tp,fp,fn]}). `probe_deploy_verify.py` and `probe_d6_product.py` writes.
"""
import json 
import os 
import sys 
...
def main ():
    ...
    if sy .get ("cagri"):
        ...
        f"{sy .get ('rejim_disi',0 )} | tablo none {sy .get ('tablo_yok',0 )}")
        if p6 ==0 :
