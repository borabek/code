# -*- coding: utf-8 -*-
"""T16: GORELI ORAN rejime according to ayarlanmali mi? (low-CP vs very-CP)

DAGITILAN rule: candidate, parcasindaki most high skorun 0.5 katini gecmeli VE 0.25 tabanini asmali.
Tek ratio, tum parts. Ama two regime very different:
  low-CP  (<8 CP, korpusun %89.5'i) -- few sayida real opening, KESINLIK onemli
  very-CP    (>=8 CP)                  -- very sayida real opening; 0.5 orani very SIKI may be,
                                         because most guclu adayin yarisini gecemeyen GERCEK CP'ler
                                         elenir. (Ayni sebeple threshold already 0.40/0.35 idi.)
Rejimi `robot_cp._highcp_router` GEOMETRIDEN belirliyor (manufacturer metadata'si YOK, aile-disi AUC 0.994).

TASARIM: candidates and skorlar BIR KEZ; after ratio double-ligi bedava taranir. Hem TANIDIK hem
GORULMEMIS URETICI raporlanir -- this gecenin ekseni transfer.

KILL (onceden yazili): a (dusuk_oran, cok_oran) cifti, familiar tespiti DUSURMEDEN
(>= -0.005) unseen manufacturer EN KOTU durumunu artirmazsa ALINMAZ.
"""
...
def main ():
    ...
    print (f"turetme bitti | router: {hi } very-CP / {len (DER )-hi } dusuk-CP",flush =True )
    ...
    S ={"familiar":skorla (~np .isin (tr_grp ,list (tg )),DER )}
    ...
    for od ,oc in CIFT :
        ...
        ek =min (v [a ]for a in ad if a !="familiar")
        print (f"{f'{od } / {oc }':<16}"+"".join (f"{v [a ]:>11.4f}"for a in ad )+f"{ek :>13.4f}",
        flush =True )
        out [f"{od }/{oc }"]={"degerler":v ,"en_kotu_uretici":ek }
    t =out ["0.5/0.5"]
    print ("\nKARAR (kill: familiar >= -0.005 VE most kotu manufacturer artmali):")
    for k ,v in out .items ():
        if k =="0.5/0.5":
            continue 
        dt =v ["degerler"]["familiar"]-t ["degerler"]["familiar"]
        de =v ["en_kotu_uretici"]-t ["en_kotu_uretici"]
        print (f"  {k :<12} familiar {dt :+.4f} | en kotu {de :+.4f} -> "
        f"{'GECTI'if (dt >=-0.005 and de >0 )else '-'}")
    json .dump (out ,open ("results/t16_regime_oran.json","w"),indent =1 )
    print ("receipt -> results/t16_regime_oran.json")
