...
Egitim verisi: `r16_selector_data.npz` -- 8543 row, 900 EGITIM parcasi (exam kumesi
and ikizleri DISARIDA). Her row a EKSEN ADAYI; label = this candidate GT yonune <=10 derece mi.

TEZ SADAKATI: `v_o` KONUMU does not change. Yalniz direction secilir; tezin ham `v_o - v_s`'si
adaylardan biri ("mevcut") as kalir and sonuc onunla YAN YANA raporlanir.
"""
...
from r15_axis_selector import candidates 
from r16_selector_data import ozn ,AD 

d =np .load ("results/r16_selector_data.npz")
X ,y =d ["X"],d ["y"]
clf =RandomForestClassifier (n_estimators =300 ,min_samples_leaf =5 ,n_jobs =-1 ,random_state =0 )
clf .fit (X ,y )
print (f"selector egitildi: {len (X )} satir, correct candidate %{100 *y .mean ():.0f}")
...
def kos (D ,ad ):
    ...
    for r in D :
        ...
        for q ,a_ ,b_ in sorted ((pe2 [a ,b ],a ,b )for a in range (len (P ))for b in range (len (G ))):
            ...
            up .add (a_ );ug .add (b_ )
            g =Gd [b_ ];cand =candidates (mesh ,cyl ,P [a_ ],nP [a_ ])
    ...
    diff =100 *((B <=10 ).mean ()-(A <=10 ).mean ())
    print (f"  diff {diff :+.1f} puan -> {'GECTI'if diff >=5 else 'GECMEDI'} (GO: >=+5)")
    return diff 
...
kos (SIN ,"GORULMEMIS exam");kos (D194 ,"194luk (gerileme kontrolu)")
