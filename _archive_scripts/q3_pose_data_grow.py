# -*- coding: utf-8 -*-
"""Q3: POSE training verisini TAM KORPUSTAN cikar (network cikarimi YOK).

Q2'de pose head robot-hazira +0.0222 getirdi but GA sifiri iceriyor -- only 1068 satirla
egitiliyordu (measurement kumesinin 194 parcasindan). Ogrenilmis a kafayi guclendirmenin most
guvenilir yolu more very data.

TAM KORPUS for gereken parts ZATEN diskte:
    ozellikler (X22+XR)  -> results/zengin_parite.npz
    candidate point/yonleri   -> results/gate_regrow_data_parite.npz  (pts, dirs)
    GT                   -> manufacturer JSON'lari
Eksik which is single sey GT'nin MESH CERCEVESINE tasinmasi (align_frames). O da ham STEP mesh'i
instead of ONBELLEKTEKI REMESH with yapilabiliyor -- DOGRULANDI: 18 parcada medyan difference 0.0000mm,
maks 0.0191mm. Yani gmsh cagrisi and network cikarimi GEREKMIYOR.

Cikti: results/pose_veri.npz (X58, hedef4, pid, geo) -- only TESPITTE ESLESEN candidates.
"""
import io 
import json 
import os 
...
def main ():
    ...
    # B-rep GRAF sutunlari: gate'te uctan uca OLU output (WEI -0.031) but POSE tahmini AYRI a
    # gorev -- channel kor mu/gecis mi, eseksenli zincir present mi bilgisi konum artigi for
    ...
    with io .open ("results/_strict_geometry_keys.json",encoding ="utf-8")as f :
        ...
        # Iki npz AYNI candidate ureticisinden geldi; part basina candidate SAYISI eslesmeli.
    ortak =[p for p in np .unique (zpid )if (ppid ==p ).sum ()==(zpid ==p ).sum ()]
    print (f"zengin {len (np .unique (zpid ))} part | parite {len (np .unique (ppid ))} | "
    f"candidate sayisi ESLESEN {len (ortak )}",flush =True )

    RX ,RY ,RP ,RG =[],[],[],[]
    skipped =0 
    for k ,pid in enumerate (ortak ,1 ):
        if k %200 ==0 :
            print (f"  {k }/{len (ortak )} (skipped {skipped })",flush =True )
        ...
        # Bolum 21.53: pose head large correction ONERMIYOR because training
        # verisi only ZATEN ESLESMIS adaylardan kuruluyor -- eslesmis
        # adayin artigi tanim geregi kucuktur. Bu a SECIM YANLILIGI.
    ...
    print (f"\n-> {OUT } | {len (RY )} matched candidate / {len (set (RP ))} part / {len (set (RG ))} grup")
    print (f"   lateral deviation: medyan {np .median (yan ):.2f}mm | <=2mm {np .mean (yan <=2 ):.1%}")
    print (f"   direction deviation  : medyan {np .median (aci ):.2f} deg | <=10 deg {np .mean (aci <=10 ):.1%}")
    print (f"   (Q1'de 1068 satir vardi -> {len (RY )/1068 :.1f} fold)")
