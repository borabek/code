...
adayi LOOSE detection etiketiyle (lateral <= max(3mm,%6), angle serbest) egitilmis; robot-hazir
...
def main ():
    ...
    sin_mfg =tum -DEV_MFG 
    print (f"\n{'threshold':<8}{'DEV detection':>12}{'DEV robot':>11}")
    en ,en_r ,izg =None ,-1.0 ,{}
    for e in (0.30 ,0.40 ,0.50 ,0.60 ,0.70 ):
        tf ,rf =olc (DEV_MFG ,e )
        izg [str (e )]={"detection":tf ,"robot":rf }
        print (f"{e :<8.2f}{tf :>12.4f}{rf :>11.4f}")
        if rf >en_r :
            en_r ,en =rf ,e 
    st ,sr =olc (sin_mfg ,en )
    print (f"\n--- SINAV YARISI (TEK ATIS, threshold {en :.2f}) ---")
    print (f"  detection {st :.4f} | robot {sr :.4f}")
    print (f"  KIYAS (mevcut yigin, same yari): detection 0.5394 | robot 0.2676")
    karar ="DAGIT"if sr >0.2676 else "GERI AL"
    print (f"\nKARAR: {karar }")
    with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
        json .dump ({"izgara":izg ,"dev_esik":en ,"exam":{"detection":st ,"robot":sr },
