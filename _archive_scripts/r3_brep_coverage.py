...
HIPOTEZ (R0+R1'den): CP'nin ekseni three kaynaktan gelebiliyor --
    ...
    (c) koordinat yuvarlamasi   : (a) and (b) susarsa remaining TOHUM -> egik agizda 90 dereceye up to error
...
OLCULEN: matched each GT noktasi for adayin `brep_r` degeri (gate sutunu 13; 0 = silindir
...
def main ():
    ...
    present =br >0 
    print (f"{len (SAT )} matched GT noktasi | B-rep silindiri ESLESEN: {present .sum ()} "
    f"({present .mean ():.1%}) | eslesmeYEN: {(~present ).sum ()} ({(~present ).mean ():.1%})")

    print (f"\n{'grup':<22}{'n':>6}{'lateral med':>11}{'angle med':>10}{'lateral<=2':>10}"
    f"{'angle<=10':>9}{'IKISI':>8}")
    for ad ,i in (("B-rep ESLESTI",present ),("B-rep YOK",~present )):
        if not i .any ():
            continue 
    ...
    dik =ac >45 
    if dik .any ():
        print (f"  aci>45 olanlarin B-rep eslesme orani : {present [dik ].mean ():.1%}")
        print (f"  aci<=45 olanlarin                    : {present [~dik ].mean ():.1%}")

    print ("\nEGER B-rep kapsamasi %100 OLSAYDI (matched grubun oranlariyla):")
    if present .any ():
        y2 =(ya [present ]<=2 ).mean ();a2 =(ac [present ]<=10 ).mean ()
        i2 =((ya [present ]<=2 )&(ac [present ]<=10 )).mean ()
        su =((ya <=2 )&(ac <=10 )).mean ()
        print (f"  ikisi birden: {su :.1%} -> {i2 :.1%}  (+{(i2 -su )*100 :.1f} puan)")
        print (f"  robot-hazir 0.65 for gereken: %87.4")

    print (f"\n{'regime':<10}{'n':>6}{'B-rep present':>11}{'IKISI OK':>10}")
    for rj in ("low","very"):
        i =np .array ([x ["regime"]==rj for x in SAT ])
        if i .any ():
            print (f"{rj :<10}{int (i .sum ()):>6}{present [i ].mean ():>11.1%}"
            f"{((ya [i ]<=2 )&(ac [i ]<=10 )).mean ():>10.1%}")

    with open ("results/r3_brep_coverage.json","w",encoding ="utf-8")as f :
        json .dump ({"n":len (SAT ),"brep_kapsama":float (present .mean ()),
        "eslesen_ikisi_ok":float (((ya [present ]<=2 )&(ac [present ]<=10 )).mean ())if present .any ()else None ,
        "eslesmeyen_ikisi_ok":float (((ya [~present ]<=2 )&(ac [~present ]<=10 )).mean ())if (~present ).any ()else None ,
        "dik_brep_orani":float (present [dik ].mean ())if dik .any ()else None },f ,indent =1 )
    with open ("results/r3_satir.pkl","wb")as f :
        pickle .dump (SAT ,f )
    print ("\nmakbuz -> results/r3_brep_coverage.json")
