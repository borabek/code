...
Iki cluster present:
  ...
  DAMGALA  -- kopyalamak for extra large olanlar (probability onbellekleri).
              Bunlar geri ALINAMAZ; degisirlerse `rollback.py` BAGIRIR.
              Kural: this betikten after this dosyalara YAZMA.

Kullanim:  python kontrol_point.py [label]
"""
import glob 
import hashlib 
import json 
...
KOPYALA =[
"cp_config.json",
"results/metric_dondurulmus.json",
"results/split3.json",
...
# --- only parmak izi (file count + total bayt + name/size summary) ----
PARMAK_IZI_DIZIN =[
...
def dizin_parmak_izi (d ):
    """Icerigi okumadan name+size summary. Kazara degisimi yakalar, ucuzdur."""
...
def main ():
    ...
    print (f"\nKONTROL NOKTASI: {kn }")
    print (f"  git      : {man ['git']}")
    print (f"  kopya    : {len (man ['kopya'])} file / {top /1048576 :.0f} MB")
    print (f"  damga    : {len (man ['damga'])} file (kopyasiz, degisim yakalanir)")
