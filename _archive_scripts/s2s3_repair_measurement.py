# -*- coding: utf-8 -*-
"""S2+S3: FIX OPERATORLERINI OLC -- kusur gercekten gitti mi, METRIGE ne became?

S1 three operator yazdi (body ici -> agza tasi, duvara yapisik -> merkeze cek,
onu closed -> yonu cevir). Bu betik onlari 194 parcada uygular and IKI SORUYU separates:

  S2  DEFECT GITTI MI? Onarim sonrasi bayraklar YENIDEN olculur. Bir kusuru duzeltirken
      BASKASINI yakmak mumkun (body disina tasirken mouth duvarina yapismak like).
  S3  METRIGE NE OLDU? Kusur oraninin dusmesi TEK BASINA yetmez. GECIS SAYILIR:
          TP -> FP  (onarim correct CP'yi bozdu)   -> ZARAR
          FP -> TP  (onarim wrong CP'yi kurtardi) -> FAYDA
      Net etki detection and robot F1'de raporlanir.

C1 BULGUSU (this betigin cercevesini belirledi): kusurlar SON ISLEMDE URETILMIYOR.
    HAM 249 -> POZ 218 (-31) -> TAM 222
Poz kafasi kusuru AZALTIYOR. Yani onarim, zincirin own urettigi a hatayi not,
ADAY/GATE duzeyinden MIRAS ALINAN a hatayi hedefliyor.

KILL (onceden yazildi): FP->TP >= TP->FP VE detection >= -0.005.
"""
import io 
import json 
import os 
...
def main ():
    ...
    if os .path .exists (ONB ):
        with open (ONB ,"rb")as f :
            R =pickle .load (f )
        print (f"onbellekten: {len (R )} part",flush =True )
    else :
        R ={}
        t0 =time .time ()
    ...
    for ad ,robot in (("TESPIT",False ),("ROBOT(FIZ)",True )):
        ...
        for r in DER :
            ...
            kw =dict (signed =True )if robot else {}
            a =(2.0 ,10.0 ,False )if robot else (0.0 ,180.0 ,True )
            tp0 ,fp0 ,fn0 ,b0 =match_greedy (P0 ,D0 ,G ,Gd ,r ["diag"],*a ,**kw )
            tp1 ,fp1 ,fn1 ,b1 =match_greedy (P1 ,D1 ,G ,Gd ,r ["diag"],*a ,**kw )
        ...
        if not robot :
            ...
            KILL =(f2t ,t2f ,gecti )
    print (f"\nKILL: FP->TP ({KILL [0 ]}) >= TP->FP ({KILL [1 ]}) VE detection >= -0.005 -> "
    f"{'GECTI'if KILL [2 ]else 'GECMEDI'}")

    with io .open ("results/s2s3_repair.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/s2s3_repair.json")
