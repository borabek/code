...
dropped_wire =int (((y ==1 )&dropped ).sum ())# gate yanlislikla real teli attigi (mfg-matched)
...
print (f"  manufacturer CP total: {n_gt }")
print (f"  kept TP (correct tel):                 {kept_tp }")
print (f"  tool_fp (gate ATTI, dusuk score):     {tool_fp }   <- gate'in temizledigi tool aglari")
print (f"  kept FP (unlisted-wire VEYA artik):  {unlisted_or_resid }   <- gate'ten passing but mfg-eslesmez")
print (f"  dropped_real_wire (gate YANLIS atti):{dropped_wire }   <- gate maliyeti (gercek tel gitti)")
print (f"  total FN (kacan tel):               {missed_fn }")
