# -*- coding: utf-8 -*-
"""U2: topoloji YARICAPI (R) UCTAN UCA -- familiar + each two unseen manufacturer.

T19 candidate duzeyinde R=12'yi +0.0100 with onde buldu (R=6 -> 8 -> 12 tutarli artis). Aday duzeyi
this night UC KEZ yaniltti, that yuzden karar BURADA veriliyor.

TASARIM (single variable R olsun diye):
  ...
  * each R own training npz'siyle eslesir (u1_topo_r_re.py with uretildi, R=6'da BIREBIR
    dogrulandi) -- EGITIM and TEST AYNI R'de olmak ZORUNDA

KILL (onceden yazili): new R, (a) familiar detection F1'de R=6'ya according to >= -0.01 kalmali VE
(b) unseen manufacturer EN KOTU durumunu ARTIRMALI. Ikisi birden olmazsa DAGITILMAZ.
...
def main ():
    ...
    for r_ ,f_ in NPZ_OF .items ():
        assert os .path .exists (f_ ),f"R={r_ } for training verisi YOK: {f_ }"
    ...
    print (f"\n{'split':<24}{'R':>6}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    R ={}
    for R_ in R_LISTE :
    ...
    key_ =sorted ({k [1 ]for k in R if k [1 ]!="familiar"})
    taban_tan =f1w (R [(6.0 ,"familiar")][0 ])
    taban_kotu =min (f1w (R [(6.0 ,a )][0 ])for a in key_ )
    print (f"KARAR (baseline R=6.0: familiar {taban_tan :.4f} | unseen en kotu {taban_kotu :.4f})")
    kazanan ,en_iyi =None ,None 
    for R_ in R_LISTE :
        if R_ ==6.0 :
            continue 
        dt =f1w (R [(R_ ,"familiar")][0 ])-taban_tan 
        ek =min (f1w (R [(R_ ,a )][0 ])for a in key_ )
        gecti =(dt >=-0.01 )and (ek >taban_kotu )
        print (f"  R={R_ :<5} familiar {dt :+.4f} | en kotu {taban_kotu :.4f} -> {ek :.4f} "
    ...
    open ("results/u2_topo_r_end_to_end.json","w"),indent =1 )
    pickle .dump (R ,open ("results/u2_parca.pkl","wb"))
    print ("receipt -> results/u2_topo_r_end_to_end.json")
