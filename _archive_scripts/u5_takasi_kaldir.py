...
U4 olctu: D (ham+zskor) unseen ureticide most kotu durumu 0.4832 -> 0.5702 cikariyor
...
Takasi kaldirmayi denemeye value, because bedelin kaynagi belli not. Uc hipotez, three arm:
  ...
  E  z-score YALNIZ ezberleyen sutunlara
  ...
  G  TOPLULUK: ham gate with z-score gate'in olasiliklarinin ortalamasi
     Gerekce: ensemble usually kazancin cogunu korur, kaybin a kismini geri gives.
  H  z-score YALNIZ fiziksel/topolojik sutunlara (13..21)
     Gerekce: probability istatistikleri (0..12) already parcaya according to olceklidir; kayan sey
...
def main ():
    ...
    print (f"\n{'arm':<18}{'familiar':>10}{'PXC-disi':>10}{'WEI-disi':>10}{'ORT':>9}{'EN KOTU':>9}")
    S ={}
    for ad in KOL :
        ...
        defn =f1w (puanla ([rf (M ,keep )for M in mats ],fns ,DER ))
        dis ={}
        ...
        v =list (dis .values ())
        print (f"{ad :<18}{defn :>10.4f}{dis .get ('PXC',float ('nan')):>10.4f}"
        f"{dis .get ('WEI',float ('nan')):>10.4f}{np .mean (v ):>9.4f}{min (v ):>9.4f}",flush =True )
        S [ad ]={"familiar":float (defn ),**{f"{a }_disi":float (b )for a ,b in dis .items ()},
