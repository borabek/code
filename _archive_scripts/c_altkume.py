...
def main ():
    ...
    # EGITIMINDE may be -> leakage. O yuzden only WEI parcalarinda (C for unseen)
    ...
    print (f"\neslesen cift: {len (A )}  (WEI={int ((mfg =='WEI').sum ())} = C for GORULMEMIS)")
    ...
    wei =mfg =="WEI"
    det =A [:,0 ]==1 
    print ("\n=== WEI (C for unseen -- DECISION BUNA according to) ===")
