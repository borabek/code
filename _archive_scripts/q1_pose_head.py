...
TAVAN OLCUMU (results/t_ceiling.json) net says:
    oracle GATE      robot 0.4500 -> 0.5091   (+0.059)  <- gate'in tasidigi
    + oracle YON              -> 0.6124   (+0.103)
    + oracle KONUM            -> 0.9372   (+0.325)  <- ASIL kaldirac
...
WHY LEARNED, RULE DEGIL: this night RULE tabanli four konum/direction kolu was tried and DORDU DE became
...
koordinatlarina bagimli olmaz -- part donerse de valid.

KILL (onceden yazili): robot-hazir >= +0.02 VE detection kaybi < 0.005, and GA with kanitli.
"""
import io 
import json 
import os 
...
MAKS_MM =3.0 # correction upper siniri (audit tavsiyesi: 2-3mm)
...
def main ():
    ...
    print (f"pose training: {len (RY )} matched candidate | lateral |w| medyan "
    ...
    print (f"YANAL artik: before medyan {np .median (err0 ):.2f}mm -> after {np .median (err1 ):.2f}mm | "
    ...
    print (f"YON      : before medyan {np .median (a0 ):.2f} deg -> after {np .median (a1 ):.2f} deg | "
