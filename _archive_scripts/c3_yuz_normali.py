...
def main ():
    ...
    for kk ,r in enumerate (D ["DER"],1 ):
        ...
        if r ["X"]is not None and r .get ("XR")is not None :
            ...
            if k .any ():
                ...
                # C3: B-rep BASKIN DUZLEM NORMALLERI. OBB a PCA YAKLASIGIDIR; klemenste
                # tel DUZ ON YUZDEN girer and that yuzun GERCEK normali B-rep'te durur.
                YUZN =[]
                try :
                    ...
                    if pl is not None and len (pl ):
                        ...
                        srt =np .argsort (-_rr )[:6 ]# most BUYUK 6 duzlem yuzu
                        for j in srt :
                            b =_birim (_n [j ])
                            if b is not None :
                                YUZN .append (b )
    ...
    print (f"{'upper boundary (=detection)':<28}{ust :>9.4f}")
    gecti =r2 >=0.70 
    print (f"\nKILL: oracle >= 0.70 -> {'GECTI, ayrik selector egitilir'if gecti else 'GECMEDI'}")
    with io .open ("results/c3_part_direction.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/c3_part_direction.json")
