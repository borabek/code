# -*- coding: utf-8 -*-
"""URUN-SADIK gate-score toplayici: robot_cp.extract yolunun AYNISI (4-model union + avg_probs wire-gate)
but threshold 0 -> butun cps'e wire_score yaz, TP/FP etiketle, diske dok. Hem WEI hem PXC held-out.
Sonra w1_gate_analyze.py grouped-CV with per-mfg optimal threshold finds (in-sample not, DURUST)."""
...
# part listeleri: WEI = _hw_r3 held-out; PXC = eligible in-scope, deterministik ornek
allp =list (eligible ())
...
for k ,(mfg ,pid ,jf ,stp )in enumerate (parts ,1 ):
    try :
        ...
        if not cps :continue 
        wire_gate .apply (V ,F ,avg ,cps ,CE ,CT ,threshold =0.0 )# only wire_score yaz
...
print (f"-> results/gate_scores.json  {len (rows )} pred, {nW } WEI + {nP } PXC part  {time .time ()-t0 :.0f}s")
