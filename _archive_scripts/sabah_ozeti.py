...
file adlarina yaziyor and real sonuc sanilabiliyor -- this night a times became.
...
def kademe2 ():
    ...
    ("tam3 TABAN (duz setting)","p6_kademe2_tam3_baseline.json"),
    ("B1 hard negatif (tam3)","p6_kademe2_B1_zorneg.json"),
    ("B6 ensemble (tam3)","p6_kademe2_B6_ensemble.json"),
    ("last run (uzerine written)","p6_kademe2_full.json")):
        ...
        out .append ((ad ,d .get ("directory","?"),d .get ("selected"),kollar ))
    return out 
...
def main ():
    ...
    for y in ms :
        ...
        L .append ("")
        L .append ("| ceiling | yonlu recall | option maliyeti |")
        L .append ("|---|---|---|")
    ...
    "symmetry":"ayna symmetry esi present mi (klemensler simetriktir)",
    "depth":"axis boyu radius profili (tel / vida / alet ayrimi)",
    "kafes_adet":"lattice adimindan BEKLENEN CP count -> secim baskisi",
    "ozkalib":"part-ici feat-calibration (score yuzdeligi, most iyiye difference)",
    }
    eb =ek_bloklar ()
    if not eb :
    ...
    for ad ,dz ,sec ,kollar in kademe2 ():
        ...
        L .append (f"- **{ad }** ({dz }) selected={sec } -> {ks }")
    L .append ("")
    ...
    if tv and ger :
        ...
        L .append (f"1. **Havuzla:** verimlilik fixed kalirsa tavanin "
        f"**{ger_tavan :.4f}** olmasi gerekir"
        +("  -> 1.0'i asiyor, TEK BASINA IMKANSIZ"
        if ger_tavan >1.0 else ""))
        L .append (f"2. **Seciciyle:** ceiling fixed kalirsa verimliligin "
    ...
    if not tc :
        ...
        for ad ,k in tc .get ("kumeler",{}).items ():
            if k .get ("state"):
                ...
                f"({k ['n_isaret']} isaretin hepsi same score)")
                continue 
        ...
        "Oneri: unseen brand for AUTO katmani KAPATILSIN.")
    L .append ("")

    L .append ("## 5. D7 OKUMA #2 KARARI")
    kaz =[f for _ ,_ ,_ ,f ,k in eb if f is not None and k =="GECTI"]
    top =sum (kaz )
    L .append (f"- kapiyi passing blok sayisi: **{len (kaz )}**")
    L .append (f"- this bloklarin total kazanci: **{top :+.4f}** "
    f"(gate +{KAPI_D7 :.2f})")
    L .append ("")
    if top >=KAPI_D7 :
        ...
        "okuma HARCANMAZ. Butcede remaining okuma count does not change.")
    L .append ("")
