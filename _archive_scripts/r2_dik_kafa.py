# -*- coding: utf-8 -*-
"""R2: DIK-EKSEN KAFASI -- robotun remaining angle hatasinin %56'sini hedefler.

R1 BULGUSU: detection edilen 873 ciftin %13.3'unde angle hatasi present; bunlarin 65'i (=%7.4)
...
def main ():
    ...
    print (f"\nTABAN  detection {T .f1w (d0 ):.4f} | robot {T .f1w (r0 ):.4f}")
    print (f"{'threshold':<8}{'detection':>10}{'robot':>10}{'d(robot)':>11}")
    for e in ESIKLER :
    ...
    for sec ,olc in ((altA ,altB ),(altB ,altA )):
        ...
        print (f"  yarida selected threshold {ea :.2f} -> diger yaride measured ({len (olc )} part)")
    print (f"\nDURUST (capraz threshold): robot {T .f1w (rob_t ):.4f} -> {T .f1w (rob_c ):.4f} "
    f"({T .f1w (rob_c )-T .f1w (rob_t ):+.4f}) | detection {T .f1w (det_t ):.4f} -> {T .f1w (det_c ):.4f}")
