...
GERIYE KALAN BILINMEYEN: iyi a noktaya YANLISLIKLA ates ettigimizde selected giris de
...
uctan uca robot(FIZIKSEL)+detection produces. Amaci TEK a soruyu yanitlamak:
...
a UST SINIRDIR, dagitilabilir prediction not. O yuzden ONCEDEN KAYITLI a setting da
...
from r5_position_diagnosis import yon_uygula 
...
def dagilim_oz (S ,p ,d ):
    ...
    if len (Q ):
        ...
        feat =[len (Q ),float (dp .mean ()),float (dp .max ()),float (np .median (dp )),
        float (np .linalg .norm (Q .mean (0 )-p ))]
    else :
        feat =[0 ,0.0 ,0.0 ,0.0 ,0.0 ]
    return feat +[1.0 if a in S else 0.0 for a in GIRIS [:8 ]]
...
def main ():
    ...
    for r in DER :
        ...
        for i in range (len (P )):
            ...
            feat =dagilim_oz (S ,P [i ],Pd [i ])
            AX .append (np .concatenate ([X [i ],feat ]));AY .append (int (pe [i ,b ]>2.0 ))
            ...
            for gi ,(ad ,q )in enumerate (lst_ ):
                ...
                BX .append (np .concatenate ([X [i ],feat ,e ]));BY .append (int (yy <=2.0 ))
    ...
    print (f"\ntaban: robot(FIZ) {b_rob :.4f} | detection {b_det :.4f}")
    fn =lambda rows :f1w ([q for _ ,q in rows ])-f1w ([p for p ,_ in rows ])
    print (f"\n{'esikQ':>7}{'marj':>7}{'ates':>7}{'robot':>10}{'d':>9}{'GA':>22}{'detection':>10}{'d':>9}")
    SON ={}
    ...
    with io .open ("results/r7_position_iki_asamali.json","w",encoding ="utf-8")as f :
        json .dump ({"taban_robot":b_rob ,"taban_tespit":b_det ,"tarama":SON ,
        "kayitli":kay ,"tarama_en_iyi":eniyi ,"value":bool (val_ )},f ,indent =1 )
    print ("receipt -> results/r7_position_iki_asamali.json")
