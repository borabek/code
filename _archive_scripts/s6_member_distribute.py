...
R7 (oracle):   four modelin ATILAN yonlerinde robot 0.5523 -> 0.6059 yetecek bilgi VAR (+0.0536)
R9 (ten-kestirim, YALNIZ 194 parcanin verisiyle): ogrenilmis selector +0.0301, kahinin %56'si,
    GA [+0.0070, +0.0560] KANITLI

Bu betik TAM KORPUS uye verisiyle (r8_member_data.py) secicinin last halini egitir, uctan uca
...
KILL (onceden yazili): robot >= +0.02 VE detection kaybi < 0.005 VE GA sifiri dislamali.
"""
import io 
import json 
import os 
...
def main ():
    ...
    if not os .path .exists (UYE ):
        raise SystemExit (f"YOK: {UYE } (r8_member_data.py bitmedi)")
    ...
    f"| {len (set (UG [disi ]))} grup | 'correct' orani {UY [disi ].mean ():.1%}",flush =True )
    ...
    with io .open ("results/s6_member_distribute.json","w",encoding ="utf-8")as f :
    ...
    if gecti :
        with open ("results/uye_secici.pkl","wb")as f :
            ...
            "YAPISAL SIFIR: detection olcutu aciya bakmaz.")},f )
        print ("-> results/uye_secici.pkl")
    print ("receipt -> results/s6_member_distribute.json")
