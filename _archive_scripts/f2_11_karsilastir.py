...
KILL (f2_11_parity.py'de onceden yazildi):
    candidate recall farki <= 0.01  VE  manufacturer basina detection F1 farki <= 0.015
...
ADIL KARSILASTIRMA SARTI: A kolu, same night same ortamda uretilmis `_der_kontrol.pkl`
...
def main ():
    ...
    KOLLAR ={}
    for ad ,path in (("A_STEP","results/_der_kontrol.pkl"),
    ("B_JSON","results/_der_json_B.pkl"),
    ("C_JSON_stepsiz","results/_der_json_C.pkl")):
        if not os .path .exists (path ):
            print (f"  {ad }: {path } YOK, atlaniyor")
            continue 
    ...
    if "A_STEP"in SON and "B_JSON"in SON :
        ...
        print (f"\n  manufacturer basina EN BUYUK diff: {enb :.4f}  (threshold <= 0.015)  "
        ...
        if "C_JSON_stepsiz"in SON :
            ...
            f"detection {c ['detection']:.4f}  -> B'ye per {c ['detection']-b ['detection']:+.4f}")
            print (f"  (this diff B-rep ozellik blogunun BEDELIDIR)")
        SON ["kill"]={"recall_farki":dr ,"uretici_en_buyuk":enb ,"gecti":bool (gecti )}
    with io .open ("results/f2_11_parity.json","w",encoding ="utf-8")as f :
        json .dump (SON ,f ,indent =1 ,ensure_ascii =False )
    print ("\nmakbuz -> results/f2_11_parity.json")
