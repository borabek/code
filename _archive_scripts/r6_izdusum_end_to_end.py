# -*- coding: utf-8 -*-
"""R6: NOKTAYI eksene izdusurme UCTAN UCA (R5 gate taramasindan turetildi) (kodda ASILI remaining "P2 taramasi").
...
KILL (onceden yazili): a gate degeri dagitilabilmesi for
  robot-hazir >= +0.02 VE detection >= -0.01 (ikisi de DEV and VAL'de same yonde).
"""
import json 
import os 
import pickle 
...
def main ():
    ...
    SONUC ={}
    for gate in KAPILAR :
        os .environ ["CP_AXIS_PROJECT"]="1"if gate =="izdusum"else "0"
        import cp_openings ,robot_cp 
        importlib .reload (cp_openings )
        assert cp_openings .USE_AXIS_PROJECT ==(gate =="izdusum"),cp_openings .USE_AXIS_PROJECT 
        ...
        ah =np .array (aci_hep )
        SONUC [gate ]={"detection":float (f1w (hep_d )),"robot":float (f1w (hep_r )),
        ...
        s_ =SONUC [gate ]
        print (f"\nKAPI {gate }: detection {s_ ['detection']:.4f} (dev {s_ ['tespit_dev']:.4f} / "
    ...
    t =SONUC ["baseline"]
    print (f"\n{'gate':<8}{'detection':>9}{'robot':>9}{'d_tespit':>10}{'d_robot':>9}"
    f"{'angle<=10':>9}{'KILL':>10}")
    kazanan =None 
    for gate in KAPILAR :
        s_ =SONUC [gate ]
        dt =s_ ["detection"]-t ["detection"];dr =s_ ["robot"]-t ["robot"]
        # KILL (onceden yazili): robot-hazir >= +0.01 BUYUKLUK, detection >= -0.005,
        # and DEV with VAL AYNI YONDE (havuzlanmis number DEV/VAL ayrismasini gizliyordu).
        ok =(dr >=0.01 and dt >=-0.005 and gate !="baseline"
        and (s_ ["robot_dev"]-t ["robot_dev"])>0 and (s_ ["robot_val"]-t ["robot_val"])>0 )
        print (f"{str (gate ):<10}{s_ ['detection']:>9.4f}{s_ ['robot']:>9.4f}{dt :>+10.4f}{dr :>+9.4f}"
        f"{s_ ['aci_10_alti']:>9.1%}{'GECTI'if ok else ('baseline'if gate =='baseline'else 'gecmedi'):>10}")
        if ok and (kazanan is None or s_ ["robot"]>SONUC [kazanan ]["robot"]):
            kazanan =gate 
    print (f"\nSONUC: {('KAPI '+str (kazanan )+' DAGITILABILIR')if kazanan else 'IZDUSUM GECMEDI'}")
    with open ("results/r6_izdusum_end_to_end.json","w",encoding ="utf-8")as f :
        json .dump ({str (k ):v for k ,v in SONUC .items ()}|{"kazanan":kazanan },f ,indent =1 )
    print ("receipt -> results/r6_izdusum_end_to_end.json")
