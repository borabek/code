# -*- coding: utf-8 -*-
"""U10: KAHIN ORANINI DURUST OLC -- baseline, arm and oracle AYNI KOSUDAN.

DUZELTILEN HATA (2026-08-02): T8'de "kahinin %66'si yakalandi" yazdim; oracle payini SABIT
0.0536'ya boldum. O fixed BASKA BIR ZINCIRDE olculmustu (uye selector dagitilmadan before,
...
BU BETIK ucunu de TEK KOSUDA olcer:
    ...
    O  oracle        : GT'ye bakip havuzdaki EN IYI yonu sec (ulasilabilir DEGIL, upper boundary)
and oranI (K-T)/(O-T) as gives -- payda AYNI kosudan.

Ayrica GUNCEL bosslugu gives: dagitilan zincir this an nerede, oracle nerede.
"""
import io 
import json 
import os 
...
def kahin_orani (baseline ,arm ,oracle_ ):
    """(arm - baseline) / (oracle - baseline). UCU DE AYNI KOSUDAN gelmeli.

    Sabit a oracle degerine bolmek YANLISTIR: baseline degisince ratio anlamini yitirir
...
def main ():
    ...
    def havuz_kur (P ,Pd ,i ,uyeler ):
        ...
        # (two bagimsiz betikte dogrulandi). Yani deviation URUNDE not OLCUMDE idi -- this night
        # konulan 'measurement urunu TAKLIT ETMEZ, AYNI KODU cagirir' kuralinin ihlaliydi.

    def puanla (mod ):
        """mod: 'baseline' (selector absent) | 'arm' (dagitilan selector) | 'oracle' (GT'ye bakar)."""
        det ,rob =[],[]
        for r in DER :
    ...
    ("oracle","O KAHIN (upper boundary)")):
        d ,rr =puanla (mod )
        R [mod ]=(float (f1w (d )),float (f1w (rr )),rr )
        print (f"{ad :<26}detection {R [mod ][0 ]:.4f} | ROBOT {R [mod ][1 ]:.4f}",flush =True )

    T ,K ,O =R ["baseline"][1 ],R ["arm"][1 ],R ["oracle"][1 ]
    ratio =kahin_orani (T ,K ,O )
    print (f"\n=== AYNI KOSUDAN ===")
    print (f"  baseline {T :.4f} -> arm {K :.4f} -> oracle {O :.4f}")
    ...
    print (f"\nESKI (YANLIS) HESAP: oracle payini SABIT 0.0536'ya bolmustum; o fixed")
    print (f"BASKA a zincirden geliyordu (baseline 0.5523). Dogru payda: {O -T :.4f}")
    with io .open ("results/u10_oracle_orani.json","w",encoding ="utf-8")as f :
        json .dump ({"baseline":T ,"arm":K ,"oracle":O ,"yakalama":float (ratio ),
        "kalan_bosluk":float (O -K ),"ga":[float (lo ),float (hi )]},f ,indent =1 )
    print ("receipt -> results/u10_oracle_orani.json")
