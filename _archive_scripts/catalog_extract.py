...
def main ():
    ...
    for page in reader .pages :
        ...
        # keep line structure: a range-overview row usually holds the type ten the
        # left and the order number ten the right of the SAME line
    ...
    print (f"{os .path .basename (args .pdf )}: {len (pairs )} part no bulundu, "
    f"{len (pairs )-len (new )} already elimizde, {len (new )} YENI -> {args .out }")
