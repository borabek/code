...
times is computed, 18-sutunlu arm same matrisin first 18 sutununu kullanir. Tek variable: feature kumesi.

KILL (onceden yazili): DEV'de uctan uca detection F1 artmazsa dagitilmaz. Artarsa VAL'de SINANIR;
VAL'de kaybederse yine dagitilmaz. Bu, this night L2'yi olduren and fiziksel ozellikleri gecirer rule.
"""
...
def main ():
    ...
    for tag ,(f ,ncol )in KAYNAK .items ():
        ...
        Xt =d ["X"][keep ][:,:ncol ]
        assert Xt .shape [1 ]==ncol ,f"{tag }: {d ['X'].shape [1 ]} sutunluk data, {ncol } istendi"
    ...
    print (f"\n{'gate':<26}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    R ={}
    ...
    print (f"\nKILL: uctan uca detection artmazsa DAGITILMAZ -> {dd :+.4f} => "
