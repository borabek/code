...
KUME: `results/d7_exam_set.json` (838 part / 12 manufacturer / 3201 CP, muhur
...
def main ():
    ...
    from p0_ceiling_re import kod_muhru 
    ...
    with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
        ...
        "detection":{"f1":tf ,"precision":tp_ ,"recall":tr_ },
        "robot":{"f1":rf ,"precision":rp_ ,"recall":rr_ },
        "donusum":rf /max (tf ,1e-9 ),
        "regime":{rj :{"detection":f1w (Tr [rj ]),"robot":f1w (Rr [rj ])}
        for rj in Tr },
        "manufacturer":{m :{"n":len (Tm [m ]),"detection":f1w (Tm [m ]),
