...
grup-capraz egitiliyordu. O gunden beri IKI sey degisti:
  * label ISARETLI became (training pozitif orani %31.4 -> %12.7) -> score dagilimi kaydi
...
Tespit yapisal as does not change (detection olcutu aciya bakmaz) -- yine de basilir.
"""
...
def main ():
    ...
    for r in DER :
        ...
        SK [r ["pid"]]=s 
    print (f"score hazir: {len (SK )} part",flush =True )
    ...
    setting =None # dev+atanmamis = val DISI
    print (f"\n--- AYAR (val disi, {len ([r for r in DER if r ['pid']not in val ])} part) ---")
    print (f"{'marj':>7}{'robot(FIZ)':>13}{'detection':>10}")
    AY ={}
    for m in MARJLAR :
    ...
    f"dagitilan 0.05 ({AY [0.05 ]:.4f}) -> diff {AY [en ]-AY [0.05 ]:+.4f}")

    print (f"\n--- HUKUM (YALNIZ VAL, {len (val )} part; AYAR'da selected TEK marj) ---")
    ...
    with io .open ("results/r11_marj.json","w",encoding ="utf-8")as f :
        json .dump ({"setting":{str (k ):v for k ,v in AY .items ()},"selected":en ,
