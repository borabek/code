# -*- coding: utf-8 -*-
"""PARITE TESTI: measured_path urun with DAGITILAN urun same mi?

2026-07-31 denetimi three ayrisma buldu:
...
Bu test ucunu de KODDAN dogrular. Kirmizi olursa measured_path number dagitilan urunu temsil etmiyordur.
"""
...
def main ():
    ...
    if miss :
        fails .append (f"step_path missing cagrilar: {miss }")

    import robot_cp 
    cfg =json .load (open ("cp_config.json"))
    n_ck =len (cfg ["current_product"]["checkpoints"])
    import numpy as np 
    # same modelden IKI yakin candidate -> single oy must be
    ...
    print (f"   {n_ck } different modelden same candidate -> votes={v2 [0 ]['_votes']} ({n_ck } olmali)")
    if v2 [0 ]["_votes"]!=n_ck :
    ...
    if fails :
        ...
        for f in fails :
            print (f"  - {f }")
        return 1 
    print ("PARITE TAMAM -- measured_path urun with dagitilan urun same.")
    return 0 
