# -*- coding: utf-8 -*-
"""AUTO-tier threshold kalibrasyonu (vote>=2 urunu).

conf_auto 0.75 TEK model for ayarlanmisti; vote toplulugunda temsilci-guveni = most high uyenin
...
Sizinti-siz held-out (145 WEI + PXC ornegi) ten each hayatta-remaining vote CP'sinin (confidence, oy,
...
def matched_preds (P ,G ,Gd ,tol ,axis_tol =40.0 ):
    """axis-aware greedy; each pred for TP mi (True/False) dizisi dondur."""
...
def main ():
    ...
    ap .add_argument ("--pxc-cap",type =int ,default =100 ,help ="PXC part siniri (hiz)")
    ...
    parts +=pxc 
    print (f"{len (parts )} part ({len (parts )-len (pxc )} WEI + {len (pxc )} PXC) | {len (models )} model vote",flush =True )

    # each part: vote CP'leri -> (mfg, conf, votes, tp)
    ...
    for mv ,cf in itertools .product ([2 ,3 ,4 ],[0.50 ,0.60 ,0.70 ,0.75 ,0.80 ,0.85 ,0.90 ]):
        ...
        # hedef: two ureticide de AUTO precision >=0.75, after total AUTO recall'i maksimize et
    ...
    if best :
        ...
        print (f"  (hedef: iki manufacturer AUTO precision >=0.75, after AUTO recall max)")
    else :
