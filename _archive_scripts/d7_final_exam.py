...
CIKTI ="results/d7_exam_set.json"
...
def main ():
    ...
    d5 =json .load (io .open ("results/d5_4_exam_set.json",encoding ="utf-8"))
    d6 =json .load (io .open ("results/d6_exam_set.json",encoding ="utf-8"))
    ...
    secili ,gruplar ,skipped =[],set (),collections .Counter ()
    for m ,p ,_jf ,_s in sorted (E ,key =lambda t :(t [0 ],t [1 ])):
        ...
        if p in D6P or (anah and anah in D6A ):
            skipped ["d6_cakismasi"]+=1 ;continue 
        if anah and anah in YASAK :
            skipped ["yasak_anahtar"]+=1 ;continue 
        if anah and anah in gruplar :
            skipped ["grup_ici_tekrar"]+=1 ;continue 
        if anah :
            gruplar .add (anah )
        secili .append ({"pid":p ,"mfg":m ,"cp":int (len (G )),"bucket":kova_adi (len (G ))})
