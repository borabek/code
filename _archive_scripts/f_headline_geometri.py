...
gate-ONCESI adaylari and feature matrislerini tasiyor; changed single sey gate'in hangi adaylarla
EGITILDIGI.
"""
...
def main ():
    ...
    print (f"  family_key with disarida birakilan : {int ((~keep_fam ).sum ())}")
    print (f"  geometry_key with disarida birakilan: {int ((~keep_geo ).sum ())} "
    ...
    print (f"{'gate bolmesi':<28}{'detection':>9}{'yanal2':>9}{'ROBOT':>9}{'conclusive':>8}{'recall':>8}")
    res ={}
    ...
    ("geometry_key (GERCEK)",keep_geo ,G_geo )):
        ...
        res [lab ]=dict (detection =det [0 ],yanal2 =lat [0 ],robot =rob [0 ],
    ...
    print (f"\nUCTAN UCA SIZINTI BEDELI: detection {b ['detection']-a ['detection']:+.4f}  "
    f"robot {b ['robot']-a ['robot']:+.4f}")
    json .dump (res ,open ("results/f_headline_geometri.json","w"),indent =1 ,default =str )
    print ("receipt -> results/f_headline_geometri.json")
