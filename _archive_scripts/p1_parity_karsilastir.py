...
def main ():
    ...
    print (k )
    print (f"\nNOT: parite duzeltmesi a KAZANC vaadi not, TUTARLILIK duzeltmesidir. Kural")
    print (f"gecmese bile YENI data correct olandir; rule burada 'zarar present mi' sorusunu yanitlar.")
    zarar =(SON ["YENI"]["familiar"]-SON ["ESKI"]["familiar"]<-0.02 )or any (
    SON ["YENI"][b ]-SON ["ESKI"][b ]<-0.03 for b ,_ ,_ in BOLME if b !="familiar")
    print (f"ZARAR VAR MI: {'EVET -- dagitma'if zarar else 'HAYIR -- parite verisi dagitilabilir'}")
    with io .open ("results/p1_parity_karsilastir.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/p1_parity_karsilastir.json")
