...
FIZ =["brep_r","esesenli","r_orani","bos_derinlik","passing"]


def fiz_ozellik (p ,d ,cyl ,mesh ):
    import cp_geometry as G 
    C ,A ,R =cyl 
    o =dict (brep_r =0.0 ,esesenli =0 ,r_orani =1.0 ,bos_derinlik =-1.0 ,passing =0 )
    if len (C ):
        rel =p -C 
        al =(rel *A ).sum (1 )
...
def main ():
    ...
    print (f"  -> brep_r with size korelasyonu: {c :+.3f} "
