# -*- coding: utf-8 -*-
"""U4: PARCA-ICI feature normalizasyonu UCTAN UCA (familiar + two unseen manufacturer).

U3 candidate duzeyinde: most kotu manufacturer-disi 0.4597 -> 0.5219 (+0.062), familiar bedeli +0.0003.
Aday duzeyi this night UC KEZ yaniltti (EK blogu, goreli esigin bedeli, sinif dengeleme) --
karar BURADA.
...
KILL (onceden yazili, BUYUKLUK dahil):
  (a) familiar detection F1 >= -0.01  VE
  (b) unseen manufacturer EN KOTU durumu >= +0.01 artmali.
...
def main ():
    ...
    print (f"\n{'split':<24}{'arm':>14}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    R ={}
    for ad ,fn in DONUSUM .items ():
        M =XTR [ad ]
    ...
    t_tan =f1w (R [("A ham","familiar")][0 ])
    t_kotu =min (f1w (R [("A ham",a )][0 ])for a in key_ )
    print (f"KARAR (baseline A ham: familiar {t_tan :.4f} | unseen en kotu {t_kotu :.4f})")
    kazanan =None 
    for ad in DONUSUM :
        if ad =="A ham":
            continue 
        dt =f1w (R [(ad ,"familiar")][0 ])-t_tan 
        ek =min (f1w (R [(ad ,a )][0 ])for a in key_ )
        gecti =(dt >=-0.01 )and (ek -t_kotu >=0.01 )
        print (f"  {ad :<14} familiar {dt :+.4f} | en kotu {t_kotu :.4f} -> {ek :.4f} "
    ...
    open ("results/u4_part_ici_end_to_end.json","w"),indent =1 )
    pickle .dump (R ,open ("results/u4_parca.pkl","wb"))
    print ("receipt -> results/u4_part_ici_end_to_end.json")
