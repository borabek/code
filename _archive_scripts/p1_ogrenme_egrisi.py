...
WHY SIMDI: night along nine mekanizma same cepheye carpti and geriye TEK canli arm
...
W2 kanitladi ki data real a kaldiractir: +110 new WEI parcasi unseen parcada
...
def main ():
    ...
    for pay in PAYLAR :
        ...
        for th in TOHUMLAR :
            ...
            line_ .append (S )
        hav =float (np .mean ([s ["havuzlanmis"]["detection"]for s in line_ ]))
        ud =float (np .mean ([s ["_URETICI_DISI_ORT"]for s in line_ ]))
        rob =float (np .mean ([s ["havuzlanmis"]["robot"]for s in line_ ]))
        sd =float (np .std ([s ["havuzlanmis"]["detection"]for s in line_ ]))
    ...
    print (f"\nEKSTRAPOLASYON (only egri doymuyorsa valid):")
    simdi_g =int (g [-1 ])
    for hedef in (0.80 ,0.85 ):
        if kh [1 ]>1e-6 :
            gerek =float (np .exp ((hedef -kh [0 ])/kh [1 ]))
            print (f"  havuzlanmis {hedef :.2f} for ~{gerek :,.0f} geometri grubu "
            f"({gerek /simdi_g :.1f}x, +{gerek -simdi_g :,.0f} grup)")
        else :
            print (f"  havuzlanmis {hedef :.2f}: egim <=0, data with ULASILAMAZ")
