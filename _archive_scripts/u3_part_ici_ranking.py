...
WHY BU: night tesihs "cokusun ~%42'si KALIBRASYON, ~%58'i SIRALAMA" demisti. Kalibrasyon
...
"this candidate this parcadaki most deep 2. hole" ifadesi ureticiden bagimsizdir, "depth 4.2mm" degildir.
...
KILL (onceden yazili, BUYUKLUK dahil):
  unseen manufacturer EN KOTU durumu >= +0.01 artmali VE familiar >= -0.01 kalmali.
  Gecen arm UCTAN UCA dogrulanmadan DAGITILMAZ (candidate duzeyi this night 3 times yanildi).
"""
import json 
import os 
import sys 
...
def parca_ici_z (X ,pids ):
    """Her part inside column bazli z-score (sd=0 whereas 0)."""
    out =np .zeros_like (X ,dtype =float )
    for u in np .unique (pids ):
        i =np .where (pids ==u )[0 ]
        sub =X [i ]
...
def main ():
    ...
    print (f"\n{'arm':<16}{'familiar':>10}{'mfg0-disi':>11}{'mfg1-disi':>11}{'EN KOTU':>10}")
    res_ ={}
    for ad ,M in ARM .items ():
        ...
        for k in np .unique (mfg ):
            ...
            dis [k ]=f1 (karar (s ),te )
        ek =min (dis .values ())
        print (f"{ad :<16}{defn :>10.4f}{dis ['0']:>11.4f}{dis ['1']:>11.4f}{ek :>10.4f}",flush =True )
        res_ [ad ]={"familiar":float (defn ),"mfg0_disi":float (dis ["0"]),
        "mfg1_disi":float (dis ["1"]),"en_kotu":float (ek )}

    t =res_ ["A ham"]
    print (f"\nKARAR (baseline A: familiar {t ['familiar']:.4f} | en kotu {t ['en_kotu']:.4f})")
    kazanan =None 
    for ad ,s in res_ .items ():
        if ad .startswith ("A"):
            continue 
        dt =s ["familiar"]-t ["familiar"];dk =s ["en_kotu"]-t ["en_kotu"]
        gecti =(dk >=0.01 )and (dt >=-0.01 )
        print (f"  {ad :<16} familiar {dt :+.4f} | en kotu {dk :+.4f} -> "
    ...
    json .dump (res_ |{"kazanan":kazanan },open ("results/u3_part_ici.json","w"),indent =1 )
    print ("receipt -> results/u3_part_ici.json")
