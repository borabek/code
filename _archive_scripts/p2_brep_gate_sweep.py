...
OLCULEN ETKI (DEV, correction oncesi->sonrasi): detection 0.7073 -> 0.7089 (+0.0016) but
robot-hazir 0.4974 -> 0.4841 (-0.0133). Yani correction DOGRU, kapilar BAYAT.

KILL: no setting correction-oncesi robot-hazir degerini (0.4974) gecmezse kapilar old
...
def main ():
    ...
    print (f"\n{'max_off':>8}{'r_max':>8}{'detection':>9}{'ROBOT':>9}{'conclusive':>8}{'recall':>8}")
    R ={}
    ...
    out ={"baseline":{"detection":f1w (baseline [0 ]),"robot":f1w (baseline [1 ])},
    "en_iyi_ayar":list (en_iyi ),
    "en_iyi":{"detection":f1w (R [en_iyi ][0 ]),"robot":f1w (R [en_iyi ][1 ])},
    "tum":{f"{k [0 ]}|{k [1 ]}":{"detection":f1w (v [0 ]),"robot":f1w (v [1 ])}
    for k ,v in R .items ()}}
