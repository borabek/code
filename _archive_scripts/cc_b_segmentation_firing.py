...
def main (limit =60 ):
    ...
    print (f"{len (parts )} very-CP part ({HIGH_CP }+ CP)\n",flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp ,ngt )in enumerate (parts ,1 ):
        try :
            ...
            # hangi GT BULUNDU (robotun adayiyla matched)
        ...
        if k %10 ==0 :
            print (f"  {k }/{len (parts )}  found {len (found )} / kacirilan {len (missed )}  "
    ...
    print (f"KACIRILAN GT'lerin %{100 *frac_fire :.0f}'inde yakinda CE+CT olasiligi >= 0.30 present")
