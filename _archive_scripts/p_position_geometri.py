...
BU BETIK: each two kumede, ESLESMIS (i.e. detection edilmis) candidates for ham error dagilimini
...
def hatalar (cluster ):
    ...
    for r in cache :
        ...
        if not len (G ):
            continue 
            # GEVSEK (detection) olcutle esle -> only BULUNMUS olanlarin kalitesine bak
        diff =P [:,None ,:]-G [None ,:,:]
...
def main ():
    ...
    print (f"  only lateral esigi (<=2mm) : {dv ['yanal_2mm_orani']:.3f} -> {vv ['yanal_2mm_orani']:.3f}"
    f"  ({vv ['yanal_2mm_orani']-dv ['yanal_2mm_orani']:+.3f})")
    print (f"  only aci esigi (<=10 der): {dv ['aci_10d_orani']:.3f} -> {vv ['aci_10d_orani']:.3f}"
    ...
    json .dump (out ,open ("results/p_position_geometri.json","w"),indent =1 )
    print ("\nmakbuz -> results/p_position_geometri.json")
