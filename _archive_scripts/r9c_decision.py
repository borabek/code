...
from r9b_baseline_correction import satirlari_kur ,geo_kur ,R4 
...
def main ():
    ...
    print (f"satir {len (LY )} | correct giris {LY .mean ():.1%}",flush =True )
    ...
    print (f"\nKARAR: diff GERCEK mi={tam ['real']} & veriyle BUYUYOR mu={buyuyor } "
    ...
    with io .open ("results/r9c_decision.json","w",encoding ="utf-8")as f :
        json .dump ({"marj":MARJ ,"egri":SON ,"buyuyor":bool (buyuyor ),
        "karar_r10":karar },f ,indent =1 )
    print ("receipt -> results/r9c_decision.json")
