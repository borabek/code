...
DIAGNOSIS (x1 + hafiza): gate EZBERLIYOR. Havuzlanmis 0.7584 but unseen ureticide
...
def main ():
    ...
    for k in SON :
        print (f"{k :<6}{SON [k ]['havuzlanmis']['detection']:>13.4f}"
        f"{SON [k ]['_URETICI_DISI_ORT']:>11.4f}"
        f"{SON [k ]['havuzlanmis']['detection']-baseline ['havuzlanmis']['detection']:>+11.4f}"
    ...
    and SON [k ]["havuzlanmis"]["detection"]-baseline ["havuzlanmis"]["detection"]>=-0.005 ]
    if candidate :
    ...
    with io .open ("results/g1_transfer_secim.json","w",encoding ="utf-8")as f :
        ...
        "sonuc":{str (k ):{"havuzlanmis":SON [k ]["havuzlanmis"]["detection"],
