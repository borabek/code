...
1. YANAL TAVAN (never olculmedi): poz kafasi deployed (+0.0428, CI-kanitli) but
   "konum MUKEMMEL olsa ne olurdu" never sorulmadi. Bir KONUM SOZLUGU kurulur and oracle
...
Olculen tavanlar (all of them same eslesme koduyla, same kumede):
    ...
    YANAL MUKEMMEL tavani     only angle<=10    (lateral serbest -> detection toleransi)
    ikisi mukemmel            = detection F1
...
def main ():
    ...
    KOL ={k :[]for k in ("detection","mevcut","aci_mukemmel","yanal_mukemmel",
    ...
    for kk ,r in enumerate (D ["DER"],1 ):
        ...
        KOL ["detection"].append ((rj ,)+esle (P ,Pd ,G ,Gd ,r ["diag"],0.0 ,180.0 ,True ))
        ...
        # YANAL MUKEMMEL tavani: lateral serbest (detection toleransi), angle<=10
    ...
    print (f"{'IKISI mukemmel (=detection)':<30}{F ['detection']:>9.4f}")
    ...
    print (f"  (correct payda {ac :.4f}; C1-C3'te yanlislikla {F ['detection']:.4f} kullanmistim)")
    ya =F ["yanal_mukemmel"]
    print (f"\nDUZELTME -- YON sozlugunun correct kapsami:")
    print (f"  YON sozlugu only ACI hatalarini duzeltir; tavani {ya :.4f}")
    print (f"  C3 kahini 0.6635 -> kapsam %{100 *(0.6635 -me )/max (ya -me ,1e-9 ):.0f}"
    f"  (before %44 demistim -- YANLIS PAYDA)")
    gecti =kaps >=0.70 
    print (f"\nKILL: konum kahini ACI-MUKEMMEL tavaninin >=%70'ini kapsamali -> "
    f"{'GECTI'if gecti else 'GECMEDI'}")
    with io .open ("results/r3_yanal_ceiling.json","w",encoding ="utf-8")as f :
        json .dump ({k :float (v )for k ,v in F .items ()}|
        {"konum_kapsam":float (kaps ),"gecti":bool (gecti )},f ,indent =1 )
    print ("receipt -> results/r3_yanal_ceiling.json")
