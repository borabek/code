...
karar verilemeyecek up to small a etki showed:
    DEV  detection +0.0054  GA [-0.0096, +0.0203]   robot +0.0108  GA [-0.0001, +0.0245]
    VAL  detection -0.0001                          robot -0.0033
...
def main ():
    ...
    for k in ("dev","val"):
        ...
        if not os .path .exists (f ):
            print (f"missing: {f } -- before `python sina_cluster.py {k }` kosulmali");return 
        P [k ]=pickle .load (open (f ,"rb"))

    print (f"{'cluster':<10}{'n':>5}{'J open detection':>15}{'J closed':>11}{'difference':>9}")
    for k in ("dev","val"):
        a =P [k ][URUN ][0 ];b =P [k ][KAPALI ][0 ]
        print (f"{k :<10}{len (a ):>5}{f1w (a ):>15.4f}{f1w (b ):>11.4f}{f1w (a )-f1w (b ):>+9.4f}")

    out ={}
    print (f"\nHAVUZLANMIS (DEV+VAL), paired bootstrap 4000 tekrar:")
    ...
    json .dump (out ,open ("results/j_havuzlu_decision.json","w"),indent =1 )
    print ("\nmakbuz -> results/j_havuzlu_decision.json")
