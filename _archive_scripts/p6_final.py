# -*- coding: utf-8 -*-
"""P6 FINAL OLCUM (P4 yigin dogrulamasi): uzlasma yukseltici (P1) + RENK ozelligi (P2), ESLESEN gate with.

WHY YIGIN: P1 single basina +0.0056, P2'nin most iyi varyanti +0.0078 -- ikisi de single basina
+0.01/+0.02 barajini gecemedi. Ama mekanizmalari BAGIMSIZ (biri oy cozunurlugu, digeri
malzeme sinyali) and this projede lower-threshold kaldirac YIGINI more before +0.053 measured
([[stacked-levers-2026-07-29]]). Bar yigin seviyesine yazilir, threshold dusurulmez.

P4'te yigin +0.0319 measured (48 part, seed 13). Bu BAGIMSIZ dogrulama: 100 part, seed 101,
500 training parcasi, test aileleri yine disarida. AYNI duzenek -> seviye and difference birlikte valid.
Super-toplamsal sonuc (P1 +0.006, P2 -0.003, birlikte +0.032) this up to buyukse VERIFICATION sart.

KOLLAR -- all of them KENDI dagilimiyla egitilmis gate with, test aileleri egitimden CIKARILMIS:
  A = 4 uye, 13 feature            (urun, referans)
  B = 5 uye, 13 feature            (P1 single basina -- a5_fair'i dogrular)
  C = 5 uye, 13 + RENK             (YIGIN)
  D = 4 uye, 13 + RENK             (P2 single basina)

RENK: dar rule (most iyi measured_path varyant), NaN = renk absent -> -1 + eksiklik bayragi.
"""
import os ,sys ,json ,pickle 
import numpy as np 
os .environ .setdefault ("BA_ALLOW_SEEN","1")
os .environ .setdefault ("CP_METAL_WIDE","0")# dar rule: measured_path most iyi varyant
...
def main ():
    ...
    print (f"P6 DOGRULAMA -- test {len (sel )} part (70 dusuk/30 very) | training {len (tr_parts )}",flush =True )
    ...
    def metal_of (stp ,Vr ):
        ...
        try :
            ...
            if res >1.0 :# SIKI threshold (gevsetmek measured: zarar veriyor)
                return None ,None 
            allm =(allp -t )@R 
    ...
    for k ,prt in enumerate (tr_parts ,1 ):
        try :
            ...
            continue 
        if k %100 ==0 :
            print (f"  training {k }/{len (tr_parts )} ({n_col } renkli)",flush =True )
    gates ={}
    for name in ARMS :
        ...
        print (f"  {name }: {len (y )} candidate, {X .shape [1 ]} ozellik",flush =True )
    ...
    print (f"test cache {len (tests )} part ({tc } renkli)\n",flush =True )
    ...
    open ("results/p6_final.json","w"),indent =1 )
    print ("receipt -> results/p6_final.json")
