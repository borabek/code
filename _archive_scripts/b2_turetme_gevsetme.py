...
def main ():
    ...
    for k ,r in enumerate (DER ,1 ):
        ...
        for ad ,mv ,vc ,cl in AYAR :
            try :
                lists =[cp_openings .connection_points (
                ...
                cps =RC ._vote2 (lists ,cluster_mm =5.0 ,min_votes =1 )
            except Exception :
                continue 
            if not cps :
                continue 
    ...
    print (f"\nGT total {top_gt }")
    print (f"{'setting':<12}{'candidate recall':>13}{'candidate count':>13}{'candidate/GT':>10}")
    baseline =None 
    for ad ,mv ,vc ,cl in AYAR :
