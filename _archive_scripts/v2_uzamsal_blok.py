...
DIAGNOSIS (v1): 0.85 yapisal as MUMKUN (candidate tavani 0.9354) but oracle gate'in %91'i is required;
...
ALTI SUTUN (op2_spatial with same, cerceve-bagimsiz):
    ...
    sp_cons  same SATIR/SUTUNDAKI adaylarin score toplami (dizilim tutarliligi)
    sp_dens  yerel yogunluk (score agirlikli)
...
KILL: decision_criterion five sarti (familiar >= -0.01, split ortalamasi >= +0.01, most kotu split
...
def main ():
    ...
    for ad ,ek in (("A 58",False ),("B 58+uzamsal",True )):
        ...
        for b ,mk ,alt in BOLME :
            ...
            sat +=f"{f1w (det ):>12.4f}"
        rr =f1_rejim (PARCA [(ad ,"familiar")][0 ])
