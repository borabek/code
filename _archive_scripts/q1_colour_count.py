...
def main ():
    ...
    tot =max (no +tek +very ,1 )
    print (f"\n{'renk count':<16}{'part':>8}{'ratio':>9}")
    for k in sorted (hist ):
        print (f"{k :<16}{hist [k ]:>8}{hist [k ]/tot :>9.3f}")
    print (f"\nhic renk none      : {no :>5} ({no /tot :.3f})")
    print (f"tek renk          : {tek :>5} ({tek /tot :.3f})")
    print (f">=2 ayrik renk    : {very :>5} ({very /tot :.3f})   <- KANALIN BILGISI BURADA")
    print (f"STYLED_ITEM none   : {styled_yok :>5}")
    if ornek :
        print ("\ncok renkli ornekler:")
        for a ,b in ornek :
            print (f"  {a }: {b }")
    ...
    json .dump ({"n":tot ,"never":no ,"single":tek ,"very":very ,"oran_cok":ratio ,
    "styled_yok":styled_yok ,
    "karar":"CANLI"if ratio >=0.10 else "BOS"},
    open ("results/q1_colour_count.json","w"),indent =1 )
    print ("receipt -> results/q1_colour_count.json")
