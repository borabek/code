...
%6-tolerans with 1.000, 2mm with 0.455 output -- i.e. "sifir wrong detection" tolerans eseriydi.
...
def main ():
    ...
    print (f"{len (sel )} part (70 dusuk / 30 very), {len (test_fams )} aile gate egitiminden disarida",
    flush =True )
    ...
    print (f"gate {int (keep .sum ())} candidate with egitildi",flush =True )
...
def _report (cache ):
    ...
    print ("  -> fixed 2mm toleransin a kismi HIZALAMA hatasidir, modelin not")
