# -*- coding: utf-8 -*-
"""GOREV 2a offline: tol_audit_pool'da different MATCH toleranslariyla base F1 (ws>=0.35). Amac: mevcut
tolerans FIZIKSELDEN sikiysa, correct toleransla ALL 0.756 kalkar mi (metrik duzeltmesi, model not).
...
def tol_for (mode ,diag ,size_a ):
    ...
    if mode =="opening_radius":return max (size_a /2.0 ,1.0 )# candidate acikligin yaricapi
    if mode =="size_scaled":return max (size_a ,2.0 )# full cap
    if mode =="wire_fit":return max (size_a /2.0 ,2.5 )# radius ya da min tel-yaricapi 2.5mm
...
def f1_mode (mode ):
    ...
    for pid ,d in pool .items ():
        ...
        pred =ws >=THR # base prediction
        # each candidate for tolerans (size'a bagli may be)
        tol =np .array ([tol_for (mode ,diag ,size [a ])for a in range (len (ws ))])
        ok =(perp <=tol [:,None ])&(np .abs (along )<=ALONG )
        # greedy 1-1 eslesme (only pred candidates, perp'e according to)
...
print (f"{len (pool )} part | base F1 (ws>=0.35) different MATCH toleransi")
