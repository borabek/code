...
def main ():
    ...
    lo =common -hi 
    print (f"ortak score kumesi {len (common )} temiz part  ->  dusuk-CP {len (lo )} | very-CP {len (hi )}\n")
    ...
    print (f"\ncok-CP orani score kumesinde %{100 *w :.1f}")
    json .dump ({k :{t :list (v )for t ,v in r .items ()}for k ,r in res .items ()},
    open ("results/cc_d_minv_regime.json","w"),indent =1 )
    print ("receipt -> results/cc_d_minv_regime.json")
