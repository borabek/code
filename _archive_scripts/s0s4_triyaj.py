...
kayitsiz: detection +0.0016, robot -0.0053; gecisler TP->FP 12 / FP->TP 9. Onceden written
kill GECMEDI.
...
KILL (S4, onceden yazildi): detection >= +0.005 VE fiziksel kusur orani >= -%5 mutlak.
"""
import io 
import json 
import os 
...
def main ():
    ...
    print (f"\n{'='*72 }\nS4 -- TRIYAJ (onbellekten, yeni mesh islemi none)\n{'='*72 }")
    # baseline: never onarim absent
    tb_d ,tb_r ,_ ,_ ,tb_k ,_ =kos (en_az =99 )
    print (f"{'triyaj':<28}{'detection':>9}{'robot':>9}{'kusur%':>9}{'TP->FP':>8}{'FP->TP':>8}")
    ...
    ("T4 >=2 ONAR, =3 RED",dict (en_az =2 ,red_esigi =3 ))):
        ...
        SON [ad ]={"detection":d_ ,"robot":r_ ,"kusur":kk ,"tp2fp":t2f ,"fp2tp":f2t }
        print (f"{ad :<28}{d_ :>9.4f}{r_ :>9.4f}{100 *kk :>8.1f}%{t2f :>8}{f2t :>8}")

    print (f"\nKILL (S4): detection >= baseline+0.005 VE kusur orani <= baseline-%5 mutlak")
    for ad ,v in SON .items ():
        g1 =v ["detection"]>=tb_d +0.005 
        g2 =v ["kusur"]<=tb_k -0.05 
        print (f"  {ad :<28} detection {v ['detection']-tb_d :+.4f} ({'OK'if g1 else 'x'})  "
    ...
    with io .open ("results/s0s4_triyaj.json","w",encoding ="utf-8")as f :
        ...
        for k in sorted (say )},
        "baseline":{"detection":tb_d ,"robot":tb_r ,"kusur":tb_k },
