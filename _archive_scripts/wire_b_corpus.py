# -*- coding: utf-8 -*-
"""TEL-B'yi GERCEK korpusa kos: each candidate for 10 channel ozelligi + TP/FP etiketi.

TEL-A'nin gerekcesi: hayatta remaining 263 FP'de mevcut 41 ozelligin EN IYISI AUC 0.36/0.61 -- kor.
TEL-B hipotezi: tel girisi KONTAKTA biter, alet agzi arm/yay yuvasinda -- difference ICERIDE.

ONCELIK NOTU: korpusun %89.5'i DUSUK-CP and orada ceiling 0.894 (potansiyel +0.054 GENEL);
very-CP dali only +0.016. Bu yuzden ONCE low-CP altkumesi.

Cikti: results/tel_b_feats.npz  (channel ozellikleri + label + part kimligi)
Not: sentetik dogrulama wire_b_kanal_profili.py inside, 3/3 gecti (tel-girisi/alet-agzi/tunel).
"""
...
def main ():
    ...
    from wire_b_kanal_profili import feats_for ,FEAT_NAMES 
    import wire_gate 
    ...
    print (f"{len (parts )} dusuk-CP part islenecek",flush =True )
    ...
    if os .path .exists (OUT ):# DEVAM-EDEBILIRLIK (uzun is, kesilebilir)
        ...
        print (f"  [devam] {len (done )} part already islenmis",flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp ,ngt )in enumerate (parts ,1 ):
        ...
        try :
            ...
            xb =feats_for (V ,F ,probs ,cps ,CE ,CT )# (n_aday, 10) channel ozellikleri
            x13 =wire_gate .feats_for (V ,F ,probs ,cps ,CE ,CT )# mevcut gate seti (AYNI candidates/order)

            # ETIKET: manufacturer GT'sine eksene-duyarli eslesme (big_arbiter with same konvansiyon)
            j =json .load (open (jf ,encoding ="utf-8-sig"))
    ...
    pid =np .array (PID ),mfg =np .array (MFG ),names =np .array (FEAT_NAMES ))
    yy =np .concatenate (Y )
    print (f"-> {OUT }  {len (yy )} candidate ({int (yy .sum ())} TP / {int ((yy ==0 ).sum ())} FP), "
    f"{len (done )} part  {time .time ()-t0 :.0f}s")
