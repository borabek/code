# -*- coding: utf-8 -*-
"""OYLAMA CIKARIMI + hakem skoru: oy -> cluster -> CP, mevcut urunle AYNI sette karsilastir.

Oylama basligi (train_vote.py) each baglanti-verteksinden ait oldugu instance MERKEZINE a offset
prediction eder. Burada: oy = verteks + offset; oylari radius-kumelemesi with grupla; each cluster BIR CP.
Bu, elle-ayarli post-proc'un (min_v 30 / cluster_mm 5) yerini takes -- FBI teshisi model-FN'lerinin
%63'unde modelin ZATEN atesledigini, kaybin turetmede oldugunu gostermisti.

Kume filtresi: a kumeyi CP saymak for at least --min-votes oy. (min_v'nin ogrenilmis muadili:
small-but-tutarli a fragment bile oylari AYNI merkeze yollarsa CP becomes -- min_v 30'un attigi state.)
...
def raw_forward (model ,meta ,V ,F ,device ):
    """(V, 5+3) HAM output. D.predict KULLANILAMAZ: that exp()/softmax'i TUM kanallara uygular and
    argmax'i 8 channel uzerinden takes -> offset kanallarini breaks and sinif etiketini yanlislar.
    Oylama modeli cfg loss='ce' with kuruldugu for last aktivasyon absent: first 5 ham logit, last 3 offset."""
...
def cluster_votes (votes ,weights ,radius ,min_votes ):
    """Oylari radius-kumelemesiyle grupla (agirlikca azalan cekirdek secimi)."""
    if not len (votes ):
    ...
    for i in order :
        ...
        if member .sum ()<min_votes :
            assigned [i ]=True # cekirdek single basina kaliyorsa at
            continue 
        assigned |=member 
        w =weights [member ]
...
def main ():
    ...
    print (f"{len (raw )} part | {os .path .basename (a .ckpt )} | oylama cikarimi (r={a .radius }mm)",flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp )in enumerate (raw ,1 ):
        try :
            ...
            out =raw_forward (model ,meta ,V ,F ,a .device )# HAM output sart (asagiya bak)
    ...
    print ("\n  comparison: urun recall_hard_s2 WEI = P0.597 R0.547 F1 0.571")
