# -*- coding: utf-8 -*-
"""COK-OLCEKLI inference plato-kirici (ROBOT_STRATEJI backup plani #3).

9k EGITIMI reddedildi (domain gap) but very-olcekli INFERENCE different: TEK urun modeli two
cozunurlukte (6k + 9k) kosulur, CP'ler BIRLESTIRILIR. Hipotez: minik acikliklar 9k'da,
buyukler 6k'da yakalanir -> union'dan different as AYNI modelin scale-tamamlayiciligi,
precision'i cökertmeden recall ekleyebilir. thesis_remesh STEP frame'i korur -> olcekler same
koordinatta, dogrudan birlestirilir. Ayni part setinde 6k-single / 9k-single / union'i skorlar.
...
def main ():
    ...
    print (f"{len (raw )} part | {os .path .basename (a .ckpt )} | olcekler {a .scales }",flush =True )
    ...
    print (f"  KESISIM (each olcek anlasir) F1={f1 :.3f}  P={pr :.3f}  R={rc :.3f}  (TP{T } FP{Fp } FN{Fn })")
