...
MESAFEYLE COZULEMEZ: this night olctum, GT'lerin most-yakin-komsu mesafesi medyan 5.15mm and
...
def istat (DER ):
    """(candidate_recall, oy_per_GT, aday_sayisi) -- GT'ye TESPIT toleransiyla bakar."""
    ul =0 ;n =0 ;na =0 
    for r in DER :
...
def oy_istat (DER ):
    """GT with matched adaylarin ORTALAMA oy count -- URUNUN KENDI `_votes` degeri.
    ...
    same sutunu tasiyor, comparison so elmayla elma becomes.
    """
    oy =[]
    for r in DER :
        G =np .asarray (r ["G"],float );Gd =np .asarray (r ["Gd"],float )
...
def main ():
    ...
    # Sebep: `diffusion_net` this night yeniden kuruldu and 8 parcalik dogrulamada konumlarin
    ...
    assert os .path .exists (KONTROL ),f"{KONTROL } none -- before bayrak KAPALI turetme kosmali"
    ...
    if not os .path .exists (CIKTI ):
        ...
        try :
            ...
            t0 =time .time ()
            r =subprocess .run ([sys .executable ,"derive.py","--output",CIKTI ],
        ...
        print (f"{CIKTI } already present, turetme atlandi")
    ...
    print (f"{'oy / matched GT':<22}{o0 :>18.3f}{o1 :>15.3f}{o1 -o0 :>+10.3f}")
    ...
    with io .open ("results/t3a_axis_pool_gate.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/t3a_axis_pool_gate.json")
