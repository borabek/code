...
"always al" 41 duzeltip 228 bozuyor (net -187). Kusur fikirde not, KARARDA: fixed a
...
TAVAN ONCE SABITLENDI: oracle (only faydali oldugunda al) +41 point = gecis +%4.7 ->
...
def main ():
    ...
    print (f"KAHIN (only faydaliyken al): +{duz_kahin } nokta = gecis "
    f"+{duz_kahin /max (total_ ,1 ):.2%} -> robot ~{0.4500 +0.673 *duz_kahin /max (total_ ,1 ):.4f}")
    print (f"  (each zaman al: +{duz_kahin } / -{boz_kahin } = net {duz_kahin -boz_kahin :+d})")
    ...
    with open ("results/s4_yarik_selector.json","w",encoding ="utf-8")as f :
        json .dump ({"n_cift":len (SAT ),"toplam_eslesme":total_ ,"oracle":duz_kahin ,
        "en_iyi_net":int (en_iyi ),"en_iyi_esik":en_iyi_esik ,
        "gecti":bool (gecti )},f ,indent =1 )
    print ("receipt -> results/s4_yarik_selector.json")
