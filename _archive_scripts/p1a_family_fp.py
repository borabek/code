# -*- coding: utf-8 -*-
"""P1-a: surviving-FP'ler AILE-lerde yogunlasiyor mu? (CPU, mevcut data)
Mekanizma: same body-platformu kardesleri AYNI opening tiplerini carries; a acikligin kimligi ailede
SABITTIR. Eger FP kutlesi few sayida aile-tipinde toplaniyorsa, aile basina TEK karar ('this opening-tipi
wire mi?') large kutleyi temizler -> P1-c'nin hedef listesi = this output.
GO: FP kutlesinin >=%60'i <=40 aile-tipinde. Ayrica: aile-ici FP TUTARLILIGI (same ailede hep same mi)."""
import json ,sys 
import numpy as np 
...
# OOF gate -> surviving FP (threshold 0.35)
oof =np .zeros (len (Y ))
...
surv =(Y ==0 )&(oof >=0.35 )
print (f"{len (Y )} candidate | surviving-FP (gate 0.35 passing sahte): {int (surv .sum ())}")
...
print (f"\n=== FP kutlesinin AILE yogunlasmasi ({len (fams )} different aile) ===")
...
# aile-ici tutarlilik: same ailede FP orani ne up to fixed
print (f"\n=== AILE-ICI TUTARLILIK (same ailede FP davranisi fixed mi) ===")
cons =[]
...
if cons :
    print (f"  very-parcali aile sayisi {len (cons )} | aile-ici FP-orani std ort {np .mean (cons ):.3f}")
