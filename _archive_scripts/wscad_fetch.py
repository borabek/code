...
def todo_census (mfg ,limit ,census_path =CENSUS ):
    ...
    if rejected :
        ...
        print (f"  [manufacturer uyusmuyor] {len (rejected )} part ATLANDI (numara same, brand baska): "
...
def main ():
    ...
    print (f"{len (rows )} part indirilecek ({a .mfg or 'all of them'}) | source: {src } | "
    f"already diskte olanlar atlaniyor",flush =True )
        ...
        for i ,r in enumerate (rows ,1 ):
            ...
            try :
                ...
                # more first manufacturer blogunda durduruyordu.
                    nostep +=1 ;streak =0 
                    print (f"  [{i }/{len (rows )}] {pid } STEP none (404)",flush =True )
                else :
                    fail +=1 ;streak +=1 
                    print (f"  [{i }/{len (rows )}] {pid } failed (status {res .get ('status')}, "
            ...
            time .sleep (a .delay )
        ctx .close ()
    print (f"\nbitti: {ok } indi, {fail } failed, {time .time ()-t0 :.0f}s",flush =True )
