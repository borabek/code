...
for r in R :
    ...
    if not len (X ):
        continue 
    rec_ .append ({"pid":r ["pid"],"mfg":r ["mfg"],"n_cp":len (G ),
    "cevre":float (np .median (X [:,i_cev ])),
    "conf":float (np .median (X [:,i_conf ]))})
print (f"candidate part {len (rec_ )}")
cv =np .asarray ([k ["cevre"]for k in rec_ ]);cf =np .asarray ([k ["conf"]for k in rec_ ])
...
score =z (cv )+z (cf )# DUSUK score = zayif mouth
for i ,k in enumerate (rec_ ):
    k ["score"]=float (score [i ])
    k ["oncelik"]=(1.0 if k ["mfg"]in ONCELIK else 0.0 )+(0.5 if k ["n_cp"]>=8 else 0.0 )
zor =sorted (rec_ ,key =lambda k :(-k ["oncelik"],k ["score"]))[:N_ZOR ]
kolay =sorted (rec_ ,key =lambda k :-k ["score"])[:N_KOLAY ]
...
io .open ("results/p2_label_kuyrugu.json","w",encoding ="utf-8"),indent =1 )
print ("receipt -> results/p2_label_kuyrugu.json")
