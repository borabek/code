# -*- coding: utf-8 -*-
"""U7: part-ici z-skoru YALNIZ COKUS halinde uygula (takasi yonlendirme with kaldir).

U6 DESENI (7 seri-disi split + 2 manufacturer-disi split, all of them uctan uca):
    D, TABAN ZAYIFKEN kazaniyor, TABAN SAGLIKLIYKEN kaybediyor.
      WEI disarida  baseline 0.4832 (COKUS) -> D +0.0869
      seri 25       baseline 0.5654         -> D +0.0639
      seri 10       baseline 0.5980         -> D +0.0086
      seri 17/15/30/32/16 baseline 0.66-0.78 -> D -0.008 .. -0.060
      PXC disarida  baseline 0.7203         -> D -0.0375
Yani D genel a iyilestirme DEGIL, a KURTARMA. Her parcaya uygulamak, cokmeyen parcalarda
vergi odemek demek.

FIKIR: cokusu CALISMA ANINDA sez, only orada z-score gate'e gec. Sezgi metadata GEREKTIRMEZ --
ham gate'in KENDI score dagilimi already cokusu gosteriyordu: olculmus teshis, cokus halinde model
adaylarin %10.3'une pozitif diyor, real %24.1. Yani parcanin most high ham skoru DUSUKSE,
gate that parcada kararsizdir.

YONLENDIRICI: parcanin ham-gate maksimum skoru, EGITIM korpusunun q'inci yuzdeligininin ALTINDA
whereas -> z-score gate. Esik EGITIM parcalarindan (test geometrisi haric) is computed; test tarafina
BAKMAZ.

KILL (onceden yazili, dorduncu sart dahil):
  (a) WEI-disi (cokus) kazancinin EN AZ %80'ini korumali          (>= A + 0.0695)
  (b) PXC-disi kaybi most extra 0.01 must be                          (>= A - 0.01)
  (c) familiar >= A - 0.01
  (d) 7 seri-disi bolmede ORTALAMA difference >= -0.002                  (vergiyi KALDIRMALI)
Dordu birden saglanmazsa yonlendirme DAGITILMAZ and D with A arasindaki secim ACIKCA raporlanir.
"""
import collections 
import json 
import os 
...
def main ():
    ...
    def olc (alt ,keep ):
        """A / D / yonlendirilmis(q) kollarini AYNI training bolmesiyle olc."""
        ca =rf (Xtr ,keep );cd =rf (Ztr ,keep )
        # ESIK: only EGITIM parcalarinin ham-gate maks skorlarindan. Test tarafina BAKMAZ.
        egit_maks =[]
        for u in np .unique (tr_pid [keep ]):
            i =np .where ((tr_pid ==u )&keep )[0 ]
            if len (i ):
                egit_maks .append (float (ca .predict_proba (Xtr [i ])[:,1 ].max ()))
        threshold ={q :float (np .quantile (egit_maks ,q ))for q in QLER }
        ...
        for r in alt :
            ...
            for ad in det :
                if sa is None :
                    ...
                    q =float (ad [1 :])
                    very =float (sa .max ())<threshold [q ]# COKUS SEZGISI
                    yon_say [q ]+=int (very )
                    s =sd if very else sa 
    ...
    print ("=== URETICI-DISI (asil axis) ===")
    print (f"{'split':<16}{'n':>4}{'A':>9}{'D':>9}"+"".join (f"{'R'+str (q ):>9}"for q in QLER )
    +"  yonlendirilen")
    MF ={}
    ...
    print ("\n=== SERI-DISI (7 split, vergi kontrolu) ===")
    ...
    print (f"\n{'arm':<8}{'WEI-disi':>10}{'PXC-disi':>10}{'familiar':>10}"
    f"{'seri ORT':>10}{'seri kazanan':>14}")
    A =MF ["WEI"]["A"],MF ["PXC"]["A"],MF ["familiar"]["A"]
    for k in ["A","D"]+[f"R{q }"for q in QLER ]:
        print (f"{k :<8}{MF ['WEI'][k ]:>10.4f}{MF ['PXC'][k ]:>10.4f}{MF ['familiar'][k ]:>10.4f}"
    ...
    print ("receipt -> results/u7_yonlendirme.json")
