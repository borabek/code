...
FINDING (2026-08-04): direction tarafindaki TUM oracle olcumleri `abs(v . Gd)` with secim yapiyordu.
Yani oracle, 180 derece TERS a yonu "mukemmel" sayip seciyordu; after sonuc ISARETLI
...
IKI SONUCU INVALID KILAR:
  1. `c1_direction_dictionary` KILL'i ("oracle 0.6263 < 0.70 -> Rota C KAPANIR", gecti=false).
     ...
     -- but secimi yapan oracle ISARETSIZDI. Sozlukte correct cevap YOK demek DEGIL.
...
BU BETIK: two sozlugu de AYNI parcalarda kurar and UC tavani ISARETLI kahinle olcer:
    ...
    BIRLESIK
Her biri for hem ISARETLI hem ISARETSIZ oracle raporlanir ki artefakt gorunur olsun.

CIKTI: results/_r8_sozluk.pkl (selector egitimi for) + results/r8_signed_oracle.json
DAGITIM YOK.
"""
...
from c1_direction_dictionary import yerel_yonler 
...
def main ():
    ...
    print (f"dictionary boyu: D1 medyan "
    ...
    print (f"\n{'dictionary':<12}{'ISARETLI oracle':>16}{'ISARETSIZ oracle':>17}{'artefakt':>11}")
    S ={"baseline":b }
    ...
    "C1 sozlugu ISARETLI tavana anlamli katki VERMIYOR -- kill valid kalir")
    print (f"\nHUKUM: {verdict }")
    with io .open ("results/r8_signed_oracle.json","w",encoding ="utf-8")as f :
        json .dump ({"baseline":b ,"sozlukler":S ,"c1_katkisi":bri -d1i ,
        "kapsam":kaps ,"verdict":verdict },f ,indent =1 )
    print ("receipt -> results/r8_signed_oracle.json")
