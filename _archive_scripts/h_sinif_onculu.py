...
KILL (olcumden before): detection F1 gerilerse duser. Recall artip precision'i gate geri
alamiyorsa duser.
"""
...
def main ():
    ...
    prior =acc /acc .sum ()
    print (f"sinif oncul (average probability): {np .round (prior ,4 ).tolist ()}",flush =True )
    ...
    def score (alpha ):
        ...
        for r in cache :
            ...
            # URUNLE AYNI must be: router very-CP derse promote with YENIDEN derive.
    ...
    print (f"{'alpha':>7}{'detection F1':>11}{'low':>9}{'very':>9}{'precision':>10}{'recall':>9}")
    res ={}
    for al in (0.0 ,0.15 ,0.3 ,0.5 ,0.7 ,1.0 ):
        w_ ,lo_ ,hi_ ,p_ ,r_ =score (al )
        res [al ]=dict (f1 =w_ ,dusuk =lo_ ,very =hi_ ,precision =p_ ,recall =r_ )
