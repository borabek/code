...
R5 TESHISI: kurtarma adayi 42, bozma riski tasiyan passing point 857. 1:20 ASIMETRI.
...
def main ():
    ...
    print (f"dictionary girisleri: {dict (sorted (adlar .items (),key =lambda x :-x [1 ]))}\n")

    def kos (sel_ ):
        """secim(i, S, p) -> point. Robot(FIZIKSEL) + detection returns."""
        rob ,det ,gg =[],[],[]
        for r in DER :
            h =H [r ["pid"]]
    ...
    print (f"{'giris':<14}{'robot(FIZ)':>12}{'d':>9}{'GA':>22}{'detection':>10}{'d':>9}{'kapsam':>9}")
    ...
    for a in sorted (adlar ,key =lambda x :-adlar [x ]):
        ...
        "detection":f1w (da ),"d_tespit":f1w (da )-b_det ,"n":n_uyg [0 ]}
    ...
    for r in DER :
        ...
        for i in range (len (P )):
            ...
            if len (Q ):
                ...
                feat =[len (Q ),float (dperp .mean ()),float (dperp .max ()),float (np .median (dperp )),
                float (np .linalg .norm (Q .mean (0 )-P [i ]))]
            else :
                feat =[0 ,0.0 ,0.0 ,0.0 ,0.0 ]
            feat +=[1.0 if a in S else 0.0 for a in 
            ("mouth","cember","open","kirpik","axis","kesit1","kesit3","kesit5")]
            FX .append (np .concatenate ([X [i ],feat ]))
    ...
    for q in (0.90 ,0.95 ,0.99 ):
        ...
        sel =oof >=t 
        print (f"  en yuksek %{100 *(1 -q ):.0f} score: {int (sel .sum ())} nokta, "
        f"gercekten kotu which {FY [sel ].mean ():.1%} (baseline {FY .mean ():.1%})")
    B_gecti =auc >=0.65 
    ...
    io .open ("results/r6_position_iki_test.json","w"),indent =1 )
    print ("\nmakbuz -> results/r6_position_iki_test.json")
