...
def esle_ayrinti (P ,Pd ,G ,Gd ,diag ):
    """detection modu eslesmesi -> (tp, fp, fn, secilen_aday_indisleri)."""
...
def main ():
    ...
    for r in DER :
        ...
        # KAHIN GATE: GT with matched adaylarin TAMAMI, baskasi absent
        ...
        # ADAY TAVANI: each matched GT TP, kalani FN, FP=0
    ...
    print (f"  su an   : dusuk {d0 :.4f} | very {c0 :.4f} -> agirlikli {S ['1 SU AN']['agirlikli_F1']:.4f}")
    print (f"  TAVAN   : dusuk {dT :.4f} | very {cT :.4f} -> agirlikli "
    f"{W ['low']*dT +W ['very']*cT :.4f}")
    # very-CP tavanda olsa, low-CP ne must be?
    ger_d =(0.85 -W ["very"]*cT )/W ["low"]
    print (f"\n  very-CP TAVANINDA ({cT :.4f}) olsaydi, dusuk-CP {ger_d :.4f} olmali")
    print (f"    -> dusuk-CP tavani {dT :.4f} | {'MUMKUN'if ger_d <=dT else 'IMKANSIZ'}")
    ger_c =(0.85 -W ["low"]*dT )/W ["very"]
    print (f"  dusuk-CP TAVANINDA ({dT :.4f}) olsaydi, very-CP {ger_c :.4f} olmali")
    print (f"    -> very-CP tavani {cT :.4f} | {'MUMKUN'if ger_c <=cT else 'IMKANSIZ'}")
    ...
    print (f"    -> 0.85 for oracle gate'in {0.85 /S ['2 KAHIN GATE']['agirlikli_F1']:.0%}'i gerekli")
    with io .open ("results/v1_tespit_085.json","w",encoding ="utf-8")as f :
        ...
        "candidate_recall":R ,"aday_tavani":{"low":dT ,"very":cT ,"agirlikli":ust },
