# -*- coding: utf-8 -*-
"""R13: YAKINLIK-ONCELIKLI B-REP EKSENI GT'yi more iyi buluyor mu? (kod DEGISMEDEN)

R12 bulgusu: adayin %74'unde 5mm inside a silindir VAR, but `axis_at`'in ACI KAPISI
(max_turn_deg=60) onlarin %40'ini reddediyor -- because seed direction, egimi goremeyen
channel_axis+snap'ten geliyor. Yanlis seed -> correct silindir "90 derece sapik" gorunuyor -> red.

BU BETIK URUNU DEGISTIRMEZ. Yalnizca olcer: silindiri YAKINLIKLA secip angle kapisini
kaldirirsak, axis GT'ye more mi yakin becomes? Kazanmazsa kod DEGISMEZ.
"""
...
def kos (DER ,ad ,bound_ =70 ,maxoff =5.0 ):
    A_mev =[];A_yak =[];kur =0 ;boz =0 ;none =0 
    n =0 
    for r in DER :
        if n >=bound_ :break 
        ...
        try :cyl =BX .cylinders (path [r ['pid']])
        ...
        for dd ,a_ ,b_ in sorted ((pe2 [a ,b ],a ,b )for a in range (len (P ))for b in range (len (G ))):
            ...
            if C is None :A_yak .append (am );none +=1 ;continue 
            rel =P [a_ ]-C ;t =(rel *AX ).sum (1 )
            dik =np .linalg .norm (rel -t [:,None ]*AX ,axis =1 )
            j =int (np .argmin (dik ))
            if dik [j ]>maxoff :
                A_yak .append (am );none +=1 ;continue 
    ...
    print (f'=== {ad } (n={len (Am )}, silindirsiz/uzak {none }) ===')
