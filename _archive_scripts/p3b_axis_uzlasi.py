...
ALGORITMA
  1. Parcadaki tum eligible silindirlerin eksenlerini topla (sign duyarsiz: +a ~ -a)
...
SECIM YANLILIGI: settings DEV yarisinda (SUPU/NIT/S+S/SE) secilir, SINAV yarisinda
...
MAKBUZ ="results/p3b_axis_uzlasi.json"
...
def main ():
    ...
    print (f"DEV oturtmasiz: detection {d0t :.4f} robot {d0r :.4f}\n")
    ...
    for tol in (5 ,10 ,15 ,25 ):
        ...
        for mm in (4 ,6 ,8 ,12 ):
            ...
            izgara [f"{tol }/{mm }"]={"detection":tf ,"robot":rf }
            line_ .append (rf )
            if rf >en_iyi_r :
    ...
    if en_iyi is None :
        print ("\nDEV'de no setting tabani gecmedi -> KOL CLOSED")
        with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
            json .dump ({"karar":"CLOSED","dev_taban_robot":d0r ,"izgara":izgara },f ,
            indent =1 )
        return 
    tol ,mm =en_iyi 
    print (f"\nDEV'de selected: tol {tol } derece / mesafe {mm }mm -> robot {en_iyi_r :.4f} "
    f"({en_iyi_r -d0r :+.4f})")
    ...
    with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
        ...
        "sinav_uzlasisiz":{"detection":s0t ,"robot":s0r },
        "sinav_uzlasili":{"detection":s1t ,"robot":s1r },
