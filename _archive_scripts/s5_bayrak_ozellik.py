...
kayitsiz (detection +0.0016, robot -0.0053) and four triyajin dordu de kill'i gecemedi.
...
satirlariyla, gate'e 3 bayrak eklemenin detection F1'ine etkisi. Gecerse training korpusunda
...
KILL: detection >= +0.010 VE manufacturer-disi dusmuyor.
"""
import io 
import json 
import os 
...
def main ():
    ...
    print (f"\nuctan uca detection: TABAN {a :.4f} -> +BAYRAK {b :.4f}  ({b -a :+.4f})")
    gecti =(b -a )>=0.010 
    print (f"KILL: detection >= +0.010 -> {'GECTI -- training korpusunda bayrak uretilir'if gecti else 'GECMEDI'}")
