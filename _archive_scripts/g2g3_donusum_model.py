...
G2 DONUSUM. Su an part-ici Z-SKOR. Z-score OLCEGE bagimlidir: a manufacturer ailesinin
mutlak buyuklukleri digerinden farkliysa z-score da kayar. SIRA (rank) donusumu scale-serbesttir
...
Hepsi AYNI ozniteliklerle; changed single sey karar yuzeyinin karmasikligi.
...
def main ():
    ...
    for ad in SON :
        ...
        if ok and ad !="G0 TABAN  zskor+rf":
            kazanan .append (ad )
        print (f"{ad :<24}{SON [ad ]['havuzlanmis']['detection']:>9.4f}"
    ...
    with io .open ("results/g2g3_donusum_model.json","w",encoding ="utf-8")as f :
        ...
        **{b :SON [ad ][b ]["detection"]for b in SON [ad ]if b .endswith ("-disi")}}
