...
TARGET: detection 0.7584 -> 0.85 (+0.092) | robot-hazir 0.5893 -> 0.70 (+0.111)

SORULAR (all of them mevcut onbellekten, YENI CIKARIM YOK):
  A. TESPIT bosluğu: oracle gate with ceiling ne? Boslugun ne kadari FP-ATMA, ne kadari FN-KURTARMA?
  B. ROBOT bosluğu: TESPIT EDILMIS (matched) noktalarin kaci YANAL'dan, kaci EKSEN'den,
     kaci IKISINDEN birden dusuyor? -- 0.111 nereden gelecek, bunu this number soyler.
  C. ROBOT KAHINI: each matched point for UYE havuzundaki EN IYI direction secilebilseydi
...
def main ():
    ...
    for r in DER :
        ...
        # URUNUN DUZELTME ZINCIRI (headline.py:208-216 with AYNI). Bunsuz measured_path sey dagitilan
        ...
        # detection edilmis ciftlerde HANGI criterion dusuyor?
        for a_ ,b_ in c_urun :
    ...
    print (f"  oracle TP {tp_k } -> gate'in KACIRDIGI correct candidate: {tp_k -tp_u }")
    ...
    print (f"\n{'='*66 }\nB. ROBOT BOSLUGU ({len (ya )} detection edilmis cift)")
    ...
    print (f"  only YANAL dusuyor : {(~y_ok &a_ok ).mean ():6.1%}")
    print (f"  only ACI  dusuyor  : {(y_ok &~a_ok ).mean ():6.1%}")
    ...
    print (f"  lateral <=2 VE aci-oracle <=10: {(y_ok &(ak <=10 )).mean ():6.1%}")
    ...
    with io .open ("results/x1_gap.json","w",encoding ="utf-8")as f :
