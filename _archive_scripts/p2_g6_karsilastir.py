...
OLCU: ADAY KAHINI (mukemmel gate, one-to-one Macar) + real detection (dagitilan gate v5).
...
import d6_record 
from p1_birlesme_sweep import pool ,oracle_ 
...
def main ():
    ...
    for ag ,kok in (("g5","results/_p1_olasilik"),("g6","results/_p1_olasilik_g6")):
        ...
        for ad ,(cl ,de ,oy )in (("3/10/5",(3.0 ,10.0 ,5.0 )),("1/2/2",(1.0 ,2.0 ,2.0 ))):
            ...
            res_ [f"{ag }|{ad }"]={"oracle":v ,"low":dl ,"very":ck ,"candidate":n_aday ,
            "part":n_p }
    a =res_ .get ("g5|3/10/5");b =res_ .get ("g6|3/10/5")
    c =res_ .get ("g5|1/2/2");e =res_ .get ("g6|1/2/2")
    if a and b :
        print (f"\nAG ETKISI (yaricap fixed 3/10/5): g5 {a ['oracle']:.4f} -> g6 "
        f"{b ['oracle']:.4f} = {b ['oracle']-a ['oracle']:+.4f} "
        f"(very-CP {b ['very']-a ['very']:+.4f})")
    if a and c :
        print (f"YARICAP ETKISI (ag fixed g5): {c ['oracle']-a ['oracle']:+.4f} "
        f"(very-CP {c ['very']-a ['very']:+.4f})")
    if a and e :
        print (f"IKISI BIRDEN: {e ['oracle']-a ['oracle']:+.4f} (very-CP {e ['very']-a ['very']:+.4f})")
