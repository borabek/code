...
WHY -- before each parcaya part-ici z-score dagitilmisti (D). Sonra 9 bolmede measured and desen
TEKDUZE output: z-score a KURTARMA, genel iyilestirme DEGIL.
...
Yani cokmeyen parcalarda vergi odeniyordu. Yonlendirme this vergiyi kaldirir:
    ...
    arm            WEI-disi  PXC-disi   familiar  7 seri ORT  manufacturer ORT
...
def main ():
    ...
    # because threshold full da "familiar data boyle gorunur" seviyesini tanimlamak for present; new a
    ...
    print (f"cokus esigi (training maks-score {int (Q *100 )}. yuzdelik): {threshold :.4f} "
    ...
    with open (PKL ,"wb")as f :
        ...
        "Uctan uca: WEI-disi 0.4832->0.5682, PXC-disi 0.7203->0.7029, familiar "
        "0.7410->0.7439, 7 seri-disi mean -0.0029. Her parcaya z-score "
    ...
    "Olcum: results/u7_yonlendirme.json, results/u8_yonlendirme_verify.json")
    ...
    print (f"path A (saglikli, ham kalir) : {np .round (yol1 [:3 ],3 ).tolist ()} "
    f"-> {'HAM'if np .allclose (yol1 ,threshold +0.2 )else 'DEGISTI'}")
    print (f"path B (cokus, z-skora gecer): {np .round (yol2 [:3 ],3 ).tolist ()} "
