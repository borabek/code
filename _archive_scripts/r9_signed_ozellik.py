# -*- coding: utf-8 -*-
"""R9: YON SECICISINE ISARET GOREN OZNITELIK VER -- remaining +0.0399'un pesinde.

R8 OLCUMU: dagitilan zincir 0.6181; D1 sozlugunun ISARETLI tavani 0.6580. Yakalanan
pay %48, remaining **+0.0399**.

ROOT CAUSE (koda bakinca gorunuyor, `d1_direction_distribute.satirla`):
    feat.append(... abs(float(v @ d0)) ...)      # ISARETSIZ
    feat.append(... abs(float(v @ UZ)) ...)      # ISARETSIZ
Sozluk BILEREK +- ciftleri iceriyor (obb/dik/yuzn). Bu two feature `+a` and `-a` for
BIREBIR AYNI degeri veriyor. Geriye remaining single difference, girisin DIZIN NUMARASI (`gi`).
...
def main ():
    ...
    for r in DER :
        ...
        for i in range (len (P )):
            ...
            if pe [i ,b ]>tol :
                continue # label only matched adaylarda tanimli
            d0 =Pd [i ]
    ...
    print (f"\nsatir {len (LY )} | correct giris {LY .mean ():.1%} | "
    ...
    print (f"\ntaban (dagitilan zincir): robot(FIZ) {b_rob :.4f} | detection {b_det :.4f}")
    print (f"ISARETLI ceiling (R8): 0.6580 -> remaining alan {0.6580 -b_rob :+.4f}")
    fn =lambda rows :f1w ([q for _ ,q in rows ])-f1w ([p for p ,_ in rows ])
    print (f"\n{'feature':<8}{'marj':>6}{'robot':>10}{'d':>9}{'GA':>22}{'detection':>10}{'pay':>7}")
    SON ={}
    for ad in ("ESKI","YENI"):
        for marj in (0.02 ,0.05 ,0.10 ):
            ...
            "d_robot":dr ,"ga":[lo ,hi ],"detection":f1w (da ),
            "kalan_pay_yuzde":pay }
    ...
    print (f"\nHUKUM: {'ISARETLI OZNITELIKLER KAZANDIRIYOR -> R10 (training korpusunda yeniden derive + dagit)'if val_ else 'signed features anlamli katki vermiyor'}")
    with io .open ("results/r9_signed_ozellik.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/r9_signed_ozellik.json")
