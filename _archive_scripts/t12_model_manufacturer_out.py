...
Aday duzeyinde (t11, DAGITILAN goreli esikle) measured:
    model              familiar   PXC-disi  WEI-disi  EN KOTU
...
ExtraTrees most iyi transferi veriyor VE familiar bedeli butce inside (-0.0093).

AMA kayitli uyari: "ExtraTrees does NOT transfer, manufacturer-disi -0.061" -- that measurement 13 SUTUN and
SABIT ESIK donemindendi; ikisi de degisti. Uctan uca olcmeden hangi hukmun valid oldugu bilinemez.

KILL: ExtraTrees, manufacturer-disi UCTAN UCA detection F1'de RF'i HER IKI bolmede de gecmezse ALINMAZ.
...
def main ():
    ...
    print (f"\n{'manufacturer (test)':<18}{'rule':<26}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    out ={}
    ...
    for ad ,v in out .items ():
        sb =v ["RF leaf3 (mevcut)"]["detection"]
        for nm ,vv in v .items ():
            if nm =="RF leaf3 (mevcut)":
                continue 
            print (f"  {ad }: {nm } {vv ['detection']-sb :+.4f}")
        ok =ok and (v ["ExtraTrees leaf3"]["detection"]>sb )
    print (f"\nKILL: goreli threshold HER IKI manufacturer-disi bolmede sabiti gecmezse DAGITILMAZ -> "
    f"{'GECTI'if ok else 'GECMEDI'}")
    json .dump (out ,open ("results/t12_model_manufacturer_out.json","w"),indent =1 )
    print ("receipt -> results/t12_model_manufacturer_out.json")
