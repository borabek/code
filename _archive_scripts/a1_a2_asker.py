...
A2 a A/B'dir: same gate, same parts, single difference min_v -> DELTA valid, seviye not.
"""
...
def main ():
    ...
    print (f"{len (sel )} part (26 dusuk / 22 very)",flush =True )
    ...
    print (f"  dogruluk {acc :.3f}  (TP{tp } FP{fp } FN{fn } TN{tn }) | very-CP yakalama {rec :.3f}")
    ...
    def score (mv ,add_avg =False ):
        ...
        v1_tp =n_tp =0 # matched GERCEKLERIN kaci single-oy (mekanizma kaniti)
        for r in cache :
    ...
    print ("=== A2 min_v izgara kenari (SEVIYE sismis, DELTA valid) ===")
    ...
    d5 =r5 ["w"]-r4 ["w"]
    print (chr (10 )+f"  A5 diff: {d5 :+.4f}  (mekanizma: tek-oy gercek payi "
    f"%{100 *r4 ['v1_share']:.0f} -> %{100 *r5 ['v1_share']:.0f})")
    print (f"  NOT: gate 4-uyeli dagilimla egitildi; A5 persistent olacaksa gate YENIDEN uydurulmali."
