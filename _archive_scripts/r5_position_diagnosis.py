...
DORT SORU:
  1. Yon secicisi dagitildiktan SONRA lateral tarafta ne up to oda kaldi?
     (angle passing but lateral gecmeyen ciftler = konumun real av sahasi)
  ...
  4. Konum TESPITI de oynatir (detection olcutu lateral max(3mm, %6 diag)). Kahin tespitte
     ne up to kazandirir/kaybettirir?

Bu betik HICBIR SEY DAGITMAZ. Sadece results/r5_position_diagnosis.json produces.
"""
...
def main ():
    ...
    assert os .path .exists (ONB ),f"{ONB } none -- before r4_merged_selector kosmali"
    PARCA =pickle .load (open (ONB ,"rb"))
    YS =wire_gate ._load ("results/yon_secici.pkl")
    print (f"dictionary {len (PARCA )} part | direction selector {'VAR'if YS else 'YOK'}",flush =True )

    # ---- sayaclar
    n_cift =0 # detection toleransinda matched (candidate, GT) cifti
    ...
    print (f"tespitte matched cift        {n_cift }")
    print (f"  acisi <=10 (signed)      {n_aci_gecen }  ({n_aci_gecen /max (n_cift ,1 ):.1%})")
    print (f"  lateral already <=2mm          {n_zaten }  ({n_zaten /max (n_cift ,1 ):.1%})")
    print (f"  AV: aci gecti + lateral>2mm  {n_av }  ({n_av /max (n_cift ,1 ):.1%} tum ciftin)")
    print (f"\n--- 2. KURTARMA vs HASAR ---")
    print (f"av sahasinda dictionary kurtarabiliyor  {n_kurtarma }/{n_av } "
    f"({n_kurtarma /max (n_av ,1 ):.1%})")
    print (f"passing noktada bozan giris VAR       {n_hasar_riski }/{n_zaten } "
    ...
    f"ort {D_ .mean ():.2f} | tamami-pozitif which candidate {float ((D_ >=0.999 ).mean ()):.1%} | "
    f"no-pozitif-olmayan {float ((D_ <=0.001 ).mean ()):.1%}")
    ...
    print (f"{'':<22}{'robot(FIZIKSEL)':>17}{'detection':>10}")
    ...
    "verdict":verdict },
    io .open ("results/r5_position_diagnosis.json","w"),indent =1 )
    print ("\nmakbuz -> results/r5_position_diagnosis.json")
