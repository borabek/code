...
Fikir: gate this ana up to SADECE 5-sinif olasiligini and el-yapimi geometriyi gordu. Modelin
last-layer-oncesi 128-dim vertex temsili very more zengin -- never kullanilmadi.
Cikarim (hook: last_lin girdisi):
  - part basina 128-dim vertex embedding (birincil model recall_hard_s2; embedding uzaylari
    models arasi hizali OLMADIGI for TEK model is used -- mean almak wrong olurdu)
  - AsIRI-UYUM kontrolu: 128-dim'i global PCA with 24'e indir (fit SADECE train-fold'da not,
    unsupervised oldugu for tum vertexlerden ornekle -- label kullanilmaz, leakage absent)
  - candidate basina: 6mm region mean+max (48) | 3/6/12/24mm halka mean (96) | global part mean (24)
                 + insert-channel probability profili (5 sinif x 4 depth = 20)
Cikti: results/embed_feats.npz (EMB, part_ids with hizali)  -> p2_p1_eval.py with GO/NO-GO."""
...
class EmbedTap :
    """last_lin'in GIRDISINI yakala = last-layer-oncesi vertex temsili (C_width=128)."""
...
def main ():
    ...
    print (f"{len (parts )} part | embedding dim {prim_model .last_lin .in_features } -> PCA {NPCA }",flush =True )

    # --- 1. gecis: PCA'yi fit etmek for vertex embedding ornekle (ETIKET KULLANILMAZ) ---
    samp =[]
    ...
    for k ,(mfg ,pid ,jf ,stp )in enumerate (parts ,1 ):
        try :
            ...
            for c in cps :
                ...
                f =[]
                m6 =dist <=6.0 
                f +=list (Z [m6 ].mean (0 ))if m6 .any ()else [0.0 ]*NPCA # region mean
                f +=list (Z [m6 ].max (0 ))if m6 .any ()else [0.0 ]*NPCA # region max
                for lo ,hi in RINGS :# halkalar
                    mr =(dist >lo )&(dist <=hi )
                    f +=list (Z [mr ].mean (0 ))if mr .any ()else [0.0 ]*NPCA 
                f +=list (gz )# global part
    ...
    pca_W =W ,pca_mu =mu )
    print (f"-> results/embed_feats.npz  {E .shape [0 ]} candidate x {E .shape [1 ]} embedding-feature, "
    f"{len (NGT )} part  {time .time ()-t0 :.0f}s")
