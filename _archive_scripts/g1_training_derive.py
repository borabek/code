...
AMA uctan uca detection kipirdamadi (+0.0042, noise). Sebebi K1'in KENDI SINIRI:
...
def main ():
    ...
    for k ,(mfg ,pid ,jf ,sp )in enumerate (E ):
        ...
        if k %20 ==0 and k >basla :
            ...
            remaining =(len (E )-k )/max (hz ,1e-6 )/60 
            print (f"  {k }/{len (E )} ok={ok } candidate={len (Y )}  {time .time ()-t0 :.0f}s "
            f"(remaining ~{remaining :.0f} dk)",flush =True )
    ...
    with io .open (CIKTI .replace (".npz","_koken.json"),"w",encoding ="utf-8")as f :
        json .dump ({"ckpt":CKS ,"n_parca":ok ,"n_aday":len (Y ),"skipped":sorted (ATLA ),
        "eksen_havuz":True ,
