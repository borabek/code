...
KILL: v6 rt2'yi detection F1'de gecmezse dagitilan gate does not change.
"""
...
def main ():
    ...
    for tag in ("rt2","v6"):
        ...
        print (f"gate {tag }: {int (keep .sum ())}/{len (pids )} candidate with egitildi",flush =True )
    ...
    print (f"{len (DER )} part | {sum (len (r ['P'])for r in DER )} candidate (AYNI pool, iki gate for de)",
    flush =True )
    ...
    print (f"\n{'gate training verisi':<24}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    R ={}
    for tag in ("rt2","v6"):
        det ,rob =kos (tag )
        R [tag ]=(det ,rob )
    ...
    print (f"\nKILL: v6 rt2'yi tespitte gecmezse gate DEGISMEZ -> diff {d_ :+.4f} => "
