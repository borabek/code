# -*- coding: utf-8 -*-
"""P1.6 smoke test seti: robot_cp'yi temsili parcalarda runs, output sagligini dogrular (crash absent,
makul CP count, dup absent, tier atanmis). normal WEI / PXC-tipik / high-CP PXC / hard aile (2770/3002).
...
def validate (pid ,cps ,tag ):
    ...
    if dmin <2.0 :ok =False ;msgs .append (f"DUP: {dmin :.2f}mm cift present")
    # tier atanmis
...
def main ():
    ...
    for pid ,desc ,mode ,N in CASES :
        path =stp .get (pid )
        if not path :
            print (f"[ATLA] {pid } ({desc }): STEP none");continue 
        try :
            if mode =="highcp":
    ...
    return 0 if (ntested >0 and npass ==ntested )else 1 # regression gate: nonzero exit ten any FAIL
