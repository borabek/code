...
Her WEI held-out parcasi for: prob'lari al, urun CP'lerini derive, manufacturer G'ye axis-aware esle;
each FN (unmatched G) for, G'nin (mesh frame'e tasinmis) 8mm cevresinde kac CE/CT verteks present +
...
def main ():
    ...
    if n_fn :
        ...
        print (f"  FN'lerde yakin baglanti-verteks sayisi: medyan {np .median (near_counts ):.0f}  max-probability medyan {np .median (near_maxp ):.2f}")
