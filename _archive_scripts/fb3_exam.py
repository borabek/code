...
KILL (onceden): detection +0.01 VE grup bootstrap GA'si sifiri disliyor. Uretici-disi DUSMEMELI.
"""
import io 
import json 
import os 
...
def main ():
    ...
    print (f"\n{'arm':<20}{'candidate':>7}{'candidate-recall':>13}{'TESPIT':>9}{'+alet feat.':>11}")
    SON ,ROWS ={},{}
    for ad ,K in KOL .items ():
        ...
        SON [ad ]={"candidate":int (len (RY )),"candidate_recall":ul /max (ngt ,1 ),
        "detection":f1w (d0 ),"tespit_alet":f1w (d1 )}
        ROWS [ad ]=(d0 ,d1 ,gg )
        s =SON [ad ]
        print (f"{ad :<20}{s ['candidate']:>7}{s ['candidate_recall']:>13.4f}{s ['detection']:>9.4f}"
        f"{s ['tespit_alet']:>11.4f}")

    if "A_ESKI4_duz"in ROWS :
        baseline =SON ["A_ESKI4_duz"]["detection"]
        fn =lambda rows :f1w ([q for _ ,q in rows ])-f1w ([p for p ,_ in rows ])
        print ()
        for ad in SON :
            for j ,et in ((0 ,"detection"),(1 ,"detection+alet")):
                if ad =="A_ESKI4_duz"and j ==0 :
                    continue 
                _ ,lo ,hi =measure_set .grup_bootstrap (
                list (zip (ROWS ["A_ESKI4_duz"][0 ],ROWS [ad ][j ])),ROWS [ad ][2 ],fn ,n =2000 )
                v =SON [ad ]["detection"if j ==0 else "tespit_alet"]
                print (f"  {ad } [{et }]: {v :.4f} ({v -baseline :+.4f}) GA[{lo :+.4f},{hi :+.4f}] "
                f"{'GERCEK'if (lo >0 or hi <0 )else 'noise'}")
    with io .open ("results/fb3_exam.json","w",encoding ="utf-8")as f :
        json .dump (SON ,f ,indent =1 )
    print ("receipt -> results/fb3_exam.json")
