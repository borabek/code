...
COZUM (this night IKI KEZ whereas yaradi -- cokus yonlendirmesi and yarik secicisi): before
...
KILL (onceden yazili): robot-hazir >= +0.02 VE detection kaybi < 0.005 VE GA sifiri dislamali.
"""
import io 
import json 
import os 
...
def main ():
    ...
    wrong =(aci >10.0 ).astype (int )
    print (f"aci selector egitimi: {len (RY )} satir | wrong ratio {wrong .mean ():.1%} "
    f"| {len (set (RG ))} grup",flush =True )

    sec =RandomForestClassifier (n_estimators =400 ,min_samples_leaf =5 ,n_jobs =-1 ,
    random_state =0 ).fit (RX ,wrong )
    ...
    print (f"\n{'arm':<22}{'detection':>10}{'ROBOT':>10}{'d_robot':>10}")
    ...
    with io .open ("results/q4_angle_selector.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/q4_angle_selector.json")
