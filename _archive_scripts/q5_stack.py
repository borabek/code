...
Elimde two ALT-ESIK arm present, ikisi de real but single baslarina +0.02'yi gecmiyor:
    ACI SECICI      robot  +0.0126  GA [+0.0002, +0.0325]   detection bedeli 0.0000
    DUR/DEVAM KESIM detection +0.0150  5/5 seed, GA sifiri kil payi iceriyor
...
KILL (yigin for, onceden yazili): robot >= +0.02 VEYA detection >= +0.02, digerinde loss
...
def main ():
    ...
    for u in np .unique (oof_pid ):
        ...
        # oracle K: F1'i at most yapan onek
        en ,enk =-1 ,0 
    ...
    KOL ={"A dagitilan":(False ,False ),"B +angle selector":(True ,False ),
    "C +kesim":(False ,True ),"D YIGIN (ikisi)":(True ,True )}
    print (f"\n{'arm':<20}{'detection':>10}{'ROBOT':>10}{'d_tespit':>10}{'d_robot':>10}")
    SON ,PAR ={},{}
    ...
    for ad in KOL :
        if ad .startswith ("A"):
            continue 
        for mi ,ad2 in ((1 ,"robot"),(0 ,"detection")):
    ...
    with io .open ("results/q5_stack.json","w",encoding ="utf-8")as f :
        json .dump ({"kollar":{k :list (v )for k ,v in SON .items ()},"ga":OUT },f ,indent =1 )
    print ("receipt -> results/q5_stack.json")
