# -*- coding: utf-8 -*-
"""g9 (sig 2mm boyama) vs g7 (deep 6mm) -- D6'da ADAY KAHINI + real detection/robot.
...
real detection/robot whereas dagitilan gate with uctan uca sonucu gives.
"""
...
def main ():
    ...
    for ck ,ad in zip (a .ckpt ,a .ad ):
        ...
        print (f"\n{ad :<8} KAHIN {kh :.4f} (very {kc :.4f}) | TESPIT {tf :.4f} | "
        f"ROBOT {rf :.4f} (very {rc :.4f}) | error {error }")
        res_ [ad ]={"oracle":kh ,"kahin_cok":kc ,"detection":tf ,"robot":rf ,
        "robot_cok":rc ,"error":error }
    if len (res_ )==2 :
        a1 ,a2 =a .ad 
        print (f"\nFARK ({a2 } - {a1 }): oracle {res_ [a2 ]['oracle']-res_ [a1 ]['oracle']:+.4f} | "
        f"detection {res_ [a2 ]['detection']-res_ [a1 ]['detection']:+.4f} | "
        f"robot {res_ [a2 ]['robot']-res_ [a1 ]['robot']:+.4f}")
        ok =res_ [a2 ]["oracle"]>res_ [a1 ]["oracle"]
        print (f"KILL OLCUTU (oracle gecmeli): {'GECTI'if ok else 'KALDI -> SIG BOYAMA GERI AL'}")
    with io .open ("results/p9_ag_compare.json","w",encoding ="utf-8")as f :
        json .dump (res_ ,f ,indent =1 )
    print ("receipt -> results/p9_ag_compare.json")
