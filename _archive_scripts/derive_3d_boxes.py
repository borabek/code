# -*- coding: utf-8 -*-
"""TEZ-ONERILI 3D BOUNDING-BOX TURETICI (Masterarbeit row 2099 Ausblick).

Tez, 2D YOLOv6'yi Kontaktierung/Kabeleinfuehrung for ACIKCA REDDEDIYOR (ortulme; "nicht ueber mehr
gelabelte Daten loesbar") and YERINE sunu oneriyor:
...
Bu script full that adimi yapar: MEVCUT vertex etiketlerinden (corpus 5-sinif + insan kismi etiketleri)
baglanti-sinifi (CableEntry|Contact) INSTANCE'larini mesh-bagliligiyla separates and each instance for
3D kutu (centre, size, ana axis, verteks count) turetir. Ek etiketleme maliyeti YOK.

Cikti: results/boxes3d/<pid>.json + ozet istatistik (instance/part, kutu size dagilimi) -- a 3D
detection agi for anchor tasarimina and training etiketlerine temel.
Neden bizim for correct hedef: bugunku zayifligimiz INSTANCE AYRIMI (fragment/clustering); detection agi
N ayri CP'yi dogrudan prediction eder, segmentasyon+clustering zincirini skips.
"""
import os ,sys ,json 
import numpy as np 
...
OUT ="results/boxes3d"
MIN_V =8 # a instance sayilmak for at least verteks (small noise fragmentlerini eler)
...
def box_of (V ,vids ):
    ...
    # ana axis (PCA) -- acikligin yonelimi hakkinda ipucu
    Q =P -P .mean (0 )
    try :
        _ ,_ ,Vt =np .linalg .svd (Q ,full_matrices =False )
        axis =Vt [-1 ]# most small varyans yonu ~ surface normali
    except Exception :
...
def main ():
    ...
    "_label_targets_recall","_label_targets_recall_hard","_label_targets_recall_r3"):
        if os .path .isdir (d ):
            for s in T .load_extra (d ):
                samples .append ((d ,s ))
    print (f"{len (samples )} etiketli part (corpus + insan kismi)",flush =True )
    ...
    print (f"\n=== 3D KUTU TURETIMI ===")
    print (f"  part: {len (per_part )} | kutu written part: {n_written } | total instance: {int (nb .sum ())}")
    print (f"  instance/part: medyan {np .median (nb ):.0f} | ort {nb .mean ():.1f} | max {nb .max ()if len (nb )else 0 }")
    print (f"  0 instance'li part: {(nb ==0 ).sum ()}")
    if len (E ):
        d =np .sort (E ,axis =1 )# each kutunun kenarlari kucukten buyuge
        print (f"  kutu kenarlari (mm): en kisa medyan {np .median (d [:,0 ]):.1f} | orta {np .median (d [:,1 ]):.1f} | en uzun {np .median (d [:,2 ]):.1f}")
        print (f"  -> anchor tasarimi for tipik CP kutusu ~ {np .median (d [:,0 ]):.1f} x {np .median (d [:,1 ]):.1f} x {np .median (d [:,2 ]):.1f} mm")
