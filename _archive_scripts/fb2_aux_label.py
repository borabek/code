...
ETIKET GURULTUSU (bilinerek kabul edilir): "unmatched = alet" varsayimi, ureticinin
...
(listelenmis CP'lerde %70.1) -- i.e. unmatched aciklklar gercekten tel girisi DEGIL.
"""
import io 
import json 
import os 
...
def main ():
    ...
    for d in DIZINLER :
        ...
        for p in os .listdir (d ):
            ...
            if os .path .isdir (path )and os .path .exists (os .path .join (path ,f"{p }.obj")):
                hedef .append ((d ,p ,path ))
    print (f"{len (hedef )} etiketli part | manufacturer JSON'u which: "
    f"{sum (1 for _ ,p ,_ in hedef if p in E )}",flush =True )

    import trimesh 
    ok =tel_t =alet_t =0 
    t0 =time .time ()
    for k ,(d ,pid ,path )in enumerate (hedef ,1 ):
        if k %15 ==0 :
        ...
        if pid not in E :
            continue 
        jf ,stp =E [pid ]
        out_ =os .path .join (path ,f"{pid }.aux.txt")
        if os .path .exists (out_ ):
            ok +=1 ;continue 
        try :
            m =trimesh .load (os .path .join (path ,f"{pid }.obj"),process =False )
            ...
            for b in range (len (G ))):
                if not np .isfinite (dd )or dd >tt or a_ in matched or b_ in ug :
                    continue 
                matched .add (a_ );ug .add (b_ )
                # --- vertex etiketi
