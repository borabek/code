...
why). Measured ten 20 true pairs and 20 deliberately wrong ones, with the threshold chosen ten a
different sample and seed than the one it was tested ten: 20/20 true pairs pass, 15/20 wrong pairs
are caught.
...
one exact manufacturer id, so a sibling swap cannot arise ten that path. The hazard this file does
...
# WHICH TEST, AND WHY THIS ONE. Four candidate statistics were measured ten 14 true pairs and 14
...
# The cut sits in the measured gap, ten the safe side of it. n=14 per group: wide margin, small
...
def main ():
    ...
    help =f"failed dosyalari {REJECT_DIR }/ altina TASI")
    ...
    f"(threshold: residual <= {MAX_RESIDUAL_MM }mm, bbox farki <= {MAX_BBOX_REL :.0%})",flush =True )
    ...
    open (OUT ,"w"),indent =1 )
    print (f"\ngecti {npass } | KALDI {nfail } | atlandi (JSON none) {nskip }")
    print (f"receipt -> {OUT }")
