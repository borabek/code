...
Runs inference before over a manifest-pinned part list, decodes at the deploy
...
def build_family_map (stp_dir ):
    ...
    (millions of CARTESIAN_POINTs ten our big parts), so PRODUCT routinely sits far
    ...
    that got dropped. A family breakdown built ten that map cannot answer the only
    question it exists to answer ("is the error concentrated or spread?").

    Now: scan the WHOLE file (chunked, early-exit ten the first PRODUCT), and fall
...
def main (argv =None ):
    ...
    for p in parts :
        ...
        fam =fam_map .get (m .group (1 ),"?")if m else str (p .part_nr ).split (".")[0 ]
