...
"TEK a manufacturer with egit, baskasinda dene" demek -- mumkun which is EN ZOR setting. Gercek dagitimda
...
Hipotez: more kisitli model = more few ezber = more iyi transfer. Bedeli familiar veride.
KILL: a model, manufacturer-disi EN KOTU durumda RF'i gecmeli VE familiar veride 0.02'den extra
kaybettirmemeli.
"""
import json 
import numpy as np 
...
def main ():
    ...
    for ad ,mk in MOD .items ():
        ...
        defn =f1 (karar (o ,pids ),y )
        mv =[]
        for u in U :
            te =mfg ==u 
            s =mk ().fit (X [~te ],y [~te ]).predict_proba (X [te ])[:,1 ]
            mv .append (f1 (karar (s ,pids [te ]),y [te ]))
        print (f"{ad :<22}{defn :>9.4f}"+"".join (f"{x :>10.4f}"for x in mv )+f"{min (mv ):>10.4f}",
        flush =True )
        out [ad ]={"familiar":defn ,"manufacturer":mv ,"en_kotu":min (mv )}
    t =out ["RF leaf3 (mevcut)"]
    print ("\nKARAR (kill: most kotu > mevcut VE familiar loss <= 0.02):")
    for ad ,v in out .items ():
        if ad .startswith ("RF leaf3"):
            continue 
        g =v ["en_kotu"]>t ["en_kotu"]and (t ["familiar"]-v ["familiar"])<=0.02 
        print (f"  {ad :<22} en kotu {v ['en_kotu']-t ['en_kotu']:+.4f} | familiar "
        f"{v ['familiar']-t ['familiar']:+.4f} -> {'GECTI'if g else 'GECMEDI'}")
