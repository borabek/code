...
DataSet5 with eligible corpus 1926 -> 4720 part / 23 manufacturer became. Gate egitimi for new
...
ATLA_DOSYA ="results/_d5_atla.txt"# kilitlendigi known parts
...
def dengeli_sira (E ):
    ...
    for k in g :
        ...
        # but listelerden eleman CIKARMIYORUM, indeksle geziyorum -> lists never bosalmiyor,
...
def main ():
    ...
    ap .add_argument ("--pids-file",default ="",help ="only this dosyadaki pid'leri derive")
    ...
    if a .total_ >1 :
        ...
        GEO =f"results/_geo_new{ek }_{a .vardiya }.json"
        ISARET =f"results/_d5_su_an{ek }_{a .vardiya }.txt"
    elif ek :
        CIKTI =f"results/_der_yeni{ek }.pkl";GEO =f"results/_geo_new{ek }.json"
    ATLA =set ()
    ...
    for _k in range (a .total_ if a .total_ >1 else 1 ):
        ...
        if os .path .exists (_f ):
            with open (_f ,"rb")as _h :
                present |={r ["pid"]for r in pickle .load (_h )}
    OUT ,GEOD =[],{}
    if os .path .exists (CIKTI ):
        with open (CIKTI ,"rb")as f :
            OUT =pickle .load (f )
        present |={r ["pid"]for r in OUT }
        if os .path .exists (GEO ):
            GEOD =json .load (io .open (GEO ,encoding ="utf-8"))
        print (f"devam: {len (OUT )} part already turetilmis",flush =True )
    ...
    hedef =dengeli_sira ([t for t in E if t [1 ]not in present ])
    if a .total_ >1 :
    ...
    print (f"eligible corpus {len (E )} | already present {len (present )} | TURETILECEK {len (hedef )}")
    ...
    for k ,(mfg ,pid ,jf ,stp )in enumerate (hedef ,1 ):
        if k %5 ==0 :
            hiz =(time .time ()-t0 )/k 
            remaining =hiz *(len (hedef )-k )/60 
            print (f"  {k }/{len (hedef )}  {time .time ()-t0 :.0f}s  error={error }  "
            f"({hiz :.1f}s/part, remaining ~{remaining :.0f} dk)",flush =True )
        ...
        try :
            ...
            # Parca-yerel temp directory: AYNI parcada same k_eig'i paylasan models
