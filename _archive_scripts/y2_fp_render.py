...
CIKTI: results/fp_denetim/<pid>_<i>.png  + results/fp_denetim_measurement.json (depth sayilari)
"""
import io 
import json 
import os 
...
def main ():
    ...
    for k ,(pid ,idxs )in enumerate (sorted (byp .items ()),1 ):
        ...
        for i in idxs :
            ...
            fig .savefig (path ,dpi =88 ,bbox_inches ="tight");plt .close (fig )
            OLCUM .append ({"idx":i ,"pid":pid ,"mfg":f_ ["mfg"],"regime":f_ ["regime"],
            "png":path ,"centre":None if np .isnan (center_ )else float (center_ ),
    ...
    with io .open ("results/fp_denetim_measurement.json","w",encoding ="utf-8")as f :
        json .dump (OLCUM ,f ,indent =1 )
    print (f"\n{len (OLCUM )} kare -> {DIZIN }/")
    d =[o for o in OLCUM if o ["centre"]is not None and o ["halka"]is not None ]
    if d :
        diff =np .array ([o ["centre"]-o ["halka"]for o in d ])
        print (f"depth farki: medyan {np .median (diff ):.2f} mm | "
        f">1mm {np .mean (diff >1 ):.0%} | >3mm {np .mean (diff >3 ):.0%}")
    print ("receipt -> results/fp_denetim_measurement.json")
