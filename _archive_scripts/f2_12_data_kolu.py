...
URUNUN DECISION YOLU KULLANILIR: score `wire_gate.decision_score`, maske `decision_mask`.
Olcum betiklerinin gate'i ELDE yeniden kurmasi 2026-08-01'de yakalanmis a hataydi --
urunun `apply()` yolu arada yonlendirme does and two path ayrisirsa measured_path sey urunun
...
MAKBUZ ="results/f2_12_data_kolu.json"


def egit (npz ,seed =0 ):
    """Dagitilan gate with AYNI recete: RF 400 / leaf 3, 58 ham -> part-ici z-score -> 116."""
    from sklearn .ensemble import RandomForestClassifier 
    d =np .load (npz ,allow_pickle =True )
    X =np .hstack ([d ["X22"],d ["XR"]]).astype (float )
    pid =np .asarray (d ["pids"],str )
    # PARCA-ICI z-score PARCA BAZINDA uygulanir (calisma aninda da oyle)
...
def main ():
    ...
    # **TEK part** present; single parcalik a bucket noise produces, gerileme not. Kapiyi
    ...
    print ("     Ayrica measurement kumesi PXC+WEI agirlikli; unseen manufacturer sorusu D5-4 with yanitlanir.")
