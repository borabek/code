...
Esik siralamayi degistirmez, CALISMA NOKTASINI changes. Su anki rule:
    sec  <=>  score >= max(0.5 x parcanin_en_yuksegi, 0.25)
...
def main ():
    ...
    print (f"TAM TARAMA en iyisi    {arg }  {en :.4f}   (+{en -su_an :.4f})  <- SISIK, same kumede secildi")
    ...
    for sec_alt ,olc_alt in ((altA ,altB ),(altB ,altA )):
        ...
        print (f"  yarida selected threshold {a } -> diger yaride measured ({len (olc_alt )} part)")
    ...
    with io .open ("results/g5_threshold.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/g5_threshold.json")
