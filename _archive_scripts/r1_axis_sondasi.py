# -*- coding: utf-8 -*-
"""R1 SONDASI: DIK axis vakalarinin kaci KURTARILABILIR?

Gorulmemis ureticide matched ciftlerin %36'sinda candidate ekseni GT eksenine DIK (>80 deg).
Bu a secim hatasi (median error 0.0 deg -- i.e. yari already mukemmel, remaining wrong axis).
...
for pid ,p ,pd ,gd in sec :
    try :
        if pid not in mesh_cache :
            V ,F =step_to_mesh (path [pid ]);V ,F =thesis_remesh .remesh_uniform (V ,F ,target =6000 )
...
print (f"measured_path {olc } | kanal ekseni GT'yi (<=10deg) buluyor: {kur } (%{100 *kur /max (olc ,1 ):.0f})")
