...
DUZELTME (two degisiklik):
  ...
  2. `mevcut` 2. asamanin secenekleri arasindan CIKARILIR. 1. stage already onu reddetti;
     options between birakmak prior'u geri sokar. Karar kurali: 1. stage esigi
     GECERSE, alternatiflerin argmax'i alinir and MUTLAK confidence esigi aranir.

Bu, hasar asimetrisine (1:20) eligible single mesru mimari: YUKSEK KESINLIKLI gate + kosullu secim.
...
from r7_position_iki_asamali import dagilim_oz ,GIRIS 
...
def main ():
    ...
    for r in DER :
        ...
        for i in range (len (P )):
            ...
            kotu =int (pe [i ,b ]>2.0 )
            AX .append (np .concatenate ([X [i ],feat ]));AY .append (kotu )
    ...
    f"{'robot':>10}{'d':>9}{'GA':>22}{'detection':>10}{'d':>9}")
    SON ={}
    ...
    print (f"\nTARAMA en iyisi (UST SINIR, secim yanlisi present): robot {eniyi ['d_robot']:+.4f} "
    f"(threshold %{100 *eniyi ['esikQ']:.0f}, confidence {eniyi ['confidence']}) "
    f"GA[{eniyi ['ga'][0 ]:+.4f},{eniyi ['ga'][1 ]:+.4f}] | detection {eniyi ['d_tespit']:+.4f}")
    ...
    with io .open ("results/r7b_position_kosullu.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/r7b_position_kosullu.json")
