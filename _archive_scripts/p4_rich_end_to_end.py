...
KILL: decision_criterion -- familiar >= -0.01, split ORTALAMASI >= +0.01, most kotu split kotulesmesin,
...
def main ():
    ...
    for ad ,sec in KOL .items ():
        ...
        for b ,keep ,alt in BOLME :
            ...
            sat +=f"{f1w (det ):>12.4f}"
        sat +=f"{f1w (PARCA [(ad ,'familiar')][1 ]):>10.4f}"
    ...
    for ad in KOL :
        ...
        passing [ad ]=bool (k )
        print (f"\nA -> {ad }:  {k }")
    kazanan =max ((a for a in passing if passing [a ]),
    key =lambda a :SON [a ]["familiar"],default =None )
    print (f"\nSONUC: {kazanan +' DAGITILABILIR'if kazanan else 'HICBIRI GECMEDI'}")
    with io .open ("results/p4_rich_end_to_end.json","w",encoding ="utf-8")as f :
        json .dump ({"kollar":SON ,"passing":passing ,"kazanan":kazanan ,
        "robot":{a :float (f1w (PARCA [(a ,"familiar")][1 ]))for a in KOL }},f ,indent =1 )
    with open ("results/p4_parca.pkl","wb")as f :
        pickle .dump (PARCA ,f )
    print ("receipt -> results/p4_rich_end_to_end.json")
