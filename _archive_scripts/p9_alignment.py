...
calistiriyor -> gate, egitimde gordugunden FARKLI candidate dagilimi skorluyor. Bu, this night
+0.1273 odeyen "measured_path != running" sinifinin aynisi.
...
def main ():
    ...
    print (f"FINAL test {len (sel )} part (70 dusuk / 30 very), {len (test_fams )} aile disarida",
    flush =True )
    ...
    for tag ,npz in (("A",NPZ4 ),("B",NPZ5 )):
        ...
        gates [tag ]=clf 
        print (f"  gate {tag }: {int (keep .sum ())} candidate with egitildi",flush =True )
    ...
    "thrA":thrA ,"thrB":thrB },
    open ("results/p9_alignment.json","w"),indent =1 )
    print ("receipt -> results/p9_alignment.json")
