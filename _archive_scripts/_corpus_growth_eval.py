...
DURUSTLUK KURALLARI (see cp_config.locked_holdout_policy_2026_07_28):
  ...
  * `seen` bayrakli parts (etiketleri segmentasyon egitiminde seen) EGITIMDE kullanilabilir
  ...
  * Iki arm AYNI protokolde, AYNI skorlanan part kumesinde olculur -- single changed EGITIM havuzu.
...
def run (paths ,label ,score_pids ,lock ,fams ):
    ...
    for f_out in folds :
        ...
        k =te &(oof >=best_t )# selected threshold DIS katta uygulanir
...
if __name__ =="__main__":
    ...
    # ORTAK SKOR KUMESI: two kolda da found, TEMIZ, kilitli-olmayan parts
    _ ,_ ,P0 ,_ ,S0 ,_ =load (BASE )
    _ ,_ ,P1 ,_ ,S1 ,_ =load (BASE +NEW )
    common ={q for q in set (P0 )if S0 .get (q ,1 )==0 and q not in LOCK }
    print (f"ortak score kumesi: {len (common )} temiz part (iki kolda da same)\n")
