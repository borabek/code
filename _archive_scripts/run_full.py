# Thin convenience wrapper: run the full training ten the real ABB corpus with
...
# repo-relative so the same command works ten every machine the repo is cloned
...
# weight decay, dropout, grad clip, centernet heat loss, leakage-safe geometry
# split, frozen test split). The decode flags are the HELD-OUT-best operating point found
...
("--backbone","knngraph"),
("--device","cuda"),
("--max-gpu-verts","7000"),# safe ten a 4 GB GPU; OOM falls back to CPU
("--epochs","200"),
...
("--knn-dropout","0.1"),
("--heat-loss","centernet"),
("--split-group","geometry"),# group by actual mesh (no leak); prefix mode
# is naming-scheme-dependent and known broken
# for wscad+PXC (dumps them onto one side)
("--test-frac","0.15"),
("--eval-every","3"),
("--patience","12"),
