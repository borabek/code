...
13 feature'in KOR NOKTALARI: (a) parcada NEREDE oldugu absent -- vida USTTE, tel ONDE (kullanicinin
'vidadan ok cikiyor' bulgusu full this), (b) part-ici GORECELIK absent (benim capim digerlerine according to large mu),
(c) single scale.
Bu testte, only havuzdaki P (pozisyon) + X (13 feat) with turetilebilen zengin feature'lari deneriz:
  A) normalize konum (part bbox inside x,y,z)      B) bbox yuzeylerine uzaklik (6)
  C) each feature'in part-ici RANK'i (13)           D) part medyanina ORAN (13)
DECISION: AUC/top-N F1 materyal as artiyorsa -> temsil (feature) darbogazi GERCEK, GPU'lu zengin
inference (very-radius probability histogrami + taper profili) HAKLI. Artmiyorsa temsil tavani teyit."""
import json ,sys 
import numpy as np 
...
for gi in np .unique (G ):
    ...
    xx =X [m ]
    C [m ]=np .argsort (np .argsort (xx ,axis =0 ),axis =0 )/max (len (m )-1 ,1 )# part-ici rank
    med =np .median (xx ,axis =0 );Dr [m ]=xx /(np .abs (med )+1e-6 )# medyana ratio
...
print (f"{len (Y )} candidate / {int (Y .sum ())} TP / {len (Ns )} part | POOL {POOL }")
...
("+ part-ici rank(C)",np .hstack ([X ,C ])),
("+ medyan-ratio(D)",np .hstack ([X ,Dr ])),
