...
GO OLCUTU (kullanicinin yazdigi):
  genel oracle >= +0.01, very-CP >= +0.015, low-CP loss <= 0.005
...
MAKBUZ ="results/p1_birlesme_sweep.json"
...
def main ():
    ...
    for (cl ,de ,oy )in AYAR :
        ...
        if baseline is None :
            baseline =(v ,dl ,ck )
        diff =f"{v -baseline [0 ]:+.4f}"if baseline else ""
        print (f"{cl :.0f}/{de :.0f}/{oy :.0f}".ljust (14 )+
        f"{v :>8.4f}{dl :>8.4f}{ck :>8.4f}{n_aday :>8}{sure :>8.0f}{diff :>9}")
        res_ [f"{cl }/{de }/{oy }"]={"oracle":v ,"low":dl ,"very":ck ,
        "candidate":n_aday ,"sure_s":sure }

    en =max ((k for k in res_ ),key =lambda k :res_ [k ]["oracle"])
    t =res_ [f"3.0/10.0/5.0"]
    e =res_ [en ]
    print (f"\nMEVCUT 3/10/5 : oracle {t ['oracle']:.4f} (dusuk {t ['low']:.4f} "
    f"very {t ['very']:.4f}) candidate {t ['candidate']}")
    print (f"EN IYI  {en :<10}: oracle {e ['oracle']:.4f} (dusuk {e ['low']:.4f} "
    f"very {e ['very']:.4f}) candidate {e ['candidate']}")
    go =(e ["oracle"]-t ["oracle"]>=0.01 and e ["very"]-t ["very"]>=0.015 
    and t ["low"]-e ["low"]<=0.005 )
    print (f"\nGO OLCUTU (genel +0.01 / very-CP +0.015 / dusuk-CP loss <=0.005): "
    f"{'GECTI'if go else 'KALDI'}")
    print (f"  genel {e ['oracle']-t ['oracle']:+.4f} | very-CP {e ['very']-t ['very']:+.4f} | "
