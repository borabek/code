# -*- coding: utf-8 -*-
"""W1: YENI WEI VERISI -- korpusta never olmayan parcalari isle (denetimin P2 maddesi).

DOGRULANDI: 842 WEI parcasinin 264'u zengin korpusta YOK. LOCKED, mevcut measurement kumesi and
GEOMETRI CAKISMALARI cikarilinca **198 part / 154 grup** gercekten new.

Bu, hafizadaki "WSCAD data kaldiraci OLDU" kaydindan FARKLI: that kayit new part INDIRMEKLE
ilgiliydi (325 deneme -> 7 file). Bunlar ZATEN ELIMIZDEKI but korpusa never konmamis parts.

TEZ CIZGISI: same boru hatti, same network, same remesh, same candidate ureticisi. Degisen TEK sey
training korpusunun buyuklugu -- i.e. most temiz "only data" deneyi.

BOLME (denetimin P2.2 tavsiyesi):
    154 grubun YARISI  -> PROSPEKTIF WEI TESTI (never dokunulmaz, egitime girmez)
    diger yarisi       -> training korpusuna eklenir
Boylece data etkisi HEM havuzlanmis olcumde HEM de unseen-part testinde olculur.

Cikti: results/yeni_wei.npz (X22+XR ozellikleri, labels, split etiketi)
"""
import io 
import json 
import os 
...
def main ():
    ...
    sat ,rap =measure_set .cluster ("results/_der_tam.pkl")
    ...
    present ={str (x )for x in zen ["pids"]}
    E =list (eligible ())
    new_ =[(m ,p ,jf ,s )for m ,p ,jf ,s in E 
    if m =="WEI"and p not in present and p not in lock 
    ...
    if os .path .exists (ATLA ):
        with io .open (ATLA ,encoding ="utf-8")as f :
            skip ={x .strip ()for x in f if x .strip ()}
    if skip :
        print (f"  persistent atlama listesi: {sorted (skip )}",flush =True )
    ...
    if os .path .exists (OUT ):
        try :
            ...
            BOL .extend ([str (x )for x in _o ["split"]])
            ...
            for _p ,_n in zip ([str (x )for x in _o ["pids"]],np .asarray (_o ["ngt"])):
                NGT [_p ]=int (_n )
            skip |=_bit 
            print (f"  DEVAM: {len (_bit )} part onceki kosudan geldi",flush =True )
    ...
    for k ,(mfg ,pid ,jf ,stp )in enumerate (new_ ,1 ):
        if pid in skip :
            skipped +=1 ;continue 
        if k %25 ==0 :
            print (f"  {k }/{len (new_ )}  {time .time ()-t0 :.0f}s (skipped {skipped })",flush =True )
            _ara_kayit ()
            # SU AN ISLENEN parcayi diske yaz: run takilip oldurulurse a sonraki calistirmada
            # this part persistent atlama listesine girer and is a more same places durmaz.
    ...
    if os .path .exists (ISLENEN ):
        os .remove (ISLENEN )
    B =np .array (BOL )
    print (f"\n-> {OUT } | {len (B )} candidate / {len (set (PID ))} part (skipped {skipped })")
    print (f"   training {int ((B =='training').sum ())} candidate | PROSPEKTIF TEST {int ((B =='test').sum ())} candidate")
