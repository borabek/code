...
COZUNURLUGU uye sayisiyla sinirli. Turetilmis 5. uye (probability ortalamasi) MEASURED and KAYBETTI
because bagimsiz degildi; this run gercekten ayri tohumlu a checkpoint kullanir.

KILL (onceden yazildi): VAL kumesinde detection F1 +0.01 gelmezse uye ALINMAZ.

Not: gate `votes` ozelligini kullaniyor and training verisi 4 uyeyle uretildi. 5 uyede votes'un
olcegi degisir, that yuzden gate 5-uyeli adaylarla YENIDEN egitilmeden comparison adil olmaz.
...
def main ():
    ...
    print (f"\n{'ensemble':<20}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    R ={}
    for n in (4 ,5 ):
        det ,rob =kos (n )
        R [n ]=(det ,rob )
    ...
    print (f"\nKILL (onceden yazili): detection +0.01 gelmezse uye ALINMAZ -> "
    f"diff {d_ :+.4f} => {'ALINIR'if d_ >=0.01 else 'ALINMAZ'}")
