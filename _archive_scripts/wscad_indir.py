...
AKIS (2026-08-04'te single single kesfedildi, korukorune yazilmadi):
  ...
  4. menude UC option: "Download WSCAD" / "Download DWG" / **"Download STEP"**
...
CHECK parcalari indirir and onlar failed olursa "bulunamadi" sonuclarini INVALID
...
def hedefleri_sec (n ,manufacturer ,hepsi ):
    man =json .load (io .open ("results/manifest_corpus.json",encoding ="utf-8"))
    present =_var_olan_step ()
    candidate =[m for m in man if m ["cp"]>0 and m ["part"]and m ["part"]not in present ]
    if manufacturer :
    ...
    gruplar ={}
    for m in candidate :
        gruplar .setdefault ((m ["manufacturer"],m ["bucket"]),[]).append (m )
...
def _step_kaydet (path ,part ):
    ...
    if zipfile .is_zipfile (path ):
        with zipfile .ZipFile (path )as z :
            ...
            if not adlar :
                return None ,f"ZIP icinde .stp none ({z .namelist ()[:3 ]})"
...
def indir (pg ,ctx ,part ,bekle =25000 ):
    ...
    path ,error =_step_kaydet (gec ,part )
    try :
        os .remove (gec )
    except Exception :
        pass 
    return ("ok",os .path .basename (path ))if path else ("error",error )
