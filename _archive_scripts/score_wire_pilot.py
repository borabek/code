...
for k ,v in ans .items ():
    ...
    y =truth [pid ][i ]# 1 = manufacturer CP with matched candidate
...
print (f"  insan 'TEL' dedi but manufacturer CP DEMIYOR (FP): {fp }")
print (f"  insan 'ALET' dedi but manufacturer CP DIYOR  (FN): {fn }")
print (f"  ikisi de 'not'                        (TN): {tn }")
print (f"  insan-precision {prec :.3f} | insan-recall {rec :.3f} | insan-F1 {f1 :.3f}")
print (f"\n  KIYAS: mevcut MODEL same gorevde family-out F1 ~0.752")
print (f"         insan-F1 modelden YUKSEKse -> ogrenilecek sinyal VAR (ceiling insan seviyesi)")
print (f"         insan-F1 modele YAKINSA    -> model already insan seviyesinde")
print ("\n=== DECISION ===")
if acc >=0.90 :
