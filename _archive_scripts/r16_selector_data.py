...
R15 olctu: three candidate between correct axis NEREDEYSE HEP VAR (oracle %84 angle<=10, dik %26 -> %2)
but ELLE yazdigim secim kurali MEVCUT'defn kotu (%62 vs %68). Yani candidates iyi, RULE kotu.
Cozum: kurali OGREN -- `c4_direction_selector`nin sign for yaptigini axis for yap.
...
def main ():
    sv =set (json .load (io .open ('results/d5_4_exam_set.json',encoding ='utf-8'))['pidler'])
    ...
    np .savez ("results/r16_selector_data.npz",X =X ,y =Y ,aci =AC )
    print (f"\n{len (X )} satir | correct candidate orani %{100 *Y .mean ():.0f} -> results/r16_selector_data.npz")
