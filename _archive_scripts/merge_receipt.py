...
Kullanim:  python merge_receipt.py <output.json> <pay1.json> <pay2.json> ...
...
def main ():
    ...
    for y in paylar :
        ...
        {}).items ():
            ...
            kir [p ]=v 
    if cakisan :
        print (f"UYARI: {len (cakisan )} part birden excess payda puanlanmis "
    ...
    out ={"robot":f1 (*T ),"detection":f1 (*tes ),
    ...
    open (cik ,"w"),indent =1 )
    print (f"{len (kir )} part | robot {out ['robot']:.4f} | detection "
    f"{out ['detection']:.4f} | makro {out ['makro']:.4f} | "
