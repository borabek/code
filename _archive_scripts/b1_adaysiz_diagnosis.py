...
is done -- i.e. "yeterli probability" esigi veriden gelir, elle konmaz.
...
def main ():
    ...
    def feat (A ,ad ):
        ...
        print (f"{ad :<12}CE+CT probability max: medyan {np .median (cm ):.3f}  "
        f"|  CE/CT tepe sayisi: medyan {np .median (ct ):.0f}  "
        f"|  no CE/CT tepesi YOK: {float ((ct ==0 ).mean ()):.1%}")
        return cm ,ct 
    print ()
    cmU ,ctU =feat (ULASAN ,"ULASAN")
    cmA ,ctA =feat (ADAYSIZ ,"ADAYSIZ")
    ...
    print (f"\nESIK (ulasanlarin 10. yuzdeligi): CE+CT probability {threshold :.3f}")
    print (f"  ADAYSIZ CP'lerin {b_pay :.1%}'inde segmentasyon ZATEN this esigin ustunde")
    print ()
    ...
    with io .open ("results/b1_adaysiz_diagnosis.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/b1_adaysiz_diagnosis.json")
