...
`results/u7_yonlendirme.json` kazanan=null, `results/u8_yonlendirme_verify.json` hakimiyet=false.
...
UC KOL (dagitim merdiveninin tamami):
  ...
  D  each parcaya part-ici z-score    -> results/wire_gate.pkl.pre_yonlendirme
...
def main ():
    ...
    SON ,PARCA ={},{}
    print (f"\n{'arm':<6}"+"".join (f"{b :>12}"for b ,_ ,_ in BOLME )+f"{'robot(familiar)':>16}")
    for arm in ("A","D","R"):
        SON [arm ]={}
        sat =f"{arm :<6}"
        for b ,keep ,alt in BOLME :
            ...
            sat +=f"{f1w (det ):>12.4f}"
        sat +=f"{f1w (PARCA [(arm ,'familiar')][1 ]):>16.4f}"
