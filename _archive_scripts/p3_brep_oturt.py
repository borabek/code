...
MERDIVEN (`results/d6_ceiling_ladder.json`): poz kisitlarinin total bedeli
...
ONCEKI DENEMELERIN DERSI -- korumali oturtma SART:
  ...
  * `r13_yakinlik_axis`    : YAKINLIK-ONCELIKLI secim gorulmemiste NET -30
...
Bu yuzden here oturtma UC kapiyla korunur and all of them TARANIR:
  ...
  YARICAP: wire_gate'in R_MIN/R_MAX araligi (tel entering hole boyutu)

SECIM YANLILIGI ENGELI: 8 manufacturer ikiye bolunur; setting DEV yarisinda secilir,
...
def silindir_onbellek (rec_ ,yenile =False ):
    ...
    if os .path .exists (ONBELLEK )and not yenile :
        with open (ONBELLEK ,"rb")as f :
            ob =pickle .load (f )
    missing =[p for p in rec_ if p not in ob and p in S ]
    if missing :
...
def main ():
    ...
    print (f"\nDEV oturtmasiz: detection {d0t :.4f} robot {d0r :.4f}")
    ...
    for ac in (10 ,20 ,30 ,45 ):
        ...
        for mm in (2 ,4 ,6 ,10 ):
            ...
            izgara [f"{ac }/{mm }"]={"detection":tf ,"robot":rf }
            line_ .append (rf )
            if rf >en_iyi_r :
    ...
    if en_iyi is None :
        print ("\nDEV'de HICBIR setting oturtmasizi gecmedi -> KOL CLOSED")
        with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
            json .dump ({"karar":"CLOSED","dev_taban_robot":d0r ,"izgara":izgara },
            f ,indent =1 )
        return 
    ac ,mm =en_iyi 
    print (f"\nDEV'de selected: aci<={ac } derece, mesafe<={mm }mm -> robot {en_iyi_r :.4f} "
    ...
    with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
        ...
        "dev_taban_robot":d0r ,"sinav_oturtmasiz":{"detection":s0t ,"robot":s0r },
        "sinav_oturtmali":{"detection":s1t ,"robot":s1r },
