...
def main ():
    ...
    print (f"  10 derece icinde remaining CP orani: {(s <=10 ).mean ():.1%}")
    ...
    for r in DER :
        ...
        diff .append (float (np .degrees (np .arccos (np .clip (abs (float (bg @bp )),0 ,1 )))))
    f =np .array (diff )
    ...
    print ("\n5) UZLASI KIMI DUZELTIR, KIMI BOZAR? (tespitte matched points on)")
    ...
    with open ("results/r2_axis_varsayimi.json","w",encoding ="utf-8")as f_ :
    ...
    print ("\nmakbuz -> results/r2_axis_varsayimi.json")
