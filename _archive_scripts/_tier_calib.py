...
Mevcut kusur: tier, SEGMENTASYON guvenine bakiyor (conf>=0.5) -> each sey AUTO oluyor (holdout'ta REVIEW BOS).
Dogrusu: AUTO/REVIEW karari 'this gercekten tel girisi mi' sorusudur -> GATE skoru (wire_score).
Kalibrasyon: AUTO precision hedefini saglayan gate esigi. Robot fiziksel eylem yaptigi for
AUTO'nun precision'i high must be; gerisi insana (REVIEW) gitmeli."""
...
kept =WORK &(o >=0.34 )# urunun tuttugu candidates
gt_all =int (sum (ngt [int (g )]for g in np .unique (G [WORK ])))
print (f'urun ciktisi: {int (kept .sum ())} candidate | gercek CP {gt_all }')
print ()
print ('=== MEVCUT TIER (segmentasyon guveni >= 0.5) ===')
auto =kept &(CONF >=0.5 );rev =kept &(CONF <0.5 )
pa =int (Y [auto ].sum ())/max (int (auto .sum ()),1 )
print (f'  AUTO  : {int (auto .sum ()):4d} candidate | precision {pa :.4f}')
print (f'  REVIEW: {int (rev .sum ()):4d} candidate   <-- kusur: neredeyse bos')
print ()
...
if best :
    t ,na ,pa ,ra ,nr =best 
    print (f'>>> AUTO precision >=0.95 for threshold {t :.2f}')
    print (f'    AUTO {na } candidate (precision {pa :.3f}, gercek CPlerin %{100 *ra :.0f}i) | REVIEW {nr } candidate')
    print (f'    ie robot CPlerin %{100 *ra :.0f}ini tek basina yapar, gerisi insana gider')
