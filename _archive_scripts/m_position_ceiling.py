...
Esleme gevsek olcutle (detection) is done; so "already bulunmus" adaylarin kalitesi olculur.
...
def main ():
    ...
    json .dump ({k :float (v )for k ,v in res .items ()}|{"detection":float (det )},
    open ("results/m_position_ceiling.json","w"),indent =1 )
    print ("receipt -> results/m_position_ceiling.json")
