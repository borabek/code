...
x1 teshisi: detection edilen 873 ciftin %80.3'u two olcutu de geciyor.
    only YANAL dusen  %6.4
    only ACI  dusen   %8.0
    ikisi de dusen      %5.3
Uye havuzunun ACI kahini only +%2.3 birakiyor -> uye seciciyi already bosalttik.

BU BETIK: remaining angle hatalarinin NE OLDUGUNU soyler.
...
def main ():
    ...
    GRUP =np .array (GRUP )
    print (f"\n{len (ACI )} detection edilmis cift")

    print (f"\nACI HATASI DAGILIMI")
    bucket =[(0 ,10 ),(10 ,30 ),(30 ,60 ),(60 ,80 ),(80 ,100 ),(100 ,181 )]
    for lo ,hi in bucket :
        m =(ACI >=lo )&(ACI <hi )
        print (f"  {lo :>3}-{hi :<3} derece : {m .sum ():>4} ({m .mean ():>5.1%})")
    dik =(ACI >=60 )
    print (f"\n  ACI HATASI which {int ((ACI >10 ).sum ())} ciftin {int (dik .sum ())}'i >=60 derece "
    f"(= {dik .sum ()/max ((ACI >10 ).sum (),1 ):.0%}) -> 'YANLIS EKSEN' sinifi")
    print (f"  kucuk deviation (10-30 derece): {int (((ACI >10 )&(ACI <30 )).sum ())}")

    print (f"\nTAVANLAR (detection edilen ciftler uzerinden)")
    y_ok =YANAL <=2.0 ;a_ok =ACI <=10.0 
    print (f"  su an ikisi de passing        {(y_ok &a_ok ).mean ():6.1%}")
    print (f"  ACI mukemmel olsa           {y_ok .mean ():6.1%}")
    print (f"  YANAL mukemmel olsa         {a_ok .mean ():6.1%}")
    print (f"  IKISI de mukemmel (=detection) {1.0 :6.1%}")
    hh =T .f1w 
    print (f"\n  su anki robot F1 0.5893 | detection F1 0.7584")
    ...
    with io .open ("results/r1_angle_artigi.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/r1_angle_artigi.json")
