# -*- coding: utf-8 -*-
"""S9: COK-CP candidate kurtarma -- ikinci cozunurlukte (9k) yeniden derive.

WHY: candidate recall'i rejime according to COK different (results/candidate_recall.json):
    low-CP  0.8924  -> F1 tavani 0.9432
    very-CP    0.7691  -> F1 tavani 0.8695   <- BURASI CIVILI
Yani very-CP kolunda detection F1'i 0.87'yi no gate with GECEMEZ; missing which is ADAY.
...
def main ():
    ...
    print (f"\n{'arm':<16}{'very-CP detection':>15}{'very-CP robot':>14}")
    ...
    print (f"\ncok-CP detection farki {d :+.4f} | GA [{lo :+.4f}, {hi :+.4f}]")
    print (f"KABUL BARI (audit): very-CP +0.03 -> {'GECTI'if d >=0.03 else 'GECMEDI'}")
