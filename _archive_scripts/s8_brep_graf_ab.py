# -*- coding: utf-8 -*-
"""S8: B-REP GRAF ozellikleri A/B -- detection sıralamasi duvarina karsi.
...
DUSURULDU -- yazilmamislardi, sifir tasiyorlardi):
    ...
    g_kor / g_gecis                  channel kor mu, boydan paint mi (boydan paint = tel girisi DEGIL)

Kapsama %96.6.

TASARIM: same candidates, single variable feature kumesi. Once candidate duzeyi (ucuz eleme), gecerse
UCTAN UCA (this arastirmada candidate duzeyi ALTI KEZ yaniltti).

KILL: uctan uca detection >= +0.01 VE decision_criterion five sarti.
"""
import collections 
import io 
import json 
...
def main ():
    ...
    print (f"  58 sutun {a58 :.4f} | 58+graf {a68 :.4f} | diff {a68 -a58 :+.4f}")
    ...
    missing =sum (1 for r in DER if der_graf (r )is None )
    print (f"\nolcum tarafinda graf sutunu EKSIK which part: {missing }/{len (DER )}")
