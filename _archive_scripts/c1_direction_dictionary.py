# -*- coding: utf-8 -*-
"""C1: YON SOZLUGU + KAHIN SINAVI -- Rota C'nin first kapisi.

WHY ONCE BU: mevcut tespitle (0.7584) poz and angle MUKEMMEL olsa robot da 0.7584 olurdu;
this an 0.5893. Yani tespite never dokunmadan **0.169** pay duruyor -- 0.80 detection hedefine
giden paydan (0.042) four fold extra. Robot metriginin kaderi here.

WHY AYRIK, WHY REGRESYON DEGIL: R2'de "wrong axis" sinifini AUC 0.930 with TANIDIK
but surekli regresorle yonu yeniden kurunca robot HER ESIKTE dustu (-0.0256). Sinifi
tanimak duzeltebilmek degildir. Ayrik secim, regresyonun yapamadigini yapabilir: correct
direction already FIZIKSEL as turetilebilen birkac adaydan BIRIYSE, is onu SECMEYE indirger.

SOZLUK KUCUK VE FIZIKSEL TUTULUR. Duzlemde 24 yonluk a yelpaze koysaydim oracle
bedavaya kusursuz cikardi and measurement anlamini yitirirdi. Her giris a GEOMETRIK NESNEDEN
gelir:
    mevcut     dagitilan zincirin ciktisi (poz+angle+uye)
    ham        duzeltmesiz candidate yonu
    uye_k      birlesimde ATILAN uye yonleri (pick_member_direction'in havuzu)
    brep       B-rep analitik silindir ekseni
    channel      yerel channel ekseni (ic noktalarin PCA'si)
    normal     yerel surface normali
    duzlem_A/B mouth duzlemindeki ANA and YAN axis (yarik yonu and dikeyi)
Her biri and ISARET ESLERI (+-) -> tipik 10-18 giris.

KILL (onceden yazildi): oracle robot < 0.70 whereas Rota C KAPANIR, selector EGITILMEZ.
Gecerse ayrik selector egitilir (regresyon DEGIL).
"""
import io 
import json 
import os 
...
def yerel_yonler (V ,p ,d ):
    """Mesh'ten turetilen yonler: channel ekseni, surface normali, mouth duzlemi eksenleri."""
    out ={}
    rel =V -p 
    t =rel @d 
    ...
    if yak .sum ()>=8 :
        ...
        try :
            _ ,_ ,W =np .linalg .svd (Q ,full_matrices =False )
            # mouth duzlemindeki ANA and YAN axis (yarik yonu and dikeyi)
...
def main ():
    ...
    for kk ,r in enumerate (D ["DER"],1 ):
        ...
        rob_isar .append ((rj ,)+esle (P ,Pk ,G ,Gd ,r ["diag"],2.0 ,10.0 ,False ,signed =True ))
        gg .append (r ["geo"])
    ...
    print (f"\n{'olcu':<26}{'value':>9}")
    print (f"{'detection (does not change)':<26}{t_ :>9.4f}")
    print (f"{'robot MEVCUT':<26}{rm :>9.4f}")
    print (f"{'robot KAHIN (dictionary)':<26}{rk :>9.4f}   diff {rk -rm :+.4f}")
    print (f"{'robot KAHIN signed':<26}{ri :>9.4f}")
    print (f"{'upper boundary (=detection)':<26}{t_ :>9.4f}")
    print (f"\nsozluk, direction boslugunun %{100 *(rk -rm )/max (t_ -rm ,1e-9 ):.0f}'ini kapsiyor")
    gecti =rk >=0.70 
    print (f"\nKILL: oracle robot >= 0.70 -> {'GECTI, ayrik selector egitilir'if gecti else 'GECMEDI, Rota C KAPANIR'}")
    with io .open ("results/c1_direction_dictionary.json","w",encoding ="utf-8")as f :
        json .dump ({"detection":t_ ,"robot_mevcut":rm ,"robot_kahin":rk ,
        "robot_kahin_isaretli":ri ,"sozluk_medyan":float (np .median (boy )),
        "kapsam":say ,"gecti":bool (gecti )},f ,indent =1 )
    print ("receipt -> results/c1_direction_dictionary.json")
