...
Yani A/B'de single variable feature kumesi. (Ayri kosularda uretilmis two npz'yi karsilastirmak
...
KOLLAR:
  ...
  C  22 + very-radius-24
  D  22 + 33 (all of them)
TAPER YOK -- olculmus olu, audit de "ekleme" dedi.

Dagitilan yapinin DONUSUMU (part-ici z-score) each kolda uygulanir -- urunle same.
...
def main ():
    ...
    diff =SON [en ]["genel"]-a ["genel"]
    print (f"\nEN IYI: {en } -> {diff :+.4f}")
    print (f"ESKI KANIT (13 sutun zamani): part-out +0.0351 / family-out +0.0489")
    print (f"KARAR: {'UCTAN UCA DOGRULA'if diff >=0.01 else 'KAZANC KUCUK -- uctan uca degmez'}")
    with io .open ("results/p2_rich_ab.json","w",encoding ="utf-8")as f :
        json .dump ({"kollar":SON ,"en_iyi":en ,"difference":float (diff )},f ,indent =1 )
    print ("receipt -> results/p2_rich_ab.json")
