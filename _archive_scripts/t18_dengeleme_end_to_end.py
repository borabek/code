# -*- coding: utf-8 -*-
"""T18 (t15 turevi): SINIF DENGELEME uctan uca -- hem TANIDIK hem GORULMEMIS URETICI.

Aday duzeyinde (t17, 22 column, dagitilan goreli threshold):
    weight              TANIDIK   PXC-disi  WEI-disi  EN KOTU
    absent (mevcut)          0.7468    0.7164    0.4597    0.4597
    balanced              0.7391    0.7269    0.4862   +0.0265
    balanced_subsample    0.7479    0.7266    0.4805   +0.0208
Mekanizma: PXC %40.8 pozitif, WEI %24.1. PXC'de egitilen model WEI'ye YANLIS ONSELLE gelir;
dengeleme onseli notrlestirir.

AMA candidate duzeyi uctan uca DEGILDIR -- ExtraTrees full this adimda became (candidate duzeyinde most iyiydi,
uctan uca birinde +0.053 otekinde -0.039). Karar here.

KILL: dengeleme, familiar tespiti dusurmeden (>= -0.01) HER IKI manufacturer-disi bolmede de
mevcudu gecmezse ALINMAZ.

Aday duzeyinde measured:
  t13: kon_cevre AUC 0.709 (TP medyan 1.000 = full kind icbukey halka, FP 0.833), kon_sayi 0.703
  t14: 18 -> 18+4  familiar +0.0167 | unseen manufacturer EN KOTU +0.0176 (ikisi de POZITIF)

Bu night two times candidate duzeyi yanildi (EK blogu; goreli esigin bedeli), that yuzden karar here.

TASARIM: candidates BIR KEZ turetilir, 22 column a times is computed; 18-sutunlu arm same matrisin
first 18 sutununu kullanir. Karar kurali each two kolda da DAGITILAN goreli threshold. Tek variable:
gate'in egitildigi feature kumesi.

KILL (onceden yazili): TOPO, (a) familiar DEV+VAL detection F1'i DUSURMEMELI (<=0.01 loss tolere)
VE (b) unseen manufacturer EN KOTU durumunu ARTIRMALI. Ikisi birden saglanmazsa DAGITILMAZ.
"""
...
def main ():
    ...
    def kur (keep ,nc ,cw =None ):
        return RandomForestClassifier (n_estimators =400 ,min_samples_leaf =3 ,n_jobs =-1 ,
        random_state =0 ,class_weight =cw ).fit (
        d ["X"][keep ][:,:nc ],d ["y"][keep ])
    ...
    print (f"\n{'split':<22}{'column':>6}{'detection':>9}{'ROBOT':>9}{'conclusive':>9}{'recall':>9}")
    R ={}
    for cw ,ad in ((None ,"weight absent"),("balanced_subsample","balanced_subsample"),
    ("balanced","balanced")):
        nc =22 
        ...
        flush =True )
        R [(ad ,"familiar")]=(det ,rob )
        for k ,mad in kod .items ():
            alt =[x for x in DER if x ["mfg"]==mad ]
            if len (alt )<10 :
                continue 
            keep2 =(tr_mfg !=k )&~np .isin (tr_grp ,list (tg ))
            det2 ,rob2 =puanla (kur (keep2 ,nc ,cw ),alt ,nc )
            p2 ,r2 =pr (det2 )
            print (f"{'  '+mad +' disarida ('+str (len (alt ))+')':<22}{ad [:6 ]:>6}"
    ...
    ok_tan =f1w (R [(22 ,"familiar")][0 ])-f1w (R [(18 ,"familiar")][0 ])
    mout =[(a ,f1w (R [(22 ,a )][0 ])-f1w (R [(18 ,a )][0 ]))for a in key_ if a !="familiar"]
    for a ,v in [("familiar",ok_tan )]+mout :
        print (f"  {a :<16}{v :+.4f}")
    ...
    print (f"\n  unseen manufacturer EN KOTU: {en_kotu18 :.4f} -> {en_kotu22 :.4f} ({en_kotu22 -en_kotu18 :+.4f})")
    print (f"KILL: familiar >= -0.01 VE en kotu artmali -> {'GECTI'if gecti else 'GECMEDI'}")
    pickle .dump (R ,open ("results/t18_parca.pkl","wb"))
    json .dump ({f"{k [0 ]}|{k [1 ]}":{"detection":float (f1w (v [0 ])),"robot":float (f1w (v [1 ]))}
    for k ,v in R .items ()}|{"karar":"GECTI"if gecti else "GECMEDI"},
    open ("results/t18_dengeleme.json","w"),indent =1 )
    print ("receipt -> results/t18_dengeleme.json")
