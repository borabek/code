# -*- coding: utf-8 -*-
"""KORPUS TEMIZLIGI: unmatched dosyalar and kopyalar (SILMEZ -- ARSIVE TASIR).
...
def main ():
    ...
    for p ,fs in jk .items ():
        ...
        for f in fs :
            ...
            n1 +=1 
    print (f"\n1) STEP'i olmayan JSON -> {ARSIV_JSON }: {n1 } file")
    print ("   manufacturer:",dict (tasinan_uretici .most_common (8 )))

    # --- 3) KOPYA STEP -> archive (most eskisi kalir)
    ...
    for k ,v in sk .items ():
        ...
        for x in v [1 :]:
            ...
            n3 +=1 
    print (f"3) KOPYA STEP -> {ARSIV_STEP }: {n3 } file (each kimlikten biri KALDI)")
    ...
    eligible ={p for _ ,p ,_ ,_ in _eligible ()}
    for k in oksuz :
        assert k not in korunan ,f"OLCUM PARCASI oksuz sanildi: {k }"
        assert k not in eligible ,f"UYGUN KORPUS parcasi oksuz sanildi: {k }"
    ...
    for k in oksuz :
        for x in sk2 [k ]:
            ...
            n2 +=1 
    print (f"\n2) JSON'u olmayan STEP -> {ARSIV_OKSUZ }: {n2 } file ({len (oksuz )} kimlik)")
    ...
    with io .open ("results/corpus_temizlik.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/corpus_temizlik.json")
