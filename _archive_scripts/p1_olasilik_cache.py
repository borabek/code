...
def main ():
    ...
    ap .add_argument ("--cluster",default ="results/d6_exam_set.json")
    ...
    present ={os .path .splitext (x )[0 ]for x in os .listdir (kok )if x .endswith (".npz")}
    hedef =[p for p in sv ["pidler"]if p in S and p not in present ]
    if a .total_ >1 :
        hedef =[p for i ,p in enumerate (hedef )if i %a .total_ ==a .vardiya ]
    print (f"cluster {a .cluster } muhur {sv ['sha16']} | onbellekte {len (present )} | "
    ...
    for k ,pid in enumerate (hedef ,1 ):
        if k %20 ==0 :
            ...
            f"remaining ~{hiz *(len (hedef )-k )/60 :.0f} dk",flush =True )
