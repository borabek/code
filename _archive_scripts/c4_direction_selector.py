...
TASARIM:
  ...
  * label: this giris GT yonune <=10 derece mi (only matched ciftlerde tanimli)
...
as does not change (detection olcutu aciya bakmaz) but yine de olculur. Uretici-disi dusmemeli.
"""
import io 
import json 
import os 
...
def main ():
    ...
    if os .path .exists (ONBELLEK ):
        with open (ONBELLEK ,"rb")as f :
            PARCA =pickle .load (f )
        print (f"dictionary onbellekten: {len (PARCA )} part",flush =True )
    else :
        PARCA ={}
        t0 =time .time ()
    ...
    print (f"\negitim satiri {len (RY )} | correct giris orani {RY .mean ():.1%} | "
    f"sutun {RX .shape [1 ]}")
    ...
    print (f"\nTABAN (deviation none) {baseline :.4f} | EN IYI marj {marj :.2f} -> {en :.4f} "
    f"({en -baseline :+.4f})")
    print (f"  GA[{lo :+.4f},{hi :+.4f}] | detection {T .f1w (det0 ):.4f} -> {T .f1w (det ):.4f}")
    gecti =(en -baseline )>=0.01 and lo >0 
    print (f"\nKILL: robot +0.01 VE GA>0 -> {'GECTI'if gecti else 'GECMEDI'}")
    with io .open ("results/c4_direction_selector.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/c4_direction_selector.json")
