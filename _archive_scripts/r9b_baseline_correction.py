...
ESKI kolun da dagitilan surumden ZAYIF cikmasi BEKLENIR; comparison ESKI-vs-YENI
...
from r9_signed_ozellik import koni 
...
def main ():
    ...
    print (f"\nTABAN (selector ONCESI): robot(FIZ) {b_rob :.4f} | detection {b_det :.4f}")
    print (f"dagitilan selector this tabandan 0.6181'e cikardi (+{0.6181 -b_rob :.4f}); "
    f"ISARETLI ceiling 0.6580")
    fn =lambda rows :f1w ([q for _ ,q in rows ])-f1w ([p for p ,_ in rows ])
    print (f"\n{'feature':<8}{'marj':>6}{'robot':>10}{'d':>9}{'GA':>22}{'detection':>10}")
    SON ={}
    for ad in ("ESKI","YENI"):
    ...
    print (f"\nHUKUM: {'R10 HAK EDILDI -- training korpusunda ISARETLI oznitelikle yeniden derive and dagit'if val_ else 'signed feature farki < 0.01 -- R10 kosulmaz'}")
    with io .open ("results/r9b_baseline_correction.json","w",encoding ="utf-8")as f :
    ...
    print ("receipt -> results/r9b_baseline_correction.json")
