...
SORULAR (all of them olculur, none of them varsayilmaz):
  ...
  4. 90-derecelikler dar/deep kanallarda mi, sig agizlarda mi? (candidate ozellikleri with bak)
...
def main ():
    ...
    print (f"  {len (etkilenen )}/{len (pp )} parcada en az a dik axis present")
    tam =[p for p ,v in etkilenen .items ()if v [1 ]==v [0 ]]
    kismi =[p for p ,v in etkilenen .items ()if 0 <v [1 ]<v [0 ]]
    print (f"  TAMAMI dik which part : {len (tam )}  (part genelinde axis sorunu)")
    print (f"  KISMEN dik which part : {len (kismi )}  (part ici karisik -> cerceve sorunu DEGIL)")
    ...
    with open ("results/r1_angle_diagnosis.json","w",encoding ="utf-8")as f :
    ...
    print ("\nmakbuz -> results/r1_angle_diagnosis.json")
