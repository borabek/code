...
IMZALAR (all of them olculebilir, ogrenilmis parametre absent):
  passing      : deligin IKI ucu da serbest alana aciliyor (montaj/gecme deligi)
...
OUT ="results/geo070_g1_autopsy.json"
...
def main (n_parts =60 ):
    ...
    print (f"dev kumesi {len (dev )} part (eval yarisi this fazda DOKUNULMADI)\n",flush =True )

    sig_count ={k :0 for k in ("passing","eksen_dik","cok_kucuk","cok_buyuk","sig","only","diger")}
    n_fp =n_tp =0 
    ...
    for k ,(m ,pid ,jf ,stp )in enumerate (dev ,1 ):
        try :
            ...
            for a_ ,c in enumerate (cps ):
                ...
                # passing mi: ters yonde de serbest alana ciksiyor mu
                ...
                if passing :sig_count ["passing"]+=1 
