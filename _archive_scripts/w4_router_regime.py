...
Cikti: results/w4_router_regime.json (part basina karar + karsiliklilik)
"""
import io 
import json 
import os 
...
OUT ="results/w4_router_regime.json"
...
def main ():
    ...
    remaining =[r for r in DER if r ["pid"]not in K ]
    if remaining :
        cks =cfg ["current_product"].get ("checkpoints")or cfg ["robot_vote2_checkpoints"]
        dev ="cuda"if torch .cuda .is_available ()else "cpu"
        models =[load_any (c ,dev =dev )[:2 ]for c in cks ]
        t0 =time .time ()
        for i_ ,r in enumerate (remaining ,1 ):
            if i_ %20 ==0 :
                print (f"  {i_ }/{len (remaining )}  {time .time ()-t0 :.0f}s",flush =True )
