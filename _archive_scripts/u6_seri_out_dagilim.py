# -*- coding: utf-8 -*-
"""U6: part-ici z-score kararini YEDI bolmede sina (single bolmeye mi dayaniyordu?).

TEREDDUT: dagitim karari, unseen-manufacturer ekseninde TEK a sayiya dayaniyor (WEI disarida
...
YEDI bagimsiz split gives. Sorulan sey sudur:
    ...
    z-score GENEL a saglamlik kazanci mi, otherwise only WEI bolmesinde mi calisiyordu?
...
def main ():
    ...
    for k in seriler :
        ...
        diff =v ["D zskor"]-v ["A ham"]
        farklar .append (diff )
        mf =collections .Counter (mfg_of .get (r ["pid"],"?")for r in alt ).most_common (1 )[0 ][0 ]
        print (f"{k :<8}{mf :>9}{len (alt ):>5}{v ['A ham']:>9.4f}{v ['D zskor']:>10.4f}{diff :>+9.4f}",
        flush =True )
    ...
    print (f"\n{kazanan }/{len (f )} seride D kazandi | average {f .mean ():+.4f} | "
    ...
    "sinamak for is used.")
    with open ("results/u6_seri_out.json","w",encoding ="utf-8")as fh :
    ...
    print ("receipt -> results/u6_seri_out.json")
