...
KADEMELER (all of them AYNI kayitlar, AYNI kabul kutusu):
  ...
  4 +IKISI (=TESPIT)  poz mukemmel -> robot == detection
  5 +ATAMA            detection, mukemmel atama with (Macar) -- KALABALIK kovasinin tavani
...
MAKBUZ ="results/p0_ceiling_re.json"
...
def main ():
    ...
    for pid ,r in rec_ .items ():
        ...
        if pak is not None :
            ...
            for i in range (len (P )):
                opt =P3C .options (cy ,P [i ],D [i ],diag ,float (Sk [i ]),komsu ,len (P ))
                if len (opt )==1 :
                    continue 
        ...
        # --- acgozlu with difference (only detection and robot for)
    Ta ,Ra =[],[]
    for pid ,r in rec_ .items ():
        G =np .asarray (r ["G"],float );Gd =np .asarray (r ["Gd"],float )
        rj ="very"if r ["n"]>=8 else "low"
    ...
    f"detection acgozlu {f1w (Ta ):.4f} | robot acgozlu {f1w (Ra ):.4f}")
    with io .open (MAKBUZ ,"w",encoding ="utf-8")as f :
        json .dump ({"cluster":"d6","muhur":sv ["sha16"],"bucket":dict (bucket ),
