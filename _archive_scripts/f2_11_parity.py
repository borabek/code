...
KILL (onceden yazildi): candidate recall farki <= 0.01 VE manufacturer basina detection F1 farki
...
def main ():
    ...
    path =CIKTI .format (a .arm )
    with open (path ,"wb")as f :
        pickle .dump (OUT ,f )
    print (f"\n{len (OUT )} kayit -> {path } | error {error } | bos mesh {empty_ }")
