"""ADAY-TAVANI TESHISI: kacirilan GT CP'lerinde segmentasyon ATESLEDI mi?
  atesledi but candidate olmadi  -> POST-ISLEME sorunu (min_v/dedupe) = UCUZ correction
  never ateslemedi            -> SEGMENTASYON kor = YENIDEN EGITIM gerek
...
from cp_geometry import seat_to_mouth # ORTAK: GT yuvada, mouth baska places
...
print (f'kacirilan CP which {len (miss )} part | ilk 30 inceleniyor',flush =True )
...
for pid in sample :
    ...
    try :
        ...
        for g_ ,gd in zip (Gm ,Gdm ):
            ...
            if hit :continue # this CP already bulunmus
            # DUZELTME: GT yuvada durur; before GERCEK AGZI bul (two yone de bak), after oraya bak
...
print (f'  segmentasyon ATESLEDI but candidate olmadi : {FIRED :3d} (%{100 *FIRED /max (tot ,1 ):.0f})  -> POST-ISLEME sorunu (ucuz)')
print (f'  segmentasyon HIC ateslemedi           : {BLIND :3d} (%{100 *BLIND /max (tot ,1 ):.0f})  -> SEGMENTASYON kor (yeniden training)')
if det :
    print (f'  atesleyen ornekler (part, CE+CT vertex, bolge vertex): {det [:6 ]}')
