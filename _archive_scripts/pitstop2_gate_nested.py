...
def build_cache (n_low =40 ,n_high =24 ,seed =0 ):
    """Her part for V,F,mean-probability,candidate-CP'ler,GT'yi mesh cercevesinde saklar."""
    ...
    print (f"selected {len (sel )} part ({len (lo [:n_low ])} dusuk + {len (hi [:n_high ])} very); "
...
def load_cache (pids ):
    ...
    for pid in pids :
        ...
        for c in cps :
            ...
            if miss :
                raise RuntimeError (f"{pid }: onbellekte missing CP alani {miss } -- gate ozellikleri bozulur")
...
def main ():
    ...
    for reg in ("low","very"):
        ...
        print (f"  oracle (same kumede secim) F1 {orc [5 ]:.4f}   <- SIZINTILI, rapor edilmez")
    ...
    if len (rec ["regimes"])==2 :
        ...
        print (f"KORPUS-AGIRLIKLI (%89.5 dusuk / %10.5 very)")
        ...
        print (f"\nKAPI (dusuk >= +0.02 VE very >= -0.01): {'GECTI'if kill else 'OLU'}")
