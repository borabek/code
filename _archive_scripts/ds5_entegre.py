...
def main ():
    ...
    print (f"source: {len (term )} ElectricalTerminal JSON")
    print (f"mevcut STEP kimligi {len (var_step )} | mevcut JSON {len (var_json )}")

    n_step =n_json =skipped =stp_yok =0 
    ...
    print (f"ATLANAN STEP (kimligi already vardi): {skipped }")
    print (f"STP dosyasi bulunamayan: {stp_yok }")
    with io .open ("results/ds5_entegre.json","w",encoding ="utf-8")as fh :
        json .dump ({"json_eklenen":n_json ,"step_eklenen":n_step ,"skipped":skipped ,
