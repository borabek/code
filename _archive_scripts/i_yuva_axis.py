# -*- coding: utf-8 -*-
"""I: YUVA ekseni -- duzlem ciftlerinden analitik axis, SADECE silindir kolunun sustugu places.

A (silindir) urune input and axis hatasini %24.9 -> %15.8 indirdi. Ama silindir bulamadigi
adaylarda susuyor. Bu arm full that boslugu hedefler: yuva/kelepce girisleri silindir not,
DUZLEM duvarli kanallar; ekseni yine tum wall normallerine diktir.

TASARIM: before silindir denenir (urundeki hali). SADECE None donerse duzlem kolu works.
Boylece A'nin kazandigi yer BOZULMAZ -- only empty remaining yer doldurulur.

KILL: axis >15d hatasi %15.8'in altina inmezse VE robot-hazir F1 0.5272'yi gecmezse duser.
"""
...
def main ():
    ...
    print (f"{len (cache )} part hizalandi\n",flush =True )
    ...
    def apply (dist ,minf ,flat ,turn ):
        ...
        for (m ,p ,jf ,stp ,n ),r in zip (sel ,cache ):
            ...
            # Burada only silindirin BULAMADIGI adaylara duzlem kolu denenir.
    ...
    print (f"{'setting':<34}{'>15d':>7}{'>45d':>7}{'ROBOT':>8}{'very':>7}{'dus':>7}{'detection':>8}{'kullanim':>10}")
