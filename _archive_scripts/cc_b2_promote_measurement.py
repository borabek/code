# -*- coding: utf-8 -*-
"""CC-B2: 'yukseltme' (conn_promote) very-CP ADAY-RECALL'unu yukseltiyor mu?

CC-B bulgusu: kacirilan GT'lerin %76'sinda yakinda CE+CT olasiligi >= 0.30 present but argmax orani 0.00.
Sinyal VARDI, candidate olusumu argmax'defn basladigi for gate kapaliydi.

BU OLCUM: candidate-recall (kac GT for at least a candidate olusuyor) -- gate'ten ONCEKI ceiling.
Recall tavani yukselmezse gate'e bakmanin anlami absent.
KILL KRITERI (F1 not because here gate absent): candidate-recall katkisi < 0.02 whereas OLU.
"""
...
def main (limit =45 ):
    ...
    print (f"{len (parts )} very-CP part | min_v={MV } vc={VC } cluster={CL }\n",flush =True )

    # two post-isleme rejimi: MEVCUT (mv30/vc0.50) and GEVSEK (mv10/vc0.30)
    ...
    print (f"\n{'regime':<22}{'promote':>9}{'candidate-recall':>13}{'candidate/GT':>9}")
    out ={}
    for rk in REGIMES :
        base =None 
        for pv in PROMOTES :
    ...
    print ("receipt -> results/cc_b2_promote.json")
