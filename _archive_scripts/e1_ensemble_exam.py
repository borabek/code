...
(0.6378/0.6629/0.6597). Diskte stopped but DAGITILMAYAN two checkpoint projenin EN YUKSEK
...
KILL: detection +0.01 VE grup bootstrap GA'si sifiri dislamali. Uretici-disi DUSMEMELI.
"""
import io 
import json 
import os 
...
def main ():
    ...
    SON ,ROWS ={},{}
    for ad ,path in KOL .items ():
        if not os .path .exists (path ):
            print (f"{ad }: {path } YOK, atlaniyor");continue 
        DER ,rap =measure_set .cluster (path )
        if ad ==list (KOL )[0 ]:
        ...
        # --- part-ici z-score (urunun donusumu)
        ...
        for r in RJ :
            ...
            gg .append (r ["geo"])
        SON [ad ]={"detection":f1w (det ),"robot":f1w (rob ),"candidate":int (len (RY )),
        "pozitif":float (RY .mean ()),
        "candidate_recall":float (sum (int (RY [RP ==p ].sum ())for p in np .unique (RP ))/
        max (sum (r ["n"]for r in RJ ),1 ))}
        ROWS [ad ]=(det ,rob ,gg )
        s =SON [ad ]
        print (f"\n{ad }: detection {s ['detection']:.4f} | robot {s ['robot']:.4f} | "
        f"candidate {s ['candidate']} | candidate-recall {s ['candidate_recall']:.4f}",flush =True )

    if len (ROWS )==2 :
        a ,b =list (ROWS )
        fn =lambda rows :f1w ([q for _ ,q in rows ])-f1w ([p for p ,_ in rows ])
        for i ,ad in ((0 ,"detection"),(1 ,"robot")):
