...
G0: konkavlik sinyali + validation (manufacturer CP'leri high-konkav bolgede mi?) + first dedektor+eval.
Hizalama sorunu YOK: mesh and CP same JSON frame'inde."""
...
def concavity (mesh ,R =3.0 ):
    """yerel-oyukluk skoru (ray-casting'siz): each vertex for, R-komsularin OUTWARD-normal
    along mean izdusumu. Konkav (oyuk inside) = komsular disarida/above -> high pozitif."""
    ...
    for i ,nb in enumerate (nbrs ):
        ...
        proj =(d @N [i ])/nrm # outward-normal along (unit)
        sc [i ]=float (np .mean (proj ))# >0 = komsular disarida = oyuk icindeyim (konkav)
    return sc 


def validate (parts ):
    """manufacturer CP'lerine yakin vertexlerin konkavligi vs genel -- sinyal CP'leri buluyor mu?"""
...
def detect (mesh ,conc_thr =0.08 ,eps =4.0 ,min_pts =4 ,min_depth =1.5 ):
    """geometrik CP dedektoru: konkav vertexleri kumele -> each opening for mouth-merkezi + axis + depth.
    Filtre: depth>=min_depth (duz kontaklari at). ML YOK."""
    V =mesh .vertices .view (np .ndarray );N =mesh .vertex_normals .view (np .ndarray )
    sc =concavity (mesh )
    ...
    for c in set (lab )-{-1 }:
        ...
        ctr =V [idx ].mean (0 )
        depth =float (((V [idx ]-ctr )@axis ).max ()-((V [idx ]-ctr )@axis ).min ())# axis boyu spread
...
def evaluate (parts ,**kw ):
    ...
    for m ,(t ,f ,n )in sorted (per .items ()):
        ...
        # ============ G3: silindir/tup-fit dedektor (naif detect'ten very more few FP) ============
def opening_signal (mesh ,R =3.0 ):
    """G1: konkavlik + convex-hull mesafesi birlesik -> more temiz candidate (naif single-sinyalden iyi)."""
...
def detect2 (mesh ,conc_thr =0.10 ,eps =4.0 ,min_pts =6 ,min_depth =2.0 ,max_width =16.0 ,min_ar =0.12 ):
    """G3: konkav+recessed vertexleri kumele -> each cluster for TUP-fit (axis/depth/width) ->
    fiziksel filtre. Cable acikligi = DERIN+DAR tup (high depth/width); snap/edge = sig -> elenir."""
    V =mesh .vertices .view (np .ndarray );N =mesh .vertex_normals .view (np .ndarray )
    ...
    for c in set (lab )-{-1 }:
        ...
        mouth =P [np .argmax (al )]if abs (al .max ())>abs (al .min ())else ctr # dis-surface ucu
