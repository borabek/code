...
THE PROBLEM IT FIXES: train_seg_extra's masked BCE supervises the connection channel ten EVERY vertex
...
def main ():
    ...
    for pid ,items in sorted (by_part .items ()):
        ...
        if not (os .path .exists (of )and os .path .exists (lf )):
            print (f"  {pid }: file none, atlandi");continue 
        V ,Fh =load_obj (of )
        ...
        frags =None 
        if models is not None :# the model's own regions ten this part
    ...
    print (f"\n{answered }/{dis ['n_disagreements']} cevap islendi, {touched } part guncellendi"
    +("  [DRY RUN -- dosyalar yazilmadi]"if a .dry_run else ""))
    print (f"  POZITIFE cevrilen vertex : {n_pos }   (duzeltilen wrong negatif)")
    print (f"  YOKSAY marked vertex: {n_ign }   (emin degilim -> kayiptan cikarildi)")
    print (f"  negatif birakilan bolge  : {n_neg }   (already dogruydu)")
