...
Sonra two grubun size_mm / depth_mm / votes / confidence dagilimlarini karsilastir.
  Ayriliyorsa (or. tel more large/deep) -> GEOMETRIK KAPI genel fix (yeniden training absent).
  Ayrilmiyorsa -> lower-sinif etiketi (tool'u Contact'defn ayir) + yeniden training is required.
Sizinti-siz 145 WEI + PXC ornegi. Cikti: results/recon_wire_tool.json + ayrilabilirlik summary.
"""
...
def main ():
    ...
    print (f"\n=== KARAR DENEYI ({len (A )} robot-CP: {len (wire )} tel-girisi / {len (tool )} tool/excess) ===")
