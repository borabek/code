# -*- coding: utf-8 -*-
"""ROBOT END-TO-END TEST (FAZ B4): does the full robot pipeline give correct CPs on UNSEEN parts?

This exercises the ACTUAL product path a robot cell would run -- robot_cp.extract() on a STEP file ->
{point, direction, size_mm, depth_mm, confidence, tier} -- and scores it against the manufacturer's
ConnectionPoints on parts the model never trained on. It reports what a robot operator actually cares
about, separated by the two-tier policy:

  AUTO tier  (confidence >= --conf-auto): the CPs the robot would act on WITHOUT a human.
...
Also reports, on matched CPs, the geometric error a robot would actually see:
...
def main():
    ...
    # aggregate: per tier TP/FP, plus geometry errors on matched
    ...
    print(f"\n=== ROBOT UCTAN-UCA ({len(parts)} gorulmemis parca, {n_gt} uretici CP) ===")
    pa = pr(at, af, n_gt)
    ...
    if pos_err:
        print(f"  eslesenlerde ROBOT hatasi: konum medyan {np.median(pos_err):.1f}mm | yon medyan {np.median(dir_err):.1f} deg")
    ...
    print("  Dusuk-guven CP'ler REVIEW'e dusuyor -> insana; robotun otonom hatasi degil.")
