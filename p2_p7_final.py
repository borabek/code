# -*- coding: utf-8 -*-
"""FAZ 2 / P7 -- FINAL DOGRULAMA. KILITLI HOLDOUT TEK KEZ ACILIR.
Kural zinciri (kullanici listesi):
 * tum model ve esikler holdout ACILMADAN kilitlendi -> results/final_config.json
 * holdout SADECE burada, TEK KEZ calistirilir (hicbir secim/tuning yapilmaz)
 * ALL / WEI / PXC / high-CP / family-out AYRI raporlanir
 * 3 seed ortalamasi + bootstrap guven araligi
 * AUTO precision ile AUTO+REVIEW F1 AYRI gosterilir
 * BASARI KAPISI: locked holdout ALL CP-F1 >= 0.85
Egitim: SADECE WORK (541 parca). Tahmin: 129 kilitli parca."""
import json
...
print(f"KILITLI KONFIGURASYON: {CFG['winner']} | esik {THR:.2f} | WORK ALL {CFG['WORK_ALL']:.4f}")
...
CONF = X13[:, 12]                                                  # segmentasyon guveni (AUTO tier icin)
mu, sd = FULL[WORK].mean(0), FULL[WORK].std(0) + 1e-6
XN = (FULL - mu) / sd
print(f"EGITIM (WORK): {int(WORK.sum())} aday / {len(set(gpid[WORK]))} parca")
print(f"HOLDOUT (KILITLI): {int(HOLD.sum())} aday / {len(set(gpid[HOLD]))} parca "
...
# ---- 3 seed: her seed icin RF + SetNet egit, olasilik ortalamasi ----
...
print(f"\n================ KILITLI HOLDOUT SONUCLARI (esik {THR:.2f}, TEK KEZ) ================")
print(f"{'altkume':16s} {'parca':>6s} {'GT':>5s} {'P':>7s} {'R':>7s} {'F1 (3 seed ort)':>18s}")
final = {}
for nm, m in subsets.items():
    if not m.any(): print(f"{nm:16s} (yok)"); continue
    f1s, ps, rs = [], [], []
...
# ---- bootstrap CI (parca bazli yeniden ornekleme) ----
...
print(f"\nBOOTSTRAP %95 GA (ALL, 2000 tekrar, parca-bazli): [{lo:.4f}, {hi:.4f}]  (nokta {np.mean(boot):.4f})")
