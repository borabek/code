# -*- coding: utf-8 -*-
"""60-PARCA KOK-WHY TESHISI: modelin HIC bulamadigi (tp==0) WEI parcalarinin acikligi, modelin GORDUGU
remeshed mesh'te geometrik oyuk as VAR mi? Found (tp>0) parcalarla karsilastir.
- zero-recall parts found'dan DUZ (low konkavlik) whereas -> opening mesh'te YOK -> hole-preserving remesh sart
- benzer konkavlik whereas -> opening VAR but model firmuyor -> training/model sorunu
CPU-only (GPU absent). geo_cp.concavity + align_frames kullanir."""
import os ,sys ,json 
import numpy as np 
...
def cp_concavity (pids ):
    """each part: manufacturer CP civarindaki (mesh frame) mesh vertexlerinin max konkavligi."""
    vals =[]
    for pid in pids :
        ...
        try :
            ...
            # mouth = seat'ten disariya ~ CP civari; CP'nin R-icindeki vertexlerin max konkavligi
...
print (f"\nZERO-RECALL parts (model HIC bulamadi): CP-konkavlik ort {zc .mean ():+.3f} med {np .median (zc ):+.3f} ({len (zc )} part)")
print (f"FOUND parts (model buldu):             CP-konkavlik ort {fc .mean ():+.3f} med {np .median (fc ):+.3f} ({len (fc )} part)")
print (f"\nKARAR:")
