# -*- coding: utf-8 -*-
"""S4 OLCUMU: aday-kumesi modeli (DeepSets) vs noktasal HGB -- AYNI protokolde.

TEK DEGISKEN MODEL SINIFI. Ayni korpus, ayni marka katlari, ayni oznitelikler,
ayni kural aramasi, ayni kabul kutusu. Fark yalnizca skorlayici:
  HGB      : her secenegi TEK BASINA puanlar (bugunku urun)
  DeepSets : secenegi parcanin ORTALAMA/MAKSIMUM ozetiyle birlikte puanlar

GEREKCE (S7). Secici verimliligi iki kutuplu (UPUN %65.5 / NIT %0.5) ve sebep
POZ-NEG SKOR AYRIMI'nin cokmesi (0.847 -> 0.050). Noktasal model "bu secenek
bu parcadaki digerlerine gore iyi mi" sorusunu goremiyor.

YETENEK DOGRULANDI (tests/test_kume_modeli.py): parca baglami gerektiren
sentetik gorevde DeepSets 0.483, noktasal 0.025 (sans 0.04).

KAPI: LOMO MIKRO robot F1'de HGB'ye **+0.05**. Alti kalirsa kol KAPANIR.
...
def main():
    ...
    for d in veri:
        ...
        d["_M"] = temel(d)
        # HAM `X` ARTIK GEREKMIYOR: puanlama P/idx/YD/G/Gd ile calisiyor.
        # Ikisini birden tutmak tepe bellegi ~10 GB'a cikariyordu (korpus
        # 3051 parca x ~2600 secenek x 162 sutun), bu da bu gece uc sureci
    ...
    print(f"{len(veri)} parca | {n_giris} sutun | katlar {katlar} "
    ...
    for b in katlar:
        ...
        # ---------- AYNI kural aramasi, AYNI kabul kutusu
        kat_sonuc[b] = {}
    ...
    print(f"\n=== S4 SONUC (MIKRO, {len(katlar)} marka kati) ===")
    ...
    print("makbuz -> results/s4_kume_modeli.json")
