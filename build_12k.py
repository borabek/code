# -*- coding: utf-8 -*-
"""Build a higher-resolution copy of everything we train/measure on, with the labels carried over.

WHY (evidence, 2026-07-22): the CPs the product still misses are the SMALL openings. Measured on the
    ...
    for split, out in (("train", f"_corpus{TAG}_train"), ("val", f"_corpus{TAG}_val")):
        ...
        for s in ds.load_split("wscad_corpus_scheffler_exact", split, verify_hashes=False):
            ...
            try:
                ...
                if done % 20 == 0: print(f"  ... {done} parca ({time.time()-t0:.0f}s)", flush=True)
    ...
    for src in ("_label_targets", "_label_targets_2", "_label_targets_3", "_label_targets_4"):
        ...
        for d in sorted(glob.glob(os.path.join(src, "*"))):
            ...
            try:
                ...
                print(f"  {pid}: HATA {str(e)[:45]}"); fail += 1
    print(f"\n12k veri seti hazir: {done} parca, {fail} atlandi, {time.time()-t0:.0f}s")
