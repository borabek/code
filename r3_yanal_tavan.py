# -*- coding: utf-8 -*-
"""R3: YANAL TAVAN + DOGRU PAYDA -- robot fazinin hic olculmemis yarisi.

IKI IS BIRDEN, TEK KOSUDA (paydalar tutarli olsun diye):

1. YANAL TAVAN (hic olculmedi): poz kafasi dagitildi (+0.0428, CI-kanitli) ama
   "konum MUKEMMEL olsa ne olurdu" hic sorulmadi. Bir KONUM SOZLUGU kurulur ve kahin
   sinavi yapilir -- yon tarafinda yaptigimizin aynisi.

2. PAYDA DUZELTMESI: C1-C3'te yon sozlugunun kapsamini 0.7584'e bolerek hesapladim.
   O YANLIS. 0.7584 HEM yanal HEM aci mukemmelken ulasilan tavandir; yon sozlugu
   YALNIZ ACI hatalarini duzeltebilir. Dogru payda "ACI MUKEMMEL" tavanidir.

Olculen tavanlar (hepsi ayni eslesme koduyla, ayni kumede):
    robot MEVCUT              yanal<=2 VE aci<=10
    ACI MUKEMMEL tavani       yalniz yanal<=2   (aci serbest)
    YANAL MUKEMMEL tavani     yalniz aci<=10    (yanal serbest -> tespit toleransi)
    ikisi mukemmel            = tespit F1

KONUM SOZLUGU (hepsi calisma aninda turetilebilir, fiziksel):
    mevcut      poz kafasi ciktisi
    ham         duzeltmesiz aday noktasi
    uye_k       uye havuzundaki noktalar
    agiz        yerel agiz sinir noktalarinin ortalamasi (tezin v_o'su, yeniden hesap)
    eksen       B-rep silindir EKSENINE dik izdusum (yanal hatayi dogrudan hedefler)
    kesit_d     kanal kesitinin d mm derinlikteki agirlik merkezi (d = 1,3,5)

KILL: konum kahini, ACI-MUKEMMEL tavaninin %70'ini kapsamiyorsa yanal kolu kapanir.
"""
import io
import json
import os
...
def konum_sozlugu(V, p, d, cyl, ba):
    """Aday icin fiziksel KONUM adaylari."""
    S = {}
    d = _birim(d)
    ...
    # AGIZ: |t|<=1.5mm bandindaki noktalarin ortalamasi (tezin v_o'su)
    m = (np.abs(t) <= 1.5) & (rad <= 9.0)
    if m.sum() >= 6:
        S["agiz"] = V[m].mean(0)
    # KESIT: d mm derinlikte kanal kesitinin merkezi
    for dd in (1.0, 3.0, 5.0):
        m2 = (np.abs(t + dd) <= 1.0) & (rad <= 9.0)
        if m2.sum() >= 5:
            S[f"kesit{int(dd)}"] = V[m2].mean(0)
    # B-REP EKSENINE dik izdusum: yanal hatayi DOGRUDAN hedefler
...
def main():
    ...
    for kk, r in enumerate(D["DER"], 1):
        ...
        # ACI MUKEMMEL tavani: aci serbest, yanal<=2
        KOL["aci_mukemmel"].append((rj,) + esle(P, Pd, G, Gd, r["diag"], 2.0, 180.0, False))
        # YANAL MUKEMMEL tavani: yanal serbest (tespit toleransi), aci<=10
    ...
    print(f"\n{'tavan / kol':<30}{'robot':>9}")
    print(f"{'MEVCUT':<30}{F['mevcut']:>9.4f}")
    print(f"{'ACI mukemmel olsa (yanal<=2)':<30}{F['aci_mukemmel']:>9.4f}")
    ...
    print("makbuz -> results/r3_yanal_tavan.json")
