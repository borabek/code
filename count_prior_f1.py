# -*- coding: utf-8 -*-
"""Sayi onculu URUN metrigine ne kazandiriyor? (CP-F1, sizinti-siz)

BAGLAM: metadata modu (gercek CP sayisi biliniyorsa top-N) olculmus +0.025 veriyor ama katalog
verisi gerektiriyor. count_prior.py gosterdi ki geometri sayiyi +-1 dogrulukla %77 biliyor
(MAE 1.30, taban 6.04). Bu betik, o TAHMINI sayiyla ne kadarini geri alabildigimizi olcer.

KOLLAR
  A) taban        : gate esigi (urunun bugunku davranisi)
  B) top-N GERCEK : gercek CP sayisi bilinseydi (TAVAN -- ulasilamaz, referans)
  C) top-N TAHMIN : geometriden tahmin edilen sayi (YENI KALDIRAC)
  D) yumusak      : tahmine gore parca basina esik kaydirmasi (sert top-N'den daha guvenli)

SIZINTI: hem gate skoru hem sayi tahmini AILE-DISI OOF. Bir parca, kendisini (ya da ailesini)
gormemis modellerle skorlanir.

KILL (olcumden once): korpus-agirlikli CP-F1 katkisi < +0.02 -> olu.
"""
import os, sys, json
import numpy as np
...
def main():
    ...
    # --- OOF sayi tahmini (aile-disi, AYNI bolme mantigi) ---
    ...
    def arm_soft(getn, lo=0.20, hi=0.60):
        """Esigi parca basina kaydir: beklenenden COK aday varsa esik yukselir."""
        def f(g, s, yy):
            n = max(1, getn(g))
    ...
    arms = {
        "A taban (esik 0.35)": arm_threshold(0.35),
    ...
    print(f"{len(true_n)} parca | {len(y)} aday\n")
    print(f"{'kol':<22}{'dusuk-CP':>10}{'cok-CP':>10}{'agirlikli':>12}{'fark':>10}")
    base = None; res = {}
    ...
    print(f"TAVAN (gercek sayi): {res['B top-N GERCEK']['weighted']-base:+.4f}")
    ...
    print("makbuz -> results/count_prior_f1.json")
