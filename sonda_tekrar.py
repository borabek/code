...
TESHISIMDEKI KUSUR (2026-08-12). S7'de "dogru secenegin sira yuzdeligi 0.005"
diye olctugum sey, parcadaki **EN IYI siralanmis** dogru secenegin sirasiydi.
Yani "bir tane dogru secenek tepeye yakin" diyor -- 24 CP'li bir parcada
24.'sunun nerede oldugu hakkinda HICBIR SEY soylemiyor. "Siralama iyi"
cikarimim bu yuzden fazla iyimserdi.

DOGRU SORU: k'inci dogru secenek kacinci sirada? (k = parcanin CP sayisi)

HIPOTEZ: model ILK CP'yi buluyor, TEKRARLARI bulamiyor. Bu, gozlenen deseni
birebir aciklar -- az CP'li marka (UPUN 3.2) calisiyor, cok CP'li (NIT 24.4)
cokuyor; cunku bugun HER CP kendi basina markalar-arasi bir karar gerektiriyor.

AYRICA OLCULUR -- TEKRAR TAVANI: en iyi tohumdan TEK BIR OTELEME ile kac GT
uretilebilir? Bu, "kendine benzerlik ile yayilim" kolunun ust siniridir ve
markalar-arasi transfer GEREKTIRMEZ: gorulmemis markada da 24 delik
...
EKSEN_TOL = 40.0
OTEL_TOL = 1.0        # oteleme ile uretilen nokta GT'ye bu kadar yakinsa TUTAR
...
def tekrar_tavani(G):
    """EN IYI TEK OTELEME ile kac GT uretilebilir (bir tohum + n*adim)?

    Butun GT ciftlerinin fark vektorleri aday adimdir; her aday icin
    'tohum + n*adim' izgarasina OTEL_TOL icinde dusen GT sayilir.
    """
    ...
    # aday adimlar: en yakin komsu farklari (hepsi degil -- O(n^2) yeter)
    farklar = (G[:, None, :] - G[None, :, :]).reshape(-1, 3)
    boy = np.linalg.norm(farklar, axis=1)
    aday = farklar[(boy > 0.5) & (boy < np.percentile(boy[boy > 0.5], 40))]
    if not len(aday):
        return 1, None
    if len(aday) > 400:
        aday = aday[np.random.default_rng(0).choice(len(aday), 400, False)]
    for adim in aday:
        L = np.linalg.norm(adim)
        u = adim / L
        for tohum in G:
            t = (G - tohum) @ u
            dik = np.linalg.norm((G - tohum) - t[:, None] * u[None, :], axis=1)
            k = np.round(t / L)
            hata = np.abs(t - k * L)
            tut = int(((dik <= OTEL_TOL) & (hata <= OTEL_TOL)).sum())
            if tut > en_iyi:
...
def main():
    ...
    for d, s in zip(veri, oof):
        ...
        # HER GT icin: onu tutan seceneklerin EN IYI sirasi
        gt_sira = []
        ...
        # ilk k sirada kac GT var (k = gercek CP sayisi)
    ...
    print(f"\n{'marka':<7}{'CP/p':>7}{'ILK GT':>9}{'ORTANCA':>9}{'SON GT':>9}"
    ...
    print(f"makbuz -> results/tekrar_sondasi_{KUME}.json")
