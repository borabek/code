# -*- coding: utf-8 -*-
"""B-kolu: GATE HEDEFI = ROBOT-HAZIR (tespit yerine).

Urunun hedefi ROBOT ama gate "bu aday bir CP mi" diye egitiliyor. Robot-hazir
olmak yanal<=2mm VE isaretli aci<=10 gerektirir; gate bu ikisini HIC gormuyor.
Hipotez: hedefi degistirmek robot F1'i artirir (kesinlik bedeliyle).

Korpus v4 (g10), MARKA-DISI bolme. Tek degisken: y.
KILL: marka-disi robot-F1 artmiyorsa kol NULL.
"""
...
# ROBOT-HAZIR etiketi turetmeden yeniden uretilir (korpusta yok)
...
for u in np.unique(pid):
    i = np.where(pid == u)[0]
    M[i] = wire_gate.parca_ici(X[i], "zskor")
print(f"aday {len(M)} | tespit-poz %{100*y_tes.mean():.1f} | "
...
for m in test_mf:
    ...
    for ad, yy in (("hedef=TESPIT", y_tes), ("hedef=ROBOT", y_rob)):
        ...
        p = s >= 0.30
        # ROBOT F1 ile degerlendir (urunun hedefi)
...
if out:
    ...
    print("makbuz -> results/b_gate_hedefi.json")
