# -*- coding: utf-8 -*-
"""Retrain the 5-class DiffusionNet segmentation on the ENLARGED corpus: our 71 train + the 132
downloaded EEC parts (`_eec_extra/`) = 203 train, validate on our 20 val, and NEVER touch the
...
extra parts by parsing OBJ+labels. Operators stay on CPU and move per-part (a 4GB GPU cannot hold
...
# OPERATOR ONBELLEGI k_eig'e GORE AYRILIR (2026-07-29): tek bir mesh-anahtarli dizin, 64 ve 96
# eigen ile calisan kosular arasinda surekli "overwriting cache -- not enough eigenvalues" +
# WinError 32 dosya kilidi dongusune giriyordu (egitim ilerlemiyordu). robot_cp ayni sorunu
# ops_k{64,96} ayrimiyla cozmustu; burada da ayni ayrim yapiliyor.
...
def load_extra(root="_eec_extra"):
    ...
    for d in sorted(glob.glob(f"{root}/*/")):
        # NOTE: os.path.normpath, NOT d.rstrip("/") -- on Windows glob returns a trailing BACKSLASH,
        ...
        if len(V) and len(F) and len(V) == len(L) and F.max() < len(V) and set(np.unique(L)) <= set(range(5)):
            ...
            # Tezin 5-sinif etiketini DEGISTIRMEZ: ayri dosya, ayri kafa, ayri kayip.
            ...
            # down on every unmarked vertex) -- i.e. a guess would become a hard training signal.
...
def miou(model, meta, data, dev):
    ...
    Conn-IoU 0.5878 -> 0.0995'e COKTU. Sebep: kismi etikette YALNIZ CableEntry isaretli,
    geri kalan 0 (Housing). Maskesiz IoU, modelin DOGRU tahmin ettigi Contact bolgelerini
    YANLIS POZITIF sayiyordu. Bu, EGITIM HEDEFIYLE CELISIR -- egitimde kayip maskeli
    (`partial_ce` dallarinda yalniz CE-vs-degil denetleniyor).
    Boyle bir sinyalle secim yapmak, Contact'i DAHA AZ tahmin eden modeli "iyi" sanmak
    demekti; yani duzeltme diye konan sey YENI bir yanlilik yaratirdi.

    Cozum: kismi ornekte yalnizca ISARETLI bolgenin CIVARINI puanla -- isaretli tepeler
    ve modelin baglanti dedigi tepeler. Isaretsiz govde bolgeleri metrikten CIKARILIR.
    ...
    with torch.no_grad():
        for d in data:
            ...
            if kismi:
                # SECIM SINYALI = EGITIM HEDEFININ AYNISI (2026-08-07, ikinci duzeltme).
                # Ilk duzeltmem yetmedi: Conn-IoU'nun BIRLESIMI zaten (pc|gc) oldugu icin
                # maske orada hicbir sey degistirmiyordu ve acc %6'ya dusuyordu -- cunku
                # kismi etikette isaret CE, egitim hedefi ise CE+CT; SINIF-BAZLI esitlik
                # modelin DOGRU Contact tahminini yanlis sayiyor.
                # Metrik ICAT ETMEK yerine egitim kaybinin KENDISINI kullaniyoruz:
                # maskeli BCE, ayrik veride. Tanim geregi egitim hedefiyle BIREBIR ayni.
                ...
                kayip = (_Fn.binary_cross_entropy(pp, yy, weight=w)).item()
                kismi_kayip.append(kayip)
            pc = np.isin(pr, (NCLS_CE, NCLS_CT)); gc = np.isin(gt, (NCLS_CE, NCLS_CT))
            if kismi:
                # MASKE: isaretli tepeler + modelin baglanti dedigi tepeler.
                # Isaretsiz govde metrige GIRMEZ (orada GT bilgisi YOK).
                m = gc | pc
                if not m.any():
                    continue
    ...
    # KISMI val varsa SECIM SINYALI maskeli BCE'nin NEGATIFI olur (buyuk = iyi),
    # boylece cagiran taraf "connection_iou'yu ENBUYUKLE" mantigini degistirmeden calisir.
    if kismi_kayip:
...
def main():
    ...
    # FEW-SHOT (K6.5-b): k parca 200 parcalik korpusun icinde KAYBOLUR. Iki kip:
    #  --yalniz-kismi : YALNIZ k parcayla adapte ol (agresif; unutma riski VAR)
    #  --kismi-tekrar : korpusu koru ama k parcayi N kez tekrarla (gerceksi recete)
    ap.add_argument("--yalniz-kismi", action="store_true",
                    help="FEW-SHOT: egitim kumesi YALNIZ --partial-dir olsun")
    ...
    ap.add_argument("--val-partial", action="store_true",
                    help="VAL dizini KISMI etiketli (yalniz CableEntry isaretli). Dogrulama "
                         "metrigi MASKELI olcer -- isaretsiz govde metrige girmez. Bu bayrak "
                         "olmadan Conn-IoU yapay olarak COKER (0.5878 -> 0.0995 olculdu) ve "
    ...
    # verildiginde dizin yoksa torch.save ILK KAYITTA patliyor -- yani egitim
    # 10 epoch bosuna kosuyor ve olduktan sonra fark ediliyor (40 dk kayip).
    _cd = os.path.dirname(a.checkpoint_out)
    if _cd:
        os.makedirs(_cd, exist_ok=True)
    # FINE-TUNE MIMARI UYUMU (K6.5-b, 2026-08-07): kaynak ckpt kendi cfg'sini tasir.
    # g10 `n_eig=96` ile egitilmis, bu betigin varsayilani 64. Uyusmazsa model
    # EGITILDIGINDEN BASKA bir oztabanla kosar; hata VERMEZ, sadece kotu sonuc
    # verir ve "few-shot calismiyor" diye YANLIS bir kol kapatilirdi.
    _ilk_cfg = None
    if a.init_from:
        import torch as _t
    ...
    if a.val_partial:
        # VAL de kismi etiketli -> metrik MASKELI olsun (yoksa maske hic devreye girmez:
        # bayrak yalnizca EGITIM listesine konuyordu, val'e degil).
        for s in va: s["partial_ce"] = True
        # GUARD: an all-zero partial file is NOT neutral -- under the masked BCE every vertex becomes
        # "not CableEntry", actively teaching the model to suppress CableEntry on a part that does
    ...
    if a.yalniz_kismi:
        ...
        print(f"  [few-shot] YALNIZ KISMI KIP: korpus ({len(tr)}) + extra ({len(extra)}) "
              f"BIRAKILDI, egitim {len(partial)} ornek", flush=True)
        tr, extra = [], []
    ...
    w[NCLS_CE] *= a.ce_weight_mult   # boost CableEntry (the class the model misses on unseen parts)
    ...
    # yukleme "fine-tune ettim" sanip aslinda rastgele agirlikla egitmeye yol acar.
    if a.init_from:
        # weights_only=False BILEREK: PyTorch 2.6 varsayilani True ve KENDI
        # ckpt'lerimizdeki numpy skalerlerini reddediyor. Dosya bizim egitim
        # betigimizin ciktisi (guvenilir kaynak); disaridan gelen ckpt yuklenmez.
    ...
    # FB-2: YARDIMCI KAFA. Govde paylasilir (last_lin ONCESI 128-d gizli temsil),
    # cikis ayridir. Ana 5-sinif kafasi ve kaybi HIC DEGISMEZ.
    aux_lin = None; _gizli = {}
    if a.aux_wire:
        # GIZLI BOYUT MODELDEN OKUNUR: cfg["width"]=64 ama last_lin'in girdisi 128.
        # (cfg'ye guvenmek "mat1 and mat2 shapes cannot be multiplied" ile patliyordu.)
    ...
    for ep in range(1, a.epochs + 1):
        ...
        for i in np.random.permutation(len(tr_d)):
            ...
            if d.get("partial_ce"):
                ...
                # MEASURED 2026-07-20: on the human-marked vertices the corpus-trained model says
                ...
                # the CableEntry channel alone therefore fights the corpus on ~half the marks. The
            ...
            if aux_lin is not None and d.get("aux") is not None and "z" in _gizli:
                ...
                mk = ya >= 0
                if bool(mk.any()):
                    # gizli temsil (1, V, 128) geliyor -> duzlestir, yoksa maske
                    # [V] ile tensor [1,V] uyusmuyor.
    ...
    # #8 FIX: korpus-val WEI-underfit epoch secebilir. Last-epoch'u da kaydet -> ikisini WEI arbiter'da
    # (gercek hedef) karsilastirip en iyisi secilir (post-hoc WEI selection). NcLS/masked-WEI-val yerine bu.
