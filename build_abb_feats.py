# -*- coding: utf-8 -*-
"""ABB KOLU -- ogrenme egrisi tavanlanmadigi for (still +0.010 AUC/step) VERI ekliyoruz.
Kaynak: Desktop/JSON'daki 386 ABB parcasi = 11,228 manufacturer CP (mevcut 1888 GT'nin 6 KATI),
mean 29 CP/part -> full da modelin coktugu HIGH-CP rejimi.
AVANTAJ: mesh JSON Graphic3d'den gelir and CP'ler AYNI FRAME'dedir -> align_frames YOK,
hizalama error kaynagi tamamen ortadan kalkar.
Cikti: results/abb_feats.npz (X13, XR, y, groups, part_ids, ngt) -- rich_feats with same format."""
...
def main ():
    ...
    print (f"-> {out }  {len (y )} candidate, {len (NGT )} part, GT {int (ng .sum ())}, "
    f"candidate-ceiling recall {y .sum ()/max (ng .sum (),1 ):.3f}  {time .time ()-t0 :.0f}s")
