...
def _arrow_lines(origin, direction, length=10.0, head_frac=0.25, spread=0.15):
    ...
    The arrow is: shaft from origin to tip, plus 3 head fins from the tip.
    """
    ...
    pts = [origin, tip] + fins      # 0=origin, 1=tip, 2,3,4=fin bases
...
def build_scene(glb: _GlbBuilder, vertices, faces,
    ...
    # 4. FIZIKSEL KUSUR ISARETCILERI (M10, 2026-08-05) -- AYRI GORSEL KANAL
    #
    # NEDEN AYRI KANAL: sozlesmenin M6 maddesi "RENK = ESLESME DURUMU" diyor ve denetci
    # renk sayimlarini makbuzun TP/FP/FN'iyle karsilastiriyor. Kusurlari yeni RENKLERLE
    # gostermek M6'nin sayimini BOZARDI. Bunun yerine ayni noktaya DAHA KUCUK ikinci bir
    # kure konur; kendi ad onekiyle (`fiz_*`) sayilir, M6 aynen calismaya devam eder.
    #
    # NEDEN GEREKLI: A4 olcumu -- FP'ler fiziksel kusurda ZENGIN (govde ici 3.16x,
    # dar agiz 2.18x, onu kapali 1.85x). Bu kusurlar SAYIDA gorunmuyor ama GLB'ye bakan
    # insan icin "bu nokta neden yanlis" sorusunun DOGRUDAN yaniti.
    #
    # Bayrak kaynagi: her CP sozlugunde opsiyonel `fiz_bayrak` (govde_ici/onu_kapali/
    # dar_agiz/ters_yon). Yoksa hicbir sey cizilmez -- eski GLB'ler BIT-OZDES kalir.
    _FIZ_RENK = {
        "govde_ici":  [1.00, 0.00, 0.00, 1.0],   # kirmizi   -- nokta govdenin ICINDE
        "onu_kapali": [1.00, 0.55, 0.00, 1.0],   # turuncu   -- ileride engel var
        "dar_agiz":   [1.00, 1.00, 0.00, 1.0],   # sari      -- agiz telden dar
        "ters_yon":   [0.00, 0.80, 1.00, 1.0],   # camgobegi -- yon govdeye bakiyor
    }
    if ml_cps:
