...
Denetimin P1 maddesi. Simdiye kadar B-rep'ten yalniz TEK YUZ okunuyordu (yaricap, eksen);
yuzlerin BIRBIRIYLE ILISKISI hic kullanilmadi. Sekiz ozellik (iki tanesi bos ciktigi icin
DUSURULDU -- yazilmamislardi, sifir tasiyorlardi):

    g_kom_n / g_kom_cyl / g_kom_pl   agiz cevresindeki B-rep yuz sayilari
    g_esek_n / _r_ort / _r_yay       eseksenli silindir zinciri (huni/kademe imzasi)
    g_kor / g_gecis                  kanal kor mu, boydan boya mi (boydan boya = tel girisi DEGIL)

Kapsama %96.6.

TASARIM: ayni adaylar, tek degisken ozellik kumesi. Once aday duzeyi (ucuz eleme), gecerse
UCTAN UCA (bu arastirmada aday duzeyi ALTI KEZ yaniltti).

KILL: uctan uca tespit >= +0.01 VE karar_olcutu bes sarti.
"""
import collections
import io
import json
...
def main():
    ...
    # --- 1) ADAY DUZEYI eleme
    print("\nADAY DUZEYI (grup-capraz OOF, parca-ici goreli esik):")
    def olc(M):
        o = np.zeros(len(y))
    ...
    # olcum tarafinda graf sutunlari: DER parcalarinin satirlarindan al
    def der_graf(r):
        """Graf sutunlari ADAYIN KENDI noktasindan hesaplandi (`_der_zengin_graf.pkl`).

        Ilk surumde zengin npz'den PID ile eslestiriliyordu ve 34/194 parcada aday sayisi
        tutmuyordu -> o parcalar SIFIR CP aliyor ve metrik cokuyordu. Ozellik degil OLCUM
        hatasiydi."""
        g = r.get("GX")
        return g[:, tut] if g is not None else None

    eksik = sum(1 for r in DER if der_graf(r) is None)
    print(f"\nolcum tarafinda graf sutunu EKSIK olan parca: {eksik}/{len(DER)}")

    SON, PARCA = {}, {}
    print(f"\n{'kol':<16}" + "".join(f"{b:>12}" for b, _, _ in BOLME))
        ...
        for b, mk, alt in BOLME:
            ...
            for r in alt:
                ...
                if Xr is not None and ek:
                    G_ = der_graf(r)
                    # Graf sutunu yoksa NOTR (sifir) -- parcayi DUSURME (ilk surumun hatasi)
    ...
    print("makbuz -> results/s8_brep_graf_ab.json")
