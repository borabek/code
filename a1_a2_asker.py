# -*- coding: utf-8 -*-
"""ASKER LISTESI A1 + A2 -- tek GPU kosusu, ayni cikarim paylasiliyor.

A1  ROUTER TAZELIK DENETIMI: router 1753-parca, min_v=10 donemi union sayilariyla egitildi.
    min_v 4'e indi -> girdi dagilimi kaydi. Dogru girdiyle (promote'suz min_v=4 union sayisi)
    hala dogru rota veriyor mu? N2b referansi: dogruluk 0.953, cok-CP yakalama 0.875.
    -> dusmediyse DOGRULANDI (degisiklik yok); dustuyse rt2 dagilimiyla yeniden egitilir.

A2  min_v IZGARA KENARI: K4b izgarasi (4,6,10,15) 4'te durdu ve 4 KENARDAYDI -- kendi
    "izgara kenari optimum ise araligi genislet" kuralimin ihlali. 2 ve 3 hic olculmedi.
    TAVAN (onceden): dusuk-CP aday-R zaten ~0.95+ -> kazanc ancak cok-CP'den, butcesi +0.033.
    Beklenen 0...+0.01. KILL: agirlikli < +0.01 -> urune girmez.

SIZINTI NOTU: dagitilan gate (wire_gate.pkl) bu parcalari egitimde gordu -> SEVIYELER sismis.
A2 bir A/B'dir: ayni gate, ayni parcalar, tek fark min_v -> DELTA gecerli, seviye degil.
"""
...
def main():
    ...
    print(f"{len(sel)} parca (26 dusuk / 22 cok)", flush=True)
    ...
    for mfg, pid, jf, stp, n in sel:
        try:
            ...
            continue
    print(f"onbellek {len(cache)} parca\n", flush=True)
    ...
    # ---------------- A1: router tazelik denetimi (dogru girdi: promote'suz min_v=4 union) ----
    tp = fp = fn = tn = 0
    for r in cache:
    ...
    print(f"  SONUC: {'DOGRULANDI -- yeniden egitim gereksiz' if a1_ok else 'DUSTU -> yeniden egitilmeli'}\n",
          flush=True)

    # ---------------- A2: min_v 2/3/4, calisma mantigiyla (router + promote) -----------------
    def score(mv, add_avg=False):
        agg = {"dusuk": [0, 0, 0], "cok": [0, 0, 0]}
        v1_tp = n_tp = 0                      # eslesen GERCEKLERIN kaci tek-oy (mekanizma kaniti)
        for r in cache:
    ...
    # ---------------- A5: UZLASMA YUKSELTICI (5. uye = ortalama harita) ----------------------
    # K2: gate'in oldurdugu gercekler TEK-OY (votes AUC 0.86). Ortalama haritanin adaylari,
    # sinirda-gercek acikliklari 1 oydan 2 oya cikarir; tek-model gurultusu ortalamada kaybolur
    # ve oyu YUKSELMEZ. best_full/keig128'den farki: yabanci uye degil, uyelerin kendi ortalamasi
    # -> uzlasmayi sulandirmaz, keskinlestirir. Mekanizma kaniti = tek-oy gercek payinin dusmesi.
    print(chr(10) + "=== A5 uzlasma yukseltici (4 uye vs 4+ortalama) ===")
    r4 = res[4]
    r5 = score(4, add_avg=True)
    print(f"{'kol':>16}{'dusuk':>9}{'cok':>9}{'agirlikli':>11}{'tek-oy gercek':>15}")
    ...
    print(f"  NOT: gate 4-uyeli dagilimla egitildi; A5 kalici olacaksa gate YENIDEN uydurulmali."
          f" Bu olcum ILK SINYAL.")

    # ---------------- A6: modeller-arasi ANLASMAZLIK ozelligi (hizli AUC) --------------------
    ...
    print(chr(10) + "makbuz -> results/a1_a2_asker.json")
