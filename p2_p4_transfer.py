 ...
 * template CP'leri yalnizca TRAIN-fold parcalarindan alinir (part-out CV; kardesler farkli fold'a duser)
...
print(f"WORK {len(work)} parca | {len(fam)} aile | cok-parcali {len(multi)} aile / "
      f"{sum(len(v) for v in multi.values())} parca", flush=True)
...
for f, members in multi.items():
    for tgt in members:
        # part-out CV benzetimi: template = AYNI ailenin BASKA bir parcasi (train-fold'da varsayilir)
...
print(f"\ntransfer denenen: {len(rows)} parca")
...
print("\nNOT: unseen-family (singleton) parcalar transfer ALAMAZ -> agregada SIFIR katki (kural geregi dahil).")
