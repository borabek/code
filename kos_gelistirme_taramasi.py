# -*- coding: utf-8 -*-
"""GELISTIRME TARAMASI — dort kaldirac TEK kosuda, UCTAN UCA

Olcum fasli kapandi. Bu betik yalnizca F1'i YUKSELTMEYE calisir.
Taranan dort kaldirac (hicbiri sistematik denenmedi):

  1. HPO          : ogrenme hizi / yaprak / iterasyon / L2
  2. ZOR NEGATIF  : negatif orani ve SECIMI (rastgele vs skor-yakin)
  3. AGIRLIK      : parca-esitleyici agirlik -- NIT GT'nin %51'i, egitimi
                    eziyor; her parca esit agirlik alirsa seyrek markalar
                    bogulmaz
  4. KANONIK      : gunun tek uctan uca kazanci (+0.0093) taban kabul edilir

Her yapilandirma AYNI marka-disi katlarda, AYNI karar kuraliyla, UCTAN UCA
robot F1 ile olculur. Kazanan, `tam` katlarinda AYRICA dogrulanmadan urune
...
def negatif_sec(M, Y, agirlik_p, neg_kat, zor, rng, on_skor=None):
    """negatif ornekleme. zor=True ise SKOR-YAKIN negatifler tercih edilir."""
    poz = np.where(Y == 1)[0]
    neg = np.where(Y == 0)[0]
    n_al = min(len(neg), int(neg_kat * max(len(poz), 1)))
    if zor and on_skor is not None:
        # ZOR NEGATIF: on gecisin en yuksek skorlu negatifleri. Yarisini
        # zor, yarisini rastgele al -- salt zor secim dagilimi bozar.
...
def main():
    ...
    print(f"{len(veri)} parca | katlar {katlar} | taban = temel + kanonik",
          flush=True)

    # ---- taranan yapilandirmalar (TABAN ilk sirada)
    TABAN = dict(lr=0.06, yaprak=63, iter=200, l2=1.0, neg=6, zor=False,
                 esit=False)
    if os.environ.get("GT_MOD") == "4":
        # URETIM TABANI DUZELTMESI. Taramanin tabani neg=6 idi, ama URETIM
        # egiticisi (`kos_p6_kademe2.py`) `P6_NEG_KAT` varsayilani **8**
        # kullaniyor. Yani olculen "+0.0088" 6->12 farkidir; uretimin gercek
        # kazanci 8->12 ve DAHA KUCUK olmali. Dagitilan sayi bu olmali.
        ADAYLAR = [{**TABAN, "neg": 8}, {**TABAN, "neg": 12}]
    elif os.environ.get("GT_MOD") == "3":
        # DOGRULAMA TURU. d6'da secilen ayar (neg=12, +0.0088) `tam`
        # katlarinda tekrar edilir. d6'da ayar yapip d6'da ilan etmek
        # d6'ya ezberlemektir; kapi ONCE ilan edilmisti.
        ADAYLAR = [TABAN, {**TABAN, "neg": 12}]
    elif os.environ.get("GT_MOD") == "2":
        # IKINCI TUR. Birinci tur tek bir ekseni acti: NEGATIF ORANI
        # (neg=12 +0.0088, neg=3 -0.0498). Ogrenme hizi/yaprak/L2 notr ya da
        # zararli; zor negatif -0.0195; parca-esitleyici agirlik -0.1120
        # (KAPANDI -- NIT baskinligi egitime zarar degil FAYDA veriyor).
        # Bu tur yalniz negatif oranini sonuna kadar surer ve tek yardimci
        # eksenle (lr=0.03/iter=400, +0.0045) birlestirir.
        ADAYLAR = [TABAN]
    ...
    for ci, c in enumerate(ADAYLAR):
        ...
        for b in katlar:
            ...
            for i in ic:
                m_ = veri[i]["_M"]
                M[o:o + len(m_)] = m_
                # PARCA-ESITLEYICI AGIRLIK: her parcanin toplam agirligi 1.
                # Boylece 24 CP'li NIT parcasi 3 CP'li parcadan 8 kat fazla
                # soz sahibi olmaz.
            ...
            on = None
            if c["zor"]:
                # on gecis: kucuk ve hizli bir model, KAT-ICI capraz degil --
                # amaci yalnizca "hangi negatifler kafa karistirici" demek
                alt = np.concatenate([
                    np.where(Y == 1)[0],
    ...
    json.dump({"damga": makbuz_hash.damga(), "kume": KUME, "taban": taban,
               "en_iyi": {"f1": sonuc[0][0], "yapilandirma": sonuc[0][1]},
               "hepsi": [{"f1": r, "c": c} for r, c in sonuc],
               "not": "HPO + zor negatif + parca-esitleyici agirlik, taban "
    ...
    print(f"makbuz -> results/gelistirme_taramasi_{KUME}.json")
