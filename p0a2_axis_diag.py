...
P0-a: saflik z=+0.09 (sinyal yok) AMA sifir-TP kumesinde FP %60.6 -> bu taban-orani artefakti olabilir
(adaylarin %85'i FP). Uc soru:
 (1) NULL-DUZELTILMIS money metrik: sifir-TP kumesindeki FP kutlesi SANSTAN yuksek mi?
 (2) ALET calisiyor mu: TP yonleri parca icinde birbirine YOGUN mu (wire girisleri ortak eksen paylasir
     beklentisi)? Yogun degilse ya cok-yonlu urun ya YON TAHMINIMIZ GURULTU -> hipotez test EDILEMEMIS olur.
 (3) TP yonleri FP yonlerinden daha mi yogun (ayirici guc var mi)?
CPU-only, mevcut havuz."""
import json, sys
import numpy as np
...
def resultant(D):
    """yon yogunlugu (mean resultant length): 1=hepsi ayni yon, 0=dagilmis."""
...
for pid, v in d.items():
    ...
    tp_span.append(len(np.unique(lab[y == 1])))          # TP kac ayri eksen kumesine yayilmis
...
print(f"POOL {POOL} | {len(obs_mass)} parca\n")
...
print("\n=== (2)+(3) ALET KONTROLU: yon tahminleri anlamli mi ===")
v_tp = tp_res[~np.isnan(tp_res)]; v_fp = fp_res[~np.isnan(fp_res)]
print(f"  TP yon-yogunlugu (resultant): ort {v_tp.mean():.3f} med {np.median(v_tp):.3f}")
print(f"  FP yon-yogunlugu            : ort {v_fp.mean():.3f} med {np.median(v_fp):.3f}")
print(f"  TP kac eksen-kumesine yayiliyor: ort {tp_span.mean():.2f} (1.0 = hepsi tek eksende)")
print(f"  TP'nin tek-eksende oldugu parca orani: {np.mean(tp_span==1):.2f}")
if v_tp.mean() > 0.85:
    print("  -> TP yonleri COK YOGUN: alet calisiyor, wire girisleri ortak eksen paylasiyor.")
    print("     O HALDE ayirici guc yoklugu = FP'ler de AYNI eksende (ayni yuzde, tel deliklerinin yaninda).")
elif v_tp.mean() > 0.6:
    print("  -> TP yonleri ORTA yogun: kismen ortak eksen; alet muhtemelen calisiyor.")
else:
