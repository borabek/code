# -*- coding: utf-8 -*-
"""E: gate'e EKSEN GUVENI ozelligi. Ucuz olcum -- gate korpusunu bastan kurmadan.

ON-TARAMA (yapildi): eksen kaynagi TP/FP'yi ayiriyor mu?
    B-rep SILINDIR eslesti   n=393  TP %94.7
    B-rep DUZLEM eslesti     n= 74  TP %89.2
    hicbiri (belirsiz)       n= 80  TP %85.0
    -> eksen BELIRLI %93.8 vs BELIRSIZ %85.0 = +8.8 puan. Gercek ama zayif.

Zayif diye ATMIYORUZ: RF 13 ozelligi zaten kullaniyor, biri daha marj katabilir. Ama pahali
yolu (gate_regrow, saatler) kosmadan once UCUZ ve DOGRU olcum yapilir: _h_probs.pkl icinde
V/F/olasilik var; adaylar turetilir, 13 ozellik + eksen ozellikleri cikarilir, PARCA-GRUPLU
capraz dogrulamayla RF egitilir ve iki kol ayni bolmede karsilastirilir.

KILL: OOF CP-F1 farki < +0.005 ise E duser (gate korpusunu bastan kurmaya degmez).
"""
...
def main():
    ...
    for gi, r in enumerate(cache):
        ...
        for i in range(len(cps)):
            ...
            # 4 ozellik: silindir mi, duzlem mi, hic mi, ve eslesen silindirin yaricapi
    ...
    print(f"{'kol':<28}{'CP-F1':>9}{'dusuk':>9}{'cok':>9}")
