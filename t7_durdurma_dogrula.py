...
Dalin gidisati: -0.0148 (parca-regresyonu) -> +0.0089 (yalniz-azalt) -> +0.0142 (dur/devam)
-> +0.0150 (F1-agirlikli). Kahinin %15'i, bar +0.02'nin altinda.

Karar vermeden once iki sey lazim:
  1. ESLESTIRILMIS BOOTSTRAP: fark sifiri iceriyor mu? Iceriyorsa kol OLU, "bar altinda"
     demeye bile gerek yok.
  2. TOHUM DAYANIKLILIGI: tek bir random_state ile alinan +0.0150, tohum degisince duruyor mu?
     (bu projede daha once "+0.053" diye bir kazanc tohum/bolme degisince erimisti)

Ikisi de gecerse kol GERCEK ama KUCUK demektir -> istiflenebilir aday olarak kayda gecer.
...
def main():
    ...
    def satirlar(Kf):
        ...
        for p in PAR:
            ...
            out.append((p["rj"],) + esle3(p["P"][:K], p["Pd"][:K], p["G"], p["Gd"], p["diag"]))
        return out

    taban = satirlar(lambda p: p["su_K"])
    print(f"TABAN {f1w(taban):.4f}\n")
    print(f"{'tohum':<8}{'tespit':>9}{'fark':>9}")
    farklar, kollar = [], []
    for tohum in (0, 1, 2, 3, 4):
        ...
        yeni = satirlar(dur)
        kollar.append(yeni)
        farklar.append(f1w(yeni) - f1w(taban))
        print(f"{tohum:<8}{f1w(yeni):>9.4f}{farklar[-1]:>+9.4f}", flush=True)
    ...
    v = []
    for _ in range(4000):
        i = rng.integers(0, len(taban), len(taban))
        v.append(f1w([yeni0[k] for k in i]) - f1w([taban[k] for k in i]))
    v = np.array(v)
    lo, hi = np.percentile(v, 2.5), np.percentile(v, 97.5)
    gercek = lo > 0
    print(f"ESLESTIRILMIS BOOTSTRAP (tohum 0): {v.mean():+.4f} [{lo:+.4f}, {hi:+.4f}] -> "
    ...
    print(f"  tohum dayanikli mi   : {'EVET' if (f > 0).all() else 'HAYIR'}")
