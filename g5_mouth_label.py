...
WHY (G1 olcumu, 2026-08-04): unseen manufacturer sinavinda FN'lerin **%68.1'i ADAY_YOK**
...
h3 WHY COKTU VE BU WHY FARKLI ([[h3-highcp-finetune-dead]]):
  ...
  Burada two difference present:
    1. YARICAP OLCULUYOR, fixed not: `mouth_width` (A0'da kalibre edildi, %1 deviation)
       each CP'de acikligin GERCEK genisligini gives.
    2. BOYANAN SEY KANAL, duz surface not: axis along [-1mm, +depth] bandinda and
       eksene dik mesafesi measured_path yaricapin under remaining vertices.
...
MAKBUZ ="results/g5_mouth_label.json"
...
# oldugu for feat-tutarlilik kapisi that parcalari ELIYORDU. Reddedilen 29 parcada
...
YARICAP_PAYI =1.15 # measured_path yaricapa emniyet payi
...
def agiz_etiketle (V ,F ,mesh ,G ,Gd ,CE ,HOUSING =0 ):
    ...
    for p ,d in zip (G ,Gd ):
        ...
        if r <=1e-6 :
            bilgi .append (None );continue # olculemedi -> UYDURMA, skip
        rel =V -m 
...
def main ():
    ...
    ap .add_argument ("--feat-tut-threshold",type =float ,default =0.6 ,
    help ="feat-tutarlilik kapisi: GT'nin this orani geri gelmeli (0.6 default)")
    ap .add_argument ("--tol-carpan",type =float ,default =1.0 ,
    help ="feat-tutarlilik kapisi tolerans carpani")
    ...
    help ="boyama output dizini (default _label_auto)")
    a =ap .parse_args ()
    ...
    present ={os .path .splitext (x )[0 ]for x in os .listdir (CIKTI )if x .endswith (".npz")}
    E =[t for t in eligible ()if t [1 ]not in present ]
    # SINAV DISLAMASI -- PARCA VE URETICI DUZEYINDE (2026-08-05'te found kirlilik).
    ...
    # measured_path "unseen manufacturer" sayilari SISIKTI. Kaynak buydu, here kapatiliyor.
    import io as _io 
    _SP ,_SM =set (),set ()
    ...
    "ETIKET_DISLA",
    "results/d5_4_exam_set.json,results/d6_exam_set.json").split (",")if x )
    ...
    from d5_derive_new import dengeli_sira 
    E =dengeli_sira (E )
    if a .total_ >1 :
    ...
    if a .bound_ :
        E =E [:a .bound_ ]
    print (f"etiketlenecek {len (E )} part (already present {len (present )})",flush =True )

    say =collections .Counter ();t0 =time .time ();UR =collections .Counter ()
    for k ,(mfg ,pid ,jf ,stp )in enumerate (E ,1 ):
        if k %25 ==0 :
            h =(time .time ()-t0 )/k 
            print (f"  {k }/{len (E )}  {h :.1f}s/part  remaining ~{h *(len (E )-k )/60 :.0f} dk  "
        ...
        try :
            ...
            # each GT CP'sini measured_path yaricapla boyar). Kapi, etiketin dogrulugunu not
            ...
            # dongusel. h3 cokusu farkliydi: orada ETIKETIN KENDISI yanlisti (fixed 2mm
            # disk duz yuzeyi boyuyordu), here label GT'den geliyor.
            #
            # Yine de BAGLAYICI KILL valid: atesleme %78 altina duserse ya da
            # unseen F1 gerilerse GERI ALINIR.
