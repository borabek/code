...
# GATE A: YALNIZCA corpus TAM ise. Eksik korpusta measured_path number yaniltici olur
...
for k in d6 tam; do
  ...
  if [ "$n" -ge "$hedef" ]; then
    ...
    say "  measured_path number yaniltici olurdu. Esli kiyas for: probe_paired_ceiling.py"
  fi
done
