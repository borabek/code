...
Non-high-CP standart gate ile. Overall metadata F1 -> 0.80?"""
...
def lattice_feats(P, ws):
    """her aday icin lattice sinyalleri: consistency, grid-fit, n_row, n_col, u/v-position."""
    ...
    # grid-fit: HER iki eksende (u,v) pitch tespit -> en yakin grid cizgisine yakinlik
    ...
    # yerel yogunluk: 8mm ici yuksek-ws komsu agirligi
    dens = np.zeros(n)
    for i in range(n):
...
# high-CP OZEL secici: leave-one-part-out, lattice-augmented
hc_tp = hc_nk = hc_gt = 0
for p in pids:
