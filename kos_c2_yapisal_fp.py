# -*- coding: utf-8 -*-
"""C2: YAPISAL FP BASTIRMA -- gercek CP'ler SIRA olusturur.

HATA BANKASI v2 (isaret duzeltmeli yigin, `results/hata_bankasi_v2.json`):
TP 786 / FN 2301 / **FP 1247** (B-rep 639, seg 382) -> F1 0.3070.
Kova aritmetigi: **FP yariya inerse F1 0.350** (+0.043).

FIZIK: klemensler tekrarli yapilardir; gercek tel girisleri parca uzerinde
DUZENLI ARALIKLI SIRALAR olusturur. Yalitik duran bir aday (komsusu yok, sira
uyumu yok) buyuk olasilikla vida deligi / alet yuvasi.

KOL: gate+NMS sonrasi her aday icin YAPI SKORU hesaplanir --
  komsu_sayisi   parcanin baskin ekseni boyunca es-aralikli komsu adedi
  aralik_uyumu   en yakin komsu araliginin parca MEDYAN araligina uyumu
  dizilim        adayin, komsularinin olusturdugu dogruya diklik sapmasi
Skoru dusuk olanlar ATILIR. Esik D6'da secilir, D7'de YENIDEN TARANMAZ.

KIYAS: K2.1 periyodik yayilim daha once robot -0.0100 vermisti, ama BOZUK
etiketli gate ve ESKI havuzla; ayrica o kol aday EKLIYORDU, bu kol aday SILIYOR.
Dayanagi cokmus, yeniden olculur.

TEZE SADIK: konum/yon uretimi degismez, yalniz son kabul.
"""
import collections
import json
import os
...
GIRME, ERISIM = 3, 5
ARALIK_TOL = 0.25          # medyan araliktan bu goreli sapmaya kadar "uyumlu"
YAPI_ESIKLERI = (None, 0, 1, 2)   # None = kol KAPALI


def yapi_skoru(P):
    """Her aday icin: es-aralikli komsu sayisi (0..n).

    Parcanin baskin ekseni bulunur, adaylar o eksene yansitilir, ardisik
    araliklarin MEDYANI referans alinir. Bir adayin skoru, kendisine medyan
    aralik +/- tolerans mesafede duran komsu sayisidir. Yalitik aday -> 0.
    """
    n = len(P)
    if n < 3:
        return np.full(n, 99)          # az adayli parcada kol devre disi
    Q = P - P.mean(0)
...
def main():
    ...
    for ye in YAPI_ESIKLERI:
        ...
        if ye is not None and (en is None or r["robot"] > en[1]["robot"]):
            en = (ye, r)
    ye = en[0]
    taban = kos(gate, te, None, S, tam=True)
    yeni = kos(gate, te, ye, S, tam=True)
    print(f"\nTABAN (kol kapali)   robot {taban['robot']:.4f} | tespit "
          f"{taban['tespit']:.4f} | TP {taban['TP']} FP {taban['FP']}")
    print(f"YAPI  (esik={ye})       robot {yeni['robot']:.4f} | tespit "
