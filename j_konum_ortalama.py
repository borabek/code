...
NEDEN: `robot_cp._vote2` anlasan modelleri gruplayip EN YUKSEK GUVENLI uyeyi temsilci aliyor
ve digerlerinin konumunu ATIYOR. Toplulugun en klasik faydasi -- bagimsiz hatalarin ortalamada
sonmesi -- boylece hic kullanilmiyor.

NEDEN SIMDI: durust (geometri) bolmede ayrisim degisti:
    tespit 0.6250 -> yanal<=2mm 0.4715 -> robot-hazir 0.4159
yani KONUM kaybi -0.154, EKSEN kaybi -0.056. Eksen I ile buyuk olcude kapandi; asil kalem artik
konum. B (silindir eksenine izdusurme) olculup oldu ama o DIS bir referansa tasima denemesiydi;
bu farkli: kendi uyelerimizin ortalamasi, dis referans yok.

KILL: yanal<=2mm ya da tespit gerilerse duser.
"""
...
def vote_avg(cp_lists, cluster_mm=5.0, min_votes=1, mode="rep", dir_mode="rep"):
    """mode='rep'   -> mevcut davranis (en guvenli uye temsilci)
       mode='mean'  -> anlasan uyelerin KONUM ortalamasi
       mode='wmean' -> guven agirlikli konum ortalamasi"""
    ...
    if dir_mode != "rep":
        for c in out:
            ...
            if len(D) > 1:
                # ISARETSIZ ortalama: uyeler ayni ekseni ZIT isaretle verebilir; ham ortalama
                # onlari birbirini goturur. Once ilk uyeye gore isaret hizalanir.
...
def main():
    ...
    # (775 grup vs keskin 1220). Bu, sizintiyi ve dolayisiyla kaybi ABARTIYORDU.
    ...
    _SET = os.environ.get("OLCUM_KUME", "")   # "" = eski 100, ya da dev/val/locked
    ...
    if _SET:
        ...
        print(f"KUME={_SET}: {len(sel)} parca", flush=True)
    ...
    # dagitilandan farkli bir isletim noktasinda calistirir (2026-07-31 denetimi: olcum
    ...
    print(f"GEOMETRI bolmesi | gate {int(keep.sum())} aday | esikler {thr}", flush=True)
    print(f"{len(cache)} test parca", flush=True)
    ...
    def run(mode):
        ...
        for r in cache:
            ...
            base = robot_cp._vote2(der, min_votes=1)   # PARITE: urunun kendi fonksiyonu
    ...
    print()
    print(f"{'kol':<26}{'tespit':>9}{'yanal2':>9}{'ROBOT':>9}{'yanal med':>11}{'<=2mm':>8}")
    out = {}
    ...
    json.dump(out, open("results/j_konum_ortalama.json", "w"), indent=1)
    print("\nKILL: yanal<=2mm ya da tespit gerilerse duser.")
    print("makbuz -> results/j_konum_ortalama.json")
