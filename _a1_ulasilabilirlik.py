# -*- coding: utf-8 -*-
"""A1: 188 ADAYSIZ GT -- kaci gercekten ULASILABILIR bir aciklikta?

Kaybin en buyuk tek kalemi (GT'nin %14.2'si) ve bugune kadar NEDEN oldugu sorulmadi.
B2 gosterdi ki turetme esiklerini gevsetmek kurtarmiyor -> ag orada CE/CT uretmiyor.
Ama iki BAMBASKA sebep olabilir:
   (a) AG HATASI      : disaridan acik bir kanal var, ag gormuyor -> KAZANILABILIR
   (b) YAPISAL        : CP malzemenin arkasinda, disaridan hicbir aciklik yok
                        (uretici IC KONTAGI listelemis) -> hicbir yuzey yontemi bulamaz

AYIRMA TESTI: CP'nin ekseni boyunca DISARIDAN iceri isin at. Isin, malzemeye carpmadan
CP'ye 2mm'ye kadar yaklasabiliyorsa ACIK KANAL vardir (a). Yaklasamıyorsa CP arkada (b).

Kendi Moller-Trumbore isinimimiz kullanilir (trimesh.ray rtree olmadan patliyor -- bkz.
...
from y2_fp_render import kesisim          # Moller-Trumbore, tek isin

stp={p:s for m,p,jf,s in eligible()}
DER,rap=olcum_kumesi.kume("results/_der_tam.pkl"); olcum_kumesi.rapor_bas(rap)
ACIK=KAPALI=0; DERIN=[]
t0=time.time()
for k,r in enumerate(DER,1):
    ...
    for b in np.where(~ulasan)[0]:
        ...
        # iki yonu de dene: hangisi DISARIYI gosteriyorsa oradan isin at
        en=None
        for s in (+1,-1):
            o=g+s*d*60.0
            t=kesisim(V,F,o,-s*d)          # ilk carpma mesafesi
            if t is None: continue
            kalan=60.0-t                    # carpma noktasinin CP'ye uzakligi (isin boyunca)
...
print()
print("OKUMA: KAPALI orani yuksekse aday havuzu tavani (0.8579) SAHTE DUSUK --")
...
print("makbuz -> results/a1_ulasilabilirlik.json")
