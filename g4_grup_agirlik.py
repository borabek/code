# -*- coding: utf-8 -*-
"""G4: GRUP-AGIRLIKLI EGITIM -- listenin son gate kalemi.

NEDEN: korpus GEOMETRI IKIZLERIYLE dolu (1903 parca -> 765 grup). Baskin bir aile,
kendi ornek sayisiyla karar yuzeyini dikte ediyor olabilir. Her geometri grubunu
ESIT agirlikta saymak bu baskiyi kaldirir; yeni bilgi eklemez, VAR OLANI dengeler.

KILL (onceden): uretici-disi ORT +0.01 VE havuzlanmis -0.005'ten iyi.
"""
import io, json
import numpy as np
...
def main():
    ...
    D = T.yukle(); olcum_kumesi.rapor_bas(D["rap"])
    ...
    for ad, mod in (("G0 TABAN", "yok"), ("G4 grup-agirlikli", "grup"),
                    ("G4 parca-agirlikli", "parca")):
        SON[ad], PARCA[ad] = T.calistir(D, yap(mod), ad)
    tb = SON["G0 TABAN"]
    print(f"\n{'kol':<22}{'d(havuz)':>11}{'d(uret)':>10}{'karar':>9}")
    kaz = []
    for ad in SON:
    ...
    if not kaz:
        print("\nKILL: agirliklandirma uretici-disi +0.01 vermedi -> G4 KAPANDI")
    ...
    print("makbuz -> results/g4_grup_agirlik.json")
