# -*- coding: utf-8 -*-
"""ZENGIN GATE'in DURUST DEPLOY sayisi. analyze_rich'teki 'best-thr' IN-SAMPLE esik secimiydi (iyimser).
Burada esik NESTED-CV ile secilir (train-fold'da sec, test-fold'da uygula) = deployable.
Ayrica: taper(C)+egrilik(D) katkisiz/negatif cikti -> 13+A+B (46 dim) varyantini da olc (sade = iyi).
...
AB = np.hstack([X13, XR[:, 0:33]])            # 13 + konum(9) + cok-yaricap(24)
...
def nested_thr_f1(sc, mask, groups, K=5):
    """esik train-fold'da secilir, test-fold'da uygulanir -> in-sample iyimserlik YOK."""
...
print(f"{NPZ}: {len(Y)} aday ({len(Y)/max(gt_tot,1):.2f}x GT) | {len(ngt_of)} parca | {len(uf)} aile"
