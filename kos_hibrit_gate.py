# -*- coding: utf-8 -*-
"""HIBRIT GATE: v6 seg adaylarini, AYRI bir model B-rep adaylarini puanlar.

NEDEN BU MIMARI:
* Dagitilan v6 tez-saf havuzda 0.1970 veriyor; benim ayni recete ile elde
  kurdugum gate 0.1159. Yani v6'yi YENIDEN kurmaya calismak ~0.08 kaybettiriyor.
* v6'yi genisletilmis havuza oldugu gibi vermek de dusuruyor (0.1435): B-rep
  adaylarini HIC gormedi, onlari yanlis puanliyor ([[gate-refit-minv4]]).
* Cozum: v6'ya DOKUNMA. Seg adaylari AYNEN v6 ile puanlanir -- yani dagitilan
  taban birebir korunur. B-rep adaylari AYRI bir model ile puanlanir ve ancak
  KENDI esigini gecerse havuza katilir. Kol kotu calisirsa taban KAYBEDILMEZ.

TEZE SADIKLIK: `v_o` turetmesi, 5 sinif, ~6000 remesh DEGISMEDI. Seg kolu
tezin kendisidir ve dokunulmamistir; B-rep kolu EK bir aday kaynagidir ve
...
sys.path.insert(0, ".")
import wire_gate, kanonik_d7 as K, d6_kayit
...
for f in sorted(os.listdir(OZ)):
    ...
    if len(Xb) < 2:
        continue
    M.append(wire_gate.parca_ici(Xb, DONUSUM)); Y.append(yb)
...
def kos(b_esik):
    """b_esik=None -> B-rep kolu KAPALI (tez-saf taban)."""
    en = None
...
out = {}
for ad, be in (("TEZ-SAF (B-rep KAPALI)", None), ("+B-rep esik 0.50", 0.50),
               ("+B-rep esik 0.60", 0.60), ("+B-rep esik 0.70", 0.70),
               ("+B-rep esik 0.80", 0.80), ("+B-rep esik 0.90", 0.90)):
    out[ad] = kos(be)
    r = out[ad]
...
json.dump({"damga": makbuz_hash.damga(), "sonuc": out,
           "not": "HIBRIT: seg adaylari DAGITILAN v6 ile, B-rep adaylari AYRI "
                  "model ile puanlanir. Tez-saf kol DEGISMEDI. D7 marka-disi, MIKRO."},
          open("results/hibrit_gate_d7.json", "w"), indent=1)
print("makbuz -> results/hibrit_gate_d7.json")
