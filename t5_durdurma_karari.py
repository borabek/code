# -*- coding: utf-8 -*-
"""T5: kesimi DURDURMA KARARI olarak kur (regresyonun coktugu yeri duzelt).

T3/T4 kahin K'yi PARCA BASINA tek bir sayi olarak kestirmeye calisti ve kaybetti:
    dogrudan regresyon  -0.0148 | buzulme +0.0001 | secici -0.0005 | yalniz-azalt +0.0089

Iki yapisal kusuru vardi:
  1. EGITIM SATIRI SAYISI 200 (parca basina bir hedef). Ogrenilecek sey icin cok az.
  2. HEDEF GURULTULU: ayni F1'i veren bircok K var; kare-hata en aza indiren tahmin,
     F1'i en aza indiren tahmin DEGIL.

DOGRU KURULUM: kesim, parca basina tek sayi degil, ADAY BASINA bir DUR/DEVAM kararidir.
Adaylar skora gore sirali; k. aday icin etiket "kahin K >= k mi" (yani bu adayi almali miyiz).
Boylece egitim satiri 200 -> 3277 olur ve her satirin etiketi TEMIZ.

CIKARIM: siradaki adaylar icin olasilik hesaplanir, ilk P < esik olan yerde DURULUR (onek
kuralina saygi). Esik taranir.

OZELLIKLER (hepsi calisma aninda, GT'siz):
  aday: skor, skor/parca-maks, sira, sira/n, onceki skorla fark, sonraki skorla fark,
        o ana kadarki kumulatif skor
  parca: n_aday, maks, ortalama, std, kosegen, cok-CP bayragi
  (+ istege bagli: adayin kendi 22 gate ozelligi)

KILL (t3/t4 ile AYNI, degistirilmedi): tespit >= +0.02 VE DEV ile VAL ayni yonde VE
gorulmemis uretici ortalamasi dusmeyecek.
"""
import json
import os
import pickle
...
def main():
    ...
    for r in DER:
        if r["X"] is None or not len(r["G"]):
            continue
        s = np.asarray(wire_gate.karar_skoru(m, r["X"]), float)
        o = np.argsort(-s)
        PAR.append({"geo": gk.get(r["pid"], "yok:" + r["pid"]),
                    "kume": kume_of.get(r["pid"], "dev"), "mfg": mfg_of.get(r["pid"], "?"),
    ...
    for p in PAR:
        ...
        for K in range(0, len(p["s"]) + 1):
            tp, fp, fn = esle3(p["P"][:K], p["Pd"][:K], p["G"], p["Gd"], p["diag"])
            f = 2 * tp / max(2 * tp + fp + fn, 1)
            if f > en:
                en, enk = f, K
        p["kahin_K"] = enk
        p["su_K"] = int(wire_gate.karar_maskesi(p["s"]).sum())
    ...
    print(f"{len(PAR)} parca | {len(Y)} ADAY satiri (regresyon 200 satirla calisiyordu) | "
    ...
    # parca basina olasilik dizisine geri dagit
    i0 = 0
    for p in PAR:
        n = len(p["s"])
        p["pr"] = oof[i0:i0 + n]
    ...
    def puanla(Kf, sec=None):
        ...
        for p in PAR:
            ...
            rows.append((p["rj"],) + esle3(p["P"][:K], p["Pd"][:K], p["G"], p["Gd"], p["diag"]))
        return f1w(rows) if rows else float("nan")

    a = puanla(lambda p: p["su_K"])
    a_dev = puanla(lambda p: p["su_K"], lambda p: p["kume"] == "dev")
    a_val = puanla(lambda p: p["su_K"], lambda p: p["kume"] == "val")
    a_mfg = np.mean([puanla(lambda p: p["su_K"], lambda p, x=x: p["mfg"] == x)
                     for x in ("PXC", "WEI")])
    kah = puanla(lambda p: p["kahin_K"])
    print(f"\nTABAN {a:.4f} (DEV {a_dev:.4f} / VAL {a_val:.4f} / uretici ort {a_mfg:.4f}) | "
          f"KAHIN {kah:.4f} (+{kah-a:.4f})")

    def dur(p, t):
        """ILK P < t olan yerde DUR (onek kurali)."""
        pr = p["pr"]
        k = 0
    ...
    def say(p, t):
        """P >= t olanlarin SAYISI (onek zorunlulugu yok)."""
        return int((p["pr"] >= t).sum())

    print(f"\n{'kol':<26}{'tespit':>9}{'fark':>9}{'DEV':>9}{'VAL':>9}{'mfg ort':>10}{'KILL':>8}")
    KOL = {}
    ...
    for ad, Kf in KOL.items():
        v = puanla(Kf)
        dv = puanla(Kf, lambda p: p["kume"] == "dev") - a_dev
        vl = puanla(Kf, lambda p: p["kume"] == "val") - a_val
