...
Karsilastirma: linear probe (logistic) / RF / kucuk MLP -- AYNI OOF split'lerinde.
Taban: rich (13+konum+cok-yaricap). Aday: rich + embedding feature'lari.
TUM olcumler TEMIZ CALISMA setinde (sizinti-muhafizi acik, KILITLI holdout haric)."""
import json
import numpy as np
...
# --- iki npz'yi part_id + parca-ici sira uzerinden hizala ---
...
for gi_e, pid in enumerate(ep):
    ...
    if len(ir) != len(ie): continue                       # aday sayisi tutmuyorsa parcayi atla (guvenli)
...
print(f"hizalandi: {len(Y)} aday / {len(set(gpid))} parca | rich {RICH.shape[1]}d + emb {EMB.shape[1]}d = {FULL.shape[1]}d")
...
print(f"WORK: {int(WORK.sum())} aday / {len(set(gpid[WORK]))} parca\n")
