...
SORU: GEO-BREP'in urettigi yanlis pozitifler ACIKLANABILIR mi? (289 FP vs 69 TP idi)
Aciklanabilirse hedefli suzgec yazilabilir; dagitiksa F1 0.70 ulasilamaz ve plan BURADA kapanir.

KAPI 1 (baslamadan yazildi): FP'lerin >= %60'i, <= 4 acik geometrik imzali kumeye dusmeli.

IMZALAR (hepsi olculebilir, ogrenilmis parametre yok):
  ...
  cok_kucuk  : yaricap < 1.0 mm  (pim/percin)
  cok_buyuk  : yaricap > 3.0 mm  (govde bosaltmasi/kanal)
  sig        : kanal derinligi < 3 mm (pah/yuva)
  yalniz     : parca icinde ayni yaricap+eksende tekrari YOK (dizi degil)
  diger      : hicbirine uymayan

Ayrica FN otopsisi: kacan GT'ler silindirik bir yuzeyin yakininda MI? Degilse yarik/push-in
girisidir ve recall tavani G2 (yarik adaylari) olmadan asilamaz.
"""
...
def main(n_parts=60):
    ...
    # G0: dev kumesi = ilk yari (deterministik), eval = ikinci yari (BU FAZDA DOKUNULMAZ)
    idx = rng.permutation(len(parts))
    dev = [parts[i] for i in idx[:n_parts]]
    print(f"dev kumesi {len(dev)} parca (eval yarisi bu fazda DOKUNULMADI)\n", flush=True)
    ...
    for k, (m, pid, jf, stp) in enumerate(dev, 1):
        try:
            ...
            # GT'lerin baskin ekseni (parca ici cogunluk)
