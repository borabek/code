# -*- coding: utf-8 -*-
"""GEO-G2: YARIK / PUSH-IN aday uretimi -- silindir OLMAYAN tel girisleri.

GEREKCE (G1 otopsisi, olculdu): kacan 75 GT'nin **70'i (%93.3)** hicbir silindirik B-rep yuzeyine
3mm'den yakin degil. Yani tel girislerinin buyuk cogunlugu SILINDIR DEGIL -- yay-kelepceli /
push-in terminallerde giris, KARSILIKLI IKI DUZLEM arasindaki bir yariktir. Silindir arayan bir
dedektor onlari asla goremez; yaricap penceresini daraltmak recall'u bu yuzden coketmisti.

YONTEM (ogrenilmis parametre YOK):
  1. B-rep duzlemlerinden KARSILIKLI CIFTLER bul: normalleri zit, aralari SLOT_MIN..SLOT_MAX mm,
     bbox'lari ortusuyor.
  2. Ciftin orta duzleminde, ortusme bolgesinin merkezini AGIZ adayi say.
  3. Fiziksel dogrulama (cp_geometry.ray_hits): agizdan disari isin bos olmali, iceri dogru kanal
     en az MIN_DEPTH devam etmeli.

NOT: duzlem normali gmsh'ten dogrudan gelmiyor; bbox'i YASSI olan yuzeyin ince ekseni normaldir.
Bu, duz duzlemler icin kesin; egri yuzeyler zaten Cylinder/Cone/Torus olarak gelir.
"""
import numpy as np

SLOT_MIN, SLOT_MAX = 1.0, 6.0     # tel girisi yarik acikligi (mm)
MIN_DEPTH = 2.0                   # kanal en az bu kadar devam etmeli
MIN_OVERLAP = 2.0                 # cift, en az bu kadar ortusmeli (mm)
MERGE_MM = 3.0
...
def detect_slots(surfaces, mesh, slot_min=SLOT_MIN, slot_max=SLOT_MAX,
    ...
    for i in range(len(planes)):
        ...
        for j in range(i + 1, len(planes)):
            ...
            if ki != kj:                       # ayni eksene dik olmalilar (karsilikli)
                continue
            gap = abs(float(ci[ki] - cj[ki]))
            if not (slot_min <= gap <= slot_max):
                continue
            # diger iki eksende ORTUSME
            ...
            # yarik uzunlugu = ortusmenin UZUN ekseni -> agiz o eksenin UCUNDA
