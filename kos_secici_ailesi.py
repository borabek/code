# -*- coding: utf-8 -*-
"""SECICI AILESI: ayni havuz/oznitelik/etiket, FARKLI siniflandirici.

Kazanan kol: B-rep havuzu + agiz tanimlayicilari, RF 400/leaf3 -> D7 TAM ZINCIR
robot **0.2649** (urun 0.2029). Havuz ve oznitelik tarafi tuketildi; bu betik
SECICININ KENDISINI degistirir. Tek degisken: model.

Kollar:
  RF-400/3   (kazanan taban)
  RF-800/1   daha yuksek kapasite
  HGB        HistGradientBoosting (tabular veride RF'yi sik geciyor)
  HGB derin  daha cok yaprak / iterasyon
  ENSEMBLE   RF + HGB skor ortalamasi

Esik HER MODEL ICIN D6'da AYRI secilir (model kalibrasyonu farkli olur; sabit
esik kullanmak zayif modeli haksiz cezalandirirdi). D7'de yeniden TARANMAZ.
"""
import collections
import json
import os
...
def skorla(modeller, X):
    Xd = wire_gate.parca_ici(X, DONUSUM)
...
def main():
    ...
    print(f"egitim {M.shape} pozitif {Y.mean():.4f} | dev {len(dev)} | "
          f"sinav {len(te)}", flush=True)
    ...
    for ad, yap in kurulum.items():
        ...
        sonuc[ad] = dict(r7, esik=e)
        print(f"{ad:<12} esik {e:.2f} -> D7 robot **{r7['robot']:.4f}** | tespit "
    ...
    sonuc["ENSEMBLE RF+HGB"] = dict(r7, esik=e)
    print(f"{'ENSEMBLE':<12} esik {e:.2f} -> D7 robot **{r7['robot']:.4f}** | "
    ...
    json.dump({"damga": makbuz_hash.damga(), "sonuc": sonuc, "en_iyi": iyi,
               "urun": 0.2029,
               "not": "Ayni havuz (B-rep + tanimlayici), ayni etiket; TEK DEGISKEN "
    ...
    print("makbuz -> results/secici_ailesi.json")
