...
NEDEN: otopsi (2026-08-04) "uretilen CP'lerin %40/%50/%96'si fiziksel olarak kusurlu"
dedi ve bu B1-B4 red kriterlerinin dayanagi. Ama o hukmu veren IKI FONKSIYON hic
dogrulanmadi:
  * `cp_geometry.is_inside` : 5 rastgele isinla PARITE oyu. Parite testi WATERTIGHT
    mesh varsayar; urun ~6000 tepeye REMESH edilmis, su gecirmez OLMAYAN orgu kullaniyor.
  * `cp_geometry.mouth_width`: 24 yonde isin atip MINIMUMU aliyor. Otopside gorulen
    0.19mm "ic cap" gercek bir duvar yakinligi da olabilir, tek bir isinin facet kenarina
    denk gelmesi de.

[[trimesh-rtree-silent-failure]]: bu proje sessizce yanlis cevap veren bir geometri
fonksiyonuyla aylarca kostu. Dogrulanmamis bir aleti RED KRITERINE terfi ettirmek,
iyi CP'leri silmek demektir.

SINAV: bilinen geometriler (dolu kutu / gecisli delik / kor cep / ince yarik). Her biri
IKI halde olculur -- TAM (CSG, watertight) ve REMESH (urunun gercekten gordugu hal).

KILL (onceden yazildi):
    is_inside  yanilma <= %5
    mouth_width sapmasi <= %10 (bilinen capa gore) VE isin sayisina duyarliligi <= %10
GECMEZSE: fiziksel bayraklar yalniz RAPOR kalir; B1-B4'te RED KRITERI OLAMAZ.
"""
import io
import json
import os
...
R_DELIK = 3.0                  # gecisli delik yaricapi
R_CEP = 2.5                    # kor cep yaricapi
CEP_DERINLIK = 6.0
YARIK = (1.6, 9.0)             # ince yarik: genislik x uzunluk


def geometriler():
    """(ad, mesh, sinav_noktalari) -- sinav noktasi: (nokta, iceride_mi_DOGRUSU)."""
    import trimesh
    g = {}
    z = KUTU[2] / 2.0
    ...
    g["dolu_kutu"] = (kutu, [
        (np.array([0., 0., 0.]), True),              # tam ortada -> ICERIDE
        (np.array([8., 8., 0.]), True),
    ...
    # KOR CEP -- DIKKAT (ilk surumde KENDI TESTIM hataliydi): silindir IKI KEZ otelenmisti
    # (z, sonra +CEP_DERINLIK), toplam 12. Kutu z<=6'da bittigi icin silindir [6,18] araligina
    # gidiyor ve HIC KESMIYORDU: "kor cep" aslinda DOLU KUTUYDU. is_inside'in "yanlis" dedigi
    # nokta tam o dolu bolgedeydi -- ALET DOGRUYDU, GEOMETRI YANLISTI. mouth_width'in 24.00
    # (= kutu genisligi) vermesi de ayni sebeptendi.
    # DOGRUSU: yukseklik 2*CEP_DERINLIK olan silindiri z=+z'ye otele -> [0, 12] araligini kaplar,
    # kutuyla kesisimi z in [0, 6] = ust yuzeyden CEP_DERINLIK derinliginde KOR cep.
    ...
    g["kor_cep"] = (cep, [
        ...
        (np.array([0., 0., -z + 2.0]), True),        # cebin ALTINDA malzeme var
        (np.array([9., 9., 0.]), True),
    ])
...
def olc(mesh, noktalar, is_inside):
    dogru = sum(1 for p, t in noktalar if bool(is_inside(mesh, p)) == t)
    return dogru, len(noktalar), [(p.tolist(), t, bool(is_inside(mesh, p)))
...
def main():
    ...
    print(f"\nis_inside TOPLAM: {top_d}/{top_n} dogru -> YANILMA %{100*yanilma:.1f}")
    ...
    for ad, r_bek in (("gecisli_delik", R_DELIK), ("kor_cep", R_CEP)):
        ...
        for hal in ("TAM", "REMESH"):
            ...
            mw_sapma.append(sap)
            rapor["mouth_width"][f"{ad}_{hal}"] = {"beklenen": bek, "olcum": ol,
                                                   "sapma": sap, "isin_duyarliligi": tut}
    ...
    print(f"      mouth_width sapma %{100*mw_max:.0f} (<=%10 -> {'GECTI' if g2 else 'KALDI'})")
    ...
    print("makbuz -> results/a0_alet_kalibrasyon.json")
