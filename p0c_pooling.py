# -*- coding: utf-8 -*-
"""P0-c: SKOR-HAVUZLAMA testi (feature-ekleme DEGIL). FBI argumaninin son kolu: bir sirada/kumede
8 aciklik ayni tipse, gate skorlarini HAVUZLAMAK gurultuyu sqrt(N) azaltir -> ayrim guclenir.
Eksen-feature'lari eklemek katmadi (+0.0036); ama HAVUZLAMA farkli bir mekanizma.
Leave-one-out havuzlama (kendi skorunu disla = sizinti yok), 3 kume tanimi:
  (a) eksen kumesi  (b) benzer-yaricap+eksen  (c) uzamsal komsu (nn)
KARAR: havuzlanmis skor AUC'yi/top-N F1'i artiriyor mu."""
import json, sys
import numpy as np
...
print(f"{len(Y)} aday / {int(Y.sum())} TP / {len(Ns)} parca | BAZ gate OOF AUC {base_auc:.4f}")


def loo_pool(members, scores):
    """leave-one-out kume ortalamasi (kendi skorunu disla)."""
...
for gi in np.unique(G):
    ...
    # uzamsal komsu: en yakin 3 adayin ort skoru
    ...
    # parca-geneli ortalama (en kaba havuz)
...
print(f"  13+havuz AUC {a2:.4f}   (baz {base_auc:.4f})  KAZANC {a2-base_auc:+.4f}")
...
print(f"  13+havuz      : {topn_f1(o2):.4f}   KAZANC {topn_f1(o2)-topn_f1(oof):+.4f}")
