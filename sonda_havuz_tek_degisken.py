# -*- coding: utf-8 -*-
"""TEK DEGISKENLI KIYAS: ayni recete, ayni parcalar, TEZ-SAF vs GENISLETILMIS havuz.

Genisletilmis havuz uctan uca 0.1515 verdi, kanonik taban 0.2029. Ama kanonik
taban DAGITILAN gate v6 (baska korpus, baska recete, `_cokus_yonlendir` dahil);
benimki elde kuruldu. Bu farki havuza yazmak HATA olur.

Burada tek degisken HAVUZ: ayni egitim parcalari, ayni recete (parca_ici zskor
PARCA PARCA, RF 400/leaf3), ayni esik taramasi, ayni NMS.

KAYNAK ISARETI YENIDEN HESAP GEREKTIRMIYOR: `brep_havuz.birlesik_havuz`
segmentasyon adaylarini HER ZAMAN basa koyuyor, yani ilk `len(r["P"])` aday
...
for f in sorted(os.listdir(OZ)):
    if f.endswith(".npz") and (f.startswith("tam_") or f.startswith("d6_")):
        ...
        X = z["X"]
        if nseg > len(X):
            continue                       # tutarsiz onbellek -> atla (sessiz kabul YOK)
...
print(f"EGITIM {len(tr)} parca | seg aday {segp} | toplam aday "
...
def egit(segtek):
    ...
    for d in tr:
        ...
        if len(X) < 2:
            continue
        M.append(wire_gate.parca_ici(X, DONUSUM)); Y.append(y)
...
out = {}
for ad, segtek in (("TEZ-SAF havuz", True), ("GENISLETILMIS havuz", False)):
    ...
    print(f"{ad:<22} robot {r['robot']:.4f} | tespit {r['tespit']:.4f} | makro "
          ...
          f"egitim {sh} poz {poz:.4f}", flush=True)
a, b = out["TEZ-SAF havuz"], out["GENISLETILMIS havuz"]
print(f"\nHAVUZUN TEK-DEGISKENLI ETKISI: robot {b['robot']-a['robot']:+.4f} | "
      f"tespit {b['tespit']-a['tespit']:+.4f} | makro {b['makro']-a['makro']:+.4f}")
print(f"(dagitilan gate v6 + NMS, tez-saf havuz: robot 0.2029 / tespit 0.4523 -- "
...
json.dump({"damga": makbuz_hash.damga(), "sonuc": out,
           ...
           "not": "TEK DEGISKEN = havuz. Ayni parcalar, ayni recete, ayni esik "
                  "taramasi, ayni NMS. D7 marka-disi, MIKRO."},
