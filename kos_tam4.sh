...
#
# NEDEN (olculdu 2026-08-12, sonda_max_sec.py):
#   UPUN/SUPU 30 parca : yonlu recall 0.8462 -> 0.9077  (+0.0615, maliyet 1.20x)
#   NIT       18 parca : yonlu recall 0.5913 -> 0.8755  (+0.2842, maliyet 1.33x)
#   tavan 48'de IKISINDE DE ek kazanc YOK -> diz noktasi 24.
#
# NIT, D6 GT'sinin %45.7'si ve havuz tavanini TEK BASINA asagi ceken marka
...
#
# Diger butun ayarlar `_p6_oz_tam3` ile BIREBIR ayni tutulur; yoksa iki korpus
...
for i in 0 1 2 3 4; do
  ( BH_MESH_ESIK=0.05 YB_FAN=256 P6_KAYNAK=012 YB_MAX_SEC=24 \
    ...
    python kos_p6_oznitelik.py d6 > "$G/tam4_d6_$i.log" 2>&1
    BH_MESH_ESIK=0.05 YB_FAN=256 P6_KAYNAK=012 YB_MAX_SEC=24 \
    P6_MESH_R=2.5 P6_MESH_MAX=350 P6_MESH_KAT=5 \
    P6_CIK=results/_p6_oz_tam4 P6_SHARD="$i/5" \
    python kos_p6_oznitelik.py tam > "$G/tam4_tam_$i.log" 2>&1 ) &
done
wait
say "=== TAVAN-24 CIKARIMI BITTI -- $(ls results/_p6_oz_tam4 2>/dev/null | wc -l) dosya ==="

# KAPI A'yi HEM d6 HEM tam kumesinde olc; makbuz adlari kumeyi tasir.
for k in d6 tam; do
  say "KAPI A olcumu ($k)"
