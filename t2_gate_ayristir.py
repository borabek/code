...
Tavan merdiveni (t_tavan.json) sunu gosterdi:
    su an tespit 0.7439  ->  kahin GATE 0.9372   (+0.1933)
    aday tavani da 0.9372  -> yani ADAYLAR ZATEN YETERLI, sorun SECIMDE.

Ama "secim" iki ayri seydir ve tedavileri BAMBASKA:
    SIRALAMA : gate dogru adaylara yuksek skor veriyor mu? (kotuyse -> yeni BILGI gerekir)
    ESIK     : skorlar iyi ama kesme yeri mi kotu? (kotuyse -> UCUZ, kural degisir)

Bu betik bosslugu ikiye boler:
    A) su anki kural            : goreli esik (0.5 x parca-maks, taban 0.25)
    B) EN IYI GLOBAL esik       : tum korpusta tek sabit esik, en iyisi secilir
    C) EN IYI GORELI oran       : goreli oran taranir
    D) KAHIN PARCA-ICI esik     : her parca icin EN IYI kesim (skorlara bakip)
    E) KAHIN top-K              : parcadaki GT sayisi bilinseydi, en yuksek K aday
    F) KAHIN GATE               : dogru adaylarin TAMAMI (siralamadan bagimsiz)

D ile F arasindaki fark = SIRALAMANIN kaybi (skorlar dogru adaylari uste tasiyamiyor).
A ile D arasindaki fark  = ESIGIN kaybi (ucuz kazanc alani).
"""
import json
import os
import pickle
...
def main():
    ...
    for r in DER:
        ...
        PAR.append({"rj": "cok" if r["n"] >= 8 else "dusuk", "s": np.asarray(s, float),
                    ...
                    "ngt": len(r["G"])})
    print(f"{len(PAR)} parca | {sum(len(p['s']) for p in PAR)} aday", flush=True)
    ...
    def puanla(secici):
        ...
        for p in PAR:
            ...
            rows.append((p["rj"],) + esle3(P, Pd, p["G"], p["Gd"], p["diag"]))
        return f1w(rows)

    SON = {}
    SON["A su anki kural"] = puanla(lambda p: wire_gate.karar_maskesi(p["s"]))
    ...
    for e in np.arange(0.05, 0.96, 0.05):
        ...
        if v > en:
            en, en_e = v, float(e)
    SON[f"B en iyi GLOBAL esik ({en_e:.2f})"] = en
    ...
    for o in np.arange(0.1, 0.96, 0.05):
        ...
        if v > en2:
            en2, en_o = v, float(o)
    SON[f"C en iyi GORELI oran ({en_o:.2f})"] = en2

    def kahin_esik(p):
        """Bu parca icin EN IYI kesim (skorlara gore) -- kahin, calisma aninda bilinemez."""
        aday = sorted(set(p["s"].tolist()) | {0.0})
        en_k, en_m = -1, np.zeros(len(p["s"]), bool)
        for e in aday:
            k = p["s"] >= e
            P = p["P"][k] if k.any() else np.zeros((0, 3))
            Pd = p["Pd"][k] if k.any() else np.zeros((0, 3))
            tp, fp, fn = esle3(P, Pd, p["G"], p["Gd"], p["diag"])
            f = 2 * tp / max(2 * tp + fp + fn, 1)
            if f > en_k:
                en_k, en_m = f, k
        return en_m
    SON["D KAHIN parca-ici esik"] = puanla(kahin_esik)
    ...
    def kahin_gate(p):
        tp, fp, fn = esle3(p["P"], p["Pd"], p["G"], p["Gd"], p["diag"])
    ...
    print(f"\n{'kol':<34}{'TESPIT F1':>11}{'A''ya gore':>12}")
    a = SON["A su anki kural"]
    for k, v in SON.items():
        print(f"{k:<34}{v:>11.4f}{(v - a):>+12.4f}")

    d_ = SON["D KAHIN parca-ici esik"]; f_ = SON["F KAHIN gate (tam secim)"]
    ...
    print(f"\n  gercekci esik kollari: B {SON[f'B en iyi GLOBAL esik ({en_e:.2f})'] - a:+.4f} | "
          f"C {SON[f'C en iyi GORELI oran ({en_o:.2f})'] - a:+.4f}")
