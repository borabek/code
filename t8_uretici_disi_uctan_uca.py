# -*- coding: utf-8 -*-
"""T8 KARAR OLCUMU: goreli esik, URETICI-DISI bolmede UCTAN UCA kazandiriyor mu?
...
def main():
    ...
    for k, ad in kod.items():
        ...
        for kural in ("sabit", (0.5, 0.25), (0.5, 0.20)):
            ...
            p_, r_ = pr(det)
            nm = "sabit (mevcut)" if kural == "sabit" else f"goreli {kural[0]} + taban {kural[1]}"
    ...
    for ad, v in out.items():
        ...
        ok = ok and (v["goreli 0.5 + taban 0.25"]["tespit"] > sb)
    print(f"\nKILL: goreli esik HER IKI uretici-disi bolmede sabiti gecmezse DAGITILMAZ -> "
          f"{'GECTI' if ok else 'GECMEDI'}")
    json.dump(out, open("results/t8_uretici_disi_uctan_uca.json", "w"), indent=1)
    print("makbuz -> results/t8_uretici_disi_uctan_uca.json")
