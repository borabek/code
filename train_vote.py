# -*- coding: utf-8 -*-
"""INSTANCE-OYLAMA BASLIGI (tezin Ausblick'inin veri-verimli hali, Masterarbeit satir 2099).

TEZ NE DIYOR: 2D YOLOv6 Kontaktierung/Kabeleinfuehrung icin "nicht empfehlenswert" (ortulme; daha
fazla etiketle cozulemez) -> YERINE "ein 3D-basiertes YOLO ... auf Punktwolken ... 3D-Bounding-Box
Labels ausgehend von den Scheitelpunkt-Labels". Sifirdan 3D detektor bizim 853 instance'imiz icin AZ
veri; bu yuzden ayni fikri MEVCUT DiffusionNet uzerine bir OYLAMA BASLIGI olarak kuruyoruz:

  her baglanti-verteksi -> ait oldugu INSTANCE MERKEZINE offset (3 ek cikti kanali)
  cikarim: oy = verteks + offset -> oylari kumele -> her kume BIR CP

NEDEN: bugun CP'leri kaybettigimiz yer post-proc. min_v 30 kucuk fragmentleri atiyor, cluster_mm 5
komsu aciklikari birlestiriyor/boluyor -- FBI teshisi model-FN'lerinin %63'unde modelin ZATEN
atesledigini gosterdi. Oylama, elle-ayarli esiklerin yerine KUMELEMEYI OGRENIR.

Cikti: 8 kanal (0-4 sinif logit, 5-7 offset). Sinif kaybi ilk 5'te (mevcut recete aynen), offset
kaybi sadece baglanti-vertekslerinde (smooth-L1). Egitim verisi certain-source (korpus + insan kismi).
...
def offset_targets(V, F, L):
    """her baglanti-verteksi icin: ait oldugu instance'in merkezine offset. (N,3) hedef + (N,) maske."""
...
def evaluate(model, meta, data, dev):
    """sinif metrikleri (ilk 5 kanal) + offset hatasi (mm)."""
    model.eval()
...
def main():
    ...
    print(f"train {len(tr)} korpus + {len(partial)} insan kismi | val {len(va)} | k_eig {a.k_eig}", flush=True)

    # sinif agirliklari (inv-freq) -- mevcut receteyle ayni
    cnt = np.zeros(NCLS)
    for s in tr:
    ...
    # log_softmax on the LAST layer across ALL C_out channels -- which would squash the 3 offset
