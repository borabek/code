# -*- coding: utf-8 -*-
"""PITSTOP-2: rejim-ayrimli gate esigi, ICE-ICE (nested) secimle.

NEDEN ICE-ICE: skorlanan kumede esik secmek, secim-testte sizintisidir. Bugun
cok-CP'de dogru yaptik (ayri tarama kumesi), burada da yapiyoruz: esik aile-disi
EGITIM katlarinda secilir, DISARIDA birakilan katta skorlanir.

KILL (olcumden ONCE yazildi):
  * dusuk-CP F1 katkisi (ice-ice) < +0.02 -> OLU, urune girmez
  * VEYA cok-CP'de -0.01'den fazla kayip -> REDDEDILIR

CIKTI: results/pitstop2_gate_nested.json (makbuz)
...
DEPLOYED = {"dusuk": 0.35, "cok": 0.25}      # su an urunde olan


def build_cache(n_low=40, n_high=24, seed=0):
    """Her parca icin V,F,ortalama-olasilik,aday-CP'ler,GT'yi mesh cercevesinde saklar."""
    ...
    print(f"secilen {len(sel)} parca ({len(lo[:n_low])} dusuk + {len(hi[:n_high])} cok); "
    ...
    for i, (m, pid, jf, stp, n) in enumerate(todo, 1):
        try:
            ...
            for model, meta in models:
                ...
                # URUNDEKI turetme: promote yalnizca cok-CP'de acik
            ...
            # Alt kume saklamak 13 ozelligin 3'unu sessizce sifirliyor (olculdu: tablo tamamen kayiyor).
            import pickle
            ...
            np.savez_compressed(
                ...
                fam=family_key(pid))
            print(f"  [{i}/{len(todo)}] {pid} n={n} aday={len(cps)}", flush=True)
...
def _match(P, G, Gd, tol):
    """big_arbiter konvansiyonu: eksene dik mesafe + +-40mm eksenel pencere."""
...
def nested(rows, gate_cache, k=5):
    """Aile-disi katlar: esik EGITIM katlarinda secilir, DISARIDAKI katta skorlanir."""
...
def main():
    ...
    for reg in ("dusuk", "cok"):
        ...
        if not sub:
            continue
        print(f"=== {reg.upper()}-CP  ({len(sub)} parca, {len({r['fam'] for r in sub})} aile) ===")
        print(f"{'esik':>7}{'TP':>5}{'FP':>5}{'FN':>5}{'P':>8}{'R':>8}{'F1':>8}")
        tbl = {}
        for thr in GRID:
        ...
        print(f"\n  urundeki esik ({DEPLOYED[reg]:.2f})      F1 {dep:.4f}")
    ...
    if len(rec["regimes"]) == 2:
        ...
        kill = rec["regimes"]["dusuk"]["honest_gain"] >= 0.02 and \
            rec["regimes"]["cok"]["honest_gain"] >= -0.01
