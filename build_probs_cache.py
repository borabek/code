# -*- coding: utf-8 -*-
"""Bir KUME icin cikarim onbellegi kur (VAL / LOCKED / DEV).

Neden ayri betik: `_h_probs.pkl` yalnizca DEV kumesini tasiyor. VAL uzerinde karar SINAMAK
icin o parcalarin olasiliklari gerekiyor; LOCKED icin de tek atislik final olcumde gerekecek.

IKI KORUMA (ikisi de bu gece yasanmis hatalardan):
 1) ZEHIRLI PARCA: gmsh tek bir STEP'te asilabiliyor. Islenmeden ONCE ad INFLIGHT dosyasina
    yazilir; yeniden baslatmada orada duran parca kalici olarak atlanir.
 2) DEVAM PARCA KIMLIGINE gore: dongu indeksine gore devam etmek, havuz degisince yanlis
    parcalari atlar ve metrigi SESSIZCE bozar (2026-07-29'da yasandi).
...
def main():
    ...
    OUT = f"results/_probs_{kume}.pkl"
    INFLIGHT = f"results/_probs_{kume}.inflight"
    SKIP = f"results/_probs_{kume}.skip"
    ...
    want = set(json.load(open("results/split3.json"))[kume]["parts"])
    ...
    print(f"KUME={kume}: {len(parts)}/{len(want)} parca bulundu | cihaz={dev}", flush=True)
    ...
    if os.path.exists(INFLIGHT):
        ...
        if st:
            ...
            with open(SKIP, "a") as fh:
                fh.write(st + "\n")
            print(f"  [zehirli parca] {st} kalici atlandi", flush=True)
        os.remove(INFLIGHT)

    todo = [p for p in parts if p[1] not in done and p[1] not in skip]   # KIMLIGE gore
    ...
    for k, (mfg, pid, jf, stp, n) in enumerate(todo, 1):
        ...
        try:
            ...
            print(f"  [hata] {pid}: {type(e).__name__}", flush=True)
        if os.path.exists(INFLIGHT):
            os.remove(INFLIGHT)
        if k % 10 == 0:
            pickle.dump(cache, open(OUT, "wb"))
            print(f"  {k}/{len(todo)} (onbellek {len(cache)})", flush=True)
    pickle.dump(cache, open(OUT, "wb"))
    print(f"-> {OUT}: {len(cache)} parca", flush=True)
